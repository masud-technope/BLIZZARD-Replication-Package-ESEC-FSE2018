...
public class MemberParser {
    ...
    private static ArrayList<String> createTypeList(String typeQualifiedName) {
        ...
        //ensure proper format was used.
        newname = newname.replace('$', '.');
        //$NON-NLS-1$
        String parsed[] = newname.split("\\.");
    ...
    private static IType[] getAllTypes(ICompilationUnit[] cunits) throws JavaModelException {
        ...
        for (int cunitNum = 0; cunitNum < cunits.length; cunitNum++) {
            //get all topLevel types
            IType types[] = cunits[cunitNum].getTypes();
            for (int typeNum = 0; typeNum < types.length; typeNum++) {
    ...
    private static IType getType(ArrayList<String> typeList, ICompilationUnit[] cunits) throws JavaModelException {
        ...
        char separator = '.';
        while (//search all types for desired target - will return internally.
        true) {
            //check all the types we have
            int typeNum = 0;
            while (//search current list of types
            typeNum < types.length) {
                if (//must ensure format is same for both.
                targetIsAnonymous) {
                ...
                targetFound) {
                    ...
                    types = getAllTypes(new //get next level
                    IType[] //get next level
                    { types[typeNum] });
    ...
    public IMember getDeepest(ICompilationUnit cu, String target) {
        for (int i = 0; i < target.length(); i++) {
            if (target.charAt(i) == '$') //EnclosingType$InnerType$MoreInner
            {
    ...
    protected IMember getDeepest(IMember top, String tail) {
        ...
        for (int i = 0; i < newtail.length(); i++) {
            if (//nested Item?
            newtail.charAt(i) == '$') //Enclosing$Inner$MoreInner
            {
        ...
        if (//we are at last item to parse
        head == null) //swap Members
        {
            head = newtail;
            newtail = null;
        }
        ...
        else if (top instanceof IMethod)
            return getNextFromMethod(top, head, newtail);
        else if (top instanceof IField)
