...
public class LaunchModeTests extends AbstractDebugTest {
	 ...
	 * @param object
	 */
    public static void assertContains(String message, Object[] array, Object object) {
    ...
    public void testModeSpecificTabGroups() throws CoreException {
        ...
        Class<?>[] classes = new Class<?>[] { JavaMainTab.class, JavaArgumentsTab.class, JavaJRETab.class, JavaClasspathTab.class, SourceLookupTab.class, EnvironmentTab.class, CommonTab.class };
        //$NON-NLS-1$
        assertTrue("Tab set does not contain all default java tabs", tabset.containsAll(new HashSet<Class<?>>(Arrays.asList(classes))));
        ...
        //$NON-NLS-1$
        assertTrue("Test tab set does not contain all default tabs", tabset.containsAll(new HashSet<Class<?>>(Arrays.asList(classes))));
    ...
    public void testResetDebugLaunchPerspective() {
        ...
        "Java debug perspective should be debug", //$NON-NLS-1$
        IDebugUIConstants.ID_DEBUG_PERSPECTIVE, DebugUITools.getLaunchPerspective(javaType, ILaunchManager.DEBUG_MODE));
        // set to NONE
        ...
        "Java debug perspective should now be null", DebugUITools.getLaunchPerspective(javaType, ILaunchManager.DEBUG_MODE));
        // re-set to default
        DebugUITools.setLaunchPerspective(javaType, ILaunchManager.DEBUG_MODE, IDebugUIConstants.PERSPECTIVE_DEFAULT);
        assertEquals(//$NON-NLS-1$
        "Java debug perspective should now be debug", //$NON-NLS-1$
        IDebugUIConstants.ID_DEBUG_PERSPECTIVE, DebugUITools.getLaunchPerspective(javaType, ILaunchManager.DEBUG_MODE));
    }
    ...
    public void testResetRunLaunchPerspective() {
        ILaunchConfigurationType javaType = getLaunchManager().getLaunchConfigurationType(IJavaLaunchConfigurationConstants.ID_JAVA_APPLICATION);
        assertNull(//$NON-NLS-1$
        "Java run perspective should be null", DebugUITools.getLaunchPerspective(javaType, ILaunchManager.RUN_MODE));
        // set to Java perspective
        DebugUITools.setLaunchPerspective(javaType, ILaunchManager.RUN_MODE, JavaUI.ID_PERSPECTIVE);
        assertEquals(//$NON-NLS-1$
        "Java run perspective should now be java", //$NON-NLS-1$
        JavaUI.ID_PERSPECTIVE, DebugUITools.getLaunchPerspective(javaType, ILaunchManager.RUN_MODE));
        // re-set to default
        DebugUITools.setLaunchPerspective(javaType, ILaunchManager.RUN_MODE, IDebugUIConstants.PERSPECTIVE_DEFAULT);
        assertNull(//$NON-NLS-1$
        "Java run perspective should now be null", DebugUITools.getLaunchPerspective(javaType, ILaunchManager.RUN_MODE));
    }
    ...
    public void testContributedLaunchPerspective() {
        ILaunchConfigurationType javaType = getLaunchManager().getLaunchConfigurationType(IJavaLaunchConfigurationConstants.ID_JAVA_APPLICATION);
        assertEquals(//$NON-NLS-1$
        "perspective for TEST_MODE should be 'java'", //$NON-NLS-1$
        JavaUI.ID_PERSPECTIVE, //$NON-NLS-1$
