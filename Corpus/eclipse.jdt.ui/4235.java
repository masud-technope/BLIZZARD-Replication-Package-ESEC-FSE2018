...
public class JavaTokenComparator implements ITokenComparator {
    ...
    private void recordTokenRange(int start, int length) {
        ...
        if (DEBUG)
            ...
            System.out.println(fCount + " (" + start + "-" + length + ")>" + fText.substring(start, start + length) + "<");
        fCount++;
    }
    ...
    private void parseSubrange(int start, String text, boolean javaCode) {
        if (DEBUG)
            ...
            System.out.println("parsingText>" + text + "<(" + start + ")");
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (DEBUG)
                //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$//$NON-NLS-4$
                System.out.println("   " + i + " (" + subStart + "-" + subLength + ")");
