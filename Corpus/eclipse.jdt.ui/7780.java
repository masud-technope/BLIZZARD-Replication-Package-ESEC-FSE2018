...
public class PropertiesQuickAssistProcessor {
    ...
    public static boolean hasAssists(PropertiesAssistContext invocationContext) {
        try {
            return getEscapeUnescapeBackslashProposals(invocationContext, null) || getCreateFieldsInAccessorClassProposals(invocationContext, null) || getRemovePropertiesProposals(invocationContext, null) || getRenameKeysProposals(invocationContext, null);
    ...
    private static boolean getEscapeUnescapeBackslashProposals(IQuickAssistInvocationContext invocationContext, ArrayList<ICompletionProposal> resultingCollections) throws BadLocationException, BadPartitioningException {
        ...
        if (selectionLength == 0) {
            if (selectionOffset != document.getLength()) {
                ...
                if (//see PropertiesFilePartitionScanner()
                ch == '=' || ch == ':') {
                    return false;
                }
            }
            ...
            if (type.equals(IPropertiesFilePartitions.PROPERTY_VALUE)) {
                //see PropertiesFilePartitionScanner()
    ...
    private static boolean isEclipseNLSUsed(IType accessor) {
        ...
        try {
            //$NON-NLS-1$
            IType nls = javaProject.findType("org.eclipse.osgi.util.NLS");
            if (nls == null)
                return false;
                    ...
                    if (fChanges[i] instanceof TextChange) {
                        ...
                        EditAnnotator ea = new EditAnnotator(buf, currentContent) {
                            ...
                            protected boolean rangeRemoved(TextEdit edit) {
                                return annotateEdit(//$NON-NLS-1$ //$NON-NLS-2$
                                edit, //$NON-NLS-1$ //$NON-NLS-2$
                                "<del>", //$NON-NLS-1$ //$NON-NLS-2$
                                "</del>");
                            }
                        };
                        rootEdit.accept(ea);
