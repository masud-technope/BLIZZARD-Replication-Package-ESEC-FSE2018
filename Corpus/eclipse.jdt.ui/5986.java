...
public class TypeEnvironmentTests extends AbstractCUTestCase {
    ...
    private void checkCanAssignTo(ITypeBinding rhsBinding, ITypeBinding lhsBinding, TType rhs, TType lhs) {
        ...
        assertEquals("Different assignment rule(" + PrettySignatures.get(lhsBinding) + "= " + PrettySignatures.get(rhsBinding) + "): ", coreResult, uiResult);
    }
            ...
            for (int i = 0; i < bindings.length; i++) {
                ...
                if (!BUG_83616_core_wildcard_assignments) {
                    if (coreResult != uiResult && !oType.isWildcardType()) {
                        System.out.println("Different assignment rule(" + PrettySignatures.get(iBinding) + "= " + PrettySignatures.get(oBinding) + "): Bindings<" + coreResult + "> TType<" + uiResult + ">");
                    }
                }
            }
        }
    ...
    public void _testAssignment() throws Exception {
        ...
        System.out.println(PrettySignatures.get(bindings[0]) + "= " + PrettySignatures.get(bindings[1]) + ": " + bindings[1].isAssignmentCompatible(bindings[0]));
    ...
    public void _testParameterizedToGeneric() throws Exception {
        ...
        System.out.println(PrettySignatures.get(bindings[0]) + "= " + PrettySignatures.get(bindings[1]) + ": " + bindings[1].isAssignmentCompatible(bindings[0]));
        System.out.println(PrettySignatures.get(bindings[0]) + "= " + PrettySignatures.get(bindings[0]) + ": " + bindings[0].isAssignmentCompatible(bindings[0]));
