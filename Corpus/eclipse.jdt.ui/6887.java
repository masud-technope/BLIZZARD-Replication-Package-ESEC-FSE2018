...
public class JavaDocAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
    ...
    private String prepareTemplateComment(String comment, String indentation, IJavaProject project, String lineDelimiter) {
        ...
        while (nonSpace < len && Character.getType(comment.charAt(nonSpace)) == Character.SPACE_SEPARATOR) nonSpace++;
    ...
    private String createMethodTags(IDocument document, DocumentCommand command, String indentation, String lineDelimiter, IMethod method) throws CoreException, BadLocationException {
        ...
        if (comment != null) {
            ...
            if (!isFirstComment(document, command, method, javadocComment))
                return null;
            //$NON-NLS-1$
            boolean isJavaDoc = partition.getLength() >= 3 && document.get(partition.getOffset(), 3).equals("/**");
