...
public class JUnitLaunchConfigurationDelegate extends AbstractJavaLaunchConfigurationDelegate {
    ...
    public synchronized void launch(ILaunchConfiguration configuration, String mode, ILaunch launch, IProgressMonitor monitor) throws CoreException {
        ...
        try {
            if (mode.equals(JUnitLaunchConfigurationConstants.MODE_RUN_QUIETLY_MODE)) {
                //$NON-NLS-1$
                launch.setAttribute(//$NON-NLS-1$
                JUnitLaunchConfigurationConstants.ATTR_NO_DISPLAY, //$NON-NLS-1$
                "true");
    ...
    private ITestKind getTestRunnerKind(ILaunchConfiguration configuration) {
        ...
        if (testKind.isNull()) {
            // backward compatible for launch configurations with no runner
    ...
    protected IMember[] evaluateTests(ILaunchConfiguration configuration, IProgressMonitor monitor) throws CoreException {
        ...
        //$NON-NLS-1$
        String testMethodName = configuration.getAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_NAME, "");
    ...
    protected void collectExecutionArguments(ILaunchConfiguration configuration, List<String> vmArguments, List<String> programArguments) throws CoreException {
        ...
        //$NON-NLS-1$
        String testFailureNames = configuration.getAttribute(JUnitLaunchConfigurationConstants.ATTR_FAILURES_NAMES, "");
        /*
        ...
        if (testElements.length == 1) {
            if (testElements[0] instanceof IMethod) {
                ...
                //$NON-NLS-1$
                programArguments.add(//$NON-NLS-1$
                "-test");
    ...
    private String createTestNamesFile(IMember[] testElements) throws CoreException {
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR, JUnitCorePlugin.CORE_PLUGIN_ID, IStatus.ERROR, "", e));
        }
    }
        ...
        public List<String> localizeClasspath(ITestKind kind) {
            ...
            for (int i = 0; i < entries.length; i++) {
                try {
                    ...
                    Assert.isTrue(false, entries[i].getPluginId() + " is available (required JAR)");
                }
            }
            return junitEntries;
        }
    ...
    private final IJavaElement getTestTarget(ILaunchConfiguration configuration, IJavaProject javaProject) throws CoreException {
        //$NON-NLS-1$
        String containerHandle = configuration.getAttribute(JUnitLaunchConfigurationConstants.ATTR_TEST_CONTAINER, "");
