...
public class UnpackFatJarBuilder extends FatJarBuilder {
    ...
    private void addFile(String destinationPath, ZipEntry jarEntry, ZipFile zipFile) {
        ...
        if (//$NON-NLS-1$
        destinationPath.equalsIgnoreCase("META-INF/MANIFEST.MF") || //$NON-NLS-1$//$NON-NLS-2$
        (isRemoveSigners() && destinationPath.startsWith("META-INF/") && destinationPath.endsWith(".SF"))) {
            return;
        }
        try {
