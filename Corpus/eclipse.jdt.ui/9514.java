...
public class MarkOccurrenceTest18 extends TestCase {
    ...
    public void testThrowingAnnotatedException1() throws Exception {
        ...
        // in Javadoc
        int offset = 8 + s.indexOf("@throws E.InnerException");
        int length = 0;
        OccurrenceLocation[] ranges = { find(s, "E.InnerException", 1), find(s, "@Critical() E.@Critical() InnerException", 1), find(s, "throw", 3), find(s, "throw", 4) };
        checkSelection(s, offset, length, ranges);
        fFinder = new ExceptionOccurrencesFinder();
        // in annotation
        offset = 1 + s.indexOf("@Critical() E.@Critical() InnerException");
        length = 0;
        checkSelection(s, offset, length, ranges);
        fFinder = new ExceptionOccurrencesFinder();
        // in annotated type qualifier
        offset = s.indexOf("E.@Critical() InnerException");
        length = 1;
        checkSelection(s, offset, length, ranges);
        fFinder = new ExceptionOccurrencesFinder();
        // in annotated catch type (does NOT include modifier "@Critical() "!)
        offset = 1 + s.indexOf("InnerException e)");
        length = 0;
        ranges = new OccurrenceLocation[] { find(s, "foo", 2), find(s, "E.@Critical() InnerException", 3) };
        checkSelection(s, offset, length, ranges);
        fFinder = new ExceptionOccurrencesFinder();
        // in annotated union type
        offset = s.indexOf("RuntimeException |");
        length = 0;
        ranges = new OccurrenceLocation[] { find(s, "foo", 3), find(s, "RuntimeException", 2) };
        checkSelection(s, offset, length, ranges);
        fFinder = new ExceptionOccurrencesFinder();
        // in annotation expression
        offset = s.indexOf("He");
        length = 0;
