...
public class PerformanceTestCase2 extends TestCase {
    ...
    protected void runTest() throws Throwable {
        ...
        try {
            ...
            for (int i = 0; i < warmUpRuns; i++) runMethod.invoke(this, args);
            int measuredRuns = getMeasuredRuns();
            PerformanceMeter meter = createPerformanceMeter();
            args[0] = meter;
            for (int i = 0; i < measuredRuns; i++) runMethod.invoke(this, args);
    ...
    protected final void commitAllMeasurements() {
        if (fPerformanceMeters != null)
            for (Iterator<PerformanceMeter> iter = fPerformanceMeters.iterator(); iter.hasNext(); ) iter.next().commit();
    }
    ...
    protected final void assertAllPerformance() {
        if (fPerformanceMeters != null)
            for (Iterator<PerformanceMeter> iter = fPerformanceMeters.iterator(); iter.hasNext(); ) assertPerformance(iter.next());
    }
