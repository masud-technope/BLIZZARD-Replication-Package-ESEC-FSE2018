...
public class CompilationUnitEditor extends JavaEditor implements IJavaReconcilingListener {
    ...
    private class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
        ...
        private boolean isAngularIntroducer(String identifier) {
            return identifier.length() > 0 && (Character.isUpperCase(identifier.charAt(0)) || //$NON-NLS-1$
            identifier.startsWith(//$NON-NLS-1$
            "final") || //$NON-NLS-1$
            identifier.startsWith(//$NON-NLS-1$
            "public") || //$NON-NLS-1$
            identifier.startsWith(//$NON-NLS-1$
            ...
            try {
                ...
                switch(event.character) {
                    case '(':
                        ...
                        break;
                    case '<':
                        if (!(fCloseAngularBrackets && fCloseBrackets) || nextToken == Symbols.TokenLESSTHAN || nextToken == Symbols.TokenQUESTIONMARK || nextToken == Symbols.TokenIDENT && isTypeArgumentStart(next) || prevToken != Symbols.TokenLBRACE && prevToken != Symbols.TokenRBRACE && prevToken != Symbols.TokenSEMICOLON && prevToken != Symbols.TokenSYNCHRONIZED && prevToken != Symbols.TokenSTATIC && (prevToken != Symbols.TokenIDENT || !isAngularIntroducer(previous)) && prevToken != Symbols.TokenEOF)
                            return;
                        break;
                    case '[':
                    ...
                    case '\'':
                    case '"':
                        if (!fCloseStrings || nextToken == Symbols.TokenIDENT || prevToken == Symbols.TokenIDENT || next != null && next.length() > 1 || previous != null && previous.length() > 1)
                            return;
                        break;
                    default:
                        return;
    ...
    private class RememberedSelection {
        ...
        public void restore() {
            ...
            try {
                ...
                if (startOffset == -1) {
                    // fallback to caret offset
                    startOffset = endOffset;
                    revealStartOffset = revealEndOffset;
                }
                if (endOffset == -1) {
                    // fallback to other offset
            ...
            if (containsOffset(element, offset)) {
                if (offset > 0) {
                    ...
                    if (element.getHandleIdentifier().equals(alternateElement.getParent().getHandleIdentifier()))
                        // Solves test case 2 from https://bugs.eclipse.org/bugs/show_bug.cgi?id=47727#c3
                        return offset - 1;
                }
                return offset;
            } else if (offset > 0 && containsOffset(element, offset - 1))
                // Solves test case 1 from https://bugs.eclipse.org/bugs/show_bug.cgi?id=47727#c3
                return offset - 1;
            return -1;
        }
    ...
    private static final String CODE_FORMATTER_TAB_SIZE = DefaultCodeFormatterConstants.FORMATTER_TAB_SIZE;

    /** Preference key for inserting spaces rather than tabs */
    private static final String SPACES_FOR_TABS = DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR;
    ...
    protected void createActions() {
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Uncomment.", this, ITextOperationTarget.STRIP_PREFIX);
        ...
        //$NON-NLS-1$
        action = new ToggleCommentAction(JavaEditorMessages.getBundleForConstructedKeys(), "ToggleComment.", this);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Format.", this, ISourceViewer.FORMAT);
        ...
        //$NON-NLS-1$
        action = new AddBlockCommentAction(JavaEditorMessages.getBundleForConstructedKeys(), "AddBlockComment.", this);
        action.setActionDefinitionId(IJavaEditorActionDefinitionIds.ADD_BLOCK_COMMENT);
        //$NON-NLS-1$
        setAction("AddBlockComment", action);
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        action = new RemoveBlockCommentAction(JavaEditorMessages.getBundleForConstructedKeys(), "RemoveBlockComment.", this);
        action.setActionDefinitionId(IJavaEditorActionDefinitionIds.REMOVE_BLOCK_COMMENT);
        //$NON-NLS-1$
        setAction("RemoveBlockComment", action);
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        action = new IndentAction(JavaEditorMessages.getBundleForConstructedKeys(), "Indent.", this, false);
        ...
        //$NON-NLS-1$
        action = new IndentAction(JavaEditorMessages.getBundleForConstructedKeys(), "Indent.", this, true);
        //$NON-NLS-1$
        ...
        fContextMenuGroup = new CompositeActionGroup(new ActionGroup[] { fGenerateActionGroup, fRefactorActionGroup, surroundWith, new LocalHistoryActionGroup(this, ITextEditorActionConstants.GROUP_EDIT) });
    ...
    protected void openSaveErrorDialog(String title, String message, CoreException exception) {
        ...
        if (JavaUI.ID_PLUGIN.equals(status.getPlugin()) && (status.getCode() == IJavaStatusConstants.EDITOR_POST_SAVE_NOTIFICATION || status.getCode() == IJavaStatusConstants.EDITOR_CHANGED_REGION_CALCULATION)) {
    ...
    private void openSaveListenerWarningDialog(String title, String message, CoreException exception) {
        ...
        ErrorDialog dialog = new ErrorDialog(getSite().getShell(), title, message, status, mask) {
            ...
            protected Control createMessageArea(Composite parent) {
                ...
                // filler as parent has 2 columns (icon and label)
