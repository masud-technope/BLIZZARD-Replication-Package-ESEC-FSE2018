...
public final class ReferenceTracker {
    ...
    private static boolean isInteresting(Class<?> clazz) {
        ...
        if (//$NON-NLS-1$
        name.startsWith(CURRENT_PKG_NAME) || name.startsWith("sun.reflect.")) {
            return false;
        }
        return true;
    }
    ...
    private static boolean isWeakOrSoftReference(Object curr, Field fld) {
        if (REFERENCE_NAME.equals(fld.getDeclaringClass().getName())) {
            return "referent".equals(fld.getName()) && (curr instanceof WeakReference || curr instanceof SoftReference);
        }
        return false;
    }
    ...
    public IStatus start(Object root) {
        // 2 M -> 8 MB
        fVisitedElements = new IdentityHashSet(1 << 21);
