...
class NLSSearchResultRequestor extends SearchRequestor {
    ...
    public void acceptSearchMatch(SearchMatch match) throws CoreException {
        ...
        if (javaElement.getElementType() == IJavaElement.CLASS_FILE)
            //matches in import statements of class files
            return;
        if (javaElement.getElementType() == IJavaElement.TYPE)
            //classes extending the accessor class and workaround for bug 61286
            return;
        ...
        if (javaElement instanceof ISourceReference) {
            ...
            if (source != null) {
                if (//$NON-NLS-1$
                source.indexOf("NLS.initializeMessages") != //$NON-NLS-1$
                -1)
                    return;
            }
        }
    ...
    private void addMatch(FileEntry groupElement, String propertyName) {
        ...
        int length;
        if (// not found -> report at beginning
        start == -1) {
            start = 0;
            length = 0;
        } else {
                ...
                if (nextToken == ITerminalSymbols.TokenNameStringLiteral) {
                    ...
                    for (int i = 0; i < fields.length; i++) {
                        if (fields[i].getElementName().equals(identifier)) {
                            if (!//$NON-NLS-1$
                            Signature.getSignatureSimpleName(fields[i].getTypeSignature()).equals(//$NON-NLS-1$
                            "String"))
                                return null;
    ...
    private int findPropertyNameStartPosition(String propertyName) {
        ...
        try {
            ...
            if (eols != -17)
                //key not found in file. See bug 63794. This can happen if the key contains escaped characters.
                start = -1;
