...
public class JavaSearchPage extends DialogPage implements ISearchPage {
    ...
    private static class SearchPatternData {
        ...
        public void store(IDialogSettings settings) {
            ...
            //$NON-NLS-1$
            settings.put("matchLocations", matchLocations);
            //$NON-NLS-1$ //$NON-NLS-2$
            settings.put("javaElement", javaElement != null ? javaElement.getHandleIdentifier() : "");
            //$NON-NLS-1$
            ...
            if (workingSets != null) {
                ...
                //$NON-NLS-1$
                settings.put(//$NON-NLS-1$
                "workingSets", //$NON-NLS-1$
                wsIds);
            } else {
                //$NON-NLS-1$
                settings.put(//$NON-NLS-1$
            ...
            //$NON-NLS-1$
            settings.put("includeMask", includeMask);
        }
        ...
        public static SearchPatternData create(IDialogSettings settings) {
            ...
            try {
                int searchFor = //$NON-NLS-1$
                settings.getInt(//$NON-NLS-1$
                "searchFor");
                int scope = //$NON-NLS-1$
                settings.getInt(//$NON-NLS-1$
                "scope");
                int limitTo = //$NON-NLS-1$
                settings.getInt(//$NON-NLS-1$
                "limitTo");
                int matchLocations = 0;
                if (//$NON-NLS-1$
                settings.get("matchLocations") != null) {
                    matchLocations = //$NON-NLS-1$
                    settings.getInt(//$NON-NLS-1$
                    "matchLocations");
                }
                boolean isCaseSensitive = //$NON-NLS-1$
                settings.getBoolean(//$NON-NLS-1$
                "isCaseSensitive");
                int includeMask;
                if (//$NON-NLS-1$
                settings.get("includeMask") != null) {
                    includeMask = //$NON-NLS-1$
                    settings.getInt(//$NON-NLS-1$
                    "includeMask");
                } else {
                    includeMask = JavaSearchScopeFactory.NO_JRE;
                    if (//$NON-NLS-1$ //$NON-NLS-2$
                    settings.get("includeJRE") == null ? forceIncludeAll(limitTo, elem) : settings.getBoolean("includeJRE")) {
    ...
    // search for
    private static final int TYPE = IJavaSearchConstants.TYPE;

    private static final int METHOD = IJavaSearchConstants.METHOD;

    private static final int PACKAGE = IJavaSearchConstants.PACKAGE;

    private static final int CONSTRUCTOR = IJavaSearchConstants.CONSTRUCTOR;

    private static final int FIELD = IJavaSearchConstants.FIELD;

    // limit to
    private static final int DECLARATIONS = IJavaSearchConstants.DECLARATIONS;

    private static final int IMPLEMENTORS = IJavaSearchConstants.IMPLEMENTORS;

    private static final int REFERENCES = IJavaSearchConstants.REFERENCES;

    private static final int SPECIFIC_REFERENCES = IJavaSearchConstants.CAST_TYPE_REFERENCE;

    private static final int ALL_OCCURRENCES = IJavaSearchConstants.ALL_OCCURRENCES;

    private static final int READ_ACCESSES = IJavaSearchConstants.READ_ACCESSES;

    private static final int WRITE_ACCESSES = IJavaSearchConstants.WRITE_ACCESSES;
    ...
    private int setLimitTo(int searchFor, int limitTo) {
        ...
        if (searchFor != TYPE && /*&& searchFor != FIELD*/
        searchFor != METHOD && searchFor != CONSTRUCTOR && limitTo == SPECIFIC_REFERENCES) {
            limitTo = REFERENCES;
        }
    ...
    private SearchPatternData getPatternData() {
        ...
        match = new SearchPatternData(getSearchFor(), getLimitTo(), fMatchLocations, pattern, fCaseSensitive.getSelection(), fJavaElement, getContainer().getSelectedScope(), getContainer().getSelectedWorkingSets(), getIncludeMask());
        // insert on top
    ...
    private void fillLimitToGroup(int searchFor, int limitTo) {
        ...
        if (searchFor == METHOD || searchFor == CONSTRUCTOR) {
            buttons.add(createMethodLocationRadio(limitTo == SPECIFIC_REFERENCES));
        }
        ...
        // re-apply font as we disposed the previous widgets
        Dialog.applyDialogFont(fLimitToGroup);
    ...
    private Control createIncludeMask(Composite parent) {
        ...
        fIncludeMasks = new Button[] { createButton(result, SWT.CHECK, SearchMessages.SearchPage_searchIn_sources, JavaSearchScopeFactory.SOURCES, true), createButton(result, SWT.CHECK, SearchMessages.SearchPage_searchIn_projects, JavaSearchScopeFactory.PROJECTS, true), createButton(result, SWT.CHECK, SearchMessages.SearchPage_searchIn_jre, JavaSearchScopeFactory.JRE, false), createButton(result, SWT.CHECK, SearchMessages.SearchPage_searchIn_libraries, JavaSearchScopeFactory.LIBS, true) };
        SelectionAdapter listener = new SelectionAdapter() {
    ...
    private SearchPatternData getDefaultInitValues() {
        ...
        //$NON-NLS-1$
        return new SearchPatternData(TYPE, REFERENCES, 0, fIsCaseSensitive, "", null, getLastIncludeMask());
    }
