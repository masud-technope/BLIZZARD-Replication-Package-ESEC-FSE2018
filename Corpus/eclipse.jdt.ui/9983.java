        ...
        private Runnable createQueryRunnable(final String question, final int[] result) {
            return new Runnable() {
                ...
                public void run() {
                    ...
                    MessageDialog dialog = new MessageDialog(fShell, fDialogTitle, null, question, MessageDialog.QUESTION, getButtonLabels(), 0);
                    dialog.open();
                    if (//MessageDialog closed without choice => cancel | no
                    dialog.getReturnCode() == -1) {
                ...
                private int[] getResultIDs() {
                    if (YesYesToAllNoNoToAllQuery.this.fAllowCancel)
                        return new int[] { IDialogConstants.YES_ID, IDialogConstants.YES_TO_ALL_ID, IDialogConstants.NO_ID, IDialogConstants.NO_TO_ALL_ID, IDialogConstants.CANCEL_ID };
                    else
            ...
            return new Runnable() {
                ...
                public void run() {
                    ...
                    switch(dialog.getReturnCode()) {
                        //MessageDialog closed without choice => cancel | no
                        case -1:
                            //see also https://bugs.eclipse.org/bugs/show_bug.cgi?id=48400
                            result[0] = fAllowCancel ? IDialogConstants.CANCEL_ID : IDialogConstants.NO_ID;
                            break;
