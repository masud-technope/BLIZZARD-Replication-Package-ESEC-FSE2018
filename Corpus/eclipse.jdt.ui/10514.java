...
public class PullUpMemberPage extends UserInputWizardPage {
    ...
    private void editSelectedMembers() {
        ...
        try {
            ...
            final String labelText = selectedMembers.length == 1 ? Messages.format(RefactoringMessages.PullUpInputPage1_Mark_selected_members_singular, JavaElementLabels.getElementLabel(selectedMembers[0].getMember(), JavaElementLabels.M_PARAMETER_TYPES)) : Messages.format(RefactoringMessages.PullUpInputPage1_Mark_selected_members_plural, String.valueOf(selectedMembers.length));
        ...
        for (int index = 0; index < infos.length; index++) {
            ...
            if (isMethodInfo && isDestinationInterface) {
                if (action == PULL_UP_ACTION)
                    // skip methods pulled up to interface
                    continue;
                if (action == DECLARE_ABSTRACT_ACTION && infoAction == PULL_UP_ACTION)
                    // method pulled to interface must be declared abstract
                    infoAction = DECLARE_ABSTRACT_ACTION;
            }
    ...
    private void updateStatusLine() {
        ...
        final String msg = selected == 1 ? Messages.format(RefactoringMessages.PullUpInputPage1_status_line_singular, JavaElementLabels.getElementLabel(((MemberActionInfo) selectedMembers[0]).getMember(), JavaElementLabels.M_PARAMETER_TYPES)) : Messages.format(RefactoringMessages.PullUpInputPage1_status_line_plural, String.valueOf(selected));
