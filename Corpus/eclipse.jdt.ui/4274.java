                    ...
                    for (final Iterator<TextEditBasedChangeGroup> outer = change.getGroups().iterator(); outer.hasNext(); ) {
                        ...
                        for (final Iterator<TextEdit> inner = group.getCachedEdits().iterator(); inner.hasNext(); ) {
                            ...
                            if (copiedEdit != null)
                                ...
                                RefactoringCorePlugin.logErrorMessage(//$NON-NLS-1$
                                "Could not find a copy for the indexed text edit " + //$NON-NLS-1$
                                originalEdit.toString());
                        }
                    }
                    ...
                    final TextEditProcessor processor = new TextEditProcessor(document, copy, TextEdit.NONE) {
                        ...
                        protected final boolean considerEdit(final TextEdit edit) {
                            ...
                            if (originalEdit != null) {
                                ...
                                RefactoringCorePlugin.logErrorMessage(//$NON-NLS-1$
                                "Could not find the original of the copied text edit " + //$NON-NLS-1$
                                edit.toString());
                            }
                            return true;
                        }
            ...
            try {
                ...
                try {
                    ...
                    for (int index = 0; index < positions.length; index++) {
                        ...
                        document.replace(position.offset, position.length, position.getText() != null ? //$NON-NLS-1$
                        position.getText() : "");
                    }
                } catch (BadPositionCategoryException exception) {
    ...
    public final RefactoringStatus isValid(IProgressMonitor monitor) throws CoreException, OperationCanceledException {
        ...
        try {
            if (fValidationState == null)
                ...
                new Status(IStatus.ERROR, RefactoringCorePlugin.getPluginId(), "MultiStateTextFileChange has not been initialialized"));
