...
public class JavadocHelpContext implements IContext2 {
    ...
    public  JavadocHelpContext(IContext context, Object[] elements) throws JavaModelException {
            ...
            if (elements[i] instanceof IJavaElement) {
                ...
                if (BUG_85721_FIXED) {
                    if (javadocSummary == null) {
                        ...
                        // no Javadoc summary for multiple selection //$NON-NLS-1$
                        javadocSummary = "";
                    }
                }
        ...
        if (BUG_85721_FIXED) {
            if (javadocSummary != null && javadocSummary.length() > 0) {
                if (fText != null)
                    fText = //$NON-NLS-1$
                    context.getText() + "<br><br>" + //$NON-NLS-1$
                    javadocSummary;
                else
    ...
    private String getURLString(URL url) {
        ...
        if (url.getRef() != null) {
            ...
            if (anchorIdx != -1) {
                return location.substring(0, anchorIdx) + "?noframes=true" + //$NON-NLS-1$
                location.substring(//$NON-NLS-1$
                anchorIdx);
            }
        }
        //$NON-NLS-1$
        return location + "?noframes=true";
    }
