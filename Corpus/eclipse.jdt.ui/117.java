...
public class UndoableOperation2ChangeAdapter implements IUndoableOperation, IAdvancedUndoableOperation {
    ...
    public  UndoableOperation2ChangeAdapter(Change change) {
        ...
        // remember it now: can't access anymore from executed change
        fChangeDescriptor = change.getDescriptor();
    }
            ...
            public void run(IProgressMonitor monitor) throws CoreException {
                ...
                try {
                    ...
                    if (result.reverseChange != null) {
                        result.reverseChange.initializeValidationData(new NotCancelableProgressMonitor(new SubProgressMonitor(monitor, 1)));
    ...
    private IStatus createStatus(ExecuteResult result) {
        if (!result.validationStatus.isOK()) {
            ...
            return new Status(IStatus.ERROR, RefactoringCorePlugin.getPluginId(), IStatus.ERROR, RefactoringCoreMessages.UndoableOperation2ChangeAdapter_error_message, null);
        }
    }
