...
public final class WhiteSpaceOptions {
    ...
    public static class InnerNode extends Node {
        ...
        public void setChecked(boolean checked) {
            for (final Iterator<Node> iter = fChildren.iterator(); iter.hasNext(); ) iter.next().setChecked(checked);
        }
	 ...
	 * Preview snippets.
	 */
    private final PreviewSnippet FOR_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "for (int i= 0, j= array.length; i < array.length; i++, j--) {}\nfor (String s : names) {}");

    private final PreviewSnippet WHILE_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "while (condition) {}; do {} while (condition);");

    private final PreviewSnippet CATCH_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "try { number= Integer.parseInt(value); } catch (NumberFormatException e) {}");

    private final PreviewSnippet TRY_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "try (FileReader reader1 = new FileReader(\"file1\"); FileReader reader2 = new FileReader(\"file2\")) {}");

    private final PreviewSnippet IF_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "if (condition) { return foo; } else {return bar;}");

    private final PreviewSnippet SYNCHRONIZED_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "synchronized (list) { list.add(element); }");

    private final PreviewSnippet SWITCH_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "switch (number) { case RED: return GREEN; case GREEN: return BLUE; case BLUE: return RED; default: return BLACK;}");

    private final PreviewSnippet CONSTRUCTOR_DECL_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "MyClass() throws E0, E1 { this(0,0,0);}" + //$NON-NLS-1$
    "MyClass(int x, int y, int z) throws E0, E1 { super(x, y, z, true);}");

    private final PreviewSnippet METHOD_DECL_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "void foo() throws E0, E1 {};" + //$NON-NLS-1$
    "void bar(int x, int y) throws E0, E1 {}");

    private final PreviewSnippet ARRAY_DECL_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "int [] array0= new int [] {};\n" + //$NON-NLS-1$
    "int [] array1= new int [] {1, 2, 3};\n" + //$NON-NLS-1$
    "int [] array2= new int[3];");

    private final PreviewSnippet ARRAY_REF_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "array[i].foo();");

    private final PreviewSnippet METHOD_CALL_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "foo();\n" + //$NON-NLS-1$
    "bar(x, y);");
    ...
    //    "this();\n\n" +  //$NON-NLS-1$
    //    "this(x, y);\n"); //$NON-NLS-1$
    private final PreviewSnippet ALLOC_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "String str= new String(); Point point= new Point(x, y);");

    private final PreviewSnippet LABEL_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "label: for (int i= 0; i<list.length; i++) {for (int j= 0; j < list[i].length; j++) continue label;}");

    private final PreviewSnippet SEMICOLON_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "int a= 4; foo(); bar(x, y);");

    private final PreviewSnippet CONDITIONAL_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "String value= condition ? TRUE : FALSE;");

    private final PreviewSnippet CLASS_DECL_PREVIEW = new PreviewSnippet(CodeFormatter.K_COMPILATION_UNIT, //$NON-NLS-1$
    "class MyClass implements I0, I1, I2 {}");

    private final PreviewSnippet ANON_CLASS_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "AnonClass= new AnonClass() {void foo(Some s) { }};");

    private final PreviewSnippet OPERATOR_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "List list= new ArrayList(); int a= -4 + -9; b= a++ / --number; c += 4; boolean value= true && false;");

    private final PreviewSnippet CAST_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "String s= ((String)object);");

    private final PreviewSnippet MULT_LOCAL_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "int a= 0, b= 1, c= 2, d= 3;");

    private final PreviewSnippet MULT_FIELD_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "int a=0,b=1,c=2,d=3;");

    private final PreviewSnippet BLOCK_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "if (true) { return 1; } else { return 2; }");

    private final PreviewSnippet PAREN_EXPR_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "result= (a *( b +  c + d) * (e + f));");

    private final PreviewSnippet ASSERT_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "assert condition : reportError();");

    private final PreviewSnippet RETURN_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "return (o);");

    private final PreviewSnippet THROW_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "throw (e);");

    private final PreviewSnippet ANNOTATION_DECL_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "@interface MyAnnotation { String value(); }\n@interface OtherAnnotation { }\n");

    private final PreviewSnippet ANNOTATION_MODIFIER_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "@Annot(x=23, y=-3)\npublic class A { }\n");

    private final PreviewSnippet ENUM_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "enum MyEnum { GREEN(0, 1), RED() {\nvoid process() {}\n}\n}");

    private final PreviewSnippet PARAMETERIZED_TYPE_REFERENCE_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "Map<String, Element> map=\n new HashMap<String, Element>();");

    private final PreviewSnippet TYPE_ARGUMENTS_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "x.<String, Element>foo();");

    private final PreviewSnippet TYPE_PARAMETER_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "class MyGenericType<S, T extends Element & List> { }");

    private final PreviewSnippet VARARG_PARAMETER_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "void format(String s, Object ... args) {}");

    private final PreviewSnippet WILDCARD_PREVIEW = new PreviewSnippet(CodeFormatter.K_CLASS_BODY_DECLARATIONS, //$NON-NLS-1$
    "Map<X<?>, Y<? extends K, ? super V>> t;");

    private final PreviewSnippet LAMBDA_EXPR_PREVIEW = new PreviewSnippet(CodeFormatter.K_STATEMENTS, //$NON-NLS-1$
    "Runnable r = ()->process();");
