...
public class TokenScanner {
    ...
    public  TokenScanner(IDocument document, IJavaProject project) {
        ...
        // no line info required
        fScanner = ToolFactory.createScanner(true, false, false, sourceLevel, complianceLevel);
    ...
    public  TokenScanner(ITypeRoot typeRoot) throws CoreException {
        ...
        if (buffer == null) {
            //$NON-NLS-1$
            throw new CoreException(createError(DOCUMENT_ERROR, "Element has no source", null));
        }
        String sourceLevel = project.getOption(JavaCore.COMPILER_SOURCE, true);
        String complianceLevel = project.getOption(JavaCore.COMPILER_COMPLIANCE, true);
        // line info required
        fScanner = ToolFactory.createScanner(true, false, true, sourceLevel, complianceLevel);
        fScanner.setSource(buffer.getCharacters());
        // use scanner for line information
        fDocument = null;
        fEndPosition = fScanner.getSource().length - 1;
    }
        ...
        do {
            try {
                ...
                if (curr == ITerminalSymbols.TokenNameEOF) {
                    throw new CoreException(createError(//$NON-NLS-1$
                    END_OF_FILE, //$NON-NLS-1$
                    "End Of File", //$NON-NLS-1$
                    null));
                }
