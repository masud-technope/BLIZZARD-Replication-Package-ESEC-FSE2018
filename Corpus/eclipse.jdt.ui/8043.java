...
class JavaNode extends DocumentRangeNode implements ITypedElement {
    ...
    public  JavaNode(IDocument document) {
        //$NON-NLS-1$
        super(CU, JavaCompareUtilities.buildID(CU, "root"), document, 0, document.getLength());
    }
    ...
    public String getName() {
        ...
        // we strip away the type character
    ...
    public String toString() {
        return //$NON-NLS-1$
        getType() + ": " + getName() + "[" + getRange().offset + "+" + getRange().length + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "]";
    }
}
