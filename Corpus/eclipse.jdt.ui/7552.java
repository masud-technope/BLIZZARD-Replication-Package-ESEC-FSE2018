...
public final class RefactoringDescriptorDeleteQuery implements IRefactoringDescriptorDeleteQuery {
    ...
    //$NON-NLS-1$;
    private static final String PREFERENCE_DO_NOT_WARN_DELETE = RefactoringUIPlugin.getPluginId() + ".do.not.warn.delete.descriptor";
                        ...
                        if (!fShell.isDisposed()) {
                            ...
                            if (fProject != null) {
                                if (fCount == 1) {
                                    ...
                                    message = Messages.format(RefactoringUIMessages.RefactoringPropertyPage_confirm_delete_pattern_plural, new String[] { count, BasicElementLabels.getResourceName(fProject) });
                                }
                            } else {
                                if (fCount == 1) {
