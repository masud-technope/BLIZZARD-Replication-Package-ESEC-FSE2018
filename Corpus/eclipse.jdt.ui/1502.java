...
public class JavaTypeCompletionProcessor extends CUPositionCompletionProcessor {
    ...
    public void setPackageFragment(IPackageFragment packageFragment) {
        ...
        if (packageFragment == null) {
            ...
            //$NON-NLS-1$//$NON-NLS-2$
            String before = "public class " + DUMMY_CLASS_NAME + " { ";
            //$NON-NLS-1$
            String after = " }";
    ...
    public void setExtendsCompletionContext(IJavaElement javaElement) {
        if (javaElement instanceof IPackageFragment) {
            ...
            while (parent instanceof IType) {
                type = (IType) parent;
                //$NON-NLS-1$ //$NON-NLS-2$
                before += "public class " + type.getElementName() + " {";
                //$NON-NLS-1$
                after += "}";
    ...
    protected static class TypeCompletionRequestor extends CUPositionCompletionRequestor {
        ...
        private static final List<String> BASE_TYPES = Arrays.asList(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$
            ...
            switch(proposal.getKind()) {
                case CompletionProposal.PACKAGE_REF:
                    ...
                    if (TypeFilter.isFiltered(packageName))
                        return;
                    addAdjustedCompletion(new String(packageName), new String(proposal.getCompletion()), proposal.getReplaceStart(), proposal.getReplaceEnd(), proposal.getRelevance(), JavaPluginImages.DESC_OBJS_PACKAGE);
                    return;
                case CompletionProposal.TYPE_REF:
                    ...
                    if (buf.length() == 0)
                        // this is the dummy class, whose $ have been converted to dots
                        return;
                    ...
                    addAdjustedTypeCompletion(name, new String(proposal.getCompletion()), proposal.getReplaceStart(), proposal.getReplaceEnd(), proposal.getRelevance(), typeImageDescriptor, fullyQualify ? new String(fullName) : null);
                    return;
                case CompletionProposal.KEYWORD:
                    ...
                    if ((fEnableVoid && VOID.equals(keyword)) || (fEnableBaseTypes && BASE_TYPES.contains(keyword)))
                        addAdjustedCompletion(keyword, new String(proposal.getCompletion()), proposal.getReplaceStart(), proposal.getReplaceEnd(), proposal.getRelevance(), null);
                    return;
                default:
                    return;
            }
