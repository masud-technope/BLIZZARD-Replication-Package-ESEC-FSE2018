...
public class KeyboardProbe {
    ...
    void probe() {
        ...
        for (fKeyCode = 0; fKeyCode < 128; fKeyCode++) {
            ...
            if (TRACE)
                System.out.println("" + fKeyCode + "content[NONE]: " + c);
            fCodes[fKeyCode][NONE] = c;
            clearText(text);
            postShiftKeyPress(fKeyCode);
            c = getContent(text);
            if (TRACE)
                System.out.println("" + fKeyCode + "content[SHIFT]: " + c);
    ...
    private void clearText(Text text) {
        fTextContinue = false;
        text.setText("");
        do driveEventQueue(); while (!fTextContinue);
    }
    ...
    private void postNaturalKeyPress(int i) {
        ...
        fDisplay.post(event2);
        do driveEventQueue(); while (!fKeyContinue);
    }
