...
final class CompilationUnitCompletion extends CompletionRequestor {
    ...
    public final class Variable {
        ...
        public boolean isCollection() {
            ...
            if (//$NON-NLS-1$
            (fType == UNKNOWN || fType == ITERABLE) && (fChecked & COLLECTION) == 0 && isSubtypeOf("java.util.Collection"))
                fType = COLLECTION;
            fChecked |= COLLECTION;
        ...
        public boolean isIterable() {
            if (//$NON-NLS-1$
            fType == UNKNOWN && (fChecked & ITERABLE) == 0 && isSubtypeOf("java.lang.Iterable"))
                fType = ITERABLE;
            fChecked |= ITERABLE;
            // Collection extends Iterable
            return fType == ITERABLE || fType == COLLECTION;
        }
                ...
                if (fMemberTypes == null) {
                    try {
                        try {
                            ...
                            fMemberTypes = //$NON-NLS-1$
                            util.computeBinding(//$NON-NLS-1$
                            "java.lang.Iterable", //$NON-NLS-1$
                            0);
                        } catch (JavaModelException e) {
                            try {
                                TypeParameterResolver util = new TypeParameterResolver(this);
                                fMemberTypes = util.computeBinding("java.util.Collection", 0);
        ...
        public String toString() {
            ...
            return "LocalVariable [name=\"" + name + "\" signature=\"" + signature + "\" type=\"" + type + "\" member=\"" + getMemberTypeSignature() + "\"]";
        }
    }
    ...
    private final class TypeParameterResolver {
        ...
        public String[] computeBinding(String superType, int index) throws JavaModelException, IndexOutOfBoundsException {
            ...
            if (type == null)
                throw new //$NON-NLS-1$
                JavaModelException(//$NON-NLS-1$
                new CoreException(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "No such type", null)));
        ...
        private void computeTypeParameterBinding(final IType superType, final int index) throws JavaModelException, IndexOutOfBoundsException {
            ...
            // take the first, as they all lead to fType
        ...
        private boolean isTrueSubtypeOf(String subTypeSignature, String superTypeSignature) {
            ...
            try {
                ...
                for (int i = 0; i < types.length; i++) if (types[i].equals(superType))
                    return true;
            } catch (JavaModelException e) {
            }
            return false;
