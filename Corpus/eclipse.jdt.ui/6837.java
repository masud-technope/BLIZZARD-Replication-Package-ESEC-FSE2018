...
public class SmartSemicolonAutoEditStrategy implements IAutoEditStrategy {
    ...
    public void customizeDocumentCommand(IDocument document, DocumentCommand command) {
        ...
        try {
            ...
            if (manager != null && JavaPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.EDITOR_SMART_BACKSPACE)) {
                ...
                UndoSpec s1 = new UndoSpec(command.offset + command.text.length(), new Region(command.offset, 0), new TextEdit[] { e1 }, 0, null);
                ...
                UndoSpec s2 = new UndoSpec(position + insertion.length(), new Region(command.offset + command.text.length(), 0), new TextEdit[] { smart, raw }, 2, s1);
                manager.register(s2);
            }
            // 3: modify command
    ...
    private String adjustSpacing(IDocument doc, int position, char character) {
        if (character == BRACECHAR) {
            if (position > 0 && position <= doc.getLength()) {
                ...
                doc, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                "try") || looksLike(//$NON-NLS-1$
                doc, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                "else") || looksLike(doc, pos, "synchronized") || looksLike(//$NON-NLS-1$
                doc, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                "static") || looksLike(//$NON-NLS-1$
                doc, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                "finally") || looksLike(//$NON-NLS-1$
                doc, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                "do"))
    ...
    protected static int computeCharacterPosition(IDocument document, ITextSelection line, int offset, char character, String partitioning) {
        ...
        if (character == BRACECHAR) {
            ...
            if (isForStatement(text, offset)) {
                // don't do anything in for statements, as semis are vital part of these
                insertPos = -1;
            } else {
    ...
    private static int computeAfterTryDoElse(IDocument doc, ITextSelection line, int offset) {
        ...
        p--;
        if (//$NON-NLS-1$
        looksLike(doc, p, "try") || looksLike(//$NON-NLS-1$
        doc, //$NON-NLS-1$
        p, //$NON-NLS-1$
        "do") || looksLike(//$NON-NLS-1$
        doc, //$NON-NLS-1$
        p, //$NON-NLS-1$
        "synchronized") || looksLike(//$NON-NLS-1$
        doc, //$NON-NLS-1$
        p, //$NON-NLS-1$
        "static") || looksLike(//$NON-NLS-1$
        doc, //$NON-NLS-1$
    ...
    private static boolean looksLikeIfWhileForCatch(IDocument document, int position, String partitioning) {
        ...
        if (position == -1)
            return false;
        return //$NON-NLS-1$
        looksLike(document, position, "if") || looksLike(//$NON-NLS-1$
        document, //$NON-NLS-1$
        position, //$NON-NLS-1$
        "while") || looksLike(//$NON-NLS-1$
        document, //$NON-NLS-1$
        position, //$NON-NLS-1$
        "catch") || looksLike(document, position, "synchronized") || looksLike(//$NON-NLS-1$
        document, //$NON-NLS-1$
        position, //$NON-NLS-1$
        "switch") || looksLike(//$NON-NLS-1$
        document, //$NON-NLS-1$
        position, //$NON-NLS-1$
        "for");
    }
