...
public abstract class ModifyDialogTabPage implements IModifyDialogTabPage {
    ...
    protected final class ComboPreference extends Preference {
        ...
        public  ComboPreference(Composite composite, int numColumns, Map<String, String> preferences, String key, String[] values, String text, String[] items) {
            ...
            int max = 0;
            for (int i = 0; i < items.length; i++) if (items[i].length() > max)
                max = items[i].length();
	 ...
	 * index.
	 */
    protected static final class DefaultFocusManager extends FocusAdapter {

        //$NON-NLS-1$
        private static final String PREF_LAST_FOCUS_INDEX = JavaUI.ID_PLUGIN + "formatter_page.modify_dialog_tab_page.last_focus_index";
    ...
    protected Group createGroup(int numColumns, Composite parent, String text) {
        ...
        //layout.marginHeight= fPixelConverter.convertVerticalDLUsToPixels(IDialogConstants.VERTICAL_MARGIN);
        //layout.marginWidth= fPixelConverter.convertHorizontalDLUsToPixels(IDialogConstants.HORIZONTAL_MARGIN);
        //createGridLayout(numColumns, true));
