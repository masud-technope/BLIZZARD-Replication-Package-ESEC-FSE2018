...
public abstract class ProfileConfigurationBlock {
                ...
                switch(value) {
                    ...
                    case ProfileManager.SETTINGS_CHANGED_EVENT:
                        try {
                            ...
                            fProfileStore.writeProfiles(fProfileManager.getSortedProfiles(), fInstanceScope);
    ...
    class ButtonController implements Observer, SelectionListener {
        ...
        private void exportAllButtonPressed() {
            ...
            //$NON-NLS-1$
            final String lastPath = JavaPlugin.getDefault().getDialogSettings().get(fLastSaveLoadPathKey + ".loadpath");
            if (lastPath != null) {
            ...
            if (path == null)
                return;
            //$NON-NLS-1$
            JavaPlugin.getDefault().getDialogSettings().put(fLastSaveLoadPathKey + ".savepath", dialog.getFilterPath());
            ...
            //$NON-NLS-1$
            final IContentType type = Platform.getContentTypeManager().getContentType("org.eclipse.core.runtime.xml");
            if (type != null)
        ...
        private void deleteButtonPressed() {
            if (MessageDialog.openQuestion(fComposite.getShell(), FormatterMessages.CodingStyleConfigurationBlock_delete_confirmation_title, Messages.format(FormatterMessages.CodingStyleConfigurationBlock_delete_confirmation_question, fProfileManager.getSelected().getName()))) {
        ...
        private void loadButtonPressed() {
            ...
            if (path == null)
                return;
            //$NON-NLS-1$
            JavaPlugin.getDefault().getDialogSettings().put(fLastSaveLoadPathKey + ".loadpath", dialog.getFilterPath());
            final File file = new File(path);
