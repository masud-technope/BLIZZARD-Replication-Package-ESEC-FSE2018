...
public class TextViewerUndoManagerLeakTest extends LeakTestCase {
    ...
    public void testTextViewerUndoManager() throws Exception {
        ...
        fShell = null;
        fTextViewer = null;
        String[] types = { TextViewer.class.getName(), TextViewerUndoManager.class.getName(), getSharedUndoManagersInnerClass("TextInputListener").getName(), getSharedUndoManagersInnerClass("DocumentUndoListener").getName(), getSharedUndoManagersInnerClass("KeyAndMouseListener").getName(), DocumentUndoManager.class.getName(), getDocumentUndoManagersInnerClass("DocumentListener").getName(), getDocumentUndoManagersInnerClass("HistoryListener").getName() };
    ...
    private void internalTestRandomAccess() {
        ...
        assertTrue(fUndoManager.undoable());
        while (fUndoManager.undoable()) fUndoManager.undo();
        final String reverted = document.get();
        assertEquals(original, reverted);
    }
    ...
    private static String createRandomString(int length) {
        ...
        for (int i = 0; i < length; i++) buffer.append(getRandomCharacter());
