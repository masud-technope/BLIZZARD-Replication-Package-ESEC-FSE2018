...
public class ConstructorFromSuperclassProposal extends LinkedCorrectionProposal {
    ...
    public Image getImage() {
        return JavaPlugin.getImageDescriptorRegistry().get(new JavaElementImageDescriptor(JavaPluginImages.DESC_MISC_PUBLIC, JavaElementImageDescriptor.CONSTRUCTOR, JavaElementImageProvider.SMALL_SIZE));
    }
    ...
    private MethodDeclaration createNewMethodDeclaration(AST ast, IMethodBinding binding, ASTRewrite rewrite, ImportRewriteContext importRewriteContext, CodeGenerationSettings commentSettings) throws CoreException {
        ...
        if (binding == null) {
            ...
            for (int i = 0; i < paramNames.length; i++) {
                ...
                addLinkedPosition(//$NON-NLS-1$
                rewrite.track(argument), //$NON-NLS-1$
                false, //$NON-NLS-1$
                "arg_name_" + paramNames[i]);
            }
        }
        //$NON-NLS-1$
