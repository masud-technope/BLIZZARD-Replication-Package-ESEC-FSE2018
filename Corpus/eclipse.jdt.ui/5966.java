...
public final class ContributedProcessorDescriptor {
    ...
    public IStatus checkSyntax() {
        ...
        if (children.length > 1) {
            ...
            //$NON-NLS-1$
            return new StatusInfo(IStatus.ERROR, "Only one < enablement > element allowed. Disabling " + id);
        }
        //$NON-NLS-1$
        return new StatusInfo(IStatus.OK, "Syntactically correct quick assist/fix processor");
    }
    ...
    private boolean matches(ICompilationUnit cunit) {
        ...
        if (children.length == 1) {
            try {
                ...
                //$NON-NLS-1$
                evalContext.addVariable(//$NON-NLS-1$
                ...
                //$NON-NLS-1$
                evalContext.addVariable(//$NON-NLS-1$
                "projectNatures", //$NON-NLS-1$
                Arrays.asList(natures));
                //$NON-NLS-1$
                evalContext.addVariable(//$NON-NLS-1$
                "sourceLevel", //$NON-NLS-1$
            ...
            if (fProcessorInstance == null) {
                try {
                    ...
                    if (expectedType.isInstance(extension)) {
                        ...
                        String message = "Invalid extension to " + //$NON-NLS-1$
                        fConfigurationElement.getName() + ". Must extends '" + expectedType.getName() + "'." + //$NON-NLS-1$ //$NON-NLS-2$
