...
public class JavadocConfigurationBlock {
    ...
    private void initializeSelections() {
        //$NON-NLS-1$
        String initialValue = fInitialURL != null ? fInitialURL.toExternalForm() : "";
        if (fIsForSource) {
        ...
        if (isArchive) {
            ...
            //$NON-NLS-1$
            final String resourceProtocol = "platform:/resource/";
    ...
    private class EntryValidator implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                protocol.startsWith("http") || protocol.equals("jar")) {
        ...
        private void validateFile(URL location) throws MalformedURLException {
            ...
            if (folder.isDirectory()) {
                File indexFile = new //$NON-NLS-1$
                File(//$NON-NLS-1$
                folder, //$NON-NLS-1$
                "index.html");
                if (indexFile.isFile()) {
                    File packageList = new //$NON-NLS-1$
                    File(//$NON-NLS-1$
                    folder, //$NON-NLS-1$
                    "package-list");
        ...
        private void validateURL(URL location) throws MalformedURLException, URISyntaxException {
            ...
            //$NON-NLS-1$
            URI index = URIUtil.append(path, "index.html");
            //$NON-NLS-1$
            URI packagelist = URIUtil.append(path, "package-list");
    ...
    private String chooseJavaDocFolder() {
        ...
        String initPath = "";
        if (//$NON-NLS-1$
        fURLResult != null && "file".equals(fURLResult.getProtocol())) {
    ...
    private URL getArchiveURL() throws MalformedURLException {
        ...
        try {
            ...
            //$NON-NLS-1$
            return new URI("jar:" + encodeExclamationMarks(baseUri.toString()) + '!' + encodeExclamationMarks(encodedInnerPath)).toURL();
        } catch (URISyntaxException e) {
