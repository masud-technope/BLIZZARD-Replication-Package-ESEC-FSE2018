            ...
            CollectingSearchRequestor requestor = new CollectingSearchRequestor(binaryRefs) {
                ...
                public void acceptSearchMatch(SearchMatch match) throws CoreException {
                    ...
                    if (match.isInsideDocComment())
                        // TODO: should warn user (with something like a ReferencesInBinaryContext)
                        return;
                    ICompilationUnit unit = SearchUtils.getCompilationUnit(match);
                    if (match.getAccuracy() == SearchMatch.A_INACCURATE) {
                        if (unit != null) {
                            status.addError(RefactoringCoreMessages.TargetProvider_inaccurate_match, JavaStatusContext.create(unit, new SourceRange(match.getOffset(), match.getLength())));
                        } else {
