...
public class HTMLTidyTest extends TestCase {
    ...
    private StringMatcher[] getIgnores() throws IOException {
        ...
        while (reader.ready()) {
            ...
            if (line.length() > 0) {
                ...
                if (line.endsWith("/*"))
                    // stops at directory during tree traversal
    ...
    private static boolean isRelevant(String line) {
        return !EXCLUSION_PATTERN_TABLE_SUMMARY.matcher(line).matches() && !EXCLUSION_PATTERN_EMPTY_TAG.matcher(line).matches() && !EXCLUSION_PATTERN_MALFORMED_URI.matcher(line).matches() && !eXCLUSION_PATTERN_SCRIPT_TYPE.matcher(line).matches() && !eXCLUSION_PATTERN_IMG_LACKS_ALT.matcher(line).matches();
    }
