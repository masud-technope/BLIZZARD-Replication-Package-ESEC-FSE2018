...
class ExtractMethodAnalyzer extends CodeAnalyzer {
            ...
            astNode.accept(new ASTVisitor() {
                ...
                public boolean visit(BreakStatement node) {
                    ...
                    if (label != null && !fLocalLoopLabels.contains(label.getIdentifier())) {
                        continueMatchesLoopProblem[0] = Messages.format(RefactoringCoreMessages.ExtractMethodAnalyzer_branch_break_mismatch, new Object[] { (//$NON-NLS-1$
                        "break " + //$NON-NLS-1$
                        label.getIdentifier()) });
                    }
                    return false;
                ...
                public void endVisit(ContinueStatement node) {
                    ...
                    if (label != null && !fLocalLoopLabels.contains(label.getIdentifier())) {
                        if (fEnclosingLoopLabel == null || !label.getIdentifier().equals(fEnclosingLoopLabel.getIdentifier())) {
                            continueMatchesLoopProblem[0] = Messages.format(RefactoringCoreMessages.ExtractMethodAnalyzer_branch_continue_mismatch, new Object[] { //$NON-NLS-1$
                            "continue " + //$NON-NLS-1$
                            label.getIdentifier() });
                        }
                    }
    ...
    private boolean isVoidMethod() {
        ...
        if (//$NON-NLS-1$
        fEnclosingBodyDeclaration.getAST().resolveWellKnownType("void").equals(binding))
            return true;
        return false;
    }
