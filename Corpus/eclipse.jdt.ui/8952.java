...
public abstract class AbstractDocumentPerformanceTest extends TextPerformanceTestCase2 {
    ...
    public void measureSet(PerformanceMeter meter) {
        meter.start();
        for (int accumulated = 0; accumulated < 30; accumulated++) fDocument.set(FAUST1);
        meter.stop();
    }
    ...
    public void measureRepeatedReplace(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 30000; times++) fDocument.replace(OFFSET, REPLACE_LENGTH, FAUST100);
        meter.stop();
    }
    ...
    public void measureTypingReplaceInLargeFile(PerformanceMeter meter) throws BadLocationException {
        ...
        int offset = 12;
        meter.start();
        for (int times = 0; times < 54000000; times++) while (offset++ < 400) fDocument.replace(offset, 0, ";");
        meter.stop();
    }
    ...
    public void measureInsertAtStart(PerformanceMeter meter) throws BadLocationException {
        fDocument.set(FAUST1);
        meter.start();
        for (int times = 0; times < 3000; times++) fDocument.replace(0, 0, FAUST100);
        meter.stop();
    }
    ...
    public void measureDeleteInsert(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 1000; times++) // delete phase
        fDocument.replace(OFFSET, REPLACE_LENGTH, null);
        for (int times = 0; times < 1000; times++) // insert phase
        fDocument.replace(OFFSET, 0, FAUST100);
        meter.stop();
    }
    ...
    public void measureRandomReplace(PerformanceMeter meter) throws BadLocationException {
        ...
        // chosen to fit in the gap text store size
        int REPLACE_LENGTH = 100;
        for (int i = 0; i < indices.length; i++) indices[i] = rand.nextInt(FAUST1.length() - REPLACE_LENGTH);
        char[] replacement = new char[REPLACE_LENGTH];
        Arrays.fill(replacement, ' ');
        String replace = new String(replacement);
        meter.start();
        for (int times = 0; times < 4; times++) for (int i = 0; i < indices.length; i++) {
    ...
    public void measureGetChar(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 8; times++) for (int offset = 0; offset < length; offset++) fDocument.getChar(offset);
        meter.stop();
    }
    ...
    public void measureGetLength(PerformanceMeter meter) {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 500; times++) for (int line = 0; line < lines; line++) fDocument.getLength();
        meter.stop();
    }
    ...
    public void measureLineByOffset(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 2; times++) for (int offset = 0; offset <= chars; offset++) fDocument.getLineOfOffset(offset);
        meter.stop();
    }
    ...
    public void measureLineInfoByOffset(PerformanceMeter meter) throws BadLocationException {
        ...
        int chars = FAUST1.length();
        meter.start();
        for (int times = 0; times < 2; times++) for (int offset = 0; offset <= chars; offset++) fDocument.getLineInformationOfOffset(offset);
        meter.stop();
    }
    ...
    public void measureLineByIndex(PerformanceMeter meter) throws BadLocationException {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 250; times++) for (int line = 0; line < lines; line++) fDocument.getLineOffset(line);
        meter.stop();
    }
    ...
    public void measureLineInfoByIndex(PerformanceMeter meter) throws BadLocationException {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 250; times++) for (int line = 0; line < lines; line++) fDocument.getLineInformation(line);
        meter.stop();
    }
    ...
    public void measureLineLengthByIndex(PerformanceMeter meter) throws BadLocationException {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 250; times++) for (int line = 0; line < lines; line++) fDocument.getLineLength(line);
        meter.stop();
    }
    ...
    public void measureLineDelimiterByIndex(PerformanceMeter meter) throws BadLocationException {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 250; times++) for (int line = 0; line < lines; line++) fDocument.getLineDelimiter(line);
        meter.stop();
    }
    ...
    public void measureGetNumberOfLines(PerformanceMeter meter) {
        ...
        int lines = fDocument.getNumberOfLines();
        meter.start();
        for (int times = 0; times < 250; times++) for (int line = 0; line < lines; line++) fDocument.getNumberOfLines();
        meter.stop();
    }
}
