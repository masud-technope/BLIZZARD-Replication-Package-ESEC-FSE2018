...
public class GenerateToStringAction extends GenerateMethodAbstractAction {
    ...
    boolean generateCandidates() throws JavaModelException {
        ...
        for (int i = 0; i < candidateMethods.length; i++) {
            if (!Modifier.isStatic(candidateMethods[i].getModifiers()) && candidateMethods[i].getParameterTypes().length == 0 && !candidateMethods[i].getReturnType().getName().equals("void") && !candidateMethods[i].getName().equals("toString") && //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            !candidateMethods[i].getName().equals("clone")) {
        ...
        while ((typeBinding = typeBinding.getSuperclass()) != null) {
            ...
            for (int i = 0; i < candidateFields.length; i++) {
                if (!Modifier.isPrivate(candidateFields[i].getModifiers()) && !Modifier.isStatic(candidateFields[i].getModifiers()) && !contains(fFields, candidateFields[i]) && !contains(fInheritedFields, candidateFields[i])) {
                    fieldsToBindings.put(candidateFields[i].getJavaElement(), candidateFields[i]);
                }
            }
            ...
            for (int i = 0; i < candidateMethods.length; i++) {
                if (!Modifier.isPrivate(candidateMethods[i].getModifiers()) && !Modifier.isStatic(candidateMethods[i].getModifiers()) && candidateMethods[i].getParameterTypes().length == 0 && !candidateMethods[i].getReturnType().getName().equals("void") && !contains(fMethods, candidateMethods[i]) && !contains(fInheritedMethods, candidateMethods[i]) && //$NON-NLS-1$ //$NON-NLS-2$
                !candidateMethods[i].getName().equals("clone")) {
                    methodsToBindings.put(candidateMethods[i].getJavaElement(), candidateMethods[i]);
                }
            }
