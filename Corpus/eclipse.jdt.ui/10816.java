...
public class LineTrackerPerformanceTest extends TextPerformanceTestCase {
    ...
    public void testSet() throws Exception {
        ...
        for (int run = 0; run < runs; run++) measureSet(meter);
    ...
    public void testReplace() throws Exception {
        ...
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureReplace(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureReplace(meter);
        commitAllMeasurements();
    ...
    private void measureReplace(PerformanceMeter meter) throws BadLocationException {
        meter.start();
        for (int times = 0; times < 15000; times++) fLineTracker.replace(12, 500, FAUST500);
        meter.stop();
    }
    ...
    public void testRandomReplace() throws Exception {
        ...
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureRandomReplace(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureRandomReplace(meter);
        commitAllMeasurements();
    ...
    private void measureRandomReplace(PerformanceMeter meter) throws BadLocationException {
        ...
        for (int i = 0; i < indices.length; i++) indices[i] = rand.nextInt(FAUST1.length() - 100);
    ...
    public void testLineByOffset() throws Exception {
        fLineTracker.set(FAUST1);
        // trigger replacement with TreeLineTracker implementation
        fLineTracker.replace(0, 0, "");
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureLineByOffset(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureLineByOffset(meter);
        commitAllMeasurements();
    ...
    private void measureLineByOffset(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 30; times++) for (int offset = 0; offset <= chars; offset++) fLineTracker.getLineNumberOfOffset(offset);
        meter.stop();
    }
    ...
    public void testLineByOffset2() throws Exception {
        ...
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureLineByOffset2(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureLineByOffset2(meter);
        commitAllMeasurements();
    ...
    public void testLineByIndex() throws Exception {
        ...
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureLineByIndex(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureLineByIndex(meter);
        commitAllMeasurements();
    ...
    private void measureLineByIndex(PerformanceMeter meter) throws BadLocationException {
        ...
        meter.start();
        for (int times = 0; times < 1000; times++) for (int line = 0; line < lines; line++) fLineTracker.getLineOffset(line);
        meter.stop();
    }
    ...
    public void testLineByIndex2() throws Exception {
        ...
        PerformanceMeter meter = getNullPerformanceMeter();
        int runs = getWarmUpRuns();
        for (int run = 0; run < runs; run++) measureLineByIndex2(meter);
        meter = createPerformanceMeter();
        runs = getMeasuredRuns();
        for (int run = 0; run < runs; run++) measureLineByIndex2(meter);
        commitAllMeasurements();
