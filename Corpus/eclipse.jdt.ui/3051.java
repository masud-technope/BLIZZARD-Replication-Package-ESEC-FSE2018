...
public class JavadocContentAccess2 {
    ...
    private abstract static class InheritDocVisitor {
        ...
        public static final Object STOP_BRANCH = new Object() {
            ...
            @Override
            public String toString() //$NON-NLS-1$
            {
                return "STOP_BRANCH";
            }
        };
        ...
        public Object visitInheritDoc(IType currentType, ITypeHierarchy typeHierarchy) throws JavaModelException {
            ...
            if (currentType.isInterface())
                ...
                currentType.getJavaProject().findType(//$NON-NLS-1$
                "java.lang.Object");
            else
    ...
    public static String getHTMLContent(IJavaElement element, boolean useAttachedJavadoc) throws CoreException {
        ...
        if (//$NON-NLS-1$
        sourceJavadoc == null || sourceJavadoc.length() == 0 || sourceJavadoc.trim().equals("{@inheritDoc}")) {
            if (useAttachedJavadoc) {
                if (// only if no source available
                element.getOpenable().getBuffer() == null) {
    ...
    private static String getHTMLContentFromSource(IJavaElement element) throws JavaModelException {
        ...
        if (javadocRange == null) {
            if (canInheritJavadoc(member)) {
                ...
                String inheritedJavadoc = javadoc2HTML(//$NON-NLS-1$
                member, //$NON-NLS-1$
                element, //$NON-NLS-1$
                "/***/");
    ...
    private static String getJavaFxPropertyDoc(IMember member) throws JavaModelException {
        ...
        if (member instanceof IMethod) {
            ...
            if (isGetter || isBooleanGetter || isSetter) {
                ...
                IMethod method = //$NON-NLS-1$
                type.getMethod(//$NON-NLS-1$
                propertyName + "Property", //$NON-NLS-1$
                new String[0]);
                if (method.exists()) {
    ...
    private static CompilationUnit createAST(IJavaElement element, String cuSource) {
        ...
        // workaround for https://bugs.eclipse.org/bugs/show_bug.cgi?id=212207
        ...
        return (String) new InheritDocVisitor() {
            ...
            public Object visit(IType currType) throws JavaModelException {
                ...
                if (overridden == null)
                    return InheritDocVisitor.CONTINUE;
                if (// only if no source available
                overridden.getOpenable().getBuffer() == null) {
                    ...
                    if (attachedJavadoc != null) {
                        ...
                        if (baseURL != null) {
                            attachedJavadoc = //$NON-NLS-1$
                            BASE_URL_COMMENT_INTRO + baseURL + "\"--> " + //$NON-NLS-1$
                            attachedJavadoc;
                        }
                    ...
                    if (first instanceof SimpleName) {
                        ...
                        firstText)) {
                            ...
                            if (second instanceof SimpleName && third instanceof TextElement) {
                                ...
                                if (//$NON-NLS-1$
                                elementName.equals(name) && //$NON-NLS-1$
                                ">".equals(thirdText)) {
    ...
    private void elementToHTML() {
        ...
        if (hasParameters || hasTypeParameters || hasReturnTag || hasExceptions || versions.size() > 0 || authors.size() > 0 || since.size() > 0 || sees.size() > 0 || rest.size() > 0 || (fBuf.length() > 0 && (parameterDescriptions.length > 0 || exceptionDescriptions.length > 0))) {
    ...
    private void handleDeprecatedTag(TagElement tag) {
        ...
        //$NON-NLS-1$ TODO: Why not </p>? See https://bugs.eclipse.org/bugs/show_bug.cgi?id=243318 .
        fBuf.append("</i><p>");
    }
                ...
                if (TagElement.TAG_PARAM.equals(tagName)) {
                    ...
                    if (fragments.size() > 2) {
                        ...
                        if (first instanceof TextElement && second instanceof SimpleName && third instanceof TextElement) {
                            ...
                            "<".equals(firstText) && ">".equals(thirdText)) {
                                String name = ((SimpleName) second).getIdentifier();
        ...
        for (Iterator<? extends ASTNode> iter = nodes.iterator(); iter.hasNext(); ) {
            ...
            if (previousNode != null) {
                ...
                if (previousEnd > childStart) {
                    ...
                    "Illegal ASTNode positions: previousEnd=" + //$NON-NLS-1$
                    previousEnd + ", childStart=" + childStart + ", element=" + //$NON-NLS-1$
    ...
    private String removeDocLineIntros(String textWithStars) {
        ...
        String noBreakSpace = "[^\r\n&&\\s]";
        //$NON-NLS-1$ //$NON-NLS-2$
        return textWithStars.replaceAll(lineBreakGroup + noBreakSpace + "*\\*", /*+ noBreakSpace + '?'*/
        "$1");
    }
    ...
    private void handleLink(List<? extends ASTNode> fragments) {
        ...
        if (fs > 0) {
            ...
            if (first instanceof Name) {
                ...
                refTypeName = qualifier == null ? "" : //$NON-NLS-1$
        ...
        if (sourceAttachmentPath != null) {
            ...
            if (file.isDirectory()) {
                ...
                try {
                    ...
                    try {
                        if (zipFile != null) {
                            //this will close the InputStream also
                            zipFile.close();
                        }
                    } catch (IOException e) {
                    }
