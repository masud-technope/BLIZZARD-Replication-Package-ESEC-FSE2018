...
public final class TypeVariableUtil {
    ...
    private static void extractTypeVariables(final String signature, final Set<String> variables) {
        ...
        if (arguments.length == 0) {
            ...
            for (int index = 0; index < arguments.length; index++) variables.add(Signature.toString(arguments[index]));
        }
    }
    ...
    private static String[] getReferencedVariables(final IType declaring, final IMember member) throws JavaModelException {
        ...
        if (member instanceof IField) {
            ...
            for (int index = 0; index < types.length; index++) extractTypeVariables(types[index], set);
        ...
        for (int index = 0; index < variables.length; index++) {
            ...
            for (int offset = 0; offset < result.length; offset++) if (variable.equals(result[offset]))
