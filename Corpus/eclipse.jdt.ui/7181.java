...
public class NLSStringHover extends AbstractJavaEditorTextHover {
    ...
    private NLSHoverControlInput internalGetHoverInfo(ITextViewer textViewer, IRegion hoverRegion) {
        ...
        if (node instanceof StringLiteral) {
            ...
            try {
                ...
                if (!//$NON-NLS-1$
                Signature.getSignatureSimpleName(field.getTypeSignature()).equals(//$NON-NLS-1$
                "String"))
                    return null;
        ...
        try {
            ...
            if (propertiesFile == null)
                //$NON-NLS-1$ //$NON-NLS-2$
                return new NLSHoverControlInput(toHtml(JavaHoverMessages.NLSStringHover_NLSStringHover_PropertiesFileNotDetectedWarning, "", null, false), (IStorage) null, "", getEditor());
        ...
        try {
            ...
            return new NLSHoverControlInput(toHtml(propertiesFileName, JavaHoverMessages.NLSStringHover_NLSStringHover_PropertiesFileCouldNotBeReadWarning, e.getLocalizedMessage(), false), propertiesFile, identifier, getEditor());
        }
        if (properties == null)
            return null;
        if (properties.isEmpty())
            //$NON-NLS-1$
            return new NLSHoverControlInput(toHtml(propertiesFileName, JavaHoverMessages.NLSStringHover_NLSStringHover_missingKeyWarning, null, false), propertiesFile, "", getEditor());
    ...
    private String toHtml(String header, String string, String errorString, boolean addPreFormatted) {
        ...
        if (string != null) {
            if (addPreFormatted) {
                //$NON-NLS-1$
                HTMLPrinter.addParagraph(//$NON-NLS-1$
                buffer, //$NON-NLS-1$
                "");
