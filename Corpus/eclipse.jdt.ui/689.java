...
public class PropertiesStructureCreator extends StructureCreator {
    ...
    private void parsePropertyFile(DocumentRangeNode root, IDocument doc, IProgressMonitor monitor) {
        ...
        for (; ; ) {
            ...
            if (firstChar == '#' || firstChar == '!') {
                if (// comment belongs to next key/value pair
                start < 0)
                    start = lineStart;
                // comment
                continue;
            ...
            while (needNextLine(line)) {
                ...
                int startPos = 0;
                for (; startPos < nextLine.length(); startPos++) if (WHITESPACE.indexOf(nextLine.charAt(startPos)) == -1)
                    break;
            ...
            int valuePos;
            for (valuePos = separatorPos; valuePos < len; valuePos++) if (WHITESPACE.indexOf(line.charAt(valuePos)) == -1)
                break;
            if (valuePos < len)
    ...
    private boolean needNextLine(String line) {
        ...
        while ((ix >= 0) && (line.charAt(ix--) == '\\')) slashes++;
        return slashes % 2 == 1;
    }
