...
public class SemanticHighlightings {
    ...
    private static final class EnumHighlighting extends SemanticHighlighting {
        ...
        public boolean consumes(SemanticToken token) {
            ...
            ASTNode node = name.getParent();
            int nodeType = node.getNodeType();
            if (nodeType != ASTNode.METHOD_INVOCATION && nodeType != ASTNode.SIMPLE_TYPE && nodeType != ASTNode.QUALIFIED_TYPE && nodeType != ASTNode.QUALIFIED_NAME && nodeType != ASTNode.QUALIFIED_NAME && nodeType != ASTNode.ENUM_DECLARATION)
                return false;
            while (nodeType == ASTNode.QUALIFIED_NAME) {
                node = node.getParent();
                nodeType = node.getNodeType();
    ...
    private static final class AnnotationHighlighting extends SemanticHighlighting {
        ...
        @Override
        public boolean isEnabledByDefault() {
            // as it replaces the syntax based highlighting which is always enabled
            return true;
        }
        ...
        public boolean consumes(SemanticToken token) {
            ...
            ASTNode node = name.getParent();
            int nodeType = node.getNodeType();
            if (nodeType != ASTNode.SIMPLE_TYPE && nodeType != ASTNode.QUALIFIED_TYPE && nodeType != ASTNode.QUALIFIED_NAME && nodeType != ASTNode.ANNOTATION_TYPE_DECLARATION && nodeType != ASTNode.MARKER_ANNOTATION && nodeType != ASTNode.NORMAL_ANNOTATION && nodeType != ASTNode.SINGLE_MEMBER_ANNOTATION)
                return false;
            while (nodeType == ASTNode.QUALIFIED_NAME) {
                node = node.getParent();
                nodeType = node.getNodeType();
    ...
    public static boolean affectsEnablement(IPreferenceStore store, PropertyChangeEvent event) {
        ...
        for (int i = 0; i < highlightings.length; i++) {
            ...
            if (store.getBoolean(key))
                // another is still enabled or was enabled before
                return false;
        }
    ...
    private static void convertMethodHighlightingPreferences(IPreferenceStore store) {
        ...
        if (conditionalReset(store, oldColorkey, colorkey) || conditionalReset(store, oldBoldkey, boldkey) || conditionalReset(store, oldItalickey, italickey)) {
    ...
    private static void convertAnnotationHighlightingPreferences(IPreferenceStore store) {
        ...
        if (conditionalReset(store, oldColorkey, colorkey) || conditionalReset(store, oldBoldkey, boldkey) || conditionalReset(store, oldItalickey, italickey) || conditionalReset(store, oldStrikethroughKey, strikethroughKey) || conditionalReset(store, oldUnderlineKey, underlineKey)) {
            store.setValue(enabledkey, true);
        }
    }
