...
public class CoreTestSearchEngine {
    ...
    public static void findTestImplementorClasses(ITypeHierarchy typeHierarchy, IType testInterface, IRegion region, Set<IType> result) throws JavaModelException {
        ...
        for (int i = 0; i < subtypes.length; i++) {
            ...
            if (// do the cheaper tests first
            !Flags.isInterface(cachedFlags) && !Flags.isAbstract(cachedFlags) && region.contains(type) && CoreTestSearchEngine.isAccessibleClass(type)) {
                result.add(type);
            }
        }
    }
    ...
    public static void findSuiteMethods(IJavaElement element, Set<IType> result, IProgressMonitor pm) throws CoreException {
        ...
        //$NON-NLS-1$
        SearchPattern suitePattern = SearchPattern.createPattern("suite() Test", IJavaSearchConstants.METHOD, IJavaSearchConstants.DECLARATIONS, matchRule);
