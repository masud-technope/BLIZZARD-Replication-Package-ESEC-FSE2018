    ...
    private static class MoveMembersInputPage extends UserInputWizardPage {
        ...
        public void setVisible(boolean visible) {
            if (visible) {
                ...
                String message = membersToMoveCount == 1 ? Messages.format(RefactoringMessages.MoveMembersInputPage_descriptionKey_singular, new String[] { JavaElementLabels.getTextLabel(membersToMove[0], JavaElementLabels.ALL_FULLY_QUALIFIED), label }) : Messages.format(RefactoringMessages.MoveMembersInputPage_descriptionKey_plural, new String[] { new Integer(membersToMoveCount).toString(), label });
        ...
        private void addLabel(Composite parent) {
            ...
            if (members.length == 1) {
                label.setText(Messages.format(RefactoringMessages.MoveMembersInputPage_destination_single, JavaElementLabels.getElementLabel(members[0], LABEL_FLAGS)));
            } else {
        ...
        private String createInitialFilter() {
            if (//$NON-NLS-1$
            !fDestinationField.getText().trim().equals(""))
