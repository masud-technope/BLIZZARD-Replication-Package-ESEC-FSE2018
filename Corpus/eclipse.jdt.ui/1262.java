...
public class CleanUpRefactoring extends Refactoring implements IScheduledRefactoring {
    ...
    public static class CleanUpChange extends CompilationUnitChange {
        ...
        public Change perform(final IProgressMonitor pm) throws CoreException {
            if (Display.getCurrent() == null) {
                ...
                if (exs[0] != null) {
                    IStatus status = new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IJavaStatusConstants.INTERNAL_ERROR, Messages.format(FixMessages.CleanUpRefactoring_exception, getCompilationUnit().getResource().getFullPath().toString()), exs[0]);
    ...
    private class CleanUpFixpointIterator {
        ...
        // map from primary to working copy
    ...
    public RefactoringStatus checkFinalConditions(IProgressMonitor pm) throws CoreException, OperationCanceledException {
        ...
        //$NON-NLS-1$
        pm.beginTask("", cuCount * 2 * fCleanUps.size() + 4 * cleanUps.length);
        try {
    ...
    private RefactoringStatus setOptionsFromProfile(IJavaProject javaProject, ICleanUp[] cleanUps) {
        ...
        for (int i = 0; i < cleanUps.length; i++) cleanUps[i].setOptions(cleanUpOptions);
        return new RefactoringStatus();
    }
