...
public abstract class JavaEditor extends AbstractDecoratedTextEditor implements IViewPartInputProvider {
    ...
    protected abstract class NextSubWordAction extends TextNavigationAction {
        ...
        public void run() {
            ...
            try {
                ...
                if (isBlockSelectionModeEnabled() && document.getLineOfOffset(next) != document.getLineOfOffset(position)) {
                    // may navigate into virtual white space
                    super.run();
        ...
        protected int findNextPosition(int position) {
            ...
            int next = position;
            while (// XXX: optimize
            next != BreakIterator.DONE && widget == -1) {
    ...
    protected class DeleteNextSubWordAction extends NextSubWordAction implements IUpdate {
        ...
        protected void setCaretPosition(final int position) {
            ...
            if (isBlockSelectionModeEnabled() && widgetSelection.y != widgetSelection.x) {
                ...
                try {
                    //$NON-NLS-1$
                    viewer.getDocument().replace(//$NON-NLS-1$
                    caret, //$NON-NLS-1$
                    length, //$NON-NLS-1$
                    "");
                } catch (BadLocationException exception) {
    ...
    protected abstract class PreviousSubWordAction extends TextNavigationAction {
        ...
        protected int findPreviousPosition(int position) {
            ...
            while (// XXX: optimize
            previous != BreakIterator.DONE && widget == -1) {
                    ...
                    if (element != null && element.exists()) {
                        try {
                            ...
                            if (kind == IJavaElement.TYPE || kind == IJavaElement.METHOD || kind == IJavaElement.INITIALIZER) {
                                ...
                                final boolean seletionInNameRange = nameRange != null && selection.x >= nameRange.getOffset() && selection.x + selection.y <= nameRange.getOffset() + nameRange.getLength();
	 ...
	 * @since 3.8
	 */
    protected static final String HIGHLIGHT_BRACKET_AT_CARET_LOCATION = PreferenceConstants.EDITOR_HIGHLIGHT_BRACKET_AT_CARET_LOCATION;
	 ...
	 * @since 3.8
	 */
    protected static final String ENCLOSING_BRACKETS = PreferenceConstants.EDITOR_ENCLOSING_BRACKETS;
	 ...
	 */
    protected abstract IJavaElement getElementAt(int offset);
	 ...
	 */
    protected abstract IJavaElement getCorrespondingElement(IJavaElement element);
    ...
    protected final ISourceViewer createSourceViewer(Composite parent, IVerticalRuler verticalRuler, int styles) {
        ...
        if (sourceViewer instanceof ProjectionViewer) {
            ...
            if (markerAnnotationPreferences != null) {
                ...
                //$NON-NLS-1$
                fProjectionSupport.addSummarizableAnnotationType(//$NON-NLS-1$
    ...
    String getBreadcrumbPreferenceKey() {
        ...
        if (perspective == null)
            return null;
        //$NON-NLS-1$
        return JavaEditor.EDITOR_SHOW_BREADCRUMB + "." + perspective.getId();
    }
    ...
    private void showBreadcrumb() {
        ...
        if (fBreadcrumbComposite.isDisposed()) {
            // not expected
            SWT.error(SWT.ERROR_WIDGET_DISPOSED, null, ". Editor not properly disposed; please report scenario to <https://bugs.eclipse.org/446203>. Editor input: " + getInputJavaElement());
        }
    ...
    protected void doSelectionChanged(ISelection selection) {
        ...
        CompilationUnit ast;
        try {
            ast = SharedASTProvider.getAST(inputJavaElement, SharedASTProvider.WAIT_NO, /* DO NOT USE WAIT_ACTIVE_ONLY */
            monitor);
        } finally {
            monitor.done();
        }
    ...
    protected void createActions() {
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "ShowOutline.", this, JavaSourceViewer.SHOW_OUTLINE, true);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "OpenStructure.", this, JavaSourceViewer.OPEN_STRUCTURE, true);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "OpenHierarchy.", this, JavaSourceViewer.SHOW_HIERARCHY, true);
        ...
        //$NON-NLS-1$
        setAction("RemoveOccurrenceAnnotations", action);
        // add annotation actions for roll-over expand hover
        //$NON-NLS-1$
        action = new JavaSelectMarkerRulerAction2(JavaEditorMessages.getBundleForConstructedKeys(), "Editor.RulerAnnotationSelection.", this);
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        action = new ClipboardOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Editor.Cut.", this, ITextOperationTarget.CUT);
        setAction(ITextEditorActionConstants.CUT, action);
        //$NON-NLS-1$
        action = new ClipboardOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Editor.Copy.", this, ITextOperationTarget.COPY);
        setAction(ITextEditorActionConstants.COPY, action);
        //$NON-NLS-1$
        action = new ClipboardOperationAction(JavaEditorMessages.getBundleForConstructedKeys(), "Editor.Paste.", this, ITextOperationTarget.PASTE);
    ...
    private void createDeprecatedShowInPackageExplorerAction() {
        ...
        //$NON-NLS-1$
        setAction("ShowInPackageView", action);
    }
    ...
    protected void setActionsActivated(boolean state) {
        ...
        try {
            //$NON-NLS-1$
            method = AbstractTextEditor.class.getDeclaredMethod("setActionActivation", new Class[] { boolean.class });
    ...
    public int getOrientation() {
        //Java editors are always left to right by default
    ...
    protected void configureSourceViewerDecorationSupport(SourceViewerDecorationSupport support) {
        ...
        support.setMarginPainterPreferenceKeys(AbstractDecoratedTextEditorPreferenceConstants.EDITOR_PRINT_MARGIN, AbstractDecoratedTextEditorPreferenceConstants.EDITOR_PRINT_MARGIN_COLOR, DefaultCodeFormatterConstants.FORMATTER_LINE_SPLIT);
    }
    ...
    class OccurrencesFinderJob extends Job {
        ...
        private boolean isCanceled(IProgressMonitor progressMonitor) {
            return fCanceled || progressMonitor.isCanceled() || fPostSelectionValidator != null && !(fPostSelectionValidator.isValid(fSelection) || fForcedMarkOccurrencesSelection == fSelection) || LinkedModeModel.hasInstalledModel(fDocument);
        }
    ...
    protected void updateOccurrenceAnnotations(ITextSelection selection, CompilationUnit astRoot) {
        ...
        if (locations == null) {
            if (!fStickyOccurrenceAnnotations)
                removeOccurrenceAnnotations();
            else if (// check consistency of current annotations
            hasChanged)
                removeOccurrenceAnnotations();
            return;
        }
    ...
    void removeOccurrenceAnnotations() {
        ...
        synchronized (getLockObject(annotationModel)) {
            if (annotationModel instanceof IAnnotationModelExtension) {
                ...
                for (int i = 0, length = fOccurrenceAnnotations.length; i < length; i++) annotationModel.removeAnnotation(fOccurrenceAnnotations[i]);
            }
    ...
    protected void installOverrideIndicator(boolean provideAST) {
        ...
        if (provideAST) {
            ...
            try {
                ...
                CompilationUnit ast = SharedASTProvider.getAST(inputElement, SharedASTProvider.WAIT_ACTIVE_ONLY, subMonitor.newChild(1, SubMonitor.SUPPRESS_NONE));
    ...
    protected Annotation findAnnotation(final int offset, final int length, boolean forward, Position annotationPosition) {
        ...
        while (e.hasNext()) {
            ...
            forward && p.offset == offset || !forward && p.offset + p.getLength() == offset + length) {
