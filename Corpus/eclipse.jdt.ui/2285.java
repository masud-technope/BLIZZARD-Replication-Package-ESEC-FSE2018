...
public class RefactoringWizardDialog2 extends TrayDialog implements IWizardContainer {
    ...
    private Map<String, Object> saveUIState(boolean keepCancelEnabled) {
        ...
        //$NON-NLS-1$
        saveEnableStateAndSet(getButton(PREVIEW_ID), savedState, "preview", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(getButton(IDialogConstants.OK_ID), savedState, "ok", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(getButton(IDialogConstants.BACK_ID), savedState, "back", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(getButton(IDialogConstants.NEXT_ID), savedState, "next", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(getButton(IDialogConstants.CANCEL_ID), savedState, "cancel", keepCancelEnabled);
        //$NON-NLS-1$
        savedState.put("page", ControlEnableState.disable(fVisiblePage.getControl()));
        return savedState;
    }
    ...
    private void setDisplayCursor(Display d, Cursor c) {
        ...
        for (int i = 0; i < shells.length; i++) shells[i].setCursor(c);
    }
    ...
    private void stopped(Map<String, Object> savedState) {
        ...
        if (shell != null) {
            ...
            //$NON-NLS-1$
            Control focusControl = (Control) savedState.get("focus");
            if (focusControl != null)
    ...
    private void restoreUIState(Map<String, Object> state) {
        //$NON-NLS-1$
        restoreEnableState(getButton(PREVIEW_ID), state, "preview");
        //$NON-NLS-1$
        restoreEnableState(getButton(IDialogConstants.OK_ID), state, "ok");
        //$NON-NLS-1$
        restoreEnableState(getButton(IDialogConstants.BACK_ID), state, "back");
        //$NON-NLS-1$
        restoreEnableState(getButton(IDialogConstants.NEXT_ID), state, "next");
        //$NON-NLS-1$
        restoreEnableState(getButton(IDialogConstants.CANCEL_ID), state, "cancel");
        //$NON-NLS-1$
    ...
    private boolean showErrorDialog(ErrorWizardPage page) {
        RefactoringStatusDialog dialog = new RefactoringStatusDialog(getShell(), page, fWizard.internalShowBackButtonOnStatusDialog(InternalAPI.INSTANCE));
        switch(dialog.open()) {
    ...
    private Composite createPreviousAndNextButtons(Composite parent) {
        ...
        // parent is assumed to have a GridLayout (see javadoc of Dialog#createButton(..))
