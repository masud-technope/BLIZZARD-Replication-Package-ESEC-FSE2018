...
public class MethodExitsFinder extends ASTVisitor implements IOccurrencesFinder {
    ...
    private void markReferences() {
        ...
        if (block != null) {
            ...
            // closing bracket
            int offset = fMethodDeclaration.getStartPosition() + fMethodDeclaration.getLength() - 1;
        ...
        for (VariableDeclarationExpression variable : resources) {
            ...
            if (methodBinding != null) {
                ...
                for (int j = 0; j < exceptionTypes.length; j++) {
                    if (// a close() throws an uncaught exception
                    isExitPoint(exceptionTypes[j])) {
                        ...
                        if (!exitMarked) {
                            ...
                            fResult.add(new OccurrenceLocation(offset, 1, 0, Messages.format(SearchMessages.MethodExitsFinder_occurrence_exit_impclict_close_description, BasicElementLabels.getJavaElementName(fMethodDeclaration.getName().toString()))));
                        }
                    }
                }
            }
