...
public abstract class TextStorePerformanceTest extends TextPerformanceTestCase2 {
    ...
    public void measureSet(PerformanceMeter meter) {
        meter.start();
        for (int accumulated = 0; accumulated < 400; accumulated++) fTextStore.set(FAUST1);
        meter.stop();
    }
    ...
    public void measureRepeatedReplace(PerformanceMeter meter) {
        ...
        meter.start();
        for (int times = 0; times < 200000; times++) fTextStore.replace(OFFSET, REPLACE_LENGTH, FAUST100);
        meter.stop();
    }
    ...
    public void measureTypingReplaceInLargeFile(PerformanceMeter meter) {
        ...
        int offset = 12;
        meter.start();
        for (int times = 0; times < 9000000; times++) while (offset++ < 400) fTextStore.replace(offset, 0, ";");
        meter.stop();
    }
    ...
    public void measureInsertAtStart(PerformanceMeter meter) {
        fTextStore.set(FAUST1);
        meter.start();
        for (int times = 0; times < 1000; times++) fTextStore.replace(0, 0, FAUST100);
        meter.stop();
    }
    ...
    public void measureGetChar(PerformanceMeter meter) {
        ...
        meter.start();
        for (int times = 0; times < 150; times++) for (int offset = 0; offset < length; offset++) fTextStore.get(offset);
        meter.stop();
    }
    ...
    public void measureGetAll(PerformanceMeter meter) {
        ...
        int length = fTextStore.getLength();
        meter.start();
        for (int times = 0; times < 500; times++) fTextStore.get(0, length);
        meter.stop();
    }
}
