...
public class ReorgCorrectionsSubProcessor {
    ...
    private static final class ChangeToRequiredCompilerCompliance extends ChangeCorrectionProposal implements IWorkspaceRunnable {
                ...
                for (int k = 0; k < installs.length; k++) {
                    ...
                    if (fRequiredVersion.equals(vmInstallCompliance)) {
                        ...
                        if (JavaModelUtil.isVersionLessThan(bestMatchingCompliance, vmInstallCompliance)) {
                            // the other one is the least matching
                            continue;
                        }
                    }
        ...
        private boolean updateJRE(IProgressMonitor monitor) throws CoreException, JavaModelException {
            ...
            try {
                if (fChangeOnWorkspace) {
                    ...
                    if (vmInstall != null) {
                        IVMInstall install = // can be null
                        JavaRuntime.getVMInstall(// can be null
                        fProject);
                        monitor.beginTask(CorrectionMessages.ReorgCorrectionsSubProcessor_50_compliance_operation, 4);
                        IVMInstall defaultVM = // can be null
        ...
        private IExecutionEnvironment findBestMatchingEE() {
            ...
            for (int i = 0; i < ees.length; i++) {
                ...
                } else // possible match
                {
                    if (bestEE != null) {
                        if (!//$NON-NLS-1$
                        eeId.startsWith(//$NON-NLS-1$
                        "J")) {
                            // avoid taking e.g. OSGi profile if a Java profile is available
                            continue;
                        }
        ...
        public Object getAdditionalProposalInfo(IProgressMonitor monitor) {
            ...
            try {
                // can be null
                IVMInstall install = JavaRuntime.getVMInstall(fProject);
                if (fChangeOnWorkspace) {
                    IVMInstall vmInstall = findRequiredOrGreaterVMInstall();
                    if (vmInstall != null) {
                        IVMInstall defaultVM = // can be null
        ...
        public void apply(IDocument document) {
            ...
            if (!fRequiredJREFound) {
                MessageDialog.openInformation(JavaPlugin.getActiveWorkbenchShell(), Messages.format(CorrectionMessages.ReorgCorrectionsSubProcessor_no_required_jre_title, fRequiredVersion), Messages.format(CorrectionMessages.ReorgCorrectionsSubProcessor_no_required_jre_message, fRequiredVersion));
            }
        }
    }
