...
public class EditTemplateDialog extends StatusDialog {
    ...
    public  EditTemplateDialog(Shell parent, Template template, boolean edit, boolean isNameModifiable, ContextTypeRegistry registry) {
        ...
        for (Iterator<TemplateContextType> it = registry.contextTypes(); it.hasNext(); ) {
            ...
            if (//$NON-NLS-1$
            type.getId().equals("javadoc"))
                //$NON-NLS-1$
                contexts.add(//$NON-NLS-1$
                new String[] { type.getId(), type.getName(), "/**" + delim });
            else
                //$NON-NLS-1$
                contexts.add(//$NON-NLS-1$
                0, //$NON-NLS-1$
                new String[] { type.getId(), type.getName(), "" });
        }
    ...
    private void fillContextMenu(IMenuManager menu) {
        ...
        //$NON-NLS-1$
        menu.appendToGroup(IContextMenuConstants.GROUP_GENERATE, fGlobalActions.get("ContentAssistProposal"));
    }

    protected void updateSelectionDependentActions() {
        Iterator<String> iterator = fSelectionActions.iterator();
        while (iterator.hasNext()) updateAction(iterator.next());
    }
