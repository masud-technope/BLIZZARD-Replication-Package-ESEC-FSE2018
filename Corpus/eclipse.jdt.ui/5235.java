...
public class JavaSelectAnnotationRulerAction extends SelectMarkerRulerAction {
    ...
    public void update() {
        findJavaAnnotation();
        // super.update() might change this later
        setEnabled(true);
        if (fAnnotation instanceof OverrideIndicatorManager.OverrideIndicator) {
            //$NON-NLS-1$
            initialize(fBundle, "JavaSelectAnnotationRulerAction.OpenSuperImplementation.");
            return;
        }
        if (fHasCorrection) {
            if (fAnnotation instanceof AssistAnnotation)
                ...
                fBundle, //$NON-NLS-1$
                "JavaSelectAnnotationRulerAction.QuickAssist.");
            else
                initialize(//$NON-NLS-1$
                fBundle, //$NON-NLS-1$
                "JavaSelectAnnotationRulerAction.QuickFix.");
            return;
        }
        //$NON-NLS-1$;
        initialize(fBundle, "JavaSelectAnnotationRulerAction.GotoAnnotation.");
        super.update();
    }
