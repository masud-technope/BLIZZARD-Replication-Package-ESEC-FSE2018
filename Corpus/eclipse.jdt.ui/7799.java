...
public class ExtractMethodInputPage extends UserInputWizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        if (!fRefactoring.getParameterInfos().isEmpty()) {
            ChangeParametersControl cp = new ChangeParametersControl(result, SWT.NONE, RefactoringMessages.ExtractMethodInputPage_parameters, new IParameterListChangeListener() {
    ...
    private String getLabel(ASTNode node) {
        if (node instanceof AbstractTypeDeclaration) {
            ...
            if (node.getLocationInParent() == ClassInstanceCreation.ANONYMOUS_CLASS_DECLARATION_PROPERTY) {
                ...
                return Messages.format(RefactoringMessages.ExtractMethodInputPage_anonymous_type_label, BasicElementLabels.getJavaElementName(ASTNodes.asString(creation.getType())));
