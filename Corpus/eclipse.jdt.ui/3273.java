...
public class ASTView extends ViewPart implements IShowInSource {
    ...
    public  ASTView() {
        ...
        // inverse so that default is to create bindings
        fCreateBindings = !fDialogSettings.getBoolean(SETTINGS_NO_BINDINGS);
        // inverse so that default is use recovery
        fStatementsRecovery = !fDialogSettings.getBoolean(SETTINGS_NO_STATEMENTS_RECOVERY);
        // inverse so that default is use recovery
        fBindingsRecovery = !fDialogSettings.getBoolean(SETTINGS_NO_BINDINGS_RECOVERY);
    ...
    public void setInput(ITextEditor editor) throws CoreException {
        ...
        if (editor != null) {
            ...
            if (typeRoot == null) {
                ...
                getErrorStatus("Editor not showing a CU or class file", null));
            }
            fTypeRoot = typeRoot;
    ...
    private CompilationUnit internalSetInput(ITypeRoot input, int offset, int length) throws CoreException {
        ...
        try {
            ...
            showAndLogError("Could not select node for editor selection", e);
        }
        return root;
    }
    ...
    private CompilationUnit createAST(ITypeRoot input, int offset) throws JavaModelException, CoreException {
        ...
        if ((getCurrentInputKind() == ASTInputKindAction.USE_RECONCILE)) {
            final IProblemRequestor problemRequestor = new //strange: don't get bindings when supplying null as problemRequestor
            IProblemRequestor() {
    ...
    public void dispose() {
        if (fSuperListener != null) {
            ...
            // removes reference to view
            fSuperListener.dispose();
            fSuperListener = null;
        }
    ...
    private void hookTrayContextMenu() {
        ...
        //$NON-NLS-1$
        getSite().registerContextMenu("#TrayPopupMenu", menuMgr, fTray);
    }
    ...
    private void makeActions() {
        ...
        //$NON-NLS-1$
        fClearAction.setText("&Clear AST");
        //$NON-NLS-1$
        fClearAction.setToolTipText("Clear AST and release memory");
        fClearAction.setEnabled(false);
        ASTViewImages.setImageDescriptors(fClearAction, ASTViewImages.CLEAR);
        fASTInputKindActions = new ASTInputKindAction[] { new //$NON-NLS-1$
        ASTInputKindAction(//$NON-NLS-1$
        "Use ASTParser.&createAST", //$NON-NLS-1$
        ASTInputKindAction.USE_PARSER), new //$NON-NLS-1$
        ASTInputKindAction(//$NON-NLS-1$
        "Use ASTParser with &focal position", //$NON-NLS-1$
        ASTInputKindAction.USE_FOCAL), new //$NON-NLS-1$
        ASTInputKindAction(//$NON-NLS-1$
        "Use ICompilationUnit.&reconcile", //$NON-NLS-1$
        ASTInputKindAction.USE_RECONCILE), new ASTInputKindAction("Use SharedASTProvider.&getAST", //$NON-NLS-1$
        ASTInputKindAction.USE_CACHE) };
        fCreateBindingsAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Create Bindings", //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        fCreateBindingsAction.setToolTipText("Create Bindings");
        fCreateBindingsAction.setEnabled(true);
        fStatementsRecoveryAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Statements Recovery", //$NON-NLS-1$
        IAction.AS_CHECK_BOX) {
        ...
        fBindingsRecoveryAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Bindings Recovery", //$NON-NLS-1$
        IAction.AS_CHECK_BOX) {
        ...
        fIgnoreMethodBodiesAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Ignore Method Bodies", //$NON-NLS-1$
        IAction.AS_CHECK_BOX) {
        ...
        fFilterNonRelevantAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Hide Non-Relevant Attributes", //$NON-NLS-1$
        IAction.AS_CHECK_BOX) {
        ...
        //$NON-NLS-1$
        fFilterNonRelevantAction.setToolTipText("Hide non-relevant binding attributes");
        fFilterNonRelevantAction.setEnabled(true);
        fFindDeclaringNodeAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "Find &Declaring Node...", //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        fFindDeclaringNodeAction.setToolTipText("Find Declaring Node...");
        fFindDeclaringNodeAction.setEnabled(false);
        fParseBindingFromElementAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "&Parse Binding from &Element Handle...", //$NON-NLS-1$
        IAction.AS_PUSH_BUTTON) {
        ...
        //$NON-NLS-1$
        fParseBindingFromElementAction.setToolTipText("Parse Binding from Element Handle...");
        fParseBindingFromElementAction.setEnabled(true);
        fParseBindingFromKeyAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "Parse Binding from &Key...", //$NON-NLS-1$
        IAction.AS_PUSH_BUTTON) {
        ...
        //$NON-NLS-1$
        fParseBindingFromKeyAction.setToolTipText("Parse Binding from Key...");
        ...
        //$NON-NLS-1$
        fLinkWithEditor.setText("&Link with Editor");
        //$NON-NLS-1$
        fLinkWithEditor.setToolTipText("Link With Editor");
        ...
        fASTVersionToggleActions = new ASTLevelToggle[] { new //$NON-NLS-1$
        ASTLevelToggle(//$NON-NLS-1$
        "AST Level &2 (1.2)", //$NON-NLS-1$
        JLS2), new //$NON-NLS-1$
        ASTLevelToggle(//$NON-NLS-1$
        "AST Level &3 (1.5)", //$NON-NLS-1$
        JLS3), new //$NON-NLS-1$
        ASTLevelToggle(//$NON-NLS-1$
        "AST Level &4 (1.7)", //$NON-NLS-1$
        JLS4), new //$NON-NLS-1$
        ASTLevelToggle(//$NON-NLS-1$
        "AST Level &8 (1.8)", //$NON-NLS-1$
        JLS8) };
        ...
        //$NON-NLS-1$
        fAddToTrayAction.setText("&Add to Comparison Tray");
        //$NON-NLS-1$
        fAddToTrayAction.setToolTipText("Add Selected Node to Comparison Tray");
        ...
        //$NON-NLS-1$
        fDeleteAction.setToolTipText("Delete Binding from Tray");
        fDeleteAction.setEnabled(false);
    ...
    protected void performLinkWithEditor() {
        ...
        if (fDoLinkWithEditor && fEditor != null) {
            ...
            if (// can be null when editor is closed
    ...
    private void showAndLogError(String message, CoreException e) {
        ...
        //$NON-NLS-1$
        ErrorDialog.openError(getSite().getShell(), "AST View", message, e.getStatus());
    }
    ...
    private void showAndLogError(String message, Throwable e) {
        ...
        //$NON-NLS-1$
        ErrorDialog.openError(getSite().getShell(), "AST View", null, status);
    }
    ...
    protected void performFindDeclaringNode() {
        ...
        if (node != null) {
            ...
            MessageDialog.openError(getSite().getShell(), "Find Declaring Node from Key", "The declaring node for key '" + key + "' could not be found");
        }
    }
    ...
    protected void performDoubleClick() {
        ...
        if (obj instanceof ASTNode) {
            ...
            if (javaElement instanceof IPackageFragment) {
                try {
                    ...
                    showAndLogError("Could not open Package Explorer.", e);
                }
            } else {
                try {
                    ...
                    showAndLogError("Could not open editor.", e);
        ...
        if (node != null) {
            ...
            if (nodeEnd == null) {
                ...
                length = isTripleClick ? fRoot.getExtendedStartPosition(nodeEnd) + fRoot.getExtendedLength(nodeEnd) - fRoot.getExtendedStartPosition(node) : nodeEnd.getStartPosition() + nodeEnd.getLength() - node.getStartPosition();
            }
