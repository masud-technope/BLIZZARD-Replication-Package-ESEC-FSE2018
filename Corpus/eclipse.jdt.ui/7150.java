...
public abstract class AbstractResourceMappingMerger extends ResourceMappingMerger {
    ...
    private IDiff[] getDiffs(final IMergeContext context) {
        ...
        for (int index = 0; index < mappings.length; index++) {
            ...
            for (int offset = 0; offset < diffs.length; offset++) set.add(diffs[offset]);
        }
        ...
        try {
            ...
            if (storage != null) {
                ...
                try {
                    ...
                    final RefactoringDescriptorProxy[] proxies = history.getDescriptors();
                    for (int offset = 0; offset < proxies.length; offset++) descriptors.add(proxies[offset].requestDescriptor(null));
                } catch (CoreException exception) {
                    RefactoringUIPlugin.log(exception);
                } finally {
                    if (stream != null) {
