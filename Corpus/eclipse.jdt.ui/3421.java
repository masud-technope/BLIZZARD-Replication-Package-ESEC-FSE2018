...
public abstract class RefactoringDescriptor implements Comparable<RefactoringDescriptor> {
    ...
    protected  RefactoringDescriptor(final String id, final String project, final String description, final String comment, final int flags) {
        Assert.isNotNull(id);
        //$NON-NLS-1$ //$NON-NLS-2$
        Assert.isLegal(!"".equals(id), "Refactoring id must not be empty");
        //$NON-NLS-1$ //$NON-NLS-2$
        Assert.isLegal(project == null || !"".equals(project), "Project must either be null or non-empty");
        Assert.isNotNull(description);
        //$NON-NLS-1$//$NON-NLS-2$
        Assert.isLegal(!"".equals(description), "Description must not be empty");
        //$NON-NLS-1$
        Assert.isLegal(flags >= NONE, "Flags must be non-negative");
        fRefactoringId = id;
