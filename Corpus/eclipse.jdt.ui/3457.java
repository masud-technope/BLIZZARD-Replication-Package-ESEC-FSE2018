...
public class TypeConstraintFactory implements ITypeConstraintFactory {
    ...
    public boolean filter(ConstraintVariable v1, ConstraintVariable v2, ConstraintOperator operator) {
        if ((v1.getBinding() != null && v1.getBinding().isPrimitive() && v2.getBinding() != null && v2.getBinding().isPrimitive()) || v1 == v2) {
            if (PRINT_STATS)
                fNrFiltered++;
            if (PRINT_STATS)
                dumpStats();
    ...
    protected void dumpStats() {
        ...
        System.out.println("Constraints: " + fNrCreated + " created, " + fNrRetrieved + " retrieved, " + fNrFiltered + " filtered");
    }
}
