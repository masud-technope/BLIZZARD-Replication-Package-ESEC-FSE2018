...
public class NLSScanner {
    ...
    private static NLSLine[] scan(IScanner scanner, char[] content) throws InvalidInputException, BadLocationException {
        ...
        // counting up tokens starting with 'default'
        ...
        while (token != ITerminalSymbols.TokenNameEOF) {
            switch(token) {
                ...
                case ITerminalSymbols.TokenNameIdentifier:
                    if (!insideAnnotation.isEmpty()) {
                        ...
                        if (parenCounter[0] == -1)
                            ...
                            // identifier after annotation name -> was a simple annotation
                ...
                case ITerminalSymbols.TokenNameCOLON:
                    if (defaultCounter == 1)
                        // reset in switch statement's "default :" ...
                        defaultCounter = 0;
                    else if (defaultCounter > 0)
                        // ... but not in conditional in annotation member default value
                        defaultCounter++;
                    break;
                ...
                case ITerminalSymbols.TokenNameLBRACE:
                    if (defaultCounter > 1)
                        // reset in default interface method declaration
                        defaultCounter = 0;
