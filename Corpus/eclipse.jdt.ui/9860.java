...
public class TypeSelectionDialog2 extends SelectionStatusDialog {
    ...
    private class TitleLabel implements TypeSelectionComponent.ITitleLabel {
        ...
        public void setText(String text) {
            if (text == null || text.length() == 0) {
                ...
                getShell().setText(Messages.format(JavaUIMessages.TypeSelectionDialog2_title_format, new String[] { fTitle, text }));
            }
        }
    }
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        fContent = new TypeSelectionComponent(area, SWT.NONE, getMessage(), fMultipleSelection, fScope, fElementKind, fInitialFilter, new TitleLabel(), fExtension);
    ...
    protected void handleWidgetSelected(TypeNameMatch[] selection) {
        ...
        if (selection.length == 0) {
            //$NON-NLS-1$
            status = new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, "", null);
        } else {
            if (fValidator != null) {
                ...
                for (int i = 0; i < selection.length; i++) {
                    ...
                    if (type != null) {
                        ...
                        status = new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, Messages.format(JavaUIMessages.TypeSelectionDialog_error_type_doesnot_exist, selection[i].getFullyQualifiedName()), null);
                        break;
                    }
                }
                if (status == null) {
    ...
    private void ensureConsistency() throws InvocationTargetException, InterruptedException {
        ...
        if (!runnable.needsExecution())
            return;
        IRunnableContext context = fRunnableContext != null ? fRunnableContext : PlatformUI.getWorkbench().getProgressService();
