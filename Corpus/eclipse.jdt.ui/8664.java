...
public class ChangeExceptionHandler {
    ...
    public void handle(Change change, RuntimeException exception) {
        ...
        if (exception.getMessage() == null) {
            status = new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, RefactoringMessages.ChangeExceptionHandler_status_without_detail, exception);
        } else {
            status = new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, exception.getMessage(), exception);
        }
    ...
    private void handle(Change change, IStatus status) {
        if (change instanceof CompositeChange) {
            ...
            if (undo != null) {
                ...
                final ErrorDialog dialog = new RefactorErrorDialog(fParent, change.getName(), Messages.format(RefactoringMessages.ChangeExceptionHandler_dialog_message, fName), status, IStatus.OK | IStatus.INFO | IStatus.WARNING | IStatus.ERROR);
        ...
        ErrorDialog dialog = new ErrorDialog(fParent, change.getName(), Messages.format(RefactoringMessages.ChangeExceptionHandler_dialog_message, fName), status, IStatus.OK | IStatus.INFO | IStatus.WARNING | IStatus.ERROR);
        dialog.open();
    }
    ...
    private void performUndo(final Change undo) {
        ...
        try {
            ...
            ExceptionHandler.handle(e, fParent, RefactoringMessages.ChangeExceptionHandler_undo_dialog_title, RefactoringMessages.ChangeExceptionHandler_undo_dialog_message + fName);
