...
public class TomcatClasspathProvider extends StandardClasspathProvider {
    ...
    public IRuntimeClasspathEntry[] computeUnresolvedClasspath(ILaunchConfiguration configuration) throws CoreException {
        ...
        if (useDefault) {
            ...
            // keeps track of whether a tools.jar was found	
            boolean tools = false;
            for (int i = 0; i < libs.length; i++) {
                LibraryLocation lib = libs[i];
                if (//$NON-NLS-1$
                lib.getSystemLibraryPath().toString().endsWith("tools.jar")) {
                    tools = true;
                }
            }
            if (!tools) {
                // add a tools.jar
                //$NON-NLS-1$ //$NON-NLS-2$
                IPath toolsPath = new Path(vm.getInstallLocation().getAbsolutePath()).append("lib").append("tools.jar");
