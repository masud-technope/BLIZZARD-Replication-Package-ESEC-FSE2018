...
public class TextEditChangePreviewViewer implements IChangePreviewViewer {
        ...
        try {
            ...
            if (input instanceof TextEditBasedChangeInput) {
                ...
                if (extended.group != null && extended.surroundingLines >= 0) {
                    ...
                    setInput(editChange, editChange.getCurrentContent(group.getRegion(), true, 2, new NullProgressMonitor()), editChange.getPreviewContent(new TextEditBasedChangeGroup[] { group }, group.getRegion(), true, 2, new NullProgressMonitor()), editChange.getTextType());
                    return;
                    ...
                    setInput(editChange, editChange.getCurrentContent(extended.range, true, 0, new NullProgressMonitor()), editChange.getPreviewContent(groups, extended.range, true, 0, new NullProgressMonitor()), editChange.getTextType());
                    return;
                }
    ...
    private void setInput(TextEditBasedChange change, String left, String right, String type) {
        ...
        fViewer.setInput(new DiffNode(new CompareElement(left, type, resource), new CompareElement(right, type, resource)));
    }
}
