...
public class OverrideMethodDialog extends SourceActionDialog {
    ...
    private static class OverrideMethodComparator extends ViewerComparator {
        ...
        public int compare(Viewer viewer, Object first, Object second) {
            if (first instanceof ITypeBinding && second instanceof ITypeBinding) {
                ...
                if (//$NON-NLS-1$
                right.getQualifiedName().equals(//$NON-NLS-1$
                "java.lang.Object"))
                    return -1;
    ...
    public  OverrideMethodDialog(Shell shell, CompilationUnitEditor editor, IType type, boolean isSubType) throws JavaModelException {
        ...
        if (binding != null) {
            //$NON-NLS-1$
            ITypeBinding cloneable = getSuperType(binding, "java.lang.Cloneable");
            if (cloneable != null) {
                IMethodBinding[] methods = //$NON-NLS-1$
                fUnit.getAST().resolveWellKnownType("java.lang.Object").getDeclaredMethods();
                for (int index = 0; index < methods.length; index++) {
                    IMethodBinding method = methods[index];
                    if (method.getName().equals("clone") && //$NON-NLS-1$
                    method.getParameterTypes().length == //$NON-NLS-1$
                    0)
    ...
    protected CheckboxTreeViewer createTreeViewer(Composite composite) {
        ...
        // create after tree is created
        manager.add(new OverrideFlatTreeAction());
        manager.update(true);
