...
public class CallHierarchyUI {
    ...
    public static boolean openInEditor(Object element, Shell shell, boolean activateOnOpen) {
        ...
        try {
            ...
            JavaPlugin.log(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IJavaStatusConstants.INTERNAL_ERROR, CallHierarchyMessages.CallHierarchyUI_open_in_editor_error_message, e));
            ErrorDialog.openError(shell, CallHierarchyMessages.OpenLocationAction_error_title, CallHierarchyMessages.CallHierarchyUI_open_in_editor_error_message, e.getStatus());
            return false;
            ...
            if (callLocation != null)
                ...
                name = "";
            MessageDialog.openError(shell, CallHierarchyMessages.OpenLocationAction_error_title, Messages.format(CallHierarchyMessages.CallHierarchyUI_open_in_editor_error_messageArgs, new String[] { name, x.getMessage() }));
            return false;
        }
    }
    ...
    public static CallHierarchyViewPart openView(IMember[] input, IWorkbenchWindow window) {
        if (input.length == 0) {
            MessageDialog.openInformation(window.getShell(), CallHierarchyMessages.CallHierarchyUI_selectionDialog_title, CallHierarchyMessages.CallHierarchyUI_open_operation_unavialable);
            return null;
        }
        IWorkbenchPage page = window.getActivePage();
        try {
            //find the first view which is not pinned
            CallHierarchyViewPart viewPart = getDefault().findLRUCallHierarchyViewPart(page);
            String secondaryId = null;
            if (viewPart == null) {
                if (//all the current views are pinned, open a new instance
                page.findViewReference(CallHierarchyViewPart.ID_CALL_HIERARCHY) != null)
