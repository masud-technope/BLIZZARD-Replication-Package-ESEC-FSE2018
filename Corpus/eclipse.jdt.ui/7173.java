...
public final class DefaultPhoneticDistanceAlgorithm implements IPhoneticDistanceAlgorithm {
    ...
    public final int getDistance(final String from, final String to) {
        //$NON-NLS-1$
        final char[] first = (" " + from).toCharArray();
        //$NON-NLS-1$
        final char[] second = (" " + to).toCharArray();
        final int rows = first.length;
        final int columns = second.length;
        final int[][] metric = new int[rows][columns];
        for (int column = 1; column < columns; column++) metric[0][column] = metric[0][column - 1] + COST_REMOVE;
        for (int row = 1; row < rows; row++) metric[row][0] = metric[row - 1][0] + COST_INSERT;
        char source, target;
