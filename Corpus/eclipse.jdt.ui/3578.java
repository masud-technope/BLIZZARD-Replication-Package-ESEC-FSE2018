...
public class FindBrokenNLSKeysAction extends SelectionDispatchAction {
        ...
        for (int i = 0; i < selected.length; i++) {
            try {
                if (selected[i] instanceof IJavaElement) {
                    ...
                    if ("properties".equalsIgnoreCase(//$NON-NLS-1$
    ...
    private static boolean importsOSGIUtil(ICompilationUnit unit) throws JavaModelException {
        ...
        for (int i = 0; i < imports.length; i++) {
            if (//$NON-NLS-1$
            imports[i].getElementName().startsWith("org.eclipse.osgi.util."))
                return true;
        }
        return false;
    }
    ...
    private static boolean isValueAccessor(IMethod method) throws JavaModelException {
        if (//$NON-NLS-1$
        !"getString".equals(method.getElementName()))
            return false;
    ...
    private static IField getBundleNameField(IField[] fields) {
        for (int i = 0; i < fields.length; i++) {
            if (//$NON-NLS-1$
            "BUNDLE_NAME".equals(fields[i].getElementName()))
                return fields[i];
        }
        return null;
    }
    ...
    public static IType getAccessorType(IFile file) {
        if (//$NON-NLS-1$
        !"properties".equalsIgnoreCase(file.getFileExtension()))
            return null;
        ...
        for (int i = 0; i < javaExtensions.length; i++) {
            ...
            if (//try with uppercase first char
            cuFile == null) {
