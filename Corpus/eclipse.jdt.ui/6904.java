...
public class InlineMethodTests extends AbstractSelectionTestCase {
    ...
    /* *********************** Binary Tests ******************************* */
    public // uses classes.Target#
    void testBinaryInlineSingle() throws // uses classes.Target#
    Exception {
        performTestInlineCall(fgTestSetup.getBinaryPackage(), getName(), COMPARE_WITH_OUTPUT, "binary_out");
    }

    public // inlines all classes.Target2#logMessage(..)
    void testBinaryInlineAll() throws // inlines all classes.Target2#logMessage(..)
    Exception {
        String id = getName();
        ...
        // method 0 is ctor
        IMethod logMessage = target2type.getMethods()[1];
        InlineMethodRefactoring refactoring = InlineMethodRefactoring.create(target2ClassFile, new RefactoringASTParser(ASTProvider.SHARED_AST_LEVEL).parse(target2ClassFile, true), logMessage.getNameRange().getOffset(), logMessage.getNameRange().getLength());
    ...
    public // uses classes.BinEnum
    void test_133575() throws // uses classes.BinEnum
    Exception {
        performTestInlineCall(fgTestSetup.getBinaryPackage(), getName(), COMPARE_WITH_OUTPUT, "binary_out");
    }
