...
public class RenameInformationPopup implements IWidgetTokenKeeper, IWidgetTokenKeeperExtension {
    ...
    private class PopupVisibilityManager implements IPartListener2, ControlListener, MouseListener, KeyListener, ITextListener, IViewportListener {
        ...
        public void controlMoved(ControlEvent e) {
            ...
            //only for hiding outside editor area
            updateVisibility();
        }
    ...
    private Point computePopupLocation(int snapPosition) {
        ...
        switch(snapPosition) {
            ...
            // same as SNAP_POSITION_UNDER_LEFT_FIELD
            default:
                {
                    LinkedPosition position = fRenameLinkedMode.getCurrentLinkedPosition();
                    if (position == null)
                    ...
                    ISourceViewer viewer = fEditor.getViewer();
                    ITextViewerExtension5 viewer5 = (ITextViewerExtension5) viewer;
                    int widgetOffset = viewer5.modelOffset2WidgetOffset(/* + position.length*/
                    position.offset);
                    StyledText textWidget = viewer.getTextWidget();
                    Point pos = textWidget.getLocationAtOffset(widgetOffset);
                    Point pSize = getExtent();
    ...
    private void addMoveSupport(final Shell popupShell, final Control movedControl) {
        movedControl.addMouseListener(new MouseAdapter() {
            ...
            public void mouseDown(final MouseEvent downEvent) {
                ...
                final Point[] LOCATIONS = { textWidget.toControl(computePopupLocation(SNAP_POSITION_UNDER_RIGHT_FIELD)), textWidget.toControl(computePopupLocation(SNAP_POSITION_OVER_RIGHT_FIELD)), textWidget.toControl(computePopupLocation(SNAP_POSITION_UNDER_LEFT_FIELD)), textWidget.toControl(computePopupLocation(SNAP_POSITION_OVER_LEFT_FIELD)), textWidget.toControl(computePopupLocation(SNAP_POSITION_LOWER_RIGHT)) };
                final Rectangle[] DROP_TARGETS = { Geometry.createRectangle(LOCATIONS[0], pSize), Geometry.createRectangle(LOCATIONS[1], pSize), new Rectangle(LOCATIONS[2].x, LOCATIONS[2].y + HAH, pSize.x, pSize.y), Geometry.createRectangle(LOCATIONS[3], pSize), Geometry.createRectangle(LOCATIONS[4], pSize) };
                ...
                ControlListener moveListener = new ControlAdapter() {
                    ...
                    public void controlMoved(ControlEvent moveEvent) {
                        ...
                        Point popupLoc = new Point(POPUP_SOURCE.x + mouseMoveCurrent.x - MOUSE_MOVE_SOURCE.x, POPUP_SOURCE.y + mouseMoveCurrent.y - MOUSE_MOVE_SOURCE.y);
    ...
    private int[] getPolygon(boolean border) {
        ...
        switch(fSnapPosition) {
            case SNAP_POSITION_OVER_LEFT_FIELD:
                poly = new int[] { 0, 0, e.x - b, 0, e.x - b, e.y - b, ha1, e.y - b, ha2, e.y + HAH - b, ha3, e.y - b, 0, e.y - b, 0, 0 };
                break;
            case SNAP_POSITION_UNDER_LEFT_FIELD:
                poly = new int[] { 0, HAH, ha3 + b, HAH, ha2, b, ha1 - b, HAH, e.x - b, HAH, e.x - b, e.y + HAH - b, 0, e.y + HAH - b, 0, HAH };
                break;
            default:
