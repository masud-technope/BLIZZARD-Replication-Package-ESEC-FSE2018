...
public class JavaAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
    ...
    private static boolean isNewMatch(IDocument document, int offset, int length, String partitioning) {
        ...
        try {
            ...
            //$NON-NLS-1$
            int pos = text.indexOf("new");
            while (pos != -1 && !isDefaultPartition(document, pos + offset, partitioning)) pos = //$NON-NLS-1$
            text.indexOf(//$NON-NLS-1$
            "new", //$NON-NLS-1$
            pos + 2);
            if (pos < 0)
                return false;
    ...
    private static boolean looksLikeAnonymousClassDef(IDocument document, String partitioning, JavaHeuristicScanner scanner, int position) {
        ...
        if (// 2 for borders, 3 for "new"
        previousCommaParenEqual == -1 || position < previousCommaParenEqual + 5)
            return false;
    ...
    private boolean isClosed(IDocument document, int offset, int length) {
        ...
        if (length == 0) {
            while (node != null && (relativeOffset == node.getStartPosition() || relativeOffset == node.getStartPosition() + node.getLength())) node = node.getParent();
        }
        if (node == null)
            return false;
    ...
    private void smartPaste(IDocument document, DocumentCommand command) {
        ...
        try {
            ...
            // don't format the first line per default, as it has other content before it
            int firstLine = 1;
            ...
            // don't format first line
            int first = document.computeNumberOfLines(prefix) + firstLine;
            int lines = temp.getNumberOfLines();
            int tabLength = getVisualTabLengthPreference();
            boolean changed = false;
            for (// we don't change the number of lines while adding indents
            int l = first; // we don't change the number of lines while adding indents
            l < lines; // we don't change the number of lines while adding indents
            l++) {
                IRegion r = temp.getLineInformation(l);
                int lineOffset = r.getOffset();
                int lineLength = r.getLength();
                if (// don't modify empty lines
                lineLength == 0)
                    continue;
                if (!isIndentDetected) {
                    ...
                    if (l != first && temp.get(lineOffset, lineLength).trim().length() != 0) {
                        ...
                        if (insertLength == 0) {
                            ...
                            if (firstLine == 0) {
                                ...
                                if (changed)
                                    // still need to get the leading indent of the first line
                                    break;
                            }
                            return;
                        }
    ...
    private static String getCurrentIndent(Document document, int line) throws BadLocationException {
        ...
        if (//$NON-NLS-1$
        to > from && to < endOffset - 1 && document.get(to - 1, 2).equals(" *")) {
    ...
    private void addIndent(Document document, int line, CharSequence indent, int tabLength) throws BadLocationException {
        ...
        while (newInsert < endOffset - 2 && document.get(newInsert, 2).equals(LINE_COMMENT)) newInsert += 2;
    ...
    private int getPeerPosition(IDocument document, DocumentCommand command) {
        ...
        try {
            switch(afterToken) {
                case Symbols.TokenRBRACE:
                    //$NON-NLS-1$
                    pasted.replace(//$NON-NLS-1$
                    pasted.getLength(), //$NON-NLS-1$
                    0, //$NON-NLS-1$
                    "}");
                    break;
        ...
        // paste text position (increasing from 0)
        int pPos = 0;
        // document position (decreasing from paste offset)
