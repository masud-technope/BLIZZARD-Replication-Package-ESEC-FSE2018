...
public class GetterSetterCompletionProposal extends JavaTypeCompletionProposal implements ICompletionProposalExtension4 {
    ...
    public  GetterSetterCompletionProposal(IField field, int start, int length, boolean isGetter, int relevance) throws JavaModelException {
        //$NON-NLS-1$
        super("", field.getCompilationUnit(), start, length, JavaPluginImages.get(JavaPluginImages.IMG_MISC_PUBLIC), getDisplayName(field, isGetter), relevance);
    ...
    private static StyledString getDisplayName(IField field, boolean isGetter) throws JavaModelException {
        ...
        if (isGetter) {
            //$NON-NLS-1$
            buf.append(BasicElementLabels.getJavaElementName(GetterSetterUtil.getGetterName(field, null) + "() : " + fieldTypeName));
            //$NON-NLS-1$
            ...
            //$NON-NLS-1$
            buf.append(BasicElementLabels.getJavaElementName(GetterSetterUtil.getSetterName(field, null) + '(' + fieldTypeName + ") : void"));
            //$NON-NLS-1$
            buf.append(" - ", StyledString.QUALIFIER_STYLER);
