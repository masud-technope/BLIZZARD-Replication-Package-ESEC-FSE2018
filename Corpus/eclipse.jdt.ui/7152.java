...
public class CodeStyleFix extends CompilationUnitRewriteOperationsFix {
    ...
    private static final class CodeStyleVisitor extends GenericVisitor {
        ...
        public  CodeStyleVisitor(CompilationUnit compilationUnit, boolean findUnqualifiedAccesses, boolean findUnqualifiedStaticAccesses, boolean findUnqualifiedMethodAccesses, boolean findUnqualifiedStaticMethodAccesses, List<CompilationUnitRewriteOperation> resultingCollection) {
    ...
    private static class ThisQualifierVisitor extends GenericVisitor {
        ...
        public  ThisQualifierVisitor(boolean removeFieldQualifiers, boolean removeMethodQualifiers, CompilationUnit compilationUnit, List<CompilationUnitRewriteOperation> result) {
    ...
    private static final class AddThisQualifierOperation extends CompilationUnitRewriteOperation {
        ...
        public String getDescription() {
            ...
            if (fQualifier == null) {
                //$NON-NLS-1$
                qualifierLabel = //$NON-NLS-1$
                "this";
            } else {
                qualifierLabel = //$NON-NLS-1$
                BasicElementLabels.getJavaElementName(//$NON-NLS-1$
                fQualifier + ".this");
            }
        ...
        for (int i = 0; i < problems.length; i++) {
            ...
            if (isNonStaticAccess || isIndirectStaticAccess) {
                ...
                if (nonStaticAccessInformation != null) {
                    ...
                    for (// see bug 346230
                    Iterator<CompilationUnitRewriteOperation> it = result.iterator(); // see bug 346230
                    it.hasNext(); ) {
    ...
    public static boolean isIndirectStaticAccess(IProblemLocation problem) {
        return (problem.getProblemId() == IProblem.IndirectAccessToStaticField || problem.getProblemId() == IProblem.IndirectAccessToStaticMethod);
    }

    public static boolean isNonStaticAccess(IProblemLocation problem) {
        return (problem.getProblemId() == IProblem.NonStaticAccessToStaticField || problem.getProblemId() == IProblem.NonStaticAccessToStaticMethod || problem.getProblemId() == IProblem.NonStaticOrAlienTypeReceiver);
    }
