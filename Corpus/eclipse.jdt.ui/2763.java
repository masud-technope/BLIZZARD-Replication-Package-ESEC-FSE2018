    ...
    private class ActivationAnalyzer extends ASTVisitor {
        ...
        public boolean visit(SimpleName node) {
            ...
            if (binding == null && !status.hasFatalError()) {
                ...
                if (!ASTNodes.isLabel(node)) {
                    status.addFatalError(RefactoringCoreMessages.InlineMethodRefactoring_SourceAnalyzer_declaration_has_errors, JavaStatusContext.create(fTypeRoot, fDeclaration));
                    return false;
                }
            }
            return true;
        ...
        public boolean visit(ThisExpression node) {
            if (node.getQualifier() != null) {
                status.addFatalError(RefactoringCoreMessages.InlineMethodRefactoring_SourceAnalyzer_qualified_this_expressions, JavaStatusContext.create(fTypeRoot, node));
                return false;
            }
            return true;
        }
            ...
            if (binding instanceof ITypeBinding) {
                ...
                if (vb.isField() && !isStaticallyImported(node)) {
                    ...
                    if (node == topName || node == ASTNodes.getLeftMostSimpleName(topName)) {
                        ...
                        if (location != SingleVariableDeclaration.NAME_PROPERTY && location != VariableDeclarationFragment.NAME_PROPERTY) {
                            fImplicitReceivers.add(node);
                        }
                    }
                } else if (!vb.isField()) {
