...
public abstract class AbstractSpellDictionary implements ISpellDictionary {
    ...
    protected final Set<RankedWordProposal> getCandidates(final String word, final boolean sentence, final ArrayList<String> hashs) {
        ...
        for (int index = 0; index < hashs.size(); index++) {
            ...
            @SuppressWarnings("unchecked") final ArrayList<byte[]> candidateList = (ArrayList<byte[]>) candidates;
            // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=195357
    ...
    protected final void hashWord(final String word) {
        ...
        if (bucket == null) {
            ...
            @SuppressWarnings("unchecked") ArrayList<byte[]> bucketList = (ArrayList<byte[]>) bucket;
    ...
    protected String stripNonLetters(String word) {
        ...
        int i = 0;
        int j = word.length() - 1;
        while (i <= j && !Character.isLetter(word.charAt(i))) i++;
        if (i > j)
            //$NON-NLS-1$
            return "";
        while (j > i && !Character.isLetter(word.charAt(j))) j--;
