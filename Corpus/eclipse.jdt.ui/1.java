...
public class JUnitCorePlugin extends Plugin {
    ...
    //$NON-NLS-1$ //$NON-NLS-2$
    public static final String ID_EXTENSION_POINT_TESTRUN_LISTENERS = PLUGIN_ID + "." + "testRunListeners";

    //$NON-NLS-1$ //$NON-NLS-2$
    public static final String ID_EXTENSION_POINT_TEST_KINDS = PLUGIN_ID + "." + "internal_testKinds";
    ...
    private synchronized void loadTestRunListeners() {
        ...
        //$NON-NLS-1$
        MultiStatus status = new MultiStatus(CORE_PLUGIN_ID, IStatus.OK, "Could not load some testRunner extension points", null);
        for (int i = 0; i < configs.length; i++) {
            try {
                Object testRunListener = //$NON-NLS-1$
