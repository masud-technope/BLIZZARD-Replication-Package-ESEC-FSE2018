...
public class ControlBodyUpdates {
    ...
    private void foo() {
        for (int i = 0; i < parameterObject.getArraylists().get(0); i++) parameterObject.setArraylists(null);
        for (int intValue : parameterObject.getInts()) parameterObject.setArraylists(new ArrayList(parameterObject.getArraylists().size()));
        while (parameterObject.getInts().length != 0) parameterObject.setArraylists(null);
        ...
        if (newRange[0] > 0)
            ...
            parameterObject.setNewStart(newRange[0] - 1);
        else
