...
public class NullAnnotationsFix extends CompilationUnitRewriteOperationsFix {
    ...
    public static NullAnnotationsFix createNullAnnotationInSignatureFix(CompilationUnit compilationUnit, IProblemLocation problem, ChangeKind changeKind, boolean isArgumentProblem) {
        ...
        NullAnnotationsRewriteOperations.SignatureAnnotationRewriteOperation operation = NullAnnotationsRewriteOperations.createAddAnnotationOperation(compilationUnit, problem, annotationToAdd, annotationToRemove, null, /*thisUnitOnly*/
        false, /*allowRemove*/
        true, isArgumentProblem, changeKind);
        if (operation == null)
            return null;
        ...
        return new // note that this uses the findings from createAddAnnotationOperation(..)
        NullAnnotationsFix(// note that this uses the findings from createAddAnnotationOperation(..)
        operation.getMessage(), // note that this uses the findings from createAddAnnotationOperation(..)
        operation.getCompilationUnit(), new NullAnnotationsRewriteOperations.SignatureAnnotationRewriteOperation[] { operation });
    }
    ...
    private static void createAddNullAnnotationOperations(CompilationUnit compilationUnit, IProblemLocation[] locations, List<CompilationUnitRewriteOperation> result) {
        ...
        for (int i = 0; i < locations.length; i++) {
            ...
            SignatureAnnotationRewriteOperation fix = NullAnnotationsRewriteOperations.createAddAnnotationOperation(compilationUnit, problem, annotationToAdd, annotationToRemove, handledPositions, /*thisUnitOnly*/
            true, /*allowRemove*/
            false, isArgumentProblem, ChangeKind.LOCAL);
            if (fix != null) {
                if (annotationToAdd == nonNullAnnotationName) {
