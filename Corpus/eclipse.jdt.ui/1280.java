...
public class OpenTypeHierarchyUtil {
    ...
    public static TypeHierarchyViewPart open(IJavaElement[] candidates, IWorkbenchWindow window) {
        ...
        try {
            ...
            ExceptionHandler.handle(e, window.getShell(), JavaUIMessages.OpenTypeHierarchyUtil_error_open_perspective, e.getMessage());
        } catch (JavaModelException e) {
            ExceptionHandler.handle(e, window.getShell(), JavaUIMessages.OpenTypeHierarchyUtil_error_open_editor, e.getMessage());
        }
        return null;
    }
    ...
    private static TypeHierarchyViewPart openInViewPart(IWorkbenchWindow window, IJavaElement[] input) {
        ...
        try {
            ...
            if (result != null) {
                // avoid refresh of old hierarchy on 'becomes visible'
    ...
    private static TypeHierarchyViewPart openInPerspective(IWorkbenchWindow window, IJavaElement[] input) throws WorkbenchException, JavaModelException {
        ...
        if (perspectiveInput != null) {
            if (page.getEditorReferences().length == 0) {
                // only open when the perspective has been created
                JavaUI.openInEditor(input[0], false, false);
            }
        }
        return part;
    }
