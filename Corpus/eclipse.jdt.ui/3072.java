...
public abstract class AbstractInfoView extends ViewPart implements ISelectionListener, IMenuListener, IPropertyChangeListener {
	 ...
	 */
    protected abstract Object computeInput(Object element);
        ...
        do {
            ...
            if (paren == JavaHeuristicScanner.NOT_FOUND) {
                try {
                    ...
                    if (pos == contextPosition - 1 && document.getChar(pos) == ')') {
    ...
    private void computeAndDoSetInput(final IWorkbenchPart part, final IJavaElement element, final boolean resetIfInvalid) {
        ...
        if (element != null)
            ...
            if (// if selection is empty then we need to check if the current view input is valid. If not the view should be cleared
            selection == null)
                return;
        }
        if (fComputeProgressMonitor != null)
        ...
        "Info view input computer") {
            ...
            public void run() {
                ...
                input != null ? //$NON-NLS-1$
                computeDescription(part, selection, je, computeProgressMonitor) : //$NON-NLS-1$
                "";
	 ...
	 */
    //$NON-NLS-1$
    private static final String SYNCED_GIF = "synced.png";
	 ...
	 */
    //$NON-NLS-1$
    private static final String SYNC_BROKEN_GIF = "sync_broken.png";
