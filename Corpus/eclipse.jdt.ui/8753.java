...
public class LazyGenericTypeProposal extends LazyJavaTypeCompletionProposal {
    ...
    private void adaptOffsets(int[] offsets, StringBuffer buffer) {
        ...
        // due to using an import instead of package
    ...
    private TypeArgumentProposal computeTypeProposal(ITypeParameter parameter) throws JavaModelException {
        ...
        if (//$NON-NLS-1$
        bounds.length == 1 && !"java.lang.Object".equals(bounds[0]))
    ...
    private String computeTypeParameterDisplayName(ITypeParameter parameter, String[] bounds) {
        if (//$NON-NLS-1$
        bounds.length == 0 || bounds.length == 1 && "java.lang.Object".equals(bounds[0]))
    ...
    private IType[] computeInheritancePath(IType subType, IType superType) throws JavaModelException {
        ...
        do {
            ...
            } catch (ArrayIndexOutOfBoundsException // see bug 433587
            e) {
                String message = "Illegal supertype hierarchy for " + subType.getFullyQualifiedName('.') + ". No subtype found for " + superType.getFullyQualifiedName('.') + " in the hierarchy.";
    ...
    private String findMatchingSuperTypeSignature(IType subType, IType superType) throws JavaModelException {
        ...
        //$NON-NLS-1$
        throw new JavaModelException(new CoreException(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "Illegal hierarchy", null)));
    }
    ...
    private boolean shouldAppendArguments(IDocument document, int offset, char trigger) {
        ...
        try {
            ...
            while (index != line.length() && Character.isUnicodeIdentifierPart(line.charAt(index))) ++index;
    ...
    private void installLinkedMode(final IDocument document, int[] offsets, int[] lengths, TypeArgumentProposal[] typeArgumentProposals, boolean withParentheses, final boolean onlyAppendArguments) {
        ...
        try {
            ...
            if (// see bug 301990
            !onlyAppendArguments && (document instanceof IDocumentExtension)) {
                FormatterPrefs prefs = getFormatterPrefs();
                                ...
                                public void perform(IDocument d, IDocumentListener owner) {
                                    try {
                                        if ((firstBracketPosition.length == 0 || firstBracketPosition.isDeleted) && !secondBracketPosition.isDeleted) {
                                            d.replace(firstBracketPosition.offset, //$NON-NLS-1$
                                            secondBracketPosition.offset - //$NON-NLS-1$
                                            firstBracketPosition.offset, "");
                                        }
                                    } catch (BadLocationException e) {
                                        JavaPlugin.log(e);
                                    }
    ...
    private void openErrorDialog(BadLocationException e) {
        ...
        MessageDialog.openError(shell, JavaTextMessages.FilledArgumentNamesMethodProposal_error_msg, message == null ? e.toString() : message);
    }
