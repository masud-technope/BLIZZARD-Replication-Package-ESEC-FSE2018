...
public class JavadocHover extends AbstractJavaEditorTextHover {
    ...
    public static JavadocBrowserInformationControlInput getHoverInfo(IJavaElement[] elements, ITypeRoot editorInputElement, IRegion hoverRegion, JavadocBrowserInformationControlInput previousInput) {
        ...
        if (buffer.length() > 0) {
            ...
            if (base != null) {
                int endHeadIdx = //$NON-NLS-1$
                buffer.indexOf(//$NON-NLS-1$
                "</head>");
                //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public static String getImageAndLabel(IJavaElement element, boolean allowImage, String label) {
        ...
        if (imageSrcPath != null) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            buf.append("<span ").append(tooltip).append("style=\"").append(imageStyle).append("filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='").append(imageSrcPath).append(//$NON-NLS-1$ //$NON-NLS-2$
            "')\"></span>\n");
            //$NON-NLS-1$
    ...
    private static void addValue(StringBuffer buf, IJavaElement element, Object value) throws URISyntaxException {
        ...
        if (value instanceof ITypeBinding) {
            ...
            //$NON-NLS-1$
            buf.append(".class");
        } else if (// only enum constants
        value instanceof IVariableBinding) {
    ...
    public static String getHexConstantValue(Object constantValue) {
        if (constantValue instanceof Character) {
            ...
            //$NON-NLS-1$
            return formatWithHexValue(constantValue, "0x" + Integer.toHexString(byteValue));
        } else if (constantValue instanceof Short) {
            int shortValue = ((Short) constantValue).shortValue() & 0xFFFF;
            //$NON-NLS-1$
            return formatWithHexValue(constantValue, "0x" + Integer.toHexString(shortValue));
        } else if (constantValue instanceof Integer) {
            int intValue = ((Integer) constantValue).intValue();
            //$NON-NLS-1$
            return formatWithHexValue(constantValue, "0x" + Integer.toHexString(intValue));
        } else if (constantValue instanceof Long) {
            long longValue = ((Long) constantValue).longValue();
            //$NON-NLS-1$
            return formatWithHexValue(constantValue, "0x" + Long.toHexString(longValue));
        } else {
