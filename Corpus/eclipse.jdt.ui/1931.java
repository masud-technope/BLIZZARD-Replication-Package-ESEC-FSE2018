...
public class JavaCompareUtilities {
        ...
        try {
            ...
            while (true) {
                ...
                if (c == -1)
                    break;
                sb.append((char) c);
                if (// single CR or a CR followed by LF
                c == '\r') {
                    c = reader.read();
                    if (c == -1)
                        break;
    ...
    static void initAction(IAction a, ResourceBundle bundle, String prefix) {
        ...
        if (relPath != null && relPath.trim().length() > 0) {
            ...
            // we set the disabled image first (see PR 1GDDE87)
    ...
    static void initToggleAction(IAction a, ResourceBundle bundle, String prefix, boolean checked) {
        ...
        if (tooltip == null)
            //$NON-NLS-1$
            tooltip = getString(bundle, prefix + "tooltip", null);
        if (tooltip != null)
            a.setToolTipText(tooltip);
        String description = null;
        if (checked)
            //$NON-NLS-1$
            description = getString(bundle, prefix + "description.checked", null);
        else
            //$NON-NLS-1$
            description = getString(bundle, prefix + "description.unchecked", null);
        if (description == null)
            //$NON-NLS-1$
