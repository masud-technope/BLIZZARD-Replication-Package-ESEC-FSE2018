...
public abstract class AbstractExceptionAnalyzer extends ASTVisitor {
    ...
    // Elements in this list are of type TypeBinding
    ...
    public boolean visit(VariableDeclarationExpression node) {
        if (node.getLocationInParent() == TryStatement.RESOURCES_PROPERTY) {
            ...
            if (resourceTypeBinding != null) {
                IMethodBinding methodBinding = //$NON-NLS-1$
                Bindings.findMethodInHierarchy(//$NON-NLS-1$
                resourceTypeBinding, //$NON-NLS-1$
                "close", //$NON-NLS-1$
                new ITypeBinding[0]);
