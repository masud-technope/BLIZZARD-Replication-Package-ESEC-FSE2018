...
public class CompletionProposalLabelProvider {
    ...
    private StyledString appendUnboundedParameterList(StyledString buffer, CompletionProposal methodProposal) {
        ...
        for (int i = 0; i < parameterTypes.length; i++) parameterTypes[i] = createTypeDisplayName(SignatureUtil.getLowerBound(parameterTypes[i]));
    ...
    private char[] createTypeDisplayName(char[] typeSignature) throws IllegalArgumentException {
        ...
        if (useShortGenerics) {
            ...
            do {
                pos = //$NON-NLS-1$
                buf.indexOf(//$NON-NLS-1$
                "? extends ");
                if (pos >= 0) {
                    //$NON-NLS-1$
                    buf.replace(//$NON-NLS-1$
                    pos, //$NON-NLS-1$
                    pos + 10, //$NON-NLS-1$
                    "+");
                } else {
    ...
    ImageDescriptor createTypeImageDescriptor(CompletionProposal proposal) {
        ...
        return decorateImageDescriptor(JavaElementImageProvider.getTypeImageDescriptor(/* in order to get all visibility decorations */
        true, isInterfaceOrAnnotation, flags, false), proposal);
    }
