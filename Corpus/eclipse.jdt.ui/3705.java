...
public class CategoryFilterActionGroup extends ActionGroup {
    ...
    private class CategoryFilterSelectionDialog extends SelectionStatusDialog implements IListAdapter<String> {
        ...
        public  CategoryFilterSelectionDialog(Shell parent, List<String> categories, List<String> selectedCategories) {
            ...
            String[] buttons = { ActionMessages.CategoryFilterActionGroup_SelectAllCategories, ActionMessages.CategoryFilterActionGroup_DeselectAllCategories };
    ...
    //WORKAROUND for Bug 132669 https://bugs.eclipse.org/bugs/show_bug.cgi?id=132669
    ...
    private void loadSettings() {
        ...
        //$NON-NLS-1$
        string = store.getString(getPreferenceKey() + ".LRU");
        if (string != null && string.length() > 0) {
            //$NON-NLS-1$
            String[] categories = string.split(";");
        ...
        //$NON-NLS-1$
        fFilterUncategorizedMembers = store.getBoolean(getPreferenceKey() + ".FilterUncategorized");
    }
    ...
    private void storeSettings() {
        ...
        if (fFilteredCategories.size() == 0) {
            //$NON-NLS-1$
            store.setValue(getPreferenceKey(), "");
        } else {
            StringBuffer buf = new StringBuffer();
            ...
            //$NON-NLS-1$
            store.setValue(getPreferenceKey() + ".LRU", buf.toString());
            //$NON-NLS-1$
            store.setValue(getPreferenceKey() + ".FilterUncategorized", fFilterUncategorizedMembers);
        }
    }
    ...
    private //HashSet result, int max, LinkedHashMap lruList) {
    boolean collectCategories(//HashSet result, int max, LinkedHashMap lruList) {
    IJavaElement element, //HashSet result, int max, LinkedHashMap lruList) {
    ...
    private String getPreferenceKey() {
        //$NON-NLS-1$
        return "CategoryFilterActionGroup." + fViewerId;
    }
