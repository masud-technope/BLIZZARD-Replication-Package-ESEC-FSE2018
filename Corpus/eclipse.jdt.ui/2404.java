...
public class NlsRefactoringCreateChangeTest extends TestCase {
    ...
    public void testWithoutPreviousNlsing() throws Exception {
        //$NON-NLS-1$//$NON-NLS-2$
        fHelper.createPackageFragment("p2", "/TestSetupProject/src2");
        // class to NLS
        ...
        buf.append("}");
        ICompilationUnit cu = //$NON-NLS-1$//$NON-NLS-2$
        RefactoringTest.createCU(//$NON-NLS-1$//$NON-NLS-2$
        fHelper.getPackageFragment("/TestSetupProject/src1/p"), //$NON-NLS-1$//$NON-NLS-2$
        "Test.java", buf.toString());
        NLSRefactoring nls = createDefaultNls(cu);
        //$NON-NLS-1$
        nls.setAccessorClassPackage(fHelper.getPackageFragment("/TestSetupProject/src2/p2"));
        performChange(nls);
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        checkContentOfFile("properties", fHelper.getFile("/TestSetupProject/src2/p/test.properties"), buf.toString());
    }
    ...
    public void testCreateChangeWithCollidingImport() throws Exception {
        ...
        buf.append("}");
        //$NON-NLS-1$ //$NON-NLS-2$
        ICompilationUnit cu = RefactoringTest.createCU(fHelper.getPackageFragment("/TestSetupProject/src1/p"), "Test.java", buf.toString());
        NLSRefactoring nls = createDefaultNls(cu);
        performChange(nls);
        buf = new StringBuffer();
        buf.append("package p;\n");
    ...
    public void testCreateChangeWithNonDefaultSubstitution() throws Exception {
        ...
        NLSRefactoring nls = createDefaultNls(cu);
        //$NON-NLS-1$ //$NON-NLS-2$
        String string = "nonDefault(" + NLSRefactoring.KEY + ")";
        ...
        buf.append("}");
        checkContentOfCu(//$NON-NLS-1$
        "manipulated class", cu, buf.toString());
        buf = new StringBuffer();
        buf.append("test0=helloworld\n");
        //$NON-NLS-1$ //$NON-NLS-2$
        checkContentOfFile("properties", fHelper.getFile("/TestSetupProject/src2/p/test.properties"), buf.toString());
    ...
    private NLSRefactoring createDefaultNls(ICompilationUnit cu) {
        ...
        //$NON-NLS-1$
        nls.setAccessorClassName("Messages");
        NLSSubstitution[] substitutions = nls.getSubstitutions();
        nls.setPrefix("test");
        substitutions[0].setState(NLSSubstitution.EXTERNALIZED);
