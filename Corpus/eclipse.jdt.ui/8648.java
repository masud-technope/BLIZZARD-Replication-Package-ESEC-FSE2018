...
class ExternalizeWizardPage extends UserInputWizardPage {
    ...
    static {
        ...
        //$NON-NLS-1$
        PROPERTIES[STATE_PROP] = "task";
        //$NON-NLS-1$
        PROPERTIES[KEY_PROP] = "key";
        //$NON-NLS-1$
        PROPERTIES[VAL_PROP] = "value";
    ...
    private class NLSSubstitutionLabelProvider extends LabelProvider implements ITableLabelProvider, IFontProvider {
        ...
        private Image getNLSImage(NLSSubstitution sub) {
            ...
            } else if (sub.isConflicting(fSubstitutions) || !isKeyValid(sub, null)) {
    ...
    private void updateAccessorChoices() {
        AccessorDescription configured = new AccessorDescription(fNLSRefactoring.getAccessorClassName(), fNLSRefactoring.getAccessorClassPackage(), fNLSRefactoring.getResourceBundleName(), fNLSRefactoring.getResourceBundlePackage());
    ...
    protected void handleButtonPressed(Widget widget) {
        // make sure cell editor is applied on all platforms, see https://bugs.eclipse.org/bugs/show_bug.cgi?id=269611
