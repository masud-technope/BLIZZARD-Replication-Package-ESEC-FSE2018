...
class AccessAnalyzer extends ASTVisitor {
    ...
    public boolean visit(SimpleName node) {
        if (!node.isDeclaration() && considerBinding(node.resolveBinding(), node)) {
            fReferencingGetter = true;
            fRewriter.replace(node, //$NON-NLS-1$
            fRewriter.createStringPlaceholder(//$NON-NLS-1$
            fGetter + "()", //$NON-NLS-1$
            ASTNode.METHOD_INVOCATION), createGroupDescription(READ_ACCESS));
        }
        return true;
    }
    ...
    public void endVisit(CompilationUnit node) {
        ...
        if (fRemoveStaticImport) {
            //$NON-NLS-1$
            fImportRewriter.removeStaticImport(typeName + "." + fieldName);
        }
    ...
    private MethodInvocation createInvocation(AST ast, Expression operand, String operator) {
        ...
        //$NON-NLS-1$
        argument.setRightOperand(ast.newNumberLiteral("1"));
        fReferencingGetter = true;
        fReferencingSetter = true;
        return invocation;
    }
