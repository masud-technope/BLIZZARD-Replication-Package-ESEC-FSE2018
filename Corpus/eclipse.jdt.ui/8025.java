...
public abstract class DescriptionMatcher {
    ...
    private static class CompositeMatcher extends DescriptionMatcher {
        ...
        public boolean matches(Description description) {
            for (DescriptionMatcher matcher : fMatchers) if (matcher.matches(description))
                return true;
            return false;
        }
    ...
    private static class ExactMatcher extends DescriptionMatcher {
        ...
        public  ExactMatcher(String className, String testName) {
            ...
            //$NON-NLS-1$
            fDisplayName = String.format("%s(%s)", testName, className);
        }
        ...
        public String toString() {
            //$NON-NLS-1$
            return String.format("{%s:fDisplayName=%s]", getClass().getSimpleName(), fDisplayName);
        }
    }
    ...
    private static class LeadingIdentifierMatcher extends DescriptionMatcher {
        ...
        public String toString() {
            //$NON-NLS-1$
            return String.format("{%s:fClassName=%s,fLeadingIdentifier=%s]", getClass().getSimpleName(), fClassName, fLeadingIdentifier);
        }
    }
