...
public class UpdateTestSuite implements IObjectActionDelegate {
    ...
    public void run(IAction action) {
        ...
        if (fIsJunit4) {
            ...
            //$NON-NLS-1$
            fSuiteClasses = testSuiteType.getAnnotation("SuiteClasses");
            ...
            IType testSuiteType = fTestSuite.findPrimaryType();
            //$NON-NLS-1$
            fSuiteMethod = testSuiteType.getMethod("suite", new String[] {});
    ...
    public void selectionChanged(IAction action, ISelection selection) {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (testSuiteObj != null && testSuiteObj instanceof ICompilationUnit) {
                ...
                if (primaryType != null) {
                    fIsJunit4 = //$NON-NLS-1$
                    primaryType.getAnnotation("RunWith").exists();
                }
            }
        }
    }
    ...
    public static String getUpdatableString(Object[] selectedClasses) {
        ...
        for (Object selectedClasse : selectedClasses) {
            if (selectedClasse instanceof IType) {
                ...
                IMethod suiteMethod = //$NON-NLS-1$
                testType.getMethod(//$NON-NLS-1$
                "suite", //$NON-NLS-1$
                new String[] {});
    ...
    public static String getUpdatableAnnotations(Object[] selectedClasses) {
        //$NON-NLS-1$
        StringBuffer buffer = new StringBuffer("@SuiteClasses({");
    ...
    private void cannotUpdateSuiteError() {
        MessageDialog.openError(fShell, WizardMessages.UpdateAllTests_cannotUpdate_errorDialog_title, Messages.format(WizardMessages.UpdateAllTests_cannotUpdate_errorDialog_message, new String[] { NewTestSuiteWizardPage.START_MARKER, NewTestSuiteWizardPage.END_MARKER }));
    }
