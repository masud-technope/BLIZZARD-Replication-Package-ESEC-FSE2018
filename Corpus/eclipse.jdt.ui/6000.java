...
public class Bindings {
    ...
    public static boolean isRuntimeException(ITypeBinding thrownException) {
        if (thrownException == null || thrownException.isPrimitive() || thrownException.isArray())
            return false;
        //$NON-NLS-1$
        return findTypeInHierarchy(thrownException, "java.lang.RuntimeException") != null;
    }
    ...
    private static boolean containsTypeVariables(ITypeBinding[] types) {
        for (int i = 0; i < types.length; i++) if (containsTypeVariables(types[i]))
            return true;
        return false;
    }
        ...
        if (type.isPrimitive() || type.isTypeVariable()) {
            ...
            if (candidate.charAt(Signature.getArrayCount(candidate)) == Signature.C_RESOLVED) {
                ...
                if (qualifiedCandidates == null || qualifiedCandidates.length == 0)
                    return false;
                String packageName = type.getPackage().isUnnamed() ? "" : //$NON-NLS-1$
    ...
    private static String getBoxedTypeName(String primitiveName) {
        if (//$NON-NLS-1$
        "long".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Long";
        else if (//$NON-NLS-1$
        "int".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Integer";
        else if (//$NON-NLS-1$
        "short".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Short";
        else if (//$NON-NLS-1$
        "char".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Character";
        else if (//$NON-NLS-1$
        "byte".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Byte";
        else if (//$NON-NLS-1$
        "boolean".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Boolean";
        else if (//$NON-NLS-1$
        "float".equals(primitiveName))
            //$NON-NLS-1$
            return "java.lang.Float";
        else if (//$NON-NLS-1$
        "double".equals(primitiveName))
            //$NON-NLS-1$
    ...
    private static String getUnboxedTypeName(String boxedName) {
        if (//$NON-NLS-1$
        "java.lang.Long".equals(boxedName))
            //$NON-NLS-1$
            return "long";
        else if (//$NON-NLS-1$
        "java.lang.Integer".equals(boxedName))
            //$NON-NLS-1$
            return "int";
        else if (//$NON-NLS-1$
        "java.lang.Short".equals(boxedName))
            //$NON-NLS-1$
            return "short";
        else if (//$NON-NLS-1$
        "java.lang.Character".equals(boxedName))
            //$NON-NLS-1$
            return "char";
        else if (//$NON-NLS-1$
        "java.lang.Byte".equals(boxedName))
            //$NON-NLS-1$
            return "byte";
        else if (//$NON-NLS-1$
        "java.lang.Boolean".equals(boxedName))
            //$NON-NLS-1$
            return "boolean";
        else if (//$NON-NLS-1$
        "java.lang.Float".equals(boxedName))
            //$NON-NLS-1$
            return "float";
        else if (//$NON-NLS-1$
        "java.lang.Double".equals(boxedName))
            //$NON-NLS-1$
            return "double";
        else
            return null;
    ...
    public static boolean isAnyNullAnnotation(ITypeBinding annotationType, IJavaProject project) {
        String qualifiedName = annotationType.getQualifiedName();
        return qualifiedName.equals(project.getOption(JavaCore.COMPILER_NONNULL_ANNOTATION_NAME, true)) || qualifiedName.equals(project.getOption(JavaCore.COMPILER_NULLABLE_ANNOTATION_NAME, true));
    }
