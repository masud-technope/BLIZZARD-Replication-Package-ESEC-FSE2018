...
public class RenameLinkedMode {
    ...
    private class FocusEditingSupport implements IEditingSupport {
        ...
        public boolean isOriginator(DocumentEvent event, IRegion subjectRegion) {
            //leave on external modification outside positions
            return false;
        }
    }
    ...
    private class ExitPolicy extends DeleteBlockingExitPolicy {
        ...
        public ExitFlags doExit(LinkedModeModel model, VerifyEvent event, int offset, int length) {
            fShowPreview = (event.stateMask & SWT.CTRL) != 0 && (event.character == SWT.CR || event.character == SWT.LF);
    ...
    void doRename(boolean showPreview) {
        ...
        try {
            ...
            Shell shell = fEditor.getSite().getShell();
            boolean executed;
            if (// could have been updated by undoAndCreateRenameSupport(..)
            fShowPreview) {
