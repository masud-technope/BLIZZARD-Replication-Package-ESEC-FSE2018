...
public class TextFieldNavigationHandler {
    ...
    private abstract static class WorkaroundNavigable extends Navigable {
        ...
        void selectionChanged() {
            ...
            selection.x == selection.y) {
                ...
                //same end -> assume caret at start
                fCaretPosition = selection.x;
            } else {
    ...
    private static class TextNavigable extends WorkaroundNavigable {
        ...
        static final boolean BUG_106024_TEXT_SELECTION = "win32".equals(//$NON-NLS-1$
        SWT.getPlatform()) || // on carbon, getCaretPosition() always returns getSelection().x
        Util.isMac();
        ...
        private KeyAdapter getKeyListener() {
            if (fKeyListener == null) {
                fKeyListener = new KeyAdapter() {
                    ...
                    private final boolean IS_WORKAROUND = (fNavigable instanceof ComboNavigable) || (fNavigable instanceof TextNavigable && TextNavigable.BUG_106024_TEXT_SELECTION);
                    ...
                    public void keyPressed(KeyEvent e) {
                        ...
                        for (Iterator<Submission> iter = getSubmissions().iterator(); iter.hasNext(); ) {
                            ...
                            for (int i = 0; i < triggerSequences.length; i++) {
                                if (// XXX does not work for multi-stroke bindings
                                triggerSequences[i].equals(keySequence)) {
                                    e.doit = false;
                                    submission.execute();
                                    return;
                                }
