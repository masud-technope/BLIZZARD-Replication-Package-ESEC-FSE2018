...
public class JavaSearchResult extends AbstractJavaSearchResult {
    ...
    public String getLabel() {
        return fQuery.getSpecification().size() == 1 ? fQuery.getResultLabel(getMatchCount()) : Messages.format(SearchMessages.JavaSearchQuery_multi_selection_search_description, fQuery.getResultLabel(getMatchCount()));
    }
    ...
    boolean addMatch(Match match, IMatchPresentation participant) {
        ...
        if (fElementsToParticipants.get(element) != null) {
            ...
            //$NON-NLS-1$
            JavaPlugin.log(new Status(IStatus.WARNING, JavaPlugin.getPluginId(), 0, "A second search participant was found for an element", null));
            return false;
        }
