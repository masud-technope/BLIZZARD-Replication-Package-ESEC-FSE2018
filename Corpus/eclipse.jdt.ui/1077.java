...
public class CompilationUnitDocumentProvider extends TextFileDocumentProvider implements ICompilationUnitDocumentProvider, IAnnotationModelFactory {
	 ...
	 */
    protected static class CompilationUnitInfo extends FileInfo {
    ...
    public static class ProblemAnnotation extends Annotation implements IJavaAnnotation, IAnnotationPresentation, IQuickFixableAnnotation {
        ...
        static {
            ...
            //$NON-NLS-1$
            TASK_LAYER = computeLayer("org.eclipse.ui.workbench.texteditor.task", lookup);
            //$NON-NLS-1$
            INFO_LAYER = computeLayer("org.eclipse.jdt.ui.info", lookup);
            //$NON-NLS-1$
            WARNING_LAYER = computeLayer("org.eclipse.jdt.ui.warning", lookup);
            //$NON-NLS-1$
            ERROR_LAYER = computeLayer("org.eclipse.jdt.ui.error", lookup);
        }
    ...
    protected static class CompilationUnitAnnotationModel extends ResourceMarkerAnnotationModel implements IProblemRequestor, IProblemRequestorExtension {
        ...
        private void overlayMarkers(Position position, ProblemAnnotation problemAnnotation) {
            ...
            if (value instanceof List) {
                ...
                for (Iterator<?> e = list.iterator(); e.hasNext(); ) setOverlay(e.next(), problemAnnotation);
            } else {
        ...
        public synchronized void setIsActive(boolean isActive) {
            // must not be enabled from UI threads
            Assert.isLegal(!isActive || Display.getCurrent() == null);
            fIsActive = isActive;
            if (fIsActive)
        ...
        protected void addAnnotation(Annotation annotation, Position position, boolean fireModelChanged) throws BadLocationException {
            ...
            synchronized (getLockObject()) {
                ...
                if (cached == null)
                    ...
                    @SuppressWarnings("unchecked") List<Object> list = (List<Object>) cached;
                    list.add(annotation);
    ...
    private static final String HANDLE_TEMPORARY_PROBLEMS = PreferenceConstants.EDITOR_EVALUTE_TEMPORARY_PROBLEMS;
    ...
    private IJavaProject findJavaProject(IPath path) {
        ...
        for (int i = 0; i < projects.length; i++) {
            ...
            for (int j = 0; j < pathSegments.length; j++) if (projectSegment.equals(pathSegments[j]))
                return projects[i];
        }
        return null;
    }
            ...
            if (fSavePolicy != null) {
                ...
                if (unit != null && info.fModel instanceof AbstractMarkerAnnotationModel) {
                    ...
                    if (markers != null && markers.length > 0) {
                        AbstractMarkerAnnotationModel model = (AbstractMarkerAnnotationModel) info.fModel;
                        for (int i = 0; i < markers.length; i++) model.updateMarker(document, markers[i], null);
                    }
                }
            }
        } finally {
    ...
    public void shutdown() {
        ...
        while (e.hasNext()) disconnect(e.next());
            ...
            for (int i = 0; i < listeners.length; i++) {
                ...
                SafeRunner.run(new ISafeRunnable() {
                    ...
                    public void handleException(Throwable ex) {
                        String msg = Messages.format("The save participant ''{0}'' caused an exception: {1}", new String[] { //$NON-NLS-1$
                        listener.getId(), //$NON-NLS-1$
                        ex.toString() });
