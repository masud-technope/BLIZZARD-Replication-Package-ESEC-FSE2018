...
public class JavaSpellingProblem extends SpellingProblem {
    ...
    public ICompletionProposal[] getProposals(IQuickAssistInvocationContext context) {
        ...
        if (checker != null) {
            ...
            if ((sentence && match) && !fixed)
                result = new IJavaCompletionProposal[] { new ChangeCaseProposal(arguments, getOffset(), getLength(), context, engine.getLocale()) };
            else {
                ...
                for (index = 0; index < size; index++) {
                    ...
                    result[index] = new WordCorrectionProposal(proposal.getText(), arguments, getOffset(), getLength(), context, proposal.getRank());
                }
                if (extendable)
    ...
    public String[] getArguments() {
        ...
        return new String[] { word, prefix, postfix, isSentenceStart() ? Boolean.toString(true) : Boolean.toString(false), isDictionaryMatch() ? Boolean.toString(true) : Boolean.toString(false) };
    }
