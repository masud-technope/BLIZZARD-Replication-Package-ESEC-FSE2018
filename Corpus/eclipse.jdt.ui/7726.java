...
public class TypeRules {
    ...
    public static boolean canCast(ITypeBinding castType, ITypeBinding bindingToCast) {
        ...
        if (bindingToCast.isArray()) {
            if (!castType.isArray()) {
                // can not cast an arraytype to a non array type (except to Object, Serializable...)
                return isArrayCompatible(castType);
            }
            ...
            if (toCastDim == castTypeDim) {
                ...
                if (castType.isPrimitive() && castType != bindingToCast) {
                    // can't assign arrays of different primitive types to each other
                    return false;
                }
            // fall through
