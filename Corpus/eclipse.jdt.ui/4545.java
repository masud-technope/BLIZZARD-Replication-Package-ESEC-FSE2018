...
public class JavaElementImageDescriptor extends CompositeImageDescriptor {
	 ...
	 * @since 3.12
	 */
    public static final int INFO = 0x40000;
    ...
    private void drawBottomRight() {
        ...
        // methods:
        if (// both flags set: merged overlay image
        (flags & syncAndOver) == syncAndOver) {
            addBottomRightImage(JavaPluginImages.DESC_OVR_SYNCH_AND_OVERRIDES, pos);
            // clear to not render again
            flags &= ~syncAndOver;
        } else if (// both flags set: merged overlay image
        (flags & syncAndImpl) == syncAndImpl) {
