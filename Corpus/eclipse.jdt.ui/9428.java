...
public class JavadocOptionsManager {
    ...
    private void loadFromXML(Element element) {
        ...
        if (destination.length() == 0) {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                if (//$NON-NLS-1$
                child.getNodeName().equals("doclet")) {
    ...
    private IPath makeAbsolutePathFromRelative(IPath path) {
        if (!path.isAbsolute()) {
            ...
            // relative to the ant file location
            IPath basePath = fXmlfile.getParent().getLocation();
            if (basePath == null) {
                return null;
            }
    ...
    public IStatus getArgumentArray(List<String> vmArgs, List<String> toolArgs) {
        ...
        //$NON-NLS-1$
        toolArgs.add("-" + fAccess);
        if (fFromStandard) {
            if (//$NON-NLS-1$
            fSource.length() > 0 && !fSource.equals("-")) {
                //$NON-NLS-1$
                toolArgs.add(//$NON-NLS-1$
                "-source");
        ...
        if (fAdditionalParams.length() + fVMParams.length() != 0) {
            ...
            for (int i = 0; i < argsArray.length; i++) {
                ...
                if (//$NON-NLS-1$
                "-locale".equals(arg) && i + 1 < argsArray.length) {
                    locale = argsArray[++i];
                } else
                    toolArgs.add(arg);
            }
        ...
        if (fAntpath.length() > 0) {
            try {
                ...
                if (root.findFilesForLocationURI(URIUtil.toURI(filePath.makeAbsolute())).length > 0) {
                    // only do relative path if ant file is stored in the workspace
    ...
    public File writeXML(Element javadocElement) throws CoreException {
        ...
        try {
            ...
            //$NON-NLS-1$
            IContentType type = Platform.getContentTypeManager().getContentType("org.eclipse.ant.core.antBuildFile");
            if (type != null)
    ...
    private static String initJavadocCommandDefault() {
        ...
        // old location
        String cmd = store.getString(PreferenceConstants.JAVADOC_COMMAND);
