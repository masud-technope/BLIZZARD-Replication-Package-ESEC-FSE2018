...
public abstract class RefactoringTest extends TestCase {
    ...
    protected static final List<String> PROJECT_RESOURCE_CHILDREN = Arrays.asList(new String[] { ".project", ".classpath", ".settings" });
            ...
            for (int i = 0; i < packages.length; i++) {
                ...
                if (!pack.equals(getPackageP()) && pack.exists() && !pack.isReadOnly())
                    if (pack.isDefaultPackage())
                        ...
                        JavaProjectHelper.deletePackage(pack);
                    else
                        // also delete packages with subpackages
                        JavaProjectHelper.delete(pack.getResource());
            }
            // Restore package 'p'
            if (!pExists)
    ...
    private void refreshFromLocal() throws CoreException {
        ...
        else if (//don't refresh package if root already refreshed
        getPackageP().exists())
    ...
    protected IType getType(ICompilationUnit cu, String name) throws JavaModelException {
        ...
        for (int i = 0; i < types.length; i++) if (types[i].getTypeQualifiedName('.').equals(name) || types[i].getElementName().equals(name))
            return types[i];
        return null;
    }
