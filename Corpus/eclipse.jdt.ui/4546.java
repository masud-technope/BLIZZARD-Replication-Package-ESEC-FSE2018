...
public class PropertyKeyHyperlink implements IHyperlink {
    ...
    private KeyReference select(final KeyReference[] keyReferences) {
        ...
        ILabelProvider labelProvider = new WorkbenchLabelProvider() {
            ...
            public String decorateText(String input, Object element) {
                ...
                String name = javaElement != null ? JavaElementLabels.getElementLabel(javaElement, JavaElementLabels.ALL_DEFAULT | JavaElementLabels.ALL_FULLY_QUALIFIED | JavaElementLabels.USE_RESOLVED | JavaElementLabels.P_COMPRESSED) : resource.getName();
                if (name == null)
                    return input;
                int count = 0;
    ...
    private void open(KeyReference keyReference) {
        ...
        try {
            ...
            if (wbAdapter != null)
                message = Messages.format(PropertiesFileEditorMessages.OpenAction_error_messageArgs, new String[] { wbAdapter.getLabel(keyReference), x.getLocalizedMessage() });
            if (message == null)
                message = Messages.format(PropertiesFileEditorMessages.OpenAction_error_message, x.getLocalizedMessage());
            MessageDialog.openError(fShell, PropertiesFileEditorMessages.OpenAction_error_messageProblems, message);
        }
    }
                ...
                public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
                    ...
                    try {
                        ...
                        if (useDoubleQuotedKey) {
                            SearchPattern pattern = SearchPattern.createPattern(key, IJavaSearchConstants.FIELD, IJavaSearchConstants.REFERENCES, SearchPattern.R_PATTERN_MATCH | SearchPattern.R_CASE_SENSITIVE);
                            if (pattern == null)
                                return;
                            try {
    ...
    private static TextSearchScope createScope(IResource scope) {
        ...
        for (int i = 0; i < javaExtensions.length; i++) //$NON-NLS-1$
