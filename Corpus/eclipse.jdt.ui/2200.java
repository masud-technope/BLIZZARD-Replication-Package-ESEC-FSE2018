...
public class WorkspaceScopeTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        //$NON-NLS-1$ //$NON-NLS-2$
        fProject1 = createStandardProject("Test", "test");
        //$NON-NLS-1$
        IPackageFragment pkg = fProject1.findPackageFragment(new Path("/Test/src/test"));
        //$NON-NLS-1$
        fCompilationUnit = pkg.createCompilationUnit("Test.java", getSource(), true, null);
        //$NON-NLS-1$//$NON-NLS-2$
        fProject2 = createStandardProject("Test2", "test2");
        JavaProjectHelper.addRequiredProject(fProject2, fProject1);
        //$NON-NLS-1$ //$NON-NLS-2$
        fProject3 = createStandardProject("Test3", "test3");
        //$NON-NLS-1$//$NON-NLS-2$
        fProject4 = createStandardProject("Test4", "test4", false);
    }
    ...
    private IJavaProject createStandardProject(String name, String pkgName, boolean includeJRE) throws CoreException, JavaModelException {
        ...
        if (includeJRE) {
            //$NON-NLS-1$
            IClasspathEntry jreLib = JavaCore.newContainerEntry(new Path("org.eclipse.jdt.launching.JRE_CONTAINER"));
    ...
    public void testPrivateScope() throws JavaModelException {
        ...
        //$NON-NLS-1$
        type.getMethod("privateMethod", new String[0]);
    ...
    public void testDefaultScope() throws JavaModelException {
        IType type = fCompilationUnit.findPrimaryType();
        //$NON-NLS-1$
        type.getMethod("defaultMethod", new String[0]);
        IJavaSearchScope scope = createWorkspaceScope(true);
    ...
    public void testPublicMethod() throws JavaModelException {
        IType type = fCompilationUnit.findPrimaryType();
        //$NON-NLS-1$
        type.getMethod("publicMethod", new String[0]);
        IJavaSearchScope scope = createWorkspaceScope(true);
    ...
    public void testProtectedMethod() throws JavaModelException {
        IType type = fCompilationUnit.findPrimaryType();
        //$NON-NLS-1$
        type.getMethod("protectedMethod", new String[0]);
        IJavaSearchScope scope = createWorkspaceScope(true);
        checkNoJreRoots(scope, fProject1);
        checkNoJreRoots(scope, fProject2);
        checkNoRoots(scope, fProject3);
    ...
    public void testJREProtected() throws JavaModelException {
        //$NON-NLS-1$
        IType object = fProject1.findType("java.lang.Object");
        //$NON-NLS-1$
        object.getMethod("clone", new String[0]);
        IJavaSearchScope scope = createWorkspaceScope(true);
