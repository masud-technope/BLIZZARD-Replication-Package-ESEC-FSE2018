...
public class CombinedWordRule implements IRule {
    ...
    public static class CharacterBuffer {
        ...
        public int hashCode() {
            if (fIsHashCached)
                return fHashCode;
            int hash = 0;
            for (int i = 0, n = fLength; i < n; i++) hash = 29 * hash + fContent[i];
            fHashCode = hash;
            fIsHashCached = true;
            return hash;
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (length != fLength)
                return false;
            for (int i = 0; i < length; i++) if (buffer.charAt(i) != fContent[i])
                return false;
            return true;
        }
        ...
        public boolean equals(String string) {
            ...
            if (length != fLength)
                return false;
            for (int i = 0; i < length; i++) if (string.charAt(i) != fContent[i])
                return false;
            return true;
        }
    }
    ...
    private void unreadBuffer(ICharacterScanner scanner) {
        for (int i = fBuffer.length() - 1; i >= 0; i--) scanner.unread();
    }
}
