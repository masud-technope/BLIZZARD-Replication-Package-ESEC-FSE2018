...
public class GetterSetterCorrectionSubProcessor {
    ...
    public static class SelfEncapsulateFieldProposal extends // public for tests
    ChangeCorrectionProposal {
        ...
        public TextFileChange getChange(IFile file) throws CoreException {
            ...
            //private field references are just searched in local file
        ...
        public void apply(IDocument document) {
            try {
                ...
                if (fNoDialog) {
                    ...
                    new RefactoringStarter().activate(new SelfEncapsulateFieldWizard(refactoring), JavaPlugin.getActiveWorkbenchShell(), "", //$NON-NLS-1$
    ...
    private static ChangeCorrectionProposal addSetterProposal(ProposalParameter context, int relevance) {
        ...
        if (method != null && Bindings.isVoidType(method.getReturnType()) && (Modifier.isStatic(method.getModifiers()) == Modifier.isStatic(context.variableBinding.getModifiers()))) {
            ...
            if (assignedValue == null)
                //we don't know how to handle those cases.
                return null;
    ...
    private static boolean isBoolean(ProposalParameter context) {
        AST ast = context.astRewrite.getAST();
        //$NON-NLS-1$
        boolean isBoolean = ast.resolveWellKnownType("boolean") == context.variableBinding.getType();
        if (!isBoolean)
            //$NON-NLS-1$
