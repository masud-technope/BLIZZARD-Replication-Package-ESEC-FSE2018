...
public final class JavaHeuristicScanner implements Symbols {
    ...
    private int getToken(String s) {
        ...
        switch(s.length()) {
            ...
            case 3:
                if (//$NON-NLS-1$
                "for".equals(//$NON-NLS-1$
                s))
                    return TokenFOR;
                if (//$NON-NLS-1$
                "try".equals(//$NON-NLS-1$
                s))
                    return TokenTRY;
                if (//$NON-NLS-1$
                "new".equals(//$NON-NLS-1$
                s))
                    return TokenNEW;
                break;
            case 4:
                ...
                s))
                    return TokenCASE;
                if (//$NON-NLS-1$
                "else".equals(//$NON-NLS-1$
                s))
                    return TokenELSE;
                if (//$NON-NLS-1$
                "enum".equals(//$NON-NLS-1$
                s))
                    return TokenENUM;
                if (//$NON-NLS-1$
                "goto".equals(//$NON-NLS-1$
                s))
                    return TokenGOTO;
                break;
            case 5:
                if (//$NON-NLS-1$
                "break".equals(//$NON-NLS-1$
                s))
                    return TokenBREAK;
                if (//$NON-NLS-1$
                "catch".equals(//$NON-NLS-1$
                s))
                    return TokenCATCH;
                if (//$NON-NLS-1$
                "class".equals(//$NON-NLS-1$
                s))
                    return TokenCLASS;
                if (//$NON-NLS-1$
                "while".equals(//$NON-NLS-1$
                s))
                    return TokenWHILE;
                break;
            case 6:
                ...
                s))
                    return TokenRETURN;
                if (//$NON-NLS-1$
                "static".equals(//$NON-NLS-1$
                s))
                    return TokenSTATIC;
                if (//$NON-NLS-1$
                "switch".equals(//$NON-NLS-1$
                s))
                    return TokenSWITCH;
                if (//$NON-NLS-1$
                "throws".equals(//$NON-NLS-1$
                s))
                    return TokenTHROWS;
                break;
            case 7:
            ...
            case 9:
                if (//$NON-NLS-1$
                "interface".equals(//$NON-NLS-1$
                s))
                    return TokenINTERFACE;
                break;
            case 12:
                if (//$NON-NLS-1$
                "synchronized".equals(//$NON-NLS-1$
                s))
    ...
    private ITypedRegion getPartition(int position) {
        if (!contains(fCachedPartition, position)) {
            ...
            try {
                ...
                fCachedPartition = new TypedRegion(position, 0, "__no_partition_at_all");
            }
        }
