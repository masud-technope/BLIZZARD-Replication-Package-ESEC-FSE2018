...
public final class GenerateHashCodeEqualsOperation implements IWorkspaceRunnable {
	 ...
	 */
    public  GenerateHashCodeEqualsOperation(final ITypeBinding type, final IVariableBinding[] fields, final CompilationUnit unit, final IJavaElement insert, final CodeGenerationSettings settings, final boolean useInstanceof, final boolean force, final boolean apply, final boolean save) {
    ...
    private Statement[] createAddSimpleHashCode(ITypeBinding type, IHashCodeAccessProvider provider, String name, boolean singleTemp) {
        ...
        if (!type.isPrimitive()) {
            ...
            //$NON-NLS-1$
            ce.setThenExpression(fAst.newNumberLiteral("0"));
    ...
    private Statement createAddArrayHashCode(IVariableBinding binding) {
        ...
        if (JavaModelUtil.is50OrHigher(fRewrite.getCu().getJavaProject())) {
            ...
            if (//$NON-NLS-1$
            element != null && !"".equals(element.getElementName()))
    ...
    private MethodDeclaration createHashCodeHelper(ITypeBinding binding) {
        ...
        //$NON-NLS-1$
        returnStatement.setExpression(fAst.newNumberLiteral("0"));
        ...
        //$NON-NLS-1$
        indexDeclaration.setInitializer(fAst.newNumberLiteral("0"));
        ...
        //$NON-NLS-1$
        access.setName(fAst.newSimpleName("length"));
    ...
    private Statement createAddQualifiedHashCode(IVariableBinding binding) {
        ...
        //$NON-NLS-1$
        cexpr.setThenExpression(fAst.newNumberLiteral("0"));
    ...
    private Expression createShiftAssignment(Expression shift1, Expression shift2) {
        ...
        //$NON-NLS-1$
        unsignedShiftRight.setRightOperand(fAst.newNumberLiteral("32"));
    ...
    private MethodDeclaration createEqualsMethod() throws CoreException {
        ...
        body.statements().add(createReturningIfStatement(fAst.newThisExpression(), fAst.newSimpleName(VARIABLE_NAME_EQUALS_PARAM), Operator.EQUALS, true));
        if (needsNoSuperCall(fType, METHODNAME_EQUALS, new ITypeBinding[] { fAst.resolveWellKnownType(JAVA_LANG_OBJECT) })) {
            // if (obj == null) return false;
            body.statements().add(createReturningIfStatement(fAst.newSimpleName(VARIABLE_NAME_EQUALS_PARAM), fAst.newNullLiteral(), Operator.EQUALS, false));
        } else {
        ...
        if (fSettings != null) {
            ...
            IMethodBinding objectMethod = null;
            for (int i = 0; i < objms.length; i++) {
                if (objms[i].getName().equals(METHODNAME_EQUALS) && objms[i].getParameterTypes().length == 1 && objms[i].getParameterTypes()[0].getQualifiedName().equals(JAVA_LANG_OBJECT))
                    objectMethod = objms[i];
            }
    ...
    private Statement createSimpleComparison(IVariableBinding binding) {
        if (isPrimitiveType(binding.getType(), PrimitiveType.FLOAT)) {
            return createReturningIfStatement(createFloatInvocation(getThisAccessForEquals(binding.getName())), createFloatInvocation(getOtherAccess(binding.getName())), Operator.NOT_EQUALS, false);
        } else if (isPrimitiveType(binding.getType(), PrimitiveType.DOUBLE)) {
            return createReturningIfStatement(createDoubleInvocation(getThisAccessForEquals(binding.getName())), createDoubleInvocation(getOtherAccess(binding.getName())), Operator.NOT_EQUALS, false);
        } else
    ...
    private Expression createFloatInvocation(Expression access) {
        //$NON-NLS-1$ //$NON-NLS-2$
        return createMethodInvocation(access, "java.lang.Float", "floatToIntBits");
    }

    private Expression createDoubleInvocation(Expression access) {
        //$NON-NLS-1$ //$NON-NLS-2$
        return createMethodInvocation(access, "java.lang.Double", "doubleToLongBits");
    }
    ...
    private boolean needsThisQualification(String name, boolean isHashCode) {
        if (isHashCode)
            return ((fDoubleCount > 0 && name.equals(VARIABLE_NAME_DOUBLE_TEMPORARY)) || (name.equals(VARIABLE_NAME_PRIME)) || (name.equals(VARIABLE_NAME_RESULT)));
