...
public class NullAnnotationsRewriteOperations {
    ...
    public enum ChangeKind implements  {

        // do the normal thing locally in the current method
        LOCAL() {
        }
        , // insert the inverse annotation in the local method
        INVERSE() {
        }
        , // change the overridden method
        OVERRIDDEN() {
        }
        , // change the target method of a method call
        TARGET() {
        }
        ;
    }

    abstract static class SignatureAnnotationRewriteOperation extends CompilationUnitRewriteOperation {
            ...
            for (int i = 0; i < annotations.length; i++) {
                ...
                if (annot.getAnnotationType().getQualifiedName().equals(fNonNullByDefaultName)) {
                    ...
                    if (pairs.length > 0) {
                        ...
                        for (int j = 0; j < pairs.length; j++) if (pairs[j].getKey() == null || //$NON-NLS-1$
                        pairs[j].getKey().equals(//$NON-NLS-1$
                        "value"))
    ...
    static class ReturnAnnotationRewriteOperation extends SignatureAnnotationRewriteOperation {
        ...
        private final MethodDeclaration fBodyDeclaration;
         ...
         ReturnAnnotationRewriteOperation(CompilationUnit unit, MethodDeclaration method, String annotationToAdd, String annotationToRemove, boolean allowRemove, String message) {
            fUnit = unit;
            //$NON-NLS-1$
            fKey = method.resolveBinding().getKey() + "<return>";
        ...
        public void rewriteAST(CompilationUnitRewrite cuRewrite, LinkedProposalModel model) throws CoreException {
            ...
            if (hasNonNullDefault(fBodyDeclaration.resolveBinding()))
                // should be safe, as in this case checkExisting() should've already produced a change (remove existing annotation).
                return;
            ...
            // null annotation is last modifier, directly preceding the return type
    ...
    static class ParameterAnnotationRewriteOperation extends SignatureAnnotationRewriteOperation {
        ...
        static ParameterAnnotationRewriteOperation create(CompilationUnit unit, LambdaExpression lambda, String annotationToAdd, String annotationToRemove, String paramName, boolean allowRemove, String message) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            throw new RuntimeException("Argument " + paramName + " not found in method " + lambda.toString());
        }
    ...
    public static SignatureAnnotationRewriteOperation createAddAnnotationOperation(CompilationUnit compilationUnit, IProblemLocation problem, String annotationToAdd, String annotationToRemove, Set<String> handledPositions, boolean thisUnitOnly, boolean allowRemove, boolean isArgumentProblem, ChangeKind changeKind) {
        // precondition:
        ...
        if (changeKind == ChangeKind.OVERRIDDEN)
            ...
            result = createAddAnnotationOperation(compilationUnit, problem, annotationToAdd, annotationToRemove, changeKind == ChangeKind.TARGET, thisUnitOnly, allowRemove, isArgumentProblem);
                    ...
                    if (paramName != null) {
                        switch(declaringNode.getNodeType()) {
                            ...
                            case ASTNode.LAMBDA_EXPRESSION:
                                ...
                                message = Messages.format(FixMessages.NullAnnotationsRewriteOperations_change_method_parameter_nullness, new Object[] { paramName, annotationNameLabel });
    ...
    private static SignatureAnnotationRewriteOperation createAddAnnotationToOverriddenOperation(CompilationUnit compilationUnit, IProblemLocation problem, String annotationToAdd, String annotationToRemove, boolean allowRemove) {
        ...
        if (declaringNode instanceof MethodDeclaration) {
            ...
            switch(problem.getProblemId()) {
                ...
                case IProblem.IllegalReturnNullityRedefinition:
                    if (// don't adjust super if local has no explicit annotation (?)
                    hasNullAnnotation(declaration)) {
    ...
    private static String findAffectedParameterName(ASTNode selectedNode) {
        VariableDeclaration argDecl = (selectedNode instanceof VariableDeclaration) ? (VariableDeclaration) selectedNode : (VariableDeclaration) ASTNodes.getParent(selectedNode, VariableDeclaration.class);
        if (argDecl != null)
    ...
    private static CompilationUnit findCUForMethod(CompilationUnit compilationUnit, ICompilationUnit cu, IMethodBinding methodBinding) {
        ...
        if (methodDecl == null) {
            ...
            if (declaringTypeDecl.isFromSource()) {
                ...
                } catch (JavaModelException /* can't do better */
                e) {
                }
                if (targetCU != null) {
