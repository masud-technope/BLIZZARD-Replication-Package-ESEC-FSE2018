...
public class ToggleBreadcrumbAction extends ResourceAction implements IPropertyChangeListener, IPerspectiveListener {
    ...
    public  ToggleBreadcrumbAction(IWorkbenchPage page, boolean inContextMenu) {
        super(JavaEditorMessages.getBundleForConstructedKeys(), //$NON-NLS-1$ //$NON-NLS-2$
        inContextMenu ? "ToggleBreadcrumbAction.Hide." : "ToggleBreadcrumbAction.", inContextMenu ? IAction.AS_PUSH_BUTTON : IAction.AS_CHECK_BOX);
        PlatformUI.getWorkbench().getHelpSystem().setHelp(this, IJavaHelpContextIds.TOGGLE_BREADCRUMB_ACTION);
        fPage = page;
        if (!inContextMenu) {
            //$NON-NLS-1$
            JavaPluginImages.setToolImageDescriptors(this, "toggle_breadcrumb.png");
    ...
    private String getPreferenceKey() {
        ...
        //$NON-NLS-1$
        return JavaEditor.EDITOR_SHOW_BREADCRUMB + "." + perspectiveID;
    }
