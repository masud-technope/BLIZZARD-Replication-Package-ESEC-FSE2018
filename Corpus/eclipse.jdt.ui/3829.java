...
public class IntroduceParameterObjectProcessor extends ChangeSignatureProcessor {
            ...
            } else // new signature would be String...args, int
            {
                if (lastNode instanceof NullLiteral) {
                    ...
                    if (expressions.isEmpty())
                        creation.dimensions().add(//$NON-NLS-1$
                        ast.newNumberLiteral(//$NON-NLS-1$
                        "0"));
                    else
    ...
    private final class RewriteParameterBody extends BodyUpdater {
        ...
        public void updateBody(MethodDeclaration methodDeclaration, final CompilationUnitRewrite cuRewrite, RefactoringStatus result) throws CoreException {
            ...
            if (// abstract methods don't have bodies
            body != null) {
    ...
    public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException, OperationCanceledException {
        ...
        for (Iterator<ParameterInfo> iter = parameterInfos.iterator(); iter.hasNext(); ) {
            ...
            if (!pi.isAdded()) {
                if (// may have been
                pi.getOldName().equals(pi.getNewName()))
                    // set to
                    // something
                    // else after
                    // creation
    ...
    protected boolean shouldReport(IProblem problem, CompilationUnit cu) {
        ...
        if (node instanceof Name) {
            ...
            if (problem.getID() == IProblem.MissingTypeInMethod) {
                ...
                if ((locationInParent == MethodInvocation.NAME_PROPERTY || locationInParent == SuperMethodInvocation.NAME_PROPERTY) && arguments.length > 3 && arguments[3].endsWith(getClassName()))
                    return false;
            }
        }
        return true;
    ...
    private void updateReferenceType() {
        if (fCreateAsTopLevel)
            fParameterObjectReference.setNewTypeName(JavaModelUtil.concatenateName(fParameterObjectFactory.getPackage(), fParameterObjectFactory.getClassName()));
        else
            fParameterObjectReference.setNewTypeName(JavaModelUtil.concatenateName(fParameterObjectFactory.getEnclosingType(), fParameterObjectFactory.getClassName()));
    }
