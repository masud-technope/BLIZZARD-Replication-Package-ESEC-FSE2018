...
public class InferTypeArgumentsConstraintCreator extends HierarchicalASTVisitor {
    ...
    public void endVisit(Assignment node) {
        ...
        if (//$NON-NLS-1$
        op == Assignment.Operator.PLUS_ASSIGN && (lhs.resolveTypeBinding() == node.getAST().resolveWellKnownType("java.lang.String"))) {
    ...
    public void endVisit(MethodInvocation node) {
        ...
        "getClass".equals(methodBinding.getName()) && methodBinding.getParameterTypes().length == 0) {
            ...
            if (wildcard.getBound() == null)
                ...
                return;
            ImmutableTypeVariable2 wildcardType = fTCModel.makeImmutableTypeVariable(/*no boxing*/
            wildcard, null);
    ...
    private boolean isSpecialCloneInvocation(IMethodBinding methodBinding, Expression receiver) {
        return fAssumeCloneReturnsSameType && "clone".equals(//$NON-NLS-1$
        methodBinding.getName()) && methodBinding.getParameterTypes().length == 0 && receiver != null && receiver.resolveTypeBinding() != methodBinding.getMethodDeclaration().getReturnType();
    }
