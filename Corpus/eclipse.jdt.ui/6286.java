...
public final class CompatibilityTemplateStore extends ContributionTemplateStore {
    ...
    public void load() throws IOException {
        ...
        if (fLegacySet != null) {
            ...
            for (Iterator<Template> it = legacyTemplates.listIterator(); it.hasNext(); ) {
                ...
                if (// no contributed match for the old template found
                orig == null) {
                    if (!isCodeTemplates())
    ...
    private static boolean isSimilar(Template t, Template orig, boolean isCodeTemplates) {
        return orig.getName().equals(t.getName()) && orig.getContextTypeId().equals(t.getContextTypeId()) && // only use description for templates (for, while...)
        (isCodeTemplates || orig.getDescription().equals(t.getDescription()));
    }
