        ...
        if (accessorBinding == null) {
            if (parent instanceof MethodInvocation) {
                ...
                if (args.size() != 1 && args.indexOf(nlsStringLiteral) != 0) {
                    // must be the only argument in lookup method
                    return null;
                }
                Expression firstArgument = args.get(0);
                ITypeBinding argumentBinding = firstArgument.resolveTypeBinding();
                if (//$NON-NLS-1$
                argumentBinding == null || !argumentBinding.getQualifiedName().equals("java.lang.String")) {
                    return null;
                }
                ITypeBinding typeBinding = methodInvocation.resolveTypeBinding();
                if (//$NON-NLS-1$
                typeBinding == null || !typeBinding.getQualifiedName().equals("java.lang.String")) {
                    return null;
                }
                IMethodBinding methodBinding = methodInvocation.resolveMethodBinding();
                if (methodBinding == null || !Modifier.isStatic(methodBinding.getModifiers())) {
                    // only static methods qualify
                    return null;
                }
    ...
    public static String getResourceBundleName(CompilationUnit astRoot) {
        ...
        astRoot.accept(new ASTVisitor() {
            ...
            public boolean visit(MethodInvocation node) {
                ...
                if (//old school //$NON-NLS-1$ //$NON-NLS-2$
                !("java.util.ResourceBundle".equals(name) && "getBundle".equals(method.getName()) && node.arguments().size() > 0) && //Eclipse style //$NON-NLS-1$ //$NON-NLS-2$
                !("org.eclipse.osgi.util.NLS".equals(name) && "initializeMessages".equals(method.getName()) && node.arguments().size() == 2))
                    return true;
        ...
        while (iter.hasNext()) {
            ...
            fieldName.equals("BUNDLE_NAME") || fieldName.equals("RESOURCE_BUNDLE") || fieldName.equals("bundleName")) {
