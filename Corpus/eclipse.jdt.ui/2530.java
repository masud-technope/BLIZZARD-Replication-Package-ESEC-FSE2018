...
public final class ASTProvider {
    ...
    private class ActivationListener implements IPartListener2, IWindowListener {
        ...
        public void partClosed(IWorkbenchPartReference ref) {
            if (isActiveEditor(ref)) {
                if (JavaPlugin.DEBUG_AST_PROVIDER)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "closed active editor: " + ref.getTitle());
        ...
        public void windowClosed(IWorkbenchWindow window) {
            if (fActiveEditor != null && fActiveEditor.getSite() != null && window == fActiveEditor.getSite().getWorkbenchWindow()) {
                if (JavaPlugin.DEBUG_AST_PROVIDER)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "closed active editor: " + fActiveEditor.getTitle());
                activeJavaEditorChanged(null);
            }
    ...
    void install() {
        ...
        for (int i = 0, length = windows.length; i < length; i++) windows[i].getPartService().addPartListener(fActivationListener);
    }
    ...
    void activeJavaEditorChanged(IWorkbenchPart editor) {
        ...
        if (JavaPlugin.DEBUG_AST_PROVIDER)
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "active editor is: " + toString(javaElement));
    ...
    void aboutToBeReconciled(ITypeRoot javaElement) {
        ...
        if (JavaPlugin.DEBUG_AST_PROVIDER)
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "about to reconcile: " + toString(javaElement));
        synchronized (fReconcileLock) {
    ...
    private synchronized void disposeAST() {
        ...
        if (JavaPlugin.DEBUG_AST_PROVIDER)
            ...
            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "disposing AST: " + toString(fAST) + " for: " + toString(fActiveJavaElement));
        fAST = null;
        cache(null, null);
    }
    ...
    private String toString(CompilationUnit ast) {
        ...
        if (types != null && types.size() > 0)
            //$NON-NLS-1$//$NON-NLS-2$
            return types.get(0).getName().getIdentifier() + "(" + ast.hashCode() + ")";
        else
            //$NON-NLS-1$
    ...
    private synchronized void cache(CompilationUnit ast, ITypeRoot javaElement) {
        if (fActiveJavaElement != null && !fActiveJavaElement.equals(javaElement)) {
            if (// don't report call from disposeAST()
            JavaPlugin.DEBUG_AST_PROVIDER && javaElement != null)
                //$NON-NLS-1$ //$NON-NLS-2$
                System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "don't cache AST for inactive: " + toString(javaElement));
            return;
        }
        if (// don't report call from disposeAST()
        JavaPlugin.DEBUG_AST_PROVIDER && (javaElement != null || ast != null))
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "caching AST: " + toString(ast) + " for: " + toString(javaElement));
        if (fAST != null)
            disposeAST();
        fAST = ast;
        // Signal AST change
            ...
            try {
                ...
                synchronized (this) {
                    if (activeElement == fActiveJavaElement && fAST != null) {
                        if (JavaPlugin.DEBUG_AST_PROVIDER)
                            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "...got AST: " + toString(fAST) + " for: " + input.getElementName());
                        return fAST;
                    }
                }
        ...
        SafeRunner.run(new ISafeRunnable() {
            ...
            public void run() {
                try {
                    ...
                    if (JavaPlugin.DEBUG_AST_PROVIDER)
                        //$NON-NLS-1$ //$NON-NLS-2$
                        System.err.println(getThreadName() + " - " + DEBUG_PREFIX + "creating AST for: " + input.getElementName());
            ...
            public void handleException(Throwable ex) {
                //$NON-NLS-1$
                IStatus status = new Status(IStatus.ERROR, JavaUI.ID_PLUGIN, IStatus.OK, "Error in JDT Core during AST creation", ex);
    ...
    void reconciled(CompilationUnit ast, ITypeRoot javaElement, IProgressMonitor progressMonitor) {
        if (JavaPlugin.DEBUG_AST_PROVIDER)
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "reconciled: " + toString(javaElement) + ", AST: " + toString(ast));
        synchronized (fReconcileLock) {
            fIsReconciling = false;
            if (javaElement == null || !javaElement.equals(fReconcilingJavaElement)) {
                if (JavaPlugin.DEBUG_AST_PROVIDER)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    System.out.println(getThreadName() + " - " + DEBUG_PREFIX + "  ignoring AST of out-dated editor");
