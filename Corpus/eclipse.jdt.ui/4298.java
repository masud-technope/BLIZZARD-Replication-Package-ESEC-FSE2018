    ...
    private static class TypeIndicatorOverlay extends CompositeImageDescriptor {
        ...
        protected void drawCompositeImage(int width, int height) {
            if (fDeprecated) {
                ...
                // looks better, esp. together with interface indicator
                drawImage(imageData, -1, 1);
            }
            if (// on top of deprecated indicator
            fType != null) {
    ...
    protected void processDelta(IJavaElementDelta delta, List<IJavaElement> result) {
        ...
        switch(elem.getElementType()) {
            case IJavaElement.JAVA_PROJECT:
                if (isRemoved || (isChanged && (flags & IJavaElementDelta.F_CLOSED) != 0)) {
                    return;
                }
                processChildrenDelta(delta, result);
                return;
            case IJavaElement.PACKAGE_FRAGMENT_ROOT:
                if (isRemoved || (isChanged && ((flags & IJavaElementDelta.F_ARCHIVE_CONTENT_CHANGED) != 0 || (flags & IJavaElementDelta.F_REMOVED_FROM_CLASSPATH) != 0))) {
                    return;
                }
                processChildrenDelta(delta, result);
                return;
