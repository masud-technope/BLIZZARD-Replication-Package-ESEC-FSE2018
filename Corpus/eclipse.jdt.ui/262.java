...
public class TypeContextChecker {
    ...
    private static class MethodTypesChecker {
        ...
        public RefactoringStatus[] checkAndResolveMethodTypes() throws CoreException {
            ...
            for (int i = 0; i < results.length; i++) if (results[i] != null && results[i].hasFatalError())
                return results;
            int parameterCount = fParameterInfos.size();
            String[] types = new String[parameterCount + 1];
            for (int i = 0; i < parameterCount; i++) types[i] = ParameterInfo.stripEllipsis((fParameterInfos.get(i)).getNewTypeName());
            ...
            for (int i = 0; i < types.length; i++) if (typeBindings[i] == null || !semanticsResults[i].isOK())
            ...
            try {
                ...
                for (int i = 0; i < types.length; i++) {
                    ...
                    if (problems.length > 0) {
                        for (int p = 0; p < problems.length; p++) if (isError(problems[p], type))
        ...
        private int appendMethodDeclaration(StringBuffer cuString, String[] types, int parameterCount) throws JavaModelException {
            ...
            for (int i = 0; i < parameterCount; i++) {
                if (i > 0)
                    cuString.append(',');
                //$NON-NLS-1$
                cuString.append(types[i]).append(" p").append(//$NON-NLS-1$
                i);
            }
            //$NON-NLS-1$
            cuString.append(");");
    ...
    public static StubTypeContext createAnnotationStubTypeContext(/*@NonNull*/
    IProject project) {
        try {
            for (IPackageFragmentRoot root : JavaCore.create(project).getPackageFragmentRoots()) {
                if (!root.isReadOnly()) {
                    IPackageFragment packageFragment = //$NON-NLS-1$
