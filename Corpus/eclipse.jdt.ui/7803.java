...
public class SpellingConfigurationBlock extends OptionsConfigurationBlock {
    ...
    private void createEncodingFieldEditor(Composite composite, List<Control> allControls) {
        ...
        //$NON-NLS-1$
        fEncodingEditor = new EncodingFieldEditor(PREF_SPELLING_USER_DICTIONARY_ENCODING.getName(), "", null, fEncodingEditorParent);
    ...
    protected void setEnabled(boolean enabled) {
        ...
        if (enabled && fEnabledControls != null) {
            for (int i = fEnabledControls.length - 1; i >= 0; i--) fEnabledControls[i].setEnabled(true);
