...
public class JavaSourceHover extends AbstractJavaEditorTextHover {
    ...
    private String getBracketHoverInfo(final ITextViewer textViewer, IRegion region) {
        ...
        try {
            ...
            if (ast == null)
                return null;
            ASTNode bracketNode = NodeFinder.perform(ast, match.getOffset(), match.getLength());
            if (bracketNode == null)
                return null;
            ASTNode node;
            ...
            if (parent instanceof IfStatement) {
                ...
                if (// if [else if]* else
                (parentIfStmt.getElseStatement() != null && ASTNodes.getInclusiveEnd(parentIfStmt.getElseStatement()) == offset) || // if [else if]+ else?
                (parentIfStmt.getLocationInParent() == IfStatement.ELSE_STATEMENT_PROPERTY && ASTNodes.getInclusiveEnd(parentIfStmt.getThenStatement()) == offset)) {
                    isElsePart = true;
            ...
            if ((line1 < topLine) || (wLine1 != -1 && (wLine2 - wLine1 != line2 - line1))) {
                ...
                if (isElsePart) {
                    // see bug 377141, 410650
                    return getBracketHoverInfo((IfStatement) node, bracketNode, document, editorInput, delim);
                }
                noOfSourceLines = 3;
        ...
        while (currentStatement != null) {
            ...
            if (currentStatement != bracketNode && ifNode != null && ifNode.getElseStatement() != null) {
                ...
                if (elseStartOffset != -1) {
                    line3 = // next 'else'
                    document.getLineOfOffset(// next 'else'
                    elseStartOffset);
                }
            }
                ...
                return new SourceViewerInformationControl(parent, isResizable, orientation, statusFieldText) {
                    ...
                    public void setLocation(Point location) {
                        ...
                        if (doShiftUp && fUpwardShiftInLines > 0) {
                            ...
                            //AbstractInformationControlManager.fMarginY = 5
                    ...
                    public Point computeSizeConstraints(int widthInChars, int heightInChars) {
                        if (doShiftUp && fUpwardShiftInLines > 0) {
                            ...
                            //set height as 0 to ensure selection of bottom anchor in AbstractInformationControlManager.computeInformationControlLocation(...)
                    ...
                    public IInformationControlCreator getInformationPresenterControlCreator() {
                        if (doShiftUp && fUpwardShiftInLines > 0) {
                            ...
                            for (StackTraceElement element : Thread.currentThread().getStackTrace()) {
                                if (//$NON-NLS-1$
                                "canMoveIntoInformationControl".equals(//$NON-NLS-1$
                                element.getMethodName()) && //$NON-NLS-1$
                                "org.eclipse.jface.text.AbstractHoverInformationControlManager".equals(//$NON-NLS-1$
                                element.getClassName()))
                                    //do not enrich bracket hover
                                    return null;
                            }
