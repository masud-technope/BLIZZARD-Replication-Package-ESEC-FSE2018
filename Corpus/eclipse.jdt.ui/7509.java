...
public class InvocationCountPerformanceMeter extends InternalPerformanceMeter {
    ...
    public class EventReader implements Runnable {
        ...
        private void enableBreakpoints() {
            for (int i = 0; i < fBreakpointRequests.length; i++) fBreakpointRequests[i].enable();
        }
        ...
        private void disableBreakpoints() {
            for (int i = 0; i < fBreakpointRequests.length; i++) fBreakpointRequests[i].disable();
        }
        ...
        private void handleBreakpointEvent(BreakpointEvent event) {
            ...
            if (fVerbose)
                try {
                    ...
                    String classKey = method.declaringType().name() + "#" + method.name() + //$NON-NLS-1$
                    method.signature();
                    //$NON-NLS-1$ //$NON-NLS-2$
                    String instanceKey = fCollectInstanceResults ? (thisObject.referenceType().name() + " (id=" + thisObject.uniqueID() + ")") : "all instances";
    ...
    public static class Results {
		 ...
		 */
        public void print() {
            for (Iterator<Object> iter = fResultsMap.keySet().iterator(); iter.hasNext(); ) print(iter.next());
        }
        ...
        public void print(Object key1) {
            ...
            for (Iterator<Object> iter = results.keySet().iterator(); iter.hasNext(); ) {
                ...
                //$NON-NLS-1$ //$NON-NLS-2$
                System.out.println("\t" + key2 + ": " + results.get(key2).intValue());
            }
        }
    }
    ...
    public void start() {
        try {
            ...
            for (int i = 0; i < fMethods.length; i++) requests.add(createBreakpointRequest(fMethods[i]));
            for (int i = 0; i < fConstructors.length; i++) requests.add(createBreakpointRequest(fConstructors[i]));
    ...
    private String getJNISignature(Class<?>[] paramTypes) {
        ...
        signature.append('(');
        for (int i = 0; i < paramTypes.length; ++i) signature.append(getJNISignature(paramTypes[i]));
        signature.append(')');
