...
public class JUnitPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
    ...
    private static class FilterLabelProvider extends LabelProvider implements ITableLabelProvider {
        ...
        public String getColumnText(Object object, int column) {
            //$NON-NLS-1$
            return (column == 0) ? getText(object) : "";
        }
        ...
        public Image getColumnImage(Object object, int column) {
            ...
            if (//$NON-NLS-1$
            name.indexOf(".*") != -1 || name.equals(JUnitMessages.JUnitMainTab_label_defaultpackage)) {
                //package
                return IMG_PKG;
            } else if (//$NON-NLS-1$
            "".equals(name)) {
    ...
    private void createFilterTable(Composite container) {
        ...
        @SuppressWarnings("unused") TableColumn tableColumn = new TableColumn(fFilterTable, SWT.NONE);
    ...
    private void editFilter() {
        ...
        "win32".equals(platform) || "gtk".equals(platform))
    ...
    private void addPackage() {
        ...
        //$NON-NLS-1$
        SelectionDialog dialog = JavaUI.createPackageDialog(shell, context, createWorkspaceScope, true, true, "");
    ...
    private void checkAllFilters(boolean check) {
        Object[] filters = fStackFilterContentProvider.getElements(null);
        for (int i = (filters.length - 1); i >= 0; --i) ((Filter) filters[i]).setChecked(check);
    ...
    private void setDefaultValues() {
        fEnableAssertionsCheckBox.setSelection(DefaultScope.INSTANCE.getNode(JUnitCorePlugin.CORE_PLUGIN_ID).getBoolean(JUnitPreferencesConstants.ENABLE_ASSERTIONS, JUnitPreferencesConstants.ENABLE_ASSERTIONS_DEFAULT));
        fShowInAllViewsCheckBox.setSelection(DefaultScope.INSTANCE.getNode(JUnitPlugin.PLUGIN_ID).getBoolean(JUnitUIPreferencesConstants.SHOW_IN_ALL_VIEWS, JUnitUIPreferencesConstants.SHOW_IN_ALL_VIEWS_DEFAULT));
    ...
    protected List<String> createInactiveStackFiltersList() {
        String[] strings = JUnitPreferencesConstants.parseList(getPreferenceStore().getString(JUnitPreferencesConstants.PREF_INACTIVE_FILTERS_LIST));
