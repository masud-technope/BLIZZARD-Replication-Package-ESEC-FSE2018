...
public class JavaHeuristicScannerTest extends TestCase {
    ...
    protected void setUp() {
        ...
        String[] types = new String[] { IJavaPartitions.JAVA_DOC, IJavaPartitions.JAVA_MULTI_LINE_COMMENT, IJavaPartitions.JAVA_SINGLE_LINE_COMMENT, IJavaPartitions.JAVA_STRING, IJavaPartitions.JAVA_CHARACTER, IDocument.DEFAULT_CONTENT_TYPE };
    ...
    public void testIndentation22() {
        ...
        fDocument.set("\tif (true)\n" + "\t\tif (true) {\n" + "\t\t\tstuff();" + "\t\t}\n" + "");
        String indent = fScanner.computeIndentation(fDocument.getLength()).toString();
        // because of possible dangling else
        assertEquals("\t", indent);
    }
    ...
    public void testClassInstanceCreationHeuristic() throws Exception {
        ...
        for (int offset = 0; offset < 15; offset++) assertFalse(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 15; offset < 19; offset++) assertTrue(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 19; offset < 20; offset++) assertFalse(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 20; offset < 24; offset++) assertTrue(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 24; offset < 25; offset++) assertFalse(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 25; offset < 34; offset++) assertTrue(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 34; offset < 57; offset++) assertFalse(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 57; offset < 60; offset++) assertTrue(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
        for (int offset = 60; offset < 63; offset++) assertFalse(fHeuristicScanner.looksLikeClassInstanceCreationBackward(offset, JavaHeuristicScanner.UNBOUND));
    }

    public void testConditional1() throws Exception {
        if (// XXX enable when https://bugs.eclipse.org/bugs/show_bug.cgi?id=65463 is fixed
        BUG_65463)
            return;
        fDocument.set("		public boolean isPrime() {\n" + "			return fPrime == true ? true\n" + "			                      : false;");
    ...
    public void testContinuationIndentationOfStrings1() throws Exception {
        ...
        // at the beginning of 4th line
        String indent = fScanner.computeIndentation(73).toString();
        assertEquals("\t\t\t", indent);
        // at the beginning of 5th line
    ...
    public void testContinuationIndentationOfStrings2() throws Exception {
        //Bug 338229
        fDocument.set("	System.out.println(\"Some\"\n" + "		+ new Object()\n" + "		+ \"string:\\n\" + definedType.toString());\n");
        //before the last +
        String indent = fScanner.computeIndentation(59).toString();
        assertEquals("\t\t", indent);
    }

    public void testContinuationIndentationOfStrings3() throws Exception {
        fDocument.set("	String test =\n" + "		\"this is the 1st string\"\n" + "		+ \"this is the 1st string\";\n");
        //at the beginning of 3rd line
        String indent = fScanner.computeIndentation(44).toString();
        assertEquals("\t\t\t", indent);
    }
