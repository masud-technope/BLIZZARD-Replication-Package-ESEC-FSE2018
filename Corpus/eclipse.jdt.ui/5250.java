...
public class TestRunSession implements ITestRunSession {
    ...
    public synchronized TestRoot getTestRoot() {
        //TODO: TestRoot should stay (e.g. for getTestRoot().getStatus())
        swapIn();
        return fTestRoot;
    }
    ...
    private File getSwapFile() throws IllegalStateException {
        ...
        //$NON-NLS-1$
        String isoTime = new SimpleDateFormat("yyyyMMdd-HHmmss.SSS").format(new Date(getStartTime()));
        //$NON-NLS-1$
    ...
    public boolean isKeptAlive() {
        if (fTestRunnerClient != null && fLaunch != null && fTestRunnerClient.isRunning() && ILaunchManager.DEBUG_MODE.equals(fLaunch.getLaunchMode())) {
    ...
    private class TestSessionNotifier implements ITestRunListener2 {
        ...
        private TestSuiteElement getUnrootedSuite() {
            if (fUnrootedSuite == null) {
                fUnrootedSuite = (TestSuiteElement) createTestElement(fTestRoot, "-2", JUnitMessages.TestRunSession_unrootedTests, true, 0);
            }
