...
public class IndentAction extends TextEditorAction {
    ...
    public void run() {
        ...
        if (document != null) {
            ...
            try {
                ...
                JavaPlugin.log(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "", e));
                return;
            }
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        JavaPlugin.log(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.OK, "ConcurrentModification in IndentAction", e));
                    } finally {
                        if (target != null)
    ...
    private static int countLeadingSlashPairs(IDocument document, int offset) throws BadLocationException {
        ...
        int pairCount = 0;
        while (//$NON-NLS-1$
        offset < max && document.get(offset, 2).equals("//")) {
            pairCount++;
            offset = offset + 2;
        }
        return pairCount;
    ...
    private static String computeJavadocIndent(IDocument document, int line, JavaHeuristicScanner scanner, ITypedRegion partition) throws BadLocationException {
        if (// impossible - the first line is never inside a javadoc comment
        line == 0)
            return null;
    ...
    private static int whiteSpaceLength(String indent, IJavaProject project) {
        if (indent == null)
            ...
            for (int i = 0; i < l; i++) size += indent.charAt(i) == '\t' ? tabSize : 1;
            return size;
        }
    }
    ...
    private boolean isValidSelection() {
        ...
        try {
            ...
            if (length == 0)
                ...
                return // only enable for empty selections for now
                false;
        } catch (BadLocationException e) {
        }
        return false;
