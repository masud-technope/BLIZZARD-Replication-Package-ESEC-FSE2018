...
public class ASTNodes {
    ...
    private static class ChildrenCollector extends GenericVisitor {
        ...
        protected boolean visitNode(ASTNode node) {
            if (// first visitNode: on the node's parent: do nothing, return true
            result == null) {
    ...
    public static boolean isLiteral(Expression expression) {
        ...
        return type == ASTNode.BOOLEAN_LITERAL || type == ASTNode.CHARACTER_LITERAL || type == ASTNode.NULL_LITERAL || type == ASTNode.NUMBER_LITERAL || type == ASTNode.STRING_LITERAL || type == ASTNode.TYPE_LITERAL;
    }

    public static boolean isLabel(SimpleName name) {
        int parentType = name.getParent().getNodeType();
        return parentType == ASTNode.LABELED_STATEMENT || parentType == ASTNode.BREAK_STATEMENT || parentType != ASTNode.CONTINUE_STATEMENT;
    }
    ...
    public static ITypeBinding getExplicitCast(Expression initializer, Expression reference) {
        ...
        } else if (// initializer is autoboxed
        initializerType.isPrimitive() && !referenceType.isPrimitive()) {
            ...
            if (!unboxedReferenceType.isEqualTo(initializerType))
                ...
                return referenceType;
        } else if (// initializer is autounboxed
        !initializerType.isPrimitive() && referenceType.isPrimitive()) {
            ...
            // don't lose the unchecked conversion
            return referenceType;
    ...
    private static boolean needsExplicitBoxing(Expression expression) {
        ...
        if (locationInParent == ClassInstanceCreation.EXPRESSION_PROPERTY || locationInParent == FieldAccess.EXPRESSION_PROPERTY || locationInParent == MethodInvocation.EXPRESSION_PROPERTY)
            return true;
        return false;
    }
            ...
            case ASTNode.FIELD_DECLARATION:
                if (Modifier.isStatic(modifiers)) {
                    ...
                    if (Modifier.isFinal(modifiers)) {
                        // first final static, then static
                        return index;
                    }
                    return index + 1;
                }
    ...
    public static int getInsertionIndex(BodyDeclaration member, List<? extends BodyDeclaration> container) {
        ...
        for (int i = containerSize - 1; i >= 0; i--) {
            ...
            if (orderIndex == currOrderIndex) {
                if (// no perfect match yet
                insertPosOrderIndex != orderIndex) {
                    // after a same kind
                    insertPos = i + 1;
                    // perfect match
                    insertPosOrderIndex = // perfect match
                    orderIndex;
                }
            } else if (// not yet a perfect match
            insertPosOrderIndex != orderIndex) {
                if (// we are bigger
                currOrderIndex < orderIndex) {
    ...
    public static Type getTopMostType(ASTNode node) {
        ASTNode result = null;
        while (node instanceof Type && !(node instanceof UnionType) || node instanceof Name || node instanceof Annotation || node instanceof MemberValuePair || // Expression could maybe be reduced to expression node types that can appear in an annotation
