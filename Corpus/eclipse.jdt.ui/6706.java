...
public class ConvertLineDelimitersProjectTest extends TextPerformanceTestCase {
    ...
    private static final ConvertLineDelimitersOperation[] OPERATIONS = new ConvertLineDelimitersOperation[] { new ConvertLineDelimitersOperation("\r"), new ConvertLineDelimitersOperation("\n"), new ConvertLineDelimitersOperation("\r\n") };
    ...
    private static IPath[] getLocations(IResource[] files) {
        ...
        for (int i = 0; i < locations.length; i++) locations[i] = files[i].getFullPath();
        return locations;
    }
}
