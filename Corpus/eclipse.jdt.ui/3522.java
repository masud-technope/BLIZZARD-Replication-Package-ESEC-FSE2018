...
public class FindStringsToExternalizeAction extends SelectionDispatchAction {
    ...
    private boolean computeEnablementState(IStructuredSelection selection) throws JavaModelException {
        ...
        for (Iterator<?> iter = selection.iterator(); iter.hasNext(); ) {
            ...
            if (elementType != IJavaElement.PACKAGE_FRAGMENT && elementType != IJavaElement.PACKAGE_FRAGMENT_ROOT && elementType != IJavaElement.JAVA_PROJECT)
                return false;
    ...
    public void run(final IStructuredSelection selection) {
        try {
            ...
            ExceptionHandler.handle(e, getShell(), ActionMessages.FindStringsToExternalizeAction_dialog_title, ActionMessages.FindStringsToExternalizeAction_error_message);
            return;
    ...
    private int countStrings() {
        int found = 0;
        if (fElements != null) {
            for (int i = 0; i < fElements.length; i++) found += fElements[i].count;
        }
        return found;
    }
    ...
    private int countNonExternalizedStrings(ICompilationUnit cu) throws CoreException {
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, Messages.format(ActionMessages.FindStringsToExternalizeAction_error_cannotBeParsed, BasicElementLabels.getFileName(cu)), e));
