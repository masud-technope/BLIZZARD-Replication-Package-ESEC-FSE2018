...
public class Implementors {
        ...
        if ((elements != null) && (elements.length > 0)) {
            ...
            try {
                if (element instanceof IMember) {
                    ...
                    if (type.isInterface()) {
                        IType[] implementingTypes = findImplementingTypes(type, progressMonitor);
    ...
    private IType[] findImplementingTypes(IType type, IProgressMonitor progressMonitor) {
        ...
        for (int i = 0; (i < finders.length) && !progressMonitor.isCanceled(); i++) {
            Collection<IType> types = finders[i].findImplementingTypes(type, new SubProgressMonitor(progressMonitor, 10, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
            if (types != null) {
    ...
    private IType[] findInterfaces(IType type, IProgressMonitor progressMonitor) {
        ...
        IImplementorFinder[] finders = getImplementorFinders();
        for (int i = 0; (i < finders.length) && !progressMonitor.isCanceled(); i++) {
            Collection<IType> types = finders[i].findInterfaces(type, new SubProgressMonitor(progressMonitor, 10, SubProgressMonitor.SUPPRESS_SUBTASK_LABEL));
            if (types != null) {
