...
public class JavaPlugin extends AbstractUIPlugin implements DebugOptionsListener {
    ...
    public void start(BundleContext context) throws Exception {
        ...
        if (PlatformUI.isWorkbenchRunning()) {
            ...
            // last call in start, see bug 191193
    ...
    private void ensurePreferenceStoreBackwardsCompatibility() {
        ...
        if (store.contains(JFaceResources.TEXT_FONT) && !store.isDefault(JFaceResources.TEXT_FONT)) {
            if (!store.getBoolean(fontPropagatedKey))
                PreferenceConverter.setValue(getDeprecatedWorkbenchPreferenceStore(), PreferenceConstants.EDITOR_TEXT_FONT, PreferenceConverter.getFontDataArray(store, JFaceResources.TEXT_FONT));
        }
        ...
        if (store.contains(DEPRECATED_CODEASSIST_ORDER_PROPOSALS)) {
            if (!store.getBoolean(proposalOrderMigrated)) {
                ...
                if (alphabetic)
                    store.setValue(PreferenceConstants.CODEASSIST_SORTER, "org.eclipse.jdt.ui.AlphabeticSorter");
            }
        }
        ...
        if (fJavaEditorTextHoverDescriptors == null) {
            ...
            for (int i = 0; i < fJavaEditorTextHoverDescriptors.length - 1; i++) {
                if (PreferenceConstants.ID_BESTMATCH_HOVER.equals(fJavaEditorTextHoverDescriptors[i].getId())) {
                    ...
                    for (int j = i; j > 0; j--) fJavaEditorTextHoverDescriptors[j] = fJavaEditorTextHoverDescriptors[j - 1];
    ...
    public void optionsChanged(DebugOptions options) {
        //$NON-NLS-1$
        DEBUG_AST_PROVIDER = options.getBooleanOption("org.eclipse.jdt.ui/debug/ASTProvider", false);
        //$NON-NLS-1$
        DEBUG_BREADCRUMB_ITEM_DROP_DOWN = options.getBooleanOption("org.eclipse.jdt.ui/debug/BreadcrumbItemDropDown", false);
        //$NON-NLS-1$
        DEBUG_TYPE_CONSTRAINTS = options.getBooleanOption("org.eclipse.jdt.ui/debug/TypeConstraints", false);
        //$NON-NLS-1$
        DEBUG_RESULT_COLLECTOR = options.getBooleanOption("org.eclipse.jdt.ui/debug/ResultCollector", false);
    }
}
