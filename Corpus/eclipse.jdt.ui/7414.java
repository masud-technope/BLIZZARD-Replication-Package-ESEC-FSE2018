...
public class Jsp2JavaReconcileStep extends AbstractReconcileStep {
    ...
    protected IReconcileResult[] reconcileModel(DirtyRegion dirtyRegion, IRegion subRegion) {
        //$NON-NLS-1$
        Assert.isTrue(getInputModel() instanceof DocumentAdapter, "wrong model");
        //$NON-NLS-1$
    ...
    protected IReconcileResult[] convertToInputModel(IReconcileResult[] inputResults) {
        ...
        for (int i = 0; i < inputResults.length; i++) {
            ...
            try {
                ...
                // document is 0-based, smap is 1-based
                int jspLine = smap[javaLine + 1];
                // Add Jsp line offset
                // document is 0-based, smap is 1-based
                pos.offset += ((DocumentAdapter) getInputModel()).getDocument().getLineOffset(jspLine - 1);
