...
public class PromoteTempToFieldRefactoring extends Refactoring {
    ...
    /*see INITIALIZE_IN_* constraints */
    ...
    public void setVisibility(int accessModifier) {
        Assert.isTrue(accessModifier == Modifier.PRIVATE || accessModifier == Modifier.NONE || accessModifier == Modifier.PROTECTED || accessModifier == Modifier.PUBLIC);
    ...
    public boolean canEnableSettingStatic() {
        return fInitializeIn != INITIALIZE_IN_CONSTRUCTOR && !isTempDeclaredInStaticMethod() && !fTempTypeUsesClassTypeVariables;
    }
    ...
    public boolean canEnableSettingDeclareInConstructors() {
        return !fDeclareStatic && !fInitializerUsesLocalTypes && !getMethodDeclaration().isConstructor() && !isDeclaredInAnonymousClass() && !isTempDeclaredInStaticMethod() && tempHasInitializer();
    }
    ...
    private static class LocalTypeAndVariableUsageAnalyzer extends HierarchicalASTVisitor {
        ...
        // List of IBinding (Variable and Type)
    ...
    private RefactoringStatus initialize(JavaRefactoringArguments arguments) {
        ...
        if (//$NON-NLS-1$
        initialize != null && !"".equals(initialize)) {
            int value = 0;
            try {
