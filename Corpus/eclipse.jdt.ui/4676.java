...
public class FilteredTypesSelectionDialog extends FilteredItemsSelectionDialog implements ITypeSelectionComponent {
    ...
    protected void fillContentProvider(AbstractContentProvider provider, ItemsFilter itemsFilter, IProgressMonitor progressMonitor) throws CoreException {
        ...
        try {
            engine.searchAllTypeNames(packPattern == null ? null : packPattern.toCharArray(), typeSearchFilter.getPackageFlags(), typePattern.toCharArray(), matchRule, typeSearchFilter.getElementKind(), typeSearchFilter.getSearchScope(), requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, progressMonitor);
        } finally {
    ...
    protected IStatus validateItem(Object item) {
        if (item == null)
            //$NON-NLS-1$
            return new Status(IStatus.ERROR, JavaPlugin.getPluginId(), IStatus.ERROR, "", null);
        if (fValidator != null) {
    ...
    private static class ConsistencyRunnable implements IRunnableWithProgress {
        ...
        private void refreshSearchIndices(IProgressMonitor monitor) throws InvocationTargetException {
            try {
                ...
                //$NON-NLS-1$
                "_______________".toCharArray(), SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE, IJavaSearchConstants.ENUM, SearchEngine.createWorkspaceScope(), new /* dummy */
                TypeNameRequestor() {
    ...
    private class TypeItemLabelProvider extends LabelProvider implements ILabelDecorator, IStyledLabelProvider {
        ...
        public StyledString getStyledText(Object element) {
            ...
            if (//$NON-NLS-1$
            namePattern != null && !"*".equals(namePattern)) {
            ...
            if (index != -1) {
                ...
                if (//$NON-NLS-1$
                packagePattern != null && !"*".equals(packagePattern)) {
            ...
            if (installType != null) {
                ...
                for (int i = 0; i < installs.length; i++) {
                    ...
                    if (libLocations != null) {
                        ...
                        if (isMac && "Home".equals(//$NON-NLS-1$
    ...
    protected class TypeSelectionHistory extends SelectionHistory {
        ...
        public void load(IMemento memento) {
            TypeNameMatch[] types = OpenTypeHistory.getInstance().getTypeInfos();
            for (// see https://bugs.eclipse.org/bugs/show_bug.cgi?id=205314
            int i = types.length - 1; // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=205314
            i >= 0; // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=205314
            i--) {
                TypeNameMatch type = types[i];
                accessed(type);
            }
