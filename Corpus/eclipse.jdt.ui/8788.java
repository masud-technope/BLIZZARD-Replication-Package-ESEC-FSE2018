...
public class TestRunnerViewPart extends ViewPart {
    ...
    private static class JUnitPasteAction extends Action {
        ...
        public void run() {
            ...
            MessageDialog.openInformation(fShell, JUnitMessages.TestRunnerViewPart_JUnitPasteAction_cannotpaste_title, JUnitMessages.TestRunnerViewPart_JUnitPasteAction_cannotpaste_message);
        }

        private boolean isValidUrl(String urlData) {
            try {
                @SuppressWarnings("unused") URL url = new URL(urlData);
        ...
        private static class URLValidator implements IInputValidator {
            ...
            public String isValid(String newText) {
                if (newText.length() == 0)
                    return null;
                try {
                    @SuppressWarnings("unused") URL url = new URL(newText);
                    return null;
                } catch (MalformedURLException e) {
        ...
        public void run() {
            ...
            InputDialog inputDialog = new InputDialog(fShell, title, message, url, validator) {
                ...
                protected IDialogSettings getDialogBoundsSettings() {
                    ...
                    settings.put("DIALOG_HEIGHT", //$NON-NLS-1$
                    Dialog.DIALOG_DEFAULT_BOUNDS);
                    return settings;
                }
    ...
    private static class ExportTestRunSessionAction extends Action {
        ...
        private String getFileName() {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            return testRunName + " " + isoTime + ".xml";
        }
    }
    ...
    private class TestRunSessionListener implements ITestRunSessionListener {
        ...
        public void sessionAdded(final TestRunSession testRunSession) {
            getDisplay().asyncExec(new Runnable() {
                ...
                @Override
                public void run() {
                    if (JUnitUIPreferencesConstants.getShowInAllViews() || getSite().getWorkbenchWindow() == JUnitPlugin.getActiveWorkbenchWindow()) {
    ...
    private class ToggleOrientationAction extends Action {
        ...
        public  ToggleOrientationAction(int orientation) {
            ...
            if (orientation == TestRunnerViewPart.VIEW_ORIENTATION_HORIZONTAL) {
                ...
                setImageDescriptor(//$NON-NLS-1$
                JUnitPlugin.getImageDescriptor("elcl16/th_horizontal.gif"));
                ...
                setImageDescriptor(//$NON-NLS-1$
                JUnitPlugin.getImageDescriptor("elcl16/th_vertical.gif"));
                ...
                setImageDescriptor(//$NON-NLS-1$
                JUnitPlugin.getImageDescriptor("elcl16/th_automatic.gif"));
            }
    ...
    private class IgnoredOnlyFilterAction extends Action {
        ...
        public  IgnoredOnlyFilterAction() {
            ...
            //$NON-NLS-1$
            setImageDescriptor(JUnitPlugin.getImageDescriptor("obj16/testignored.gif"));
        }
    ...
    public  TestRunnerViewPart() {
        ...
        //$NON-NLS-1$
        fStackViewIcon = createManagedImage("eview16/stackframe.gif");
        //$NON-NLS-1$
        fTestRunOKIcon = createManagedImage("eview16/junitsucc.gif");
        //$NON-NLS-1$
        fTestRunFailIcon = createManagedImage("eview16/juniterr.gif");
        //$NON-NLS-1$
        fTestRunOKDirtyIcon = createManagedImage("eview16/junitsuccq.gif");
        //$NON-NLS-1$
        fTestRunFailDirtyIcon = createManagedImage("eview16/juniterrq.gif");
        //$NON-NLS-1$
        fTestIcon = createManagedImage("obj16/test.gif");
        //$NON-NLS-1$
        fTestOkIcon = createManagedImage("obj16/testok.gif");
        //$NON-NLS-1$
        fTestErrorIcon = createManagedImage("obj16/testerr.gif");
        //$NON-NLS-1$
        fTestFailIcon = createManagedImage("obj16/testfail.gif");
        //$NON-NLS-1$
        fTestRunningIcon = createManagedImage("obj16/testrun.gif");
        //$NON-NLS-1$
        fTestIgnoredIcon = createManagedImage("obj16/testignored.gif");
        //$NON-NLS-1$
        fTestAssumptionFailureIcon = createManagedImage("obj16/testassumptionfailed.gif");
    ...
    public void saveState(IMemento memento) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        memento.putString(TAG_FAILURES_ONLY, fFailuresOnlyFilterAction.isChecked() ? "true" : "false");
        //$NON-NLS-1$ //$NON-NLS-2$
        memento.putString(TAG_IGNORED_ONLY, fIgnoredOnlyFilterAction.isChecked() ? "true" : "false");
        memento.putInteger(TAG_LAYOUT, fLayout);
        //$NON-NLS-1$ //$NON-NLS-2$
        memento.putString(TAG_SHOW_TIME, fShowTimeAction.isChecked() ? "true" : "false");
    }
    ...
    private ILaunchConfiguration prepareLaunchConfigForRelaunch(ILaunchConfiguration configuration) {
        try {
            //$NON-NLS-1$
            String attribute = configuration.getAttribute(JUnitLaunchConfigurationConstants.ATTR_FAILURES_NAMES, "");
            if (attribute.length() != 0) {
                ...
                //$NON-NLS-1$
                tmp.setAttribute(//$NON-NLS-1$
                JUnitLaunchConfigurationConstants.ATTR_FAILURES_NAMES, //$NON-NLS-1$
                "");
                return tmp;
            }
        } catch (CoreException e) {
    ...
    private void updateViewTitleProgress() {
        if (fTestRunSession != null) {
            if (fTestRunSession.isRunning()) {
                Image progress = fProgressImages.getImage(fTestRunSession.getStartedCount(), fTestRunSession.getTotalCount(), fTestRunSession.getErrorCount(), fTestRunSession.getFailureCount());
 ...
 */
    ...
    private SashForm createSashForm(Composite parent) {
        ...
        empty.setLayout(new Layout() {
            ...
            protected Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
                // (0, 0) does not work with super-intelligent ViewForm
    ...
    void setShowFailuresOnly(boolean failuresOnly) {
        setFilterAndLayout(failuresOnly, /*ignoredOnly must be off*/
        false, fLayout);
    }

    void setShowIgnoredOnly(boolean ignoredOnly) {
        setFilterAndLayout(/*failuresOnly must be off*/
        false, ignoredOnly, fLayout);
    }
