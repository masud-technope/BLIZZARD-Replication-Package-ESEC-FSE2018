...
public class IntroduceIndirectionRefactoring extends Refactoring {
            ...
            if (fIntermediaryType != null) {
                ...
                for (int i = 0; i < toCheck.length; i++) {
                    ...
                    if (method.getElementName().equals(fIntermediaryMethodName))
                        return RefactoringStatus.createWarningStatus(Messages.format(RefactoringCoreMessages.IntroduceIndirectionRefactoring_duplicate_method_name_in_declaring_type_error, BasicElementLabels.getJavaElementName(fIntermediaryMethodName)));
                }
            }
    ...
    public String getIntermediaryTypeName() {
        //$NON-NLS-1$
        return fIntermediaryType != null ? fIntermediaryType.getFullyQualifiedName('.') : "";
    }
    ...
    private RefactoringStatus updateReferences(IProgressMonitor monitor) throws CoreException {
        ...
        for (int i = 0; i < references.length; i++) {
            ...
            for (int j = 0; j < searchResults.length; j++) {
                ...
                //$NON-NLS-1$
                Assert.isTrue(//$NON-NLS-1$
                target instanceof MethodInvocation, //$NON-NLS-1$
                "Element of call should be a MethodInvocation.");
    ...
    private RefactoringStatus findCommonParent(ITypeBinding typeBinding) {
        ...
        for (int i1 = 0; i1 < currentAndSupers.length; i1++) {
            for (int i2 = 0; i2 < highestAndSupers.length; i2++) {
                if (highestAndSupers[i2].isEqualTo(currentAndSupers[i1]) && (Bindings.findMethodInHierarchy(highestAndSupers[i2], fTargetMethodBinding.getName(), fTargetMethodBinding.getParameterTypes()) != null)) {
    ...
    private ITypeBinding getExpressionType(MethodInvocation invocation) {
        ...
        if (typeBinding != null && typeBinding.isTypeVariable()) {
            ...
            if (typeBounds.length > 0) {
                ...
                invocation.getAST().resolveWellKnownType(//$NON-NLS-1$
                "java.lang.Object");
            }
        }
        //$NON-NLS-1$
