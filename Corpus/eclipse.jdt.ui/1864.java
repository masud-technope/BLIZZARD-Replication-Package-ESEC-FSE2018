...
public final class JavaPairMatcher extends DefaultCharacterPairMatcher implements ISourceVersionDependent {
    ...
    private IRegion performMatch(IDocument document, int offset) throws BadLocationException {
        ...
        if (//https://bugs.eclipse.org/bugs/show_bug.cgi?id=372516
        prevChar == '>' && currChar != '>') {
            offset--;
            currChar = prevChar;
    ...
    private boolean isTypeParameterClosingBracket(int offset, IDocument document, JavaHeuristicScanner scanner) {
        try {
            ...
            int prevTokenOffset = scanner.getPosition() + 1;
            String previous = prevToken == Symbols.TokenEOF ? null : document.get(prevTokenOffset, offset - prevTokenOffset).trim();
            if ((prevToken == Symbols.TokenIDENT && (previous.length() > 0 && Character.isUpperCase(previous.charAt(0)))) || prevToken == Symbols.TokenEOF || prevToken == Symbols.TokenGREATERTHAN || prevToken == Symbols.TokenQUESTIONMARK)
                return true;
        } catch (BadLocationException e) {
            return false;
        }
