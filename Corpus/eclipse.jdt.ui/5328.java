...
public abstract class AbstractInformationControl extends PopupDialog implements IInformationControl, IInformationControlExtension, IInformationControlExtension2, DisposeListener {
    ...
    protected class NamePatternFilter extends ViewerFilter {
        ...
        private boolean hasUnfilteredChild(TreeViewer viewer, TreePath parentPath, Object element) {
            if (element instanceof IParent) {
                ...
                Object[] children = contentProvider instanceof ITreePathContentProvider ? ((ITreePathContentProvider) contentProvider).getChildren(elementPath) : ((ITreeContentProvider) contentProvider).getChildren(element);
                for (int i = 0; i < children.length; i++) if (selectTreePath(viewer, elementPath, children[i]))
                    return true;
            }
            return false;
        }
    ...
    protected Control createTitleMenuArea(Composite parent) {
        ...
        fShowViewMenuAction = new //$NON-NLS-1$
        Action(//$NON-NLS-1$
        "showViewMenu") {
