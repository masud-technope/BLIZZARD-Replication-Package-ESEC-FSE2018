...
abstract class JavaBrowsingPart extends ViewPart implements IMenuListener, IViewPartInputProvider, INullSelectionListener {
    ...
    public void createPartControl(Composite parent) {
        ...
        if (// initialize linking state before creating the actions
        fMemento != null) {
    ...
    private void addOpenNewWindowAction(IMenuManager menu, Object element) {
        ...
        if (!(element instanceof IContainer))
            return;
        menu.appendToGroup(IContextMenuConstants.GROUP_OPEN, new PatchedOpenInNewWindowAction(getSite().getWorkbenchWindow(), (IContainer) element));
    }

    protected void createActions() {
        fActionGroups = new CompositeActionGroup(new ActionGroup[] { new NewWizardsActionGroup(this.getSite()), fOpenEditorGroup = new OpenEditorActionGroup(this), new OpenViewActionGroup(this), fCCPActionGroup = new CCPActionGroup(this), new GenerateActionGroup(this), new RefactorActionGroup(this), new ImportActionGroup(this), fBuildActionGroup = new BuildActionGroup(this), new JavaSearchActionGroup(this) });
        if (fHasWorkingSetFilter) {
            //$NON-NLS-1$
	 ...
	 */
    protected abstract boolean isValidInput(Object element);
    ...
    protected JavaUILabelProvider createLabelProvider() {
        return new AppearanceAwareLabelProvider(AppearanceAwareLabelProvider.DEFAULT_TEXTFLAGS, AppearanceAwareLabelProvider.DEFAULT_IMAGEFLAGS | JavaElementImageProvider.SMALL_ICONS);
    }
    ...
    protected int getLabelProviderFlags() {
        return JavaElementLabelProvider.SHOW_BASICS | JavaElementLabelProvider.SHOW_OVERLAY_ICONS | JavaElementLabelProvider.SHOW_SMALL_ICONS | JavaElementLabelProvider.SHOW_VARIABLE | JavaElementLabelProvider.SHOW_PARAMETERS;
    }
    ...
    private boolean mustSetNewInput(IJavaElement elementToSelect, IJavaElement oldInput, IJavaElement newInput) {
        return (newInput == null || !newInput.equals(oldInput)) && (elementToSelect == null || oldInput == null || (!((elementToSelect instanceof IPackageDeclaration) && (elementToSelect.getParent().equals(oldInput.getParent())) && (!isAncestorOf(getViewPartInput(), elementToSelect)))));
    }
	 ...
	 */
    protected abstract IJavaElement findElementToSelect(IJavaElement je);
