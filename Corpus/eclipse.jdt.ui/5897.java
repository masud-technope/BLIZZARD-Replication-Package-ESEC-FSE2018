...
public class QuickFixProcessor implements IQuickFixProcessor {
    ...
    public boolean hasCorrections(ICompilationUnit cu, int problemId) {
        switch(problemId) {
            ...
            case IProblem.InheritedDefaultMethodConflictsWithOtherInherited:
                return true;
            default:
                return SuppressWarningsSubProcessor.hasSuppressWarningsProposal(cu.getJavaProject(), problemId) || ConfigureProblemSeveritySubProcessor.hasConfigureProblemSeverityProposal(problemId);
        }
    }
    ...
    private void process(IInvocationContext context, IProblemLocation problem, Collection<ICommandAccess> proposals) throws CoreException {
        ...
        if (// no proposals for none-problem locations
        id == 0) {
            return;
        }
        switch(id) {
            case IProblem.UnterminatedString:
                ...
                int pos = moveBack(//$NON-NLS-1$
                problem.getOffset() + problem.getLength(), //$NON-NLS-1$
                problem.getOffset(), //$NON-NLS-1$
                "\n\r", //$NON-NLS-1$
                context.getCompilationUnit());
                //$NON-NLS-1$
                proposals.add(//$NON-NLS-1$
                new ReplaceCorrectionProposal(quoteLabel, context.getCompilationUnit(), pos, 0, "\"", IProposalRelevance.ADD_QUOTE));
                break;
            case IProblem.UnusedImport:
            case IProblem.DuplicateImport:
            case IProblem.CannotImportPackage:
