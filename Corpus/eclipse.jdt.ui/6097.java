...
public class OptionsConfigurationBlockTest extends TestCase {
    ...
    public void testKeysForOptions() throws Exception {
        ...
        assertEquals("Core constants missing in the UI", Collections.EMPTY_MAP.toString(), coreFieldLookup.toString().replace(',', '\n'));
    }
    ...
    private void removeUnusedOptions(HashMap<String, String> coreFieldLookup) {
        coreFieldLookup.keySet().removeAll(Arrays.asList(new String[] { JavaCore.COMPILER_PB_INCONSISTENT_NULL_CHECK, JavaCore.COMPILER_PB_INVALID_IMPORT, JavaCore.COMPILER_PB_UNREACHABLE_CODE, JavaCore.COMPILER_PB_UNSAFE_TYPE_OPERATION, JavaCore.COMPILER_PB_BOOLEAN_METHOD_THROWING_EXCEPTION, // gets set directly, based on org.eclipse.jdt.ui.PreferenceConstants.CODEASSIST_SHOW_VISIBLE_PROPOSALS
        JavaCore.CODEASSIST_VISIBILITY_CHECK, // useless copy of option from ResourcesPlugin
        JavaCore.CORE_ENCODING, // not relevant any more, since project references are not necessary for build path
        JavaCore.CORE_JAVA_BUILD_ORDER, // not useful without suppressWarnings for 1.4: https://bugs.eclipse.org/bugs/show_bug.cgi?id=156736
        JavaCore.COMPILER_PB_OVERRIDING_METHOD_WITHOUT_SUPER_INVOCATION, // maybe for 1.7
        JavaCore.COMPILER_PB_UNUSED_TYPE_ARGUMENTS_FOR_METHOD_INVOCATION, // TODO: not used: bug?
        JavaCore.CODEASSIST_IMPLICIT_QUALIFICATION, // default is good (don't flag trivial 'if (DEBUG)')
        JavaCore.COMPILER_PB_DEAD_CODE_IN_TRIVIAL_IF_STATEMENT, // is on the Type Filters page now, see https://bugs.eclipse.org/218487
        JavaCore.CODEASSIST_DISCOURAGED_REFERENCE_CHECK, // is on the Type Filters page now, see https://bugs.eclipse.org/218487
        JavaCore.CODEASSIST_FORBIDDEN_REFERENCE_CHECK }));
    }
        ...
        for (int i = 0; i < prefFields.length; i++) {
            ...
            if (field.getType() == Key.class) {
                ...
                if (JavaCore.PLUGIN_ID.equals(key.getQualifier())) {
                    ...
                    assertTrue("No core constant for key " + key.getName() + " in class " + configurationBlock.getName(), fieldName != null);
