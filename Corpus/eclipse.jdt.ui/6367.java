 ...
 * @since 3.2
 */
public abstract class AbstractJavaCompletionProposal implements IJavaCompletionProposal, ICompletionProposalExtension, ICompletionProposalExtension2, ICompletionProposalExtension3, ICompletionProposalExtension5, ICompletionProposalExtension6, ICompletionProposalExtension7 {
    ...
    public void apply(IDocument document, char trigger, int offset) {
        ...
        try {
            ...
            if (isSmartTrigger) {
                ...
                if (//$NON-NLS-1$
                !(trigger == ';' && (replacement.endsWith(";") || document.getChar(referenceOffset) == ';'))) {
    ...
    private boolean isSmartTrigger(char trigger) {
        return trigger == ';' && JavaPlugin.getDefault().getCombinedPreferenceStore().getBoolean(PreferenceConstants.EDITOR_SMART_SEMICOLON) || trigger == '{' && JavaPlugin.getDefault().getCombinedPreferenceStore().getBoolean(PreferenceConstants.EDITOR_SMART_OPENING_BRACE);
    }
            ...
            if (trigger != '\0') {
                try {
                    ...
                    if (trigger == '(' && autocloseBrackets()) {
                        //$NON-NLS-1$
                        document.replace(//$NON-NLS-1$
                        getReplacementOffset() + getCursorPosition(), //$NON-NLS-1$
                        0, //$NON-NLS-1$
                        ")");
    ...
    protected final String getCamelCaseCompound(String prefix, String string) {
        ...
        for (int i = 1; i <= stringChars.length; i++) if (CharOperation.camelCaseMatch(patternChars, 0, patternChars.length, stringChars, 0, i))
