...
public class AddBlockCommentAction extends BlockCommentAction {
    ...
    private ITypedRegion handleInteriorPartition(ITypedRegion partition, List<Edit> edits, Edit.EditFactory factory, IDocumentExtension3 docExtension) throws BadPartitioningException, BadLocationException {
        ...
        // true if the previous partition is javadoc
        ...
        } else // !wasJavadoc
        {
            if (partType == IJavaPartitions.JAVA_DOC) {
                ...
                //$NON-NLS-1$
                edits.add(//$NON-NLS-1$
                factory.createEdit(partition.getOffset(), getCommentStart().length(), ""));
            }
        }
        return partition;
    }
    ...
    private boolean isSpecialPartition(String partType) {
        return partType == IJavaPartitions.JAVA_CHARACTER || partType == IJavaPartitions.JAVA_STRING || partType == IJavaPartitions.JAVA_SINGLE_LINE_COMMENT;
    }
