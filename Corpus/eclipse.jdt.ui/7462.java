...
public class ComplianceConfigurationBlock extends OptionsConfigurationBlock {
	 ...
	 */
    //$NON-NLS-1$
    private static final Key INTR_COMPLIANCE_FOLLOWS_EE = getLocalKey("internal.compliance.follows.ee");
	 ...
	 */
    //$NON-NLS-1$
    private static final Key INTR_DEFAULT_COMPLIANCE = getLocalKey("internal.default.compliance");
    ...
    private static final Key[] PREFS_COMPLIANCE = new Key[] { PREF_COMPLIANCE, PREF_PB_ASSERT_AS_IDENTIFIER, PREF_PB_ENUM_AS_IDENTIFIER, PREF_SOURCE_COMPATIBILITY, PREF_CODEGEN_TARGET_PLATFORM };
    ...
    public  ComplianceConfigurationBlock(IStatusChangeListener context, IProject project, IWorkbenchPreferenceContainer container) {
        ...
        fRememberedUserCompliance = new // caution: order depends on IDX_* constants
        String[] { getValue(PREF_PB_ASSERT_AS_IDENTIFIER), getValue(PREF_PB_ENUM_AS_IDENTIFIER), getValue(PREF_SOURCE_COMPATIBILITY), getValue(PREF_CODEGEN_TARGET_PLATFORM), getValue(PREF_COMPLIANCE), getValue(PREF_CODEGEN_INLINE_JSR_BYTECODE), getValue(PREF_CODEGEN_METHOD_PARAMETERS_ATTR) };
    }
    ...
    private Composite createComplianceTabContent(Composite folder) {
        ...
        String[] values3456Labels = new String[] { PreferencesMessages.ComplianceConfigurationBlock_version13, PreferencesMessages.ComplianceConfigurationBlock_version14, PreferencesMessages.ComplianceConfigurationBlock_version15, PreferencesMessages.ComplianceConfigurationBlock_version16, PreferencesMessages.ComplianceConfigurationBlock_version17, PreferencesMessages.ComplianceConfigurationBlock_version18 };
        ...
        String[] errorWarningInfoIgnoreLabels = new String[] { PreferencesMessages.ComplianceConfigurationBlock_error, PreferencesMessages.ComplianceConfigurationBlock_warning, PreferencesMessages.ComplianceConfigurationBlock_info, PreferencesMessages.ComplianceConfigurationBlock_ignore };
    ...
    protected void validateSettings(Key changedKey, String oldValue, String newValue) {
        ...
        if (changedKey != null) {
            if (INTR_DEFAULT_COMPLIANCE.equals(changedKey)) {
                ...
                updateAssertEnumAsIdentifierEnableState();
                fComplianceStatus = validateCompliance();
            } else if (PREF_PB_ENUM_AS_IDENTIFIER.equals(changedKey) || PREF_PB_ASSERT_AS_IDENTIFIER.equals(changedKey)) {
                fComplianceStatus = validateCompliance();
            } else {
                return;
            }
    ...
    public void refreshComplianceSettings() {
        if (fOriginalStoredCompliance == null) {
            fOriginalStoredCompliance = new // caution: order depends on IDX_* constants
            String[] { getOriginalStoredValue(PREF_PB_ASSERT_AS_IDENTIFIER), getOriginalStoredValue(PREF_PB_ENUM_AS_IDENTIFIER), getOriginalStoredValue(PREF_SOURCE_COMPATIBILITY), getOriginalStoredValue(PREF_CODEGEN_TARGET_PLATFORM), getOriginalStoredValue(PREF_COMPLIANCE), getOriginalStoredValue(PREF_CODEGEN_INLINE_JSR_BYTECODE), getOriginalStoredValue(PREF_CODEGEN_METHOD_PARAMETERS_ATTR) };
        } else {
            String[] storedCompliance = new String[] { getOriginalStoredValue(PREF_PB_ASSERT_AS_IDENTIFIER), getOriginalStoredValue(PREF_PB_ENUM_AS_IDENTIFIER), getOriginalStoredValue(PREF_SOURCE_COMPATIBILITY), getOriginalStoredValue(PREF_CODEGEN_TARGET_PLATFORM), getOriginalStoredValue(PREF_COMPLIANCE), getOriginalStoredValue(PREF_CODEGEN_INLINE_JSR_BYTECODE), getOriginalStoredValue(PREF_CODEGEN_METHOD_PARAMETERS_ATTR) };
    ...
    private void validateComplianceStatus() {
        if (fJRE50InfoText != null && !fJRE50InfoText.isDisposed()) {
            ...
            // get actual value
            String compliance = getStoredValue(PREF_COMPLIANCE);
            IVMInstall install = null;
            if (// project specific settings: only test if a 50 JRE is installed
            fProject != null) {
                try {
            ...
            if (install instanceof IVMInstall2) {
                ...
                if (// Discourage using compiler with version other than compliance
                !compilerCompliance.equals(compliance)) {
    ...
    private static final boolean isOriginalDefaultCompliance() {
        ...
        return equals(JavaCore.COMPILER_COMPLIANCE, bundleDefaults, options) && equals(JavaCore.COMPILER_SOURCE, bundleDefaults, options) && equals(JavaCore.COMPILER_CODEGEN_TARGET_PLATFORM, bundleDefaults, options) && equals(JavaCore.COMPILER_PB_ASSERT_IDENTIFIER, bundleDefaults, options) && equals(JavaCore.COMPILER_PB_ENUM_IDENTIFIER, bundleDefaults, options);
    }
    ...
    private static boolean equals(String key, Preferences preferences, Map<String, String> map) {
        ...
        return defaultValue != null && defaultValue != dummy ? map.containsKey(key) && equals(defaultValue, map.get(key)) : !map.containsKey(key);
    }
