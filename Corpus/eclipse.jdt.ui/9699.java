...
public final class MoveInstanceMethodProcessor extends MoveProcessor implements IDelegateUpdating {
    ...
    protected static boolean isFieldAccess(final SimpleName name) {
        ...
        if (!variable.isField())
            return false;
        if (//$NON-NLS-1$
        "length".equals(name.getIdentifier())) {
            final ASTNode parent = name.getParent();
    ...
    protected void checkConflictingMethod(final IProgressMonitor monitor, final RefactoringStatus status) throws JavaModelException {
        ...
        if (!fTarget.isField())
            ...
            newParamCount--;
        if (needsTargetNode())
            // will add a parameter for the old 'this'
            newParamCount++;
        try {
            //$NON-NLS-1$
	 ...
	 */
    protected boolean createInlinedMethodInvocation(CompilationUnitRewrite rewriter, MethodDeclaration declaration, SearchMatch match, Map<IMember, IncomingMemberVisibilityAdjustment> adjustments, RefactoringStatus status) throws JavaModelException {
        ...
        if (node instanceof SuperMethodInvocation) {
            ...
            if (fTarget.isField()) {
                ...
                if (bindings.length > 0) {
                    int index = 0;
                    for (; index < bindings.length; index++) if (Bindings.equals(bindings[index], fTarget))
                        break;
    ...
    protected String createMethodContent(final IDocument document, final MethodDeclaration declaration, final ASTRewrite rewrite) throws BadLocationException {
        ...
        //$NON-NLS-1$
        return Strings.changeIndent(document.get(marker.getOffset(), marker.getLength()), Strings.computeIndentUnits(document.get(region.getOffset(), region.getLength()), project), project, "", TextUtilities.getDefaultLineDelimiter(document));
    }
    ...
    protected void createMethodCopy(IDocument document, MethodDeclaration declaration, ASTRewrite rewrite, Map<ICompilationUnit, CompilationUnitRewrite> rewrites, Map<IMember, IncomingMemberVisibilityAdjustment> adjustments, RefactoringStatus status, IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            if (binding != null) {
                ...
                } else if (MemberVisibilityAdjustor.hasLowerVisibility(binding.getModifiers(), same ? Modifier.NONE : keyword == null ? Modifier.NONE : keyword.toFlagValue()) && MemberVisibilityAdjustor.needsVisibilityAdjustments(fMethod, keyword, adjustments)) {
            ...
            for (IExtendedModifier modifier : (List<IExtendedModifier>) declaration.modifiers()) {
                if (modifier.isAnnotation()) {
                    ...
                    if (typeBinding != null && typeBinding.getQualifiedName().equals("java.lang.Override")) {
