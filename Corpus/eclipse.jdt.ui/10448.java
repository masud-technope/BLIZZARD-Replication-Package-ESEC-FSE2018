...
public class RemoteTestRunnerClient {
        ...
        public void run() {
            try {
                if (fDebug)
                    ...
                    "Creating server socket " + //$NON-NLS-1$
                    fServerPort);
                fServerSocket = new ServerSocket(fServerPort);
                fSocket = fServerSocket.accept();
                try {
                    fPushbackReader = new //$NON-NLS-1$
                    PushbackReader(//$NON-NLS-1$
                    new BufferedReader(new InputStreamReader(fSocket.getInputStream(), "UTF-8")));
                } catch (UnsupportedEncodingException e) {
                    fPushbackReader = new PushbackReader(new BufferedReader(new InputStreamReader(fSocket.getInputStream())));
                }
                try {
                    fWriter = new //$NON-NLS-1$
                    PrintWriter(//$NON-NLS-1$
                    new OutputStreamWriter(fSocket.getOutputStream(), "UTF-8"), //$NON-NLS-1$
                    true);
                } catch (UnsupportedEncodingException e1) {
                    fWriter = new PrintWriter(new OutputStreamWriter(fSocket.getOutputStream()), true);
                }
                String message;
                while (fPushbackReader != null && (message = readMessage(fPushbackReader)) != null) receiveMessage(message);
    ...
    public synchronized void stopWaiting() {
        if (fServerSocket != null && !fServerSocket.isClosed() && fSocket == null) {
            // will throw a SocketException in Threads that wait in ServerSocket#accept()
            shutDown();
        }
    }
    ...
    private void scanReranMessage(String arg) {
        ...
        // special treatment, since testName can contain spaces:
        int t;
    ...
    String[] extractTestId(String arg) {
        ...
        if (!hasTestId()) {
            // use the test name as the test Id
            result[0] = arg;
            result[1] = arg;
            return result;
        }
    ...
    private void notifyTestReran(final String testId, final String className, final String testName, final int statusCode, final String trace) {
        for (int i = 0; i < fListeners.length; i++) {
            ...
            SafeRunner.run(new ListenerSafeRunnable() {
                ...
                @Override
                public void run() {
                    listener.testReran(testId, className, testName, statusCode, trace, nullifyEmpty(fExpectedResult), nullifyEmpty(fActualResult));
                }
            });
        }
    }
    ...
    private void notifyTestFailed() {
        ...
        for (int i = 0; i < fListeners.length; i++) {
            ...
            SafeRunner.run(new ListenerSafeRunnable() {
                ...
                @Override
                public void run() {
                    listener.testFailed(fFailureKind, fFailedTestId, fFailedTest, fFailedTrace.toString(), nullifyEmpty(fExpectedResult), nullifyEmpty(fActualResult));
                }
            });
        }
    }
    ...
    public void rerunTest(String testId, String className, String testName) {
        if (isRunning()) {
            ...
            fExpectedResult.setLength(0);
            //$NON-NLS-1$ //$NON-NLS-2$
            fWriter.println(MessageIds.TEST_RERUN + testId + " " + className + " " + testName);
            fWriter.flush();
        }
    }
}
