...
public class IntroduceFactoryTests extends RefactoringTest {
	 ...
	 * @throws Exception
	 */
    private ICompilationUnit createCUForSimpleTest(IPackageFragment pack, boolean positive, boolean input) throws Exception {
    ...
    private String getBugTestFileName(IJavaProject project, IPackageFragment pack, String fileName, boolean input) {
        ...
        String path = TEST_PATH_PREFIX + getRefactoringPath() + "Bugzilla/" + testNumber + "/" + (project == null ? "" : project.getElementName() + "/") + (pack == getPackageP() ? "" : pack.getElementName() + "/");
	 ...
	 * @throws Exception
	 */
    private ICompilationUnit createCUForBugTestCase(IJavaProject project, IPackageFragment pack, String baseName, boolean input) throws Exception {
	 ...
	 * @throws Exception
	 */
    void singleUnitHelper(boolean protectConstructor) throws Exception {
	 ...
	 * @throws Exception
	 */
    protected void singleUnitBugHelper(String baseFileName, boolean protectConstructor) throws Exception {
    ...
    protected void singleUnitBugHelperWithWarning(String baseFileName, boolean protectConstructor) throws Exception {
        ICompilationUnit cu = createCUForBugTestCase(null, getPackageP(), baseFileName, true);
	 ...
	 * @throws Exception
	 */
    void namesHelper(String factoryMethodName, String factoryClassName) throws Exception {
        ICompilationUnit cu = createCUForSimpleTest(getPackageP(), true, true);
        ISourceRange selection = findSelectionInSource(cu.getSource());
        IntroduceFactoryRefactoring ref = new IntroduceFactoryRefactoring(cu, selection.getOffset(), selection.getLength());
        RefactoringStatus activationResult = ref.checkInitialConditions(new NullProgressMonitor());
	 ...
	 * @throws Exception
	 */
    void multiUnitHelper(boolean staticFactoryMethod, String[] inputFileBaseNames) throws Exception {
        IPackageFragment pkg = getPackageP();
        ICompilationUnit CUs[] = new ICompilationUnit[inputFileBaseNames.length];
        for (int i = 0; i < inputFileBaseNames.length; i++) CUs[i] = createCUFromFileName(inputFileBaseNames[i], pkg);
	 ...
	 * @throws Exception
	 */
    void multiUnitBugHelper(boolean staticFactoryMethod, String[] inputFileBaseNames, String factoryClassName) throws Exception {
        ICompilationUnit CUs[] = new ICompilationUnit[inputFileBaseNames.length];
    ...
    public void test59283() throws Exception {
        multiProjectBugHelper(new String[] { "proj1/pA/A", "proj2/pB/B" }, new String[] { "proj2:proj1" });
    }
