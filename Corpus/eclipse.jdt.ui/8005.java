...
public final class JavaIndenter {
    ...
    private final class CorePrefs {
        ...
        private boolean prefArrayDimensionsDeepIndent() {
            // sensible default, no formatter setting
            return true;
        }
        ...
        private int prefArrayIndent() {
            ...
            return // default
            prefContinuationIndentForArrayInitializer();
        }
        ...
        private boolean prefParenthesisDeepIndent() {
            // don't do parenthesis deep indentation (check rev. 1.60 for experimental code)
            return false;
        }
    ...
    private StringBuffer createReusingIndent(StringBuffer buffer, int additional) {
        ...
        // may be < 0
        int addLength = fPrefs.prefIndentationSize * additional;
                ...
                switch(nextToken) {
                    ...
                    case Symbols.TokenCASE:
                        if (isFirstTokenOnLine)
                            matchCase = true;
                        break;
                    case // for opening-brace-on-new-line style
                    Symbols.TokenLBRACE:
                        if (bracelessBlockStart && !fPrefs.prefIndentBracesForBlocks)
                            ...
                            indent = true;
                        break;
                    case // closing braces get unindented
                    Symbols.TokenRBRACE:
                        if (isFirstTokenOnLine)
                            matchBrace = true;
                        break;
    ...
    private boolean looksLikeMethodDeclLBrace(int offset) {
        ...
        while (true) {
            switch(fToken) {
                case Symbols.TokenTHROWS:
                // identifier for exception/annotation/annotation value
                case Symbols.TokenIDENT:
                case Symbols.TokenCOMMA:
                case Symbols.TokenAT:
                // for array valued return type
                case Symbols.TokenLBRACKET:
                case Symbols.TokenRBRACKET:
                    // possible tokens between '{' and ')' in method declaration
                    break;
                case // dot of qualification
                Symbols.TokenOTHER:
                    try {
                    ...
                    break;
                case // parenthesis for annotation value / method declaration
                Symbols.TokenRPAREN:
                    skipScope();
                    int storedPos = fPosition;
    ...
    private int handleEqual() {
        try {
            ...
            if (nonWS != Symbols.TokenEOF) {
                ...
                if (tokenAtPreviousLine != Symbols.TokenSEMICOLON && tokenAtPreviousLine != Symbols.TokenRBRACE && tokenAtPreviousLine != Symbols.TokenLBRACE && tokenAtPreviousLine != Symbols.TokenEOF)
                    return fPosition;
            }
        } catch (BadLocationException e) {
            return fPosition;
            ...
            if (isInBlock) {
                switch(fToken) {
                    ...
                    case Symbols.TokenSTATIC:
                        // treat static blocks like methods
            ...
            switch(fToken) {
                ...
                case Symbols.TokenRBRACE:
                    ...
                    if (skipScope() && looksLikeArrayInitializerIntro()) {
                        ...
                        if (isInBlock)
                            fIndent = getBlockIndent(mayBeMethodBody == READ_IDENT, isTypeBody);
                        // it's not - do as with all the above
                        return pos;
                    }
                // scopes: skip them
                case Symbols.TokenRPAREN:
    ...
    private boolean isConditional() {
        while (true) {
            ...
            switch(fToken) {
                ...
                case Symbols.TokenIDENT:
                case // dots for qualified constants
                Symbols.TokenOTHER:
                    continue;
                case Symbols.TokenCASE:
                case Symbols.TokenDEFAULT:
    ...
    private int handleScopeIntroduction(int bound) {
        switch(fToken) {
            ...
            case Symbols.TokenLBRACE:
                ...
                if (looksLikeArrayInitializerIntro() && !fPrefs.prefIndentBracesForArrays || !fPrefs.prefIndentBracesForBlocks) {
                    // restore
                    fPosition = pos;
                    // set to true to match the first if
    ...
    private int setFirstElementAlignment(int scopeIntroducerOffset, int bound) {
        // align with the first position after the scope intro
    ...
    private boolean looksLikeArrayInitializerIntro() {
        ...
        fToken == Symbols.TokenIDENT) {
            nextToken();
            while (// dot of qualification
            fToken == Symbols.TokenOTHER) {
                nextToken();
                if (// qualifying name
                fToken != Symbols.TokenIDENT)
                    return false;
    ...
    private boolean looksLikeMethodDecl() {
        ...
        if (// method name
        fToken == Symbols.TokenIDENT) {
            do nextToken(); while (// optional brackets for array valued return types
            skipBrackets());
            if (fToken == Symbols.TokenGREATERTHAN) {
                if (// parameterized return type
                skipScope(Symbols.TokenLESSTHAN, Symbols.TokenGREATERTHAN)) {
                    nextToken();
                } else {
                    return false;
