...
public class CustomFiltersActionGroup extends ActionGroup {
    ...
    class FilterActionMenuContributionItem extends ContributionItem {
        ...
        public void fill(Menu menu, int index) {
            ...
            //$NON-NLS-1$  //$NON-NLS-2$
            mi.setText("&" + fItemNumber + " " + fFilterName);
            /*
    ...
    public  CustomFiltersActionGroup(String ownerId, StructuredViewer viewer) {
        ...
        for (int i = 0; i < filterDescriptors.length; i++) {
            ...
            if (existing != null) {
                ...
                "WARNING: Duplicate id for extension-point \"org.eclipse.jdt.ui.javaElementFilters\" in " + ownerId);
            }
        }
    ...
    private boolean isSelected(Object parent, Object element, IContentProvider contentProvider, ViewerFilter filter) {
        if (// workaround for https://bugs.eclipse.org/341109
        filter instanceof EmptyLibraryContainerFilter)
            return true;
    ...
    private void setRecentlyChangedFilters(Stack<FilterDescriptor> changeHistory) {
        ...
        for (int i = 0; i < length; i++) oldestFirstStack.push(changeHistory.pop().getId());
    ...
    private void removePreviousLRUFilterActions(IMenuManager mm) {
        if (fFilterIdsUsedInLastViewMenu == null)
            return;
        for (int i = 0; i < fFilterIdsUsedInLastViewMenu.length; i++) mm.remove(fFilterIdsUsedInLastViewMenu[i]);
    }
    ...
    private boolean updateViewerFilters() {
        ...
        if (// no pattern change so far, test if the user patterns made a difference
        !patternChange) {
        ...
        if (hasChange) {
            // will refresh
            fViewer.setFilters(viewerFilters.toArray(new ViewerFilter[viewerFilters.size()]));
        }
        return hasChange;
    }

    private boolean hasChanges(HashSet<String> patterns, String[] oldPatterns) {
        @SuppressWarnings("unchecked") HashSet<String> copy = (HashSet<String>) patterns.clone();
    ...
    private void storeViewDefaults() {
        ...
        // see bug https://bugs.eclipse.org/bugs/show_bug.cgi?id=22533
        //$NON-NLS-1$
        store.setValue(getPreferenceKey(TAG_DUMMY_TO_TEST_EXISTENCE), "storedViewPreferences");
    ...
    private String getPreferenceKey(String tag) {
        //$NON-NLS-1$
        return "CustomFiltersActionGroup." + fTargetId + '.' + tag;
    }
    ...
    private void restoreUserDefinedPatterns(IMemento memento) {
        ...
        if (userDefinedPatterns != null) {
            ...
            for (int i = 0; i < children.length; i++) patterns[i] = children[i].getString(TAG_PATTERN);
    ...
    private void openDialog() {
        CustomFiltersDialog dialog = new CustomFiltersDialog(fViewer.getControl().getShell(), fTargetId, areUserDefinedPatternsEnabled(), fUserDefinedPatterns, internalGetEnabledFilterIds());
