...
public class JavaCorrectionProcessor implements org.eclipse.jface.text.quickassist.IQuickAssistProcessor {
    ...
    public ICompletionProposal[] computeQuickAssistProposals(IQuickAssistInvocationContext quickAssistContext) {
        ...
        if (res == null || res.length == 0) {
            //$NON-NLS-1$
            return new ICompletionProposal[] { new ChangeCorrectionProposal(CorrectionMessages.NoCorrectionProposal_description, new NullChange(""), IProposalRelevance.NO_SUGGESSTIONS_AVAILABLE, null) };
        }
        if (res.length > 1) {
    ...
    private static ProblemLocation getProblemLocation(IJavaAnnotation javaAnnotation, IAnnotationModel model) {
        ...
        if (problemId != -1) {
            ...
            if (pos != null) {
                // java problems all handled by the quick assist processors
