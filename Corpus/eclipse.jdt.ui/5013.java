...
public abstract class SaveEditorTest extends TextPerformanceTestCase {
    ...
    protected void measureRevert(PerformanceMeter performanceMeter, int runs) throws Exception {
        for (int i = 0; i < runs; i++) {
            for (int j = 0; j < getNumberOfEditors(); j++) dirtyEditor(fEditors[j]);
            for (int j = 0; j < getNumberOfEditors(); j++) EditorTestHelper.joinReconciler(EditorTestHelper.getSourceViewer(fEditors[j]), 100, 10000, 100);
            performanceMeter.start();
            for (int j = 0; j < getNumberOfEditors(); j++) EditorTestHelper.getActivePage().saveEditor(fEditors[j], false);
            performanceMeter.stop();
            for (int j = 0; j < getNumberOfEditors(); j++) assertFalse(fEditors[j].isDirty());
        }
    }
