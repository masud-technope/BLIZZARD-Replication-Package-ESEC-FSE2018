...
public class SemanticHighlightingManager implements IPropertyChangeListener {
    ...
    private void disposeHighlightings() {
        for (int i = 0, n = fSemanticHighlightings.length; i < n; i++) removeColor(SemanticHighlightings.getColorPreferenceKey(fSemanticHighlightings[i]));
    ...
    private void handlePropertyChangeEvent(PropertyChangeEvent event) {
        if (fPreferenceStore == null)
            // Uninstalled during event notification
            return;
