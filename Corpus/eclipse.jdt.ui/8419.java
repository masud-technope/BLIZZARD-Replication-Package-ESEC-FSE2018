...
public class BindingLinkedLabelComposer extends JavaElementLinkedLabelComposer {
    ...
    private void appendMethodBindingLabel(IMethodBinding method, long flags) {
        ...
        if (declaringMember != null) {
            if (isEnclosingElement(method.getJavaElement())) {
                ...
                // for the main part show the SAM instead
        ...
        if (!isInitializer) {
            ...
            if (getFlag(flags, JavaElementLabels.M_PARAMETER_TYPES | JavaElementLabels.M_PARAMETER_NAMES)) {
                ...
                if (getFlag(flags, JavaElementLabels.M_PARAMETER_TYPES)) {
                    ...
                    if (getFlag(flags, JavaElementLabels.USE_RESOLVED) && iMethod != null && iMethod.isResolved() && iMethod.getParameterTypes().length == 1 && JavaModelUtil.isPolymorphicSignature(iMethod)) {
                        renderVarargs = false;
                        isPolymorphic = true;
                    }
                }
    ...
    private void appendTypeBindingLabel(ITypeBinding typeBinding, long flags) {
        ...
        if (declaringMember != null && getFlag(flags, JavaElementLabels.T_FULLY_QUALIFIED | JavaElementLabels.T_CONTAINER_QUALIFIED)) {
            ...
            flags &= ~(JavaElementLabels.T_FULLY_QUALIFIED | JavaElementLabels.M_FULLY_QUALIFIED | JavaElementLabels.T_CONTAINER_QUALIFIED);
        }
        if (// different textual order for type annotations on array types
        !typeBinding.isArray()) {
        ...
        if (typeBinding.isPrimitive()) {
            ...
            if (// no relevant bound / only j.l.Object
            bound == null || bound.getSuperclass() == null) {
                fBuffer.append('?');
            } else {
    ...
    private void appendPackageLabel(IPackageBinding binding, long flags) {
        ...
        if (qualifiedName.length() > 0) {
            ...
            //$NON-NLS-1$
            fBuffer.append("(default package)");
        }
    }
    ...
    private boolean hasRelevantBound(ITypeBinding[] bounds) {
        if (bounds != null) {
            for (int i = 0; i < bounds.length; i++) if (bounds[i].isInterface() || bounds[i].getSuperclass() != null)
                return true;
        }
        return false;
    }
    ...
    private long getPostQualificationFlags(long flags) {
        ...
        flags &= ~(JavaElementLabels.T_TYPE_PARAMETERS | JavaElementLabels.M_PRE_TYPE_PARAMETERS | JavaElementLabels.M_PRE_RETURNTYPE | JavaElementLabels.M_PARAMETER_NAMES);
        return flags;
    }
    ...
    private void appendNameLink(IBinding binding, IBinding origBinding) {
        try {
            ...
            if (!isEnclosingElement(javaElement)) {
                ...
                if (// extra safety, not seen during tests
                element == null)
    ...
    private String getTypeLink(ITypeBinding typeBinding, long flags) {
        ...
        if (qualifierLength > 0) {
            if (qualifiedName.endsWith(typeName)) {
                ...
                // Not expected. Just show the whole qualifiedName.
                title = qualifiedName;
            }
        }
        try {
    ...
    private IMethod getIMethod(IMethodBinding method) {
        ...
        if (element == null || element.getElementType() != IJavaElement.METHOD)
            // extra safety, not seen in tests (triggered from ctor of local class inside initializer)
            return null;
        IMethod iMethod = (IMethod) element;
        if (isEnclosingElement(iMethod)) {
            // benefit from better details on LambdaMethod
            return (IMethod) fEnclosingElement;
        }
        return iMethod;
    }
