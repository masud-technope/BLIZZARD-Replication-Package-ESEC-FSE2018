...
public abstract class AbstractMethodDeclaration extends ASTNode implements ProblemSeverities, ReferenceContext {
        ...
        if (arguments != null && binding != null) {
            for (int i = 0, length = arguments.length; i < length; i++) {
                ...
                if (useTypeAnnotations)
                    // no business with SE7 null annotations in the 1.8 case.
                    continue;
    ...
    public void generateCode(ClassFile classFile) {
        ...
        if ((!this.binding.isNative()) && (!this.binding.isAbstract())) {
            ...
            if (this.statements != null) {
                for (int i = 0, max = this.statements.length; i < max; i++) this.statements[i].generateCode(this.scope, codeStream);
            }
    ...
    private void checkArgumentsSize() {
        ...
        // an abstract method or a native method cannot be static
        int size = 1;
    ...
    public void resolve(ClassScope upperScope) {
        ...
        try {
            ...
            if (// otherwise already checked via Argument.createBinding
            sourceLevel < ClassFileConstants.JDK1_8)
                validateNullAnnotations(this.scope.environment().usesNullTypeAnnotations());
            resolveStatements();
            // check @Deprecated annotation presence
            if (this.binding != null && (this.binding.getAnnotationTagBits() & TagBits.AnnotationDeprecated) == 0 && (this.binding.modifiers & ClassFileConstants.AccDeprecated) != 0 && sourceLevel >= ClassFileConstants.JDK1_5) {
    ...
    public void resolveReceiver() {
        ...
        if (this.isStatic() || declaringClass.isAnonymousType()) {
            ...
            // No need to do further validation
            return;
        }
        ...
        if (this.isConstructor()) {
            ...
            if (declaringClass.isStatic() || (declaringClass.tagBits & (TagBits.IsLocalType | TagBits.IsMemberType)) == 0) /* neither member nor local type */
            {
                this.scope.problemReporter().disallowedThisParameter(this.receiver);
                // No need to do further validation
                return;
    ...
    public void resolveStatements() {
        if (this.statements != null) {
            ...
            if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=319626
            !this.isConstructor() || this.arguments != null) {
            ...
            if (this.binding.parameterNonNullness != null) {
                ...
                for (int i = 0; i < length; i++) {
                    if (this.binding.parameterNonNullness[i] != null) {
                        long nullAnnotationTagBit = this.binding.parameterNonNullness[i].booleanValue() ? TagBits.AnnotationNonNull : TagBits.AnnotationNullable;
