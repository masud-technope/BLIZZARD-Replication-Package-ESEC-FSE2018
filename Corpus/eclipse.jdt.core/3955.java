...
public class ForStatement extends Statement {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        if ((condition != null) && (condition.constant == NotAConstant) && !((action == null || action.isEmptyBlock()) && (increments == null))) {
    ...
    public void traverse(ASTVisitor visitor, BlockScope blockScope) {
        if (visitor.visit(this, blockScope)) {
            if (initializations != null) {
                ...
                for (int i = 0; i < initializationsLength; i++) initializations[i].traverse(visitor, scope);
            }
            if (condition != null)
                condition.traverse(visitor, scope);
            if (increments != null) {
                int incrementsLength = increments.length;
                for (int i = 0; i < incrementsLength; i++) increments[i].traverse(visitor, scope);
            }
            if (action != null)
