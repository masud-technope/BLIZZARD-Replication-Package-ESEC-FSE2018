...
public class SampleASTs {
    ...
    public static ASTNode oneOfEach(AST target) {
        ...
        if (target.apiLevel() >= JLS3_INTERNAL) {
            ...
            //$NON-NLS-1$
            QualifiedType qt = target.newQualifiedType(st, target.newSimpleName("x"));
            WildcardType wt = target.newWildcardType();
            //$NON-NLS-1$
            ParameterizedType pmt = target.newParameterizedType(target.newSimpleType(target.newSimpleName("y")));
