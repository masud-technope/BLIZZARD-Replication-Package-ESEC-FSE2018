...
public class Javadoc extends ASTNode {
    ...
    public StringBuffer print(int indent, StringBuffer output) {
        ...
        if (this.paramTypeParameters != null) {
            for (int i = 0, length = this.paramTypeParameters.length; i < length; i++) {
                ...
                " * @param <");
                //$NON-NLS-1$
                this.paramTypeParameters[i].print(indent, output).append(//$NON-NLS-1$
                ">\n");
            }
        }
    ...
    public void resolve(MethodScope methScope) {
        ...
        boolean overriding = methDecl == null || /* field declaration */
        methDecl.binding == null ? /* compiler error */
        false : !methDecl.binding.isStatic() && ((methDecl.binding.modifiers & (ExtraCompilerModifiers.AccImplementing | ExtraCompilerModifiers.AccOverriding)) != 0);
        // @see tags
        int seeTagsLength = this.seeReferences == null ? 0 : this.seeReferences.length;
                    ...
                    if (allocationExpr.binding != null && allocationExpr.binding.isValidBinding()) {
                        ...
                        if (superType != null && TypeBinding.notEquals(superType.original(), methDecl.binding.declaringClass)) {
                            ...
                            if (superConstructor.isValidBinding() && superConstructor.original() == allocationExpr.binding.original()) {
                                ...
                                if (methScope.compilerOptions().sourceLevel >= ClassFileConstants.JDK1_8 && current.typeVariables != Binding.NO_TYPE_VARIABLES) {
        ...
        resolveParamTags(methScope, reportMissing, compilerOptions.reportUnusedParameterIncludeDocCommentReference);
    ...
    private void resolveReference(Expression reference, Scope scope) {
        ...
        if (reference instanceof JavadocFieldReference) {
            ...
            } else // Verify whether field ref should be static or not (for @value tags)
        ...
        if (reference instanceof JavadocMessageSend) {
            ...
            // tag value
            if (// cannot refer to method for @value tag
            source15 && msgSend.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) {
                if (scopeModifiers == -1)
                    scopeModifiers = scope.getDeclarationModifiers();
        ...
        } else // Verify that constructor reference are not used for @value tags
        if (reference instanceof JavadocAllocationExpression) {
            JavadocAllocationExpression alloc = (JavadocAllocationExpression) reference;
            // tag value
            if (// cannot refer to method for @value tag
            source15 && alloc.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) {
                if (scopeModifiers == -1)
                    scopeModifiers = scope.getDeclarationModifiers();
        ...
        } else // (javadoc does not accept them and this is not a referenced bug or requested enhancement)
        if (reference instanceof JavadocSingleTypeReference && reference.resolvedType != null && reference.resolvedType.isTypeVariable()) {
        ...
        if (throwsTagsLength == 0) {
            if (reportMissing) {
                for (int i = 0; i < boundExceptionLength; i++) {
                    ...
                    if (// flag only valid class name
                    exceptionBinding != null && exceptionBinding.isValidBinding()) {
                        int j = i;
    ...
    private void verifyTypeReference(Expression reference, Expression typeReference, Scope scope, boolean source15, ReferenceBinding resolvedType, int modifiers) {
        if (resolvedType.isValidBinding()) {
            ...
            if (resolvedType.isMemberType()) {
                ...
                if (topLevelScope.parent.kind != Scope.COMPILATION_UNIT_SCOPE || !CharOperation.equals(topLevelType.sourceName, topLevelScope.referenceContext.name)) {
            ...
            if (scope.referenceCompilationUnit().isPackageInfo() && typeReference instanceof JavadocSingleTypeReference) {
                if (resolvedType.fPackage.compoundName.length > 0) {
                    ...
                    // Not really needed - just in case more code added in future
                    return;
                }
            }
        }
