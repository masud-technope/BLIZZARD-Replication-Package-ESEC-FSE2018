...
public final class ImportRewrite {
	 ...
	 * </p>
	 */
    public abstract static class ImportRewriteContext {
	...
	*/
    ...
    public Annotation addAnnotation(IAnnotationBinding annotation, AST ast, ImportRewriteContext context) {
        ...
        mvps.length == 1 && "value".equals(mvps[0].getName())) {
    ...
    public Type addImportFromSignature(String typeSig, AST ast, ImportRewriteContext context) {
        ...
        switch(sigKind) {
            ...
            case Signature.CLASS_TYPE_SIGNATURE:
                ...
                if (typeArguments.length > 0) {
                    ...
                    for (int i = 0; i < typeArguments.length; i++) {
                        ...
                        if (containsNestedCapture(// see bug 103044
                        curr)) {
            ...
            default:
                ...
                "Unknown type signature kind: " + typeSig);
        }
    }
    ...
    public String addImport(ITypeBinding binding, ImportRewriteContext context) {
        ...
        if (normalizedBinding.isWildcardType()) {
            ...
            if (// bug 95942
            bound != null && !bound.isWildcardType() && !bound.isCapture()) {
        ...
        if (qualifiedName.length() > 0) {
            ...
            if (typeArguments.length > 0) {
                ...
                for (int i = 0; i < typeArguments.length; i++) {
                    ...
                    if (// see bug 103044
                    containsNestedCapture(curr, false)) {
                        res.append('?');
                    } else {
    ...
    private static ITypeBinding normalizeTypeBinding(ITypeBinding binding) {
        if (//$NON-NLS-1$
        binding != null && !binding.isNullType() && !"void".equals(binding.getName())) {
    ...
    public Type addImport(ITypeBinding binding, AST ast, ImportRewriteContext context) {
        ...
        Type type = internalAddImport(bindingPoint == null ? binding : bindingPoint, ast, context, /* getBase */
        null, true);
    ...
    private ImportRewriteConfiguration buildImportRewriteConfiguration() {
        ...
        configBuilder.setTypeContainerSorting(this.useContextToFilterImplicitImports ? ImportContainerSorting.BY_PACKAGE : ImportContainerSorting.BY_PACKAGE_AND_CONTAINING_TYPE);
        configBuilder.setStaticContainerSorting(ImportContainerSorting.BY_PACKAGE_AND_CONTAINING_TYPE);
        configBuilder.setImplicitImportIdentification(this.filterImplicitImports ? ImplicitImportIdentification.JAVA_LANG_AND_CU_PACKAGE : ImplicitImportIdentification.NONE);
    ...
    private Type internalAddImport(ITypeBinding binding, AST ast, ImportRewriteContext context, Type currentType, boolean getBase) {
        ...
        if (binding.isPrimitive()) {
            ...
            if (normalizedBinding == null) {
                return //$NON-NLS-1$
                ast.newSimpleType(//$NON-NLS-1$
                ast.newSimpleName("invalid"));
        ...
        if (getBase) {
            ...
            type = currentType != null ? (Type) ast.newQualifiedType(currentType, ast.newSimpleName(getRawName(normalizedBinding))) : ast.newSimpleType(ast.newName(getRawName(normalizedBinding)));
    ...
    private Expression addAnnotation(AST ast, Object value, ImportRewriteContext context) {
        if (value instanceof Boolean) {
        ...
        } else if (value instanceof Byte || value instanceof Short || value instanceof Integer || value instanceof Long || value instanceof Float || value instanceof Double) {
            ...
            if (type instanceof SimpleType) {
                ...
                // cut 'name' loose from its parent, so that it can be reused
                //$NON-NLS-1$
                simpleType.setName(//$NON-NLS-1$
                ast.newSimpleName("a"));
            } else {
                name = //$NON-NLS-1$
                ast.newName(//$NON-NLS-1$
                "invalid");
            }
            ...
            for (int i = 0; i < size; i++) expressions.add(addAnnotation(ast, values[i], context));
            return initializer;
        } else {
            return null;
        }
