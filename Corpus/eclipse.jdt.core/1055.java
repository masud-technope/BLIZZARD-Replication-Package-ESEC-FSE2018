...
public final class Statistics {
    ...
    //	long[] smoothedCounts;
    static final Dim[] DIMENSIONS = { InternalDimensions.CPU_TIME, InternalDimensions.ELAPSED_PROCESS, InternalDimensions.USED_JAVA_HEAP };
    ...
    public  Statistics(DataPoint[] dataPoints) {
        ...
        for (int i = 0; i < DIMENSIONS.length; i++) {
            ...
            for (int j = 0; j < this.count; j++) {
                long measure = dataPoints[2 * j + 1].getScalar(DIMENSIONS[i]).getMagnitude() - dataPoints[2 * j].getScalar(DIMENSIONS[i]).getMagnitude();
