 ...
 */
    public class NoClassNoMethodDeclarationVisitor extends ASTVisitor {
        ...
        public boolean visit(ConstructorDeclaration constructorDeclaration, ClassScope scope) {
            ...
            return (constructorDeclaration.bits & ASTNode.HasLocalType) != 0;
        }

        public boolean visit(FieldDeclaration fieldDeclaration, MethodScope scope) {
            // continue only if it has local type;
            return (fieldDeclaration.bits & ASTNode.HasLocalType) != 0;
        }

        public boolean visit(Initializer initializer, MethodScope scope) {
            // continue only if it has local type
            return (initializer.bits & ASTNode.HasLocalType) != 0;
        }

        public boolean visit(MethodDeclaration methodDeclaration, ClassScope scope) {
            // continue only if it has local type
            return (methodDeclaration.bits & ASTNode.HasLocalType) != 0;
        }
    }
    ...
    protected  MatchLocatorParser(ProblemReporter problemReporter, MatchLocator locator) {
        ...
        if ((locator.matchContainer & PatternLocator.CLASS_CONTAINER) != 0) {
            this.localDeclarationVisitor = (locator.matchContainer & PatternLocator.METHOD_CONTAINER) != 0 ? new ClassAndMethodDeclarationVisitor() : new ClassButNoMethodDeclarationVisitor();
        } else {
            this.localDeclarationVisitor = (locator.matchContainer & PatternLocator.METHOD_CONTAINER) != 0 ? new MethodButNoClassDeclarationVisitor() : new NoClassNoMethodDeclarationVisitor();
        }
    ...
    protected void consumeCastExpressionLL1WithBounds() {
        ...
        if ((this.patternFineGrain & IJavaSearchConstants.CAST_TYPE_REFERENCE) != 0) {
            ...
            for (int i = 0, length = typeReferences.length; i < length; i++) this.patternLocator.match(typeReferences[i], this.nodeSet);
        }
    }
    ...
    protected void parseBodies(TypeDeclaration type, CompilationUnitDeclaration unit) {
        ...
        if (methods != null) {
            for (int i = 0; i < methods.length; i++) {
                ...
                if (// if not synthetic
                method.sourceStart >= type.bodyStart) {
