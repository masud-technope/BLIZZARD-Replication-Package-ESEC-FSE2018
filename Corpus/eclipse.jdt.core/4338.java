...
public class UnconditionalFlowInfo extends FlowInfo {
        ...
        if (extraDefiniteInits != null) {
            if (otherInits.extraDefiniteInits != null) {
                ...
                if ((length = extraDefiniteInits.length) < (otherLength = otherInits.extraDefiniteInits.length)) {
                    ...
                    while (i < length) extraDefiniteInits[i++] = 0;
                }
            } else {
    ...
    private final boolean isDefinitelyAssigned(int position) {
        ...
        if (position < BitCacheSize) {
            // use bits
            return (definiteInits & (1L << position)) != 0;
        }
        // use extra vector
        if (extraDefiniteInits == null)
            // if vector not yet allocated, then not initialized
	 ...
	 * bits for the first 64 entries, then an array of booleans.
	 */
    private final void markAsDefinitelyNotAssigned(int position) {
        if (this != DEAD_END) {
            // position is zero-based
            if (position < BitCacheSize) {
                // use bits
        ...
        if (this.extraDefiniteInits != null) {
            if (otherInits.extraDefiniteInits != null) {
                ...
                if ((length = this.extraDefiniteInits.length) < (otherLength = otherInits.extraDefiniteInits.length)) {
                    ...
                    while (i < length) this.extraDefiniteInits[i++] = 0;
                }
            } else {
                // no extra storage on otherInits
    ...
    public String toString() {
        ...
        ((this.reachMode & UNREACHABLE) == 0) + //$NON-NLS-1$
        ">";
    }
