...
public class UnaryExpression extends OperatorExpression {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        switch((this.bits & OperatorMASK) >> OperatorSHIFT) {
            case NOT:
                ...
                break;
            case TWIDDLE:
                switch((this.expression.implicitConversion & IMPLICIT_CONVERSION_MASK) >> /* runtime */
                4) {
                    case T_int:
                        // ~int
            ...
            case MINUS:
                ...
                if (this.constant != Constant.NotAConstant) {
                    ...
                    this.expression.generateCode(currentScope, codeStream, valueRequired);
                    if (valueRequired) {
                        switch((this.expression.implicitConversion & IMPLICIT_CONVERSION_MASK) >> /* runtime type */
                        4) {
                            case T_int:
                                codeStream.ineg();
                                break;
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if ((expressionIsCast = this.expression instanceof CastExpression) == true)
            this.expression.bits |= DisableUnnecessaryCastCheck;
