...
public class LoopingFlowContext extends SwitchFlowContext {
    ...
    // slots can be null for checkType == IN_UNBOXING
    LocalVariableBinding[] nullLocals;

    // Expressions for null checking, Statements for resource analysis
    ASTNode[] nullReferences;

    // cast to Expression is safe if corresponding nullCheckType != EXIT_RESOURCE
    int[] nullCheckTypes;

    // detailed null info observed during the first visit of nullReferences[i], or null
    ...
    private static class EscapingExceptionCatchSite {
        ...
        // flow leading to the location of throwing
        ...
        void simulateThrowAfterLoopBack(FlowInfo flowInfo) {
            this.catchingContext.recordHandlingException(this.caughtException, flowInfo.unconditionalCopy().addNullInfoFrom(this.exceptionInfo).unconditionalInits(), // raised exception, irrelevant here,
            null, // we have no business altering the needed status.
            null, // we have no business altering the needed status.
            null, // we have no business altering the needed status.
            true);
        }
            ...
            for (int i = 0; i < this.nullCount; i++) {
                ...
                switch(this.nullCheckTypes[i] & ~HIDE_NULL_COMPARISON_WARNING_MASK) {
                        ...
                        if (flowInfo.isDefinitelyNull(local)) {
                            ...
                            switch(this.nullCheckTypes[i] & CONTEXT_MASK) {
                                case FlowContext.IN_COMPARISON_NULL:
                                    ...
                                    ((this.nullCheckTypes[i] & CHECK_MASK & ~HIDE_NULL_COMPARISON_WARNING_MASK) == CAN_ONLY_NULL) && (expression.implicitConversion & TypeIds.UNBOXING) != 0) {
                    ...
                    case IN_UNBOXING:
                        ...
                        // delegation to parent already handled in the above.
                        continue;
                    default:
                }
        ...
        switch(checkTypeWithoutHideNullWarning) {
            ...
            case CAN_ONLY_NULL_NON_NULL | IN_COMPARISON_NON_NULL:
                ...
                if (flowInfo.isDefinitelyNonNull(local)) {
                    ...
                    // no reason to complain, since there is definitely some uncertainty making the comparison relevant.
                    return;
                } else {
