 ...
 */
    public boolean equals(Object object) {
        ...
        if (rulesLength != otherRuleSet.accessRules.length)
            return false;
        for (int i = 0; i < rulesLength; i++) if (!this.accessRules[i].equals(otherRuleSet.accessRules[i]))
            return false;
        return true;
    }
 ...
 */
    public AccessRestriction getViolatedRestriction(char[] targetTypeFilePath) {
        for (int i = 0, length = this.accessRules.length; i < length; i++) {
            ...
            if (CharOperation.pathMatch(accessRule.pattern, targetTypeFilePath, /*case sensitive*/
            true, '/')) {
