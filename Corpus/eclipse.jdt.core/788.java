...
public class SourceTypeBinding extends ReferenceBinding {
    ...
    private void addDefaultAbstractMethod(MethodBinding abstractMethod) {
        MethodBinding defaultAbstract = new MethodBinding(abstractMethod.modifiers | AccDefaultAbstract, abstractMethod.selector, abstractMethod.returnType, abstractMethod.parameters, abstractMethod.thrownExceptions, this);
...
*/
    public FieldBinding addSyntheticField(LocalVariableBinding actualOuterLocalVariable) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(CharOperation.concat(SyntheticArgumentBinding.OuterLocalPrefix, actualOuterLocalVariable.name), actualOuterLocalVariable.type, AccPrivate | AccFinal | AccSynthetic, this, Constant.NotAConstant, synthetics[FIELD_EMUL].size());
        ...
        do {
            ...
            if ((existingField = this.getField(synthField.name, /*resolve*/
            true)) != null) {
                TypeDeclaration typeDecl = scope.referenceContext;
                for (int i = 0, max = typeDecl.fields.length; i < max; i++) {
                    FieldDeclaration fieldDecl = typeDecl.fields[i];
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(SyntheticArgumentBinding.OuterLocalPrefix, actualOuterLocalVariable.name, //$NON-NLS-1$
...
*/
    public FieldBinding addSyntheticField(ReferenceBinding enclosingType) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(CharOperation.concat(SyntheticArgumentBinding.EnclosingInstancePrefix, String.valueOf(enclosingType.depth()).toCharArray()), enclosingType, AccDefault | AccFinal | AccSynthetic, this, Constant.NotAConstant, synthetics[FIELD_EMUL].size());
...
*/
    public FieldBinding addSyntheticField(TypeBinding targetType, BlockScope blockScope) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(//$NON-NLS-1$
            ("class$" + synthetics[CLASS_LITERAL_EMUL].size()).toCharArray(), blockScope.getJavaLangClass(), AccDefault | AccStatic | AccSynthetic, this, Constant.NotAConstant, synthetics[CLASS_LITERAL_EMUL].size());
...
*/
    public FieldBinding addSyntheticField(AssertStatement assertStatement, BlockScope blockScope) {
        ...
        //$NON-NLS-1$
        FieldBinding synthField = (FieldBinding) synthetics[FIELD_EMUL].get("assertionEmulation");
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(//$NON-NLS-1$
            "$assertionsDisabled".toCharArray(), BooleanBinding, AccDefault | AccStatic | AccSynthetic | AccFinal, this, Constant.NotAConstant, synthetics[FIELD_EMUL].size());
            //$NON-NLS-1$
            synthetics[FIELD_EMUL].put("assertionEmulation", synthField);
        }
        // ensure there is not already such a field defined by the user
        // ensure there is not already such a field defined by the user
        boolean needRecheck;
        ...
        do {
            ...
            true)) != null) {
                ...
                for (int i = 0, max = typeDecl.fields.length; i < max; i++) {
                    FieldDeclaration fieldDecl = typeDecl.fields[i];
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(//$NON-NLS-1$
 ...
 */
    ...
    void faultInTypesForFieldsAndMethods() {
        fields();
        methods();
        for (int i = 0, length = memberTypes.length; i < length; i++) ((SourceTypeBinding) memberTypes[i]).faultInTypesForFieldsAndMethods();
    }
    ...
    public FieldBinding[] fields() {
        ...
        try {
            ...
            if (failed > 0) {
                ...
                for (int i = 0, j = 0, length = fields.length; i < length; i++) if (fields[i] != null)
    ...
    public char[] genericTypeSignature() {
        if (this.genericReferenceTypeSignature == null) {
            if (this.typeVariables == NoTypeVariables) {
                ...
                for (// copy all but trailing semicolon
                int i = 0; // copy all but trailing semicolon
                i < typeSig.length - 1; // copy all but trailing semicolon
                i++) {
 ...
 */
    ...
    public MethodBinding[] getDefaultAbstractMethods() {
        int count = 0;
        for (int i = methods.length; --i >= 0; ) if (methods[i].isDefaultAbstract())
            count++;
        if (count == 0)
            return NoMethods;
    ...
    public MethodBinding[] getMethods(char[] selector) {
        ...
        boolean methodsAreResolved = (modifiers & AccUnresolved) == 0;
...
*/
    public FieldBinding getSyntheticField(LocalVariableBinding actualOuterLocalVariable) {
        if (synthetics == null || synthetics[FIELD_EMUL] == null)
            return null;
 ...
 */
        ...
        try {
            ...
            for (int i = 0, length = methods.length; i < length; i++) {
                ...
                if (method != null) {
                    ...
                    method.returnType == null && methodDecl == null) {
            ...
            if (failed > 0) {
                ...
                if (newSize == 0) {
                    ...
                    for (int i = 0, j = 0, length = methods.length; i < length; i++) if (methods[i] != null)
    ...
    private FieldBinding resolveTypeFor(FieldBinding field) {
        ...
        for (int f = 0, length = fieldDecls.length; f < length; f++) {
            if (fieldDecls[f].binding != field)
                continue;
            MethodScope initializationScope = field.isStatic() ? scope.referenceContext.staticInitializerScope : scope.referenceContext.initializerScope;
    ...
    private MethodBinding resolveTypesFor(MethodBinding method) {
        ...
        if (!method.isConstructor()) {
            TypeReference returnType = methodDecl instanceof MethodDeclaration ? ((MethodDeclaration) methodDecl).returnType : ((AnnotationTypeMemberDeclaration) methodDecl).returnType;
            if (returnType == null) {
    ...
    public SyntheticAccessMethodBinding[] syntheticAccessMethods() {
        if (synthetics == null || synthetics[METHOD_EMUL] == null || synthetics[METHOD_EMUL].size() == 0)
            return null;
 ...
 */
    ...
    public String toString() {
        ...
        if (superInterfaces != null) {
            if (superInterfaces != NoSuperInterfaces) {
                ...
                for (int i = 0, length = superInterfaces.length; i < length; i++) {
                    ...
                    //$NON-NLS-1$
                    buffer.append(//$NON-NLS-1$
                    (superInterfaces[i] != null) ? superInterfaces[i].debugName() : "NULL TYPE");
                }
            }
        } else {
            //$NON-NLS-1$
        ...
        if (memberTypes != null) {
            if (memberTypes != NoMemberTypes) {
                //$NON-NLS-1$
                buffer.append("\n/*   members   */");
                for (int i = 0, length = memberTypes.length; i < length; i++) //$NON-NLS-1$ //$NON-NLS-2$
    ...
    void verifyMethods(MethodVerifier verifier) {
        ...
        for (int i = memberTypes.length; --i >= 0; ) ((SourceTypeBinding) memberTypes[i]).verifyMethods(verifier);
    }
    ...
    public FieldBinding getSyntheticField(ReferenceBinding targetEnclosingType, boolean onlyExactMatch) {
        ...
        if (!onlyExactMatch) {
            ...
            while (accessFields.hasNext()) {
                ...
                if (CharOperation.prefixEquals(SyntheticArgumentBinding.EnclosingInstancePrefix, field.name) && ((ReferenceBinding) field.type).findSuperTypeErasingTo(targetEnclosingType) != null)
                    return field;
            }
        }
        return null;
