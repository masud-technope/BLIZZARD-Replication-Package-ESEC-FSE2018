...
class PackageBinding implements IPackageBinding {
    ...
    public IAnnotationBinding[] getAnnotations() {
        try {
            ...
            IPackageFragment[] pkgs = nameLookup.findPackageFragments(pkgName, /*exact match*/
            false);
            if (pkgs == null)
                return AnnotationBinding.NoAnnotations;
                ...
                switch(fragType) {
                    ...
                    case IPackageFragmentRoot.K_BINARY:
                        ...
                        if (answer != null && answer.isBinaryType()) {
                            ...
                            org.eclipse.jdt.internal.compiler.lookup.AnnotationBinding[] binaryInstances = BinaryTypeBinding.createAnnotations(binaryAnnotations, this.binding.environment, missingTypeNames);
                            org.eclipse.jdt.internal.compiler.lookup.AnnotationBinding[] allInstances = org.eclipse.jdt.internal.compiler.lookup.AnnotationBinding.addStandardAnnotations(binaryInstances, type.getTagBits(), this.binding.environment);
    ...
    public IJavaElement getJavaElement() {
        ...
        if (nameLookup == null)
            return null;
        IJavaElement[] pkgs = nameLookup.findPackageFragments(getName(), /*exact match*/
        false);
        if (pkgs == null)
            return null;
        if (pkgs.length == 0) {
            // add additional tracing as this should not happen
            org.eclipse.jdt.internal.core.util.Util.log(new Status(IStatus.WARNING, JavaCore.PLUGIN_ID, //$NON-NLS-1$ //$NON-NLS-2$
            "Searching for package " + getName() + " returns an empty array"));
            return null;
        }
        return pkgs[0];
    }
