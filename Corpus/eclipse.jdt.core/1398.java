...
*/
    public final boolean areParametersEqual(MethodBinding method) {
        ...
        if (length != args.length)
            return false;
        for (int i = 0; i < length; i++) if (parameters[i] != args[i])
            return false;
        return true;
    }
...
*/
    public final boolean areParameterErasuresEqual(MethodBinding method) {
        ...
        for (int i = 0; i < length; i++) {
            if (parameters[i].erasure() != args[i].erasure())
                return false;
        }
        return true;
    }
 ...
 */
    public char[] genericSignature() {
        if ((this.modifiers & AccGenericSignature) == 0)
            return null;
 ...
 */
    public final boolean isMain() {
        if (this.selector.length == 4 && CharOperation.equals(this.selector, MAIN) && ((this.modifiers & (AccPublic | AccStatic)) != 0) && VoidBinding == this.returnType && this.parameters.length == 1) {
...
*/
    public final boolean isViewedAsDeprecated() {
        return (modifiers & AccDeprecated) != 0 || (modifiers & AccDeprecatedImplicitly) != 0;
    }
            ...
            if (parameters != NoParameters) {
                for (int i = 0, length = parameters.length; i < length; i++) {
                    if (i > 0)
                        ...
                        s += ", ";
                    s += //$NON-NLS-1$
                    (parameters[i] != null) ? //$NON-NLS-1$
                    parameters[i].debugName() : //$NON-NLS-1$
                    "NULL TYPE";
                }
            }
        } else {
            ...
            if (thrownExceptions != NoExceptions) {
                ...
                for (int i = 0, length = thrownExceptions.length; i < length; i++) {
                    if (i > 0)
                        ...
                        s += ", ";
                    s += //$NON-NLS-1$
                    (thrownExceptions[i] != null) ? //$NON-NLS-1$
                    thrownExceptions[i].debugName() : //$NON-NLS-1$
                    "NULL TYPE";
                }
            }
        } else {
