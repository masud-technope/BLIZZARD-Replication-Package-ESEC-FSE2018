...
public class ConstructorPattern extends JavaSearchPattern {
    ...
    public static final int FINE_GRAIN_MASK = IJavaSearchConstants.SUPER_REFERENCE | IJavaSearchConstants.QUALIFIED_REFERENCE | IJavaSearchConstants.THIS_REFERENCE | IJavaSearchConstants.IMPLICIT_THIS_REFERENCE | IJavaSearchConstants.METHOD_REFERENCE_EXPRESSION;
 ...
 */
    public static char[] createDeclarationIndexKey(char[] typeName, int argCount, char[] signature, char[][] parameterTypes, char[][] parameterNames, int modifiers, char[] packageName, int typeModifiers, int extraFlags) {
        ...
        if (argCount < 0) {
            ...
            countChars = argCount < 10 ? COUNTS[argCount] : //$NON-NLS-1$
    ...
    public  ConstructorPattern(char[] declaringSimpleName, char[] declaringQualification, char[][] parameterQualifications, char[][] parameterSimpleNames, String[] parameterSignatures, IMethod method, int limitTo, int matchRule) {
        this(declaringSimpleName, declaringQualification, parameterQualifications, parameterSimpleNames, limitTo, matchRule);
        try {
    ...
    public boolean matchesDecodedKey(SearchPattern decodedPattern) {
        ...
        return pattern.parameterCount != -1 && (this.parameterCount == pattern.parameterCount || this.parameterCount == -1 || this.varargs) && matchesName(this.declaringSimpleName, pattern.declaringSimpleName);
    }
    ...
    protected boolean mustResolve() {
        ...
        if (this.parameterSimpleNames != null)
            for (int i = 0, max = this.parameterSimpleNames.length; i < max; i++) if (this.parameterQualifications[i] != null)
                return true;
