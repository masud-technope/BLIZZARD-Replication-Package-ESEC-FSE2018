...
public class CompletionTestsRequestor2 extends CompletionRequestor {
    ...
    public  CompletionTestsRequestor2(boolean ignoreAll, boolean showParamNames, boolean showUniqueKeys, boolean showPositions, boolean shortContext, boolean showMissingTypes, boolean showTokenPositions, boolean showModifiers) {
        super(ignoreAll);
    ...
    public String getContext() {
        ...
        if (!this.shortContext) {
            ...
            if (locationType == 0) {
                ...
                if ((locationType & CompletionContext.TL_CONSTRUCTOR_START) != 0) {
                    if (!first)
                        buffer.append(',');
                    buffer.append("CONSTRUCTOR_START");
                    first = false;
                }
                buffer.append('}');
            }
        ...
        if (this.computeEnclosingElement) {
            buffer.append('\n');
            //$NON-NLS-1$
            buffer.append("enclosingElement=");
            ...
            if (enclosingElement == null) {
                ...
                buffer.append(enclosingElement.toStringWithAncestors(/*show resolved info*/
                true));
            }
        }
        if (this.computeVisibleElements) {
            ...
            if (visibleElements == null) {
                ...
                for (int i = 0; i < visibleElements.length; i++) {
                    ...
                    buffer.append('\t');
                    buffer.append(element.toStringWithAncestors(/*show resolved info*/
                    true));
                    //$NON-NLS-1$
                    buffer.append(//$NON-NLS-1$
                    ",\n");
    ...
    protected CompletionProposal[] quickSort(CompletionProposal[] collection, int left, int right) {
        ...
        do {
            while (compare(mid, collection[left]) > 0) // s[left] >= mid
            left++;
            while (compare(mid, collection[right]) < 0) // s[right] <= mid
            right--;
            if (left <= right) {
