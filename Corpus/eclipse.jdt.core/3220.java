...
public class FullSourceWorkspaceSearchTests extends FullSourceWorkspaceTests implements IJavaSearchConstants {
    ...
    public void testIndexing() throws CoreException {
        ...
        INDEX_MANAGER.request(new Measuring(/*start measuring*/
        true));
        ...
        AbstractJavaModelTests.waitUntilIndexesReady();
        // end measure
        INDEX_MANAGER.request(new Measuring(/*end measuring*/
        false));
        AbstractJavaModelTests.waitUntilIndexesReady();
        // Commit
        commitMeasurements();
    ...
    public void testSearchAllTypeNames() throws CoreException {
        ...
        for (int i = 0; i < WARMUP_COUNT; i++) {
            new SearchEngine().searchAllTypeNames(null, null, SearchPattern.R_PATTERN_MATCH | SearchPattern.R_CASE_SENSITIVE, IJavaSearchConstants.TYPE, scope, requestor, WAIT_UNTIL_READY_TO_SEARCH, null);
            if (i == 0) {
    ...
    public void testNewSearchAllTypeNames() throws CoreException {
        ...
        IJavaSearchScope scope = SearchEngine.createWorkspaceScope();
        for (int i = 0; i < WARMUP_COUNT; i++) {
            new SearchEngine().searchAllTypeNames(null, SearchPattern.R_PATTERN_MATCH | SearchPattern.R_CASE_SENSITIVE, null, SearchPattern.R_PATTERN_MATCH | SearchPattern.R_CASE_SENSITIVE, IJavaSearchConstants.TYPE, scope, requestor, WAIT_UNTIL_READY_TO_SEARCH, null);
            if (i == 0) {
                System.out.println("	All type names = " + INT_FORMAT.format(requestor.count));
                if (SEARCH_ALL_TYPE_NAMES_COUNT == -1) {
                    SEARCH_ALL_TYPE_NAMES_COUNT = requestor.count;
    ...
    public void testSearchPackageDeclarationsWorkspace() throws CoreException {
        ...
        for (int i = 0; i < MEASURES_COUNT; i++) {
            ...
            runGc();
            startMeasuring();
            for (int j = 0; j < 10; j++) search(name, PACKAGE, DECLARATIONS, scope, resultCollector);
            stopMeasuring();
        }
        // Commit
        commitMeasurements();
