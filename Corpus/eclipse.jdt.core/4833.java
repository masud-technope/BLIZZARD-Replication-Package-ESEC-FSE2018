...
public class JavadocParser extends AbstractCommentParser {
        ...
        try {
            ...
            if (this.checkDocComment) {
                ...
                nextLine: for (int line = firstLineNumber; line <= lastLineNumber; line++) {
                    ...
                    javadocStart + 3 : this.sourceParser.scanner.getLineStart(line);
                    this.index = lineStart;
                    this.lineEnd = line == lastLineNumber ? // remove trailing * /
                    javadocEnd - 2 : this.sourceParser.scanner.getLineEnd(line);
    ...
    protected boolean parseTag(int previousPosition) throws InvalidInputException {
        ...
        // first token is either an identifier or a keyword
    ...
    protected void updateDocComment() {
        ...
        while (this.astLengthPtr >= 0) {
            ...
            } else // Then continuing with class names (eg. TypeReference) coming from @throw/@exception declarations
            if (ptr == THROWS_TAG_EXPECTED_ORDER) {
                int size = this.astLengthStack[this.astLengthPtr--];
                for (int i = 0; i < size; i++) {
                    this.docComment.thrownExceptions[--sizes[ptr]] = (TypeReference) this.astStack[this.astPtr--];
                }
            } else // Finally, finishing with parameters nales (ie. Argument) coming from @param declaration
            if (ptr == PARAM_TAG_EXPECTED_ORDER) {
                int size = this.astLengthStack[this.astLengthPtr--];
                for (int i = 0; i < size; i++) {
