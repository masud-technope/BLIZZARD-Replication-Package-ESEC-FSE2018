...
public class ForeachStatement extends Statement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (!(action == null || (action.isEmptyBlock() && currentScope.environment().options.complianceLevel <= ClassFileConstants.JDK1_3))) {
        ...
        if (!(this.action == null || this.action.isEmptyBlock() || ((this.action.bits & IsUsefulEmptyStatementMASK) != 0))) {
            switch(this.kind) {
                case ARRAY:
        ...
        //end of loop
        FlowInfo mergedInfo = FlowInfo.mergedOptimizedBranches(loopingContext.initsOnBreak, false, flowInfo.initsWhenFalse(), false, /*for(;;){}while(true); unreachable(); */
        true);
        return mergedInfo;
    }
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        switch(this.kind) {
            ...
            case GENERIC_ITERABLE:
                ...
                MethodBinding iteratorMethodBinding = new MethodBinding(AccPublic, //$NON-NLS-1$
                "iterator".toCharArray(), scope.getJavaUtilIterator(), TypeConstants.NoParameters, TypeConstants.NoExceptions, (ReferenceBinding) collectionTypeBinding);
    ...
    public void resolve(BlockScope upperScope) {
        ...
        if (!hasError) {
            ...
            collectionType.isArrayType()) {
                ...
                ReferenceBinding iterableType = ((ReferenceBinding) collectionType).findSuperTypeErasingTo(T_JavaLangIterable, /*Iterable is not a class*/
                false);
                if (iterableType != null) {
                    if (// for(E e : Iterable<E>)
                    iterableType.isParameterizedType()) {
                        ParameterizedTypeBinding parameterizedType = (ParameterizedTypeBinding) iterableType;
                        if (// per construction can only be one
                        parameterizedType.arguments.length == 1) {
                    ...
                    } else if (// for (T t : Iterable<T>) - in case used inside Iterable itself
                    ...
                    } else if (// for(Object o : Iterable)
                    iterableType.isRawType()) {
