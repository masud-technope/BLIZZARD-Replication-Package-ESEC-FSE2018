...
public class DiskIndex {
    ...
    // 1, 2 or more bytes... depends on # of document names
    ...
    private int bufferIndex, bufferEnd;
    ...
    //$NON-NLS-1$
    public static final String SIGNATURE = "INDEX VERSION 1.129";
    ...
    SimpleSet addDocumentNames(String substring, MemoryIndex memoryIndex) throws IOException {
        ...
        if (substring == null) {
            if (memoryIndex == null) {
                for (int i = 0, l = docNames.length; i < l; i++) results.add(docNames[i]);
            } else {
            ...
            if (memoryIndex == null) {
                for (int i = 0, l = docNames.length; i < l; i++) if (docNames[i].startsWith(substring, 0))
    ...
    HashtableOfObject addQueryResults(char[][] categories, char[] key, int matchRule, MemoryIndex memoryIndex) throws IOException {
        ...
        if (key == null) {
            for (int i = 0, l = categories.length; i < l; i++) {
                ...
                HashtableOfObject wordsToDocNumbers = readCategoryTable(categories[i], true);
                if (wordsToDocNumbers != null) {
                    ...
                    for (int j = 0, m = words.length; j < m; j++) if (words[j] != null)
    ...
    private String[] computeDocumentNames(String[] onDiskNames, int[] positions, SimpleLookupTable indexedDocuments, MemoryIndex memoryIndex) {
        ...
        if (onDiskLength == 0) {
            ...
            for (int i = 0, l = referenceTables.length; i < l; i++) if (referenceTables[i] != null)
            ...
            for (int i = 0, l = added.length; i < l; i++) if (added[i] != null)
                newDocNames[count++] = (String) added[i];
            Util.sort(newDocNames);
            for (int i = 0, l = newDocNames.length; i < l; i++) indexedDocuments.put(newDocNames[i], Integer.valueOf(i));
            return newDocNames;
        }
        // initialize positions as if each document will remain in the same position
        for (int i = 0; i < onDiskLength; i++) positions[i] = i;
        ...
        if (numDeletedDocNames > 0 || indexedDocuments.elementSize > 0) {
            ...
            int count = 0;
            for (int i = 0; i < onDiskLength; i++) if (positions[i] >= RE_INDEXED)
                // keep each unchanged document
                newDocNames[count++] = onDiskNames[i];
            Object[] added = indexedDocuments.keyTable;
            for (int i = 0, l = added.length; i < l; i++) if (added[i] != null)
                // add each new document
                newDocNames[count++] = (String) added[i];
            Util.sort(newDocNames);
            for (int i = 0, l = newDocNames.length; i < l; i++) if (indexedDocuments.containsKey(newDocNames[i]))
            ...
            switch(positions[i]) {
                ...
                case RE_INDEXED:
                    ...
                    if (newName.equals(onDiskNames[i])) {
                        ...
                        indexedDocuments.put(newName, Integer.valueOf(count));
                        i++;
                    }
                    break;
                default:
        ...
        if (this.indexLocation.exists()) {
            if (reuseExistingFile) {
                ...
                try {
                    ...
                    if (// file is empty if its not set
                    this.headerInfoOffset > 0) {
                        // assume that the header info offset is over current buffer end
            ...
            if (!this.indexLocation.delete()) {
                if (DEBUG)
                    ...
                    "initialize - Failed to delete index " + this.indexLocation);
                //$NON-NLS-1$
    ...
    private void initializeFrom(DiskIndex diskIndex, File newIndexFile) throws IOException {
        if (// delete the temporary index file
        newIndexFile.exists() && !newIndexFile.delete()) {
            if (DEBUG)
                //$NON-NLS-1$
    ...
    private void mergeCategories(DiskIndex onDisk, int[] positions, FileOutputStream stream) throws IOException {
        ...
        for (int i = 0, l = categoryNames.length; i < l; i++) if (categoryNames[i] != null)
            ...
            nextWord: for (int i = 0, l = oldWords.length; i < l; i++) {
                ...
                if (oldWord != null) {
                    ...
                    for (int j = 0; j < length; j++) {
                        ...
                        if (// forget any reference to a document which was deleted or re_indexed
                        pos > RE_INDEXED)
                    ...
                    if (o == null) {
                        ...
                        for (int j = 0; j < count; j++) list.add(mappedNumbers[j]);
                    }
                }
            }
            // flush cached table
    ...
    DiskIndex mergeWith(MemoryIndex memoryIndex) throws IOException {
        ...
        // for each new/changed document in the memoryIndex
        ...
        if (docNames.length == 0) {
            // nothing to do... memory index contained deleted documents that had never been saved
            if (previousLength == 0)
                return this;
        ...
        if (// the tmp file could not be renamed last time
        indexFilePath.endsWith(TMP_EXT)) {
        ...
        try {
            ...
            try {
                ...
                if (indexedDocuments.elementSize > 0) {
                    ...
                    for (int i = 0, l = names.length; i < l; i++) if (names[i] != null)
                        newDiskIndex.copyQueryResults((HashtableOfObject) memoryIndex.docsToReferences.get(names[i]), ((Integer) integerPositions[i]).intValue());
                }
                // free up the space
            ...
            if (oldIndexFile.exists() && !oldIndexFile.delete()) {
                if (DEBUG)
                    //$NON-NLS-1$
                    System.out.println(//$NON-NLS-1$
                    "mergeWith - Failed to delete " + this.indexLocation);
                //$NON-NLS-1$
            ...
            if (!usingTmp && !newIndexFile.renameTo(oldIndexFile)) {
                ...
                if (!newIndexFile.renameTo(oldIndexFile)) {
                    if (DEBUG)
                        System.out.println("mergeWith - Failed to rename " + //$NON-NLS-1$
                        this.indexLocation);
                    usingTmp = true;
                }
            }
        ...
        if (// rename done, use the new file
        !usingTmp)
    ...
    private synchronized String[] readAllDocumentNames() throws IOException {
        ...
        try {
            ...
            for (int i = 0; i < this.numberOfChunks; i++) readChunk(docNames, stream, i * CHUNK_SIZE, i < lastIndex ? CHUNK_SIZE : this.sizeOfLastChunk);
            return docNames;
        } finally {
            stream.close();
            this.indexLocation.close();
    ...
    private synchronized HashtableOfObject readCategoryTable(char[] categoryName, boolean readDocNumbers) throws IOException {
        ...
        if (this.categoryTables == null) {
            ...
            if (cachedTable != null) {
                if (// must cache remaining document number arrays
                readDocNumbers) {
                    Object[] arrayOffsets = cachedTable.valueTable;
                    for (int i = 0, l = arrayOffsets.length; i < l; i++) if (arrayOffsets[i] instanceof Integer)
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (arrayOffset <= 0) {
                    ...
                    categoryTable.putUnsafely(word, readStreamDocumentArray(stream, arrayOffset));
                } else {
                    // read actual offset
                    arrayOffset = readStreamInt(stream);
                    if (readDocNumbers) {
                    ...
                    categoryTable.putUnsafely(word, Integer.valueOf(arrayOffset));
                }
            }
        ...
        if (matchingWords != null && count > 0) {
            ...
            try {
                ...
                this.bufferIndex = 0;
                this.bufferEnd = stream.read(this.streamBuffer, 0, this.streamBuffer.length);
                for (// each array follows the previous one
                int i = 0; // each array follows the previous one
                i < count; // each array follows the previous one
                i++) {
    ...
    private void readHeaderInfo(InputStream stream) throws IOException {
        ...
        for (int i = 0; i < this.numberOfChunks; i++) this.chunkOffsets[i] = readStreamInt(stream);
 ...
 */
    ...
    private void writeAllDocumentNames(String[] sortedDocNames, FileOutputStream stream) throws IOException {
        ...
        this.documentReferenceSize = size <= 0x7F ? 1 : (size <= 0x7FFF ? 2 : 4);
        this.chunkOffsets = new int[this.numberOfChunks];
        int lastIndex = this.numberOfChunks - 1;
        for (int i = 0; i < this.numberOfChunks; i++) {
            ...
            if (o != null) {
                ...
                if (o instanceof int[]) {
                    ...
                    if (documentNumbers.length == 1)
                        ...
                        writeStreamInt(stream, -documentNumbers[0]);
                    else
                        writeDocumentNumbers(documentNumbers, stream);
                } else {
                    // mark to identify that an offset follows
                    writeStreamInt(stream, largeArraySize);
                    // offset in the file of the array of document numbers
                    writeStreamInt(stream, ((Integer) o).intValue());
                }
            }
        }
    }
    ...
    private void writeOffsetToHeader(int offsetToHeader) throws IOException {
        if (offsetToHeader > 0) {
            //$NON-NLS-1$
            RandomAccessFile file = new RandomAccessFile(this.indexLocation.getIndexFile(), "rw");
            try {
