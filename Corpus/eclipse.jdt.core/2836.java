...
public class SelectionOnMessageSend extends MessageSend {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (type != null && type.isPolyType())
            // wait for more inference/resolution
            return type;
        // tolerate some error cases
        if (this.binding == null || !(this.binding.isValidBinding() || this.binding.problemId() == ProblemReasons.NotVisible || this.binding.problemId() == ProblemReasons.InheritedNameHidesEnclosingName || this.binding.problemId() == ProblemReasons.NonStaticReferenceInConstructorInvocation || this.binding.problemId() == ProblemReasons.NonStaticReferenceInStaticContext)) {
            throw new SelectionNodeFound();
        } else {
            if (this.binding.isDefaultAbstract()) {
                throw new SelectionNodeFound(findNonDefaultAbstractMethod(// 23594
                this.binding));
            } else {
