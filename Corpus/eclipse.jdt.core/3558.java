...
public class ConstructorLocator extends PatternLocator {
    ...
    public int match(ASTNode node, MatchingNodeSet nodeSet) {
        // interested in ExplicitConstructorCall
 ...
 */
    ...
    protected void matchReportReference(ASTNode reference, IJavaElement element, Binding elementBinding, int accuracy, MatchLocator locator) throws CoreException {
        ...
        constructorBinding instanceof ParameterizedGenericMethodBinding) {
        ...
        this.pattern.hasConstructorArguments()) {
            this.match.setRule(SearchPattern.R_ERASURE_MATCH);
        }
        // impossible match
    ...
    protected int resolveLevel(TypeDeclaration type) {
        ...
        if (methods != null) {
            for (int i = 0, length = methods.length; i < length; i++) {
                AbstractMethodDeclaration method = methods[i];
                if (// if synthetic
                method.isDefaultConstructor() && method.sourceStart < type.bodyStart)
