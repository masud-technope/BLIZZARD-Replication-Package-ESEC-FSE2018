...
public class ExternalAnnotationProvider {
    ...
    private void initialize(InputStream input) throws IOException {
        try (LineNumberReader reader = new LineNumberReader(new InputStreamReader(input))) {
            ...
            if (line.startsWith(TYPE_PARAMETER_PREFIX)) {
                if (// skip first line, second line may contain type parameter annotations
            ...
            do {
                ...
                try {
                    ...
                    if (// first signature line is mandatory
                    line != null && !line.isEmpty() && line.charAt(0) == ' ')
                    ...
                    if (line == null || line.isEmpty())
                        // skip since optional line with annotations is missing
                        continue;
                    if (line.charAt(0) != ' ') {
                        // push back what appears to be the next selector, not a signature
                        pendingLine = line;
                        continue;
                    }
                ...
                if (rawSig == null || annotSig == null) {
                    ...
                    "Illegal format for annotation file at line " + //$NON-NLS-1$
                    errLine);
                }
    ...
    public String toString() {
        ...
        sb.append("Methods:\n");
        if (this.methodAnnotationSources != null)
            for (Entry<String, String> e : this.methodAnnotationSources.entrySet()) sb.append('\t').append(e.getKey()).append('\n');
        return sb.toString();
    }
    ...
    public class TypeParametersAnnotationWalker extends BasicAnnotationWalker {
        ...
        // indices of start positions for type parameters per rank
        int[] rankStarts;
                    ...
                    for (int i = pos; i < length; i++) {
                        switch(this.source[i]) {
                            ...
                            case Util.C_COLON:
                                ...
                                // peek next
                                i++;
                                while (i < length && this.source[i] == Util.C_ARRAY) i++;
    ...
    class MethodAnnotationWalker extends BasicAnnotationWalker implements IMethodAnnotationWalker {
        ...
        public ITypeAnnotationWalker toMethodParameter(short index) {
            ...
            // leverage the fact that all parameters are evaluated in order
