...
public class DoStatement extends Statement {
	 ...
	 */
        ...
        FlowInfo condInfo = this.condition.analyseCode(currentScope, (condLoopContext = new LoopingFlowContext(flowContext, flowInfo, this, null, null, currentScope, true)), (this.action == null ? actionInfo : (actionInfo.mergedWith(loopingContext.initsOnContinue))).copy());
	...
	*/
        ...
        // end of loop
        FlowInfo mergedInfo = FlowInfo.mergedOptimizedBranches((loopingContext.initsOnBreak.tagBits & FlowInfo.UNREACHABLE) != 0 ? loopingContext.initsOnBreak : flowInfo.unconditionalCopy().addInitializationsFrom(loopingContext.initsOnBreak), // recover upstream null info
        isConditionOptimizedTrue, (condInfo.tagBits & FlowInfo.UNREACHABLE) == 0 ? // https://bugs.eclipse.org/bugs/show_bug.cgi?id=380927
        flowInfo.copy().addInitializationsFrom(condInfo.initsWhenFalse()) : condInfo, // never consider opt false case for DO loop, since break can always occur (47776)
        false, /*do{}while(true); unreachable(); */
        !isConditionTrue);
