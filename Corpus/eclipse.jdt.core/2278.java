...
public class JavaModelStatus extends Status implements IJavaModelStatus, IJavaModelStatusConstants {
    ...
    public  JavaModelStatus(int severity, int code, String string) {
        //$NON-NLS-1$
        super(severity, JavaCore.PLUGIN_ID, code, "JavaModelStatus", null);
        this.elements = JavaElement.NO_ELEMENTS;
        this.path = null;
        this.string = string;
    }
            ...
            switch(getCode()) {
                ...
                case INVALID_PATH:
                    if (this.string != null) {
                        ...
                        return Messages.bind(Messages.status_invalidPath, new String[] { //$NON-NLS-1$
                        getPath() == null ? //$NON-NLS-1$
                        "null" : //$NON-NLS-1$
                        getPath().toString() });
                    }
                ...
                case NAME_COLLISION:
                    ...
                    if (this.string != null) {
                        ...
                        return Messages.bind(//$NON-NLS-1$
                ...
                case CANNOT_RETRIEVE_ATTACHED_JAVADOC:
                    if (this.elements != null && this.elements.length == 1) {
                        ...
                        return Messages.bind(Messages.status_cannot_retrieve_attached_javadoc, //$NON-NLS-1$
                        ((JavaElement) this.elements[0]).toStringWithAncestors(), "");
                    }
                    if (this.string != null) {
                        return //$NON-NLS-1$
                        Messages.bind(//$NON-NLS-1$
                        Messages.status_cannot_retrieve_attached_javadoc, //$NON-NLS-1$
                ...
                case CANNOT_RETRIEVE_ATTACHED_JAVADOC_TIMEOUT:
                    if (this.elements != null && this.elements.length == 1) {
                        ...
                        return Messages.bind(Messages.status_timeout_javadoc, //$NON-NLS-1$
                        ((JavaElement) this.elements[0]).toStringWithAncestors(), "");
                    }
                    if (this.string != null) {
                        return //$NON-NLS-1$
                        Messages.bind(//$NON-NLS-1$
                        Messages.status_timeout_javadoc, //$NON-NLS-1$
                        this.string, //$NON-NLS-1$
                        "");
                    }
                    break;
