...
public class ErrorsTests extends BuilderTests {
    ...
    public void testRenameToNonJava() throws JavaModelException {
        ...
        //$NON-NLS-1$
        env.setOutputFolder(projectPath, "bin");
        IPath cuPath = //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "p1", //$NON-NLS-1$ //$NON-NLS-2$
        "X", //$NON-NLS-1$
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        expectingOnlySpecificProblemFor(cuPath, new Problem("X", "Y cannot be resolved to a type", cuPath, 35, 36, CategorizedProblem.CAT_TYPE, IMarker.SEVERITY_ERROR));
    ...
    public void test0104() throws JavaModelException {
        ...
        fullBuild();
        Problem[] prob1 = env.getProblemsFor(classTest1);
        expectingSpecificProblemFor(classTest1, new Problem("p1", "The type java.lang.Object cannot be resolved. It is indirectly referenced from required .class files", classTest1, 0, 1, CategorizedProblem.CAT_BUILDPATH, IMarker.SEVERITY_ERROR));
        assertEquals(JavaBuilder.SOURCE_ID, prob1[0].getSourceId());
    }
    ...
    public void test0108() throws JavaModelException {
        ...
        env.addEntry(projectPath, JavaCore.newSourceEntry(root, null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE }));
    ...
    public void test0109() throws JavaModelException {
        ...
        env.addEntry(projectPath, JavaCore.newSourceEntry(src, null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE }));
    ...
    public void test0110() throws JavaModelException {
        ...
        IPath src2 = new Path("/P/src2");
        env.addEntry(projectPath, JavaCore.newSourceEntry(src, null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE }));
        env.addEntry(projectPath, JavaCore.newSourceEntry(src2, null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE }));
        env.addClass(src, "p", "X", "package p;\n" + "public class X {\n" + "	public void foo() {\n" + "		int i;\n" + "	}\n" + "}");
    ...
    public void test0111() throws JavaModelException {
        ...
        IPath root = new Path("/P/src");
        env.addEntry(projectPath, JavaCore.newSourceEntry(root, null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE }));
        IPath classX = env.addClass(root, "p", "X", "package p;\n" + "public class X {\n" + "	public void foo() {\n" + "		int i;\n" + "	}\n" + "	public void bar() {\n" + "		a++;\n" + "	}\n" + "}");
        fullBuild(projectPath);
