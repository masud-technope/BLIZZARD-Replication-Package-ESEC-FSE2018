...
class JavaProjectElementInfo extends /* package */
OpenableElementInfo {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    switch(res.getType()) {
                        case IResource.FILE:
                            ...
                            if (isClasspathResolved && isClasspathEntryOrOutputLocation(/* see https://bugs.eclipse.org/bugs/show_bug.cgi?id=244406 */
                            resFullPath, res.getLocation(), classpath, projectOutput)) {
                                break;
                            }
                            // ignore .java file if src == project
                            if (srcIsProject && Util.isValidCompilationUnitName(resName, sourceLevel, complianceLevel) && !Util.isExcluded(res, inclusionPatterns, exclusionPatterns)) {
                                break;
                            }
                        ...
                        case IResource.FOLDER:
                            ...
                            if ((srcIsProject && !Util.isExcluded(res, inclusionPatterns, exclusionPatterns) && Util.isValidFolderNameForPackage(res.getName(), sourceLevel, complianceLevel)) || (isClasspathResolved && isClasspathEntryOrOutputLocation(resFullPath, res.getLocation(), classpath, projectOutput))) {
                                break;
                            }
                            // else add non java resource
                            if (resources.length == resourcesCounter) {
    ...
    NameLookup newNameLookup(JavaProject project, ICompilationUnit[] workingCopies) {
        ...
        if (allPkgFragmentsCache == null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (// see https://bugs.eclipse.org/bugs/show_bug.cgi?id=183833
                fragmentsCache == null) {
