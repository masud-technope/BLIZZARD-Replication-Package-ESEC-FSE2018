        ...
        if (expressions != null) {
            ...
            for (int i = 0; i < expressionLength; i++) {
                ...
                if ((expr = expressions[i]).constant != NotAConstant) {
                    switch(// filter out initializations to default values
                    elementsTypeID) {
                        case T_int:
                        case T_short:
                        case T_byte:
        ...
        if (expectedTb.isArrayType()) {
            ...
            if (expectedElementsTb.isBaseType()) {
                for (int i = 0, length = expressions.length; i < length; i++) {
                    ...
                    TypeBinding expressionTb = (expression instanceof ArrayInitializer) ? expression.resolveTypeExpecting(scope, expectedElementsTb) : expression.resolveType(scope);
                ...
                for (int i = 0, length = expressions.length; i < length; i++) if (expressions[i].resolveTypeExpecting(scope, expectedElementsTb) == null)
                    return null;
            }
            return binding;
        }
    ...
    public void traverse(ASTVisitor visitor, BlockScope scope) {
        if (visitor.visit(this, scope)) {
            if (expressions != null) {
                ...
                for (int i = 0; i < expressionsLength; i++) expressions[i].traverse(visitor, scope);
            }
        }
