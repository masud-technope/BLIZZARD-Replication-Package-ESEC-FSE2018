...
public class TypeSystem {
    ...
    public final class HashedParameterizedTypes {
        ...
        ParameterizedTypeBinding get(ReferenceBinding genericType, TypeBinding[] typeArguments, ReferenceBinding enclosingType, AnnotationBinding[] annotations) {
            ...
            for (int i = 0, length = parameterizedTypeBindings == null ? 0 : parameterizedTypeBindings.length; i < length; i++) {
                ...
                parameterizedType.actualType() != genericTypeToMatch) {
                    continue;
                }
                if (//$IDENTITY-COMPARISON$
                parameterizedType.enclosingType() != enclosingTypeToMatch || !Util.effectivelyEqual(parameterizedType.typeArguments(), typeArgumentsToMatch))
                    continue;
    ...
    // auxiliary fast lookup table for parameterized types.
    protected HashedParameterizedTypes parameterizedTypes;

    // cannot store in types, since AnnotationBinding is not a TypeBinding and we don't want types to operate at Binding level.
    ...
    public final TypeBinding getUnannotatedType(TypeBinding type) {
        ...
        if (type.isUnresolvedType()) {
            ...
            if (resolvedType != null) {
                ...
                try {
                    // to ensure unique id assignment (when enclosing type is parameterized, inner type is also) 
        ...
        try {
            if (type.id == TypeIds.NoId) {
                ...
                if (nakedType != null)
                    return nakedType;
                // well known type, assigned id elsewhere.
	...
	*/
    public ArrayBinding getArrayType(TypeBinding leafType, int dimensions) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (!derivedType.isArrayType() || derivedType.hasTypeAnnotations())
                continue;
            if (//$IDENTITY-COMPARISON$
            derivedType.leafComponentType() == unannotatedLeafType && derivedType.dimensions() == dimensions)
    ...
    public ReferenceBinding getMemberType(ReferenceBinding memberType, ReferenceBinding enclosingType) {
        // nothing to do for plain vanilla type system, they are already hooked.
        return memberType;
    }
    ...
    public RawTypeBinding getRawType(ReferenceBinding genericType, ReferenceBinding enclosingType) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (derivedType == null)
                break;
            if (//$IDENTITY-COMPARISON$
            !derivedType.isRawType() || derivedType.actualType() != unannotatedGenericType || derivedType.hasTypeAnnotations())
                continue;
            if (//$IDENTITY-COMPARISON$
	...
	*/
    public WildcardBinding getWildcard(ReferenceBinding genericType, int rank, TypeBinding bound, TypeBinding[] otherBounds, int boundKind) {
        ...
        // by construction, cachedInfo != null now.
        TypeBinding[] derivedTypes = this.types[unannotatedGenericType.id];
        int i, length = derivedTypes.length;
        for (i = 0; i < length; i++) {
            TypeBinding derivedType = derivedTypes[i];
            if (derivedType == null)
                break;
            if (//$IDENTITY-COMPARISON$
            !derivedType.isWildcard() || derivedType.actualType() != unannotatedGenericType || derivedType.hasTypeAnnotations())
                continue;
            if (//$IDENTITY-COMPARISON$
            derivedType.rank() != rank || derivedType.boundKind() != boundKind || derivedType.bound() != unannotatedBound)
                continue;
    ...
    public final CaptureBinding getCapturedWildcard(WildcardBinding wildcard, ReferenceBinding contextType, int start, int end, ASTNode cud, int id) {
		...
		*/
        ...
        for (i = length - 1; i >= -1; --i) {
            ...
            if (// Searching further to the left is futile, exit the loop.
            prior.cud != cud) {
                i = nullSlot;
                break;
            }
            if (//$IDENTITY-COMPARISON$
            prior.sourceType != contextType || prior.start != start || prior.end != end)
                continue;
            return prior;
        }
        if (i == length) {
    ...
    public TypeBinding getAnnotatedType(TypeBinding type, AnnotationBinding[][] annotations) {
        // Nothing to do for plain vanilla type system.
        return type;
    }
	...
	*/
    ...
    public void updateCaches(UnresolvedReferenceBinding unresolvedType, ReferenceBinding resolvedType) {
        ...
        if (// update the key
        this.annotationTypes.get(unresolvedType) != null) {
    ...
    public final TypeBinding getIntersectionType18(ReferenceBinding[] intersectingTypes) {
        ...
        next: for (i = 0; i < length; i++) {
            ...
            for (int j = 0; j < intersectingTypesLength; j++) {
                if (//$IDENTITY-COMPARISON$
                intersectingTypes[j] != priorIntersectingTypes[j])
                    continue next;
            }
            return derivedType;
        }
