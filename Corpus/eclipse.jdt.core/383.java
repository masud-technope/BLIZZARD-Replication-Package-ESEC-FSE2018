...
public class LabeledStatement extends Statement {
    ...
    public  LabeledStatement(char[] label, Statement statement, int sourceStart, int sourceEnd) {
        ...
        if (statement instanceof EmptyStatement)
            statement.bits |= IsUsefulEmptyStatementMASK;
        this.label = label;
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (statement == null) {
            ...
            FlowInfo mergedInfo = statement.analyseCode(currentScope, (labelContext = new LabelFlowContext(flowContext, this, label, (targetLabel = new Label()), currentScope)), flowInfo).mergedWith(labelContext.initsOnBreak);
    ...
    public StringBuffer printStatement(int tab, StringBuffer output) {
        //$NON-NLS-1$
        printIndent(tab, output).append(label).append(": ");
