...
public class Assignment extends Expression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        boolean shouldAnalyseResource = local != null && flowInfo.reachMode() == FlowInfo.REACHABLE && compilerOptions.analyseResourceLeaks && (FakedTrackingVariable.isAnyCloseable(this.expression.resolvedType) || this.expression.resolvedType == TypeBinding.NULL);
        ...
        if (compilerOptions.isAnnotationBasedNullAnalysisEnabled) {
            ...
            if (var != null) {
                ...
                if (nullStatus == FlowInfo.NON_NULL && var instanceof FieldBinding && this.lhs instanceof Reference && compilerOptions.enableSyntacticNullAnalysisForFields) {
                    int timeToLive = (this.bits & InsideExpressionStatement) != 0 ? // assignment is statement: make info survives the end of this statement
                    2 : // assignment is expression: expire on next event.
                    1;
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (lhsType != null) {
            // make it unique, `this' shares source end with 'this.expression'.
        ...
        if (localVariableBinding != null && (localVariableBinding.isCatchParameter() || localVariableBinding.isParameter())) {
            // as it is already definitely assigned, we can conclude already. Also note: catch parameter cannot be compound assigned.
        ...
        TypeBinding.notEquals(lhsType, rhsType)) {
 ...
 */
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
        ...
        if (TypeBinding.equalsEquals(expectedType, TypeBinding.BOOLEAN) && TypeBinding.equalsEquals(lhsType, TypeBinding.BOOLEAN) && (this.lhs.bits & IsStrictlyAssigned) != 0) {
