...
public class ParticipantBuildTests extends BuilderTests {
    ...
    public void testBuildStarting() throws JavaModelException {
        ...
        new BuildTestParticipant() {
            ...
            public void buildStarting(BuildContext[] files, boolean isBatchBuild) {
                ...
                IFile genedType = //$NON-NLS-1$
                result.getFile().getParent().getFile(//$NON-NLS-1$
                new Path("GeneratedType.java"));
                if (this.buildPass == 0 || this.buildPass == 3) {
                    try {
                        //$NON-NLS-1$
                        genedType.create(//$NON-NLS-1$
                        new ByteArrayInputStream("public class GeneratedType {}".getBytes()), //$NON-NLS-1$
                        true, //$NON-NLS-1$
                        null);
    ...
    public void testDefaultValue() throws JavaModelException {
        ...
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "test", //$NON-NLS-1$ //$NON-NLS-2$
        "ClassWithNestedAnnotation", //$NON-NLS-1$
        "package test;\n" + //$NON-NLS-1$
        ...
        new BuildTestParticipant() {
            ...
            public void processAnnotations(BuildContext[] files) {
                for (int i = 0, total = files.length; i < total; i++) {
                    ...
                    if (//$NON-NLS-1$
                    !"EntryPoint.java".equals(file.getName()))
                        //$NON-NLS-1$
                        continue;
                        ...
                        for (int f = 0, fLength = fieldBindings.length; f < fLength; f++) {
                            ...
                            if ("nestedAnnotation".equals(field.getName())) {
                                ...
                                for (int nf = 0, nfLength = nestedFields.length; nf < nfLength; nf++) {
                                    if (!nestedFields[nf].getName().equals("FOUR"))
                                        continue;
            ...
            public void processAnnotations(BuildContext[] files) {
                ...
                if (this.count == 2) {
                    ...
                    try {
                        //$NON-NLS-1$
                        genedType.create(//$NON-NLS-1$
                        new ByteArrayInputStream("public @interface MissingAnnotation {}".getBytes()), //$NON-NLS-1$
                        true, //$NON-NLS-1$
                        null);
                    } catch (CoreException e) {
                        e.printStackTrace();
    ...
    public void testProcessAnnotationQualifiedReferences() throws JavaModelException {
        ...
        new BuildTestParticipant() {
            ...
            public void processAnnotations(BuildContext[] files) {
                ...
                BuildContext result = files[0];
                IFile genedType = //$NON-NLS-1$
                result.getFile().getProject().getFile(//$NON-NLS-1$
                new Path("src/p1/p2/GeneratedType.java"));
                if (genedType.exists())
                    return;
                try {
                    ...
                    //$NON-NLS-1$
                    genedType.create(//$NON-NLS-1$
                    new ByteArrayInputStream("package p1.p2; public class GeneratedType { public static void method(){} }".getBytes()), //$NON-NLS-1$
                    true, //$NON-NLS-1$
                    null);
                } catch (CoreException e) {
                    e.printStackTrace();
    ...
    public void testProcessAnnotationReferences() throws JavaModelException {
        ...
        new BuildTestParticipant() {
            ...
            public void processAnnotations(BuildContext[] files) {
                ...
                IFile genedType = //$NON-NLS-1$
                result.getFile().getParent().getFile(//$NON-NLS-1$
                new Path("GeneratedAnnotation.java"));
                if (genedType.exists())
                    return;
                try {
                    //$NON-NLS-1$
                    genedType.create(//$NON-NLS-1$
                    new ByteArrayInputStream("@interface GeneratedAnnotation {}".getBytes()), //$NON-NLS-1$
                    true, //$NON-NLS-1$
                    null);
                } catch (CoreException e) {
                    e.printStackTrace();
