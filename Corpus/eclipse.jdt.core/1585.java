...
*/
    ...
    void addType(ReferenceBinding element) {
        ...
        if (this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled)
            if (// unresolved types don't yet have the modifiers set
            element.isAnnotationType() || element instanceof UnresolvedReferenceBinding)
...
*/
    public Binding getTypeOrPackage(char[] name) {
        ...
        if (referenceBinding != null && referenceBinding != LookupEnvironment.TheNotFoundType) {
            referenceBinding = (ReferenceBinding) BinaryTypeBinding.resolveType(referenceBinding, this.environment, /* no raw conversion for now */
            false);
            if (referenceBinding.isNestedType()) {
                return new ProblemReferenceBinding(new char[][] { name }, referenceBinding, ProblemReasons.InternalNameProvided);
            }
        ...
        if (// have not looked for it before
        packageBinding == null) {
            if ((packageBinding = findPackage(name)) != null) {
                return packageBinding;
            }
            if (referenceBinding != null && referenceBinding != LookupEnvironment.TheNotFoundType) {
                // found cached missing type - check if package conflict
                return referenceBinding;
            }
            addNotFoundPackage(name);
        }
...
*/
    ...
    private boolean isPackageOfQualifiedTypeName(char[][] packageName, char[][] typeName) {
        ...
        if (typeName == null || (length = packageName.length) != typeName.length - 1)
            return false;
        for (int i = 0; i < length; i++) if (!CharOperation.equals(packageName[i], typeName[i]))
            return false;
        return true;
    }
    ...
    void checkIfNullAnnotationType(ReferenceBinding type) {
        ...
        if (this.environment.nullableAnnotationPackage == this && CharOperation.equals(type.compoundName, this.environment.getNullableAnnotationName())) {
            ...
            if (// unresolved will need to check back for the resolved type
