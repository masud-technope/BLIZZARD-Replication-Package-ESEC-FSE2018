...
public class ParameterizedGenericMethodBinding extends ParameterizedMethodBinding implements Substitution {
    ...
    // only set to true for instances resulting from method invocation inferrence
    ...
    public static MethodBinding computeCompatibleMethod(MethodBinding originalMethod, TypeBinding[] arguments, Scope scope, InvocationSite invocationSite) {
        ...
        computeSubstitutes: {
            ...
            if (inferenceContext.hasUnresolvedTypeArgument()) {
                if (// only remember unchecked status post 15.12.2.7
                inferenceContext.isUnchecked) {
    ...
    public static MethodBinding computeCompatibleMethod18(MethodBinding originalMethod, TypeBinding[] arguments, final Scope scope, InvocationSite invocationSite) {
        ...
        for (int i = 0, length = arguments.length, parametersLength = parameters.length; i < length; i++) {
            ...
            if (// Scope.cCM incorrectly but harmlessly uses isBaseType which answers true for null.
            argument.isPrimitiveType() != parameter.isPrimitiveType()) {
        ...
        try {
            ...
            if (result == null && originalMethod.isVarargs()) {
                ...
                infCtx18 = // start over
                invocationSite.freshInferenceContext(// start over
                scope);
            ...
            if (result != null) {
                ...
                if (solutions != null) {
                    ...
                    try {
                        if (// illegally working from the provisional result?
                        hasReturnProblem) {
    ...
    private static ParameterizedGenericMethodBinding inferFromArgumentTypes(Scope scope, MethodBinding originalMethod, TypeBinding[] arguments, TypeBinding[] parameters, InferenceContext inferenceContext) {
        ...
        if (!resolveSubstituteConstraints(scope, originalVariables, inferenceContext, /*ignore Ti<:Uk*/
        false))
            // impossible substitution
            return null;
    ...
    private static boolean resolveSubstituteConstraints(Scope scope, TypeVariableBinding[] typeVariables, InferenceContext inferenceContext, boolean considerEXTENDSConstraints) {
        ...
        nextTypeParameter: for (int i = 0; i < varLength; i++) {
            ...
            if (equalSubstitutes != null) {
                nextConstraint: for (int j = 0, equalLength = equalSubstitutes.length; j < equalLength; j++) {
    ...
    public  ParameterizedGenericMethodBinding(MethodBinding originalMethod, RawTypeBinding rawType, LookupEnvironment environment) {
        ...
        for (int i = 0; i < length; i++) {
            rawArguments[i] = environment.convertToRawType(originalVariables[i].erasure(), /*do not force conversion of enclosing types*/
            false);
        }
        this.isRaw = true;
        ...
        this.parameters = Scope.substitute(this, ignoreRawTypeSubstitution ? // no substitution if original was static
        originalMethod.parameters : Scope.substitute(rawType, originalMethod.parameters));
        this.thrownExceptions = Scope.substitute(this, ignoreRawTypeSubstitution ? // no substitution if original was static
        originalMethod.thrownExceptions : Scope.substitute(rawType, originalMethod.thrownExceptions));
        // error case where exception type variable would have been substituted by a non-reference type (207573)
        if (this.thrownExceptions == null)
            this.thrownExceptions = Binding.NO_EXCEPTIONS;
        this.returnType = Scope.substitute(this, ignoreRawTypeSubstitution ? // no substitution if original was static
        originalMethod.returnType : Scope.substitute(rawType, originalMethod.returnType));
    ...
    public  ParameterizedGenericMethodBinding(MethodBinding originalMethod, TypeBinding[] typeArguments, LookupEnvironment environment, boolean inferredWithUncheckConversion, boolean hasReturnProblem) {
        ...
        inferredWithUncheckConversion) {
            // propagate simulation of Bug JDK_8026527
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        buffer.append(this.originalMethod.computeUniqueKey(/*not a leaf*/
        false));
        buffer.append('%');
        buffer.append('<');
        if (!this.isRaw) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                buffer.append(typeArgument.computeUniqueKey(/*not a leaf*/
                false));
            }
        }
        buffer.append('>');
    ...
    private ParameterizedGenericMethodBinding inferFromExpectedType(Scope scope, InferenceContext inferenceContext) {
        ...
        if (!resolveSubstituteConstraints(scope, originalVariables, inferenceContext, /*consider Ti<:Uk*/
        true))
            // incompatible
            return null;
    ...
    private static class LingeringTypeVariableEliminator implements Substitution {
        ...
        // when null, substitute type variables by unbounded wildcard
        ...
        public TypeBinding substitute(TypeVariableBinding typeVariable) {
            if (// not kosher, don't touch.
            typeVariable.rank >= this.variables.length || TypeBinding.notEquals(this.variables[typeVariable.rank], typeVariable)) {
                return typeVariable;
            }
    ...
    public MethodBinding genericMethod() {
        if (// mostSpecificMethodBinding() would need inference, but that doesn't work well for raw methods
        this.isRaw)
            // -> prefer traditional comparison using the substituted method
            return this;
