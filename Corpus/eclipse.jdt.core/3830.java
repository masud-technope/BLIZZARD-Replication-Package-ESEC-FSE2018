...
public class ArrayInitializer extends Expression {
    ...
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
        ...
        if (expectedType instanceof ArrayBinding) {
            // allow new List<?>[5]
            if (// annotation default value need only to be commensurate JLS9.7
            (this.bits & IsAnnotationDefaultValue) == 0) {
            ...
            for (int i = 0, length = this.expressions.length; i < length; i++) {
                ...
                TypeBinding.notEquals(elementType, expressionType))
    ...
    public void traverse(ASTVisitor visitor, BlockScope scope) {
        if (visitor.visit(this, scope)) {
            if (this.expressions != null) {
                ...
                for (int i = 0; i < expressionsLength; i++) this.expressions[i].traverse(visitor, scope);
            }
        }
