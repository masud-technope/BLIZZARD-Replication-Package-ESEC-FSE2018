...
public class ExternalAnnotations18Test extends ModifyingResourceTests {
    ...
    static class TestContainerInitializer implements ContainerInitializer.ITestInitializer {
        ...
        public void initialize(IPath containerPath, IJavaProject project) throws CoreException {
            ...
            for (int i = 0; i < jars.length; i++) entries[i] = JavaCore.newLibraryEntry(new Path(jars[i]), null, null);
            JavaCore.setClasspathContainer(new Path(TEST_CONTAINER_NAME), new IJavaProject[] { project }, new IClasspathContainer[] { new TestContainer(new Path(TEST_CONTAINER_NAME), entries) }, null);
        }
    ...
    void setupJavaProject(String name) throws CoreException, IOException {
        //$NON-NLS-1$
        this.project = setUpJavaProject(name, this.compliance);
    ...
    protected void addLibraryWithExternalAnnotations(IJavaProject javaProject, String jarName, String externalAnnotationPath, String[] pathAndContents, Map options) throws CoreException, IOException {
        ...
        IClasspathEntry entry = JavaCore.newLibraryEntry(new Path(jarPath), new Path('/' + javaProject.getProject().getName() + "/src.zip"), /*src attach root*/
        null, /*access rules*/
        null, extraAttributes, /*exported*/
        false);
        addClasspathEntry(this.project, entry);
    }

    protected void addProjectDependencyWithExternalAnnotations(IJavaProject javaProject, String referencedProjectName, String externalAnnotationPath, Map options) throws CoreException, IOException {
        IClasspathAttribute[] extraAttributes = new IClasspathAttribute[] { new ClasspathAttribute(IClasspathAttribute.EXTERNAL_ANNOTATION_PATH, externalAnnotationPath) };
        IClasspathEntry entry = JavaCore.newProjectEntry(new Path(referencedProjectName), /*access rules*/
        null, /*combine access rules*/
        false, extraAttributes, /*exported*/
        false);
        addClasspathEntry(this.project, entry);
    }
    ...
    protected void assertNoMarkers(IMarker[] markers) throws CoreException {
        for (int i = 0; i < markers.length; i++) System.err.println("Unexpected marker: " + markers[i].getAttributes().entrySet());
        assertEquals("Number of markers", 0, markers.length);
    }

    protected void assertNoProblems(IProblem[] problems) throws CoreException {
        for (int i = 0; i < problems.length; i++) System.err.println("Unexpected marker: " + problems[i]);
    ...
    protected void assertProblems(IProblem[] problems, String[] messages, int[] lines) throws CoreException {
        ...
        for (int i = 0; i < problems.length; i++) {
            for (int j = 0; j < messages.length; j++) {
                if (messages[j] == null)
                    continue;
                if (problems[i].toString().equals(messages[j]) && problems[i].getSourceLineNumber() == lines[j]) {
                    messages[j] = null;
                    problems[i] = null;
                    nMatch++;
                    break;
    ...
    public void testLibsWithFields() throws Exception {
        ...
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        IProblem[] problems = reconciled.getProblems();
        assertProblems(problems, new String[] { "Pb(953) Null type mismatch (type annotations): required '@NonNull String' but this expression has type '@Nullable String'" }, new int[] { 8 });
    }
    ...
    public void testLibsWithFieldsExternalZipped() throws Exception {
        ...
        addLibraryWithExternalAnnotations(this.project, "lib1.jar", zipPath, new String[] { "/UnannotatedLib/libs/Lib1.java", "package libs;\n" + "\n" + "public interface Lib1 {\n" + "	String one = \"1\";\n" + "	String none = null;\n" + "}\n" }, null);
        Util.createSourceZip(new String[] { "libs/Lib1.eea", "class libs/Lib1\n" + "\n" + "one\n" + " Ljava/lang/String;\n" + " L1java/lang/String;\n" + "\n" + "none\n" + " Ljava/lang/String;\n" + " L0java/lang/String;\n" + "\n" }, zipPath);
        IPackageFragment fragment = this.project.getPackageFragmentRoots()[0].createPackageFragment("tests", true, null);
        ICompilationUnit unit = fragment.createCompilationUnit("Test1.java", "package tests;\n" + "import org.eclipse.jdt.annotation.*;\n" + "\n" + "import libs.Lib1;\n" + "\n" + "public class Test1 {\n" + "	@NonNull String test0() {\n" + "		return Lib1.none;\n" + "	}\n" + "	@NonNull String test1() {\n" + "		return Lib1.one;\n" + "	}\n" + "}\n", true, new NullProgressMonitor()).getWorkingCopy(new NullProgressMonitor());
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
    ...
    public void testLibsWithTypeArgOfSuper() throws Exception {
        ...
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        IProblem[] problems = reconciled.getProblems();
        assertProblems(problems, new String[] { "Pb(953) Null type mismatch (type annotations): required '@NonNull Exception' but this expression has type '@Nullable Exception'", "Pb(953) Null type mismatch (type annotations): required '@NonNull String' but this expression has type '@Nullable String'" }, new int[] { 8, 9 });
    }
    ...
    public void testAnnotateFieldWithParameterizedType() throws Exception {
        myCreateJavaProject("TestLibs");
        String lib1Content = "package libs;\n" + "\n" + "public abstract class Lib1<T> {\n" + "	public Lib1<T> one;\n" + "	public abstract T get();\n" + "}\n";
        addLibraryWithExternalAnnotations(this.project, "lib1.jar", "annots", new String[] { "/UnannotatedLib/libs/Lib1.java", lib1Content }, null);
        // type check sources:
        IPackageFragment fragment = this.project.getPackageFragmentRoots()[0].createPackageFragment("tests", true, null);
        ICompilationUnit cu = fragment.createCompilationUnit("Test1.java", "package tests;\n" + "import org.eclipse.jdt.annotation.*;\n" + "\n" + "import libs.Lib1;\n" + "\n" + "public class Test1 {\n" + "	@NonNull String test0(Lib1<String> stringLib) {\n" + "		return stringLib.one.get();\n" + "	}\n" + "}\n", true, new NullProgressMonitor()).getWorkingCopy(new NullProgressMonitor());
        CompilationUnit reconciled = cu.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertProblems(reconciled.getProblems(), new String[] { "Pb(955) Null type safety (type annotations): The expression of type 'String' needs unchecked conversion to conform to '@NonNull String'" }, new int[] { 8 });
        // acquire library AST:
        ...
        ExternalAnnotationUtil.annotateMember("libs/Lib1", annotationFile, "one", originalSignature, "Llibs/Lib1<T0T;>;", MergeStrategy.OVERWRITE_ANNOTATIONS, null);
        ...
        assertProblems(reconciled.getProblems(), new String[] { "Pb(953) Null type mismatch (type annotations): required '@NonNull String' but this expression has type '@Nullable String'" }, new int[] { 8 });
    }
    ...
    public void testAnnotateMethodParameter() throws Exception {
        ...
        CompilationUnit reconciled = cu.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertProblems(reconciled.getProblems(), new String[] { "Pb(953) Null type mismatch (type annotations): required 'Lib1<Lib1.X,@Nullable String>' but this expression has type 'Lib1<Lib1.@Nullable X,@NonNull String>'" }, new int[] { 7 });
        // acquire library AST:
        IType type = this.project.findType("libs.Lib1");
        ICompilationUnit libWorkingCopy = type.getClassFile().getWorkingCopy(this.wcOwner, null);
        ASTParser parser = ASTParser.newParser(AST.JLS8);
        ...
        ExternalAnnotationUtil.annotateMember("libs/Lib1", annotationFile, "take", originalSignature, // <- two annotations: @Nullable X and @NonNull U
        "(Llibs/Lib1<L0libs/Lib1$X;T1U;>;)V", MergeStrategy.OVERWRITE_ANNOTATIONS, null);
        assertTrue("file should exist", annotationFile.exists());
    ...
    public void testAnnotateConstructorParameter() throws Exception {
        ...
        CompilationUnit reconciled = cu.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertProblems(reconciled.getProblems(), new String[] { "Pb(910) Null type mismatch: required '@NonNull String' but the provided value is null" }, new int[] { 7 });
        // acquire library AST:
        IType type = this.project.findType("libs.Lib1");
        ICompilationUnit libWorkingCopy = type.getClassFile().getWorkingCopy(this.wcOwner, null);
        ASTParser parser = ASTParser.newParser(AST.JLS8);
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        ExternalAnnotationUtil.annotateMember("libs/Lib1", annotationFile, "<init>", originalSignature, // <- @Nullable U
        "(IT0U;)V", MergeStrategy.OVERWRITE_ANNOTATIONS, null);
        assertTrue("file should exist", annotationFile.exists());
        // check that the error is resolved now:
        reconciled = cu.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertNoProblems(reconciled.getProblems());
        // invert annotation:
        ExternalAnnotationUtil.annotateMethodParameterType("libs/Lib1", annotationFile, "<init>", originalSignature, // <- @NonNull U
        "T1U;", // position 
        1, MergeStrategy.OVERWRITE_ANNOTATIONS, null);
        assertTrue("file should exist", annotationFile.exists());
    ...
    public void testBug470666a() throws CoreException, IOException {
        ...
        addLibraryWithExternalAnnotations(this.project, "lib1.jar", "annots", new String[] { "/UnannotatedLib/libs/Collectors.java", lib1Content }, null);
        // acquire library AST:
        ...
        libWorkingCopy.discardWorkingCopy();
        // find type binding:
        // bound of type param of method param
        int start = lib1Content.indexOf("T, ? extends U>");
        ASTNode name = NodeFinder.perform(unit, start, 0);
        assertTrue("should be simple name", name.getNodeType() == ASTNode.SIMPLE_NAME);
        ASTNode method = name.getParent();
        while (!(method instanceof MethodDeclaration)) method = method.getParent();
        IMethodBinding methodBinding = ((MethodDeclaration) method).resolveBinding();
        // find annotation file (not yet existing):
        IFile annotationFile = ExternalAnnotationUtil.getAnnotationFile(this.project, methodBinding.getDeclaringClass(), null);
        assertFalse("file should not exist", annotationFile.exists());
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull T
        "Llibs/Function<-T1T;+TU;>;", 0, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[<T:Ljava/lang/Object;U:Ljava/lang/Object;A:Ljava/lang/Object;R:Ljava/lang/Object;>(, " + "Llibs/Function<-TT;+TU;>;, " + // <- @NonNull T
        "Llibs/Function<-T1T;+TU;>;, " + "Llibs/Collector<-TU;TA;TR;>;)Llibs/Collector<TT;*TR;>;]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug470666b() throws CoreException, IOException {
        ...
        int start = lib1Content.indexOf("T, ?, R>");
        ASTNode name = NodeFinder.perform(unit, start, 0);
        assertTrue("should be simple name", name.getNodeType() == ASTNode.SIMPLE_NAME);
        ASTNode method = name.getParent();
        while (!(method instanceof MethodDeclaration)) method = method.getParent();
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateReturnType(originalSignature, // <- @NonNull T
        "Llibs/Collector<T1T;*TR;>;", MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[<T:Ljava/lang/Object;U:Ljava/lang/Object;A:Ljava/lang/Object;R:Ljava/lang/Object;>(Llibs/Function<-TT;+TU;>;Llibs/Collector<-TU;TA;TR;>;), " + "Llibs/Collector<TT;*TR;>;, " + // <- @NonNull T
        "Llibs/Collector<T1T;*TR;>;, " + "]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug464081() throws CoreException, IOException {
        ...
        addLibraryWithExternalAnnotations(this.project, "lib1.jar", "annots", new String[] { "/UnannotatedLib/libs/Collections.java", lib1Content }, null);
        // acquire library AST:
        ...
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull T
        "Llibs/List<+T1T;>;", 0, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[<T:Ljava/lang/Object;>(, " + "Llibs/List<+TT;>;, " + // <- @NonNull T
        "Llibs/List<+T1T;>;, " + ")Llibs/List<TT;>;]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug471352() throws CoreException, IOException {
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull Random
        "L1libs/Random;", 1, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[(Llibs/List<*>;, " + "Llibs/Random;, " + // <- @NonNull Random
        "L1libs/Random;, " + ")V]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug471034a() throws CoreException, IOException {
        ...
        addLibraryWithExternalAnnotations(this.project, "lib1.jar", "annots", new String[] { "/UnannotatedLib/libs/Thread.java", lib1Content }, null);
        // acquire library AST:
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull Thread
        "[L1libs/Thread;", 0, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[(, " + "[Llibs/Thread;, " + // <- @NonNull Thread
        "[L1libs/Thread;, " + ")I]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug471034b() throws CoreException, IOException {
        ...
        // annotate:
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull array
        "[1[Llibs/Thread;", 0, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[(, " + "[[Llibs/Thread;, " + // <- @NonNull array
        "[1[Llibs/Thread;, " + ")I]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBug471034c() throws CoreException, IOException {
        ...
        String originalSignature = ExternalAnnotationUtil.extractGenericSignature(methodBinding);
        String[] annotatedSign = ExternalAnnotationUtil.annotateParameterType(originalSignature, // <- @NonNull array
        "[1Llibs/Thread;", 0, MergeStrategy.OVERWRITE_ANNOTATIONS);
        assertEquals("dry-run result", "[(, " + "[Llibs/Thread;, " + // <- @NonNull array
        "[1Llibs/Thread;, " + ")I]", Arrays.toString(annotatedSign));
    }
    ...
    public void testBrokenConfig1() throws Exception {
        ...
        try {
            ...
            ICompilationUnit unit2 = fragment.createCompilationUnit("Test2.java", "package tests;\n" + "import libs.Lib2;\n" + "\n" + "public class Test2 {\n" + "	void test1(Lib2 lib) {\n" + "		lib.test(null);\n" + "	}\n" + "}\n", true, new NullProgressMonitor()).getWorkingCopy(new NullProgressMonitor());
    ...
    public void testProjectDependencyReconcile1() throws Exception {
        try {
            setupJavaProject("Lib");
            this.project.getProject().build(IncrementalProjectBuilder.FULL_BUILD, null);
            // prepare to get the root from project Test1
            this.root = null;
            setupJavaProject("Test1");
            addProjectDependencyWithExternalAnnotations(this.project, "/Lib", "annots", null);
            IPackageFragment fragment = this.root.getPackageFragment("test1");
    ...
    public void testProjectDependencyReconcile3() throws Exception {
        try {
            ...
            CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
            assertProblems(reconciled.getProblems(), new String[] { "Pb(964) Null constraint mismatch: The type '@Nullable B' is not a valid substitute for the type parameter '@NonNull R'", "Pb(964) Null constraint mismatch: The type '@Nullable String' is not a valid substitute for the type parameter '@NonNull V'" }, new int[] { 12, 17 });
        } finally {
            deleteProject("Lib");
        }
    }
    ...
    public void testFreeTypeVariableReturn() throws Exception {
        ...
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertProblems(reconciled.getProblems(), new String[] { "Pb(953) Null type mismatch (type annotations): required '@NonNull String' but this expression has type '@Nullable String'", "Pb(980) Unsafe interpretation of method return type as '@NonNull' based on the receiver type 'Lib1<@NonNull String>'. Type 'Lib1<T>' doesn't seem to be designed with null type annotations in mind" }, new int[] { 8, 11 });
    ...
    public void testFreeTypeVariableReturnSeverities() throws Exception {
        ...
        IPackageFragment fragment = this.project.getPackageFragmentRoots()[0].createPackageFragment("tests", true, null);
        fragment.createCompilationUnit("Lib3.java", "package tests;\n" + "public interface Lib3<T> {\n" + "	T get();\n" + "}\n", true, new NullProgressMonitor());
        ...
        "		return lib.get();\n" + "	}\n" + "}\n", true, new NullProgressMonitor()).getWorkingCopy(new NullProgressMonitor());
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        assertProblems(reconciled.getProblems(), new String[] { "Pb(980) Unsafe interpretation of method return type as '@NonNull' based on the receiver type 'Lib1<@NonNull String>'. Type 'Lib1<T>' doesn't seem to be designed with null type annotations in mind", "Pb(980) Unsafe interpretation of method return type as '@NonNull' based on the receiver type 'Lib2<@NonNull String>'. Type 'Lib2<T>' doesn't seem to be designed with null type annotations in mind" }, new int[] { 10, 13 }, new int[] { ProblemSeverities.Warning, ProblemSeverities.Info });
    }
    ...
    public void testBug490343() throws Exception {
        ...
        IPackageFragment fragment = this.project.getPackageFragmentRoots()[0].createPackageFragment("tests", true, null);
        ICompilationUnit unit = fragment.createCompilationUnit("Test.java", "package tests;\n" + "\n" + "import libs.Map;\n" + "\n" + "public class Test {\n" + "    static boolean f() {\n" + "        if(Map.Entry.comparingByKey() == null) {\n" + "            return false;\n" + "        }\n" + "        return true;\n" + "    }\n" + "}\n" + "", true, new NullProgressMonitor()).getWorkingCopy(new NullProgressMonitor());
        CompilationUnit reconciled = unit.reconcile(AST.JLS8, true, null, new NullProgressMonitor());
        IProblem[] problems = reconciled.getProblems();
        assertProblems(problems, new String[] { "Pb(149) Dead code" }, new int[] { 7 });
    }
}
