    ...
    public MethodDeclaration addMissingAbstractMethodFor(MethodBinding methodBinding) {
        ...
        if (argumentsLength > 0) {
            ...
            for (int i = argumentsLength; --i >= 0; ) {
                arguments[i] = new Argument((baseName + i).toCharArray(), /*type ref*/
                0L, null, ClassFileConstants.AccDefault);
            }
        }
        ...
        //============BINDING UPDATE==========================
        methodDeclaration.binding = new MethodBinding(//methodDeclaration
        methodDeclaration.modifiers | ClassFileConstants.AccSynthetic, methodBinding.selector, methodBinding.returnType, //arguments bindings
        argumentsLength == 0 ? Binding.NO_PARAMETERS : argumentTypes, //exceptions
        methodBinding.thrownExceptions, //declaringClass
        this.binding);
 ...
 */
            ...
            for (int i = this.methods.length; --i >= 0; ) {
                ...
                if ((am = this.methods[i]).isConstructor()) {
                    if (!CharOperation.equals(am.selector, this.name)) {
                        ...
                        if (//changed to a method
                        c.constructorCall == null || c.constructorCall.isImplicitSuper()) {
    ...
    public MethodBinding createDefaultConstructorWithBinding(MethodBinding inheritedConstructorBinding, boolean eraseThrownExceptions) {
        ...
        constructor.binding = new MethodBinding(//methodDeclaration
        constructor.modifiers, //arguments bindings
        argumentsLength == 0 ? Binding.NO_PARAMETERS : argumentTypes, //exceptions
        thrownExceptions, //declaringClass
        sourceType);
        ...
        if (// this implies that annotation based null analysis is enabled
        inheritedConstructorBinding.parameterNonNullness != null && argumentsLength > 0) {
 ...
 */
    public void generateCode(ClassFile enclosingClassFile) {
        ...
        try {
            ...
            if (// trigger problem type generation for code gen errors
    ...
    public void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
        ...
        if (nestedType.isAnonymousType()) {
            ...
            if (superclassBinding.enclosingType() != null && !superclassBinding.isStatic()) {
                if (!superclassBinding.isLocalType() || ((NestedTypeBinding) superclassBinding).getSyntheticField(superclassBinding.enclosingType(), true) != null || superclassBinding.isMemberType()) {
 ...
 */
    ...
    public void resolve() {
        ...
        try {
            ...
            if ((annotationTagBits & TagBits.AnnotationDeprecated) == 0 && (sourceType.modifiers & ClassFileConstants.AccDeprecated) != 0 && this.scope.compilerOptions().sourceLevel >= ClassFileConstants.JDK1_5) {
            ...
            boolean needSerialVersion = this.scope.compilerOptions().getSeverity(CompilerOptions.MissingSerialVersion) != ProblemSeverities.Ignore && sourceType.isClass() && sourceType.findSuperTypeOriginatingFrom(TypeIds.T_JavaIoExternalizable, /*Externalizable is not a class*/
            false) == null && sourceType.findSuperTypeOriginatingFrom(TypeIds.T_JavaIoSerializable, /*Serializable is not a class*/
            false) != null;
            if (needSerialVersion) {
                ...
                needSerialVersion = methodBinding == null || !methodBinding.isValidBinding() || methodBinding.returnType.id != TypeIds.T_JavaLangObject || (throwsExceptions = methodBinding.thrownExceptions).length != 1 || throwsExceptions[0].id != TypeIds.T_JavaIoObjectStreamException;
                if (needSerialVersion) {
                    ...
                    if (argumentTypeBinding.isValidBinding()) {
                        ...
                        hasWriteObjectMethod = methodBinding != null && methodBinding.isValidBinding() && methodBinding.modifiers == ClassFileConstants.AccPrivate && methodBinding.returnType == TypeBinding.VOID && (throwsExceptions = methodBinding.thrownExceptions).length == 1 && throwsExceptions[0].id == TypeIds.T_JavaIoException;
                    }
                    ...
                    if (argumentTypeBinding.isValidBinding()) {
                        ...
                        hasReadObjectMethod = methodBinding != null && methodBinding.isValidBinding() && methodBinding.modifiers == ClassFileConstants.AccPrivate && methodBinding.returnType == TypeBinding.VOID && (throwsExceptions = methodBinding.thrownExceptions).length == 1 && throwsExceptions[0].id == TypeIds.T_JavaIoException;
                    }
                ...
                for (int i = 0, count = this.fields.length; i < count; i++) {
                    ...
                    switch(field.getKind()) {
                        ...
                        case AbstractVariableDeclaration.FIELD:
                            ...
                            if (needSerialVersion && ((fieldBinding.modifiers & (ClassFileConstants.AccStatic | ClassFileConstants.AccFinal)) == (ClassFileConstants.AccStatic | ClassFileConstants.AccFinal)) && CharOperation.equals(TypeConstants.SERIALVERSIONUID, fieldBinding.name) && TypeBinding.equalsEquals(TypeBinding.LONG, fieldBinding.type)) {
                    ...
                    if (this.binding.isAbstract()) {
                        if (!hasEnumConstants) {
                            ...
                            for (int i = 0, count = this.methods.length; i < count; i++) {
                                final AbstractMethodDeclaration methodDeclaration = this.methods[i];
                                if (methodDeclaration.isAbstract() && methodDeclaration.binding != null) {
                                    for (int f = 0, l = enumConstantsWithoutBody.length; f < l; f++) if (enumConstantsWithoutBody[f] != null)
            ...
            if (existing instanceof ReferenceBinding && existing != this.binding && existing.isValidBinding()) {
                ...
                if (existingType instanceof TypeVariableBinding) {
                    ...
                    checkOuterScope: while (outerScope != null) {
                        ...
                        } else if (existing2 instanceof ReferenceBinding && existing2.isValidBinding() && outerScope.isDefinedInType((ReferenceBinding) existing2)) {
                            blockScope.problemReporter().typeCollidesWithEnclosingType(this);
                            break checkOuterScope;
                    ...
                    blockScope.problemReporter().typeCollidesWithEnclosingType(this);
                } else if (// only consider hiding inside same unit
                blockScope.isDefinedInSameUnit(existingType)) {
                    // hiding sibling
        ...
        try {
            if (visitor.visit(this, unitScope)) {
                ...
                if (this.annotations != null) {
                    ...
                    for (int i = 0; i < annotationsLength; i++) this.annotations[i].traverse(visitor, this.staticInitializerScope);
                }
                if (this.superclass != null)
                    this.superclass.traverse(visitor, this.scope);
                if (this.superInterfaces != null) {
                    int length = this.superInterfaces.length;
                    for (int i = 0; i < length; i++) this.superInterfaces[i].traverse(visitor, this.scope);
                }
                if (this.typeParameters != null) {
                ...
                if (this.memberTypes != null) {
                    int length = this.memberTypes.length;
                    for (int i = 0; i < length; i++) this.memberTypes[i].traverse(visitor, this.scope);
                }
                if (this.fields != null) {
                    int length = this.fields.length;
                    for (int i = 0; i < length; i++) {
                ...
                if (this.methods != null) {
                    int length = this.methods.length;
                    for (int i = 0; i < length; i++) this.methods[i].traverse(visitor, this.scope);
                }
            }
 ...
 */
    void updateMaxFieldCount() {
        ...
        if (this.maxFieldCount > outerMostType.maxFieldCount) {
            outerMostType.maxFieldCount = // up
            this.maxFieldCount;
        } else {
            // down
