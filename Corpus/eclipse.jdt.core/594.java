...
public class BinaryTypeBinding extends ReferenceBinding {
    ...
    public enum ExternalAnnotationStatus implements  {
    ...
    static Object convertMemberValue(Object binaryValue, LookupEnvironment env, char[][][] missingTypeNames, boolean resolveEnumConstants) {
        ...
        if (binaryValue instanceof EnumConstantSignature) {
            ...
            enumType = (ReferenceBinding) resolveType(enumType, env, /* no raw conversion */
            false);
        ...
        if (binaryValue instanceof Object[]) {
            ...
            for (int i = 0; i < length; i++) values[i] = convertMemberValue(objects[i], env, missingTypeNames, resolveEnumConstants);
            return values;
        }
	...
	*/
    ...
    static AnnotationBinding createAnnotation(IBinaryAnnotation annotationInfo, LookupEnvironment env, char[][][] missingTypeNames) {
        ...
        for (int i = 0; i < length; i++) pairs[i] = new ElementValuePair(binaryPairs[i].getName(), convertMemberValue(binaryPairs[i].getValue(), env, missingTypeNames, false), null);
 ...
 */
	 ...
	 */
        ...
        this.typeVariables = typeSignature != null && typeSignature.length > 0 && typeSignature[0] == Util.C_GENERIC_START ? // is initialized in cachePartsFrom (called from LookupEnvironment.createBinaryTypeFrom())... must set to null so isGenericType() answers true
        null : Binding.NO_TYPE_VARIABLES;
        ...
        if (enclosingTypeName != null) {
            ...
            this.enclosingType = environment.getTypeFromConstantPoolName(enclosingTypeName, 0, -1, true, /* could not be missing */
            null);
 ...
 */
    ...
    void cachePartsFrom(IBinaryType binaryType, boolean needFieldsAndMethods) {
            ...
            if (memberTypeStructures != null) {
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        this.memberTypes[i] = this.environment.getTypeFromConstantPoolName(memberTypeStructures[i].getName(), /* could not be missing */
                        0, -1, false, null);
                    }
		 ...
		 */
            ...
            // use generic signature even in 1.4
            char[] typeSignature = binaryType.getGenericSignature();
            ...
            if (typeSignature != null) {
                ...
                if (wrapper.signature[wrapper.start] == Util.C_GENERIC_START) {
                    ...
                    // skip '<'
                    wrapper.start++;
                    this.typeVariables = createTypeVariables(wrapper, true, missingTypeNames, toplevelWalker/*class*/
                    , true);
                    // skip '>'
                    wrapper.start++;
            ...
            if (typeSignature == null) {
                ...
                if (interfaceNames != null) {
                    ...
                    if (size > 0) {
                        ...
                        for (short i = 0; i < size; i++) // attempt to find each superinterface if it exists in the cache (otherwise - resolve it when requested)
        ...
        if (iFields != null) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (// below 1.8, external annotations will be attached later
	 ...
	 */
        ...
        // always use generic signature, even in 1.4
        ...
        methodSignature == null) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < numOfParams; i++) {
                    ...
                    if (// skip the synthetic arg if necessary
                    i >= startIndex) {
            ...
            if (exceptionTypes != null) {
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) exceptions[i] = this.environment.getTypeFromConstantPoolName(exceptionTypes[i], 0, -1, false, missingTypeNames, walker.toThrows(i));
                }
            }
            if (!method.isConstructor())
            ...
            if (startIndex > 0 && argumentNamesLength > 0) {
                ...
                if (startIndex >= argumentNamesLength) {
                    // We know nothing about the argument names
            ...
            if (wrapper.signature[wrapper.start] == Util.C_GENERIC_START) {
                ...
                // skip '<'
                wrapper.start++;
                typeVars = createTypeVariables(wrapper, false, missingTypeNames, walker, /*class*/
                false);
                // skip '>'
                wrapper.start++;
            }
            if (wrapper.signature[wrapper.start] == Util.C_PARAM_START) {
                ...
                if (wrapper.signature[wrapper.start] == Util.C_PARAM_END) {
                    ...
                    java.util.ArrayList types = new java.util.ArrayList(2);
                    short rank = 0;
                    while (wrapper.signature[wrapper.start] != Util.C_PARAM_END) types.add(this.environment.getTypeFromTypeSignature(wrapper, typeVars, this, missingTypeNames, walker.toMethodParameter(rank++)));
                    // skip ')'
                    wrapper.start++;
                    ...
                    if (this.environment.globalOptions.storeAnnotations) {
                        ...
                        for (int i = 0; i < numParam; i++) paramAnnotations[i] = createAnnotations(method.getParameterAnnotations(i, this.fileName), this.environment, missingTypeNames);
                    }
                }
            }
        ...
        if (this.environment.globalOptions.storeAnnotations) {
            ...
            if (annotations == null || annotations.length == 0)
                if (method.isConstructor())
                    // FIXME: When both exist, order could become an issue.
                    annotations = walker.toMethodReturn().getAnnotationsAtCursor(this.id);
            result.setAnnotations(createAnnotations(annotations, this.environment, missingTypeNames), paramAnnotations, isAnnotationType() ? convertMemberValue(method.getDefaultValue(), this.environment, missingTypeNames, true) : null, this.environment);
        }
        ...
        for (int i = 0, length = typeVars.length; i < length; i++) this.environment.typeSystem.fixTypeVariableDeclaringElement(typeVars[i], result);
        return result;
    }
 ...
 */
    private IBinaryMethod[] createMethods(IBinaryMethod[] iMethods, IBinaryType binaryType, long sourceLevel, char[][][] missingTypeNames) {
        ...
        if (iMethods != null) {
            ...
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=330347
                ...
                switch(typeSignature[i]) {
                    ...
                    default:
                        if (pendingVariable) {
                            ...
                            AnnotationBinding[] annotations = BinaryTypeBinding.createAnnotations(walker.toTypeParameter(isClassTypeParameter, rank++).getAnnotationsAtCursor(0), this.environment, missingTypeNames);
...
*/
    public ReferenceBinding enclosingType() {
        // should not delegate to prototype.
        ...
        if (numOfParams > 0) {
            ...
            for (int i = 0; i < numOfParams; i++) {
                ...
                if (param instanceof UnresolvedReferenceBinding) {
                    param = resolveType(param, this.environment, /* raw conversion */
                    true);
                }
                parameters[i] = param;
                index = end + 1;
 ...
 */
    ...
    public MethodBinding getExactMethod(char[] selector, TypeBinding[] argumentTypes, CompilationUnitScope refScope) {
        ...
        if (foundNothing) {
            if (isInterface()) {
                if (// ensure superinterfaces are resolved before checking
                superInterfaces().length == 1) {
                    if (refScope != null)
                        refScope.recordTypeReference(this.superInterfaces[0]);
                    return this.superInterfaces[0].getExactMethod(selector, argumentTypes, refScope);
                }
            } else if (// ensure superclass is resolved before checking
            superclass() != null) {
                if (refScope != null)
    ...
    public ReferenceBinding getMemberType(char[] typeName) {
        ...
        for (int i = this.memberTypes.length; --i >= 0; ) {
            ...
            if (memberType instanceof UnresolvedReferenceBinding) {
                ...
                if (// enclosing $ typeName
                name.length == (prefixLength + typeName.length))
                    if (// only check trailing portion
                    CharOperation.fragmentEquals(typeName, name, prefixLength, true))
                        return this.memberTypes[i] = (ReferenceBinding) resolveType(memberType, this.environment, /* no raw conversion for now */
                        false);
    ...
    public MethodBinding[] getMethods(char[] selector, int suggestedParameterLength) {
        ...
        if ((range = ReferenceBinding.binarySearch(selector, this.methods)) >= 0) {
            ...
            if (count == 0) {
                MethodBinding[] result = new MethodBinding[length];
                // iterate methods to resolve them
                for (int i = start, index = 0; i <= end; i++) result[index++] = resolveTypesFor(this.methods[i]);
                return result;
            } else {
 ...
 */
    ...
    public ReferenceBinding[] memberTypes() {
        if (!isPrototype()) {
            ...
            if (memberTypesLength > 0) {
                ...
                for (int i = 0; i < memberTypesLength; i++) this.memberTypes[i] = this.environment.createMemberType(members[i], this);
            }
            this.tagBits &= ~TagBits.HasUnresolvedMemberTypes;
            return this.memberTypes;
        }
        if ((this.tagBits & TagBits.HasUnresolvedMemberTypes) == 0)
            return this.memberTypes;
        for (int i = this.memberTypes.length; --i >= 0; ) this.memberTypes[i] = (ReferenceBinding) resolveType(this.memberTypes[i], this.environment, /* no raw conversion for now */
        false);
        this.tagBits &= ~TagBits.HasUnresolvedMemberTypes;
        return this.memberTypes;
    }
    ...
    MethodBinding resolveTypesFor(MethodBinding method) {
        ...
        if ((method.modifiers & ExtraCompilerModifiers.AccUnresolved) == 0)
            return method;
        if (!method.isConstructor()) {
            TypeBinding resolvedType = resolveType(method.returnType, this.environment, /* raw conversion */
            true);
        ...
        for (int i = method.parameters.length; --i >= 0; ) {
            TypeBinding resolvedType = resolveType(method.parameters[i], this.environment, /* raw conversion */
            true);
        ...
        for (int i = method.thrownExceptions.length; --i >= 0; ) {
            ReferenceBinding resolvedType = (ReferenceBinding) resolveType(method.thrownExceptions[i], this.environment, /* raw conversion */
            true);
    ...
    private void scanFieldForNullAnnotation(IBinaryField field, FieldBinding fieldBinding, boolean isEnum, ITypeAnnotationWalker externalAnnotationWalker) {
        ...
        if (isEnum && (field.getModifiers() & ClassFileConstants.AccEnum) != 0) {
            ...
            // we know it's nonnull, no need to look for null *annotations* on enum constants.
            return;
        }
        if (// avoid dangerous re-entry via usesNullTypeAnnotations()
        !CharOperation.equals(this.fPackage.compoundName, TypeConstants.JAVA_LANG_ANNOTATION) && this.environment.usesNullTypeAnnotations()) {
            TypeBinding fieldType = fieldBinding.type;
            if (fieldType != null && !fieldType.isBaseType() && (fieldType.tagBits & TagBits.AnnotationNullMASK) == 0 && fieldType.acceptsNonNullDefault() && hasNonNullDefaultFor(DefaultLocationField, true)) {
            ...
            // not using fieldBinding.tagBits when we have type annotations.
            return;
        }
        if (fieldBinding.type == null || fieldBinding.type.isBaseType())
            // null annotations are only applied to reference types
            return;
        boolean explicitNullness = false;
        IBinaryAnnotation[] annotations = externalAnnotationWalker != ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER ? externalAnnotationWalker.getAnnotationsAtCursor(fieldBinding.type.id) : field.getAnnotations();
        if (annotations != null) {
    ...
    private void scanMethodForNullAnnotation(IBinaryMethod method, MethodBinding methodBinding, ITypeAnnotationWalker externalAnnotationWalker, boolean useNullTypeAnnotations) {
        ...
        if (isEnum()) {
            int purpose = 0;
            if (CharOperation.equals(TypeConstants.VALUEOF, method.getSelector()) && methodBinding.parameters.length == 1 && methodBinding.parameters[0].id == TypeIds.T_JavaLangString) {
        ...
        IBinaryAnnotation[] annotations = returnWalker != ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER ? returnWalker.getAnnotationsAtCursor(methodBinding.returnType.id) : method.getAnnotations();
        if (annotations != null) {
            for (int i = 0; i < annotations.length; i++) {
                char[] annotationTypeName = annotations[i].getTypeName();
                if (annotationTypeName[0] != Util.C_RESOLVED)
        ...
        int numParamAnnotations = externalAnnotationWalker instanceof IMethodAnnotationWalker ? ((IMethodAnnotationWalker) externalAnnotationWalker).getParameterCount() : method.getAnnotatedParametersCount();
        if (numParamAnnotations > 0) {
            for (int j = 0; j < numVisibleParams; j++) {
                if (numParamAnnotations > 0) {
                    ...
                    IBinaryAnnotation[] paramAnnotations = parameterWalker != ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER ? parameterWalker.getAnnotationsAtCursor(parameters[j].id) : method.getParameterAnnotations(j + startIndex, this.fileName);
                    if (paramAnnotations != null) {
    ...
    private void scanTypeForNullDefaultAnnotation(IBinaryType binaryType, PackageBinding packageBinding) {
        ...
        if (CharOperation.equals(CharOperation.splitOn('/', binaryType.getName()), nonNullByDefaultAnnotationName))
            // don't recursively apply @NNBD on @NNBD, neither directly nor via the 'enclosing' package-info.java
            return;
    ...
    int getNonNullByDefaultValue(IBinaryAnnotation annotation) {
        ...
        if (elementValuePairs == null || elementValuePairs.length == 0) {
            ...
            // custom unconfigurable NNBD
            return NONNULL_BY_DEFAULT;
        } else if (elementValuePairs.length > 0) {
            // evaluate the contained EnumConstantSignatures:
            int nullness = 0;
            for (int i = 0; i < elementValuePairs.length; i++) nullness |= Annotation.nullLocationBitsFromAnnotationValue(elementValuePairs[i].getValue());
            return nullness;
        } else {
    ...
    private char[][] signature2qualifiedTypeName(char[] typeSignature) {
        // cut off leading 'L' and trailing ';'
...
*/
    ...
    public ReferenceBinding[] superInterfaces() {
        ...
        for (int i = this.superInterfaces.length; --i >= 0; ) {
            this.superInterfaces[i] = (ReferenceBinding) resolveType(this.superInterfaces[i], this.environment, /* raw conversion */
            true);
    ...
    public String toString() {
        ...
        if (this.fields != null) {
            if (this.fields != Binding.NO_FIELDS) {
                //$NON-NLS-1$
                buffer.append("\n/*   fields   */");
                for (int i = 0, length = this.fields.length; i < length; i++) //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (this.methods != null) {
            if (this.methods != Binding.NO_METHODS) {
                //$NON-NLS-1$
                buffer.append("\n/*   methods   */");
                for (int i = 0, length = this.methods.length; i < length; i++) //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (this.memberTypes != null) {
            if (this.memberTypes != Binding.NO_MEMBER_TYPES) {
                //$NON-NLS-1$
                buffer.append("\n/*   members   */");
                for (int i = 0, length = this.memberTypes.length; i < length; i++) //$NON-NLS-1$ //$NON-NLS-2$
