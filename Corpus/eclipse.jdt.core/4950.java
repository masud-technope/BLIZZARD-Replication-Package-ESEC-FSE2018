...
public class MessageSend extends Expression implements IPolyExpression, Invocation {
    ...
    private SimpleLookupTable /*<PGMB,InferenceContext18>*/
    inferenceContexts;

    private HashMap<TypeBinding, MethodBinding> solutionsPerTargetType;

    // resolving within the context of an outer (lambda) inference?
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
            ...
            if (nonStatic) {
                ...
                for (int i = 0; i < TypeConstants.closeMethods.length; i++) {
                    ...
                    if (CharOperation.equals(record.selector, this.selector) && CharOperation.equals(record.typeName, this.binding.declaringClass.compoundName)) {
                        ...
                        for (int j = 0; j < len; j++) recordCallingClose(currentScope, flowContext, flowInfo, this.arguments[j]);
                        break;
                    }
                }
            }
        ...
        // no longer trust this info after any message send
    ...
    private void recordCallingClose(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, Expression closeTarget) {
        ...
        if (// null happens if target is not a local variable or not an AutoCloseable
    ...
    private FlowInfo analyseNullAssertion(BlockScope currentScope, Expression argument, FlowContext flowContext, FlowInfo flowInfo, boolean expectingNull) {
        ...
        if (// beyond this point the argument can only be null/nonnull
        local != null) {
            if (expectingNull)
            ...
            if (!expectingNull && argument instanceof Reference && currentScope.compilerOptions().enableSyntacticNullAnalysisForFields) {
    ...
    public boolean checkNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, int ttlForFieldCheck) {
        ...
        // note that flowInfo is not used inside nullStatus(..)
 ...
 */
    public void computeConversion(Scope scope, TypeBinding runtimeTimeType, TypeBinding compileTimeType) {
        ...
        if (this.binding != null && this.binding.isValidBinding()) {
            ...
            if (ArrayBinding.isArrayClone(this.actualReceiverType, this.binding) && runtimeTimeType.id != TypeIds.T_JavaLangObject && scope.compilerOptions().sourceLevel >= ClassFileConstants.JDK1_5) {
 ...
 */
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        } else if (// outer access ?
        (this.bits & ASTNode.DepthMASK) != 0 && this.receiver.isImplicitThis()) {
        ...
        if (this.syntheticAccessor == null) {
            ...
            } else if (// interface or annotation type
            constantPoolDeclaringClass.isInterface()) {
            ...
            codeStream.invoke(Opcodes.OPC_invokestatic, this.syntheticAccessor, /* default declaringClass */
            null, this.typeArguments);
        }
 ...
 */
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
        ...
        if (this.binding.isPrivate()) {
            ...
            if (TypeBinding.notEquals(currentScope.enclosingSourceType(), codegenBinding.declaringClass)) {
                this.syntheticAccessor = ((SourceTypeBinding) codegenBinding.declaringClass).addSyntheticMethod(codegenBinding, /* not super access there */
                false);
            ...
            if (this.actualReceiverType.isInterface())
                // invoking an overridden default method, which is accessible/public by definition
                return;
            ...
            if (((this.bits & ASTNode.DepthMASK) != 0) && codegenBinding.declaringClass.getPackage() != (enclosingSourceType = currentScope.enclosingSourceType()).getPackage()) {
    ...
    public int nullStatus(FlowInfo flowInfo, FlowContext flowContext) {
        ...
        if (this.binding.isValidBinding()) {
            ...
            if (// alternatively look for type annotation (will only be present in 1.8+):
            (tagBits & TagBits.AnnotationNullMASK) == 0L)
 ...
 */
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.constant != Constant.NotAConstant) {
            ...
            if (this.typeArguments != null) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if ((this.genericTypeArguments[i] = typeReference.resolveType(/* check bounds*/
                    scope, true)) == null) {
                ...
                if (this.argumentsHaveErrors) {
                    if (// still attempt to resolve arguments
                    this.arguments != null) {
            ...
            if (this.arguments != null) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (this.arguments[i].resolvedType != null)
                        //$NON-NLS-1$
                        scope.problemReporter().genericInferenceError(//$NON-NLS-1$
                        "Argument was unexpectedly found resolved", //$NON-NLS-1$
                        this);
                    ...
                    if (this.actualReceiverType instanceof ReferenceBinding) {
                        ...
                        if (this.binding != null && !this.binding.isValidBinding()) {
                            ...
                            if (closestMatch != null) {
                                if (closestMatch.original().typeVariables != // generic method
        ...
        if (!this.binding.isStatic()) {
            ...
            if (this.receiverIsType) {
                ...
                if (// record need for explicit cast at codegen since receiver could not handle it
                TypeBinding.notEquals(this.actualReceiverType, oldReceiverType) && TypeBinding.notEquals(this.receiver.postConversionType(scope), this.actualReceiverType)) {
    ...
    protected TypeBinding findMethodBinding(BlockScope scope) {
        ...
        if (// first look up or a "cache miss" somehow.
        this.binding == null) {
 ...
 */
    ...
    public boolean isBoxingCompatibleWith(TypeBinding targetType, Scope scope) {
        if (this.argumentsHaveErrors || this.binding == null || !this.binding.isValidBinding() || targetType == null || scope == null)
            return false;
        if (// i.e it is dumb to trigger inference, checking boxing compatibility against say Collector<? super T, A, R>.
        isPolyExpression() && !targetType.isPrimitiveOrBoxedPrimitiveType())
            return false;
    ...
    public void traverse(ASTVisitor visitor, BlockScope blockScope) {
        if (visitor.visit(this, blockScope)) {
            ...
            if (this.arguments != null) {
                ...
                for (int i = 0; i < argumentsLength; i++) this.arguments[i].traverse(visitor, blockScope);
            }
        }
    ...
    public void cleanUpInferenceContexts() {
        if (this.inferenceContexts == null)
            return;
        for (Object value : this.inferenceContexts.valueTable) if (value != null)
