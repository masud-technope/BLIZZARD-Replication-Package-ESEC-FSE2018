...
public class Assignment extends Expression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        return ((Reference) lhs).analyseAssignment(currentScope, flowContext, flowInfo, this, false).unconditionalInits();
    }
    ...
    void checkAssignmentEffect(BlockScope scope) {
        ...
        if (left != null && left == getDirectBinding(this.expression)) {
            ...
            // record assignment has no effect
            this.bits |= IsAssignmentWithNoEffectMASK;
        }
    }
    ...
    void checkAssignment(BlockScope scope, TypeBinding lhsType, TypeBinding rhsType) {
        ...
        } else if (leftField != null && leftField.declaringClass != null && /*length pseudo field*/
        leftField.declaringClass.isRawType() && (rhsType.isParameterizedType() || rhsType.isGenericType())) {
    ...
    public StringBuffer printExpressionNoParenthesis(int indent, StringBuffer output) {
        //$NON-NLS-1$
        lhs.printExpression(indent, output).append(" = ");
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        expression.setExpectedType(lhsType);
        ...
        if ((expression.isConstantValueOfTypeAssignableToType(rhsType, lhsType) || (lhsType.isBaseType() && BaseTypeBinding.isWidening(lhsType.id, rhsType.id))) || rhsType.isCompatibleWith(lhsType)) {
    ...
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
        ...
        if (expectedType == BooleanBinding && lhsType == BooleanBinding && (this.lhs.bits & IsStrictlyAssignedMASK) != 0) {
