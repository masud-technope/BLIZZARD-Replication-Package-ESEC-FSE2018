...
public class TypeDeclaration extends Statement implements ProblemSeverities, ReferenceContext {
    ...
    public MethodDeclaration addMissingAbstractMethodFor(MethodBinding methodBinding) {
        ...
        methodDeclaration.modifiers, methodBinding.selector, methodBinding.returnType, //arguments bindings
        argumentsLength == 0 ? NoParameters : argumentTypes, //exceptions
    ...
    public ConstructorDeclaration createsInternalConstructor(boolean needExplicitConstructorCall, boolean needToInsert) {
        ...
        if (modifiers != AccDefault) {
            constructor.modifiers = (((this.bits & ASTNode.IsMemberTypeMASK) != 0) && (modifiers & AccPrivate) != 0) ? AccDefault : modifiers & AccVisibilityMASK;
        }
    ...
    public MethodBinding createsInternalConstructorWithBinding(MethodBinding inheritedConstructorBinding) {
        ...
        //============BINDING UPDATE==========================
        cd.binding = new MethodBinding(//methodDeclaration
        cd.modifiers, //arguments bindings
        argumentsLength == 0 ? NoParameters : argumentTypes, //exceptions
        inheritedConstructorBinding.thrownExceptions, //declaringClass
        binding);
        ...
        if (binding.methods == null) {
            ...
            System.arraycopy(binding.methods, 0, newMethods = new MethodBinding[binding.methods.length + 1], 1, binding.methods.length);
    ...
    public void generateCode(BlockScope blockScope, CodeStream codeStream) {
        ...
        if (binding != null)
            ((NestedTypeBinding) binding).computeSyntheticArgumentSlotSizes();
        ...
        if (fields != null) {
            for (int i = 0, count = fields.length; i < count; i++) {
                ...
                if (field.isStatic()) {
                    ...
                    staticInitializerContext.handledExceptions = AnyException;
                    /*}*/
                    ...
                    // tolerate them all, and record them
                    initializerContext.handledExceptions = AnyException;
                    /*}*/
        ...
        if (memberTypes != null) {
            for (int i = 0, count = memberTypes.length; i < count; i++) {
                ...
                flowContext != null) {
                    ...
                    memberTypes[i].analyseCode(scope, flowContext, nonStaticFieldInfo.copy().setReachMode(flowInfo.reachMode()));
                } else {
        ...
        if (methods != null) {
            ...
            for (int i = 0, count = methods.length; i < count; i++) {
                ...
                if (method.isInitializationMethod()) {
                    if (// <clinit>
                    method.isStatic()) {
                        method.analyseCode(scope, staticInitializerContext, // reset reach mode in case initializers did abrupt completely
                        staticFieldInfo.unconditionalInits().discardNonFieldInitializations().addInitializationsFrom(outerInfo).setReachMode(flowInfo.reachMode()));
                    } else // constructor
                    {
                        // reset reach mode in case initializers did abrupt completely
                        method.analyseCode(scope, initializerContext, constructorInfo.copy().setReachMode(flowInfo.reachMode()));
                    }
                } else // regular method
                {
    ...
    public void manageEnclosingInstanceAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
        ...
        if (binding.isAnonymousType()) {
            ...
            if (superclassBinding.enclosingType() != null && !superclassBinding.isStatic()) {
                if (!superclassBinding.isLocalType() || ((NestedTypeBinding) superclassBinding).getSyntheticField(superclassBinding.enclosingType(), true) != null) {
    ...
    public void parseMethod(Parser parser, CompilationUnitDeclaration unit) {
        ...
        if (memberTypes != null) {
            ...
            for (int i = 0; i < length; i++) memberTypes[i].parseMethod(parser, unit);
        }
        //methods
        if (methods != null) {
            int length = methods.length;
            for (int i = 0; i < length; i++) methods[i].parseStatements(parser, unit);
        }
        //initializers
        if (fields != null) {
    ...
    public StringBuffer printHeader(int indent, StringBuffer output) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        output.append(isInterface() ? "interface " : "class ");
        output.append(name);
        ...
        if (superInterfaces != null && superInterfaces.length > 0) {
            //$NON-NLS-2$ //$NON-NLS-1$
            output.append(isInterface() ? " extends " : " implements ");
    ...
    public void resolve() {
        ...
        try {
            ...
            boolean needSerialVersion = this.scope.environment().options.getSeverity(CompilerOptions.MissingSerialVersion) != ProblemSeverities.Ignore && sourceType.isClass() && !sourceType.isAbstract() && sourceType.findSuperTypeErasingTo(T_JavaIoSerializable, /*Serializable is not a class*/
            false) != null;
            ...
            if (this.fields != null) {
                for (int i = 0, count = this.fields.length; i < count; i++) {
                    ...
                    if (field.isField()) {
                        ...
                        if (needSerialVersion && ((fieldBinding.modifiers & (AccStatic | AccFinal)) == (AccStatic | AccFinal)) && CharOperation.equals(TypeConstants.SERIALVERSIONUID, fieldBinding.name) && BaseTypes.LongBinding == fieldBinding.type) {
        ...
        try {
            if (visitor.visit(this, unitScope)) {
                ...
                if (this.superInterfaces != null) {
                    ...
                    for (int i = 0; i < length; i++) this.superInterfaces[i].traverse(visitor, scope);
                }
                ...
                if (this.memberTypes != null) {
                    ...
                    for (int i = 0; i < length; i++) this.memberTypes[i].traverse(visitor, scope);
                }
                if (this.enums != null) {
                    int length = this.enums.length;
                    for (int i = 0; i < length; i++) this.enums[i].traverse(visitor, scope);
                }
                if (this.fields != null) {
                ...
                if (this.methods != null) {
                    ...
                    for (int i = 0; i < length; i++) this.methods[i].traverse(visitor, scope);
                }
            }
