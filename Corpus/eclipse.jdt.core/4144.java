...
public class TokenManager implements Iterable<Token> {
    ...
    private String getEscapedTokenString(Token token) {
        if (token.getLineBreaksBefore() > 0 && charAt(token.originalStart) == '@') {
            ...
            if (matcher.find()) {
                return matcher.group(1) + "&#64;" + //$NON-NLS-1$
                text.substring(//$NON-NLS-1$
                matcher.end(2));
            }
        }
        return token.toString(this.source);
    ...
    public int findFirstTokenInLine(int startIndex, boolean includeWraps, boolean includeForced) {
        // going backwards, previous has higher index than current
        Token previous = get(startIndex);
        for (int i = startIndex - 1; i >= 0; i--) {
            Token token = get(i);
            if (token.getLineBreaksAfter() > 0 || previous.getLineBreaksBefore() > 0) {
                boolean include = previous.getWrapPolicy() != null && (previous.getWrapPolicy().wrapMode == WrapMode.FORCED ? includeForced : includeWraps);
                if (!include)
                    return i + 1;
            }
            previous = token;
    ...
    private boolean tokenInside(ASTNode node, int index) {
        return get(index).originalStart >= node.getStartPosition() && get(index).originalEnd <= node.getStartPosition() + node.getLength();
    }
