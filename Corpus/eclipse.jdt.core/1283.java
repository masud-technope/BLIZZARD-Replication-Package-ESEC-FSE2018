...
public abstract class TypeReference extends Expression {
	 ...
	 */
    public static enum AnnotationPosition implements  {
 ...
 */
    ...
    protected TypeBinding internalResolveType(Scope scope, int location) {
            ...
            if (this.resolvedType.isValidBinding()) {
                ...
                switch(this.resolvedType.problemId()) {
                    ...
                    case ProblemReasons.InheritedNameHidesEnclosingName:
                        ...
                        if (type == null)
                            return null;
                        return scope.environment().convertToRawType(/*do not force conversion of enclosing types*/
                        type, false);
                    default:
                        return null;
                }
        ...
        if (// QualifiedTypeReference#getTypeBinding called above will have already checked deprecation
        !(this instanceof QualifiedTypeReference) && isTypeUseDeprecated(type, scope)) {
            reportDeprecatedType(type, scope);
        }
        type = scope.environment().convertToRawType(type, false);
        if (type.leafComponentType().isRawType() && (this.bits & ASTNode.IgnoreRawTypeCheck) == 0 && scope.compilerOptions().getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore) {
            scope.problemReporter().rawTypeReference(this, type);
        }
        if (hasError) {
            // don't apply null defaults to buggy type
            ...
            // pick up value that may have been changed in resolveAnnotations(..)
            return this.resolvedType;
        }
    }
    ...
    public TypeBinding resolveTypeArgument(BlockScope blockScope, ReferenceBinding genericType, int rank) {
        ...
        Binding.DefaultLocationTypeArgument);
    }
    ...
    protected void resolveAnnotations(Scope scope, int location) {
                ...
                if (annotationsOnDimensions != null) {
                    ...
                    if (this.resolvedType instanceof ArrayBinding) {
                        ...
                        if (nullTagBitsPerDimension != null) {
                            for (// skip last annotations at [dimensions] (concerns the leaf type)
                            int i = 0; // skip last annotations at [dimensions] (concerns the leaf type)
                            i < dimensions; // skip last annotations at [dimensions] (concerns the leaf type)
                            i++) {
        ...
        if (scope.compilerOptions().isAnnotationBasedNullAnalysisEnabled && this.resolvedType != null && (this.resolvedType.tagBits & TagBits.AnnotationNullMASK) == 0 && !this.resolvedType.isTypeVariable() && !this.resolvedType.isWildcard() && location != 0 && scope.hasDefaultNullnessFor(location)) {
