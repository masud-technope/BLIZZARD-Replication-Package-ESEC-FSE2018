...
public class MethodDeclaration extends AbstractMethodDeclaration {
    ...
    public  MethodDeclaration(CompilationResult compilationResult) {
        ...
        // Start with this assumption, will course correct during resolve and analyseCode.
    ...
    public void analyseCode(ClassScope classScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        try {
            ...
            this.scope.checkUnclosedCloseables(flowInfo, /*don't report against a specific location*/
            null, null, null);
        } catch (AbortMethod e) {
    ...
    public boolean hasNullTypeAnnotation(AnnotationPosition position) {
        ...
        (this.returnType != null && this.returnType.hasNullTypeAnnotation(position));
    }
    ...
    public void resolveStatements() {
        ...
        checkOverride: {
            ...
            if (hasOverrideAnnotation && !hasUnresolvedArguments) {
                ...
                if (complianceLevel >= ClassFileConstants.JDK1_6 && ((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccImplementing)) == ExtraCompilerModifiers.AccImplementing))
                    break checkOverride;
                ...
                if (!this.binding.declaringClass.isInterface()) {
                    if ((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccOverriding)) == ExtraCompilerModifiers.AccOverriding) {
                        ...
                        if (complianceLevel >= ClassFileConstants.JDK1_6 && compilerOptions.reportMissingOverrideAnnotationForInterfaceMethodImplementation && this.binding.isImplementing()) {
                ...
                {
                    //Also check if the method has a signature that is override-equivalent to that of any public method declared in Object.
                    if (complianceLevel >= ClassFileConstants.JDK1_6 && compilerOptions.reportMissingOverrideAnnotationForInterfaceMethodImplementation && (((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccOverriding)) == ExtraCompilerModifiers.AccOverriding) || this.binding.isImplementing())) {
                        // actually overrides, but did not claim to do so
                        this.scope.problemReporter().missingOverrideAnnotationForInterfaceMethodImplementation(this);
                    }
                }
        ...
        switch(TypeDeclaration.kind(this.scope.referenceType().modifiers)) {
            ...
            case TypeDeclaration.INTERFACE_DECL:
                if (compilerOptions.sourceLevel >= ClassFileConstants.JDK1_8 && (this.modifiers & (ExtraCompilerModifiers.AccSemicolonBody | ClassFileConstants.AccAbstract)) == ExtraCompilerModifiers.AccSemicolonBody) {
        ...
        if (compilerOptions.getSeverity(CompilerOptions.OverridingMethodWithoutSuperInvocation) != ProblemSeverities.Ignore) {
            if (this.binding != null) {
                int bindingModifiers = this.binding.modifiers;
                if ((bindingModifiers & (ExtraCompilerModifiers.AccOverriding | ExtraCompilerModifiers.AccImplementing)) == ExtraCompilerModifiers.AccOverriding && (this.bits & ASTNode.OverridingMethodWithSupercall) == 0) {
