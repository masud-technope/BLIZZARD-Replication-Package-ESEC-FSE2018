...
*/
    public void addInnerEmulationDependent(BlockScope dependentScope, boolean wasEnclosingInstanceSupplied) {
        ...
        if (this.dependents == null) {
            ...
            for (int i = 0; i < index; i++) if (this.dependents[i].scope == dependentScope)
                // already stored
                return;
 ...
 */
    ...
    protected void checkRedundantNullnessDefaultRecurse(ASTNode location, Annotation[] annotations, long nullBits, boolean useNullTypeAnnotations) {
        ...
        if (this.enclosingMethod != null) {
            outerDefault = useNullTypeAnnotations ? this.enclosingMethod.defaultNullness : this.enclosingMethod.tagBits & (TagBits.AnnotationNonNullByDefault | TagBits.AnnotationNullUnspecifiedByDefault);
        }
        if (outerDefault != 0) {
 ...
 */
    ...
    public String toString() {
        if (this.hasTypeAnnotations())
            //$NON-NLS-1$
            return annotatedDebugName() + " (local)";
        if (isAnonymousType())
            //$NON-NLS-1$
