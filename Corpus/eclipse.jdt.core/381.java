...
public class CompilerInvocationTests extends AbstractRegressionTest {
    ...
    public void test001_irritant_warning_token() {
        ...
        // all gets undetected in the From/To loop
        matcher.put("all", "all");
        ...
        try {
            ...
            for (int i = 0, length = fields.length; i < length; i++) {
                ...
                if (field.getType() == Integer.TYPE) {
                    ...
                    if (maskedProblemId != 0 && maskedProblemId != IProblem.IgnoreCategoriesMask && ProblemReporter.getProblemCategory(ProblemSeverities.Error, problemId) == CategorizedProblem.CAT_UNSPECIFIED) {
    ...
    public void runTaskTagsOptionsTest(String[] testFiles, Map customOptions, String expectedTags) {
        ...
        this.runConformTest(testFiles, "", /* no extra class libraries */
        null, /* flush output directory */
        true, /* no VM args */
        null, options, reader, /* skip javac */
        true);
    ...
    public void _test008_task_tags_options() {
        ...
        customOptions.put(JavaCore.COMPILER_TASK_PRIORITIES, "NORMAL,NORMAL");
        runTaskTagsOptionsTest(new String[] { "X.java", "public class X {\n" + "  void foo(X x) {\n" + "    // FIXME TODO XXX message contents\n" + "  }\n" + "}\n" }, customOptions, "[FIXME,message contents,NORMAL]\n" + "[TODO,message contents,NORMAL]\n" + "[XXX,message contents,NORMAL]\n");
    }
    ...
    public void test009_missing_message_templates() {
        assertEquals("Unable to retrieve the error message for problem id: 16777215. Check compiler resources.", new DefaultProblemFactory().getLocalizedMessage(Integer.MAX_VALUE, new String[] {}));
    }

    // https://bugs.eclipse.org/bugs/show_bug.cgi?id=206423
    public void test010_missing_elaboration_templates() {
        assertEquals("Unable to retrieve the error message elaboration for elaboration id: 1073741823. Check compiler resources.", new DefaultProblemFactory().getLocalizedMessage(0, Integer.MAX_VALUE / 2, new String[] { "Zork" }));
    }
    ...
    public void test011_problem_categories() {
        try {
            ...
            // TODO should be removed via https://bugs.eclipse.org/404675
            expectedProblemAttributes.put("GenericInferenceError", new ProblemAttributes(CategorizedProblem.CAT_INTERNAL));
