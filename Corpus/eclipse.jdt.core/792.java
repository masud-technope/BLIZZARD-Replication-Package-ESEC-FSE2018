...
public class DebugEvaluationTest extends EvaluationTest {
    ...
    public void compileAndDeploy(String source, String className) {
        // needed to reinitialize the caches
        resetEnv();
        ...
        buffer.append("\"").append(fileName).append("\" -d \"").append(EvaluationSetup.EVAL_DIRECTORY + File.separator + LocalVMLauncher.REGULAR_CLASSPATH_DIRECTORY).append("\" -nowarn -g -classpath \"").append(Util.getJavaClassLibsAsString()).append(SOURCE_DIRECTORY).append("\"");
    ...
    public void compileAndDeploy15(String source, String className) {
        ...
        StringBuffer buffer = new StringBuffer();
        buffer.append("\"").append(fileName).append("\" -d \"").append(EvaluationSetup.EVAL_DIRECTORY + File.separator + LocalVMLauncher.REGULAR_CLASSPATH_DIRECTORY).append("\" -nowarn -1.5 -g -classpath \"").append(Util.getJavaClassLibsAsString()).append(SOURCE_DIRECTORY).append("\"");
        BatchCompiler.compile(buffer.toString(), new PrintWriter(System.out), new PrintWriter(System.err), /*progress*/
        null);
    }
 ...
 */
    public void test002() throws Exception {
        try {
            ...
            this.context.evaluate(snippet, // local var type names
            null, // local var names
            null, // local modifiers
            null, stackFrame.declaringTypeName(), stackFrame.isStatic(), stackFrame.isConstructorCall(), getEnv(), getCompilerOptions(), requestor, getProblemFactory());
            assertTrue("Should get one result but got " + requestor.resultIndex + 1, requestor.resultIndex == 0);
            EvaluationResult result = requestor.results[0];
            assertTrue("Code snippet should not have problems", !result.hasProblems());
            assertTrue("Result should have a value", result.hasValue());
 ...
 */
    public void test003() throws Exception {
        try {
            ...
            DebugRequestor requestor = new DebugRequestor();
            char[] snippet = "return this;".toCharArray();
            this.context.evaluate(snippet, stackFrame.localVariableTypeNames(), stackFrame.localVariableNames(), stackFrame.localVariableModifiers(), // declaring type -- NO DELEGATE THIS
            null, stackFrame.isStatic(), stackFrame.isConstructorCall(), getEnv(), getCompilerOptions(), requestor, getProblemFactory());
            assertTrue("Should get one result but got " + requestor.resultIndex + 1, requestor.resultIndex == 0);
            EvaluationResult result = requestor.results[0];
            // 'this' cannot be referenced since there is no declaring type
            assertTrue("Code snippet should have a problem", result.hasProblems());
 ...
 */
    public void test032() throws Exception {
        try {
            ...
            assertEquals("Unexpected errors", "The field B32.j is not visible|", buffer == null ? "none" : buffer.toString());
        } finally {
            removeTempClass("B32");
            removeTempClass("A32");
        }
 ...
 */
    public void test033() throws Exception {
        try {
            ...
            this.context.evaluate(snippet, stackFrame.localVariableTypeNames(), stackFrame.localVariableNames(), stackFrame.localVariableModifiers(), stackFrame.declaringTypeName(), stackFrame.isStatic(), stackFrame.isConstructorCall(), getEnv(), compilerOptions, requestor, getProblemFactory());
            assertTrue("Should get one result but got " + (requestor.resultIndex + 1), requestor.resultIndex == 0);
            EvaluationResult result = requestor.results[0];
            assertTrue("Code snippet should not have problems", !result.hasProblems());
            assertTrue("Result should have a value", result.hasValue());
 ...
 */
    public void testNegative001() throws InstallException {
        try {
            ...
            this.context.evaluate(snippet, stackFrame.localVariableTypeNames(), stackFrame.localVariableNames(), stackFrame.localVariableModifiers(), stackFrame.declaringTypeName(), // force is static
            true, stackFrame.isConstructorCall(), getEnv(), getCompilerOptions(), requestor, getProblemFactory());
            ...
            assertEquals("Unexpected errors", "Cannot use this in a static context|", buffer == null ? "none" : buffer.toString());
        } finally {
 ...
 */
    public void testNegative002() throws Exception {
        try {
            ...
            assertEquals("Unexpected errors", "Cannot make a static reference to the non-static field x|", buffer == null ? "none" : buffer.toString());
        } finally {
 ...
 */
    public void testNegative003() throws InstallException {
        try {
            ...
            assertEquals("Unexpected errors", "zork cannot be resolved to a variable|", buffer == null ? "none" : buffer.toString());
        } finally {
 ...
 */
    public void testNegative004() throws InstallException {
        ...
        assertEquals("Unexpected errors", "The final field System.out cannot be assigned|", buffer == null ? "none" : buffer.toString());
    }
 ...
 */
    public void test063() throws Exception {
        ...
        try {
            ...
            this.context.evaluate(snippet, stackFrame.localVariableTypeNames(), stackFrame.localVariableNames(), stackFrame.localVariableModifiers(), stackFrame.declaringTypeName(), stackFrame.isStatic(), stackFrame.isConstructorCall(), getEnv(), compilerOpts, requestor, getProblemFactory());
            assertTrue("Should get one result but got " + (requestor.resultIndex + 1), requestor.resultIndex == 0);
            EvaluationResult result = requestor.results[0];
            assertTrue("Code snippet should not have problems", !result.hasProblems());
            assertTrue("Result should have a value", result.hasValue());
    ...
    public void test069() {
        ...
        try {
            ...
            assertTrue("Should get two results but got " + (requestor.resultIndex + 1), requestor.resultIndex == 1);
            EvaluationResult result = requestor.results[0];
            assertTrue("Code snippet should not have problems", result.hasProblems());
            assertEquals("Wrong size", 1, result.getProblems().length);
