...
public class LookupEnvironment implements BaseTypes, ProblemReasons, TypeConstants {
    ...
    // set to false once actual arrays are allocated
...
*/
    ArrayBinding createArrayType(TypeBinding type, int dimensionCount) {
        if (// cache local type arrays with the local type itself
        ...
        while (++index < length) {
            ...
            if (// no matching array, but space left
            currentBinding == null)
    ...
    public BinaryTypeBinding createBinaryTypeFrom(IBinaryType binaryType, PackageBinding packageBinding, boolean needFieldsAndMethods) {
        ...
        if (cachedType != null) {
            if (// sanity check before the cast... at this point the cache should ONLY contain unresolved types
...
*/
    ...
    public ParameterizedTypeBinding createParameterizedType(ReferenceBinding genericType, TypeBinding[] typeArguments, ReferenceBinding enclosingType) {
        ...
        if (cachedInfo != null) {
            nextCachedType: // iterate existing parameterized for reusing one with same type arguments if any
            for (int i = 0, max = cachedInfo.length; i < max; i++) {
                ParameterizedTypeBinding cachedType = cachedInfo[i];
                // remain of unresolved type
                if (cachedType.type != genericType)
    ...
    public WildcardBinding createWildcard(ReferenceBinding genericType, int rank, TypeBinding bound, int kind) {
        ...
        boolean needToGrow = false;
        if (cachedInfo != null) {
            nextCachedType: // iterate existing wildcards for reusing one with same information if any
            for (int i = 0, max = cachedInfo.length; i < max; i++) {
...
*/
    public ReferenceBinding getType(char[][] compoundName) {
        ...
        referenceBinding = BinaryTypeBinding.resolveType(referenceBinding, this, false);
...
*/
    ...
    TypeBinding getTypeFromTypeSignature(SignatureWrapper wrapper, TypeVariableBinding[] staticVariables, ReferenceBinding enclosingType) {
        ...
        if (wrapper.signature[wrapper.start] == 'T') {
            ...
            do {
                if (// per construction can only be binary type binding
                enclosingType instanceof BinaryTypeBinding) {
                    // do not trigger resolution of variables
                    TypeVariableBinding[] enclosingVariables = ((BinaryTypeBinding) enclosingType).typeVariables;
                    for (int i = enclosingVariables.length; --i >= 0; ) if (CharOperation.equals(enclosingVariables[i].sourceName, wrapper.signature, varStart, varEnd))
    ...
    TypeBinding getTypeFromVariantTypeSignature(SignatureWrapper wrapper, TypeVariableBinding[] staticVariables, ReferenceBinding enclosingType, ReferenceBinding genericType, int rank) {
        //   or '*'
...
*/
    ...
    public MethodVerifier methodVerifier() {
        if (verifier == null)
            verifier = this.options.sourceLevel < ClassFileConstants.JDK1_5 ? new MethodVerifier(this) : new MethodVerifier15(this);
        return verifier;
    }
    ...
    public void reset() {
        ...
        for (int i = this.uniqueArrayBindings.length; --i >= 0; ) {
            ...
            if (arrayBindings != null)
                for (int j = arrayBindings.length; --j >= 0; ) arrayBindings[j] = null;
        }
        this.uniqueParameterizedTypeBindings = new SimpleLookupTable(3);
        this.uniqueRawTypeBindings = new SimpleLookupTable(3);
        this.uniqueWildcardBindings = new SimpleLookupTable(3);
    ...
    void updateCaches(UnresolvedReferenceBinding unresolvedType, ReferenceBinding resolvedType) {
        ...
        if (// update the key
        uniqueParameterizedTypeBindings.get(unresolvedType) != null) {
        ...
        if (// update the key
        uniqueWildcardBindings.get(unresolvedType) != null) {
            Object[] keys = uniqueWildcardBindings.keyTable;
            for (int i = 0, l = keys.length; i < l; i++) {
                if (keys[i] == unresolvedType) {
                    // hashCode is based on compoundName so this works
                    keys[i] = resolvedType.isGenericType() ? createRawType(resolvedType, null) : resolvedType;
                    break;
                }
            }
        }
