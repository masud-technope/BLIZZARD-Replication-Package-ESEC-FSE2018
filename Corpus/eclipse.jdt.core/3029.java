    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        if (initialization != null) {
            ...
            if ((inlinedValue = initialization.constant) != NotAConstant) {
                ...
                if (// may need to preserve variable
                binding.resolvedPosition != -1) {
            ...
            } else // initializing to non-constant value
            {
                ...
                if (binding.resolvedPosition != -1) {
                    ...
                    if (binding.type.isArrayType() && (// arrayLoc = null
                    initialization.resolvedType == // arrayLoc = null
                    NullBinding || (// arrayLoc = (type[])null
                    (initialization instanceof CastExpression) && (((CastExpression) initialization).innermostCastedExpression().resolvedType == NullBinding)))) {
    ...
    public void resolve(BlockScope scope) {
        ...
        if (initialization != null) {
            if (initialization instanceof ArrayInitializer) {
                ...
                if (initializationType != null) {
                    if (initialization.isConstantValueOfTypeAssignableToType(initializationType, variableType) || (variableType.isBaseType() && BaseTypeBinding.isWidening(variableType.id, initializationType.id)) || initializationType.isCompatibleWith(variableType)) {
            ...
            if (binding != null) {
                binding.setConstant(binding.isFinal() ? initialization.constant.castTo((variableType.id << 4) + initialization.constant.typeID()) : NotAConstant);
            }
        }
    }
