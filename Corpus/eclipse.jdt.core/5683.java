...
public class BasicSearchEngine {
    ...
    void findMatches(SearchPattern pattern, SearchParticipant[] participants, IJavaSearchScope scope, SearchRequestor requestor, IProgressMonitor monitor) throws CoreException {
        try {
            if (VERBOSE) {
                ...
                "Searching for pattern: " + pattern.toString());
            ...
            for (int i = 0; i < length; i++) {
                ...
                try {
                    ...
                    indexManager.performConcurrentJob(new PatternSearchJob(pattern, participant, scope, pathCollector), IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, iterationMonitor.split(50));
    ...
    private ICompilationUnit[] getWorkingCopies() {
        ...
        if (this.workingCopies != null) {
            if (this.workingCopyOwner == null) {
                copies = JavaModelManager.getJavaModelManager().getWorkingCopies(DefaultWorkingCopyOwner.PRIMARY, /*don't add primary WCs a second time*/
                false);
                if (copies == null) {
            ...
            copies = JavaModelManager.getJavaModelManager().getWorkingCopies(this.workingCopyOwner, /*add primary WCs*/
            true);
        } else {
            copies = JavaModelManager.getJavaModelManager().getWorkingCopies(DefaultWorkingCopyOwner.PRIMARY, /*don't add primary WCs a second time*/
            false);
    ...
    boolean match(char[] patternPkg, int matchRulePkg, char[] patternDeclaringQualifier, int matchRuleDeclaringQualifier, char[] patternDeclaringSimpleName, int matchRuleDeclaringSimpleName, char[] patternMethodName, int methodMatchRule, char[] packageName, char[] declaringQualifier, char[] declaringSimpleName, char[] methodName) {
        if (patternPkg != null && !CharOperation.equals(patternPkg, packageName, (matchRulePkg & SearchPattern.R_CASE_SENSITIVE) != 0))
            return false;
        return match(patternDeclaringQualifier, matchRuleDeclaringQualifier, declaringQualifier) && match(patternDeclaringSimpleName, matchRuleDeclaringSimpleName, declaringSimpleName) && match(patternMethodName, methodMatchRule, methodName);
    }
    ...
    boolean match(char[] patternFusedQualifier, int matchRuleFusedQualifier, char[] patternMethodName, int methodMatchRule, char[] packageName, char[] declaringQualifier, char[] declaringSimpleName, char[] methodName) {
        ...
        return match(patternFusedQualifier, matchRuleFusedQualifier, q) && match(patternMethodName, methodMatchRule, methodName);
    }
    ...
    public void searchAllConstructorDeclarations(final char[] packageName, final char[] typeName, final int typeMatchRule, IJavaSearchScope scope, final IRestrictedAccessConstructorRequestor nameRequestor, int waitingPolicy, IProgressMonitor progressMonitor) throws JavaModelException {
        try {
            ...
            // Used as TYPE_SUFFIX has no effect in method #match(char, char[] , int, char[], int , int, char[], char[])
            final char NoSuffix = IIndexConstants.TYPE_SUFFIX;
            // Debug
            if (VERBOSE) {
                //$NON-NLS-1$
                Util.verbose(//$NON-NLS-1$
                "BasicSearchEngine.searchAllConstructorDeclarations(char[], char[], int, IJavaSearchScope, IRestrictedAccessConstructorRequestor, int, IProgressMonitor)");
                //$NON-NLS-1$ //$NON-NLS-2$
                ...
                //$NON-NLS-1$
                Util.verbose("	- type match rule: " + getMatchRuleString(typeMatchRule));
            ...
            // invalid match rule => return no results
            if (validatedTypeMatchRule == -1)
                return;
            // Create pattern
            IndexManager indexManager = JavaModelManager.getIndexManager();
            final ConstructorDeclarationPattern pattern = new ConstructorDeclarationPattern(packageName, typeName, validatedTypeMatchRule);
            ...
            getDefaultSearchParticipant(), scope, searchRequestor), waitingPolicy, subMonitor.split(Math.max(1000 - copiesLength, 0)));
            ...
            if (copies != null) {
                for (int i = 0; i < copiesLength; i++) {
                    ...
                    if (scope instanceof HierarchyScope) {
                        if (!((HierarchyScope) scope).encloses(workingCopy, iterationMonitor))
                            continue;
                    } else {
                    ...
                    if (workingCopy.isConsistent()) {
                        ...
                        for (int j = 0, allTypesLength = allTypes.length; j < allTypesLength; j++) {
                            ...
                            if (match(NoSuffix, packageName, pkgMatchRule, typeName, /*no kind*/
                            validatedTypeMatchRule, 0, packageDeclaration, simpleName) && !type.isMember()) {
                                ...
                                for (int k = 0; k < methods.length; k++) {
                                    ...
                                    if (method.isConstructor()) {
                                        ...
                                        nameRequestor.acceptConstructor(method.getFlags(), simpleName, parameterNames.length, // signature is not used for source type
                                        null, parameterTypes, parameterNames, type.getFlags(), packageDeclaration, extraFlags, path, null);
                                    }
                                }
                                if (!hasConstructor) {
                                    nameRequestor.acceptConstructor(Flags.AccPublic, simpleName, -1, // signature is not used for source type
                                    null, CharOperation.NO_CHAR_CHAR, CharOperation.NO_CHAR_CHAR, type.getFlags(), packageDeclaration, extraFlags, path, null);
                                }
                            }
                        }
                    } else {
                            ...
                            class AllConstructorDeclarationsVisitor extends ASTVisitor {
                                ...
                                private void endVisit(TypeDeclaration typeDeclaration) {
                                    if (!hasConstructor(typeDeclaration) && typeDeclaration.enclosingType == null) {
                                        if (match(NoSuffix, packageName, pkgMatchRule, /*no kind*/
                                        typeName, validatedTypeMatchRule, 0, packageDeclaration, typeDeclaration.name)) {
                                            nameRequestor.acceptConstructor(Flags.AccPublic, typeName, -1, // signature is not used for source type
                                            null, CharOperation.NO_CHAR_CHAR, CharOperation.NO_CHAR_CHAR, typeDeclaration.modifiers, packageDeclaration, ExtraFlags.getExtraFlags(typeDeclaration), path, null);
                                        }
                                    }
                                ...
                                public boolean visit(ConstructorDeclaration constructorDeclaration, ClassScope classScope) {
                                    ...
                                    typeName, validatedTypeMatchRule, 0, packageDeclaration, typeDeclaration.name)) {
                                        ...
                                        nameRequestor.acceptConstructor(constructorDeclaration.modifiers, typeName, parameterNames.length, // signature is not used for source type
                                        null, parameterTypes, parameterNames, typeDeclaration.modifiers, packageDeclaration, ExtraFlags.getExtraFlags(typeDeclaration), path, null);
                                    }
                                    // no need to find constructors from local/anonymous type
                                    return false;
                                }
    ...
    public void searchAllMethodNames(final char[] qualifier, final int qualifierMatchRule, final char[] methodName, final int methodMatchRule, IJavaSearchScope scope, final IRestrictedAccessMethodRequestor nameRequestor, int waitingPolicy, IProgressMonitor progressMonitor) throws JavaModelException {
        ...
        if (VERBOSE) {
            ...
            //$NON-NLS-1$
            Util.verbose("	- method match rule: " + getMatchRuleString(methodMatchRule));
        ...
        IndexQueryRequestor searchRequestor = new IndexQueryRequestor() {
            ...
            public boolean acceptIndexMatch(String documentPath, SearchPattern indexRecord, SearchParticipant participant, AccessRuleSet access) {
                ...
                if (match(qualifier, qualifierMatchRule, methodName, methodMatchRule, record.declaringPackageName, record.declaringQualification, record.declaringSimpleName, record.selector)) {
                    nameRequestor.acceptMethod(record.selector, record.parameterCount, record.declaringQualification, record.declaringSimpleName, record.declaringTypeModifiers, record.declaringPackageName, record.signature, record.parameterTypes, record.parameterNames, record.returnSimpleName, record.modifiers, documentPath, accessRestriction, /* method index not applicable as there is no IType here */
                    -1);
                }
                return true;
            }
                ...
                if (workingCopy.isConsistent()) {
                    ...
                    for (int j = 0, allTypesLength = allTypes.length; j < allTypesLength; j++) {
                        ...
                        if (!match(qualifier, qualifierMatchRule, q))
                            continue;
                        reportMatchingMethods(methodName, methodMatchRule, nameRequestor, path, packageDeclaration, type, rDeclaringQualification, rSimpleName);
                    }
                } else {
                    Parser basicParser = getParser();
                    org.eclipse.jdt.internal.compiler.env.ICompilationUnit unit = (org.eclipse.jdt.internal.compiler.env.ICompilationUnit) workingCopy;
                    ...
                    if (parsedUnit != null) {
                        ...
                        class AllMethodDeclarationVisitor extends ASTVisitor {
                            ...
                            public boolean visit(MethodDeclaration methodDeclaration, ClassScope classScope) {
                                ...
                                if (typeInfo.visitMethods && match(methodName, methodMatchRule, methodDeclaration.selector)) {
                                    reportMatchingMethod(path, packageDeclaration, typeInfo.enclosingTypeName, typeInfo.typeDecl, methodDeclaration, getCurrentType(), nameRequestor);
                                }
                                // no need to find methods from local/anonymous type
                                return false;
                            }

                            public boolean visit(TypeDeclaration typeDeclaration, BlockScope blockScope) {
                                // do not visit local/anonymous types
                                return false;
                            }
                            ...
                            public boolean visit(TypeDeclaration memberTypeDeclaration, ClassScope s) {
                                TypeInfo typeInfo = this.typeInfoStack.peek();
                                addStackEntry(memberTypeDeclaration, typeInfo.enclosingTypeName == CharOperation.NO_CHAR ? typeInfo.typeDecl.name : CharOperation.concat(typeInfo.enclosingTypeName, typeInfo.typeDecl.name, '.'));
                                return true;
                            }
                        }
    ...
    public void searchAllMethodNames(final char[] packageName, final int pkgMatchRule, final char[] declaringQualification, final int declQualificationMatchRule, final char[] declaringSimpleName, final int declSimpleNameMatchRule, final char[] methodName, final int methodMatchRule, IJavaSearchScope scope, final IRestrictedAccessMethodRequestor nameRequestor, int waitingPolicy, IProgressMonitor progressMonitor) throws JavaModelException {
        ...
        IndexQueryRequestor searchRequestor = new IndexQueryRequestor() {
            ...
            public boolean acceptIndexMatch(String documentPath, SearchPattern indexRecord, SearchParticipant participant, AccessRuleSet access) {
                ...
                if (match(packageName, pkgMatchRule, declaringQualification, declQualificationMatchRule, declaringSimpleName, declSimpleNameMatchRule, methodName, methodMatchRule, record.declaringPackageName, record.declaringQualification, record.declaringSimpleName, record.selector)) {
                    nameRequestor.acceptMethod(record.selector, record.parameterCount, record.declaringQualification, record.declaringSimpleName, record.declaringTypeModifiers, record.declaringPackageName, record.signature, record.parameterTypes, record.parameterNames, record.returnSimpleName, record.modifiers, documentPath, accessRestriction, /* method index not applicable as there is no IType here */
                    -1);
                }
                return true;
            ...
            for (int i = 0; i < copiesLength; i++) {
                ...
                if (workingCopy.isConsistent()) {
                    ...
                    for (int j = 0, allTypesLength = allTypes.length; j < allTypesLength; j++) {
                        ...
                        char[] rDeclaringQualification = parent instanceof IType ? ((IType) parent).getTypeQualifiedName('.').toCharArray() : CharOperation.NO_CHAR;
                        char[] rSimpleName = type.getElementName().toCharArray();
                        if (!match(declaringQualification, declQualificationMatchRule, rDeclaringQualification) || !match(declaringSimpleName, declSimpleNameMatchRule, rSimpleName))
                            continue;
                        reportMatchingMethods(methodName, methodMatchRule, nameRequestor, path, packageDeclaration, type, rDeclaringQualification, rSimpleName);
                    }
                } else {
                    ...
                    if (parsedUnit != null) {
                        ...
                        class AllMethodDeclarationVisitor extends ASTVisitor {
                            ...
                            private void addStackEntry(TypeDeclaration typeDeclaration, char[] enclosingTypeName) {
                                boolean visitMethods = match(declaringQualification, declQualificationMatchRule, enclosingTypeName) && match(declaringSimpleName, declSimpleNameMatchRule, typeDeclaration.name);
                                this.typeInfoStack.push(new TypeInfo(typeDeclaration, visitMethods, enclosingTypeName));
                            }
    ...
    void reportMatchingMethod(final String path, final char[] packageDeclaration, final char[] declaringQualifier, final TypeDeclaration typeDeclaration, final MethodDeclaration methodDeclaration, final IType type, final IRestrictedAccessMethodRequestor nameRequestor) {
        ...
        if (nameRequestor instanceof MethodNameMatchRequestorWrapper) {
            ...
            nameRequestor.acceptMethod(methodDeclaration.selector, argsLength, declaringQualifier, typeDeclaration.name, typeDeclaration.modifiers, packageDeclaration, null, parameterTypes, parameterNames, returnType, methodDeclaration.modifiers, path, null, /* method index */
            -1);
        }
    }

    void reportMatchingMethods(final char[] methodName, final int methodMatchRule, final IRestrictedAccessMethodRequestor nameRequestor, final String path, char[] packageDeclaration, IType type, char[] rDeclaringQualification, char[] rSimpleName) throws JavaModelException {
        IMethod[] methods = type.getMethods();
        for (int k = 0; k < methods.length; k++) {
            IMethod method = methods[k];
            if (method.isConstructor())
            ...
            if (match(methodName, methodMatchRule, rMethodName)) {
                if (nameRequestor instanceof MethodNameMatchRequestorWrapper) {
                    ...
                    nameRequestor.acceptMethod(rMethodName, parameterNames.length, rDeclaringQualification, rSimpleName, type.getFlags(), packageDeclaration, // signature not used for source 
                    null, parameterTypes, parameterNames, returnTypeName, method.getFlags(), path, null, k);
                }
            }
        }
    }
    ...
    public void searchAllSecondaryTypeNames(IPackageFragmentRoot[] sourceFolders, final IRestrictedAccessTypeRequestor nameRequestor, boolean waitForIndexes, IProgressMonitor progressMonitor) throws JavaModelException {
        try {
            if (VERBOSE) {
                //$NON-NLS-1$
                Util.verbose(//$NON-NLS-1$
                "BasicSearchEngine.searchAllSecondaryTypeNames(IPackageFragmentRoot[], IRestrictedAccessTypeRequestor, boolean, IProgressMonitor)");
            ...
            try {
                ...
                indexManager.performConcurrentJob(new PatternSearchJob(pattern, getDefaultSearchParticipant(), createJavaSearchScope(sourceFolders), searchRequestor), waitForIndexes ? IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH : IJavaSearchConstants.FORCE_IMMEDIATE_SEARCH, subMonitor.split(100));
    ...
    public void searchAllTypeNames(final char[] packageName, final int packageMatchRule, final char[] typeName, final int typeMatchRule, int searchFor, IJavaSearchScope scope, final IRestrictedAccessTypeRequestor nameRequestor, int waitingPolicy, IProgressMonitor progressMonitor) throws JavaModelException {
        try {
            ...
            if (VERBOSE) {
                //$NON-NLS-1$
                Util.verbose(//$NON-NLS-1$
                "BasicSearchEngine.searchAllTypeNames(char[], char[], int, int, IJavaSearchScope, IRestrictedAccessTypeRequestor, int, IProgressMonitor)");
                //$NON-NLS-1$ //$NON-NLS-2$
                Util.verbose("	- package name: " + (packageName == null ? "null" : new String(packageName)));
                //$NON-NLS-1$
                Util.verbose("	- package match rule: " + getMatchRuleString(packageMatchRule));
                //$NON-NLS-1$ //$NON-NLS-2$
                Util.verbose("	- type name: " + (typeName == null ? "null" : new String(typeName)));
                //$NON-NLS-1$
                Util.verbose("	- type match rule: " + getMatchRuleString(typeMatchRule));
            ...
            final TypeDeclarationPattern pattern = packageMatchRule == SearchPattern.R_EXACT_MATCH ? new TypeDeclarationPattern(packageName, null, typeName, typeSuffix, validatedTypeMatchRule) : new QualifiedTypeDeclarationPattern(packageName, packageMatchRule, typeName, typeSuffix, validatedTypeMatchRule);
            // Get working copy path(s). Store in a single string in case of only one to optimize comparison in requestor
            final HashSet workingCopyPaths = new HashSet();
            String workingCopyPath = null;
            ICompilationUnit[] copies = getWorkingCopies();
	 ...
	 * 	for detailed comment
	 */
    public void searchAllTypeNames(final char[][] qualifications, final char[][] typeNames, final int matchRule, int searchFor, IJavaSearchScope scope, final IRestrictedAccessTypeRequestor nameRequestor, int waitingPolicy, IProgressMonitor progressMonitor) throws JavaModelException {
        try {
            // Debug
            if (VERBOSE) {
                //$NON-NLS-1$
                ...
                //$NON-NLS-1$
                Util.verbose("	- match rule: " + getMatchRuleString(matchRule));
                //$NON-NLS-1$
                Util.verbose("	- search for: " + searchFor);
                //$NON-NLS-1$
                Util.verbose("	- scope: " + scope);
            ...
            // add type names from indexes
            indexManager.performConcurrentJob(new PatternSearchJob(pattern, // Java search only
            getDefaultSearchParticipant(), scope, searchRequestor), waitingPolicy, subMonitor.split(100));
            // add type names from working copies
            if (copies != null) {
                for (int i = 0, length = copies.length; i < length; i++) {
                    ICompilationUnit workingCopy = copies[i];
                    ...
                    if (workingCopy.isConsistent()) {
                        ...
                        if (parsedUnit != null) {
                            ...
                            class AllTypeDeclarationsVisitor extends ASTVisitor {
                                ...
                                public boolean visit(TypeDeclaration typeDeclaration, CompilationUnitScope compilationUnitScope) {
                                    SearchPattern decodedPattern = new QualifiedTypeDeclarationPattern(packageDeclaration, typeDeclaration.name, convertTypeKind(TypeDeclaration.kind(typeDeclaration.modifiers)), matchRule);
                                ...
                                public boolean visit(TypeDeclaration memberTypeDeclaration, ClassScope classScope) {
                                    ...
                                    SearchPattern decodedPattern = new QualifiedTypeDeclarationPattern(qualification, memberTypeDeclaration.name, convertTypeKind(TypeDeclaration.kind(memberTypeDeclaration.modifiers)), matchRule);
                                    if (pattern.matchesDecodedKey(decodedPattern)) {
                                        nameRequestor.acceptType(memberTypeDeclaration.modifiers, packageDeclaration, memberTypeDeclaration.name, enclosingTypeNames, path, null);
                                    }
                                    return true;
            ...
            try {
                if (resource instanceof IFile) {
                    try {
                        ...
                        SearchDocument[] documents = MatchLocator.addWorkingCopies(pattern, new SearchDocument[] { new JavaSearchDocument(enclosingElement.getPath().toString(), participant) }, getWorkingCopies(enclosingElement), participant);
                        participant.locateMatches(documents, pattern, scope, requestor, monitor);
                    } finally {
                        requestor.endReporting();
                    }
                } else {
                    search(pattern, new SearchParticipant[] { getDefaultSearchParticipant() }, scope, requestor, monitor);
                }
