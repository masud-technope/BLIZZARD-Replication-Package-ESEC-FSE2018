...
public class TypeAnnotationWalker implements ITypeAnnotationWalker {
    ...
    // the actual material we're managing here
    protected final IBinaryTypeAnnotation[] typeAnnotations;

    // bit mask of indices into typeAnnotations, 1 means active, 0 is filtered during the walk
    protected final long matches;

    // pointer into the typePath
    ...
    public  TypeAnnotationWalker(IBinaryTypeAnnotation[] typeAnnotations) {
        // initialize so lowest length bits are 1
    ...
    public ITypeAnnotationWalker toTypeParameterBounds(boolean isClassTypeParameter, int parameterRank) {
        ...
        if (newMatches == 0)
            return EMPTY_ANNOTATION_WALKER;
        int length = this.typeAnnotations.length;
        int targetType = isClassTypeParameter ? AnnotationTargetTypeConstants.CLASS_TYPE_PARAMETER_BOUND : AnnotationTargetTypeConstants.METHOD_TYPE_PARAMETER_BOUND;
        long mask = 1;
        for (int i = 0; i < length; i++, mask = mask << 1) {
            IBinaryTypeAnnotation candidate = this.typeAnnotations[i];
    ...
    public ITypeAnnotationWalker toTypeArgument(int rank) {
        ...
        for (int i = 0; i < length; i++, mask = mask << 1) {
            ...
            if (this.pathPtr >= path.length || path[this.pathPtr] != AnnotationTargetTypeConstants.TYPE_ARGUMENT || path[this.pathPtr + 1] != rank) {
                newMatches &= ~mask;
            }
        }
