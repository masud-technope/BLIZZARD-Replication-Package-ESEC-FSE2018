...
public class TypeParameterDeclarationImpl extends DeclarationImpl implements TypeParameterDeclaration, TypeVariable, EclipseMirrorType {
    ...
    public  TypeParameterDeclarationImpl(final ITypeBinding binding, final BaseProcessorEnv env) {
        super(binding, env);
    ...
    public MirrorKind kind() {
        return MirrorKind.TYPE_PARAMETER_VARIABLE;
    }
    ...
    public boolean isSubTypeCompatible(EclipseMirrorType type) {
        if (type.kind() == MirrorKind.TYPE_PARAMETER_VARIABLE) {
            ...
            return getOwnerBinding() == other.getOwnerBinding() && getSimpleName().equals(other.getSimpleName());
        }
