...
public class State {
    ...
    void copyFrom(State lastState) {
        ...
        try {
            ...
            for (int i = 0, l = keyTable.length; i < l; i++) if (keyTable[i] != null)
    ...
    public boolean isKnownPackage(String qualifiedPackageName) {
        if (this.knownPackageNames == null) {
            ...
            for (int i = 0, l = keyTable.length; i < l; i++) {
                if (keyTable[i] != null) {
                    // is a type name of the form p1/p2/A
        ...
        for (int i = 0, l = this.knownPackageNames.length; i < l; i++) if (this.knownPackageNames[i].equals(qualifiedPackageName))
            return true;
        return false;
    }
    ...
    boolean isSourceFolderEmpty(IContainer sourceFolder) {
        ...
        for (int i = 0, l = table.length; i < l; i++) if (table[i] != null && ((String) table[i]).startsWith(sourceFolderName))
            return false;
        return true;
    }
    ...
    void record(String typeLocator, char[][][] qualifiedRefs, char[][] simpleRefs, char[][] rootRefs, char[] mainTypeName, ArrayList typeNames) {
        if (typeNames.size() == 1 && CharOperation.equals(mainTypeName, (char[]) typeNames.get(0))) {
            ...
            // can be empty when no types are defined
    ...
    void recordStructuralDependency(IProject prereqProject, State prereqState) {
        if (prereqState != null)
            if (// can skip if 0 (full build) since its assumed to be 0 if unknown
            prereqState.lastStructuralBuildTime > 0)
    ...
    static State read(IProject project, DataInputStream in) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            ClasspathMultiDirectory md = (ClasspathMultiDirectory) ClasspathLocation.forSourceFolder(sourceFolder, outputFolder, readNames(in), readNames(in), in.readBoolean());
            if (in.readBoolean())
        ...
        for (int i = 0; i < length; i++) newState.structuralBuildTimes.put(in.readUTF(), Long.valueOf(in.readLong()));
        ...
        for (int i = 0; i < length; i++) {
            ...
            switch(in.readByte()) {
                case 1:
                    ...
                    for (int j = 0, m = rootNames.length; j < m; j++) rootNames[j] = internedRootNames[in.readInt()];
                ...
                case 2:
                    ...
                    for (int j = 0, m = rNames.length; j < m; j++) rNames[j] = internedRootNames[in.readInt()];
        ...
        for (int i = 0, l = valueTable.length; i < l; i++) {
            if (valueTable[i] != null) {
                ...
                for (int j = 0, m = rNames.length; j < m; j++) {
                    char[] rName = rNames[j];
                    if (// remember the names have been interned
                    !internedRootNames.containsKey(rName))
 ...
 */
    public String toString() {
        return //$NON-NLS-1$
        "State for " + this.javaProjectName + " (#" + //$NON-NLS-1$
