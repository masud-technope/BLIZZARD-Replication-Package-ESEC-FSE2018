...
public abstract class AbstractCommentParser implements JavadocTagConstants {
    ...
    public static final int COMPIL_PARSER = 0x0001;

    public static final int DOM_PARSER = 0x0002;

    public static final int SELECTION_PARSER = 0x0004;

    public static final int COMPLETION_PARSER = 0x0008;

    public static final int SOURCE_PARSER = 0x0010;

    public static final int FORMATTER_COMMENT_PARSER = 0x0020;

    protected static final int PARSER_KIND = 0x00FF;

    // flag saying that text must be stored
    protected static final int TEXT_PARSE = 0x0100;

    // flag saying that text must be verified
    protected static final int TEXT_VERIF = 0x0200;
    ...
    protected  AbstractCommentParser(Parser sourceParser) {
        ...
        this.scanner = new Scanner(false, false, false, ClassFileConstants.JDK1_3, null, null, /*taskCaseSensitive*/
        true);
    ...
    protected boolean commentParse() {
        ...
        try {
            ...
            // supposed text end, it will be refined later...
    ...
    protected Object parseArguments(Object receiver) throws InvalidInputException {
        ...
        nextArg: while (this.index < this.scanner.eofPosition) {
            ...
            if (readToken() == TerminalTokens.TokenNameIdentifier) {
                ...
                if (// verify that all arguments name are declared
                argName == null) {
                    if (!firstArg) {
                        break nextArg;
                    }
                ...
                iToken++;
            } else if (// verify that no argument name is declared
            argName != null) {
                break nextArg;
            }
        ...
        try {
            ...
            if (currentChar == 'a' || currentChar == 'A') {
                ...
                if (readToken() == TerminalTokens.TokenNameIdentifier) {
                    consumeToken();
                    try {
                        if (CharOperation.equals(this.scanner.getCurrentIdentifierSource(), HREF_TAG, false) && readToken() == TerminalTokens.TokenNameEQUAL) {
                            consumeToken();
                            if (readToken() == TerminalTokens.TokenNameStringLiteral) {
                                consumeToken();
                                while (// main loop to search for the </a> pattern
                                this.index < this.javadocEnd) {
                                    ...
                                    while (readToken() != TerminalTokens.TokenNameGREATER) {
                                        ...
                                        // consume token without updating line end
                                        this.currentTokenType = -1;
                                    }
            ...
            this.currentTokenType = -1;
            // Signal syntax error
            if (// do not report error for @value tag, this will be done after...
    ...
    protected Object parseMember(Object receiver) throws InvalidInputException {
        ...
        if (readToken() == TerminalTokens.TokenNameIdentifier) {
            if (// member name may be qualified (inner class constructor reference)
        ...
        try {
            ...
            nextToken: while (true) {
                ...
                switch(token) {
                    ...
                    // $FALL-THROUGH$ - fall through next case to report error
                    default:
                        if (token == TerminalTokens.TokenNameLEFT_SHIFT)
                            isTypeParam = true;
    ...
    protected Object parseQualifiedName(boolean reset) throws InvalidInputException {
        ...
        nextToken: for (int iToken = 0; ; iToken++) {
            ...
            switch(token) {
                case TerminalTokens.TokenNameIdentifier:
                    if (// identifiers must be odd tokens
                    ((iToken & 1) != 0)) {
                        break nextToken;
                    }
                ...
                switch(token) {
                    ...
                    TerminalTokens.TokenNameStringLiteral:
                        ...
                        if (this.tagValue == TAG_VALUE_VALUE) {
                            ...
                            if (this.reportProblems)
                                this.sourceParser.problemReporter().javadocInvalidValueReference(start, getTokenEndPosition(), this.sourceParser.modifiers);
                            return false;
                        }
                        // verify end line
                        ...
                        return false;
                    case // @see <a href="URL#Value">label</a>
                    TerminalTokens.TokenNameLESS:
                        // Currently as javadoc 1.4.2 ignore it, we do the same (see bug 69302)
                        if (typeRef != null)
                            break nextToken;
                        consumeToken();
                        ...
                        if (parseHref()) {
                            ...
                            if (this.tagValue == TAG_VALUE_VALUE) {
                                // String reference are not allowed for @value tag
                                if (this.reportProblems)
                                    this.sourceParser.problemReporter().javadocInvalidValueReference(start, getIndexPosition(), this.sourceParser.modifiers);
                                return false;
                            }
                            // verify end line
                            if (verifyEndLine(previousPosition)) {
                    ...
                    case TerminalTokens.TokenNameERROR:
                        consumeToken();
                        if (// @see ...#member
                        this.scanner.currentCharacter == // @see ...#member
                        '#') {
                        ...
                        if (currentError.length > 0 && currentError[0] == '"') {
                            if (this.reportProblems) {
                                ...
                                if (this.tagValue == TAG_SEE_VALUE) {
                                    int /* first char is " */
                                    length = currentError.length, i = 1;
    ...
    protected void pushOnAstStack(Object node, boolean newLength) {
        if (node == null) {
            ...
            if (++this.astLengthPtr >= stackLength) {
                System.arraycopy(this.astLengthStack, 0, this.astLengthStack = new int[stackLength + AST_STACK_INCREMENT], 0, stackLength);
            }
        ...
        if (++this.astPtr >= stackLength) {
            System.arraycopy(this.astStack, 0, this.astStack = new Object[stackLength + AST_STACK_INCREMENT], 0, stackLength);
    ...
    protected void recordInheritedPosition(long position) {
        if (this.inheritedPositions == null) {
            ...
            if (this.inheritedPositionsPtr == this.inheritedPositions.length) {
                System.arraycopy(this.inheritedPositions, 0, this.inheritedPositions = new long[this.inheritedPositionsPtr + INHERITED_POSITIONS_ARRAY_INCREMENT], 0, this.inheritedPositionsPtr);
            }
        }
        ...
        while (this.index < this.source.length) {
            switch(ch) {
                case '*':
                    ...
                    break;
                case '/':
                    if (// valid only if a star was the previous character
