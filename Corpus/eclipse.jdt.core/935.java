...
public class ClasspathDirectory extends ClasspathLocation {
    ...
    // ability to only consider one kind of files (source vs. binaries), by default use both
    private int mode;
    ...
    public NameEnvironmentAnswer findClass(char[] typeName, String qualifiedPackageName, String qualifiedBinaryFileName, boolean asBinaryOnly) {
        ...
        if (sourceExists && !asBinaryOnly) {
            ...
            if (!binaryExists)
                return new NameEnvironmentAnswer(new CompilationUnit(null, fullSourcePath, this.encoding, this.destinationPath), fetchAccessRestriction(qualifiedBinaryFileName));
        ...
        if (binaryExists) {
            try {
                ...
                String typeSearched = qualifiedPackageName.length() > 0 ? //$NON-NLS-1$
                qualifiedPackageName.replace(File.separatorChar, '/') + "/" + //$NON-NLS-1$
                fileName : fileName;
    ...
    public NameEnvironmentAnswer findSecondaryInClass(char[] typeName, String qualifiedPackageName, String qualifiedBinaryFileName) {
        ...
        return prereqs ? null : findSourceSecondaryType(typeNameString, qualifiedPackageName, qualifiedBinaryFileName);
    /* only secondary types */
    }
 ...
 */
    private Hashtable<String, String> getPackageTypes(String qualifiedPackageName) {
        ...
        String[] dirList = (String[]) this.directoryCache.get(qualifiedPackageName);
        if (// package exists in another classpath directory or jar 
        dirList == this.missingPackageHolder || dirList == null)
            return packageEntry;
        File dir = new File(this.path + qualifiedPackageName);
    ...
    private NameEnvironmentAnswer findSourceSecondaryType(String typeName, String qualifiedPackageName, String qualifiedBinaryFileName) {
        ...
        return fileName != null ? new NameEnvironmentAnswer(new CompilationUnit(null, fileName, this.encoding, this.destinationPath), fetchAccessRestriction(qualifiedBinaryFileName)) : null;
    }
