...
public class Compiler implements ITypeRequestor, ProblemSeverities {
    ...
    public void accept(IBinaryType binaryType, PackageBinding packageBinding) {
        if (options.verbose) {
            System.out.println(Util.bind("compilation.loadBinary", new String[] { new String(binaryType.getName()) }));
    ...
    public void accept(ICompilationUnit sourceUnit) {
        ...
        CompilationResult unitResult = new CompilationResult(sourceUnit, totalUnits, totalUnits, this.options.maxProblemsPerUnit);
        try {
            if (options.verbose) {
    ...
    public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding) {
        problemReporter.abortDueToInternalError(Util.bind(//$NON-NLS-1$
        "abort.againstSourceModel", String.valueOf(sourceTypes[0].getName()), String.valueOf(sourceTypes[0].getFileName())));
    }
    ...
    protected void beginToCompile(ICompilationUnit[] sourceUnits) {
        ...
        for (int i = 0; i < maxUnits; i++) {
            ...
            try {
                if (options.verbose) {
                    System.out.println(Util.bind(//$NON-NLS-1$
                    "compilation.request", new String[] { String.valueOf(i + 1), String.valueOf(maxUnits), new String(sourceUnits[i].getFileName()) }));
                }
    ...
    public void compile(ICompilationUnit[] sourceUnits) {
        ...
        try {
            ...
            for (; i < totalUnits; i++) {
                ...
                try {
                    if (options.verbose)
                        System.out.println(Util.bind(//$NON-NLS-1$
                        "compilation.process", new String[] { String.valueOf(i + 1), String.valueOf(totalUnits), new String(unitsToProcess[i].getFileName()) }));
                    process(unit, i);
                } finally {
                ...
                if (options.verbose)
                    System.out.println(Util.bind("compilation.done", new String[] { String.valueOf(i + 1), String.valueOf(totalUnits), new String(unit.getFileName()) }));
            }
        ...
        if (options.verbose) {
            if (totalUnits > 1) {
                System.out.println(Util.bind("compilation.units", //$NON-NLS-1$
                String.valueOf(//$NON-NLS-1$
                totalUnits)));
            } else {
                System.out.println(Util.bind("compilation.unit", //$NON-NLS-1$
                String.valueOf(//$NON-NLS-1$
                totalUnits)));
            }
        }
    ...
    protected void handleInternalException(Throwable internalException, CompilationUnitDeclaration unit, CompilationResult result) {
        ...
        if (// basing result upon the current unit if available
        (unit != null))
        ...
        if (result != null) {
            ...
            String[] pbArguments = new String[] { //$NON-NLS-1$
            Util.bind(//$NON-NLS-1$
            "compilation.internalError") + //$NON-NLS-1$
            "\n" + buffer.toString() };
            result.record(problemReporter.createProblem(result.getFileName(), IProblem.Unclassified, pbArguments, pbArguments, // severity
            Error, // source start
            0, // source end
            0, // line number		
            0), unit);
