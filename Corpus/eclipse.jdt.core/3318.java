...
public class LocalVariable extends SourceRefElement implements ILocalVariable {
    ...
    public  LocalVariable(JavaElement parent, String name, int declarationSourceStart, int declarationSourceEnd, int nameStart, int nameEnd, String typeSignature, org.eclipse.jdt.internal.compiler.ast.Annotation[] astAnnotations, int flags, boolean isParameter) {
        super(parent);
        this.name = name;
    ...
    public boolean exists() {
        // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=46192
            ...
            if (this.isParameter) {
                ...
                for (int i = 0; i < parameters.length; i++) {
                    if (this.equals(parameters[i])) {
                        // always first occurrence, followed by parameter rank //$NON-NLS-1$
