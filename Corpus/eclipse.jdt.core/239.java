...
public class SourceTypeBinding extends ReferenceBinding {
    ...
    // MUST NOT be modified directly, use setter !
...
*/
    public FieldBinding addSyntheticFieldForInnerclass(LocalVariableBinding actualOuterLocalVariable) {
        ...
        do {
            ...
            if ((existingField = getField(synthField.name, /*resolve*/
            true)) != null) {
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(TypeConstants.SYNTHETIC_OUTER_LOCAL_PREFIX, actualOuterLocalVariable.name, //$NON-NLS-1$
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    if (fieldDecl.binding == existingField) {
                        if (this.scope.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_5) {
                            synthField.name = CharOperation.concat(synthField.name, //$NON-NLS-1$
                            "$".toCharArray());
                            needRecheck = true;
                        } else {
...
*/
    public FieldBinding addSyntheticFieldForClassLiteral(TypeBinding targetType, BlockScope blockScope) {
        ...
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(CharOperation.concat(TypeConstants.SYNTHETIC_CLASS, String.valueOf(this.synthetics[SourceTypeBinding.CLASS_LITERAL_EMUL].size()).toCharArray()), blockScope.getJavaLangClass(), ClassFileConstants.AccDefault | ClassFileConstants.AccStatic | ClassFileConstants.AccSynthetic, this, Constant.NotAConstant, this.synthetics[SourceTypeBinding.CLASS_LITERAL_EMUL].size());
            this.synthetics[SourceTypeBinding.CLASS_LITERAL_EMUL].put(targetType, synthField);
        }
        // ensure there is not already such a field defined by the user
        FieldBinding existingField;
        if ((existingField = getField(synthField.name, true)) != null) {
...
*/
    public FieldBinding addSyntheticFieldForAssert(BlockScope blockScope) {
        ...
        if (this.synthetics[SourceTypeBinding.FIELD_EMUL] == null)
            this.synthetics[SourceTypeBinding.FIELD_EMUL] = new HashMap(5);
        //$NON-NLS-1$
        FieldBinding synthField = (FieldBinding) this.synthetics[SourceTypeBinding.FIELD_EMUL].get("assertionEmulation");
        if (synthField == null) {
            synthField = new SyntheticFieldBinding(TypeConstants.SYNTHETIC_ASSERT_DISABLED, TypeBinding.BOOLEAN, (isInterface() ? ClassFileConstants.AccPublic : ClassFileConstants.AccDefault) | ClassFileConstants.AccStatic | ClassFileConstants.AccSynthetic | ClassFileConstants.AccFinal, this, Constant.NotAConstant, this.synthetics[SourceTypeBinding.FIELD_EMUL].size());
            //$NON-NLS-1$
            this.synthetics[SourceTypeBinding.FIELD_EMUL].put("assertionEmulation", synthField);
        }
        // ensure there is not already such a field defined by the user
        // ensure there is not already such a field defined by the user
        boolean needRecheck;
        ...
        do {
            ...
            true)) != null) {
                ...
                for (int i = 0; i < max; i++) {
                    ...
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(TypeConstants.SYNTHETIC_ASSERT_DISABLED, //$NON-NLS-1$
                        ("_" + String.valueOf(index++)).toCharArray());
                        needRecheck = true;
                        break;
                    }
                }
...
*/
    public FieldBinding addSyntheticFieldForEnumValues() {
        ...
        if (this.synthetics[SourceTypeBinding.FIELD_EMUL] == null)
            this.synthetics[SourceTypeBinding.FIELD_EMUL] = new HashMap(5);
        //$NON-NLS-1$
        FieldBinding synthField = (FieldBinding) this.synthetics[SourceTypeBinding.FIELD_EMUL].get("enumConstantValues");
        if (synthField == null) {
            ...
            //$NON-NLS-1$
            this.synthetics[SourceTypeBinding.FIELD_EMUL].put("enumConstantValues", synthField);
        }
        // ensure there is not already such a field defined by the user
        // ensure there is not already such a field defined by the user
        boolean needRecheck;
        ...
        do {
            ...
            true)) != null) {
                ...
                for (int i = 0; i < max; i++) {
                    FieldDeclaration fieldDecl = fieldDeclarations[i];
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(TypeConstants.SYNTHETIC_ENUM_VALUES, //$NON-NLS-1$
                        ("_" + String.valueOf(index++)).toCharArray());
                        needRecheck = true;
                        break;
                    }
        ...
        do {
            ...
            true)) != null) {
                ...
                for (int i = 0; i < max; i++) {
                    FieldDeclaration fieldDecl = fieldDeclarations[i];
                    if (fieldDecl.binding == existingField) {
                        synthField.name = CharOperation.concat(fieldName, //$NON-NLS-1$
                        ("_" + String.valueOf(index++)).toCharArray());
                        needRecheck = true;
                        break;
                    }
...
*/
    public SyntheticMethodBinding addSyntheticMethod(MethodBinding targetMethod, boolean isSuperAccess) {
        ...
        if (targetMethod.declaringClass.isStatic()) {
            if ((targetMethod.isConstructor() && targetMethod.parameters.length >= 0xFE) || targetMethod.parameters.length >= 0xFF) {
                this.scope.problemReporter().tooManyParametersForSyntheticMethod(targetMethod.sourceMethod());
            }
        } else if ((targetMethod.isConstructor() && targetMethod.parameters.length >= 0xFD) || targetMethod.parameters.length >= 0xFE) {
            this.scope.problemReporter().tooManyParametersForSyntheticMethod(targetMethod.sourceMethod());
        }
        return accessMethod;
    }
 ...
 */
    ...
    private void checkAnnotationsInType() {
        ...
        for (int i = 0, length = this.memberTypes.length; i < length; i++) ((SourceTypeBinding) this.memberTypes[i]).checkAnnotationsInType();
    }
    ...
    private void internalFaultInTypeForFieldsAndMethods() {
        fields();
        methods();
        for (int i = 0, length = this.memberTypes.length; i < length; i++) ((SourceTypeBinding) this.memberTypes[i]).internalFaultInTypeForFieldsAndMethods();
    }
    ...
    public char[] genericSignature() {
        ...
        if (this.typeVariables != Binding.NO_TYPE_VARIABLES) {
            ...
            noSignature: if (this.superclass == null || !this.superclass.isParameterizedType()) {
                for (int i = 0, length = this.superInterfaces.length; i < length; i++) if (this.superInterfaces[i].isParameterizedType())
                    break noSignature;
                return null;
            }
            sig = new StringBuffer(10);
 ...
 */
    ...
    public MethodBinding[] getDefaultAbstractMethods() {
        ...
        int count = 0;
        for (int i = this.methods.length; --i >= 0; ) if (this.methods[i].isDefaultAbstract())
            count++;
        if (count == 0)
            ...
            if ((range = ReferenceBinding.binarySearch(selector, this.methods)) >= 0) {
                ...
                for (int imethod = start; imethod <= end; imethod++) {
                    ...
                    if (resolveTypesFor(method) == null || method.returnType == null) {
                        methods();
                        // try again since the problem methods have been removed
                        return getExactMethod(selector, argumentTypes, refScope);
                    }
                }
 ...
 */
        ...
        switch(otherType.kind()) {
            ...
            case Binding.PARAMETERIZED_TYPE:
                ...
                if (otherLength != length)
                    return false;
                for (int i = 0; i < length; i++) if (!this.typeVariables[i].isTypeArgumentContainedBy(otherArguments[i]))
                    return false;
                return true;
    ...
    public boolean isTaggedRepeatable() {
        // tagged but not necessarily repeatable. see isRepeatableAnnotationType.
    ...
    public MethodBinding[] methods() {
        ...
        if ((this.tagBits & TagBits.AreMethodsComplete) != 0)
            return this.methods;
        if (// https://bugs.eclipse.org/384663
        !areMethodsInitialized()) {
            this.scope.buildMethods();
        }
        // lazily sort methods
        if ((this.tagBits & TagBits.AreMethodsSorted) == 0) {
        ...
        try {
            ...
            for (int i = 0, length = this.methods.length; i < length; i++) {
                ...
                nextSibling: for (int j = i + 1; j < length; j++) {
                                ...
                                if (TypeBinding.notEquals(method.returnType.erasure(), method2.returnType.erasure())) {
                                    ...
                                    // duplicates regardless of return types
                                        ...
                                        if (index >= 0 && index < pLength) {
                                            ...
                                            for (index = pLength; --index >= 0; ) if (TypeBinding.notEquals(params1[index].erasure(), params2[index])) {
                                                // If one of them is a raw type
                    ...
                    // report duplicate
                    // do not remove if in 1.6 and just a warning given
                    boolean removeMethod2 = (severity == ProblemSeverities.Error) ? true : false;
                    if (methodDecl == null) {
                        // cannot be retrieved after binding is lost & may still be null if method is special
                        methodDecl = method.sourceMethod();
                        if (// ensure its a valid user defined method
                        methodDecl != null && methodDecl.binding != null) {
                    ...
                    if (// ensure its a valid user defined method
                    method2Decl != null && method2Decl.binding != null) {
                        if (isEnumSpecialMethod) {
                ...
                if (// forget method with invalid return type... was kept to detect possible collisions
                method.returnType == null && resolvedMethods[i] != null) {
                    methodDecl = method.sourceMethod();
                    if (methodDecl != null)
                        methodDecl.binding = null;
                    // do not alter original method array until resolution is over, due to reentrance (143259)
            ...
            if (failed > 0) {
                ...
                if (newSize == 0) {
                    ...
                    for (int i = 0, j = 0, length = resolvedMethods.length; i < length; i++) if (resolvedMethods[i] != null)
    ...
    public FieldBinding resolveTypeFor(FieldBinding field) {
        ...
        for (int f = 0; f < length; f++) {
            ...
            try {
                ...
                TypeBinding fieldType = fieldDecl.getKind() == AbstractVariableDeclaration.ENUM_CONSTANT ? // enum constant is implicitly of declaring enum type
                initializationScope.environment().convertToRawType(/*do not force conversion of enclosing types*/
                this, false) : fieldDecl.type.resolveType(initializationScope, /* check bounds*/
                true);
                ...
                if (sourceLevel >= ClassFileConstants.JDK1_8) {
                    ...
                    if (annotations != null && annotations.length != 0) {
                        ASTNode.copySE8AnnotationsToType(initializationScope, field, annotations, // type annotation is illegal on enum constant
    ...
    public MethodBinding resolveTypesFor(MethodBinding method) {
        ...
        if (sourceLevel >= ClassFileConstants.JDK1_5) {
            ...
            for (int i = 0; i < tvb.length; i++) // avoid null (see https://bugs.eclipse.org/426048)
        ...
        if (exceptionTypes != null) {
            ...
            for (int i = 0; i < size; i++) {
                resolvedExceptionType = (ReferenceBinding) exceptionTypes[i].resolveType(methodDecl.scope, /* check bounds*/
                true);
        ...
        if (methodDecl.receiver != null) {
            method.receiver = methodDecl.receiver.type.resolveType(methodDecl.scope, /* check bounds*/
            true);
        }
        ...
        if (arguments != null) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                try {
                    parameterType = arg.type.resolveType(methodDecl.scope, /* check bounds*/
                    true);
                } finally {
                    if (deferRawTypeCheck) {
                        arg.type.bits &= ~ASTNode.IgnoreRawTypeCheck;
        ...
        if (sourceLevel >= ClassFileConstants.JDK1_7) {
            ...
            } else if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=337795
            method.parameters != null && method.parameters.length > 0 && method.isVarargs()) {
        ...
        if (!method.isConstructor()) {
            ...
            if (returnType == null) {
                ...
                try {
                    methodType = returnType.resolveType(methodDecl.scope, /* check bounds*/
                    true);
                } finally {
                    if (deferRawTypeCheck) {
                        returnType.bits &= ~ASTNode.IgnoreRawTypeCheck;
        ...
        if (foundArgProblem) {
            ...
            if (typeParameters != null)
                for (int i = 0, length = typeParameters.length; i < length; i++) typeParameters[i].binding = null;
            return null;
        }
        ...
        if (compilerOptions.storeAnnotations)
            // need annotations resolved already at this point
    ...
    private static void rejectTypeAnnotatedVoidMethod(AbstractMethodDeclaration methodDecl) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (binding != null && (binding.tagBits & TagBits.AnnotationForTypeUse) != 0 && (binding.tagBits & TagBits.AnnotationForMethod) == 0) {
        ...
        if (!isPackageInfo) {
            ...
            if (pkg.defaultNullness == NO_NULL_DEFAULT && !isInDefaultPkg && !isInNullnessAnnotationPackage && !(this instanceof NestedTypeBinding)) {
                ...
                if (packageInfo == null) {
                    ...
                    if (packageInfo instanceof SourceTypeBinding && (packageInfo.tagBits & TagBits.EndHierarchyCheck) == 0) {
 ...
 */
    protected void checkRedundantNullnessDefaultRecurse(ASTNode location, Annotation[] annotations, long nullBits, boolean useNullTypeAnnotations) {
        ...
        if (this.fPackage.defaultNullness != NO_NULL_DEFAULT) {
            boolean isRedundant = useNullTypeAnnotations ? this.fPackage.defaultNullness == nullBits : (this.fPackage.defaultNullness == NONNULL_BY_DEFAULT && ((nullBits & TagBits.AnnotationNonNullByDefault) != 0));
            if (isRedundant) {
    ...
    protected boolean checkRedundantNullnessDefaultOne(ASTNode location, Annotation[] annotations, long nullBits, boolean useNullTypeAnnotations) {
        ...
        if (thisDefault != NO_NULL_DEFAULT) {
            boolean isRedundant = useNullTypeAnnotations ? thisDefault == nullBits : (nullBits & TagBits.AnnotationNonNullByDefault) != 0;
            if (isRedundant) {
                this.scope.problemReporter().nullDefaultAnnotationIsRedundant(location, annotations, this);
            }
            // different default means inner default is not redundant -> we're done
            return false;
        }
        return true;
    }
    ...
    SimpleLookupTable storedAnnotations(boolean forceInitialize) {
        ...
        if (// scope null when no annotation cached, and type got processed fully (159631)
        forceInitialize && this.storedAnnotations == null && this.scope != null) {
    ...
    public SyntheticMethodBinding[] syntheticMethods() {
        if (!isPrototype())
            throw new IllegalStateException();
        if (this.synthetics == null || this.synthetics[SourceTypeBinding.METHOD_EMUL] == null || this.synthetics[SourceTypeBinding.METHOD_EMUL].size() == 0) {
            return null;
        }
 ...
 */
    ...
    public String toString() {
        ...
        if (this.superInterfaces != null) {
            if (this.superInterfaces != Binding.NO_SUPERINTERFACES) {
                ...
                for (int i = 0, length = this.superInterfaces.length; i < length; i++) {
                    ...
                    //$NON-NLS-1$
                    buffer.append(//$NON-NLS-1$
                    (this.superInterfaces[i] != null) ? this.superInterfaces[i].debugName() : "NULL TYPE");
                }
            }
        } else {
            //$NON-NLS-1$
        ...
        if (this.fields != null) {
            if (this.fields != Binding.NO_FIELDS) {
                //$NON-NLS-1$
                buffer.append("\n/*   fields   */");
                for (int i = 0, length = this.fields.length; i < length; i++) //$NON-NLS-1$
        ...
        if (this.methods != null) {
            if (this.methods != Binding.NO_METHODS) {
                //$NON-NLS-1$
                buffer.append("\n/*   methods   */");
                for (int i = 0, length = this.methods.length; i < length; i++) //$NON-NLS-1$
                buffer.append('\n').append(//$NON-NLS-1$
                (this.methods[i] != null) ? this.methods[i].toString() : "NULL METHOD");
            }
        } else {
            //$NON-NLS-1$
        ...
        if (this.memberTypes != null) {
            if (this.memberTypes != Binding.NO_MEMBER_TYPES) {
                //$NON-NLS-1$
                buffer.append("\n/*   members   */");
                for (int i = 0, length = this.memberTypes.length; i < length; i++) //$NON-NLS-1$
                buffer.append('\n').append(//$NON-NLS-1$
                (this.memberTypes[i] != null) ? this.memberTypes[i].toString() : "NULL TYPE");
            }
        } else {
            //$NON-NLS-1$
    ...
    void verifyMethods(MethodVerifier verifier) {
        ...
        for (int i = this.memberTypes.length; --i >= 0; ) ((SourceTypeBinding) this.memberTypes[i]).verifyMethods(verifier);
    }
    ...
    public void tagIndirectlyAccessibleMembers() {
        ...
        if (this.superclass.isPrivate())
            if (// should always be true because private super type can only be accessed in same CU
            this.superclass instanceof SourceTypeBinding)
