...
public class ClassLiteralAccess extends Expression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        sourceType.isBaseType())) && currentScope.environment().options.sourceLevel <= ClassFileConstants.JDK1_5) {
    ...
    public StringBuffer printExpression(int indent, StringBuffer output) {
        //$NON-NLS-1$
        return type.print(0, output).append(".class");
    }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (classType.isGenericType()) {
            ...
            this.resolvedType = scope.createParameterizedType(classType, new TypeBinding[] { scope.boxing(targetType) }, /*not a member*/
            null);
        } else {
