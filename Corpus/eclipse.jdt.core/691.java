...
public class ClassFileMatchLocator implements IIndexConstants {
    ...
    private static final long TARGET_ANNOTATION_BITS = TagBits.AnnotationForType | TagBits.AnnotationForParameter | TagBits.AnnotationForPackage | TagBits.AnnotationForMethod | TagBits.AnnotationForLocalVariable | TagBits.AnnotationForField | TagBits.AnnotationForConstructor | TagBits.AnnotationForAnnotationType;
    ...
    private boolean checkParameters(char[] methodDescriptor, char[][] parameterSimpleNames, char[][] parameterQualifications, boolean isCaseSensitive, boolean isCamelCase) {
        ...
        if (parameterCount != arguments.length)
            return false;
        for (int i = 0; i < parameterCount; i++) if (!checkTypeName(parameterSimpleNames[i], parameterQualifications[i], Signature.toCharArray(arguments[i]), isCaseSensitive, isCamelCase))
            return false;
        return true;
    }
    ...
    private boolean checkStandardAnnotations(long annotationTagBits, TypeReferencePattern pattern) {
        ...
        if ((annotationTagBits & TagBits.AnnotationTargetMASK) != 0) {
            ...
            if (checkAnnotationTypeReference(CharOperation.concatWith(compoundName, '.'), pattern) || ((annotationTagBits & TARGET_ANNOTATION_BITS) != 0 && checkAnnotationTypeReference(JAVA_LANG_ANNOTATION_ELEMENTTYPE, pattern))) {
                return true;
            }
        }
        if ((annotationTagBits & TagBits.AnnotationRetentionMASK) != 0) {
            char[][] compoundName = TypeConstants.JAVA_LANG_ANNOTATION_RETENTION;
            if (checkAnnotationTypeReference(CharOperation.concatWith(compoundName, '.'), pattern) || checkAnnotationTypeReference(CharOperation.concatWith(TypeConstants.JAVA_LANG_ANNOTATION_RETENTIONPOLICY, '.'), pattern)) {
                return true;
            }
        }
 ...
 */
    public void locateMatches(MatchLocator locator, ClassFile classFile, IBinaryType info) throws CoreException {
            ...
            if (binding != null) {
                ...
                for (int i = 0; i < aMethodsLength; i++) {
                    ...
                    if (level != PatternLocator.IMPOSSIBLE_MATCH) {
                        IMethod methodHandle = binaryType.getMethod(new String(method.isConstructor() ? binding.compoundName[binding.compoundName.length - 1] : method.selector), CharOperation.toStrings(Signature.getParameterTypes(convertClassFileFormat(methodSignature))));
                    ...
                    if (hasUnresolvedMethods) {
                        if (// Store binary method signatures to avoid multiple computation
                        binaryMethodSignatures == null) {
        ...
        for (int i = 0; i < bMethodsLength; i++) {
            ...
            // impossible match or already reported as accurate
            if (method == null)
                continue;
        ...
        if (methods != null) {
            for (int i = 0, max = methods.length; i < max; i++) {
                ...
                if (checkAnnotations(typeReferencePattern, method.getAnnotations(), method.getTagBits())) {
                    ...
                    IMethod methodHandle = classFileBinaryType.getMethod(new String(method.isConstructor() ? binaryTypeBinding.compoundName[binaryTypeBinding.compoundName.length - 1] : method.getSelector()), CharOperation.toStrings(Signature.getParameterTypes(convertClassFileFormat(method.getMethodDescriptor()))));
 ...
 */
    boolean matchBinary(SearchPattern pattern, Object binaryInfo, IBinaryType enclosingBinaryType) {
        switch(pattern.kind) {
            ...
            case OR_PATTERN:
                SearchPattern[] patterns = ((OrPattern) pattern).patterns;
                for (int i = 0, length = patterns.length; i < length; i++) if (matchBinary(patterns[i], binaryInfo, enclosingBinaryType))
                    return true;
        }
        return false;
    }
    ...
    boolean matchTypeDeclaration(TypeDeclarationPattern pattern, Object binaryInfo, IBinaryType enclosingBinaryType) {
        ...
        if (pattern.enclosingTypeNames == null || qualifiedPattern) {
            ...
            if (!checkTypeName(simpleName, pkg, fullyQualifiedTypeName, pattern.isCaseSensitive(), pattern.isCamelCase()))
                return false;
        } else {
            char[] enclosingTypeName = CharOperation.concatWith(pattern.enclosingTypeNames, '.');
            char[] patternString = pattern.pkg == null ? enclosingTypeName : CharOperation.concat(pattern.pkg, enclosingTypeName, '.');
