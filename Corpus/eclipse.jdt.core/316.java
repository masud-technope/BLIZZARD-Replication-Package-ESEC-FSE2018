...
public class ArgsConstructorProcessor extends BaseProcessor {
    ...
    private void processArgsConstructorClasses(RoundEnvironment env, TypeElement type) {
        for (Element element : env.getElementsAnnotatedWith(type)) {
            ...
            processingEnv.getMessager().printMessage(Kind.NOTE, "Class " + element + " is processed");
        }
    }
    ...
    private void processClass(Element element) {
        ...
        if (action == null) {
            processingEnv.getMessager().printMessage(Kind.WARNING, "Class " + element + " lacks a annotation with required args", element);
            return;
        }
        ...
        if (!doesClassContainArgsConstructor(element, mirrors)) {
            ...
            processingEnv.getMessager().printMessage(Kind.ERROR, "Class " + element + " lacks a public constructor with args: " + s, element);
        } else {
            processingEnv.getMessager().printMessage(Kind.NOTE, "Processed type: " + element);
        }
    }
    ...
    private final TypeVisitor<Boolean, List<TypeMirror>> argsVisitor = new SimpleTypeVisitor6<Boolean, List<TypeMirror>>() {
        ...
        public Boolean visitExecutable(ExecutableType t, List<TypeMirror> annotatedTypes) {
            ...
            for (int i = 0; i < types.size(); i++) {
                // because same
                TypeMirror test = tutil.erasure(types.get(i));
                // type bad
                // Map<String,String>
                // != Map
