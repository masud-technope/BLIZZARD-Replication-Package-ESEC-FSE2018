 ...
 */
    protected boolean isVirtualInvoke(MethodBinding method, MessageSend messageSend) {
        return !method.isStatic() && !method.isPrivate() && !messageSend.isSuperAccess() && !(method.isDefault() && this.pattern.focus != null && !CharOperation.equals(this.pattern.declaringPackageName, method.declaringClass.qualifiedPackageName()));
    }

    protected ReferenceBinding checkMethodRef(MethodBinding method, ReferenceExpression referenceExpression) {
        boolean result = (!method.isStatic() && !method.isPrivate() && referenceExpression.isMethodReference() && !(method.isDefault() && this.pattern.focus != null && !CharOperation.equals(this.pattern.declaringPackageName, method.declaringClass.qualifiedPackageName())));
        if (result) {
        ...
        if (parameterCount > -1) {
            ...
            for (int i = 0; i < parameterCount; i++) {
                ...
                if (argType.isMemberType() || this.pattern.parameterQualifications[i] != null) {
                    ...
                    if (// textual comparison insufficient
                    focusMethodBinding != null) {
                        TypeBinding[] parameters = focusMethodBinding.parameters;
                        if (parameters.length >= parameterCount) {
                            newLevel = (isBinary ? argType.erasure().isEquivalentTo((parameters[i].erasure())) : argType.isEquivalentTo((parameters[i]))) ? ACCURATE_MATCH : IMPOSSIBLE_MATCH;
                            foundLevel = true;
                        }
                    }
                } else {
 ...
 */
    protected void matchReportReference(ASTNode reference, IJavaElement element, IJavaElement localElement, IJavaElement[] otherElements, Binding elementBinding, int accuracy, MatchLocator locator) throws CoreException {
        ...
        if (this.isDeclarationOfReferencedMethodsPattern) {
            ...
            while (element != null && !declPattern.enclosingElement.equals(element)) element = element.getParent();
            if (element != null) {
                reportDeclaration(methodBinding, locator, declPattern.knownMethods);
            }
        } else {
            MethodReferenceMatch methodReferenceMatch = locator.newMethodReferenceMatch(element, elementBinding, accuracy, -1, -1, /*not constructor*/
            false, /*not synthetic*/
            false, reference);
    ...
    void matchReportReference(MessageSend messageSend, MatchLocator locator, int accuracy, MethodBinding methodBinding) throws CoreException {
        ...
        if (// parameterized generic method
        methodBinding instanceof ParameterizedGenericMethodBinding) {
        ...
        } else if (// binding has no type params, compatible erasure if pattern does
 ...
 */
    ...
    public int resolveLevel(Binding binding) {
        ...
        int declaringLevel = subType ? resolveLevelAsSubtype(this.pattern.declaringSimpleName, this.pattern.declaringQualification, method.declaringClass, method.selector, null, method.declaringClass.qualifiedPackageName(), method.isDefault()) : resolveLevelForType(this.pattern.declaringSimpleName, this.pattern.declaringQualification, method.declaringClass);
        // return the weaker match
        return (methodLevel & MATCH_LEVEL_MASK) > (declaringLevel & MATCH_LEVEL_MASK) ? declaringLevel : methodLevel;
    }
            ...
            if (declaringLevel == IMPOSSIBLE_MATCH) {
                if (method.declaringClass == null || this.allSuperDeclaringTypeNames == null) {
                    ...
                    if (superTypeNames != null && resolveLevelAsSuperInvocation(methodReceiverType, method.parameters, superTypeNames, true)) {
                        ...
                        declaringLevel = methodLevel | // this is an overridden method => add flavor to returned level
            ...
            if (declaringLevel == IMPOSSIBLE_MATCH) {
                if (method.declaringClass == null || this.allSuperDeclaringTypeNames == null) {
                    ...
                    if (superTypeNames != null && resolveLevelAsSuperInvocation(ref, method.parameters, superTypeNames, true)) {
                        // since this is an ACCURATE_MATCH so return the possibly weaker match
                        declaringLevel = methodLevel | // TODO: not an invocation really but ref -> add flavor to returned level
                        SUPER_INVOCATION_FLAVOR;
                    }
                }
            }
 ...
 */
    protected int resolveLevelAsSubtype(char[] simplePattern, char[] qualifiedPattern, ReferenceBinding type, char[] methodName, TypeBinding[] argumentTypes, char[] packageName, boolean isDefault) {
        ...
        if (level != IMPOSSIBLE_MATCH) {
            ...
            if (// if concrete, then method is overridden
            ((method != null && !method.isAbstract()) || !type.isAbstract()) && !type.isInterface()) {
        ...
        if (!type.isInterface() && !CharOperation.equals(type.compoundName, TypeConstants.JAVA_LANG_OBJECT)) {
            ...
            if (level != IMPOSSIBLE_MATCH) {
                if (argumentTypes != null) {
                    ...
                    if (// one method match in hierarchy
                    method != null) {
                ...
                return level | SUB_INVOCATION_FLAVOR;
            }
        }
        // matches interfaces
        ReferenceBinding[] interfaces = type.superInterfaces();
        ...
        for (int i = 0; i < interfaces.length; i++) {
            ...
            if (level != IMPOSSIBLE_MATCH) {
                if (// if concrete class, then method is overridden
