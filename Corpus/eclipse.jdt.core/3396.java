 ...
 */
        ...
        if ((flowInfo.tagBits & FlowInfo.UNREACHABLE_OR_DEAD) == 0) {
            ...
            if (codegenBinding != this.binding) {
                ...
                if (codegenBinding.returnType.isTypeVariable()) {
                    ...
                    if (// no need for extra cast if same as first bound anyway
                    TypeBinding.notEquals(variableReturnType.firstBound, this.binding.returnType)) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        if (this.constant != Constant.NotAConstant) {
            ...
            if (this.typeArguments != null) {
                ...
                for (int i = 0; i < length; i++) {
                    if ((this.genericTypeArguments[i] = this.typeArguments[i].resolveType(/* check bounds*/
                    scope, true)) == null) {
        ...
        if (!this.binding.isValidBinding()) {
            if (this.binding instanceof ProblemMethodBinding && ((ProblemMethodBinding) this.binding).problemId() == ProblemReasons.NotVisible) {
                ...
                MethodBinding privateBinding = this.receiver instanceof CodeSnippetThisReference && ((CodeSnippetThisReference) this.receiver).isImplicit ? localScope.getImplicitMethod((ReferenceBinding) this.delegateThis.type, this.selector, this.argumentTypes, this) : localScope.getMethod(this.delegateThis.type, this.selector, this.argumentTypes, this);
        ...
        if (this.actualReceiverType.isArrayType() && this.binding.parameters == Binding.NO_PARAMETERS && scope.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_5 && CharOperation.equals(this.binding.selector, CLONE)) {
