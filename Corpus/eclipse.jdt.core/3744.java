...
public class MethodBinding extends Binding {
    ...
    // JSR308 - explicit this parameter
...
*/
    public final boolean areParameterErasuresEqual(MethodBinding method) {
        ...
        if (length != args.length)
            return false;
        for (int i = 0; i < length; i++) if (TypeBinding.notEquals(this.parameters[i], args[i]) && TypeBinding.notEquals(this.parameters[i].erasure(), args[i].erasure()))
            return false;
        return true;
    }
 ...
 */
    public final boolean areParametersCompatibleWith(TypeBinding[] arguments) {
        ...
        for (int i = 0; i < lastIndex; i++) if (TypeBinding.notEquals(this.parameters[i], arguments[i]) && !arguments[i].isCompatibleWith(this.parameters[i]))
            return false;
        return true;
    }
...
*/
    public final boolean areParametersEqual(MethodBinding method) {
        ...
        if (length != args.length)
            return false;
        for (int i = 0; i < length; i++) if (TypeBinding.notEquals(this.parameters[i], args[i]))
            return false;
        return true;
    }
...
*/
    public final boolean areTypeVariableErasuresEqual(MethodBinding method) {
        ...
        if (length != vars.length)
            return false;
        for (int i = 0; i < length; i++) if (TypeBinding.notEquals(this.typeVariables[i], vars[i]) && TypeBinding.notEquals(this.typeVariables[i].erasure(), vars[i].erasure()))
            return false;
        return true;
    }
    ...
    public MethodBinding asRawMethod(LookupEnvironment env) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (var.boundsCount() <= 1) {
                arguments[i] = env.convertToRawType(var.upperBound(), /*do not force conversion of enclosing types*/
                false);
            } else {
                ...
                if (var.boundsCount() == superLength) {
                    ...
                    for (int s = 1; s < superLength; s++) rawOtherBounds[s - 1] = env.convertToRawType(itsSuperinterfaces[s], false);
                } else {
                    rawFirstBound = env.convertToRawType(var.superclass(), false);
                    rawOtherBounds = new TypeBinding[superLength];
                    for (int s = 0; s < superLength; s++) rawOtherBounds[s] = env.convertToRawType(itsSuperinterfaces[s], false);
                }
...
*/
    public final boolean canBeSeenBy(TypeBinding receiverType, InvocationSite invocationSite, Scope scope) {
        ...
        do {
            if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=285002
    ...
    MethodBinding computeSubstitutedMethod(MethodBinding method, LookupEnvironment env) {
        ...
        for (int i = 0; i < length; i++) if (!this.typeVariables[i].isInterchangeableWith(vars[i], substitute))
            return null;
        return substitute;
    }
 ...
 */
    protected void fillInDefaultNonNullness(AbstractMethodDeclaration sourceMethod) {
        ...
        if (this.returnType != null && !this.returnType.isBaseType() && (this.tagBits & TagBits.AnnotationNullMASK) == 0) {
            ...
            sourceMethod.scope.problemReporter().nullAnnotationIsRedundant(sourceMethod, /*signifies method return*/
            -1);
        }
    }
    ...
    protected void fillInDefaultNonNullness18(AbstractMethodDeclaration sourceMethod, LookupEnvironment env) {
        if (hasNonNullDefaultFor(DefaultLocationParameter, true)) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                } else if (sourceMethod != null && (parameter.tagBits & TagBits.AnnotationNonNull) != 0 && sourceMethod.arguments[i].hasNullTypeAnnotation(AnnotationPosition.MAIN_TYPE)) {
                    sourceMethod.scope.problemReporter().nullAnnotationIsRedundant(sourceMethod, i);
                }
            }
            if (added)
        ...
        if (this.returnType != null && hasNonNullDefaultFor(DefaultLocationReturnType, true) && this.returnType.acceptsNonNullDefault()) {
            ...
            } else if (sourceMethod instanceof MethodDeclaration && (this.returnType.tagBits & TagBits.AnnotationNonNull) != 0 && ((MethodDeclaration) sourceMethod).hasNullTypeAnnotation(AnnotationPosition.MAIN_TYPE)) {
                sourceMethod.scope.problemReporter().nullAnnotationIsRedundant(sourceMethod, /*signifies method return*/
                -1);
            }
        }
    ...
    public MethodBinding findOriginalInheritedMethod(MethodBinding inheritedMethod) {
        ...
        if (TypeBinding.notEquals(inheritedOriginal.declaringClass, superType)) {
            ...
            for (int m = 0, l = superMethods.length; m < l; m++) if (superMethods[m].original() == inheritedOriginal)
        ...
        if ((originalMethod.tagBits & TagBits.AnnotationResolved) == 0 && originalMethod.declaringClass instanceof SourceTypeBinding) {
            ...
            if (scope != null) {
                ...
                if (options.isAnnotationBasedNullAnalysisEnabled) {
                    ...
                    long nullDefaultBits = usesNullTypeAnnotations ? this.defaultNullness : this.tagBits & (TagBits.AnnotationNonNullByDefault | TagBits.AnnotationNullUnspecifiedByDefault);
 ...
 */
    ...
    public TypeVariableBinding getTypeVariable(char[] variableName) {
        for (int i = this.typeVariables.length; --i >= 0; ) if (CharOperation.equals(this.typeVariables[i].sourceName, variableName))
 ...
 */
    public final boolean isMain() {
        if (this.selector.length == 4 && CharOperation.equals(this.selector, TypeConstants.MAIN) && ((this.modifiers & (ClassFileConstants.AccPublic | ClassFileConstants.AccStatic)) != 0) && TypeBinding.VOID == this.returnType && this.parameters.length == 1) {
...
*/
    public final char[] signature() /* (ILjava/lang/Thread;)Ljava/lang/Object; */
    {
        ...
        isConstructor && this.declaringClass.isEnum()) {
 ...
 */
    ...
    public AbstractMethodDeclaration sourceMethod() {
        ...
        if (methods != null) {
            for (int i = methods.length; --i >= 0; ) if (this == methods[i].binding)
                return methods[i];
        }
        return null;
    }
        ...
        if (this.parameters != null) {
            if (this.parameters != Binding.NO_PARAMETERS) {
                for (int i = 0, length = this.parameters.length; i < length; i++) {
                    ...
                    //$NON-NLS-1$
                    output.append(//$NON-NLS-1$
                    this.parameters[i] != null ? this.parameters[i].debugName() : "<no argument type>");
                }
            }
        } else {
            //$NON-NLS-1$
        ...
        if (this.thrownExceptions != null) {
            if (this.thrownExceptions != Binding.NO_EXCEPTIONS) {
                ...
                for (int i = 0, length = this.thrownExceptions.length; i < length; i++) {
                    ...
                    //$NON-NLS-1$
                    output.append(//$NON-NLS-1$
                    (this.thrownExceptions[i] != null) ? this.thrownExceptions[i].debugName() : "<no exception type>");
                }
            }
        } else {
            //$NON-NLS-1$
