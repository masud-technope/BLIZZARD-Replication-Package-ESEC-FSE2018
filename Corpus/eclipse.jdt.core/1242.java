...
class ConstraintTypeFormula extends ConstraintFormula {
    ...
    public Object reduce(InferenceContext18 inferenceContext) {
        switch(this.relation) {
            ...
            case TYPE_ARGUMENT_CONTAINED:
                // 18.2.3:
                if (// "If T is a type" ... all alternatives require "wildcard"
                this.right.kind() != Binding.WILDCARD_TYPE) {
    ...
    private Object reduceTypeEquality(TypeBinding object) {
        ...
        if (this.left.kind() == Binding.WILDCARD_TYPE) {
            if (this.right.kind() == Binding.WILDCARD_TYPE) {
                ...
                if ((leftWC.boundKind == Wildcard.EXTENDS && rightWC.boundKind == Wildcard.EXTENDS) || (leftWC.boundKind == Wildcard.SUPER && rightWC.boundKind == Wildcard.SUPER)) {
    ...
    boolean addConstraintsFromTypeParameters(TypeBinding subCandidate, ParameterizedTypeBinding ca, List<ConstraintFormula> constraints) {
        ...
        if (ai == null)
            // no arguments here means nothing to check
            return true;
        // C<B1,B2,...>
        TypeBinding cb = subCandidate.findSuperTypeOriginatingFrom(ca);
        if (cb == null)
            // nothing here means we failed 
            return false;
        if (// incl C#RAW vs C#RAW
        TypeBinding.equalsEquals(ca, cb))
            return true;
        ...
        if (cb.isRawType() || bi == null || bi.length == 0)
            // FALSE would conform to the spec 
            return (this.isSoft && InferenceContext18.SIMULATE_BUG_JDK_8026527) ? true : false;
        for (int i = 0; i < ai.length; i++) constraints.add(ConstraintTypeFormula.create(bi[i], ai[i], TYPE_ARGUMENT_CONTAINED, this.isSoft));
        return true;
    }
