...
public class IrritantSet {
    ...
    public static final int GROUP_MASK = ASTNode.Bit32 | ASTNode.Bit31 | ASTNode.Bit30;

    public static final int GROUP_SHIFT = 29;

    // can be increased up to 8
    public static final int GROUP_MAX = 3;

    // Group prefix for irritants
    public static final int GROUP0 = 0 << GROUP_SHIFT;

    public static final int GROUP1 = 1 << GROUP_SHIFT;

    public static final int GROUP2 = 2 << GROUP_SHIFT;
    ...
    static {
        ...
        UNUSED.set(CompilerOptions.UnusedArgument).set(CompilerOptions.UnusedExceptionParameter).set(CompilerOptions.UnusedPrivateMember).set(CompilerOptions.UnusedDeclaredThrownException).set(CompilerOptions.UnusedLabel).set(CompilerOptions.UnusedImport).set(CompilerOptions.UnusedTypeArguments).set(CompilerOptions.RedundantSuperinterface).set(CompilerOptions.DeadCode).set(CompilerOptions.UnusedObjectAllocation).set(CompilerOptions.UnusedTypeParameter).set(CompilerOptions.RedundantSpecificationOfTypeArguments);
        ...
        if (//$NON-NLS-1$
        suppressRawWhenUnchecked != null && "true".equalsIgnoreCase(suppressRawWhenUnchecked)) {
    ...
    public void initialize(int singleGroupIrritants) {
        if (singleGroupIrritants == 0)
            return;
        int group = (singleGroupIrritants & GROUP_MASK) >> GROUP_SHIFT;
        // erase group information
        this.bits[group] = singleGroupIrritants & ~GROUP_MASK;
    }
    ...
    public IrritantSet set(IrritantSet other) {
        ...
        for (int i = 0; i < GROUP_MAX; i++) {
            // erase the
            int otherIrritant = other.bits[i] & ~GROUP_MASK;
            // bits
    ...
    public IrritantSet setAll() {
        for (int i = 0; i < GROUP_MAX; i++) {
            // erase the group
            this.bits[i] |= 0xFFFFFFFF & ~GROUP_MASK;
        // bits;
        }
        return this;
    }
