...
public class UnresolvedAnnotationBinding extends AnnotationBinding {
    ...
    public void resolve() {
        // in place resolution.
        if (// the type is resolved when requested
        this.typeUnresolved) {
            boolean wasToleratingMissingTypeProcessingAnnotations = this.env.mayTolerateMissingType;
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=388042
            this.env.mayTolerateMissingType = true;
            try {
                this.type = (ReferenceBinding) BinaryTypeBinding.resolveType(this.type, this.env, /* no raw conversion for now */
                false);
