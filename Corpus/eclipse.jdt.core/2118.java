 ...
 */
    public  TypeHierarchy(IType type, ICompilationUnit[] workingCopies, IJavaSearchScope scope, boolean computeSubtypes) {
        // unsure the focus type is unresolved (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=92357)
        this.focusType = type == null ? null : (IType) ((JavaElement) type).unresolved();
 ...
 */
    public void fireChange() {
        ...
        ArrayList<ITypeHierarchyChangedListener> listeners = getClonedChangeListeners();
        if (listeners == null) {
            return;
        }
        if (DEBUG) {
 ...
 */
    ...
    private ArrayList<IType> getAllSuperInterfaces0(IType type, ArrayList<IType> supers) {
        ...
        if (// type is not part of the hierarchy
            ...
            case IJavaElementDelta.ADDED:
                try {
                    ...
                    if (this.focusType != null) {
                        ...
                        for (int i = 0; i < classpath.length; i++) {
                            if (classpath[i].getEntryKind() == IClasspathEntry.CPE_PROJECT && classpath[i].getPath().equals(hierarchyProject)) {
                                return true;
                            }
                        }
                    }
 ...
 */
    protected boolean isAffectedByOpenable(IJavaElementDelta delta, IJavaElement element, int eventType) {
        if (element instanceof CompilationUnit) {
            ...
            ICompilationUnit focusCU = this.focusType != null ? this.focusType.getCompilationUnit() : null;
            if (focusCU != null && focusCU.getOwner() != cu.getOwner())
                return false;
            //ADDED delta arising from getWorkingCopy() should be ignored
            if (eventType != ElementChangedEvent.POST_RECONCILE && !cu.isPrimary() && delta.getKind() == IJavaElementDelta.ADDED)
                return false;
            ChangeCollector collector = this.changeCollector;
            if (collector == null) {
        ...
        try {
            ...
            if (DEBUG) {
                ...
                if (this.focusType != null) {
                    ...
                    "  on type " + ((JavaElement) this.focusType).toStringWithAncestors());
                }
            }
            compute();
            initializeRegions();
