...
public class BinaryIndexer extends AbstractIndexer implements SuffixConstants {
    ...
    public void indexDocument() {
        try {
            ...
            if (methods != null) {
                for (int i = 0, max = methods.length; i < max; i++) {
                    ...
                    if (isConstructor) {
                        ...
                        addConstructorDeclaration(name, parameterTypes == null ? 0 : parameterTypes.length, signature, parameterTypes, method.getArgumentNames(), method.getModifiers(), packageName, modifiers, exceptionTypes, extraFlags);
                    } else {
                        if (!method.isClinit()) {
                            ...
                            if (name.length > 0) {
                                addMethodDeclaration(name, null, selector, parameterTypes == null ? 0 : parameterTypes.length, signature, parameterTypes, method.getArgumentNames(), returnType, method.getModifiers(), packageName, modifiers, exceptionTypes, extraFlags);
                            }
                        }
                    }
            ...
            Util.log(IStatus.WARNING, "The Java indexing could not index " + this.document.getPath() + ". This .class file doesn't follow the class file format specification. Please report this issue against the .class file vendor");
