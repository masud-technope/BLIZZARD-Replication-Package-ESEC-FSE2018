...
class MethodVerifier15 extends MethodVerifier {
    ...
    protected boolean canOverridingMethodDifferInErasure(MethodBinding overridingMethod, MethodBinding inheritedMethod) {
        if (overridingMethod.areParameterErasuresEqual(inheritedMethod))
            // no further change in signature is possible due to parameterization.
            return false;
        if (overridingMethod.declaringClass.isRawType())
            // no parameterization is happening anyways.
            return false;
        return true;
    }
    ...
    boolean canSkipInheritedMethods(MethodBinding one, MethodBinding two) {
        return // already know one is not null
        two == null || (TypeBinding.equalsEquals(one.declaringClass, two.declaringClass) && !one.declaringClass.isParameterizedType());
    }
    ...
    void checkConcreteInheritedMethod(MethodBinding concreteMethod, MethodBinding[] abstractMethods) {
        ...
        if (// is currentMethod from the current type?
        analyseNullAnnotations && this.type.equals(concreteMethod.declaringClass))
        ...
        for (int i = 0, l = abstractMethods.length; i < l; i++) {
            ...
            if (originalInherited.declaringClass.isInterface()) {
                if ((TypeBinding.equalsEquals(concreteMethod.declaringClass, this.type.superclass) && this.type.superclass.isParameterizedType() && !areMethodsCompatible(concreteMethod, originalInherited)) || this.type.superclass.erasure().findSuperTypeOriginatingFrom(originalInherited.declaringClass) == null)
    ...
    void checkForNameClash(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        if (inheritedMethod.isStatic() || currentMethod.isStatic()) {
            ...
            // no chance of bridge method's clashing
            return;
        }
        if (// check up the hierarchy for skipped inherited methods
        !detectNameClash(currentMethod, inheritedMethod, false)) {
            ...
            if (length != inheritedParams.length)
                return;
            for (int i = 0; i < length; i++) if (TypeBinding.notEquals(currentParams[i], inheritedParams[i]))
    ...
    void checkInheritedMethods(MethodBinding inheritedMethod, MethodBinding otherInheritedMethod) {
        ...
        if (this.environment.globalOptions.complianceLevel < ClassFileConstants.JDK1_7 && inheritedMethod.declaringClass.isInterface())
            // JDK7 checks for name clashes in interface inheritance, while JDK6 and below don't. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=354229
            return;
    ...
    void checkInheritedMethods(MethodBinding[] methods, int length, boolean[] isOverridden, boolean[] isInherited) {
        ...
        // invariant: playingTrump => (concreteMethod == null)
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (isInherited[i] && !methods[i].isAbstract()) {
                ...
                if (methods[i].isDefaultMethod() && // condition (a)
                abstractSuperClassMethod != null && // condition (b)...
                areParametersEqual(abstractSuperClassMethod, methods[i]) && concreteMethod == null) {
                    ...
                    if (concreteMethod != null) {
                        ...
                        problemReporter().duplicateInheritedMethods(this.type, concreteMethod, methods[i], this.environment.globalOptions.sourceLevel >= ClassFileConstants.JDK1_8);
    ...
    void checkAgainstInheritedMethods(MethodBinding currentMethod, MethodBinding[] methods, int length, MethodBinding[] allInheritedMethods) {
        ...
        if (options.isAnnotationBasedNullAnalysisEnabled && (currentMethod.tagBits & TagBits.IsNullnessKnown) == 0) {
            ...
            AbstractMethodDeclaration srcMethod = null;
            if (// is currentMethod from the current type?
            this.type.equals(currentMethod.declaringClass))
            ...
            for (int i = length; --i >= 0; ) if (!currentMethod.isStatic() && !methods[i].isStatic())
    ...
    void checkNullSpecInheritance(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod, boolean hasReturnNonNullDefault, boolean hasParameterNonNullDefault, boolean complain, MethodBinding inheritedMethod, MethodBinding[] allInherited, Scope scope, InheritedNonNullnessInfo[] inheritedNonNullnessInfos) {
        ...
        if (TypeBinding.notEquals(currentMethod.declaringClass, this.type) && (currentMethod.tagBits & TagBits.IsNullnessKnown) == 0) {
    ...
    void reportRawReferences() {
        ...
        if (// shouldn't whine at all
        compilerOptions.sourceLevel < ClassFileConstants.JDK1_5 || // must have already whined 
            ...
            for (int i = 0, length = current.length; i < length; i++) {
                ...
                if ((currentMethod.modifiers & (ExtraCompilerModifiers.AccImplementing | ExtraCompilerModifiers.AccOverriding)) == 0) {
                    ...
                    for (int j = 0, size = currentMethod.parameters.length; j < size; j++) {
                        ...
                        if (parameterType.leafComponentType().isRawType() && compilerOptions.getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (arg.type.bits & ASTNode.IgnoreRawTypeCheck) == 0) {
                    ...
                    if (!methodDecl.isConstructor() && methodDecl instanceof MethodDeclaration) {
                        ...
                        if (returnType != null) {
                            if (methodType.leafComponentType().isRawType() && compilerOptions.getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (returnType.bits & ASTNode.IgnoreRawTypeCheck) == 0) {
        ...
        for (int j = 0, size = currentMethod.parameters.length; j < size; j++) {
            ...
            if (parameterType.leafComponentType().isRawType()) {
                if (inheritedParameterType.leafComponentType().isRawType()) {
                    ...
                    if (compilerOptions.getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore && (arg.type.bits & ASTNode.IgnoreRawTypeCheck) == 0) {
                        methodDecl.scope.problemReporter().rawTypeReference(arg.type, parameterType);
                    }
                }
            }
        ...
        if (!methodDecl.isConstructor() && methodDecl instanceof MethodDeclaration && (returnType = ((MethodDeclaration) methodDecl).returnType) != null) {
            ...
            if (methodType.leafComponentType().isRawType()) {
                ...
                // 
                    ...
                    if ((returnType.bits & ASTNode.IgnoreRawTypeCheck) == 0 && compilerOptions.getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore) {
                        methodDecl.scope.problemReporter().rawTypeReference(returnType, methodType);
                    }
                }
            }
    ...
    void checkMethods() {
        ...
        nextSelector: for (int s = methodSelectors.length; --s >= 0; ) {
            ...
            if (current == null && skipInheritedMethods)
                continue nextSelector;
            if (// handle the common case
            inherited.length == 1 && current == null) {
            ...
            // null is no match, otherwise value is matching currentMethod
            ...
            for (int i = 0; i < inheritedLength; i++) {
                ...
                if (// current == null case handled already.
                matchMethod == null && current != null && this.type.isPublic()) {
                    MethodBinding inheritedMethod = inherited[i];
                    if (inheritedMethod.isPublic() && (!inheritedMethod.declaringClass.isInterface() && !inheritedMethod.declaringClass.isPublic())) {
                        this.type.addSyntheticBridgeMethod(inheritedMethod.original());
                    }
            ...
            for (int i = 0; i < inheritedLength; i++) {
                ...
                if (index > 0) {
                    ...
                    if (// transfer inherited & overridden status to align with subset of methods.
 ...
 */
    MethodBinding findReplacedMethod(MethodBinding specific, MethodBinding general) {
        ...
        if (generalSubstitute != null && // if (abstract(specific) => abstract(general)) check if 'specific' overrides 'general' 
        (!specific.isAbstract() || general.isAbstract() || (general.isDefaultMethod() && specific.declaringClass.isClass())) && isSubstituteParameterSubsignature(specific, generalSubstitute)) {
        ...
        nextSelector: for (int s = methodSelectors.length; --s >= 0; ) {
            ...
            for (int i = 0, length = inherited.length; i < length; i++) {
                ...
                if (index > 0) {
                    ...
                    if (// All inherited methods do NOT have the same vmSignature
                    count > 0) {
    ...
    boolean detectNameClash(MethodBinding current, MethodBinding inherited, boolean treatAsSynthetic) {
        ...
        if (!treatAsSynthetic) {
            ...
            if (// we know it ought to at least one and that current is NOT the override
            currentNamesakes.length > 1) {
        ...
        if (!current.areParameterErasuresEqual(original))
            return false;
        // For error reporting use, inherited.original()
    ...
    boolean isAcceptableReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        if (inheritedMethod.declaringClass.isRawType())
            // since the inheritedMethod comes from a raw type, the return type is always acceptable
            return true;
        MethodBinding originalInherited = inheritedMethod.original();
        ...
        if (originalInheritedReturnType.isParameterizedTypeWithActualArguments())
            // raw types issue a warning if inherited is parameterized
    ...
    boolean isInterfaceMethodImplemented(MethodBinding inheritedMethod, MethodBinding existingMethod, ReferenceBinding superType) {
        ...
        if (inheritedMethod == null || !doesMethodOverride(existingMethod, inheritedMethod))
            return false;
        return TypeBinding.equalsEquals(inheritedMethod.returnType, existingMethod.returnType) || (// ... not if inheriting the bridge situation from a superclass
        TypeBinding.notEquals(this.type, existingMethod.declaringClass) && !existingMethod.declaringClass.isInterface() && // may have to report incompatible return type
    ...
    boolean isUnsafeReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        ...
        if (TypeBinding.equalsEquals(currentMethod.returnType, inheritedMethod.returnType.erasure())) {
            TypeBinding[] currentParams = currentMethod.parameters;
            TypeBinding[] inheritedParams = inheritedMethod.parameters;
            for (int i = 0, l = currentParams.length; i < l; i++) if (!areTypesEqual(currentParams[i], inheritedParams[i]))
                return true;
        }
        if (currentMethod.typeVariables == Binding.NO_TYPE_VARIABLES && inheritedMethod.original().typeVariables != Binding.NO_TYPE_VARIABLES && currentMethod.returnType.erasure().findSuperTypeOriginatingFrom(inheritedMethod.returnType.erasure()) != null) {
            return true;
        }
        return false;
    }
    ...
    void verify() {
        ...
        for (int i = this.type.typeVariables.length; --i >= 0; ) {
            ...
            for (int j = itsInterfaces.length; --j >= 0; ) {
                superInterfaces[j] = itsInterfaces[j].kind() == Binding.TYPE_PARAMETER ? (ReferenceBinding) itsInterfaces[j].erasure() : itsInterfaces[j];
            }
