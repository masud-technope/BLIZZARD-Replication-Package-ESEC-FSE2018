...
public final class RewriteEventStore {
    ...
    public static final int BOTH = NEW | ORIGINAL;
    ...
    private void assertNoOverlap(ListRewriteEvent listEvent, int indexFirst, int indexLast, List innerList) {
        for (Iterator iter = innerList.iterator(); iter.hasNext(); ) {
            ...
            if (currStart < indexFirst && currEnd < indexLast && currEnd >= indexFirst || currStart > indexFirst && currStart <= currEnd && currEnd > indexLast) {
                throw new //$NON-NLS-1$
                IllegalArgumentException(//$NON-NLS-1$
                "Range overlapps with an existing copy or move range");
            }
        }
    }
    ...
    private void removeMoveRangePlaceholders() {
        for (Iterator iter = this.nodeRangeInfos.entrySet().iterator(); iter.hasNext(); ) {
            ...
            addEvent(loc.getParent(), loc.getProperty(), new ListRewriteEvent(revertedChildrenArr));
        }
    }
    ...
    private void prepareNodeRangeCopies(TargetSourceRangeComputer sourceRangeComputer) {
        for (Iterator iter = this.nodeRangeInfos.entrySet().iterator(); iter.hasNext(); ) {
            ...
            // replace the current edits
            addEvent(loc.getParent(), loc.getProperty(), new ListRewriteEvent(newChildren));
        }
    }
    ...
    private RewriteEvent[] processListWithRanges(List rangeInfos, RewriteEvent[] childEvents, TargetSourceRangeComputer sourceRangeComputer) {
        ...
        for (int k = 0; k < childEvents.length; k++) {
            ...
            while (// is this child the beginning of a range?
            nextInfo != null && node == nextInfo.getStartNode()) {
                ...
                if (nextInfo.isMove()) {
                    // remove or replace
                    newEvent = new NodeRewriteEvent(internalPlaceholder, nextInfo.replacingNode);
                } else {
