...
public class ParameterizedGenericMethodBinding extends ParameterizedMethodBinding implements Substitution {
    ...
    public  ParameterizedGenericMethodBinding(MethodBinding originalMethod, RawTypeBinding rawType, LookupEnvironment environment) {
        ...
        this.parameters = Scope.substitute(this, isStatic ? originalMethod.parameters : Scope.substitute(rawType, originalMethod.parameters));
        this.thrownExceptions = Scope.substitute(this, isStatic ? originalMethod.thrownExceptions : Scope.substitute(rawType, originalMethod.thrownExceptions));
    ...
    public static MethodBinding computeCompatibleMethod(MethodBinding originalMethod, TypeBinding[] arguments, Scope scope, InvocationSite invocationSite) {
        ...
        if (substitutes != null) {
            ...
            for (int i = 0; i < varLength; i++) collectedSubstitutes.put(typeVariables[i], new TypeBinding[1]);
            for (int i = 0; i < argLength; i++) parameters[i].collectSubstitutes(arguments[i], collectedSubstitutes);
    ...
    public void inferFromExpectedType(TypeBinding expectedType, Scope scope) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            //TypeBinding mostSpecificSubstitute = scope.mostSpecificCommonType(variableSubstitutes);
            if (mostSpecificSubstitute == null) {
                // TODO (philippe) should report no way to infer type
                return;
            }
            if (mostSpecificSubstitute != VoidBinding)
