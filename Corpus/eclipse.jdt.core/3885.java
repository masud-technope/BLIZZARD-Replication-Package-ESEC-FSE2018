...
public class JavaConventionTests extends AbstractJavaModelTests {
    ...
    static final String[] VERSIONS = new String[] { CompilerOptions.VERSION_1_1, CompilerOptions.VERSION_1_2, CompilerOptions.VERSION_1_3, CompilerOptions.VERSION_1_4, CompilerOptions.VERSION_1_5, CompilerOptions.VERSION_1_6 };
    ...
    public void testPackageFragmentRootOverlap() throws Exception {
        try {
            ...
            assertTrue("zip should not overlap source root", !JavaConventions.isOverlappingRoots(p1Zip.getUnderlyingResource().getFullPath(), p1Src.getUnderlyingResource().getFullPath()));
            this.createJavaProject("P2", new String[] { "src" }, "bin");
            IPackageFragmentRoot p2Src = getPackageFragmentRoot("P2", "src");
            assertTrue("source roots in different projects should not overlap ", !JavaConventions.isOverlappingRoots(p1Src.getUnderlyingResource().getFullPath(), p2Src.getUnderlyingResource().getFullPath()));
    ...
    //bugs.eclipse.org/bugs/show_bug.cgi?id=79392">79392</a>
    /*
	...
	*/
    ...
    public void testAssertIdentifier() {
        ...
        for (// source level
        int i = 0; // source level
        i < length; // source level
        i++) {
            for (// compliance level
            int j = 0; // compliance level
            j < length; // compliance level
            j++) {
                // source level < VERSION_1_4
                if (i < 3) {
