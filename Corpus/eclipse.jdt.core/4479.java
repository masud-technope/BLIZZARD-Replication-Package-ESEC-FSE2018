...
public class ArrayReference extends Reference {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        return position.analyseCode(currentScope, flowContext, receiver.analyseCode(currentScope, flowContext, flowInfo));
    }

    public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
        receiver.generateCode(currentScope, codeStream, true);
        if (// ((type[])null)[0]
        receiver instanceof CastExpression && ((CastExpression) receiver).innermostCastedExpression().resolvedType == NullBinding) {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (valueRequired) {
            ...
            if (this.resolvedType == LongBinding || this.resolvedType == DoubleBinding) {
                codeStream.pop2();
            } else {
                codeStream.pop();
            }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (// no cast check for ((type[])null)[0]
        receiver instanceof CastExpression && ((CastExpression) receiver).innermostCastedExpression() instanceof NullLiteral) {
            // will check later on
