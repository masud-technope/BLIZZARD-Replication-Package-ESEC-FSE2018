...
public class CodeSnippetParser extends Parser implements EvaluationConstants {
    ...
    // end of last top level statement
    int lastStatement = -1;
 ...
 */
    ...
    protected void consumeFieldAccess(boolean isSuperAccess) {
        ...
        FieldReference fr = new CodeSnippetFieldReference(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr--], this.evaluationContext);
        this.identifierLengthPtr--;
        if (isSuperAccess) {
 ...
 */
    protected void consumeMethodDeclaration(boolean isNotAbstract, boolean isDefaultMethod) {
        ...
        if (isTopLevelType()) {
            ...
            if (last >= 0 && methodDecl.statements[last] instanceof Expression) {
                ...
                methodDecl.statements[last] = new CodeSnippetReturnStatement(lastExpression, lastExpression.sourceStart, lastExpression.sourceEnd);
            }
        }
        ...
        if (this.evaluationContext.localVariableNames != null) {
            ...
            for (int i = 0; i < varCount; i++) {
                ...
                NameReference init = new SingleNameReference(CharOperation.concat(LOCAL_VAR_PREFIX, this.evaluationContext.localVariableNames[i]), position);
            ...
            // snippet statements
            tryBlock.statements = methodDecl.statements;
            ...
            for (int i = 0; i < varCount; i++) {
                ...
                finallyBlock.statements[i] = new Assignment(new SingleNameReference(CharOperation.concat(LOCAL_VAR_PREFIX, this.evaluationContext.localVariableNames[i]), position), nameRef, nameRef.sourceEnd);
            }
    ...
    protected void consumeMethodInvocationName() {
        if (this.scanner.startPosition >= this.codeSnippetStart && // 14838
        this.scanner.startPosition <= this.codeSnippetEnd + 1 + this.lineSeparatorLength && isTopLevelType()) {
    ...
    protected void consumeMethodInvocationNameWithTypeArguments() {
        // when the name is only an identifier...we have a message send to "this" (implicit)
        if (this.scanner.startPosition >= this.codeSnippetStart && this.scanner.startPosition <= this.codeSnippetEnd + 1 + // 14838
        this.lineSeparatorLength && isTopLevelType()) {
    ...
    protected void consumePrimaryNoNewArrayThis() {
        if (this.scanner.startPosition >= this.codeSnippetStart && // 14838
        this.scanner.startPosition <= this.codeSnippetEnd + 1 + this.lineSeparatorLength && isTopLevelType()) {
            pushOnExpressionStack(new CodeSnippetThisReference(this.intStack[this.intPtr--], this.endPosition, this.evaluationContext, false));
        } else {
    ...
    protected CompilationUnitDeclaration endParse(int act) {
        if (this.hasRecoveredOnExpression) {
            ...
            act != ERROR_ACTION) {
                ...
                int fieldsCount = (this.evaluationContext.localVariableNames == null ? 0 : this.evaluationContext.localVariableNames.length) + (this.evaluationContext.declaringTypeName == null ? 0 : 1);
    ...
    protected NameReference getUnspecifiedReference(boolean rejectTypeAnnotations) {
        ...
        if (this.scanner.startPosition >= this.codeSnippetStart && this.scanner.startPosition <= this.codeSnippetEnd + 1 + this.lineSeparatorLength) /*14838*/
        {
            int length;
            NameReference ref;
            if ((length = this.identifierLengthStack[this.identifierLengthPtr--]) == 1) {
                // single variable reference
                ref = new CodeSnippetSingleNameReference(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr--], this.evaluationContext);
            } else {
                ...
                ref = new CodeSnippetQualifiedNameReference(tokens, positions, (int) (// sourceStart
                this.identifierPositionStack[this.identifierPtr + 1] >> // sourceStart
                32), (int) this.identifierPositionStack[this.identifierPtr + length], // sourceEnd
	...
	look for that it is not a type reference */
        ...
        if (this.scanner.startPosition >= this.codeSnippetStart && this.scanner.startPosition <= this.codeSnippetEnd + 1 + this.lineSeparatorLength) /*14838*/
        {
            ...
            long[] positions = new long[length];
            System.arraycopy(this.identifierPositionStack, this.identifierPtr + 1, positions, 0, length);
            ref = new CodeSnippetQualifiedNameReference(tokens, positions, // sourceStart
            (int) (this.identifierPositionStack[this.identifierPtr + 1] >> 32), (int) this.identifierPositionStack[this.identifierPtr + length], // sourceEnd
            this.evaluationContext);
            ref.bits &= ~ASTNode.RestrictiveFlagMASK;
            ref.bits |= Binding.LOCAL | Binding.FIELD;
            return ref;
 ...
 */
    ...
    protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
        if (!isDietParse) {
            ...
            //TokenNameREMAINDER; // first token of th expression parse
            oldFirstToken = TokenNameTWIDDLE;
        }
 ...
 */
    protected int resumeOnSyntaxError() {
        if (// no reentering inside expression recovery
        this.diet || this.hasRecoveredOnExpression) {
            return HALT;
        }
