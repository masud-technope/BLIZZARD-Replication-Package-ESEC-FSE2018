...
public class AbstractJavaSearchTests extends ModifyingResourceTests implements IJavaSearchConstants {
    ...
    protected static final int EXACT_RULE = SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE;

    protected static final int EQUIVALENT_RULE = EXACT_RULE | SearchPattern.R_EQUIVALENT_MATCH;

    protected static final int ERASURE_RULE = EXACT_RULE | SearchPattern.R_ERASURE_MATCH;

    protected static final int RAW_RULE = EXACT_RULE | SearchPattern.R_ERASURE_MATCH | SearchPattern.R_EQUIVALENT_MATCH;
	 ...
	 */
    protected static final int SHOW_ACCURACY = 0x0001;

    protected static final int SHOW_SELECTION = 0x0002;

    protected static final int SHOW_RULE = 0x0004;

    protected static final int SHOW_INSIDE_DOC = 0x0008;

    protected static final int SHOW_POTENTIAL = 0x0010;

    protected static final int SHOW_PROJECT = 0x0020;

    protected static final int SHOW_SYNTHETIC = 0x0040;

    protected static final int SHOW_OFFSET = 0x0080;

    protected static final int SHOW_ACCESS = 0x0100;

    protected static final int SHOW_MATCH_KIND = 0x0200;

    protected static final int SHOW_JAR_FILE = 0x0400;
    ...
    public static class ConstructorDeclarationsCollector implements IRestrictedAccessConstructorRequestor {
        ...
        public void acceptConstructor(int modifiers, char[] simpleTypeName, int parameterCount, char[] signature, char[][] parameterTypes, char[][] parameterNames, int typeModifiers, char[] packageName, int extraFlags, String path, AccessRestriction access) {
            ...
            if (isMemberType) {
                buffer.append('.');
                // enclosing type names are not stored in the indexes
                buffer.append('?');
                buffer.append('?');
                buffer.append('?');
            }
    ...
    public static class JavaSearchResultCollector extends SearchRequestor {
        ...
        protected void writeLine() throws CoreException {
            try {
                ...
                if (resource instanceof IFile) {
                    ...
                    if (// retrieving attached source not implemented here
                    start == -1 || (contents != null && contents.length > 0)) {
                        this.line.append(" [");
                        if (start > -1) {
        ...
        private void append(IType type) throws JavaModelException {
            ...
            if (// don't want occurrence counts for lambdas. it can be confusing at best, as not all are built.
            isLocal && !(type instanceof LambdaExpression)) {
                this.line.append("#");
    ...
    protected void searchAllMethodNames(String pattern, int matchRule, IJavaSearchScope scope, IRestrictedAccessMethodRequestor requestor) throws JavaModelException {
        searchAllMethodNames(null, SearchPattern.R_EXACT_MATCH, pattern, matchRule, scope, requestor);
    }

    protected void searchAllMethodNames(String declSimpleNamePattern, int declSimpleNameMatchRule, String patternMethod, int methodMatchRule, IJavaSearchScope scope, IRestrictedAccessMethodRequestor requestor) throws JavaModelException {
        searchAllMethodNames(null, SearchPattern.R_EXACT_MATCH, declSimpleNamePattern, declSimpleNameMatchRule, patternMethod, methodMatchRule, scope, requestor);
    }

    protected void searchAllMethodNames(String declQualificationPattern, int declQualificationMatchRule, String declSimpleNamePattern, int declSimpleNameMatchRule, String patternMethod, int methodMatchRule, IJavaSearchScope scope, IRestrictedAccessMethodRequestor requestor) throws JavaModelException {
        searchAllMethodNames(null, SearchPattern.R_EXACT_MATCH, declQualificationPattern, declQualificationMatchRule, declSimpleNamePattern, declSimpleNameMatchRule, patternMethod, methodMatchRule, scope, requestor);
    }

    protected void searchAllMethodNames(String patternPackage, int pkgMatchRule, String declQualificationPattern, int declQualificationMatchRule, String declSimpleNamePattern, int declSimpleNameMatchRule, String patternMethod, int methodMatchRule, IJavaSearchScope scope, IRestrictedAccessMethodRequestor requestor) throws JavaModelException {
    ...
    public void searchAllMethodNames(String packageName, final int pkgMatchRule, String declaringQualification, final int declQualificationMatchRule, String declaringSimpleName, final int declSimpleNameMatchRule, String methodName, final int methodMatchRule, IJavaSearchScope scope, final MethodNameMatchRequestor nameRequestor) {
        try {
            new SearchEngine(this.workingCopies).searchAllMethodNames(packageName != null ? packageName.toCharArray() : null, pkgMatchRule, declaringQualification != null ? declaringQualification.toCharArray() : null, declQualificationMatchRule, declaringSimpleName != null ? declaringSimpleName.toCharArray() : null, declSimpleNameMatchRule, methodName != null ? methodName.toCharArray() : null, methodMatchRule, scope, nameRequestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
        } catch (JavaModelException e) {
            e.printStackTrace();
        }
    }

    public void searchAllMethodNames(String qualifier, final int qualifierMatchRule, String methodName, final int methodMatchRule, IJavaSearchScope scope, final MethodNameMatchRequestor nameRequestor) {
        try {
            new SearchEngine(this.workingCopies).searchAllMethodNames(qualifier != null ? qualifier.toCharArray() : null, qualifierMatchRule, methodName != null ? methodName.toCharArray() : null, methodMatchRule, scope, nameRequestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
        } catch (JavaModelException e) {
            e.printStackTrace();
        }
    }
    ...
    protected void searchAllTypeNames(String pattern, int matchRule, TypeNameRequestor requestor) throws JavaModelException {
        new SearchEngine(this.workingCopies).searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH, pattern.toCharArray(), matchRule, TYPE, getJavaSearchScope(), requestor, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
    }
    ...
    protected void searchAllTypeNames(String packagePattern, String typePattern, int matchRule, TypeNameMatchCollector collector) throws JavaModelException {
        new SearchEngine(this.workingCopies).searchAllTypeNames(packagePattern == null ? null : packagePattern.toCharArray(), SearchPattern.R_EXACT_MATCH, typePattern == null ? null : typePattern.toCharArray(), matchRule, TYPE, getJavaSearchScope(), collector, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
    }
