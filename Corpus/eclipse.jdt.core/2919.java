...
public class BinaryTypeConverter extends TypeConverter {
    ...
    public ImportReference[] buildImports(ClassFileReader reader) {
        ...
        for (int i = 0, length = set.length; i < length; i++) {
            char[][] typeName = set[i];
            if (typeName != null) {
                imports[index++] = new ImportReference(typeName, new long[typeName.length], /*not on demand*/
                false, 0);
            }
        }
        return imports;
    ...
    private AbstractMethodDeclaration convert(IMethod method, IType type) throws JavaModelException {
        ...
        if (this.has1_5Compliance) {
            ...
            if (typeParameters != null && typeParameters.length > 0) {
                ...
                for (int i = 0; i < parameterCount; i++) {
                    ...
                    typeParams[i] = createTypeParameter(typeParameter.getElementName().toCharArray(), stringArrayToCharArray(typeParameter.getBounds()), 0, 0);
                }
            }
        }
        ...
        for (int i = 0; i < argumentCount; i++) {
            ...
            methodDeclaration.arguments[i] = new Argument(argumentNames[i].toCharArray(), 0, typeReference, ClassFileConstants.AccDefault);
    ...
    private TypeDeclaration convert(IType type, IType alreadyComputedMember, TypeDeclaration alreadyComputedMemberDeclaration) throws JavaModelException {
        ...
        if (// add default constructor in first position
        neededCount != 0) {
