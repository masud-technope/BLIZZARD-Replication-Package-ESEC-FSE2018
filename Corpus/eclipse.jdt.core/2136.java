...
public class MirrorUtilTests extends APTTestBase {
    ...
    private void addEnvOptions(IJavaProject jproj) {
        for (int i = 0; i < MirrorUtilTestAnnotationProcessor.ENV_KEYS.length; ++i) {
            AptConfig.addProcessorOption(jproj, MirrorUtilTestAnnotationProcessor.ENV_KEYS[i], MirrorUtilTestAnnotationProcessor.ENV_VALUES[i]);
        }
    }
    ...
    private void assertNoUnexpectedProblems() {
        ...
        for (Problem problem : problems) {
            if (//$NON-NLS-1$
            problem.getMessage().startsWith("The field DeclarationsTestClass")) {
                continue;
            }
            //$NON-NLS-1$
