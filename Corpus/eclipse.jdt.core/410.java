...
public class CompilationResult {
    ...
    private static void quickSort(IProblem[] list, int left, int right) {
        ...
        do {
            while (list[left].getSourceStart() < mid) left++;
            while (mid < list[right].getSourceStart()) right--;
            if (left <= right) {
    ...
    private void quickPrioritize(IProblem[] list, int left, int right) {
        ...
        do {
            while (computePriority(list[right]) < mid) right--;
            while (mid < computePriority(list[left])) left++;
            if (left <= right) {
                IProblem tmp = list[left];
                list[left] = list[right];
                list[right] = tmp;
    ...
    public String toString() {
        ...
        if (problems != null) {
            ...
            for (int i = 0; i < this.problemCount; i++) {
                //$NON-NLS-1$
                buffer.append("\t - ").append(this.problems[i]).append(//$NON-NLS-1$
                '\n');
            }
        } else {
            //$NON-NLS-1$
