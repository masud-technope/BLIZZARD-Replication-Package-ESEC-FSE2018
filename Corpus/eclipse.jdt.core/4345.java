...
public abstract class JavaModelOperation implements IWorkspaceRunnable, IProgressMonitor {
    ...
    protected JavaModelOperation popOperation() {
        ...
        if (size > 0) {
            ...
            size == 1) {
                // release reference (see http://bugs.eclipse.org/bugs/show_bug.cgi?id=33927)
            ...
            try {
                ...
                try {
                    if (canModifyRoots()) {
                        ...
                        JavaModelManager.getDeltaState().initializeRoots(/*not initiAfterLoad*/
                        false);
                    }
                    executeOperation();
                } finally {
                ...
                try {
                    ...
                    for (int i = 0, length = this.resultElements.length; i < length; i++) {
                        ...
                        if (// a working copy must remain a child of its parent even after a move
                        !(openable instanceof CompilationUnit) || !((CompilationUnit) openable).isWorkingCopy()) {
