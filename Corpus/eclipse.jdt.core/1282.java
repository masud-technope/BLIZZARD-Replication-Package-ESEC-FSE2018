...
public class Java8ElementsTests extends TestCase {
    ...
    public void _testTypeAnnotations6WithJavac() throws Exception {
        // Disabled for now. Javac 8b108 drops annotations arrays preceding varargs.
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testTypeAnnotations6");
    }
    ...
    public // Disabled for now. Javac 8b108 misattributes annotations on type parameters 
    void _testTypeAnnotations7WithJavac() throws // Disabled for now. Javac 8b108 misattributes annotations on type parameters 
    Exception {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testTypeAnnotations7");
    }
    ...
    public void _testTypeAnnotations8WithJavac() throws Exception {
        // Disabled for now. Javac 8b108 misattributes annotations on type parameters
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testTypeAnnotations8");
    }
    ...
    public // Disabled for now. Javac returns null as receiver type where it should be type 'None'
    void _testTypeAnnotations14WithJavac() throws // Disabled for now. Javac returns null as receiver type where it should be type 'None'
    Exception {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testTypeAnnotations14");
    }
    ...
    public // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    void _testRepeatedAnnotations22WithJavac() throws // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    Exception {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testRepeatedAnnotations22", "JEP120_5.java");
    }
    ...
    public // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    void testRepeatedAnnotations24WithJavac() throws // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    Exception {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testRepeatedAnnotations24", "JEP120_6.java");
    }
    ...
    public // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    void testRepeatedAnnotations25WithJavac() throws // Disabled for now, javac 8b108 does not seem to expose any annotations on a type mirror when there are repeated annotations.
    Exception {
        JavaCompiler compiler = ToolProvider.getSystemJavaCompiler();
        internalTest(compiler, JAVA8_ANNOTATION_PROC, "testRepeatedAnnotations25", "JEP120_7.java");
    }
