...
public class IgnoreOptionalProblemsFromSourceFoldersTests extends ModifyingResourceTests {
    ...
    public void test002() throws CoreException {
        ...
        try {
            ...
            project.setRawClasspath(newCP, null);
            createFolder("/P/src/p");
            IFile fileX = createFile("/P/src/p/X.java", "package p;\n" + "public class X {\n" + "	public void foo() {\n" + "		int i;\n" + "	}\n" + "}");
            x = (ICompilationUnit) JavaCore.create(fileX);
            createFolder("/P/src2/q");
            IFile fileY = createFile("/P/src2/q/Y.java", "package q;\n" + "public class Y {\n" + "	public void foo() {\n" + "		int i;\n" + "	}\n" + "}");
    ...
    public void test003() throws CoreException {
        ...
        try {
            ...
            System.arraycopy(originalCP, 0, newCP, 0, originalCP.length);
            newCP[originalCP.length] = JavaCore.newSourceEntry(new Path("/P/src"), null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE });
            newCP[originalCP.length + 1] = JavaCore.newSourceEntry(new Path("/P/src2"), null, null, null, new IClasspathAttribute[] { ATTR_IGNORE_OPTIONAL_PROBLEMS_TRUE });
            project.setRawClasspath(newCP, null);
            createFolder("/P/src/p");
            IFile fileX = createFile("/P/src/p/X.java", "package p;\n" + "public class X {\n" + "	public void foo() {\n" + "		int i;\n" + "	}\n" + "}");
            x = (ICompilationUnit) JavaCore.create(fileX);
