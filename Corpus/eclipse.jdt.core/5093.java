...
public abstract class CompletionRequestor {
    ...
    public boolean isIgnored(int completionProposalKind) {
        if (completionProposalKind < CompletionProposal.FIRST_KIND || completionProposalKind > CompletionProposal.LAST_KIND) {
            throw new //$NON-NLS-1$
            IllegalArgumentException(//$NON-NLS-1$
            "Unknown kind of completion proposal: " + completionProposalKind);
        }
    ...
    public boolean isAllowingRequiredProposals(int proposalKind, int requiredProposalKind) {
        if (proposalKind < CompletionProposal.FIRST_KIND || proposalKind > CompletionProposal.LAST_KIND) {
            throw new //$NON-NLS-1$
            IllegalArgumentException(//$NON-NLS-1$
            "Unknown kind of completion proposal: " + requiredProposalKind);
        }
        if (requiredProposalKind < CompletionProposal.FIRST_KIND || requiredProposalKind > CompletionProposal.LAST_KIND) {
            throw new //$NON-NLS-1$
            IllegalArgumentException(//$NON-NLS-1$
            "Unknown required kind of completion proposal: " + requiredProposalKind);
        }
