...
public class NonNullDefaultAwareTypeAnnotationWalker extends TypeAnnotationWalker {
    ...
    /** Get restricted walker, still with non-empty type annotations. */
     NonNullDefaultAwareTypeAnnotationWalker(IBinaryTypeAnnotation[] typeAnnotations, long newMatches, int newPathPtr, int defaultNullness, IBinaryAnnotation nonNullAnnotation, boolean atDefaultLocation, boolean atTypeBound, LookupEnvironment environment) {
    ...
    /** Create a restricted walker without 'real' type annotations, but with a nonnull default. */
     NonNullDefaultAwareTypeAnnotationWalker(int defaultNullness, IBinaryAnnotation nonNullAnnotation, boolean atDefaultLocation, boolean atTypeBound, LookupEnvironment environment) {
        super(null, 0, 0);
        this.nonNullAnnotation = nonNullAnnotation;
        this.defaultNullness = defaultNullness;
        this.atDefaultLocation = atDefaultLocation;
    ...
    protected TypeAnnotationWalker restrict(long newMatches, int newPathPtr) {
        ...
        try {
            ...
            // expire
            this.nextIsDefaultLocation = // expire
            false;
    ...
    public IBinaryAnnotation[] getAnnotationsAtCursor(int currentTypeId) {
        ...
        if (this.atDefaultLocation && // never apply default on type variable use or wildcard
        !(currentTypeId == -1) && // for CLIMB-to-top consider a j.l.Object type bound as no explicit type bound
        !(this.atTypeBound && currentTypeId == TypeIds.T_JavaLangObject)) {
