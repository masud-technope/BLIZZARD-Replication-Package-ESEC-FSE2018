...
*/
    ...
    static boolean areMethodsCompatible(MethodBinding one, MethodBinding two, LookupEnvironment environment) {
        ...
        if (two == null)
            // method's declaringClass does not inherit from inheritedMethod's
            return false;
    ...
    boolean canSkipInheritedMethods(MethodBinding one, MethodBinding two) {
        ...
        two == null || TypeBinding.equalsEquals(one.declaringClass, two.declaringClass);
    }
    ...
    void checkAgainstInheritedMethods(MethodBinding currentMethod, MethodBinding[] methods, int length, MethodBinding[] allInheritedMethods) {
        if (// annotation cannot override any method
        this.type.isAnnotationType()) {
        ...
        nextMethod: for (int i = length; --i >= 0; ) {
            ...
            if (overriddenInheritedMethods == null || overriddenInheritedMethods[i] == 0) {
                if (// Cannot override a static method or hide an instance method
                currentMethod.isStatic() != inheritedMethod.isStatic()) {
                ...
                //					currentMethod.modifiers |= CompilerModifiers.AccImplementing;
                    ...
                    if (currentMethod.isDefaultMethod() && // overriding final is already reported, that's enough
                    !inheritedMethod.isFinal() && inheritedMethod.declaringClass.id == TypeIds.T_JavaLangObject) {
                ...
                if (!areReturnTypesCompatible(currentMethod, inheritedMethod) && (currentMethod.returnType.tagBits & TagBits.HasMissingType) == 0) {
                    if (reportIncompatibleReturnTypeError(currentMethod, inheritedMethod))
                        continue nextMethod;
                }
                // if they were deferred, emit them now.
                ...
                if (options.reportDeprecationWhenOverridingDeprecatedMethod && inheritedMethod.isViewedAsDeprecated()) {
                    if (!currentMethod.isViewedAsDeprecated() || options.reportDeprecationInsideDeprecatedCode) {
                        ...
                        if (declaringClass.isInterface())
                            for (int j = length; --j >= 0; ) if (i != j && methods[j].declaringClass.implementsInterface(declaringClass, false))
                                continue nextMethod;
        ...
        if (overridden != null) {
            for (int i = overridden.length; --i >= 0; ) {
                ...
                if (isParameterSubsignature(currentMethod, inheritedMethod) && !inheritedMethod.isStatic() && !inheritedMethod.isFinal())
                    checkForBridgeMethod(currentMethod, inheritedMethod, allInheritedMethods);
            }
        }
    }
    ...
    void checkConcreteInheritedMethod(MethodBinding concreteMethod, MethodBinding[] abstractMethods) {
        ...
        if (!concreteMethod.isPublic()) {
            ...
            if (concreteMethod.isProtected()) {
                for (; index < length; index++) if (abstractMethods[index].isPublic())
                    break;
            } else if (concreteMethod.isDefault()) {
                for (; index < length; index++) if (!abstractMethods[index].isDefault())
                    break;
            }
            if (index < length)
        ...
        if (concreteMethod.thrownExceptions != Binding.NO_EXCEPTIONS)
            for (int i = abstractMethods.length; --i >= 0; ) checkExceptions(concreteMethod, abstractMethods[i]);
...
*/
    ...
    void checkForMissingHashCodeMethod() {
        ...
        for (int i = choices.length; !overridesEquals && --i >= 0; ) overridesEquals = choices[i].parameters.length == 1 && choices[i].parameters[0].id == TypeIds.T_JavaLangObject;
        if (overridesEquals) {
                ...
                if (i != j && toCheck.implementsInterface(implementedInterface, true)) {
                    ...
                    for (int r = 0, rl = refs.length; r < rl; r++) {
                        if (TypeBinding.equalsEquals(refs[r].resolvedType, toCheck)) {
                            ...
                            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=320911
                            break;
                        }
                    }
                }
	...
	*/
        ...
        if (index > 0) {
            ...
            if (bestAbstractMethod == null)
                ...
                // can only happen in >= 1.5 since return types must be equal prior to 1.5
                problemReporter().abstractMethodMustBeImplemented(this.type, bestAbstractMethod, concreteMethod);
            return;
        }
        MethodBinding[] abstractMethods = new MethodBinding[length - 1];
        index = 0;
        for (int i = 0; i < length; i++) if (methods[i].isAbstract() || (methods[i] != concreteMethod && methods[i].isDefaultMethod()))
            abstractMethods[index++] = methods[i];
        // can happen with methods that contain 'equal' Missing Types, see bug 257384
        if (index == 0)
            return;
        ...
        if (!this.type.isInterface())
            if (method.declaringClass.isClass() || !this.type.implementsInterface(method.declaringClass, false))
                if (otherMethod.declaringClass.isClass() || !this.type.implementsInterface(otherMethod.declaringClass, false))
                    // do not complain since the superclass already got blamed
                    return true;
        return false;
    }
...
*/
    ...
    void computeInheritedMethods() {
        ReferenceBinding superclass = this.type.isInterface() ? // check interface methods against Object
        this.type.scope.getJavaLangObject() : // class or enum
...
*/
            ...
            nextMethod: for (int m = methods.length; --m >= 0; ) {
                            ...
                            if (inheritedMethod.isDefault()) {
                                if (inheritedMethod.isAbstract()) {
                                    ...
                                    if (this.type.fPackage == inheritedMethod.declaringClass.fPackage && !areReturnTypesCompatible(inheritedMethod, existingMethod))
                                        // may need to record incompatible return type
                                        continue existing;
                                }
                            }
                            if (TypeBinding.notEquals(inheritedMethod.returnType.erasure(), existingMethod.returnType.erasure()) && areReturnTypesCompatible(existingMethod, inheritedMethod)) {
                ...
                if (!inheritedMethod.isDefault() || inheritedMethod.declaringClass.fPackage == this.type.fPackage) {
                    ...
                    if (nonVisible != null && inheritedMethod.isAbstract())
                        for (int i = 0, l = nonVisible.length; i < l; i++) if (areMethodsCompatible(nonVisible[i], inheritedMethod))
                            continue nextMethod;
                    if (nonVisible == null) {
                    ...
                    if (// non visible abstract methods cannot be overridden so the type must be defined abstract
                    inheritedMethod.isAbstract() && !this.type.isAbstract())
                    ...
                    if (// non visible methods cannot be overridden so a warning is issued
                    current != null && !inheritedMethod.isStatic()) {
        ...
        for (int i = len - 1; i >= 0; i--) {
            ...
            if (superType.isValidBinding()) {
                if (skip != null && skip.includes(superType))
                    continue;
                MethodBinding[] methods = superType.unResolvedMethods();
                nextMethod: for (// Interface methods are all abstract public
                int m = methods.length; // Interface methods are all abstract public
                --m >= 0; ) {
                    MethodBinding inheritedMethod = methods[m];
    ...
    protected boolean canOverridingMethodDifferInErasure(MethodBinding overridingMethod, MethodBinding inheritedMethod) {
        // the case for <= 1.4  (cannot differ)
        return false;
    }
    ...
    void computeMethods() {
        ...
        for (int m = size; --m >= 0; ) {
            MethodBinding method = methods[m];
            if (// keep all methods which are NOT constructors or default abstract
            !(method.isConstructor() || method.isDefaultAbstract())) {
    ...
    public static MethodBinding computeSubstituteMethod(MethodBinding inheritedMethod, MethodBinding currentMethod, LookupEnvironment environment) {
        ...
        for (int i = 0; i < inheritedLength; i++) {
            ...
            // cast is safe by construction: arguments is copied from TypeVariableBinding[]
            ...
            next: for (int j = 0; j < interfaceLength; j++) {
                ...
                for (int k = 0; k < interfaceLength; k++) if (TypeBinding.equalsEquals(superType, interfaces[k]))
                    continue next;
                // not a match
                return inheritedMethod;
            }
    ...
    static boolean couldMethodOverride(MethodBinding method, MethodBinding inheritedMethod) {
        ...
        if (inheritedMethod.isDefault() && method.declaringClass.getPackage() != inheritedMethod.declaringClass.getPackage())
            return false;
        if (// inheritedMethod is either public or protected & method is less than public
        !method.isPublic()) {
    ...
    public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
        return org.eclipse.jdt.core.compiler.CharOperation.equals(method.selector, inheritedMethod.selector) && isParameterSubsignature(method, inheritedMethod);
    }
    ...
    boolean mustImplementAbstractMethod(ReferenceBinding declaringClass) {
        ...
        if (declaringClass.isClass()) {
            while (superclass.isAbstract() && TypeBinding.notEquals(superclass, declaringClass)) // find the first concrete superclass or the abstract declaringClass
            superclass = superclass.superclass();
        } else {
            if (this.type.implementsInterface(declaringClass, false))
                if (// only if a superclass does not also implement the interface
                !superclass.implementsInterface(declaringClass, true))
                    return true;
            while (superclass.isAbstract() && !superclass.implementsInterface(declaringClass, false)) // find the first concrete superclass or the superclass which implements the interface
            superclass = superclass.superclass();
        }
    ...
    ProblemReporter problemReporter(MethodBinding currentMethod) {
        ...
        if (// only report against the currentMethod if its implemented by the type
        TypeBinding.equalsEquals(currentMethod.declaringClass, this.type) && currentMethod.sourceMethod() != null)
 ...
 */
    ...
    ReferenceBinding[] resolvedExceptionTypesFor(MethodBinding method) {
        ...
        for (int i = exceptions.length; --i >= 0; ) exceptions[i] = (ReferenceBinding) BinaryTypeBinding.resolveType(exceptions[i], this.environment, /* raw conversion */
        true);
        return exceptions;
    }
