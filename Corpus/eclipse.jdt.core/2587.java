...
public class FieldReference extends Reference implements InvocationSite {
    ...
    public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, Assignment assignment, boolean isCompound) {
        ...
        isCompound) {
            if (binding.isBlankFinal() && receiver.isThis() && currentScope.allowBlankFinalFieldAssignment(binding) && (!flowInfo.isDefinitelyAssigned(binding))) {
                currentScope.problemReporter().uninitializedBlankFinalField(binding, this);
            // we could improve error msg here telling "cannot use compound assignment on final blank field"
            }
            manageSyntheticAccessIfNecessary(currentScope, flowInfo, /*read-access*/
            true);
        }
        ...
        if (binding.isFinal()) {
            ...
            if (binding.isBlankFinal() && !isCompound && receiver.isThis() && !(receiver instanceof QualifiedThisReference) && // (this).x is forbidden
            ((receiver.bits & ParenthesizedMASK) == 0) && currentScope.allowBlankFinalFieldAssignment(binding)) {
    ...
    public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
        ...
        fieldStore(codeStream, this.codegenBinding, syntheticAccessors == null ? null : syntheticAccessors[WRITE], valueRequired);
        if (valueRequired) {
        ...
        if (constant != NotAConstant) {
            ...
            if (valueRequired) {
                if (!this.codegenBinding.isConstantValue()) {
                    if (// array length
                    this.codegenBinding.declaringClass == // array length
                    null) {
    ...
    public static final Constant getConstantFor(FieldBinding binding, Reference reference, boolean isImplicit, Scope referenceScope) {
        ...
        fieldDecl.resolve(//side effect on binding 
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo, boolean isReadAccess) {
        ...
        if (binding.isPrivate()) {
            if ((currentScope.enclosingSourceType() != this.codegenBinding.declaringClass) && !binding.isConstantValue()) {
                ...
                syntheticAccessors[isReadAccess ? READ : WRITE] = ((SourceTypeBinding) this.codegenBinding.declaringClass).addSyntheticMethod(this.codegenBinding, isReadAccess);
        ...
        if (this.binding.declaringClass != this.receiverType && !this.receiverType.isArrayType() && // array.length
        this.binding.declaringClass != null && !this.binding.isConstantValue() && ((currentScope.environment().options.targetJDK >= ClassFileConstants.JDK1_2 && this.binding.declaringClass.id != T_Object) || //no change for Object fields (in case there was)
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (binding.isStatic()) {
            ...
            if (!(isImplicitThisRcv || (receiver instanceof NameReference && (((NameReference) receiver).bits & BindingIds.TYPE) != 0))) {
