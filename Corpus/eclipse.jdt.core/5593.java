...
public abstract class AbstractJavaModelTests extends SuiteOfTestCases {
    ...
    protected void assertSearchResults(String message, String expected, Object collector) {
        assertSearchResults(message, expected, collector, /* assertion */
        true);
    }
    ...
    protected void addClassFolder(IJavaProject javaProject, String folderRelativePath, String[] pathAndContents, String compliance) throws CoreException, IOException {
        ...
        addLibraryEntry(javaProject, new Path(projectPath + folderRelativePath), null, null, null, null, true);
    }
    ...
    protected void addExternalLibrary(IJavaProject javaProject, String jarPath, String[] pathAndContents, String[] nonJavaResources, String compliance) throws Exception {
        ...
        addLibraryEntry(javaProject, new Path(jarPath), /*exported*/
        true);
    }
    ...
    protected void addLibrary(String jarName, String sourceZipName, String[] pathAndContents, String compliance) throws CoreException, IOException {
        addLibrary(this.currentProject, jarName, sourceZipName, pathAndContents, /*no non-Java resources*/
        null, null, null, compliance, null);
    }
    ...
    protected void addLibrary(IJavaProject javaProject, String jarName, String sourceZipName, String[] pathAndContents, String compliance, Map options) throws CoreException, IOException {
        addLibrary(javaProject, jarName, sourceZipName, pathAndContents, /*no non-Java resources*/
        null, null, null, compliance, options);
    }
    ...
    protected void addLibrary(IJavaProject javaProject, String jarName, String sourceZipName, String[] pathAndContents, String[] nonJavaResources, String[] librariesInclusionPatterns, String[] librariesExclusionPatterns, String compliance, Map options) throws CoreException, IOException {
        IProject project = createLibrary(javaProject, jarName, sourceZipName, pathAndContents, nonJavaResources, compliance, options);
        String projectPath = '/' + project.getName() + '/';
        addLibraryEntry(javaProject, new Path(projectPath + jarName), sourceZipName == null ? null : new Path(projectPath + sourceZipName), null, toIPathArray(librariesInclusionPatterns), toIPathArray(librariesExclusionPatterns), true);
    }
    ...
    protected IProject createLibrary(IJavaProject javaProject, String jarName, String sourceZipName, String[] pathAndContents, String[] nonJavaResources, String compliance, Map options) throws IOException, CoreException {
        IProject project = javaProject.getProject();
        String projectLocation = project.getLocation().toOSString();
    ...
    protected void addLibraryEntry(IJavaProject project, String path, String srcAttachmentPath) throws JavaModelException {
        addLibraryEntry(project, new Path(path), srcAttachmentPath == null ? null : new Path(srcAttachmentPath), null, null, null, new IClasspathAttribute[0], false);
    }
    ...
    protected void addLibraryEntry(IJavaProject project, IPath path, IPath srcAttachmentPath, IPath srcAttachmentPathRoot, IPath[] accessibleFiles, IPath[] nonAccessibleFiles, boolean exported) throws JavaModelException {
        addLibraryEntry(project, path, srcAttachmentPath, srcAttachmentPathRoot, accessibleFiles, nonAccessibleFiles, new IClasspathAttribute[0], exported);
    }
    ...
    protected void addLibraryEntry(IJavaProject project, IPath path, IPath srcAttachmentPath, IPath srcAttachmentPathRoot, IPath[] accessibleFiles, IPath[] nonAccessibleFiles, IClasspathAttribute[] extraAttributes, boolean exported) throws JavaModelException {
        IClasspathEntry entry = JavaCore.newLibraryEntry(path, srcAttachmentPath, srcAttachmentPathRoot, ClasspathEntry.getAccessRules(accessibleFiles, nonAccessibleFiles), extraAttributes, exported);
    ...
    protected void assertWorkingCopyDeltas(String message, String expected) {
        assertDeltas(message, expected, /*don't wait for resource delta*/
        false);
    }
    ...
    protected void assertElementEquals(String message, String expected, IJavaElement element) {
        String actual = element == null ? "<null>" : ((JavaElement) element).toStringWithAncestors(/*don't show key*/
        false);
        if (!expected.equals(actual)) {
            if (this.displayName)
                System.out.println(getName() + " actual result is:");
    ...
    protected void assertElementExists(String message, String expected, IJavaElement element) {
        ...
        if (element != null && !element.exists()) {
            fail(((JavaElement) element).toStringWithAncestors(/*don't show key*/
            false) + " doesn't exist");
        }
    }

    protected void assertElementsEqual(String message, String expected, IJavaElement[] elements) {
        assertElementsEqual(message, expected, elements, /*don't show key*/
        false);
    }
    ...
    protected void assertExceptionEquals(String message, String expected, Exception exception) {
        String actual = exception == null ? "<null>" : (exception instanceof CoreException) ? ((CoreException) exception).getStatus().getMessage() : exception.toString();
        if (!expected.equals(actual)) {
            if (this.displayName)
                System.out.println(getName() + " actual result is:");
            System.out.println(displayString(actual, this.tabs) + this.endChar);
    ...
    protected void sortMarkers(IMarker[] markers) {
        org.eclipse.jdt.internal.core.util.Util.Comparer comparer = new org.eclipse.jdt.internal.core.util.Util.Comparer() {
            ...
            public int compare(Object a, Object b) {
                ...
                //$NON-NLS-1$ //$NON-NLS-2$
                return markerA.getAttribute(IMarker.MESSAGE, "").compareTo(markerB.getAttribute(IMarker.MESSAGE, ""));
            }
        };
    ...
    protected void assertProblems(String message, String expected, ProblemRequestor problemRequestor) {
        ...
        assertEquals(message, independantExpectedString, actual);
    }
    ...
    protected void assertSourceEquals(String message, String expected, String actual) {
        assertSourceEquals(message, expected, actual, /*convert line delimiter*/
        true);
    }
    ...
    protected void assertDeltas(String message, String expected) {
        assertDeltas(message, expected, expected.length() > 0);
    }
    ...
    protected void assertStringsEqual(String message, String expected, String[] strings) {
        String actual = org.eclipse.jdt.core.tests.util.Util.toString(strings, /*add extra new lines*/
        true);
        if (!expected.equals(actual)) {
            System.out.println(displayString(actual, this.tabs) + this.endChar);
        }
    ...
    protected void assertStringsEqual(String message, String[] expectedStrings, String[] actualStrings) {
        String expected = org.eclipse.jdt.core.tests.util.Util.toString(expectedStrings, /*don't add extra new lines*/
        false);
        String actual = org.eclipse.jdt.core.tests.util.Util.toString(actualStrings, /*don't add extra new lines*/
        false);
        if (!expected.equals(actual)) {
            System.out.println(displayString(actual, this.tabs) + this.endChar);
        }
    ...
    protected void attachSource(IPackageFragmentRoot root, String sourcePath, String sourceRoot) throws JavaModelException {
        ...
        for (int i = 0; i < entries.length; i++) {
            ...
            if (entry.getPath().toOSString().toLowerCase().equals(root.getPath().toOSString().toLowerCase())) {
                entries[i] = JavaCore.newLibraryEntry(root.getPath(), sourcePath == null ? null : new Path(sourcePath), sourceRoot == null ? null : new Path(sourceRoot), false);
                break;
            }
        }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String output) throws CoreException {
        return this.createJavaProject(projectName, sourceFolders, /*no lib*/
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String output, String[] sourceOutputs) throws CoreException {
        ...
        null, /*no exclusion pattern*/
        null, /*no exported project*/
        null, output, sourceOutputs, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, "1.4");
    }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String[] libraries, String output) throws CoreException {
        return this.createJavaProject(projectName, sourceFolders, libraries, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, /*no project*/
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, /*combine access restrictions by default*/
        true, /*no exported project*/
        null, output, /*no source outputs*/
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, "1.4", /*don't import*/
        false);
    }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String[] libraries, String output, String compliance, boolean useFullJCL) throws CoreException {
        ...
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, true, /*no exported project*/
        null, output, /*no source outputs*/
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, compliance, useFullJCL, false);
    }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String[] libraries, String[] projects, String projectOutput) throws CoreException {
        return this.createJavaProject(projectName, sourceFolders, libraries, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, projects, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, /*no exported project*/
        null, projectOutput, /*no source outputs*/
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, "1.4");
    }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String[] libraries, String[] projects, boolean[] exportedProject, String projectOutput) throws CoreException {
        ...
        null, projects, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, exportedProject, projectOutput, /*no source outputs*/
        null, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, "1.4");
    }
    ...
    protected IJavaProject createJavaProject(String projectName, String[] sourceFolders, String[] libraries, String[] projects, String projectOutput, String compliance) throws CoreException {
        return createJavaProject(projectName, sourceFolders, libraries, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, projects, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, /*no exported project*/
    ...
    protected IJavaProject createJavaProject(final String projectName, final String[] sourceFolders, final String[] libraries, final String[] projects, final boolean[] exportedProjects, final String projectOutput, final String[] sourceOutputs, final String[][] inclusionPatterns, final String[][] exclusionPatterns, final String compliance) throws CoreException {
        ...
        null, projects, /*no inclusion pattern*/
        null, /*no exclusion pattern*/
        null, exportedProjects, projectOutput, sourceOutputs, inclusionPatterns, exclusionPatterns, compliance);
    }
    ...
    protected IJavaProject createJavaProject(final String projectName, final String[] sourceFolders, final String[] libraries, final String[][] librariesInclusionPatterns, final String[][] librariesExclusionPatterns, final String[] projects, final String[][] projectsInclusionPatterns, final String[][] projectsExclusionPatterns, final boolean[] exportedProjects, final String projectOutput, final String[] sourceOutputs, final String[][] inclusionPatterns, final String[][] exclusionPatterns, final String compliance) throws CoreException {
        return createJavaProject(projectName, sourceFolders, libraries, librariesInclusionPatterns, librariesExclusionPatterns, projects, projectsInclusionPatterns, projectsExclusionPatterns, // combine access restrictions by default
        true, exportedProjects, projectOutput, sourceOutputs, inclusionPatterns, exclusionPatterns, compliance, /*don't import*/
        false);
    }
    ...
    protected IJavaProject createJavaProject(final String projectName, final String[] sourceFolders, final String[] libraries, final String[][] librariesInclusionPatterns, final String[][] librariesExclusionPatterns, final String[] projects, final String[][] projectsInclusionPatterns, final String[][] projectsExclusionPatterns, final boolean combineAccessRestrictions, final boolean[] exportedProjects, final String projectOutput, final String[] sourceOutputs, final String[][] inclusionPatterns, final String[][] exclusionPatterns, final String compliance, final boolean simulateImport) throws CoreException {
        return createJavaProject(projectName, sourceFolders, libraries, librariesInclusionPatterns, librariesExclusionPatterns, projects, projectsInclusionPatterns, projectsExclusionPatterns, combineAccessRestrictions, exportedProjects, projectOutput, sourceOutputs, inclusionPatterns, exclusionPatterns, compliance, false, simulateImport);
    }
        ...
        IWorkspaceRunnable create = new IWorkspaceRunnable() {
            ...
            public void run(IProgressMonitor monitor) throws CoreException {
                ...
                for (int i = 0; i < sourceLength; i++) {
                    ...
                    // create source entry
                    entries[i] = JavaCore.newSourceEntry(projectPath.append(sourcePath), inclusionPaths, exclusionPaths, outputPath == null ? null : projectPath.append(outputPath));
                }
                for (int i = 0; i < libLength; i++) {
                    ...
                    if (// all upper case is a var
                    lib.indexOf(File.separatorChar) == -1 && lib.charAt(0) != '/' && lib.equals(lib.toUpperCase())) {
                        char[][] vars = CharOperation.splitOn(',', lib.toCharArray());
                        entries[sourceLength + i] = JavaCore.newVariableEntry(new Path(new String(vars[0])), vars.length > 1 ? new Path(new String(vars[1])) : null, vars.length > 2 ? new Path(new String(vars[2])) : null, // ClasspathEntry.NO_ACCESS_RULES,
                        ClasspathEntry.getAccessRules(accessibleFiles, nonAccessibleFiles), ClasspathEntry.NO_EXTRA_ATTRIBUTES, false);
                    } else if (lib.startsWith("org.eclipse.jdt.core.tests.model.")) {
                        entries[sourceLength + i] = JavaCore.newContainerEntry(new Path(lib), ClasspathEntry.getAccessRules(accessibleFiles, nonAccessibleFiles), new IClasspathAttribute[0], false);
                    } else {
                        ...
                        entries[sourceLength + i] = JavaCore.newLibraryEntry(libPath, null, null, ClasspathEntry.getAccessRules(accessibleFiles, nonAccessibleFiles), new IClasspathAttribute[0], false);
                    }
                }
                for (int i = 0; i < projectLength; i++) {
                    ...
                    entries[sourceLength + libLength + i] = JavaCore.newProjectEntry(new Path(projects[i]), ClasspathEntry.getAccessRules(accessibleFiles, nonAccessibleFiles), combineAccessRestrictions, new IClasspathAttribute[0], isExported);
                }
    ...
    protected void deleteProject(String projectName) throws CoreException {
        ...
        if (// force opening so that project can be deleted without logging (see bug 23629)
    ...
    protected boolean deltaChildrenChanged(IJavaElementDelta delta) {
        return delta.getKind() == IJavaElementDelta.CHANGED && (delta.getFlags() & IJavaElementDelta.F_CHILDREN) != 0;
    }
    ...
    protected boolean deltaMovedFrom(IJavaElementDelta delta) {
        return delta.getKind() == IJavaElementDelta.ADDED && (delta.getFlags() & IJavaElementDelta.F_MOVED_FROM) != 0;
    }
    ...
    protected boolean deltaMovedTo(IJavaElementDelta delta) {
        return delta.getKind() == IJavaElementDelta.REMOVED && (delta.getFlags() & IJavaElementDelta.F_MOVED_TO) != 0;
    }
	 ...
	 */
    public IPackageFragmentRoot getPackageFragmentRoot(String projectName, String rootPath) throws JavaModelException {
    ...
    public ICompilationUnit getWorkingCopy(String path, String source, WorkingCopyOwner owner) throws JavaModelException {
        ...
        if (owner != null)
            workingCopy = workingCopy.getWorkingCopy(/*no progress monitor*/
            owner, null);
        else
            workingCopy.becomeWorkingCopy(/*no progress monitor*/
            null);
        ...
        workingCopy.makeConsistent(/*no progress monitor*/
        null);
        return workingCopy;
    }
    ...
    public ICompilationUnit getWorkingCopy(String path, String source, WorkingCopyOwner owner, IProblemRequestor problemRequestor) throws JavaModelException {
        ICompilationUnit workingCopy = getCompilationUnit(path);
        if (owner != null)
            workingCopy = workingCopy.getWorkingCopy(owner, problemRequestor, /*no progress monitor*/
            null);
        else
            workingCopy.becomeWorkingCopy(problemRequestor, /*no progress monitor*/
            null);
        workingCopy.getBuffer().setContents(source);
        if (problemRequestor instanceof ProblemRequestor)
            ((ProblemRequestor) problemRequestor).initialize(source.toCharArray());
    ...
    protected void discardWorkingCopies(ICompilationUnit[] units) throws JavaModelException {
        if (units == null)
            return;
        for (int i = 0, length = units.length; i < length; i++) if (units[i] != null)
            units[i].discardWorkingCopy();
    }

    protected String displayString(String toPrint, int indent) {
        char[] toDisplay = toPrint.toCharArray();
        toDisplay = CharOperation.replace(toDisplay, getWorkspacePath().toCharArray(), "getWorkspacePath()".toCharArray());
        toDisplay = CharOperation.replace(toDisplay, getExternalJCLPathString().toCharArray(), "getExternalJCLPathString()".toCharArray());
        toDisplay = CharOperation.replace(toDisplay, getExternalJCLPathString("1.5").toCharArray(), "getExternalJCLPathString(\"1.5\")".toCharArray());
        toDisplay = CharOperation.replace(toDisplay, getExternalPath().toCharArray(), "getExternalPath()".toCharArray());
        toDisplay = CharOperation.replace(toDisplay, org.eclipse.jdt.core.tests.util.Util.displayString(getExternalJCLSourcePathString(), 0).toCharArray(), "getExternalJCLSourcePathString()".toCharArray());
        toDisplay = CharOperation.replace(toDisplay, org.eclipse.jdt.core.tests.util.Util.displayString(getExternalJCLSourcePathString("1.5"), 0).toCharArray(), "getExternalJCLSourcePathString(\"1.5\")".toCharArray());
        toDisplay = org.eclipse.jdt.core.tests.util.Util.displayString(new String(toDisplay), indent).toCharArray();
        toDisplay = CharOperation.replace(toDisplay, "getWorkspacePath()".toCharArray(), ("\"+ getWorkspacePath() + \"").toCharArray());
        toDisplay = CharOperation.replace(toDisplay, "getExternalJCLPathString()".toCharArray(), ("\"+ getExternalJCLPathString() + \"").toCharArray());
        toDisplay = CharOperation.replace(toDisplay, "getExternalJCLPathString(\\\"1.5\\\")".toCharArray(), ("\"+ getExternalJCLPathString(\"1.5\") + \"").toCharArray());
        toDisplay = CharOperation.replace(toDisplay, "getExternalJCLSourcePathString()".toCharArray(), ("\"+ getExternalJCLSourcePathString() + \"").toCharArray());
        toDisplay = CharOperation.replace(toDisplay, "getExternalJCLSourcePathString(\\\"1.5\\\")".toCharArray(), ("\"+ getExternalJCLSourcePathString(\"1.5\") + \"").toCharArray());
        toDisplay = CharOperation.replace(toDisplay, "getExternalPath()".toCharArray(), ("\"+ getExternalPath() + \"").toCharArray());
    ...
    protected ICompilationUnit newExternalWorkingCopy(String name, final String contents) throws JavaModelException {
        return newExternalWorkingCopy(name, /*no classpath*/
    ...
    protected ICompilationUnit newExternalWorkingCopy(String name, IClasspathEntry[] classpath, final IProblemRequestor problemRequestor, final String contents) throws JavaModelException {
        ...
        return owner.newWorkingCopy(name, classpath, /*no progress monitor*/
        null);
    }
    ...
    protected void refreshExternalArchives(IJavaProject p) throws JavaModelException {
        // ensure that the auto-build job doesn't interfere with external jar refreshing
        waitForAutoBuild();
    ...
    public void setupExternalJCL(String jclName) throws IOException {
        ...
        java.io.File jclMin = new java.io.File(externalPath + jclName + ".jar");
        ...
        if (!jclDir.exists()) {
            ...
            java.io.File resourceJCLMin = new java.io.File(resourceJCLDir + separator + jclName + ".jar");
            copy(resourceJCLMin, jclMin);
            java.io.File resourceJCLMinsrc = new java.io.File(resourceJCLDir + separator + jclName + "src.zip");
    ...
    protected void setUpProjectCompliance(IJavaProject javaProject, String compliance, boolean useFullJCL) throws JavaModelException, IOException {
        ...
        // assume that the values of CompilerOptions.VERSION_* are used
        String version = compliance;
        ...
        for (int i = 0, length = classpath.length; i < length; i++) {
            ...
            if (isJCLPath(path) && !path.equals(newJclLibString)) {
                classpath[i] = JavaCore.newVariableEntry(new Path(newJclLibString), new Path(newJclSrcString), entry.getSourceAttachmentRootPath(), entry.getAccessRules(), new IClasspathAttribute[0], entry.isExported());
                break;
            }
        }
        javaProject.setRawClasspath(classpath, null);
    ...
    public void setUpJCLClasspathVariables(String compliance, boolean useFullJCL) throws JavaModelException, IOException {
        if ("1.5".equals(compliance) || "1.6".equals(compliance)) {
            if (JavaCore.getClasspathVariable("JCL15_LIB") == null) {
                ...
                JavaCore.setClasspathVariables(new String[] { "JCL15_LIB", "JCL15_SRC", "JCL_SRCROOT" }, new IPath[] { getExternalJCLPath("1.5"), getExternalJCLSourcePath("1.5"), getExternalJCLRootSourcePath() }, null);
            }
        } else if ("1.7".equals(compliance)) {
            if (JavaCore.getClasspathVariable("JCL17_LIB") == null) {
                setupExternalJCL("jclMin1.7");
                JavaCore.setClasspathVariables(new String[] { "JCL17_LIB", "JCL17_SRC", "JCL_SRCROOT" }, new IPath[] { getExternalJCLPath("1.7"), getExternalJCLSourcePath("1.7"), getExternalJCLRootSourcePath() }, null);
            }
        } else if ("1.8".equals(compliance)) {
            if (useFullJCL) {
                if (JavaCore.getClasspathVariable("JCL18_FULL") == null) {
                    // Create the whole mininmal 1.8 set, though we will need only the source zip
                    setupExternalJCL("jclMin1.8");
                    setupExternalJCL("jclFull1.8");
                    JavaCore.setClasspathVariables(new String[] { "JCL18_FULL", "JCL18_SRC", "JCL_SRCROOT" }, new IPath[] { new Path(getExternalJCLPathString("1.8", true)), getExternalJCLSourcePath("1.8"), getExternalJCLRootSourcePath() }, null);
                }
            } else if (JavaCore.getClasspathVariable("JCL18_LIB") == null) {
                setupExternalJCL("jclMin1.8");
                JavaCore.setClasspathVariables(new String[] { "JCL18_LIB", "JCL18_SRC", "JCL_SRCROOT" }, new IPath[] { getExternalJCLPath("1.8"), getExternalJCLSourcePath("1.8"), getExternalJCLRootSourcePath() }, null);
            }
        } else {
            if (JavaCore.getClasspathVariable("JCL_LIB") == null) {
                setupExternalJCL("jclMin");
                JavaCore.setClasspathVariables(new String[] { "JCL_LIB", "JCL_SRC", "JCL_SRCROOT" }, new IPath[] { getExternalJCLPath(), getExternalJCLSourcePath(), getExternalJCLRootSourcePath() }, null);
            }
        }
    }
    ...
    protected void sortElements(IJavaElement[] elements) {
        Util.Comparer comparer = new Util.Comparer() {
            ...
            public int compare(Object a, Object b) {
                ...
                String idA = new String(CharOperation.replace(elementA.toStringWithAncestors().toCharArray(), getExternalJCLPathString().toCharArray(), tempJCLPath));
                String idB = new String(CharOperation.replace(elementB.toStringWithAncestors().toCharArray(), getExternalJCLPathString().toCharArray(), tempJCLPath));
    ...
    protected void startLogListening(ILog logToListen) {
        // cleanup if we forgot to stop listening
        stopLogListening();
    ...
    protected void assertLogEquals(String expected) {
        ...
        assertSourceEquals("Unexpected entry in log", expected, actual);
    }
    ...
    protected void touch(File f) {
        ...
        int n = 1;
        while (// retry 9 times more if necessary
        n < 10) {
    ...
    protected String toString(String[] strings) {
        return org.eclipse.jdt.core.tests.util.Util.toString(strings, /*don't add extra new line*/
        false);
    }
    ...
    protected void tearDown() throws Exception {
        ...
        assertEquals("Workspace options should be back to their default", new CompilerOptions(defaultOptions).toString(), new CompilerOptions(options).toString());
    }
