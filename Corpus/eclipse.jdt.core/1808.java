...
public abstract class Scope {
	...
	*/
    ...
    public static ReferenceBinding[] greaterLowerBound(ReferenceBinding[] types) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < length; j++) {
                ...
                if (jType == null)
                    continue;
                if (// if Vi <: Vj, Vj is removed
                iType.isCompatibleWith(jType)) {
                    if (// defensive copy
                    result == types) {
    ...
    // 5.1.10
    public static TypeBinding[] greaterLowerBound(TypeBinding[] types, /*@Nullable*/
    Scope scope, LookupEnvironment environment) {
        if (types == null)
            return null;
        int length = types.length;
        if (length == 0)
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < length; j++) {
                ...
                if (jType == null)
                    continue;
                if (// if Vi <: Vj, Vj is removed
                iType.isCompatibleWith(jType, scope)) {
                    if (// defensive copy
                    result == types) {
    ...
    public static class Substitutor {
        ...
        public TypeBinding substitute(Substitution substitution, TypeBinding originalType) {
            ...
            switch(originalType.kind()) {
                ...
                case Binding.PARAMETERIZED_TYPE:
                    ...
                    if (//$IDENTITY-COMPARISON$
                    substitutedArguments != originalArguments || substitutedEnclosing != originalEnclosing) {
                ...
                case Binding.ARRAY_TYPE:
                    ArrayBinding originalArrayType = (ArrayBinding) originalType;
                    TypeBinding originalLeafComponentType = originalArrayType.leafComponentType;
                    // substitute could itself be array type, TODO(Srikanth): need a test case.
                    TypeBinding substitute = substitute(substitution, originalLeafComponentType);
                    if (//$IDENTITY-COMPARISON$
                    substitute != originalLeafComponentType) {
                ...
                case Binding.INTERSECTION_TYPE:
                    ...
                    if (wildcard.boundKind != Wildcard.UNBOUND) {
                        ...
                        if (//$IDENTITY-COMPARISON$
                        substitutedBound != originalBound || originalOtherBounds != substitutedOtherBounds) {
                ...
                case Binding.TYPE:
                    ...
                    if (//$IDENTITY-COMPARISON$
                    substitutedEnclosing != originalEnclosing && !originalType.isStatic()) {
                ...
                case Binding.GENERIC_TYPE:
                    ...
                    substitutedEnclosing = originalEnclosing;
                    if (originalEnclosing != null) {
                        substitutedEnclosing = (ReferenceBinding) (originalType.isStatic() ? substitution.environment().convertToRawType(originalEnclosing, true) : (ReferenceBinding) substitute(substitution, originalEnclosing));
                        if (isMemberTypeOfRaw(originalType, substitutedEnclosing))
                            return substitution.environment().createRawType(originalReferenceType, substitutedEnclosing, originalType.getTypeAnnotations());
                    }
                    if (substitution.isRawSubstitution()) {
        ...
        public TypeBinding[] substitute(Substitution substitution, TypeBinding[] originalTypes) {
            ...
            for (int i = 0, length = originalTypes.length; i < length; i++) {
                ...
                if (//$IDENTITY-COMPARISON$
                substitutedParameter != originalType) {
                    if (substitutedTypes == originalTypes) {
    ...
    protected final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments, InvocationSite invocationSite, boolean tiebreakingVarargsMethods) {
        ...
        if (parameters == arguments && (method.returnType.tagBits & TagBits.HasTypeVariable) == 0 && genericTypeArguments == null && typeVariables == Binding.NO_TYPE_VARIABLES)
            return method;
        ...
        if (// generic method
        typeVariables != Binding.NO_TYPE_VARIABLES && compilerOptions.sourceLevel >= ClassFileConstants.JDK1_5) {
            TypeBinding[] newArgs = null;
            if (// for 1.8+ inferred calls, we do this inside PGMB.cCM18.
            compilerOptions.sourceLevel < ClassFileConstants.JDK1_8 || genericTypeArguments != null) {
        ...
        if (// Not reached, but left in for now.
        method instanceof PolyParameterizedGenericMethodBinding)
    ...
    protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions) {
        ...
        nextVariable: for (int i = 0; i < paramLength; i++) {
            ...
            TypeBinding superType = this.kind == METHOD_SCOPE ? typeRef.resolveType((BlockScope) this, /*no bound check*/
            false, Binding.DefaultLocationTypeBound) : typeRef.resolveType((ClassScope) this, Binding.DefaultLocationTypeBound);
            if (superType == null) {
            ...
            if (boundRefs != null) {
                nextBound: for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
                    typeRef = boundRefs[j];
                    superType = this.kind == METHOD_SCOPE ? typeRef.resolveType((BlockScope) this, false) : typeRef.resolveType((ClassScope) this);
                    if (superType == null) {
                        typeVariable.tagBits |= TagBits.HierarchyHasProblems;
                        continue nextBound;
                    } else {
        ...
        if (declaresNullTypeAnnotation)
            for (int i = 0; i < paramLength; i++) // <T extends List<U>, @NonNull U> --> tag T as having null type annotations 
	...
	*/
    ...
    public final LookupEnvironment environment() {
        ...
        while ((scope = unitScope.parent) != null) unitScope = scope;
                        ...
                        if (sourceLevel18 || !(compatibleMethod.isVarargs() && compatibleMethod instanceof ParameterizedGenericMethodBinding)) {
                            for (int j = 0; j < startFoundSize; j++) {
                                ...
                                if (classMethod != null && methodVerifier.areMethodsCompatible(classMethod, compatibleMethod))
                                    // can skip this method since classMethod overrides it
                                    continue next;
                            }
                        }
    ...
    public MethodBinding findExactMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite) {
        ...
        if (exactMethod != null && exactMethod.typeVariables == Binding.NO_TYPE_VARIABLES && !exactMethod.isBridge()) {
            ...
            if (exactMethod.isAbstract() && exactMethod.thrownExceptions != Binding.NO_EXCEPTIONS)
                // may need to merge exceptions with interface method
                return null;
	...
	*/
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve, boolean invisibleFieldsOk) {
        ...
        if (field != null) {
            ...
            if (invocationSite == null || insideTypeAnnotations ? field.canBeSeenBy(getCurrentPackage()) : field.canBeSeenBy(currentType, invocationSite, this))
                return field;
            return new ProblemFieldBinding(/* closest match*/
            field, field.declaringClass, fieldName, ProblemReasons.NotVisible);
        }
            ...
            if ((field = currentType.getField(fieldName, needResolve)) != null) {
                ...
                if (field.canBeSeenBy(receiverType, invocationSite, this)) {
                    if (visibleField == null)
                        ...
                        return new ProblemFieldBinding(/* closest match*/
                        visibleField, visibleField.declaringClass, fieldName, ProblemReasons.Ambiguous);
                } else {
        ...
        if (interfacesToVisit != null) {
            ProblemFieldBinding ambiguous = null;
                ...
                if ((field = anInterface.getField(fieldName, /*resolve*/
                true)) != null) {
                    if (invisibleFieldsOk) {
                        return field;
                    }
                    if (visibleField == null) {
                        ...
                        ambiguous = new ProblemFieldBinding(/* closest match*/
                        visibleField, visibleField.declaringClass, fieldName, ProblemReasons.Ambiguous);
                        break done;
                    }
                } else {
    ...
    public ReferenceBinding findMemberType(char[] typeName, ReferenceBinding enclosingType) {
        ...
        while (keepLooking) {
            ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
            if (// needed for statically imported types which don't know their hierarchy yet
            itsInterfaces == null) {
    ...
    public MethodBinding findMethod0(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, boolean inStaticContext) {
        ...
        while (currentType != null) {
            ...
            if (currentLength > 0) {
                if (isCompliant14 && (receiverTypeIsInterface || found.size > 0)) {
                    nextMethod: for (// currentLength can be modified inside the loop
                    int i = 0, l = currentLength; // currentLength can be modified inside the loop
                    i < l; // currentLength can be modified inside the loop
                    i++) {
                        MethodBinding currentMethod = currentMethods[i];
                        if (currentMethod == null)
                            continue nextMethod;
                        if (// only public methods from Object are visible to interface receiverTypes
                        receiverTypeIsInterface && !currentMethod.isPublic()) {
                            currentLength--;
        ...
        if (candidatesCount == 0) {
            ...
            if (interfaceMethod != null) {
                if (soureLevel18 && foundSize > 0 && interfaceMethod.isVarargs() && interfaceMethod instanceof ParameterizedGenericMethodBinding) {
                    ...
                    for (int i = 0; i < foundSize; i++) {
                        ...
                        if (// this check shouldn't matter, but to compatible with javac...
            ...
            nextMethod: for (int i = 0; i < foundSize; i++) {
                ...
                next: for (int a = 0; a < argLength; a++) {
                    ...
                    for (// look one slot before & after to see if the type matches
                    int p = a == 0 ? 0 : a - 1; // look one slot before & after to see if the type matches
                    p < paramLength && p < a + 1; // look one slot before & after to see if the type matches
                    p++) {
        ...
        if (complianceLevel <= ClassFileConstants.JDK1_3) {
            ...
            return !declaringClass.isInterface() ? mostSpecificClassMethodBinding(candidates, visiblesCount, invocationSite) : mostSpecificInterfaceMethodBinding(candidates, visiblesCount, invocationSite);
        }
                ...
                if (candidate.hasSubstitutedParameters()) {
                    for (int j = i + 1; j < visiblesCount; j++) {
                        ...
                        if (otherCandidate.hasSubstitutedParameters()) {
                            if (otherCandidate == candidate || (TypeBinding.equalsEquals(candidate.declaringClass, otherCandidate.declaringClass) && candidate.areParametersEqual(otherCandidate))) {
        ...
        if (inStaticContext) {
            ...
            int staticCount = 0;
            for (int i = 0; i < visiblesCount; i++) if (candidates[i].isStatic())
        ...
        return searchForDefaultAbstractMethod ? findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, classHierarchyStart, found, candidates) : mostSpecificMethodBinding(candidates, visiblesCount, argumentTypes, invocationSite, receiverType);
    }
                ...
                if (currentMethods.length > 0) {
                    ...
                    next: for (int c = 0, l = currentMethods.length; c < l; c++) {
                        ...
                        if (foundSize > 0) {
                            ...
                            for (int f = 0; f < foundSize; f++) if (current == found.elementAt(f))
                                continue next;
                        }
                        found.add(current);
                    }
    ...
    public Binding getBinding(char[] name, int mask, InvocationSite invocationSite, boolean needResolve) {
        ...
        try {
            ...
            if ((mask & Binding.VARIABLE) != 0) {
                ...
                done: // done when a COMPILATION_UNIT_SCOPE is found
                while (true) {
                    switch(scope.kind) {
                        case METHOD_SCOPE:
                        ...
                        case BLOCK_SCOPE:
                            ...
                            if (variableBinding != null) {
                                if (foundField != null && foundField.isValidBinding())
                                    return new ProblemFieldBinding(// closest match
                                    foundField, foundField.declaringClass, name, ProblemReasons.InheritedNameHidesEnclosingName);
                                if (depth > 0)
                                    ...
                                    if (fieldBinding.isValidBinding()) {
                                        if (!fieldBinding.isStatic()) {
                                            if (insideConstructorCall) {
                                                insideProblem = new ProblemFieldBinding(// closest match
                                                fieldBinding, fieldBinding.declaringClass, name, ProblemReasons.NonStaticReferenceInConstructorInvocation);
                                            } else if (insideStaticContext) {
                                                insideProblem = new ProblemFieldBinding(// closest match
                                                fieldBinding, fieldBinding.declaringClass, name, ProblemReasons.NonStaticReferenceInStaticContext);
                                            }
                                        }
                                        if (TypeBinding.equalsEquals(receiverType, fieldBinding.declaringClass) || compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4) {
                                            ...
                                            if (foundField.isValidBinding())
                                                ...
                                                if (TypeBinding.notEquals(foundField.declaringClass, fieldBinding.declaringClass) && // https://bugs.eclipse.org/bugs/show_bug.cgi?id=316956
                                    ...
                                    if (temp != null) {
                                        if (!temp.isValidBinding()) {
                                            ...
                                            if (foundInImport)
                                                ...
                                                return new ProblemFieldBinding(// closest match
                                                foundField, foundField.declaringClass, name, ProblemReasons.Ambiguous);
                                            foundField = temp;
                                            foundInImport = true;
                                        }
                                    }
    ...
    public MethodBinding getExactMethod(TypeBinding receiverType, char[] selector, InvocationSite invocationSite) {
        ...
        if (exactMethod.declaringClass.id == TypeIds.T_JavaLangObject && CharOperation.equals(selector, TypeConstants.GETCLASS) && exactMethod.returnType.isParameterizedType()) {
            return environment().createGetClassMethod(receiverType, exactMethod, this);
        }
    ...
    public MethodBinding getExactConstructor(TypeBinding receiverType, InvocationSite invocationSite) {
        ...
        if (receiverType.isArrayType()) {
            ...
            if (!leafType.canBeSeenBy(this) || !leafType.isReifiable())
                return null;
            return new MethodBinding(ClassFileConstants.AccPublic | ClassFileConstants.AccSynthetic, TypeConstants.INIT, receiverType, new TypeBinding[] { TypeBinding.INT }, Binding.NO_EXCEPTIONS, // just lie.
            getJavaLangObject());
        }
        CompilationUnitScope unitScope = compilationUnitScope();
    ...
    public FieldBinding getField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite) {
        ...
        try {
            env.missingClassFileLocation = invocationSite;
            FieldBinding field = findField(receiverType, fieldName, invocationSite, /*resolve*/
            true);
            if (field != null)
                return field;
            ...
            switch(scope.kind) {
                ...
                case CLASS_SCOPE:
                    ...
                    if (!insideTypeAnnotation) {
                        ...
                        if (// skip it if we did not find anything
                        methodBinding != null) {
                            if (foundMethod == null) {
                                if (methodBinding.isValidBinding()) {
                                    if (!methodBinding.isStatic() && (insideConstructorCall || insideStaticContext)) {
                                        if (foundProblem != null && foundProblem.problemId() != ProblemReasons.NotVisible)
                                            // takes precedence
                                            return foundProblem;
                                        return new ProblemMethodBinding(// closest match
                                        methodBinding, methodBinding.selector, methodBinding.parameters, insideConstructorCall ? ProblemReasons.NonStaticReferenceInConstructorInvocation : ProblemReasons.NonStaticReferenceInStaticContext);
                                    } else if (!methodBinding.isStatic() && methodScope != null) {
                                        tagAsAccessingEnclosingInstanceStateOf(/* type variable access */
                                        receiverType, false);
                                    }
                                ...
                                } else // methodBinding is a problem method
                                {
                                    if (methodBinding.problemId() != ProblemReasons.NotVisible && methodBinding.problemId() != ProblemReasons.NotFound)
                                        ...
                                        return methodBinding;
                                    if (foundProblem == null) {
                                        // hold onto the first not visible/found error and keep the second not found if first is not visible
                            ...
                            {
                                if (methodBinding.problemId() == ProblemReasons.Ambiguous || (TypeBinding.notEquals(foundMethod.declaringClass, methodBinding.declaringClass) && (TypeBinding.equalsEquals(receiverType, methodBinding.declaringClass) || receiverType.getMethods(selector) != Binding.NO_METHODS)))
                                    // NOTE: Unlike fields, a non visible method hides a visible method
                                    return new ProblemMethodBinding(// closest match
                                    methodBinding, selector, argumentTypes, ProblemReasons.InheritedNameHidesEnclosingName);
                            }
                        }
                    }
                    insideTypeAnnotation = false;
	...
	*/
    public final Binding getPackage(char[][] compoundName) {
        ...
        if (!binding.isValidBinding()) {
            ...
            {
                char[][] qName = new char[][] { compoundName[0] };
                return new ProblemReferenceBinding(/* no closest match since search for pkg*/
                qName, null, ProblemReasons.NotFound);
            }
            return binding;
        }
        ...
        while (currentIndex < length) {
            ...
            if (binding == null) {
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), /* no closest match since search for pkg*/
        ...
        return new ProblemReferenceBinding(compoundName, /* no closest match since search for pkg*/
        null, ProblemReasons.NotFound);
    }
	...
	*/
    public final Binding getOnlyPackage(char[][] compoundName) {
        ...
        if (binding == null || !binding.isValidBinding()) {
            char[][] qName = new char[][] { compoundName[0] };
            return new ProblemReferenceBinding(qName, /* no closest match since search for pkg*/
            null, ProblemReasons.NotFound);
        }
        if (!(binding instanceof PackageBinding)) {
            // compoundName does not start with a package
	...
	*/
    public final TypeBinding getType(char[][] compoundName, int typeNameLength) {
        ...
        if (!binding.isValidBinding()) {
            if (binding instanceof PackageBinding) {
                char[][] qName = new char[][] { compoundName[0] };
                return new ProblemReferenceBinding(qName, environment().createMissingType(null, qName), ProblemReasons.NotFound);
            }
        ...
        if (binding instanceof PackageBinding) {
            ...
            while (currentIndex < typeNameLength) {
                ...
                if (binding == null) {
                    ...
                    return new ProblemReferenceBinding(qName, environment().createMissingType(packageBinding, qName), ProblemReasons.NotFound);
                }
                if (!binding.isValidBinding())
                    return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), binding instanceof ReferenceBinding ? (ReferenceBinding) ((ReferenceBinding) binding).closestMatch() : null, binding.problemId());
                if (!(binding instanceof PackageBinding))
        ...
        if (// handles the fall through case
        checkVisibility)
            if (!typeBinding.canBeSeenBy(this))
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), typeBinding, ProblemReasons.NotVisible);
        while (currentIndex < typeNameLength) {
	...
	*/
    final Binding getTypeOrPackage(char[] name, int mask, boolean needResolve) {
        ...
        if ((mask & Binding.TYPE) == 0) {
            Scope next = scope;
            while ((next = scope.parent) != null) scope = next;
        } else {
                    ...
                    case METHOD_SCOPE:
                        ...
                        if (methodDecl != null) {
                            if (methodDecl.binding != null) {
                                ...
                                for (int i = params == null ? 0 : params.length; --i >= 0; ) if (CharOperation.equals(params[i].name, name))
        ...
        if (typeOrPackageCache != null) {
            ...
            if (// can also include NotFound ProblemReferenceBindings if we already know this name is not found
            cachedBinding != null) {
                if (// single type import cached in faultInImports(), replace it in the cache with the type
                cachedBinding instanceof ImportBinding) {
        ...
        if ((mask & Binding.TYPE) != 0) {
            ImportBinding[] imports = unitScope.imports;
            if (// walk single type imports since faultInImports() has not run yet
            imports != null && typeOrPackageCache == null) {
                ...
                for (int i = 0, length = imports.length; i < length; i++) {
                    ...
                    if (someImport.onDemand) {
                        ...
                        if (resolvedImport instanceof PackageBinding) {
                            ...
                            temp = findMemberType(name, (ReferenceBinding) resolvedImport);
        ...
        if (foundType == null) {
            ...
            if (// only put NotFound type in cache if you know its not a package
            typeOrPackageCache != null && (mask & Binding.PACKAGE) != 0) {
    ...
    public final Binding getTypeOrPackage(char[][] compoundName) {
        ...
        // binding is now a ReferenceBinding
        ReferenceBinding typeBinding = (ReferenceBinding) binding;
        ReferenceBinding qualifiedType = (ReferenceBinding) environment().convertToRawType(typeBinding, /*do not force conversion of enclosing types*/
        false);
        if (// handles the fall through case
        checkVisibility)
            if (!typeBinding.canBeSeenBy(this))
    ...
    protected boolean isAcceptableMethod(MethodBinding one, MethodBinding two) {
        ...
        if (one.isVarargs() && two.isVarargs()) {
            if (CompilerOptions.tolerateIllegalAmbiguousVarargsInvocation && this.compilerOptions().complianceLevel < ClassFileConstants.JDK1_7 && oneParamsLength > twoParamsLength) {
            ...
            for (int i = (oneParamsLength > twoParamsLength ? twoParamsLength : oneParamsLength) - 2; i >= 0; i--) if (TypeBinding.notEquals(oneParams[i], twoParams[i]) && !oneParams[i].isCompatibleWith(twoParams[i]))
                return false;
            if (parameterCompatibilityLevel(one, twoParams, true) == NOT_COMPATIBLE && parameterCompatibilityLevel(two, oneParams, true) == VARARGS_COMPATIBLE)
                return true;
        }
        return false;
    }
	...
	*/
    public final boolean isDefinedInSameUnit(ReferenceBinding type) {
        ...
        while ((type = enclosingType.enclosingType()) != null) enclosingType = type;
        ...
        for (int i = topLevelTypes.length; --i >= 0; ) if (TypeBinding.equalsEquals(topLevelTypes[i], enclosingType.original()))
            return true;
        return false;
    }
	...
	*/
        ...
        switch(this.kind) {
            ...
            case Scope.METHOD_SCOPE:
                ...
                if (!methodScope.isInsideInitializer()) {
                    ...
                    MethodBinding context = referenceContext instanceof AbstractMethodDeclaration ? ((AbstractMethodDeclaration) referenceContext).binding : ((LambdaExpression) referenceContext).binding;
    ...
    private TypeBinding leastContainingInvocation(TypeBinding mec, Object invocationData, ArrayList lubStack) {
        ...
        if (invocationData == null)
            return mec;
        if (// only one invocation, simply return it (array only allocated if more than one)
        invocationData instanceof TypeBinding) {
    ...
    private TypeBinding leastContainingTypeArgument(TypeBinding u, TypeBinding v, ReferenceBinding genericType, int rank, ArrayList lubStack) {
                ...
                switch(wildU.boundKind) {
                        ...
                        switch(wildV.boundKind) {
                            ...
                            case Wildcard.EXTENDS:
                                ...
                                if (TypeBinding.equalsEquals(lub, TypeBinding.INT))
                                    return environment().createWildcard(/*no extra bound*/
                                    genericType, rank, null, null, Wildcard.UNBOUND);
                                return environment().createWildcard(/*no extra bound*/
                                genericType, rank, lub, null, Wildcard.EXTENDS);
                            // ? extends U, ? SUPER V
                            case Wildcard.SUPER:
                    ...
                    case Wildcard.SUPER:
                        ...
                        if (wildU.boundKind == Wildcard.SUPER) {
                            ...
                            return environment().createWildcard(genericType, /*no extra bound*/
                            rank, glb[0], null, Wildcard.SUPER);
                        }
                }
            } else {
                switch(wildV.boundKind) {
                    ...
                    case Wildcard.EXTENDS:
                        ...
                        if (TypeBinding.equalsEquals(lub, TypeBinding.INT))
                            return environment().createWildcard(genericType, /*no extra bound*/
                            rank, null, null, Wildcard.UNBOUND);
                        return environment().createWildcard(genericType, /*no extra bound*/
                        rank, lub, null, Wildcard.EXTENDS);
                    // U, ? super V
                    case Wildcard.SUPER:
        ...
        // int is returned to denote cycle detected in lub computation - stop recursion by answering unbound wildcard
        if (TypeBinding.equalsEquals(lub, TypeBinding.INT))
            return environment().createWildcard(genericType, rank, null, /*no extra bound*/
            null, Wildcard.UNBOUND);
        return environment().createWildcard(genericType, rank, lub, /*no extra bound*/
        null, Wildcard.EXTENDS);
    }
    ...
    private TypeBinding lowerUpperBound(TypeBinding[] types, ArrayList lubStack) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (commonDim == -1) {
                commonDim = dim;
            } else if (// not all types have same dimension
            dim != commonDim) {
                return null;
            }
        ...
        // pass common null annotations by synthesized annotation bindings.
    ...
    protected TypeBinding[] minimalErasedCandidates(TypeBinding[] types, Map allInvocations) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (type == TypeBinding.NULL)
                // completely ignore null-type now and further down
                types[i] = type = null;
            if (type == null)
                continue;
            if (type.isBaseType())
        ...
        for (int i = 0; i < max; i++) {
            ...
            ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
            if (// can be null during code assist operations that use LookupEnvironment.completeTypeBindings(parsedUnit, buildFieldsAndMethods)
            itsInterfaces != null) {
            ...
            nextType: for (int i = 0; i < superLength; i++) {
                ...
                nextOtherType: for (int j = 0; j < superLength; j++) {
                    ...
                    if (erasedSuperType instanceof ReferenceBinding) {
                        ...
                        if (// keep Object[...] for an interface array (same dimensions)
                        otherType.isArrayType() && otherType.leafComponentType().id == TypeIds.T_JavaLangObject && otherType.dimensions() == erasedSuperType.dimensions() && erasedSuperType.leafComponentType().isInterface())
                            continue nextOtherType;
                        if (erasedSuperType.findSuperTypeOriginatingFrom(otherType) != null) {
                            // discard non minimal supertype
                            erasedSuperTypes[j] = null;
	...
	*/
    ...
    protected final MethodBinding mostSpecificMethodBinding(MethodBinding[] visible, int visibleSize, TypeBinding[] argumentTypes, final InvocationSite invocationSite, ReferenceBinding receiverType) {
        ...
        for (int i = 0; i < visibleSize; i++) if ((compatibilityLevels[i] = parameterCompatibilityLevel(visible[i], argumentTypes, invocationSite)) != NOT_COMPATIBLE) {
        ...
        if (isJdk18) {
            ...
            nextJ: for (int j = 0; j < visibleSize; j++) {
                ...
                nextK: for (int k = 0; k < visibleSize; k++) {
                    ...
                    if (levelj > -1 && levelk > -1 && levelj != levelk) {
                        if (levelj < levelk)
                            // j is more specific than this k
                            continue nextK;
                        else
                            // j cannot be more specific
                            continue nextJ;
                    }
            ...
            InvocationSite tieBreakInvocationSite = new InvocationSite() {
                ...
                public void setDepth(/* ignore */
                int depth) {
                }

                public void setFieldIndex(/* ignore */
                int depth) {
                }
                ...
                public InferenceContext18 freshInferenceContext(Scope scope) /* no inference when ignoring genericTypeArgs */
                {
                    return null;
                }
                ...
                public void acceptPotentiallyCompatibleMethods(MethodBinding[] /* ignore */
                methods) {
                }
            };
            int count = 0;
        ...
        nextSpecific: for (int i = 0; i < visibleSize; i++) {
            ...
            if (current != null) {
                ...
                MethodBinding original = current.original();
                // only needed when selecting from interface methods
                ...
                for (int j = 0; j < visibleSize; j++) {
                    ...
                    } else if (// should not be null if original isAbstract, but be safe
                    receiverType != null) {
    ...
    public int parameterCompatibilityLevel(MethodBinding method, TypeBinding[] arguments, InvocationSite site) {
        if (method.problemId() == ProblemReasons.InvocationTypeInferenceFailure) {
            // we need to accept methods with InvocationTypeInferenceFailure, because logically overload resolution happens *before* invocation type inference
            // for compatibility checks use the actual method
    ...
    public int parameterCompatibilityLevel(MethodBinding method, TypeBinding[] arguments, boolean tiebreakingVarargsMethods) {
        ...
        if (method.isVarargs()) {
            ...
            paramLength == argLength) {
                ...
                if (// all remaining argument types must be compatible with the elementsType of varArgType
                paramLength < argLength) {
            ...
            switch(current.kind) {
                case METHOD_SCOPE:
                    ...
                    if (context instanceof LambdaExpression) {
                        ...
                        while (expression != expression.original) expression = expression.original;
                        return expression;
                    }
                    return context;
                case CLASS_SCOPE:
    ...
    public boolean deferCheck(Runnable check) {
        if (this.parent != null)
            // only ClassScope potentially records this
    ...
    public MethodBinding getStaticFactory(ParameterizedTypeBinding allocationType, ReferenceBinding originalEnclosingType, TypeBinding[] argumentTypes, final InvocationSite allocationSite) {
        ...
        while (currentType != null) {
            ...
            if (// any enclosing types cannot be parameterized, if generic treat as raw.
        ...
        for (int i = 0, length = methods.length; i < length; i++) {
            ...
            for (int j = 0; j < factoryArity; j++) {
                ...
                if (originalVariable.firstBound != null) {
                    ...
                    firstBound = TypeBinding.equalsEquals(originalVariable.firstBound, originalVariable.superclass) ? // could be array type or interface
                    substitutedSuperclass : substitutedInterfaces[0];
    ...
    public boolean validateNullAnnotation(long tagBits, TypeReference typeRef, Annotation[] annotations) {
        ...
        if (usesNullTypeAnnotations) {
            // if it's an array, the annotation applies to the leaf component type
    ...
    public void tagAsAccessingEnclosingInstanceStateOf(ReferenceBinding enclosingType, boolean typeVariableAccess) {
        ...
        if (methodScope != null && methodScope.referenceContext instanceof TypeDeclaration) {
            if (// unless invoking a method of the local type ...
        ...
        while (methodScope != null) {
            while (methodScope != null && methodScope.referenceContext instanceof LambdaExpression) {
                ...
                if (!typeVariableAccess)
                    // lambda can still be static, only when `this' is touched (implicitly or otherwise) it cannot be.
