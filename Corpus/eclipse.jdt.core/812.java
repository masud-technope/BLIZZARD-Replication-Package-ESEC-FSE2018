 ...
 */
    protected void addAffectedChild(JavaElementDelta child) {
        ...
        if (//new affected child
        existingChildIndex == null) {
            addNewChild(child);
        } else {
 ...
 */
    protected JavaElementDelta createDeltaTree(IJavaElement element, JavaElementDelta delta) {
        ...
        if (ancestors == null) {
            if (// handle case of two jars that can be equals but not in the same project
            equalsAndSameParent(delta.getElement(), getElement())) {
 ...
 */
    protected JavaElementDelta find(IJavaElement e) {
        if (// handle case of two jars that can be equals but not in the same project
        equalsAndSameParent(getElement(), e))
            return this;
 ...
 */
    protected void insertDeltaTree(IJavaElement element, JavaElementDelta delta) {
        ...
        if (// handle case of two jars that can be equals but not in the same project
        !equalsAndSameParent(element, getElement())) {
