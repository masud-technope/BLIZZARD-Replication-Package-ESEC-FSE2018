...
public class ReconcileWorkingCopyOperation extends JavaModelOperation {
            ...
            for (Iterator iteraror = this.problems.values().iterator(); iteraror.hasNext(); ) {
                ...
                for (int i = 0, length = categorizedProblems.length; i < length; i++) {
                    ...
                    if (JavaModelManager.VERBOSE) {
                        ...
                        "PROBLEM FOUND while reconciling : " + //$NON-NLS-1$
                        problem.getMessage());
                    }
    ...
    public org.eclipse.jdt.core.dom.CompilationUnit makeConsistent(CompilationUnit workingCopy) throws JavaModelException {
        ...
        try {
            ...
            if (this.astLevel != ICompilationUnit.NO_AST && unit != null) /*unit is null if working copy is consistent && (problem detection not forced || non-Java project) -> don't create AST as per API*/
            {
                Map options = workingCopy.getJavaProject().getOptions(true);
                // convert AST
                this.ast = AST.convertCompilationUnit(this.astLevel, unit, options, this.resolveBindings, source, this.reconcileFlags, this.progressMonitor);
                if (this.ast != null) {
