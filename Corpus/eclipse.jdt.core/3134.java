...
public class RawTypeBinding extends ParameterizedTypeBinding {
    ...
    public  RawTypeBinding(ReferenceBinding type, ReferenceBinding enclosingType, LookupEnvironment environment) {
        ...
        if (enclosingType == null || (enclosingType.modifiers & ExtraCompilerModifiers.AccGenericSignature) == 0) {
            // only need signature if enclosing needs one
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        if (isMemberType() && (enclosingType().isParameterizedType() || enclosingType().isRawType())) {
            if (isStatic()) {
                // don't consider generics from enclosing of static member
                char[] typeSig = enclosingType().signature();
                // copy all but trailing semicolon
                sig.append(typeSig, 0, typeSig.length - 1);
                // for consistency with keys prior to https://bugs.eclipse.org/460491
                sig.append('$');
            } else {
                char[] typeSig = enclosingType().computeUniqueKey(/*not a leaf*/
                false);
                // copy all but trailing semicolon
                sig.append(typeSig, 0, typeSig.length - 1);
                sig.append('.');
            ...
            if (// don't show <> if raw only due to a generic enclosing
            genericType().typeVariables() != Binding.NO_TYPE_VARIABLES)
                sig.append('<').append('>');
            sig.append(';');
        } else {
            sig.append(genericType().computeUniqueKey(/*not a leaf*/
            false));
            //$NON-NLS-1$
    ...
    public String debugName() {
        ...
        //$NON-NLS-1$
        nameBuffer.append(actualType().sourceName()).append("#RAW");
    ...
    public boolean isProvablyDistinct(TypeBinding otherType) {
        if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=329588
        TypeBinding.equalsEquals(this, otherType) || TypeBinding.equalsEquals(erasure(), otherType))
            return false;
        if (otherType == null)
            return true;
        switch(otherType.kind()) {
    ...
    protected void initializeArguments() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            typeArguments[i] = this.environment.convertToRawType(typeVariables[i].erasure(), /*do not force conversion of enclosing types*/
            false);
        }
