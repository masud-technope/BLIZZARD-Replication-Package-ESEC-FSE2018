...
public class ASTConverterJavadocTest extends ConverterTestSetup {
    ...
    public  ASTConverterJavadocTest(String name) {
        this(name.substring(0, name.indexOf(" - ")), name.substring(name.indexOf(" - Doc ") + 7, name.lastIndexOf("abled") + 5), name.indexOf(" - Unix") != -1 ? "true" : "false");
    }
    ...
    private char getNextChar(char[] source, int idx) {
        ...
        if (ch == '\\' && source[idx + 1] == 'u') {
            ...
            if (((c1 = ScannerHelper.getHexadecimalValue(source[idx + charLength++])) > 15 || c1 < 0) || ((c2 = ScannerHelper.getHexadecimalValue(source[idx + charLength++])) > 15 || c2 < 0) || ((c3 = ScannerHelper.getHexadecimalValue(source[idx + charLength++])) > 15 || c3 < 0) || ((c4 = ScannerHelper.getHexadecimalValue(source[idx + charLength++])) > 15 || c4 < 0)) {
                return ch;
            }
    ...
    char[] getUnicodeSource(char[] source) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (source[i] == '\\' && source[i + 1] == 'u') {
                ...
                unicodeSource[u++] = source[++i];
                if (((c1 = ScannerHelper.getHexadecimalValue(source[i + 1])) > 15 || c1 < 0) || ((c2 = ScannerHelper.getHexadecimalValue(source[i + 2])) > 15 || c2 < 0) || ((c3 = ScannerHelper.getHexadecimalValue(source[i + 3])) > 15 || c3 < 0) || ((c4 = ScannerHelper.getHexadecimalValue(source[i + 4])) > 15 || c4 < 0)) {
    ...
    private void verifyPositions(TagElement tagElement, char[] source) {
        ...
        while (elements.hasNext()) {
            ...
            if (fragment.getNodeType() == ASTNode.TEXT_ELEMENT) {
                if (previousFragment == null && TagElement.TAG_PARAM.equals(tagName) && ((TextElement) fragment).getText().equals("<")) {
                    int start = tagStart;
                    ...
                    if (previousFragment == null) {
                        ...
                        if (previousFragment.getNodeType() == ASTNode.TEXT_ELEMENT) {
                            assumeTrue(this.prefix + "Wrong length at <" + previousFragment.getStartPosition() + "> for text element " + previousFragment, (source[tagStart] == '\r' || source[tagStart] == '\n'));
                            while (source[tagStart] == '*' || Character.isWhitespace(source[tagStart])) {
                                tagStart++;
                            }
    ...
    protected void verifyComments(String test) throws JavaModelException {
        ICompilationUnit[] units = getCompilationUnits("Converter", "src", "javadoc." + test);
    ...
    protected CompilationUnit verifyComments(String fileName, char[] source, Map options) {
        ...
        if (this.compilerOption.equals(JavaCore.ERROR)) {
            assumeEquals(this.prefix + "Unexpected problems", 0, compilUnit.getProblems().length);
    ...
    protected void verifyMapper(String folder, int count, int[] indexes) throws JavaModelException {
        ICompilationUnit[] units = getCompilationUnits("Converter", "src", "javadoc." + folder);
        for (int i = 0; i < units.length; i++) {
            ...
            assumeEquals(this.prefix + "Wrong number of problems", 0, compilUnit.getProblems().length);
            ...
            assumeTrue("Not a method declaration", node.getNodeType() == ASTNode.METHOD_DECLARATION);
    ...
    public void testBug54776() throws JavaModelException {
        ...
        assumeNotNull("We should get a non-null ast node", node);
        assumeTrue("Not a type declaration", node.getNodeType() == ASTNode.TYPE_DECLARATION);
    ...
    public void testBug55221a() throws JavaModelException {
        ...
        node = method.getBody();
        assumeNotNull("We should get a non-null ast node", node);
        assumeTrue("Not a block", node.getNodeType() == ASTNode.BLOCK);
    ...
    public void testBug55221d() throws JavaModelException {
        ...
        assumeNotNull("We should get a non-null ast node", node);
        assumeTrue("Not return type", node.getNodeType() == ASTNode.PRIMITIVE_TYPE);
    ...
    public void testBug55223a() throws JavaModelException {
        ...
        ASTNode node = getASTNode(compilUnit, 0, 0);
        assumeNotNull("We should get a non-null ast node", node);
        assumeEquals("Not a method declaration", ASTNode.METHOD_DECLARATION, node.getNodeType());
        MethodDeclaration method = (MethodDeclaration) node;
        node = method.getBody();
        assumeNotNull("We should get a non-null ast node", node);
        assumeEquals("Not a block", ASTNode.BLOCK, node.getNodeType());
        Block block = (Block) node;
        Iterator statements = block.statements().iterator();
        int idx = 0;
        while (statements.hasNext()) {
            node = (ExpressionStatement) statements.next();
            assumeEquals("Not a block", ASTNode.EXPRESSION_STATEMENT, node.getNodeType());
    ...
    public void testBug55223b() throws JavaModelException {
        ...
        node = method.getReturnType();
        assumeNotNull("We should get a non-null ast node", node);
        assumeTrue("Not return type", node.getNodeType() == ASTNode.SIMPLE_TYPE);
    ...
    public void testBug50898() throws JavaModelException {
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug50898", "Test.java");
        this.packageBinding = false;
        verifyComments(unit);
    }

    public void testBug51226() throws JavaModelException {
        ICompilationUnit[] units = getCompilationUnits("Converter", "src", "javadoc.testBug51226");
        for (int i = 0; i < units.length; i++) {
            ASTNode result = runConversion(units[i], false);
            final CompilationUnit unit = (CompilationUnit) result;
            assumeEquals(this.prefix + "Wrong number of problems", 0, unit.getProblems().length);
    ...
    public void testBug53075() throws JavaModelException {
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug53075", "X.java");
    ...
    public void testBug54424() throws JavaModelException {
        this.stopOnFailure = false;
        String[] unbound = { "tho", "A#getList(int,long,boolean)", "#getList(Object,java.util.AbstractList)" };
    ...
    public void testBug51660() throws JavaModelException {
        this.stopOnFailure = false;
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug51660", "Test.java");
        CompilationUnit compilUnit = verifyComments(unit);
        if (this.docCommentSupport.equals(JavaCore.ENABLED)) {
            String[] tagNames = { "@ejb", "@ejb", "@ejb", "@ejb", "@ejb", "@ejb", "@ejb(bean", "@ejb)bean", "@ejb", "@ejb+bean", "@ejb,bean", "@ejb-bean", "@ejb.bean", "@ejb/bean", "@ejb", "@ejb;bean", "@ejb", "@ejb=bean", "@ejb", "@ejb?bean", "@ejb@bean", "@ejb[bean", "@ejb\\bean", "@ejb]bean", "@ejb^bean", "@ejb`bean", "@ejb{bean", "@ejb|bean", "@ejb", "@ejb~bean", "@unknown" };
            String[] tagTexts = { "!bean test non-java id character '!' (val=33) in tag name", "\"bean test non-java id character '\"' (val=34) in tag name", "#bean test non-java id character '#' (val=35) in tag name", "%bean test non-java id character '%' (val=37) in tag name", "&bean test non-java id character '&' (val=38) in tag name", "'bean test non-java id character ''' (val=39) in tag name", " test non-java id character '(' (val=40) in tag name", " test non-java id character ')' (val=41) in tag name", "*bean test non-java id character '*' (val=42) in tag name", " test non-java id character '+' (val=43) in tag name", " test non-java id character ',' (val=44) in tag name", " test non-java id character '-' (val=45) in tag name", " test non-java id character '.' (val=46) in tag name", " test non-java id character '/' (val=47) in tag name", ":bean test non-java id character ':' (val=58) in tag name", " test non-java id character ';' (val=59) in tag name", "<bean test non-java id character '<' (val=60) in tag name", " test non-java id character '=' (val=61) in tag name", ">bean test non-java id character '>' (val=62) in tag name", " test non-java id character '?' (val=63) in tag name", " test non-java id character '@' (val=64) in tag name", " test non-java id character '[' (val=91) in tag name", " test non-java id character '\\' (val=92) in tag name", " test non-java id character ']' (val=93) in tag name", " test non-java id character '^' (val=94) in tag name", " test non-java id character '`' (val=96) in tag name", " test non-java id character '{' (val=123) in tag name", " test non-java id character '|' (val=124) in tag name", "}bean test non-java id character '}' (val=125) in tag name", " test non-java id character '~' (val=126) in tag name", " test java id" };
            Comment comment = (Comment) compilUnit.getCommentList().get(0);
            assumeTrue(this.prefix + "Comment should be a javadoc comment ", comment.isDocComment());
            Javadoc docComment = (Javadoc) comment;
    ...
    public void testBug94150() throws JavaModelException {
        ...
        if (this.docCommentSupport.equals(JavaCore.ENABLED)) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // Do not need to verify following statement as we know it's ok as verifyComments did not fail
                Javadoc docComment = (Javadoc) compilUnit.getCommentList().get(i);
    ...
    public void testBug103304() throws JavaModelException {
        // do NOT verify that qualification only can be package name
        this.packageBinding = false;
        this.workingCopies = new ICompilationUnit[1];
    ...
    public void testBug228648() throws JavaModelException {
        ...
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug228648", "A.java");
        verifyComments(unit);
    }
    ...
    public void testBug336821() throws JavaModelException {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug336821", "Try.java");
        verifyComments(unit);
    }

    public void testBug347100() throws Exception {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        ICompilationUnit unit = getCompilationUnit("Converter", "src", "javadoc.testBug347100", "X.java");
        CompilationUnit compilUnit = verifyComments(unit);
        if (this.docCommentSupport.equals(JavaCore.ENABLED)) {
    ...
    public void testBug481143a() throws JavaModelException {
        this.workingCopies = new ICompilationUnit[1];
        this.workingCopies[0] = getWorkingCopy("/Converter15/src/javadoc/a/X.java", "package a;\n" + "public class X {\n" + "	public X subtest = new X(\"Test1\", // comment\n" + "			\"Test2\") { };\n" + "	public X(String foo, String bar) { }\n" + "}");
