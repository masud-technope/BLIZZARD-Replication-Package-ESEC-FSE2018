...
public class DeclarationsUtil implements Declarations {
    ...
    public boolean hides(MemberDeclaration sub, MemberDeclaration sup) {
        ...
        if (sub == sup || sub.equals(sup))
            return false;
        if (!((EclipseDeclarationImpl) sub).isBindingBased() || !((EclipseDeclarationImpl) sup).isBindingBased())
            return false;
	 ...
	 */
    private static boolean isVisibleForHiding(final ITypeBinding declaringTarget, final ITypeBinding declaringViewer, final int modifiers) {
