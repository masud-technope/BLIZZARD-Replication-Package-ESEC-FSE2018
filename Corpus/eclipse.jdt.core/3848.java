...
public class DoStatement extends Statement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        flowInfo = condition.analyseCode(currentScope, loopingContext, (action == null ? flowInfo : (flowInfo.mergedWith(loopingContext.initsOnContinue))));
        if (!isConditionOptimizedFalse && continueLabel != null) {
            loopingContext.complainOnFinalAssignmentsInLoop(currentScope, flowInfo);
        }
        // end of loop
        FlowInfo mergedInfo = FlowInfo.mergedOptimizedBranches(loopingContext.initsOnBreak, isConditionOptimizedTrue, flowInfo.initsWhenFalse(), // never consider opt false case for DO loop, since break can always occur (47776)
        false, !isConditionTrue);
    ...
    public StringBuffer printStatement(int indent, StringBuffer output) {
        ...
        output.append("while (");
        //$NON-NLS-1$
        return condition.printExpression(0, output).append(");");
    }
