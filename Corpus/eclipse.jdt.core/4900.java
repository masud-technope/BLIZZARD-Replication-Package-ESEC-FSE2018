...
public class ValueConversionProcessor extends BaseProcessor {
    ...
    private void testCompilerAPIPath(TypeDeclaration test) {
        ...
        if (!Annotation.class.getName().equals(annoTypeName) && !AnnotationWithArray.class.getName().equals(annoTypeName))
            return;
        final Map<AnnotationTypeElementDeclaration, AnnotationValue> elementValues = annotation.getElementValues();
    ...
    private void compare(final String name, final Object actualValue) {
        if (name.length() == 1) {
            ...
            @SuppressWarnings("unchecked") List<AnnotationValue> actualList = (List<AnnotationValue>) actualValue;
            assertArrayValueTypeMatch(name, actualList, expectedElementType, expectedElementValues);
        }
    }

    private void assertValueTypeMatch(final String name, final Object actualValue, final Class<?> expectedType, final Object expectedValue) {
        if (actualValue != null && expectedType != actualValue.getClass()) {
            final Messager msgr = _env.getMessager();
            msgr.printError("type mismatch for member " + name + " expected " + expectedType.getName() + " but got " + actualValue.getClass().getName());
        } else if (!expectedValue.equals(actualValue)) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expectedValue + " but got " + actualValue);
        }
    }

    private void assertArrayValueTypeMatch(final String name, final List<AnnotationValue> actualValues, final Class<?> expectedElementType, final Object[] expectedValues) {
        int i = 0;
    ...
    private void assertValueMatch(final String name, final boolean actual, final boolean expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    }

    private void assertValueMatch(final String name, final byte actual, final byte expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final char actual, final char expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final double actual, final double expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final float actual, final float expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final int actual, final int expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final long actual, final long expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertValueMatch(final String name, final short actual, final short expected) {
        if (actual != expected) {
            final Messager msgr = _env.getMessager();
            msgr.printError("value mismatch for member " + name + " expected " + expected + " but got " + actual);
        }
    ...
    private void assertArrayValueMatch(final String name, final boolean[] actual, final boolean[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (boolean a : actual) {
            if (a != expected[i]) {
                msgr.printError("value mismatch for member " + name + " expected " + expected[i] + " but got " + a);
            }
            i++;
        }
    }

    private void assertArrayValueMatch(final String name, final byte[] actual, final byte[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (byte a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final char[] actual, final char[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (char a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final double[] actual, final double[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (double a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final float[] actual, final float[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (float a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final int[] actual, final int[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (int a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final long[] actual, final long[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (long a : actual) {
            if (a != expected[i]) {
    ...
    private void assertArrayValueMatch(final String name, final short[] actual, final short[] expected) {
        int i = 0;
        final Messager msgr = _env.getMessager();
        for (short a : actual) {
            if (a != expected[i]) {
