            ...
            if (CharOperation.equals(attributeName, AttributeNamesConstants.ConstantValueName)) {
                ...
                switch(u1At(relativeOffset)) {
                    ...
                    case ClassFileConstants.StringTag:
                        ...
                        this.constant = StringConstant.fromValue(String.valueOf(utf8At(utf8Offset + 3, u2At(utf8Offset + 1))));
                        break;
                }
            }
 ...
 */
    ...
    protected void toStringContent(StringBuffer buffer) {
        ...
        ((modifiers & ClassFileConstants.AccDeprecated) != 0 ? "deprecated " : Util.EMPTY_STRING) + ((modifiers & 0x0001) == 1 ? "public " : //$NON-NLS-1$
        Util.EMPTY_STRING) + ((modifiers & 0x0002) == 0x0002 ? "private " : //$NON-NLS-1$
        Util.EMPTY_STRING) + ((modifiers & 0x0004) == 0x0004 ? "protected " : //$NON-NLS-1$
        Util.EMPTY_STRING) + ((modifiers & 0x0008) == 0x000008 ? "static " : //$NON-NLS-1$
        Util.EMPTY_STRING) + ((modifiers & 0x0010) == 0x0010 ? "final " : //$NON-NLS-1$
        Util.EMPTY_STRING) + ((modifiers & 0x0040) == 0x0040 ? "volatile " : //$NON-NLS-1$
        Util.EMPTY_STRING) + (//$NON-NLS-1$
        (modifiers & 0x0080) == 0x0080 ? "transient " : Util.EMPTY_STRING)).append(getTypeName()).append(' ').append(getName()).append(' ').append(getConstant()).append('}').toString();
    }
}
