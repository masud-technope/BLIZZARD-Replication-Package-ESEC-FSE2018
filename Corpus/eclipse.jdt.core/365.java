...
public class WrapPreparator extends ASTVisitor {
    ...
    private static final float PREFERRED = 7f / 8;
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        if (!node.isConstructor()) {
            ...
            while (this.tm.get(this.wrapParentIndex).isComment()) this.wrapParentIndex++;
    ...
    public boolean visit(EnumDeclaration node) {
        ...
        if (!enumConstants.isEmpty()) {
            for (EnumConstantDeclaration constant : enumConstants) this.wrapIndexes.add(this.tm.firstIndexIn(constant, -1));
            this.wrapParentIndex = (this.options.alignment_for_enum_constants & Alignment.M_INDENT_ON_COLUMN) > 0 ? this.tm.firstIndexBefore(enumConstants.get(0), TokenNameLBRACE) : this.tm.firstIndexIn(node, -1);
        ...
        if (!this.options.join_wrapped_lines) {
            ...
            while (++i < this.tm.size()) {
                ...
                if (t.tokenType == TokenNameSEMICOLON && commaIndex >= 0 && this.tm.countLineBreaksBetween(this.tm.get(commaIndex), t) == 1) {
    ...
    public boolean visit(MethodInvocation node) {
        ...
        boolean isInvocationChainRoot = !(node.getParent() instanceof MethodInvocation) || node.getLocationInParent() != MethodInvocation.EXPRESSION_PROPERTY;
    ...
    public boolean visit(InfixExpression node) {
        ...
        if ((parent instanceof InfixExpression) && samePrecedence(node, (InfixExpression) parent))
            // this node has been handled higher in the AST
            return true;
        findTokensToWrap(node, 0);
        this.wrapParentIndex = this.wrapIndexes.remove(0);
        this.wrapGroupEnd = this.tm.lastIndexIn(node, -1);
        if ((this.options.alignment_for_binary_expression & Alignment.M_INDENT_ON_COLUMN) != 0 && this.wrapParentIndex > 0)
    ...
    private void findTokensToWrap(InfixExpression node, int depth) {
        ...
        } else if (// always add first operand, it will be taken as wrap parent
        this.wrapIndexes.isEmpty() || !this.options.wrap_before_binary_operator) {
        ...
        for (int i = -1; i < extended.size(); i++) {
            ...
            while (this.tm.get(indexBefore).isComment()) indexBefore--;
    ...
    public boolean visit(ArrayInitializer node) {
        ...
        if (!this.options.join_wrapped_lines && !this.options.insert_new_line_before_closing_brace_in_array_initializer) {
            ...
            if (this.tm.countLineBreaksBetween(this.tm.get(closingBraceIndex - 1), closingBrace) == 1) {
                closingBrace.setWrapPolicy(new WrapPolicy(WrapMode.WHERE_NECESSARY, openingBraceIndex, closingBraceIndex, 0, this.currentDepth, 1, true, false));
            }
        }
        return true;
    }
    ...
    public boolean visit(Assignment node) {
        ...
        if (this.tm.get(rightSideIndex).getLineBreaksBefore() > 0)
            // must be an array initializer in new line because of brace_position_for_array_initializer
            return true;
        int operatorIndex = this.tm.firstIndexBefore(node.getRightHandSide(), -1);
        while (this.tm.get(operatorIndex).isComment()) operatorIndex--;
    ...
    public boolean visit(IfStatement node) {
        ...
        if (!(thenStatement instanceof Block)) {
            boolean keepThenOnSameLine = this.options.keep_then_statement_on_same_line || (this.options.keep_simple_if_on_one_line && elseStatement == null);
    ...
    private void forceContinuousWrapping(ASTNode node, int parentIndex) {
        ...
        for (int i = from; i <= to; i++) {
            ...
            if ((token.getLineBreaksBefore() > 0 || (previous != null && previous.getLineBreaksAfter() > 0)) && (token.getWrapPolicy() == null || token.getWrapPolicy().wrapMode == WrapMode.FORCED)) {
    ...
    private WrapPolicy getWrapPolicy(int wrappingOption, float penaltyMultiplier, boolean isFirst, ASTNode parentNode) {
        ...
        if (isAlreadyWrapped)
            // to avoid triggering top priority wrapping
            isTopPriority = false;
        if (isTopPriority)
            wrapMode = WrapMode.TOP_PRIORITY;
        extraIndent *= this.options.indentation_size;
        return new WrapPolicy(wrapMode, this.wrapParentIndex, this.wrapGroupEnd, extraIndent, this.currentDepth, penaltyMultiplier, isFirst, indentOnColumn);
    }
    ...
    private void preserveExistingLineBreaks() {
        ...
        } else if ((this.kind & CodeFormatter.K_COMPILATION_UNIT) != 0 && this.options.insert_new_line_at_end_of_file_if_missing) {
            last.breakAfter();
        }
    }
    ...
    int getLineBreaksToPreserve(Token token1, Token token2, boolean isBetweenImports) {
        ...
        if (token1 != null && token2 != null)
            // n empty lines = n+1 line breaks, except for file start and end
            toPreserve++;
