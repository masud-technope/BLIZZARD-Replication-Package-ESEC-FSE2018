...
public class RemotePublication extends Publication {
    ...
    private static Log log = LogFactory.getLog(RemotePublication.class.getName());
    ...
    protected  RemotePublication(String id, String year, String month, String stateCode, String serverDirectory, String cabinet) {
    ...
    public boolean getDocument() {
        ...
        while (attempt < _maxAttempts) {
            try {
                attempt++;
                log.info("*** Downloading " + _localFilename + EXTENTION + " attempt " + attempt + " out of " + _maxAttempts);
                if (getRemoteDocument()) {
                    log.info("Success document published " + _localFilename + EXTENTION);
                    return true;
                }
    ...
    protected boolean getRemoteDocument() throws ConvertException {
        ...
        try {
            ...
            String msg = "Convert process timed out retriving TIFF page" + e.getFileName();
            log.warn(msg);
            ...
            String msg = "Could not create the c42pdf List File" + e.getMessage();
            throw new ConvertException(msg);
    ...
    protected void getPages() throws FTPServerTimeoutException, FTPServerException, NoFTPServiceException {
    ...
    private String createC42PDFCommandLine(String documentName) {
        return (Registry.getConvertToolPath() + File.separator + Registry.getConvertToolName() + (" -o " + _workingDir + File.separator + documentName + " -l " + _workingDir + File.separator + _fileList));
    }
    ...
    private void handleMultipleDocuments(File newPdfFile) {
        ...
        String newFilename;
        if (log.isDebugEnabled()) {
            log.debug("Checking to see if " + newPdfFile.getName() + "already exists");
        }
        while (newPdfFile.exists()) {
            documentCount++;
            newFilename = (new StringBuffer(_workingDir).append(File.separator).append(_localFilename).append(documentCount).append(EXTENTION)).toString();
            if (log.isDebugEnabled()) {
    ...
    protected void createListFile() throws createListFileException {
        ...
        try {
            File tiffFileList = new File(_workingDir + File.separator + _fileList);
            if (tiffFileList.exists()) {
                tiffFileList.delete();
            }
            tiffFileListFile = new PrintWriter(new BufferedWriter(new FileWriter(_workingDir + File.separator + _fileList)));
    ...
    protected void renamePDFFile() throws CreatePDFFileException {
        String newFileName = _workingDir + File.separator + _localFilename + EXTENTION;
