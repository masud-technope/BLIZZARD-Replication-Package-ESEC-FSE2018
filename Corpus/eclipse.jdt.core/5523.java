...
public abstract class JavadocTest extends AbstractRegressionTest {
    ...
    protected void setUp() throws Exception {
        super.setUp();
        if (RUN_JAVAC) {
            javadocCommandLineHeader = // PREMATURE replace JAVA_NAME and JAVAC_NAME with locals? depends on potential reuse
            jdkRootDirPath.append("bin").append(JAVADOC_NAME).toString();
        }
    //		SHIFT = true;
    //		INDENT = 3;
    }
    ...
    protected void runNegativeReferenceTest(String[] testFiles, String expected, JavacTestOptions javacTestOptions) {
        String[] completedFiles = testFiles;
        if (!this.useLibrary) {
            completedFiles = new String[testFiles.length + referencedClasses.length];
            System.arraycopy(referencedClasses, 0, completedFiles, 0, referencedClasses.length);
	...
	*/
    protected void writeFiles(String[] testFiles) {
        //.substring(11);
        String classDirName = getClass().getName().substring(getClass().getName().lastIndexOf('.') + 1);
        ...
        createOutputTestDirectory(Character.toUpperCase(testName.charAt(0)) + testName.substring(1));
