...
public class MemoryIndex {
    ...
    void addDocumentNames(String substring, SimpleSet results) {
        ...
        if (// add all new/changed documents
        substring == null) {
            for (int i = 0, l = referenceTables.length; i < l; i++) if (referenceTables[i] != null)
                results.add(paths[i]);
        } else {
            for (int i = 0, l = referenceTables.length; i < l; i++) if (referenceTables[i] != null && ((String) paths[i]).startsWith(substring, 0))
                results.add(paths[i]);
        }
    }
