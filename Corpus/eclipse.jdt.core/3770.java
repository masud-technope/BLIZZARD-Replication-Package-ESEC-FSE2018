...
public class ImplicitNullAnnotationVerifier {
    ...
    public void checkImplicitNullAnnotations(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod, boolean complain, Scope scope) {
        ...
        try {
            ...
            boolean needToApplyReturnNonNullDefault = currentMethod.hasNonNullDefaultFor(Binding.DefaultLocationReturnType, usesTypeAnnotations);
            boolean needToApplyParameterNonNullDefault = currentMethod.hasNonNullDefaultFor(Binding.DefaultLocationParameter, usesTypeAnnotations);
            ...
            if (isInstanceMethod) {
                ...
                findAllOverriddenMethods(currentMethod.original(), currentMethod.selector, paramLen, currentType, new HashSet(), superMethodList);
                // prepare interim storage for nullness info so we don't pollute currentMethod before we know its conflict-free: 
                // index 0 is for the return type
                ...
                for (int i = length; --i >= 0; ) {
                    ...
                    if ((currentSuper.tagBits & TagBits.IsNullnessKnown) == 0) {
                        ...
                        // TODO (stephan) complain=true if currentSuper is source method??
                        checkImplicitNullAnnotations(currentSuper, null, false, scope);
                    }
    ...
    private void findAllOverriddenMethods(MethodBinding original, char[] selector, int suggestedParameterLength, ReferenceBinding currentType, Set ifcsSeen, List result) {
        ...
        if (superclass == null)
            // pseudo root of inheritance, happens in eval contexts
            return;
        ...
        for (int i = 0; i < ifcLen; i++) {
            ...
            if (// process each interface at most once
            ifcsSeen.add(currentIfc.original())) {
    ...
    private void collectOverriddenMethods(MethodBinding original, char[] selector, int suggestedParameterLength, ReferenceBinding superType, Set ifcsSeen, List result) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (MethodVerifier.doesMethodOverride(original, currentMethod, this.environment)) {
                ...
                // when overriding one or more methods from superType don't traverse to transitive superTypes
                added = true;
            }
        }
        if (!added)
    ...
    void checkNullSpecInheritance(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod, boolean hasReturnNonNullDefault, boolean hasParameterNonNullDefault, boolean shouldComplain, MethodBinding inheritedMethod, MethodBinding[] allInheritedMethods, Scope scope, InheritedNonNullnessInfo[] inheritedNonNullnessInfos) {
        ...
        returnType: {
            if (currentMethod.returnType == null || currentMethod.returnType.isBaseType())
                // no nullness for primitive types
                break returnType;
                ...
                if (shouldInherit) {
                    if (inheritedNullnessBits != 0) {
                        ...
                        if (inheritedNonNullnessInfos != null && srcMethod != null) {
                            recordDeferredInheritedNullness(scope, ((MethodDeclaration) srcMethod).returnType, inheritedMethod, Boolean.valueOf(inheritedNullnessBits == TagBits.AnnotationNonNull), inheritedNonNullnessInfos[0]);
                        } else {
                            // no need to defer, record this info now:
                            applyReturnNullBits(currentMethod, inheritedNullnessBits);
                        }
                        // compatible by construction, skip complain phase below
                        break returnType;
                    }
                }
                if (// conflict with inheritance already checked
                hasReturnNonNullDefault && (!useTypeAnnotations || currentMethod.returnType.acceptsNonNullDefault())) {
                    currentNullnessBits = TagBits.AnnotationNonNull;
                    applyReturnNullBits(currentMethod, currentNullnessBits);
                }
            }
            if (shouldComplain) {
                if ((inheritedNullnessBits & TagBits.AnnotationNonNull) != 0 && currentNullnessBits != TagBits.AnnotationNonNull) {
                    if (srcMethod != null) {
                ...
                if (useTypeAnnotations) {
                    // for TVB identity checks inside NullAnnotationMatching.analyze()
        ...
        if (// need to look for type annotations on all parameters:
        useTypeAnnotations)
        ...
        parameterLoop: for (int i = 0; i < length; i++) {
            ...
            if (currentNonNullNess == null) {
                ...
                if (inheritedNonNullNess != null) {
                    if (shouldInherit) {
                        ...
                        if (inheritedNonNullnessInfos != null && srcMethod != null) {
                            recordDeferredInheritedNullness(scope, srcMethod.arguments[i].type, inheritedMethod, inheritedNonNullNess, inheritedNonNullnessInfos[i + 1]);
                        } else {
                            // no need to defer, record this info now:
                            if (!useTypeAnnotations)
                ...
                hasParameterNonNullDefault) {
                    ...
                    if (!useTypeAnnotations)
                        ...
                        // cancel if parameter doesn't accept the default
            ...
            if (shouldComplain) {
                ...
                if (// super parameter is not restricted to @NonNull
                inheritedNonNullNess != Boolean.TRUE && // current parameter is restricted to @NonNull 
                currentNonNullNess == Boolean.TRUE) {
                    // incompatible
                    ...
                    if (inheritedNonNullNess == Boolean.FALSE) {
                        ...
                        if (allInheritedMethods != null) {
                            ...
                            for (MethodBinding one : allInheritedMethods) if (TypeBinding.equalsEquals(inheritedMethod.declaringClass, one.declaringClass) && getParameterNonNullness(one, i, useTypeAnnotations) != Boolean.TRUE)
    ...
    protected void recordDeferredInheritedNullness(Scope scope, ASTNode location, MethodBinding inheritedMethod, Boolean inheritedNonNullness, InheritedNonNullnessInfo nullnessInfo) {
        if (nullnessInfo.inheritedNonNullness != null && nullnessInfo.inheritedNonNullness != inheritedNonNullness) {
            scope.problemReporter().conflictingInheritedNullAnnotations(location, nullnessInfo.inheritedNonNullness.booleanValue(), nullnessInfo.annotationOrigin, inheritedNonNullness.booleanValue(), inheritedMethod);
    ...
    void recordArgNonNullness(MethodBinding method, int paramCount, int paramIdx, Argument currentArgument, Boolean nonNullNess) {
        ...
        if (currentArgument != null) {
            currentArgument.binding.tagBits |= nonNullNess.booleanValue() ? TagBits.AnnotationNonNull : TagBits.AnnotationNullable;
        }
    }
            ...
            if (!areTypesEqual(oneArgs[i], twoArgs[i])) {
                if (oneArgs[i].leafComponentType().isRawType()) {
                    if (oneArgs[i].dimensions() == twoArgs[i].dimensions() && oneArgs[i].leafComponentType().isEquivalentTo(twoArgs[i].leafComponentType())) {
                        ...
                        for (int j = 0; j < i; j++) if (oneArgs[j].leafComponentType().isParameterizedTypeWithActualArguments())
                            return false;
