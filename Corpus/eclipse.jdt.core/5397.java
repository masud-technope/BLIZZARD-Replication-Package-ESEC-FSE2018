...
public class ConditionalExpression extends OperatorExpression implements IPolyExpression {
    ...
    private void computeNullStatus(FlowInfo trueBranchInfo, FlowInfo falseBranchInfo, FlowContext flowContext) {
        ...
        if (// has this status been pre-computed?
        this.ifTrueNullStatus == -1) {
	...
	*/
    public void generateOptimizedBoolean(BlockScope currentScope, CodeStream codeStream, BranchLabel trueLabel, BranchLabel falseLabel, boolean valueRequired) {
        int pc = codeStream.position;
        if (// constant
        (this.constant != Constant.NotAConstant) && (this.constant.typeID() == T_boolean) || ((this.valueIfTrue.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) != T_boolean || // non boolean values
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (use15specifics && TypeBinding.notEquals(valueIfTrueType, valueIfFalseType)) {
            if (valueIfTrueType.isBaseType()) {
                if (valueIfFalseType.isBaseType()) {
                    ...
                    if (// bool ? null : 12 --> Integer
                    valueIfTrueType == TypeBinding.NULL) {
                        valueIfFalseType = // boxing
                        env.computeBoxingType(// boxing
                        valueIfFalseType);
                    } else if (// bool ? 12 : null --> Integer
                    valueIfFalseType == TypeBinding.NULL) {
                        valueIfTrueType = // boxing
                        env.computeBoxingType(// boxing
                        valueIfTrueType);
                    }
                } else {
                    ...
                    if (valueIfTrueType.isNumericType() && unboxedIfFalseType.isNumericType()) {
                        // unboxing
                        valueIfFalseType = // unboxing
                        unboxedIfFalseType;
                    } else if (// bool ? 12 : new Integer(12) --> int
                    valueIfTrueType != TypeBinding.NULL) {
                        valueIfFalseType = // unboxing
                        env.computeBoxingType(// unboxing
                        valueIfFalseType);
                    }
                }
                ...
                if (unboxedIfTrueType.isNumericType() && valueIfFalseType.isNumericType()) {
                    // unboxing
                    valueIfTrueType = // unboxing
                    unboxedIfTrueType;
                } else if (// bool ? new Integer(12) : 12 --> int
                valueIfFalseType != TypeBinding.NULL) {
                    valueIfTrueType = // unboxing
                    env.computeBoxingType(// unboxing
                    valueIfTrueType);
        ...
        TypeBinding.equalsEquals(valueIfTrueType, valueIfFalseType)) {
            ...
            if (TypeBinding.equalsEquals(valueIfTrueType, TypeBinding.BOOLEAN)) {
                ...
                (condConstant = this.condition.optimizedBooleanConstant()) != Constant.NotAConstant) {
    ...
    protected boolean computeConversions(BlockScope scope, TypeBinding targetType) {
        ...
        if (this.originalValueIfTrueType != null && this.originalValueIfTrueType.isValidBinding()) {
            if (this.valueIfTrue.isConstantValueOfTypeAssignableToType(this.originalValueIfTrueType, targetType) || this.originalValueIfTrueType.isCompatibleWith(targetType)) {
                ...
                if (this.valueIfTrue instanceof CastExpression && (this.valueIfTrue.bits & (ASTNode.UnnecessaryCast | ASTNode.DisableUnnecessaryCastCheck)) == 0) {
        ...
        if (this.originalValueIfFalseType != null && this.originalValueIfFalseType.isValidBinding()) {
            if (this.valueIfFalse.isConstantValueOfTypeAssignableToType(this.originalValueIfFalseType, targetType) || this.originalValueIfFalseType.isCompatibleWith(targetType)) {
                ...
                if (this.valueIfFalse instanceof CastExpression && (this.valueIfFalse.bits & (ASTNode.UnnecessaryCast | ASTNode.DisableUnnecessaryCastCheck)) == 0) {
    ...
    public boolean isFunctionalType() {
        // Even if only one arm is functional type, this will require a functional interface target
    ...
    public boolean isPolyExpression() throws UnsupportedOperationException {
        ...
        if (this.expressionContext != ASSIGNMENT_CONTEXT && this.expressionContext != INVOCATION_CONTEXT)
            return false;
        if (// resolution error.
        this.originalValueIfTrueType == null || this.originalValueIfFalseType == null)
            return false;
    ...
    public boolean isCompatibleWith(TypeBinding left, Scope scope) {
        return isPolyExpression() ? this.valueIfTrue.isCompatibleWith(left, scope) && this.valueIfFalse.isCompatibleWith(left, scope) : super.isCompatibleWith(left, scope);
    }

    @Override
    public boolean isBoxingCompatibleWith(TypeBinding targetType, Scope scope) {
        // Note: compatibility check may have failed in just one arm and we may have reached here.
        return isPolyExpression() ? (this.valueIfTrue.isCompatibleWith(targetType, scope) || this.valueIfTrue.isBoxingCompatibleWith(targetType, scope)) && (this.valueIfFalse.isCompatibleWith(targetType, scope) || this.valueIfFalse.isBoxingCompatibleWith(targetType, scope)) : super.isBoxingCompatibleWith(targetType, scope);
    }

    public boolean sIsMoreSpecific(TypeBinding s, TypeBinding t, Scope scope) {
        if (super.sIsMoreSpecific(s, t, scope))
            return true;
        return isPolyExpression() ? this.valueIfTrue.sIsMoreSpecific(s, t, scope) && this.valueIfFalse.sIsMoreSpecific(s, t, scope) : false;
    }
