...
public class MultiLineTestCase extends CommentTestCase {
    ...
    public void testSingleLineComment1() {
        ...
        assertEquals("/*" + DELIMITER + INFIX + "test" + DELIMITER + POSTFIX, testFormat("/*\t\t" + DELIMITER + "*\t test*/"));
    }

    public void testSingleLineComment2() {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        assertEquals("/*" + DELIMITER + INFIX + "test" + DELIMITER + POSTFIX, testFormat(PREFIX + "test" + DELIMITER + "\t" + POSTFIX));
    }

    public void testSingleLineComment3() {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        assertEquals("/*" + DELIMITER + INFIX + "test" + DELIMITER + POSTFIX, testFormat(PREFIX + DELIMITER + "* test\t*/"));
    }

    public void testSingleLineComment4() {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        assertEquals("/*" + DELIMITER + INFIX + "test" + DELIMITER + POSTFIX, testFormat("/*test" + DELIMITER + POSTFIX));
    }

    public void testSingleLineCommentSpace1() {
        //$NON-NLS-1$ //$NON-NLS-2$
        assertEquals(PREFIX + "test" + POSTFIX, testFormat("/*test*/"));
    }

    public void testSingleLineCommentSpace2() {
        //$NON-NLS-1$ //$NON-NLS-2$
        assertEquals(PREFIX + "test" + POSTFIX, testFormat("/*test" + POSTFIX));
    }
    ...
    public void testSingleLineCommentSpace4() {
        //$NON-NLS-1$ //$NON-NLS-2$
        assertEquals(PREFIX + "test test" + POSTFIX, testFormat("/* test   test*/"));
    }

    public void testSingleLineCommentTabs1() {
        //$NON-NLS-1$ //$NON-NLS-2$
        assertEquals(PREFIX + "test test" + POSTFIX, testFormat("/*\ttest\ttest" + POSTFIX));
    }
    ...
    public void testMultiLineCommentAsterisk1() {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        String input = PREFIX + INFIX + "test1" + DELIMITER + "test2" + INFIX + DELIMITER + "test3" + DELIMITER + "test4" + POSTFIX;
        ...
        //$NON-NLS-1$
        assertTrue(result.indexOf("test4") != -1);
    }
    ...
    public void test75460() {
        ...
        String input = "/*" + DELIMITER + "            Object[] objects = new Object[3];" + DELIMITER + "            objects[0] = new String(\"Hallo Welt !!!\");" + DELIMITER + "            objects[1] = new String(\"Test !!!\");" + DELIMITER + "            objects[2] = new Integer(\"1980\");" + DELIMITER + "            ObjectFile.write(pathname, objects);" + DELIMITER + "            Object[] objs = ObjectFile.read(pathname);" + DELIMITER + "            for(int i = 0; i < objs.length; i++)" + DELIMITER + "            {" + DELIMITER + "              System.out.println(objs[i].toString());" + DELIMITER + "            }" + DELIMITER + "*/";
    ...
    public void test49412() {
        ...
        String expected = "/*" + DELIMITER + " * test block comment with a blank line" + DELIMITER + " */";
        String result = testFormat(input, options);
        assertEquals(expected, result);
    }
    ...
    public void test49412_3() {
        ...
        String input = "/*" + DELIMITER + DELIMITER + " test block comment with a blank line" + DELIMITER + "*/";
        String expected = "/*" + DELIMITER + " * " + DELIMITER + " * test block comment with a blank line" + DELIMITER + " */";
        String result = testFormat(input, options);
        assertEquals(expected, result);
    }
