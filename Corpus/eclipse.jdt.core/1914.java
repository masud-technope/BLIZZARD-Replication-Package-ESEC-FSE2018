...
public abstract class ReferenceBinding extends TypeBinding {
    ...
    // additional bits characterizing this type
    int typeBits;
    ...
    protected static ProblemMethodBinding samProblemBinding = new ProblemMethodBinding(TypeConstants.ANONYMOUS_METHOD, null, ProblemReasons.NoSuchSingleAbstractMethod);
        ...
        switch(this.compoundName.length) {
            case 3:
                ...
                switch(packageName.length) {
                    // only one type in this group, yet:
                    case 3:
                                ...
                                switch(typeName[0]) {
                                    case 'C':
                                        if (CharOperation.equals(typeName, TypeConstants.JAVA_IO_CLOSEABLE[2]))
                                            // don't assign id, only typeBit (for analysis of resource leaks) 
                        ...
                        switch(packageName[0]) {
                            case 'e':
                                if (CharOperation.equals(TypeConstants.ECLIPSE, packageName)) {
                                    packageName = this.compoundName[2];
                                    if (// just to be safe
                                    packageName.length == // just to be safe
                                    0)
                                        return;
                                    switch(packageName[0]) {
                                        case 'c':
 ...
 */
    ...
    public int depth() {
        ...
        while ((current = current.enclosingType()) != null) depth++;
        return depth;
    }
    ...
    public final ReferenceBinding enclosingTypeAt(int relativeDepth) {
        ReferenceBinding current = this;
        while (relativeDepth-- > 0 && current != null) current = current.enclosingType();
        return current;
    }
    ...
    public int enumConstantCount() {
        ...
        for (int i = 0, length = fields.length; i < length; i++) {
            if ((fields[i].modifiers & ClassFileConstants.AccEnum) != 0)
                count++;
        }
        return count;
    }
 ...
 */
    ...
    public ReferenceBinding getMemberType(char[] typeName) {
        ...
        for (int i = memberTypes.length; --i >= 0; ) if (CharOperation.equals(memberTypes[i].sourceName, typeName))
 ...
 */
    ...
    public TypeVariableBinding getTypeVariable(char[] variableName) {
        ...
        for (int i = typeVariables.length; --i >= 0; ) if (CharOperation.equals(typeVariables[i].sourceName, variableName))
...
*/
    public boolean implementsInterface(ReferenceBinding anInterface, boolean searchHierarchy) {
        ...
        do {
            ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
            if (// in code assist cases when source types are added late, may not be finished connecting hierarchy
 ...
 */
    public boolean isCompatibleWith(TypeBinding otherType, /*@Nullable*/
    Scope captureScope) {
        ...
        if (this.compatibleCache == null) {
            ...
            // [dbg reset] this.compatibleCache.put(otherType,null)
 ...
 * Answer true if the receiver type can be assigned to the argument type (right)
 */
    private boolean isCompatibleWith0(TypeBinding otherType, /*@Nullable*/
    Scope captureScope) {
        ...
        switch(otherType.kind()) {
            ...
            case Binding.RAW_TYPE:
                ...
                if (// could be annotation type
                otherReferenceType.isInterface()) {
 ...
 */
    public final boolean isViewedAsDeprecated() {
        return (this.modifiers & (ClassFileConstants.AccDeprecated | ExtraCompilerModifiers.AccDeprecatedImplicitly)) != 0 || getPackage().isViewedAsDeprecated();
    }
 ...
 */
    ...
    public MethodBinding getSingleAbstractMethod(Scope scope, boolean replaceWildcards) {
        ...
        next: for (int i = length - 1; i >= 0; --i) {
            ...
            this.singleAbstractMethod[index] = new MethodBinding(theAbstractMethod.modifiers | ClassFileConstants.AccSynthetic, theAbstractMethod.selector, returnType, parameters, exceptions, theAbstractMethod.declaringClass);
