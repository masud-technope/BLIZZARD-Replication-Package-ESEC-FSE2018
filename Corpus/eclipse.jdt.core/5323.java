        ...
        if (!hasMandatoryErrors) {
            ...
            if (severity != ProblemSeverities.Ignore) {
                ...
                for (int iSuppress = 0, suppressCount = this.suppressWarningsCount; iSuppress < suppressCount; iSuppress++) {
                    ...
                    // @SuppressWarnings("all") also suppresses unused warning token
                    if (unusedWarningTokenIsWarning && irritants.areAllSet())
                        continue;
                    if (// mismatch, some warning tokens were unused
                    irritants != foundIrritants[iSuppress]) {
                                    ...
                                    if (inits != null) {
                                        for (int iToken = 0, tokenCount = inits.length; iToken < tokenCount; iToken++) {
                                            ...
                                            if (cst != Constant.NotAConstant && cst.typeID() == TypeIds.T_JavaLangString) {
                                                ...
                                                if (tokenIrritants != null && // no complaint against @SuppressWarnings("all")
                                                !tokenIrritants.areAllSet() && // if irritant is effectively enabled
                                                options.isAnyEnabled(tokenIrritants) && (foundIrritants[iSuppress] == null || !// if irritant had no matching problem
                                                foundIrritants[iSuppress].isAnySet(// if irritant had no matching problem
                                                tokenIrritants))) {
                                                    if (unusedWarningTokenIsWarning) {
                                                        ...
                                                        nextSuppress: for (int jSuppress = iSuppress - 1; jSuppress >= 0; jSuppress--) {
                                                            ...
                                                            if (end > endSuppress)
                                                                continue nextSuppress;
                                                            if (// suppress all?
                                                            this.suppressWarningIrritants[// suppress all?
                                                            jSuppress].areAllSet())
                                                                break pairLoop;
                                                        }
                                                    }
 ...
 */
    public void generateCode() {
        ...
        try {
            if (this.types != null) {
                for (int i = 0, count = this.types.length; i < count; i++) this.types[i].generateCode(this.scope);
            }
        } catch (AbortCompilationUnit e) {
        }
    }
