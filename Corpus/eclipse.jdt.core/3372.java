...
public class BlockScope extends Scope {
    ...
    public void addSubscope(Scope childScope) {
        if (subscopeCount == subscopes.length)
            System.arraycopy(subscopes, 0, (subscopes = new Scope[subscopeCount * 2]), 0, subscopeCount);
    ...
    public final boolean allowBlankFinalFieldAssignment(FieldBinding binding) {
        ...
        methodScope.isInsideInitializer() || ((AbstractMethodDeclaration) methodScope.referenceContext).isInitializationMethod();
    ...
    String basicToString(int tab) {
        ...
        //$NON-NLS-1$
        s += newLine + "locals:";
        for (int i = 0; i < localIndex; i++) //$NON-NLS-1$
        ...
        while (hasMoreVariables || hasMoreScopes) {
            if (hasMoreScopes && (!hasMoreVariables || (subscopes[iscope].startIndex() <= ilocal))) {
                ...
                ((local.declaration.bits & ASTNode.IsLocalDeclarationReachableMASK) != 0)) {
                    if (// do not report unused catch arguments
                ...
                if (generateCurrentLocalVar) {
                    ...
                    // not generated
                    local.resolvedPosition = // not generated
                    -1;
                }
    ...
    public final ReferenceBinding findLocalType(char[] name) {
        ...
        for (int i = 0, length = subscopeCount; i < length; i++) {
            if (subscopes[i] instanceof ClassScope) {
                ...
                // from 1.4 on, local types should not be accessed across switch case blocks (52221)				
                if (compliance >= ClassFileConstants.JDK1_4 && sourceType.switchCase != this.switchCase)
                    continue;
    ...
    public Binding getBinding(char[][] compoundName, int mask, InvocationSite invocationSite, boolean needResolve) {
        ...
        while (currentIndex < length) {
            ...
            if ((mask & FIELD) != 0 && (binding = findField(typeBinding, nextName, invocationSite, /*resolve*/
            true)) != null) {
                if (!binding.isValidBinding())
            ...
            if ((binding = findMemberType(nextName, typeBinding)) == null) {
                ...
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), typeBinding, NotFound);
            }
            if (!binding.isValidBinding())
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), binding.problemId());
        }
    ...
    public final Binding getBinding(char[][] compoundName, InvocationSite invocationSite) {
        int currentIndex = 0;
        int length = compoundName.length;
        Binding binding = getBinding(compoundName[currentIndex++], VARIABLE | TYPE | PACKAGE, invocationSite, /*resolve*/
        true);
        if (!binding.isValidBinding())
            return binding;
        foundType: if (binding instanceof PackageBinding) {
        ...
        foundField: if (binding instanceof ReferenceBinding) {
            while (currentIndex < length) {
                ReferenceBinding typeBinding = (ReferenceBinding) binding;
                char[] nextName = compoundName[currentIndex++];
                if ((binding = findField(typeBinding, nextName, invocationSite, /*resolve*/
                true)) != null) {
                    if (!binding.isValidBinding())
                        return new ProblemFieldBinding(((FieldBinding) binding).declaringClass, CharOperation.subarray(compoundName, 0, currentIndex), binding.problemId());
        ...
        while (currentIndex < length) {
            ...
            variableBinding = findField(typeBinding, compoundName[currentIndex++], invocationSite, /*resolve*/
            true);
    ...
    public Object[] getEmulationPath(ReferenceBinding targetEnclosingType, boolean onlyExactMatch, boolean ignoreEnclosingArgInConstructorCall) {
        ...
        if (insideConstructor) {
            ...
            if ((syntheticArg = ((NestedTypeBinding) sourceType).getSyntheticArgument(targetEnclosingType, onlyExactMatch)) != null) {
                ...
                if (ignoreEnclosingArgInConstructorCall && currentMethodScope.isConstructorCall && (sourceType == targetEnclosingType || (!onlyExactMatch && sourceType.findSuperTypeErasingTo(targetEnclosingType) != null))) {
                    return NoEnclosingInstanceInConstructorCall;
                }
        ...
        path[0] != null) {
            ...
            while ((currentEnclosingType = currentType.enclosingType()) != null) {
                //done?
                if (currentType == targetEnclosingType || (!onlyExactMatch && currentType.findSuperTypeErasingTo(targetEnclosingType) != null))
                    break;
    ...
    public String toString(int tab) {
        ...
        for (int i = 0; i < subscopeCount; i++) if (subscopes[i] instanceof BlockScope)
            s += //$NON-NLS-1$
            ((BlockScope) subscopes[i]).toString(tab + 1) + //$NON-NLS-1$
            "\n";
        return s;
    }
}
