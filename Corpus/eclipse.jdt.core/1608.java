 ...
 */
    public ReferenceBinding findSuperTypeErasingTo(ReferenceBinding erasure) {
        ...
        if (erasure.isClass()) {
            while ((currentType = currentType.superclass()) != null) {
                if (currentType.erasure() == erasure)
                    return currentType;
            }
            return null;
        }
...
*/
    ...
    boolean implementsMethod(MethodBinding method) {
        ...
        while (type != null) {
            ...
            for (int i = methods.length; --i >= 0; ) if (methods[i].areParametersEqual(method))
                return true;
...
*/
    public boolean isCompatibleWith(TypeBinding otherType) {
        ...
        if (// Explicit conversion from an interface to a class is not allowed
        isInterface())
            return false;
