 ...
 */
    public void testBug424119_001() throws CoreException {
        ...
        SearchPattern pattern = SearchPattern.createPattern("Annot", ANNOTATION_TYPE, REFERENCES, EXACT_RULE);
        new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, getJavaSearchWorkingCopiesScope(), this.resultCollector, null);
 ...
 */
    public void testBug424119_003() throws CoreException {
        try {
            ...
            assertSearchResults("src/X.java [in P1] X.tab [Annot]", collector);
        } finally {
            deleteProject("P1");
        }
    }
 ...
 */
    public void testBug427537a() throws CoreException {
        ...
        SearchPattern pattern = SearchPattern.createPattern("I", TYPE, CAST_TYPE_REFERENCE, EXACT_RULE);
        new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, getJavaSearchWorkingCopiesScope(), this.resultCollector, null);
 ...
 */
    public void testBug427677() throws CoreException {
        ...
        SearchPattern pattern = SearchPattern.createPattern("Marker1", ANNOTATION_TYPE, REFERENCES, EXACT_RULE);
        new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, getJavaSearchWorkingCopiesScope(), this.resultCollector, null);
 ...
 */
    ...
    public void testBug400905_0019() throws CoreException {
        // bad package + bad cast - make sure there is no NPE.
    ...
    public void test430159a() throws CoreException {
        ...
        // mimic https://bugs.eclipse.org/bugs/show_bug.cgi?id=429498#c6
        SearchPattern rightPattern = SearchPattern.createPattern(elements[0].getParent(), ALL_OCCURRENCES, ERASURE_RULE);
    ...
    public void test430159b() throws CoreException {
        // this test basically checks that and pattern locator does not a lambda from being found.
        this.workingCopies = new ICompilationUnit[1];
        this.workingCopies[0] = getWorkingCopy("/JavaSearchBugs/src/b429498/X.java", "interface I {\n" + "    public void doit();\n" + "}\n" + "public class X {\n" + "	I i = () -> {};\n" + "}\n");
        String str = this.workingCopies[0].getSource();
        String selection = "doit";
 ...
 */
    ...
    public void testBug435480_0001() throws CoreException {
        this.workingCopies = new ICompilationUnit[1];
        this.workingCopies[0] = getWorkingCopy("/JavaSearchBugs/src/X.java", "import java.lang.annotation.*;\n" + "@Documented\n" + "@Retention(RetentionPolicy.RUNTIME)\n" + "@Target(value={})\n" + "public @interface Ann1 {\n" + "}");
    ...
    public void testBug435480_0002() throws CoreException {
        try {
            IJavaProject project = createJavaProject("P", new String[] { "", "src" }, new String[] { "JCL18_LIB" }, null, null, "bin", null, null, new String[][] { new String[] { "src/" }, new String[0] }, "1.8");
            createFile("/P/src/X.java", "import java.lang.annotation.*;\n" + "@Documented\n" + "@Retention(RetentionPolicy.RUNTIME)\n" + "@Target(value={})\n" + "public @interface Ann1 {\n" + "}\n");
            MethodPattern pattern = (MethodPattern) SearchPattern.createPattern("*", IMPLEMENTORS, IJavaSearchConstants.METHOD_REFERENCE_EXPRESSION, EXACT_RULE);
            int mask = IJavaSearchScope.SOURCES;
            IJavaSearchScope scope = SearchEngine.createJavaSearchScope(new IJavaElement[] { project }, mask);
            new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, scope, this.resultCollector, null);
            assertSearchResults("");
        } finally {
            deleteProject("P");
        }
 ...
 */
    public void testBug454401() throws CoreException, JavaModelException {
        try {
            ...
            createFile("/P/src/com/test/X.java", "package com.test;\n" + "public class X { \n" + " \n" + "	private class Y { \n" + "		private class P { \n" + "			 \n" + "		} \n" + "		@SuppressWarnings(\"unused\") \n" + "		public void t1(P p) { \n" + "			t2(p); \n" + "		} \n" + "		protected void t2(P p) { \n" + "			 \n" + "		} \n" + "	} \n" + "	public void foo() { \n" + "		Y y = new X().new Y(); \n" + "		y.t2(y.new P()); \n" + "	} \n" + "	public static void main(String[] args) { \n" + "		 \n" + "	} \n" + "}\n");
            waitUntilIndexesReady();
    ...
    public void testBug458614_001() throws CoreException {
        try {
            // Create project and files
            IJavaProject project = createJavaProject("P1", new String[] { "src" }, new String[] { "bin" }, "bin");
            createFile("/P1/src/X.java", "interface Function<T, R> {\n" + "	R apply(T); \n" + "};\n" + "@SuppressWarnings(\"unused\")\n" + "public class X<T> {\n" + "	private void foo() {\n" + "		Function<Integer, int[]> a1 = int[]::new;\n" + "	}\n" + "}\n");
            waitUntilIndexesReady();
            IndexManager indexManager = JavaModelManager.getIndexManager();
            indexManager.indexAll(project.getProject());
            waitUntilIndexesReady();
            ...
            new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, SearchEngine.createWorkspaceScope(), this.resultCollector, null);
            assertSearchResults("");
        } finally {
            deleteProject("P1");
        }
    ...
    public void testBug461025_001() throws CoreException {
        this.workingCopies = new ICompilationUnit[1];
        this.workingCopies[0] = getWorkingCopy("/JavaSearchBugs/src/X.java", "@FunctionalInterface\n" + "interface Y<T> {\n" + "    T get();\n" + "}\n" + "\n" + "public class X {\n" + "	public X() {}\n" + "	public X(int i) {}\n" + "\n" + "	private void m1() {\n" + "		Y<X> s1 = X::new;\n" + "\n" + "		Y<X> s2 = new Y<X>() {\n" + "\n" + "			@Override\n" + "			public X get() {\n" + "				return null;\n" + "			}\n" + "		};\n" + "	}\n" + "}\n");
        SearchPattern pattern = SearchPattern.createPattern("*", CONSTRUCTOR, METHOD_REFERENCE_EXPRESSION, EXACT_RULE);
        new SearchEngine(this.workingCopies).search(pattern, new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, getJavaSearchWorkingCopiesScope(), this.resultCollector, null);
        assertSearchResults("src/X.java void X.m1() [X::new] EXACT_MATCH");
    }
    ...
    public void testBug468127_0001() throws CoreException {
        try {
            ...
            assertSearchResults("src/X.java void X.main(String[]) [accept(0)] EXACT_MATCH\n" + "lib/b468127.jar void <anonymous>.accept(java.lang.Object) EXACT_MATCH");
        } finally {
            deleteProject("P");
        }
    }
    ...
    public void testBug484367_0003() throws CoreException {
        this.workingCopies = new ICompilationUnit[1];
        this.workingCopies[0] = getWorkingCopy("/JavaSearchBugs/src/X.java", "interface  Bar1 {\n" + "	public void print();\n" + "}\n" + "class Bar implements Bar1 {\n" + "\n" + "	@Override\n" + "	public void print() {}\n" + "}\n" + "\n" + "@FunctionalInterface\n" + "interface Foo {\n" + "	void process(Bar bar);\n" + "}\n" + "class BarImpl extends Bar{\n" + "	public void print() {}\n" + "}\n" + "\n" + "public class X {\n" + "	@SuppressWarnings(\"unused\")\n" + "	public void test(){\n" + "		Foo foo1 = (bar)->bar.print();\n" + "		Foo foo2 = Bar::print;\n" + "	}\n" + "}\n");
