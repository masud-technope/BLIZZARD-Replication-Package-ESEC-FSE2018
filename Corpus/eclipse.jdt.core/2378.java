...
public abstract class JavaEditor extends ExtendedTextEditor implements IViewPartInputProvider {
    ...
    class MouseClickListener implements KeyListener, MouseListener, MouseMoveListener, FocusListener, PaintListener, IPropertyChangeListener, IDocumentListener, ITextInputListener {
        ...
        public void mouseUp(MouseEvent e) {
            ...
            if (wasActive) {
                //$NON-NLS-1$
                IAction action = getAction("OpenEditor");
                if (action != null)
                    action.run();
            }
        }
    ...
    protected static class AnnotationAccess extends DefaultMarkerAnnotationAccess {
        ...
        public void run() {
            try {
                ...
                if (Character.isJavaIdentifierPart(character)) {
                    ...
                    character = document.getChar(position);
                    while (position >= 0 && Character.isUpperCase(character)) character = document.getChar(--position);
    ...
    protected static final String LINK_COLOR = PreferenceConstants.EDITOR_LINK_COLOR;
    ...
    private static final String BROWSER_LIKE_LINKS = PreferenceConstants.EDITOR_BROWSER_LIKE_LINKS;

    /** Preference key for key modifier of browser like links */
    private static final String BROWSER_LIKE_LINKS_KEY_MODIFIER = PreferenceConstants.EDITOR_BROWSER_LIKE_LINKS_KEY_MODIFIER;
	 ...
	 * @since 2.1.1
	 */
    private static final String BROWSER_LIKE_LINKS_KEY_MODIFIER_MASK = PreferenceConstants.EDITOR_BROWSER_LIKE_LINKS_KEY_MODIFIER_MASK;
	 ...
	 */
    protected abstract void setOutlinePageInput(JavaOutlinePage page, IEditorInput input);
    ...
    protected void createActions() {
        super.createActions();
        //$NON-NLS-1$
        ResourceAction resAction = new AddTaskAction(JavaEditorMessages.getResourceBundle(), "AddTask.", this);
        ...
        fActionGroups = new CompositeActionGroup(new ActionGroup[] { oeg = new OpenEditorActionGroup(this), sg = new ShowActionGroup(this), ovg = new OpenViewActionGroup(this), jsg = new JavaSearchActionGroup(this) });
        fContextMenuGroup = new CompositeActionGroup(new ActionGroup[] { oeg, ovg, sg, jsg });
        //$NON-NLS-1$
        resAction = new TextOperationAction(JavaEditorMessages.getResourceBundle(), "ShowJavaDoc.", this, ISourceViewer.INFORMATION, true);
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        setAction("ShowJavaDoc", resAction);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getResourceBundle(), "ShowOutline.", this, JavaSourceViewer.SHOW_OUTLINE, true);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getResourceBundle(), "OpenStructure.", this, JavaSourceViewer.OPEN_STRUCTURE, true);
        ...
        //$NON-NLS-1$
        action = new TextOperationAction(JavaEditorMessages.getResourceBundle(), "OpenHierarchy.", this, JavaSourceViewer.SHOW_HIERARCHY, true);
    ...
    private void updateAnnotationViews(Annotation annotation) {
        ...
        if (marker != null && !marker.equals(fLastMarkerTarget)) {
            try {
                ...
                if (view != null) {
                    Method method = view.getClass().getMethod("setSelection", new Class[] { //$NON-NLS-1$
                    IStructuredSelection.class, //$NON-NLS-1$
                    boolean.class });
        ...
        public void run() {
            try {
                ...
                synchronized (annotationModel) {
                    ...
                    for (int i = 0, size = annotations.size(); i < size; i++) annotationModel.addAnnotation((Annotation) annotations.get(i), (Position) positions.get(i));
                }
            } finally {
    ...
    protected void updateOccurrences() {
        ...
        //$NON-NLS-1$
        Thread thread = new Thread(finder, "Occurrences Marker");
	 ...
	 * @since 3.0
	 */
    protected abstract IJavaElement getInputJavaElement();
    ...
    private static boolean isBracket(char character) {
        for (int i = 0; i != BRACKETS.length; ++i) if (character == BRACKETS[i])
            return true;
        return false;
    }
    ...
    private boolean isNavigationTargetType(Object type) {
        //$NON-NLS-1$
        Preferences preferences = Platform.getPlugin("org.eclipse.ui.workbench.texteditor").getPluginPreferences();
    ...
    protected boolean isPrefQuickDiffAlwaysOn() {
        // never show change ruler for the non-editable java editor. Overridden in subclasses like CompilationUnitEditor
        return false;
    }
