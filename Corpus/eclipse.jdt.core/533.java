...
public class ExceptionHandlingFlowContext extends FlowContext {
    ...
    public  ExceptionHandlingFlowContext(FlowContext parent, ASTNode associatedNode, ReferenceBinding[] handledExceptions, BlockScope scope, UnconditionalFlowInfo flowInfo) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            boolean isUnchecked = (scope.compareUncheckedException(handledExceptions[i]) != NotRelated);
    ...
    public void complainIfUnusedExceptionHandlers(AbstractMethodDeclaration method) {
        ...
        if ((method.binding.modifiers & (CompilerModifiers.AccOverriding | CompilerModifiers.AccImplementing)) != 0 && !scope.environment().options.reportUnusedDeclaredThrownExceptionWhenOverriding) {
            return;
        }
        ...
        for (int i = 0, count = handledExceptions.length; i < count; i++) {
            ...
            if ((isReached[cacheIndex] & bitMask) == 0) {
                scope.problemReporter().unusedDeclaredThrownException(handledExceptions[index], method, method.thrownExceptions[index]);
            }
        }
    }
    ...
    public void complainIfUnusedExceptionHandlers(BlockScope scope, TryStatement tryStatement) {
        ...
        for (int i = 0, count = handledExceptions.length; i < count; i++) {
            ...
            int bitMask = 1 << (index % BitCacheSize);
            if ((isReached[cacheIndex] & bitMask) == 0) {
                scope.problemReporter().unreachableCatchBlock(handledExceptions[index], tryStatement.catchArguments[index].type);
            } else {
                if ((isNeeded[cacheIndex] & bitMask) == 0) {
                    scope.problemReporter().hiddenCatchBlock(handledExceptions[index], tryStatement.catchArguments[index].type);
                }
            }
        }
    }
    ...
    public String individualToString() {
        ...
        //$NON-NLS-1$
        buffer.append("[initsOnReturn -").append(initsOnReturn.toString()).append(']');
    ...
    public void recordHandlingException(ReferenceBinding exceptionType, UnconditionalFlowInfo flowInfo, TypeBinding raisedException, ASTNode invocationSite, boolean wasAlreadyDefinitelyCaught) {
        ...
        initsOnExceptions[index] = initsOnExceptions[index] == FlowInfo.DEAD_END ? flowInfo.copy().unconditionalInits() : initsOnExceptions[index].mergedWith(flowInfo.copy().unconditionalInits());
    }
