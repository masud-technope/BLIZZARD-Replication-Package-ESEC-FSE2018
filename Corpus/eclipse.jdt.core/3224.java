...
public class ParameterizedMethodBinding extends MethodBinding {
    ...
    public  ParameterizedMethodBinding(final ParameterizedTypeBinding parameterizedDeclaringClass, MethodBinding originalMethod) {
        ...
        if (length == 0) {
            ...
            for (// copy original type variable to relocate
            int i = 0; // copy original type variable to relocate
            i < length; // copy original type variable to relocate
            i++) {
    ...
    public static ParameterizedMethodBinding instantiateGetClass(TypeBinding receiverType, MethodBinding originalMethod, Scope scope) {
        ...
        TypeBinding rawType = environment.convertToRawType(receiverType.erasure(), /*do not force conversion of enclosing types*/
        false);
        if (environment.usesNullTypeAnnotations())
            rawType = environment.createAnnotatedType(rawType, new AnnotationBinding[] { environment.getNonNullAnnotation() });
        method.returnType = environment.createParameterizedType(genericClassType, new TypeBinding[] { environment.createWildcard(genericClassType, 0, rawType, /*no extra bound*/
        null, Wildcard.EXTENDS) }, null);
