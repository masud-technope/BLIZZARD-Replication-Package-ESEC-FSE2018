...
public abstract class TypeBinding extends Binding {
    ...
    public static final ReferenceBinding TYPE_USE_BINDING = new ReferenceBinding() {
        ...
        /* used for type annotation resolution. */
        {
    ...
    public static final BaseTypeBinding INT = new BaseTypeBinding(TypeIds.T_int, TypeConstants.INT, new char[] { 'I' });

    public static final BaseTypeBinding BYTE = new BaseTypeBinding(TypeIds.T_byte, TypeConstants.BYTE, new char[] { 'B' });

    public static final BaseTypeBinding SHORT = new BaseTypeBinding(TypeIds.T_short, TypeConstants.SHORT, new char[] { 'S' });

    public static final BaseTypeBinding CHAR = new BaseTypeBinding(TypeIds.T_char, TypeConstants.CHAR, new char[] { 'C' });

    public static final BaseTypeBinding LONG = new BaseTypeBinding(TypeIds.T_long, TypeConstants.LONG, new char[] { 'J' });

    public static final BaseTypeBinding FLOAT = new BaseTypeBinding(TypeIds.T_float, TypeConstants.FLOAT, new char[] { 'F' });

    public static final BaseTypeBinding DOUBLE = new BaseTypeBinding(TypeIds.T_double, TypeConstants.DOUBLE, new char[] { 'D' });

    public static final BaseTypeBinding BOOLEAN = new BaseTypeBinding(TypeIds.T_boolean, TypeConstants.BOOLEAN, new char[] { 'Z' });
    ...
    public  TypeBinding(TypeBinding prototype) {
        // faithfully copy most instance state - clone operation should specialize/override suitably.
 ...
 */
    ...
    public ReferenceBinding actualType() {
        // overridden in ParameterizedTypeBinding & WildcardBinding
        return null;
    }

    TypeBinding[] additionalBounds() {
        // overridden in WildcardBinding
        return null;
    }
                ...
                do {
                    ...
                    if (itsInterfaces != null && itsInterfaces != Binding.NO_SUPERINTERFACES) {
                        if (interfacesToVisit == null) {
                            ...
                            nextInterface: for (int a = 0; a < itsLength; a++) {
                                ReferenceBinding next = itsInterfaces[a];
                                for (int b = 0; b < nextPosition; b++) if (equalsEquals(next, interfacesToVisit[b]))
                                    continue nextInterface;
                                interfacesToVisit[nextPosition++] = next;
                            }
                        }
        ...
        switch(kind()) {
            case Binding.TYPE_PARAMETER:
                ...
                if (variable.erasure().findSuperTypeOriginatingFrom(declaringClass) != null) {
                    // no need for alternate receiver type
                    return this;
                }
                ...
                // only occur if passed null declaringClass for arraylength
                return this;
            case Binding.INTERSECTION_TYPE:
 ...
 */
    public boolean isCompatibleWith(TypeBinding right) {
        // delegate from the old signature to the new implementation:
        return isCompatibleWith(right, null);
    }

    // version that allows to capture a type bound using 'scope':
    public abstract boolean isCompatibleWith(TypeBinding right, /*@Nullable*/
    Scope scope);

    public boolean isPotentiallyCompatibleWith(TypeBinding right, /*@Nullable*/
    Scope scope) {
 ...
 */
    public boolean isBoxingCompatibleWith(TypeBinding right, /*@NonNull */
    Scope scope) {
        if (right == null)
            return false;
 ...
 */
    TypeBinding substituteInferenceVariable(InferenceVariable var, TypeBinding substituteType) {
        // default: not substituting anything
        return this;
    }
        ...
        switch(kind()) {
            case Binding.PARAMETERIZED_TYPE:
                ...
                switch(otherType.kind()) {
                    ...
                    case // https://bugs.eclipse.org/bugs/show_bug.cgi?id=329588
                    Binding.TYPE:
    ...
    private boolean isProvablyDistinctTypeArgument(TypeBinding otherArgument, final ParameterizedTypeBinding paramType, final int rank) {
        ...
        if (lowerBound1 != null) {
            ...
            if (lowerBound2 != null) {
                ...
                // ground types should have been the same
                return true;
            }
        }
    }
                    ...
                    if (capture instanceof CaptureBinding18) {
                        ...
                        if (cb18.firstBound != null) {
                            if (cb18.lowerBound != null)
                                // type containment is not defined for variables with both upper and lower bound
                                return false;
                            TypeBinding[] otherBounds = null;
                            // by construction non-null if firstBound is set
                        ...
                        switch(wildcard.boundKind) {
                            case Wildcard.SUPER:
                                // T super syntax isn't allowed, impossible capture.
                                return false;
                            case Wildcard.UNBOUND:
                        ...
                        if (upperBound.id == TypeIds.T_JavaLangObject && otherBounds == null) {
                            // but given class A<T>, A<?> stays an unbounded wildcard, see https://bugs.eclipse.org/bugs/show_bug.cgi?id=348956
                            return false;
                        }
                    ...
                    case Binding.INTERSECTION_TYPE:
                        ...
                        switch(wildcard.boundKind) {
                            case Wildcard.EXTENDS:
                                ...
                                wildcard.otherBounds != // intersection type
                                null)
                                    break;
                                upperBound = wildcard.bound;
                                lowerBound = null;
                ...
                switch(otherWildcard.boundKind) {
                    case Wildcard.EXTENDS:
                        if (otherBound instanceof IntersectionTypeBinding18) {
                            ...
                            for (int i = 0, length = intersectingTypes.length; i < length; i++) if (TypeBinding.equalsEquals(intersectingTypes[i], this))
                                return true;
                        }
            ...
            case Binding.PARAMETERIZED_TYPE:
                ...
                nextArgument: for (int i = 0; i < length; i++) {
                    ...
                    switch(kind) {
                        case Binding.PARAMETERIZED_TYPE:
                            if (// recurse
                            argument.isTypeArgumentContainedBy(// recurse
                            otherArgument))
                                continue nextArgument;
                            break;
                        case Binding.WILDCARD_TYPE:
                        case Binding.INTERSECTION_TYPE:
                            ...
                            switch(wildcard.boundKind) {
                                case Wildcard.EXTENDS:
                                    ...
                                    if (otherWildcard.boundKind == Wildcard.UNBOUND && TypeBinding.equalsEquals(wildcard.bound, wildcard.typeVariable().upperBound()))
                                        continue nextArgument;
                                    break;
                                case Wildcard.SUPER:
                                    break;
                                case Wildcard.UNBOUND:
                                    ...
                                    if (otherWildcard.boundKind == Wildcard.EXTENDS && TypeBinding.equalsEquals(otherWildcard.bound, otherWildcard.typeVariable().upperBound()))
                                        continue nextArgument;
                                    break;
                            }
                            break;
 ...
 */
    ...
    public boolean mentionsAny(TypeBinding[] parameters, int idx) {
        for (int i = 0; i < parameters.length; i++) if (i != idx)
