...
public class View {
    ...
    public static void main(String[] args) {
        ...
        //$NON-NLS-1$
        System.out.println(scenarios.length + " Scenarios");
        System.out.println();
        for (int s = 0; s < scenarios.length; s++) dump(ps, scenarios[s]);
        if (ps != System.out)
            ps.close();
    }
    ...
    private static void dump(PrintStream ps, Scenario scenario) {
        ...
        //$NON-NLS-1$
        r.addCell(PerformanceTestPlugin.BUILD + ":");
        for (int j = 0; j < timeSeriesLabels.length; j++) r.addCellRight(timeSeriesLabels[j]);
        r.nextRow();
