...
public class ClassScope extends Scope {
    ...
    // contains TypeReference or Runnable. TODO consider making this a List<Runnable>
    java.util.ArrayList<Object> deferredBoundChecks;
    ...
    void buildAnonymousTypeBinding(SourceTypeBinding enclosingType, ReferenceBinding supertype) {
        ...
        // tag all anonymous types as used locally
        anonymousType.modifiers |= ExtraCompilerModifiers.AccLocallyUsed;
        // for anonymous class assume same properties as its super (as a closeable) ...
        ...
        if (supertype.isInterface()) {
            ...
            anonymousType.setSuperInterfaces(Binding.NO_SUPERINTERFACES);
            TypeReference typeReference = this.referenceContext.allocation.type;
            if (// no check for enum constant body
            typeReference != null) {
    ...
    void buildFieldsAndMethods() {
        ...
        for (int i = 0, length = memberTypes.length; i < length; i++) ((SourceTypeBinding) memberTypes[i]).scope.buildFieldsAndMethods();
    }
        ...
        if (this.referenceContext.memberTypes != null) {
            ...
            nextMember: for (int i = 0; i < length; i++) {
                ...
                switch(TypeDeclaration.kind(memberContext.modifiers)) {
                    case TypeDeclaration.INTERFACE_DECL:
                    case TypeDeclaration.ANNOTATION_TYPE_DECL:
                        if (sourceType.isNestedType() && // no need to check for enum, since implicitly static
        ...
        if (sourceType.isAbstract()) {
            for (int i = 0; i < size; i++) {
                if (i != clinitIndex) {
                    ...
                    if (// is null if binding could not be created
                    methodBinding != null) {
    ...
    private void buildTypeVariables() {
        ...
        // safety
        sourceType.setTypeVariables(Binding.NO_TYPE_VARIABLES);
        if (// handle the case of redefining java.lang.Object up front
        sourceType.id == TypeIds.T_JavaLangObject) {
    ...
    private void checkAndSetModifiers() {
        ...
        if (// interface and annotation type
        (realModifiers & ClassFileConstants.AccInterface) != 0) {
            // detect abnormal cases for interfaces
            if (isMemberType) {
                final int UNEXPECTED_MODIFIERS = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate | ClassFileConstants.AccProtected | ClassFileConstants.AccStatic | ClassFileConstants.AccAbstract | ClassFileConstants.AccInterface | ClassFileConstants.AccStrictfp | ClassFileConstants.AccAnnotation);
            ...
            if (// includes member types defined inside local types
            isMemberType) {
                ...
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0) {
                    ...
                    // avoid leaking abstract modifier
                    modifiers &= ~ClassFileConstants.AccAbstract;
            ...
            if (!sourceType.isAnonymousType()) {
                checkAbstractEnum: {
                    ...
                    for (int i = 0; i < methodsLength && !definesAbstractMethod; i++) definesAbstractMethod = methods[i].isAbstract();
	...
	*/
    ...
    public void checkParameterizedSuperTypeCollisions() {
        ...
        nextVariable: for (int i = 0, paramLength = typeParameters == null ? 0 : typeParameters.length; i < paramLength; i++) {
            ...
            if (boundRefs != null) {
                ...
                for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
                    ...
                    for (int index = typeVariable.superInterfaces.length; --index >= 0; ) if (hasErasedCandidatesCollisions(superType, typeVariable.superInterfaces[index], invocations, typeVariable, typeRef))
                        continue nextVariable;
                }
            }
        }
        ...
        if (memberTypes != null && memberTypes != Binding.NO_MEMBER_TYPES)
            for (int i = 0, size = memberTypes.length; i < size; i++) ((SourceTypeBinding) memberTypes[i]).scope.checkParameterizedSuperTypeCollisions();
    }
    ...
    private void checkForInheritedMemberTypes(SourceTypeBinding sourceType) {
        ...
        int nextPosition = 0;
        do {
            if (// avoid resolving member types eagerly
            currentType.hasMemberTypes())
                return;
        ...
        if (interfacesToVisit != null) {
            ...
            for (int i = 0; i < nextPosition; i++) {
                ...
                if (// skip interface if it already knows it has no member types
                (anInterface.tagBits & TagBits.HasNoMemberTypes) == 0) {
                    if (// avoid resolving member types eagerly
                    anInterface.hasMemberTypes())
                        return;
                    needToTag = true;
            ...
            if (needToTag) {
                for (int i = 0; i < nextPosition; i++) interfacesToVisit[i].tagBits |= TagBits.HasNoMemberTypes;
            }
        }
    ...
    public void checkParameterizedTypeBounds() {
        ...
        ReferenceBinding[] memberTypes = this.referenceContext.binding.memberTypes;
        if (memberTypes != null && memberTypes != Binding.NO_MEMBER_TYPES)
            for (int i = 0, size = memberTypes.length; i < size; i++) ((SourceTypeBinding) memberTypes[i]).scope.checkParameterizedTypeBounds();
    }
    ...
    private void connectMemberTypes() {
        ...
        ReferenceBinding[] memberTypes = sourceType.memberTypes;
        if (memberTypes != null && memberTypes != Binding.NO_MEMBER_TYPES) {
            for (int i = 0, size = memberTypes.length; i < size; i++) ((SourceTypeBinding) memberTypes[i]).scope.connectTypeHierarchy();
        }
    }
	...
	*/
    private boolean connectSuperclass() {
        ...
        if (this.referenceContext.superclass == null) {
            if (// do not connect if source < 1.5 as enum already got flagged as syntax error
            sourceType.isEnum() && compilerOptions().sourceLevel >= ClassFileConstants.JDK1_5)
        ...
        if (// is null if a cycle was detected cycle or a problem
        superclass != null) {
            ...
            } else if ((superclass.tagBits & TagBits.HierarchyHasProblems) != 0 || !superclassRef.resolvedType.isValidBinding()) {
                sourceType.setSuperClass(superclass);
                // propagate if missing supertype
    ...
    private boolean connectEnumSuperclass() {
        ...
        if (// check generic
        refTypeVariables == Binding.NO_TYPE_VARIABLES) {
        ...
        1 != refTypeVariables.length) {
            problemReporter().incorrectArityForParameterizedType(null, rootEnumType, new TypeBinding[] { sourceType });
            // cannot reach here as AbortCompilation is thrown
            return false;
        }
        // check argument type compatibility
        ParameterizedTypeBinding superType = environment().createParameterizedType(rootEnumType, new TypeBinding[] { environment().convertToRawType(sourceType, /*do not force conversion of enclosing types*/
        false) }, null);
        // propagate if missing supertpye
        sourceType.tagBits |= (superType.tagBits & TagBits.HierarchyHasProblems);
	...
	*/
    private boolean connectSuperInterfaces() {
        ...
        if (this.referenceContext.superInterfaces == null) {
            if (// do not connect if source < 1.5 as annotation already got flagged as syntax error) {
            sourceType.isAnnotationType() && compilerOptions().sourceLevel >= ClassFileConstants.JDK1_5) {
        ...
        if (// already handled the case of redefining java.lang.Object
        sourceType.id == TypeIds.T_JavaLangObject)
            return true;
        ...
        nextInterface: for (int i = 0; i < length; i++) {
            ...
            if ((superInterface.tagBits & TagBits.HierarchyHasProblems) != 0 || !superInterfaceRef.resolvedType.isValidBinding()) {
                // propagate if missing supertype
                sourceType.tagBits |= TagBits.HierarchyHasProblems;
    ...
    public boolean detectHierarchyCycle(TypeBinding superType, TypeReference reference) {
        if (!(superType instanceof ReferenceBinding))
            return false;
        if (// see findSuperType()
        reference == this.superTypeReference) {
        ...
        if (superType.isBinaryBinding()) {
            ...
            if (parentType != null) {
                ...
                if ((parentType.tagBits & TagBits.HierarchyHasProblems) != 0) {
                    ...
                    parentType.tagBits |= TagBits.HierarchyHasProblems;
                }
            }
