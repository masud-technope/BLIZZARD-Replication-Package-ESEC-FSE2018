 ...
 */
    public final boolean allowBlankFinalFieldAssignment(FieldBinding binding) {
        ...
        if (methodScope.isLambdaScope())
            return false;
        return // inside initializer
        methodScope.isInsideInitializer() || // inside constructor or clinit
    ...
    String basicToString(int tab) {
        ...
        //$NON-NLS-1$
        s += newLine + "locals:";
        for (int i = 0; i < this.localIndex; i++) //$NON-NLS-1$
        ...
        while (hasMoreVariables || hasMoreScopes) {
            if (hasMoreScopes && (!hasMoreVariables || (this.subscopes[iscope].startIndex() <= ilocal))) {
                ...
                if (generateCurrentLocalVar) {
                    ...
                    if (// no more than 65535 words of locals
                    this.offset > 0xFFFF) {
                        problemReporter().noMoreAvailableSpaceForLocal(local, local.declaration == null ? (ASTNode) methodScope().referenceContext : local.declaration);
                    }
                } else {
                    // not generated
 ...
 */
    public LocalDeclaration[] findLocalVariableDeclarations(int position) {
        ...
        while (hasMoreVariables || hasMoreScopes) {
            if (hasMoreScopes && (!hasMoreVariables || (this.subscopes[iscope].startIndex() <= ilocal))) {
                ...
                if (// do not dive in nested types
                subscope.kind == Scope.BLOCK_SCOPE) {
    ...
    public LocalVariableBinding findVariable(char[] variableName) {
        ...
        for (// lookup backward to reach latest additions first
        int i = this.localIndex - 1; // lookup backward to reach latest additions first
        i >= 0; // lookup backward to reach latest additions first
        i--) {
 ...
 */
    public Binding getBinding(char[][] compoundName, int mask, InvocationSite invocationSite, boolean needResolve) {
        ...
        while (currentIndex < length) {
            ...
            if ((mask & Binding.FIELD) != 0 && (binding = findField(referenceBinding, nextName, invocationSite, /*resolve*/
            true)) != null) {
                if (binding.isValidBinding()) {
            ...
            if ((binding = findMemberType(nextName, referenceBinding)) == null) {
                ...
                if ((mask & Binding.FIELD) != 0) {
                    ...
                    return new ProblemBinding(CharOperation.subarray(compoundName, 0, currentIndex), referenceBinding, ProblemReasons.NotFound);
                }
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), referenceBinding, ProblemReasons.NotFound);
            }
        ...
        if ((mask & Binding.FIELD) != 0 && (binding instanceof FieldBinding)) {
            ...
            if (!field.isStatic())
                return new ProblemFieldBinding(field, field.declaringClass, CharOperation.concatWith(CharOperation.subarray(compoundName, 0, currentIndex), '.'), ProblemReasons.NonStaticReferenceInStaticContext);
    ...
    public final Binding getBinding(char[][] compoundName, InvocationSite invocationSite) {
        int currentIndex = 0;
        int length = compoundName.length;
        Binding binding = getBinding(compoundName[currentIndex++], Binding.VARIABLE | Binding.TYPE | Binding.PACKAGE, invocationSite, /*resolve*/
        true);
        if (!binding.isValidBinding())
            return binding;
        foundType: if (binding instanceof PackageBinding) {
        ...
        foundField: if (binding instanceof ReferenceBinding) {
            while (currentIndex < length) {
                ...
                if ((binding = findField(receiverType, nextName, invocationSite, /*resolve*/
                true)) != null) {
                    if (!binding.isValidBinding()) {
                    ...
                    if (!((FieldBinding) binding).isStatic())
                        return new ProblemFieldBinding((FieldBinding) binding, ((FieldBinding) binding).declaringClass, CharOperation.concatWith(CharOperation.subarray(compoundName, 0, currentIndex), '.'), ProblemReasons.NonStaticReferenceInStaticContext);
                    // binding is now a field
                    break foundField;
                }
        ...
        while (currentIndex < length) {
            ...
            if (typeBinding == null) {
                return new ProblemFieldBinding(null, null, CharOperation.concatWith(CharOperation.subarray(compoundName, 0, currentIndex), '.'), ProblemReasons.NotFound);
            }
            TypeBinding receiverType = typeBinding.capture(this, invocationSite.sourceStart(), invocationSite.sourceEnd());
            variableBinding = findField(receiverType, compoundName[currentIndex++], invocationSite, /*resolve*/
            true);
            if (variableBinding == null) {
                return new ProblemFieldBinding(null, receiverType instanceof ReferenceBinding ? (ReferenceBinding) receiverType : null, CharOperation.concatWith(CharOperation.subarray(compoundName, 0, currentIndex), '.'), ProblemReasons.NotFound);
            }
 ...
 */
    public VariableBinding[] getEmulationPath(LocalVariableBinding outerLocalVariable) {
        ...
        if (variableScope == null || /*val$this$0*/
        currentMethodScope == variableScope.methodScope()) {
 ...
 */
    public Object[] getEmulationPath(ReferenceBinding targetEnclosingType, boolean onlyExactMatch, boolean denyEnclosingArgInConstructorCall) {
        ...
        if (// no emulation from within non-inner types
        !sourceType.isNestedType() || sourceType.isStatic()) {
        ...
        path[0] != null) {
            ...
            while ((currentEnclosingType = currentType.enclosingType()) != null) {
                ...
                path[count++] = ((SourceTypeBinding) syntheticField.declaringClass).addSyntheticMethod(syntheticField, /*read*/
                true, /*not super access*/
                false);
    ...
    public final boolean needBlankFinalFieldInitializationCheck(FieldBinding binding) {
        ...
        while (methodScope != null) {
            ...
            if (!enclosingType.erasure().isAnonymousType()) {
                // only check inside anonymous type
                return false;
            }
 ...
 */
    ...
    public String toString(int tab) {
        ...
        for (int i = 0; i < this.subscopeCount; i++) if (this.subscopes[i] instanceof BlockScope)
            //$NON-NLS-1$
            s += ((BlockScope) this.subscopes[i]).toString(tab + 1) + "\n";
        return s;
    }

    // can be null if no resources are tracked
    ...
    public void checkUnclosedCloseables(FlowInfo flowInfo, FlowContext flowContext, ASTNode location, BlockScope locationScope) {
        ...
        while (iterator.hasNext()) {
            ...
            if (location != null && trackingVar.hasDefinitelyNoResource(flowInfo)) {
                // reporting against a specific location, there is no resource at this flow, don't complain
                continue;
            }
            if (location != null && flowContext != null && flowContext.recordExitAgainstResource(this, flowInfo, trackingVar, location)) {
                // handled by the flow context
                continue;
            }
            ...
            if (// at end of block and not definitely unclosed
            location == null) {
                // problems at specific locations: medium priority
                if (// ... report previously recorded errors
                trackingVar.reportRecordedErrors(this, status, flowInfo.reachMode() != FlowInfo.REACHABLE))
                    continue;
            }
        ...
        if (location == null) {
            ...
            for (int i = 0; i < this.localIndex; i++) this.locals[i].closeTracker = null;
        ...
        if (this.trackingVariables != null) {
            ...
            for (int i = 0; i < trackVarCount; i++) {
                ...
                if (trackingVar.originalBinding == null)
                    continue;
                if (// closed in then branch
                thenFlowInfo.isDefinitelyNonNull(trackingVar.binding) && // null in else branch
                elseFlowInfo.isDefinitelyNull(trackingVar.originalBinding)) {
                    // -> always closed
                    elseFlowInfo.markAsDefinitelyNonNull(trackingVar.binding);
                } else if (// closed in else branch
                elseFlowInfo.isDefinitelyNonNull(trackingVar.binding) && // null in then branch
