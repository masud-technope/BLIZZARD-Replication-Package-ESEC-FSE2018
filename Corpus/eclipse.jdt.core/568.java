		...
		*/
		 ...
		 */
            ...
            if (sourcePath == null) {
                ...
                Util.setSourceAttachmentProperty(getPath(), sourcePath.toString() + //$NON-NLS-1$
 ...
 */
    ...
    SourceMapper createSourceMapper(IPath sourcePath, IPath rootPath) throws JavaModelException {
        ...
        SourceMapper mapper = new SourceMapper(sourcePath, rootPath == null ? null : rootPath.toOSString(), // cannot use workspace options if external jar is 1.5 jar and workspace options are 1.4 options
        getJavaProject().getOptions(true), encoding);
        return mapper;
    }
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    switch(member.getType()) {
                        ...
                        case IResource.FILE:
                            ...
                            if (!hasIncluded && Util.isValidCompilationUnitName(memberName, sourceLevel, complianceLevel) && !Util.isExcluded(member, inclusionPatterns, exclusionPatterns)) {
                                hasIncluded = true;
                                IPackageFragment pkg = getPackageFragment(pkgName);
                                vChildren.add(pkg);
                            }
 ...
 */
    public IClasspathEntry getResolvedClasspathEntry() throws JavaModelException {
        ...
        JavaProject project = (JavaProject) getJavaProject();
        // force the resolved entry cache to be populated
        project.getResolvedClasspath();
    ...
    public IResource resource() {
        if (// perf improvement to avoid message send in resource()
        this.resource != null)
