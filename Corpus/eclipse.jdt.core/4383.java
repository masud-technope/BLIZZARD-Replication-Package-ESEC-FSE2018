...
public class QualifiedTypeReference extends TypeReference {
    ...
    protected TypeBinding getTypeBinding(Scope scope) {
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length, last = max - 1; i < max; i++) {
            ...
            if (!this.resolvedType.isValidBinding())
                return this.resolvedType;
            if (// cannot select from a type variable
            i == 0 && this.resolvedType.isTypeVariable() && ((TypeVariableBinding) this.resolvedType).firstBound == null) {
            ...
            if (isClassScope)
                if (// must connect hierarchy to find inherited member types
                ((ClassScope) scope).detectHierarchyCycle(this.resolvedType, this))
                    return null;
            ...
            if (qualifiedType != null) {
                ...
                if (currentType.isGenericType()) {
                    ...
                    // parameterization of enclosing is irrelevant in this case
                ...
                qualifiedType = currentType.isGenericType() ? (ReferenceBinding) scope.environment().convertToRawType(currentType, /*do not force conversion of enclosing types*/
                false) : currentType;
            }
        ...
        if (visitor.visit(this, scope)) {
            if (this.annotations != null) {
                ...
                for (int i = 0; i < annotationsLevels; i++) {
                    ...
                    for (int j = 0; j < annotationsLength; j++) this.annotations[i][j].traverse(visitor, scope);
                }
            }
        }
