...
public class LambdaExpression extends FunctionalExpression implements IPolyExpression, ReferenceContext, ProblemSeverities {
    ...
    // source representation of the lambda.
    public char[] text;
    ...
    // when performing tentative resolve keep a back reference to the driving context
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        for (int i = 0, length = this.outerLocalVariables == null ? 0 : this.outerLocalVariables.length; i < length; i++) {
            ...
            if (// finally block handling results in extra spills, avoid side effect.
            this.shouldCaptureInstance && firstSpill) {
    ...
    public TypeBinding resolveType(BlockScope blockScope, boolean skipKosherCheck) {
        ...
        if (this.constant != Constant.NotAConstant) {
            ...
            if (!argumentsTypeElided) {
                for (int i = 0; i < argumentsLength; i++) this.argumentTypes[i] = this.arguments[i].type.resolveType(/* check bounds*/
                blockScope, true);
            }
        ...
        // compute & capture interface function descriptor.
        super.resolveType(blockScope, skipKosherCheck);
        final boolean haveDescriptor = this.descriptor != null;
        if (// already complained in kosher*
        !skipKosherCheck && (!haveDescriptor || this.descriptor.typeVariables != Binding.NO_TYPE_VARIABLES))
            return this.resolvedType = null;
        this.binding = new MethodBinding(ClassFileConstants.AccPrivate | ClassFileConstants.AccSynthetic | ExtraCompilerModifiers.AccUnresolved, // will be fixed up later.
        CharOperation.concat(TypeConstants.ANONYMOUS_METHOD, Integer.toString(this.ordinal).toCharArray()), haveDescriptor ? this.descriptor.returnType : TypeBinding.VOID, // for now. 
        Binding.NO_PARAMETERS, haveDescriptor ? this.descriptor.thrownExceptions : Binding.NO_EXCEPTIONS, blockScope.enclosingSourceType());
        ...
        if (haveDescriptor) {
            ...
            if (parametersLength != argumentsLength) {
                ...
                if (// no interest in continuing to error check copy.
                argumentsTypeElided || this.original != this)
                    // FUBAR, bail out ...
                    return this.resolvedType = null;
                else {
        ...
        if (!argumentsTypeElided && !argumentsHaveErrors) {
            ...
            if (groundType != null) {
                ...
                if (!this.descriptor.isValidBinding()) {
                    ...
                    groundType != expectedSAMType) {
                        if (// the ground has shifted, are we still on firm grounds ? 
                        !groundType.isCompatibleWith(expectedSAMType, this.scope)) {
                            // report deliberately against block scope so as not to blame the lambda.
    ...
    private void mergeParameterNullAnnotations(BlockScope currentScope) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (ourTagBits == 0L) {
                ...
                if (// requested @NonNull not provided
                ourTagBits == TagBits.AnnotationNonNull) {
    ...
    public boolean isPertinentToApplicability(final TypeBinding targetType, final MethodBinding method) {
        ...
        if (// assumed to signal another primary error
        targetType == null)
            return true;
        if (argumentsTypeElided())
            return false;
    ...
    public void traverse(ASTVisitor visitor, BlockScope blockScope) {
        if (visitor.visit(this, blockScope)) {
            if (this.arguments != null) {
                ...
                for (int i = 0; i < argumentsLength; i++) this.arguments[i].traverse(visitor, this.scope);
            }
            if (this.body != null) {
    ...
    private // Simple minded analysis for code assist & potential compatibility.
    void analyzeShape() {
        ...
        if (this.body instanceof Expression) {
            ...
            // expression could be of type void - we can't determine that as we are working with unresolved expressions, for potential compatibility it is OK.
    ...
    public boolean isCompatibleWith(TypeBinding targetType, final Scope skope) {
        ...
        try {
            // if argument types are elided, we don't care for result expressions against *this* target, any valid target is OK.
            copy = cachedResolvedCopy(targetType, argumentsTypeElided(), false, null);
        ...
        for (int i = 0, length = returnExpressions.length; i < length; i++) {
            if (// inference variables can reach here during nested inference
            sam.returnType.isProperType(true) && this.enclosingScope.parameterCompatibilityLevel(returnExpressions[i].resolvedType, sam.returnType) == Scope.NOT_COMPATIBLE) {
    ...
    public boolean sIsMoreSpecific(TypeBinding s, TypeBinding t, Scope skope) {
        ...
        if (tSam == null || !tSam.isValidBinding())
            // See ORT8.test450415a for a case that slips through isCompatibleWith.
            return true;
        ...
        if (r1.isCompatibleWith(r2, skope))
            return true;
        // we expect a cached copy - otherwise control won't reach here.
        LambdaExpression copy = cachedResolvedCopy(s, /* any resolved copy is good */
        true, false, null);
        Expression[] returnExpressions = copy.resultExpressions;
    ...
    public void returnsExpression(Expression expression, TypeBinding resultType) {
        if (// Not in overload resolution context. result expressions not relevant.
        this.original == this)
            return;
        if (this.body instanceof Expression) {
            this.valueCompatible = resultType != null && resultType.id == TypeIds.T_void ? false : true;
            // while code is still being written and completed, we can't ask if it is a statement
            this.voidCompatible = this.assistNode ? true : ((Expression) this.body).statementExpression();
    ...
    public SyntheticArgumentBinding getSyntheticArgument(LocalVariableBinding actualOuterLocalVariable) {
        for (int i = 0, length = this.outerLocalVariables == null ? 0 : this.outerLocalVariables.length; i < length; i++) if (this.outerLocalVariables[i].actualOuterLocalVariable == actualOuterLocalVariable)
    ...
    public TypeBinding[] getMarkerInterfaces() {
        if (this.expectedType instanceof IntersectionTypeBinding18) {
            ...
            for (int i = 0, max = intersectionTypes.length; i < max; i++) {
                ...
                if (// only interfaces
                !typeBinding.isInterface() || // except for the samType itself
                TypeBinding.equalsEquals(samType, typeBinding) || // but Serializable is captured as a bitflag
                typeBinding.id == TypeIds.T_JavaIoSerializable) {
                    continue;
                }
    ...
    public ReferenceBinding getTypeBinding() {
        ...
        class LambdaTypeBinding extends ReferenceBinding {
            ...
            public String toString() {
                StringBuffer output = new //$NON-NLS-1$
