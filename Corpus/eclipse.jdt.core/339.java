...
public class BatchImageBuilder extends AbstractImageBuilder {
    ...
    // if annotations or secondary types have to be processed after the compile loop
    ...
    protected void cleanOutputFolders(boolean copyBack) throws CoreException {
        boolean deleteAll = JavaCore.CLEAN.equals(this.javaBuilder.javaProject.getOption(JavaCore.CORE_JAVA_BUILD_CLEAN_OUTPUT_FOLDER, true));
        if (deleteAll) {
            if (this.javaBuilder.participants != null)
                for (int i = 0, l = this.javaBuilder.participants.length; i < l; i++) this.javaBuilder.participants[i].cleanStarting(this.javaBuilder.javaProject);
            ...
            for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
                ...
                if (sourceLocation.hasIndependentOutputFolder) {
                    ...
                    final char[][] exclusionPatterns = isOutputFolder ? sourceLocation.exclusionPatterns : // ignore exclusionPatterns if output folder == another source folder... not this one
                    null;
                    final char[][] inclusionPatterns = isOutputFolder ? sourceLocation.inclusionPatterns : // ignore inclusionPatterns if output folder == another source folder... not this one
                    null;
                    sourceLocation.binaryFolder.accept(new IResourceProxyVisitor() {
                        ...
                        public boolean visit(IResourceProxy proxy) throws CoreException {
                            ...
                            if (// must walk children if inclusionPatterns != null
                            exclusionPatterns != null && inclusionPatterns == null)
        ...
        sourceLocation.sourceFolder.accept(new IResourceProxyVisitor() {
            ...
            public boolean visit(IResourceProxy proxy) throws CoreException {
                ...
                switch(proxy.getType()) {
                    case IResource.FILE:
                        if (org.eclipse.jdt.internal.core.util.Util.isJavaLikeFileName(proxy.getName()) || org.eclipse.jdt.internal.compiler.util.Util.isClassFileName(proxy.getName()))
                            return false;
                        ...
                        if (copiedResource.exists()) {
                            if (deletedAll) {
                                ...
                                createProblemFor(resource, null, Messages.bind(Messages.build_duplicateResource, id), BatchImageBuilder.this.javaBuilder.javaProject.getOption(JavaCore.CORE_JAVA_BUILD_DUPLICATE_RESOURCE, true));
                                return false;
                            }
                            // last one wins
                            copiedResource.delete(// last one wins
                            IResource.FORCE, // last one wins
                            null);
                        }
                        // ensure package folder exists
                        createFolder(partialPath.removeLastSegments(1), outputFolder);
    ...
    private void printStats() {
        ...
        System.out.print(">   parse: " + compilerStats.parseTime + " ms (" + ((int) (compilerStats.parseTime * 1000.0 / time)) / 10.0 + "%)");
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.print(", resolve: " + compilerStats.resolveTime + " ms (" + ((int) (compilerStats.resolveTime * 1000.0 / time)) / 10.0 + "%)");
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.print(", analyze: " + compilerStats.analyzeTime + " ms (" + ((int) (compilerStats.analyzeTime * 1000.0 / time)) / 10.0 + "%)");
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        System.out.println(", generate: " + compilerStats.generateTime + " ms (" + ((int) (compilerStats.generateTime * 1000.0 / time)) / 10.0 + "%)");
    }
