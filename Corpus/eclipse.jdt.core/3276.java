...
public class TypeUtilsProc extends BaseProcessor {
	 ...
	 * @return true if tests passed
	 */
    private boolean innerTestAsMemberOf(String method, DeclaredType declaredContainer, TypeElement containerElement) {
        DeclaredType longType = (DeclaredType) _elementUtils.getTypeElement("java.lang.Long").asType();
        ...
        for (String m : new String[] { "f", "m", "C", "D", "e" }) {
            ...
            if ("f".equals(m)) {
                ...
                if (!_typeUtils.isSameType(tm, tmExpected) && !(tm instanceof DeclaredTypeImpl && "targets.model.pc.AsMemberOf<java.lang.Long>.C".equals(tm.toString()))) {
                ...
                if (!_typeUtils.isSameType(tm, tmExpected)) {
                    reportError(method + ": member D: Expected type " + tmExpected + " but found " + tm);
                    return false;
                }
            } else if ("e".equals(m)) {
                ...
                if (!_typeUtils.isSameType(tm, tmExpected)) {
                    reportError(method + ": member e: Expected type " + tmExpected + " but found " + tm);
                    return false;
                }
            }
        }
