...
public final class ExternalAnnotationUtil {
    ...
    public static enum MergeStrategy implements  {
    ...
    public static String insertParameterAnnotation(String methodSignature, int paramIdx, char annotation, MergeStrategy mergeStrategy) {
        ...
        wrapper.start = 1;
        for (int i = 0; i < paramIdx; i++) wrapper.start = wrapper.computeEnd() + 1;
            ...
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (relation == 0) {
                        ...
                        if (line == null) {
                            // found only the selector at EOF, append right here, ignoring 'pending'
                            break;
                        }
                        ...
                        if (relation == 0) {
                            ...
                            if (annotationLine == null || annotationLine.isEmpty() || !//$NON-NLS-1$
                            annotationLine.startsWith(//$NON-NLS-1$
                            " ")) {
                                // push back, since not a signature line
                                nextLine = annotationLine;
                                // no annotated line yet, use unannotated line instead
                                annotationLine = line;
                            }
                            if (//$NON-NLS-1$
                            annotationLine.startsWith(//$NON-NLS-1$
                            " ")) {
                                switch(mergeStrategy) {
                                    case REPLACE_SIGNATURE:
                                        // unconditionally use annotatedSignature
                                        break;
                                    ...
                                    case ADD_ANNOTATIONS:
                                        ...
                                        break;
                                    default:
                                        JavaCore.getJavaCore().getLog().log(new Status(IStatus.ERROR, JavaCore.PLUGIN_ID, //$NON-NLS-1$
                                        "Unexpected value for enum MergeStrategy"));
                                }
                            }
    ...
    private static String updateSignature(String originalSignature, String annotatedSignature, int updatePosition, MergeStrategy mergeStrategy) {
        ...
        switch(updatePosition) {
            ...
            // parameter
            default:
                // may already contain annotations
                SignatureWrapper wrapper = new SignatureWrapper(originalSignature.toCharArray(), true, true);
                // possibly skipping type parameters
                wrapper.start = CharOperation.indexOf('(', wrapper.signature) + 1;
                for (int i = 0; i < updatePosition; i++) wrapper.start = wrapper.skipAngleContents(wrapper.computeEnd()) + 1;
                int start = wrapper.start;
    ...
    private static boolean updateType(StringBuffer buf, char[] oldType, char[] newType, MergeStrategy mergeStrategy) {
        ...
        try {
            ...
            if (match(buf, oWrap, nWrap, 'L', false) || match(buf, oWrap, nWrap, 'T', false)) {
            ...
            } else if (match(buf, oWrap, nWrap, '*', false) || match(buf, oWrap, nWrap, '+', false) || match(buf, oWrap, nWrap, '-', false)) {
                // annotation allowed after this (not included in oldType / newType)
                return true;
            } else {
                buf.append(oldType);
            }
        } catch (ArrayIndexOutOfBoundsException // from several locations inside match() or mergeAnnotation().
        aioobe) {
    ...
    private static boolean match(StringBuffer buf, SignatureWrapper sig1, SignatureWrapper sig2, char expected, boolean force) {
        ...
        if (match1 != match2) {
            StringBuilder msg = //$NON-NLS-1$
            new StringBuilder("Mismatching type structures ").append(sig1.signature).append(//$NON-NLS-1$
            " vs ").append(//$NON-NLS-1$
            sig2.signature);
        ...
        switch(mergeStrategy) {
            ...
            case OVERWRITE_ANNOTATIONS:
                switch(newAnn) {
                    ...
                    case NONNULL:
                        newS.start++;
                        // new exists and is not suppressed by "ADD & old exists"
                        buf.append(newAnn);
                        switch(oldAnn) {
                            case NULLABLE:
                            case // just skip
                ...
                break;
            default:
                // REPLACE_SIGNATURE does not reach this point, see initial check in updateType() //$NON-NLS-1$
    ...
    private static void writeFile(IFile annotationFile, StringBuffer head, String annotatedSignature, String nextLines, BufferedReader tailReader, IProgressMonitor monitor) throws CoreException, IOException {
        ...
        String line;
        while ((line = tailReader.readLine()) != null) head.append(line).append('\n');
        //$NON-NLS-1$
    ...
    private static void createNewFile(IFile file, String newContent, IProgressMonitor monitor) throws CoreException {
        ...
        try {
            //$NON-NLS-1$
            file.create(new ByteArrayInputStream(newContent.getBytes("UTF-8")), false, monitor);
    ...
    private static void ensureExists(IContainer container, IProgressMonitor monitor) throws CoreException {
        ...
        if (!(container instanceof IFolder))
            throw new CoreException(new Status(IStatus.ERROR, JavaCore.PLUGIN_ID, "not a folder: " + container));
    ...
    public static String[] annotateType(String originalSignature, String annotatedType, MergeStrategy mergeStrategy) {
        // prefix, orig, replacement, postfix
    ...
    public static String[] annotateParameterType(String originalSignature, String annotatedType, int paramIdx, MergeStrategy mergeStrategy) {
        ...
        // may already contain annotations
        SignatureWrapper wrapper = new SignatureWrapper(originalSignature.toCharArray(), true, true);
        // possibly skip type parameters
        wrapper.start = CharOperation.indexOf('(', wrapper.signature) + 1;
        for (int i = 0; i < paramIdx; i++) wrapper.start = wrapper.skipAngleContents(wrapper.computeEnd()) + 1;
        int start = wrapper.start;
        int end = wrapper.skipAngleContents(wrapper.computeEnd());
