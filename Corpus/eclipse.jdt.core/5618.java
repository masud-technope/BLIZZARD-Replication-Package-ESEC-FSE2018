...
public class NameLookup implements SuffixConstants {
    ...
    public  NameLookup(IPackageFragmentRoot[] packageFragmentRoots, HashtableOfArrayToObject packageFragments, ICompilationUnit[] workingCopies, Map rootToResolvedEntries) {
        ...
        if (workingCopies == null) {
            ...
            for (int i = 0, length = workingCopies.length; i < length; i++) {
                ...
                if (rootPosition == -1)
                    // working copy is not visible from this project (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=169970)
                    continue;
                ...
                if (existing == null || existing == JavaProjectElementInfo.NO_ROOTS) {
                    ...
                    if (existing instanceof PackageFragmentRoot) {
                        ...
                        if (// if not equal
                        rootPosition != // if not equal
                        exisitingPosition) {
                            this.packageFragments.put(pkgName, exisitingPosition < rootPosition ? new IPackageFragmentRoot[] { (PackageFragmentRoot) existing, root } : new IPackageFragmentRoot[] { root, (PackageFragmentRoot) existing });
                        }
                    } else {
    ...
    protected boolean acceptType(IType type, int acceptFlags, boolean isSourceType) {
        if (acceptFlags == 0 || acceptFlags == ACCEPT_ALL)
            // no flags or all flags, always accepted
            return true;
        try {
 ...
 */
    ...
    public IPackageFragment[] findPackageFragments(String name, boolean partialMatch, boolean patternMatch) {
        ...
        if (partialMatch || hasPatternChars) {
            ...
            for (int i = 0, length = keys.length; i < length; i++) {
                ...
                if (pkgName != null) {
                    boolean match = isStarPattern || (hasPatternChars ? CharOperation.match(lowercaseName, Util.concatCompoundNameToCharArray(pkgName), false) : Util.startsWithIgnoreCase(pkgName, splittedName, partialMatch));
                    if (match) {
    ...
    private IType findSecondaryType(String packageName, String typeName, IJavaProject project, boolean waitForIndexes, IProgressMonitor monitor) {
        ...
        try {
            ...
            if (secondaryTypePaths.size() > 0) {
                Map types = (Map) //$NON-NLS-1$
                secondaryTypePaths.get(//$NON-NLS-1$
    ...
    public Answer findType(String typeName, String packageName, boolean partialMatch, int acceptFlags, boolean checkRestrictions) {
        return findType(typeName, packageName, partialMatch, acceptFlags, /* consider secondary types */
        true, /* do NOT wait for indexes */
        false, checkRestrictions, null);
    }
    ...
    public Answer findType(String typeName, String packageName, boolean partialMatch, int acceptFlags, boolean considerSecondaryTypes, boolean waitForIndexes, boolean checkRestrictions, IProgressMonitor monitor) {
        if (packageName == null || packageName.length() == 0) {
            ...
            //$NON-NLS-1$
            if (findPackageFragments(packageName + "." + typeName, false) != null)
                return null;
        }
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (type != null) {
                ...
                if (// reverse map always contains resolved CP entry
                entry != null) {
        ...
        if (type != null) {
            ...
            if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=421902
            unit != null && unit.isWorkingCopy()) {
                IType[] types = null;
                try {
                    types = unit.getTypes();
                } catch (JavaModelException e) {
    ...
    public IType findType(String name, boolean partialMatch, int acceptFlags) {
        NameLookup.Answer answer = findType(name, partialMatch, acceptFlags, /*don't check restrictions*/
        false);
        return answer == null ? null : answer.type;
    }

    public Answer findType(String name, boolean partialMatch, int acceptFlags, boolean checkRestrictions) {
        return findType(name, partialMatch, acceptFlags, /*consider secondary types*/
        true, /*wait for indexes*/
        true, checkRestrictions, null);
    }
    ...
    public Answer findType(String name, boolean partialMatch, int acceptFlags, boolean considerSecondaryTypes, boolean waitForIndexes, boolean checkRestrictions, IProgressMonitor monitor) {
        ...
        if (index == 0) {
            // bug 377710 - e.g. ".Foo" (no package, but not "default" package)
            return null;
        }
...
*/
        ...
        try {
            if (!partialMatch) {
                ...
                if (classFile.existsUsingJarTypeCache()) {
                    ...
                    if (acceptType(type, /*not a source type*/
                    acceptFlags, false)) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (elementName.regionMatches(/*ignore case*/
                    true, 0, name, 0, matchLength)) {
                        IType type = ((ClassFile) classFile).getType();
                        String typeName = type.getElementName();
                        if (//not an anonymous type
                        typeName.length() > 0 && !Character.isDigit(typeName.charAt(0))) {
                            if (nameMatches(/*partial match*/
                            unqualifiedName, type, true) && acceptType(/*not a source type*/
                            type, acceptFlags, false))
                                requestor.acceptType(type);
                        }
                    }
                }
            ...
            if (!partialMatch) {
                try {
                    ...
                    for (int i = 0, length = compilationUnits.length; i < length; i++) {
                        ...
                        if (// accept type checks for existence
                        acceptType(/*a source type*/
                        type, acceptFlags, true)) {
                            requestor.acceptType(type);
                ...
                try {
                    ...
                    for (int i = 0, length = compilationUnits.length; i < length; i++) {
                        ...
                        try {
                            ...
                            for (int j = 0, typeLength = types.length; j < typeLength; j++) seekTypesInTopLevelType(name, firstDot, types[j], requestor, acceptFlags);
                        } catch (JavaModelException e) {
                        }
                    }
                } catch (JavaModelException e) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (memberType.getElementName().toLowerCase().startsWith(memberPrefix))
                if (isMemberTypePrefix) {
                    ...
                    if (acceptType(memberType, /*a source type*/
                    acceptFlags, true)) {
    ...
    protected boolean seekTypesInTopLevelType(String prefix, int firstDot, IType topLevelType, IJavaElementRequestor requestor, int acceptFlags) {
        if (!topLevelType.getElementName().toLowerCase().startsWith(prefix))
            return false;
        if (firstDot == -1) {
            if (acceptType(topLevelType, acceptFlags, /*a source type*/
            true)) {
            ...
            if (typeMap != null) {
                ...
                if (object instanceof IType) {
                    ...
                    if (!considerSecondaryTypes && !isPrimaryType(name, (IType) object, false))
                        return false;
                    if (acceptType(type, /*a source type*/
                    acceptFlags, true)) {
                        requestor.acceptType(type);
            ...
            if (typeMap != null) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (object instanceof IType) {
                        ...
                        IType[] topLevelTypes = (IType[]) object;
                        for (int i = 0, length = topLevelTypes.length; i < length; i++) seekTypesInTopLevelType(name, firstDot, topLevelTypes[i], requestor, acceptFlags);
                    }
                }
            }
        }
