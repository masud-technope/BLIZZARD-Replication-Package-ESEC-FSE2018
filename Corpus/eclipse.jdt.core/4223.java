...
public class AllocationExpression extends Expression implements IPolyExpression, Invocation {
    ...
    // when allocation a Closeable store a pre-liminary tracking variable here
    ...
    private SimpleLookupTable /*<PMB,IC18>*/
    inferenceContexts;
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (this.arguments != null) {
            ...
            boolean hasResourceWrapperType = analyseResources && this.resolvedType instanceof ReferenceBinding && ((ReferenceBinding) this.resolvedType).hasTypeBit(TypeIds.BitWrapperCloseable);
            for (int i = 0, count = this.arguments.length; i < count; i++) {
                flowInfo = this.arguments[i].analyseCode(currentScope, flowContext, flowInfo).unconditionalInits();
                // if argument is an AutoCloseable insert info that it *may* be closed (by the target method, i.e.)
                if (// allocation of wrapped closeables is analyzed specially
                analyseResources && !hasResourceWrapperType) {
        ...
        if ((declaringClass.isMemberType() && !declaringClass.isStatic()) || (declaringClass.isLocalType() && !methodScope.isStatic && methodScope.isLambdaScope())) {
            ...
            currentScope.tagAsAccessingEnclosingInstanceStateOf(this.binding.declaringClass.enclosingType(), /* type variable access */
            false);
        ...
        // TODO whitelist of ctors that cannot throw any exc.??
    ...
    public void checkCapturedLocalInitializationIfNecessary(ReferenceBinding checkedType, BlockScope currentScope, FlowInfo flowInfo) {
        if (((checkedType.tagBits & (TagBits.AnonymousTypeMask | TagBits.LocalTypeMask)) == TagBits.LocalTypeMask) && // only check external allocations
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (// null for enum constant body
        this.type != null) {
        ...
        if (this.syntheticAccessor == null) {
            codeStream.invoke(Opcodes.OPC_invokespecial, codegenBinding, /* default declaringClass */
            ...
            codeStream.invoke(Opcodes.OPC_invokespecial, this.syntheticAccessor, /* default declaringClass */
            null, this.typeArguments);
        }
        if (valueRequired) {
 ...
 */
    ...
    public StringBuffer printExpression(int indent, StringBuffer output) {
        if (// type null for enum constant initializations
        this.type != null) {
            //$NON-NLS-1$
            output.append("new ");
        }
        ...
        if (this.constant != Constant.NotAConstant) {
            ...
            if (this.type != null) {
                ...
                checkParameterizedAllocation: {
                    if (// disallow new X<String>.Y<Integer>()
                    this.type instanceof ParameterizedQualifiedTypeReference) {
            ...
            if (this.arguments != null) {
			   ...
			   not return the partially resolved type.
				 */
                    if (isDiamond) {
                        // not the partially cooked this.resolvedType
                        return null;
                    }
	...
	*/
    ...
    public void checkTypeArgumentRedundancy(ParameterizedTypeBinding allocationType, final BlockScope scope) {
        ...
        if (allocationType.arguments == null)
            return;
        // diamond can't occur with explicit type args for constructor
    ...
    public void traverse(ASTVisitor visitor, BlockScope scope) {
        if (visitor.visit(this, scope)) {
            ...
            if (// enum constant scenario
            this.type != null) {
                this.type.traverse(visitor, scope);
            }
            if (this.arguments != null) {
                for (int i = 0, argumentsLength = this.arguments.length; i < argumentsLength; i++) this.arguments[i].traverse(visitor, scope);
            }
        }
 ...
 */
    ...
    public boolean isPolyExpression(MethodBinding method) {
        return (this.expressionContext == ASSIGNMENT_CONTEXT || this.expressionContext == INVOCATION_CONTEXT) && this.type != null && (this.type.bits & ASTNode.IsDiamond) != 0;
    }
