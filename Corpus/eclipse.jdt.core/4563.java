...
public abstract class AbstractCommentParser {
    ...
    public static final int COMPIL_PARSER = 0x00000001;

    public static final int DOM_PARSER = 0x00000002;
    ...
    private boolean parseHref() throws InvalidInputException {
        ...
        if (Character.toLowerCase(readChar()) == 'a') {
            ...
            if (readToken() == TerminalTokens.TokenNameIdentifier) {
                ...
                this.currentTokenType = -1;
                try {
                    if (CharOperation.equals(this.scanner.getCurrentIdentifierSource(), new char[] { 'h', 'r', 'e', 'f' }, false) && readToken() == TerminalTokens.TokenNameEQUAL) {
                        // do not update line end
                        this.currentTokenType = -1;
                        if (readToken() == TerminalTokens.TokenNameStringLiteral) {
                            // do not update line end
                            this.currentTokenType = // do not update line end
                            -1;
    ...
    protected boolean parseReference() throws InvalidInputException {
        ...
        try {
                ...
                switch(token) {
                    ...
                    TerminalTokens.TokenNameStringLiteral:
                        ...
                        if (this.tagValue == TAG_VALUE_VALUE) {
                            if (this.sourceParser != null)
                                this.sourceParser.problemReporter().javadocInvalidValueReference(start, getTokenEndPosition());
                        } else {
            ...
            if (reference == null) {
                ...
                if (this.sourceParser != null)
                    this.sourceParser.problemReporter().javadocMissingReference(this.tagSourceStart, this.tagSourceEnd);
                return false;
            }
        ...
        nextChar: while (this.index < this.source.length) {
            switch(ch) {
                case '*':
                    ...
                    break;
                case '/':
                    if (// valid only if a star was previous character
