 ...
 */
    public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding, AccessRestriction accessRestriction) {
        ...
        CompilationUnitDeclaration unit = SourceTypeConverter.buildCompilationUnit(// ignore secondary types, to improve laziness
        new ISourceType[] { sourceType }, // need member types
        SourceTypeConverter.MEMBER_TYPE | (this.lookupEnvironment.globalOptions.sourceLevel >= ClassFileConstants.JDK1_8 ? SourceTypeConverter.METHOD : 0), // no need for field initialization
        ...
        if (unit != null) {
            try {
                ...
                this.lookupEnvironment.completeTypeBindings(/*build constructor only*/
                unit, true);
            } catch (AbortCompilation e) {
            }
        }
        ...
        if (superBinding != null) {
            ...
            if (typeBinding.isHierarchyInconsistent()) {
                if (superBinding.problemId() == ProblemReasons.NotFound) {
                    ...
                    if (// check whether subclass of Object due to broken hierarchy (as opposed to explicitly extending it)
                    superclassName != null) {
 ...
 */
    private IType[] findSuperInterfaces(IGenericType type, ReferenceBinding typeBinding) {
        ...
        if (type instanceof IBinaryType) {
            ...
            if (// if anonymous type
            sourceType.isAnonymous()) {
        ...
        for (int current = this.typeIndex; current >= 0; current--) {
            ...
            if (typeBinding instanceof SourceTypeBinding) {
                ...
                if (scope != null) {
                    ...
                    int length;
                    ReferenceBinding[] interfaceBindings = typeBinding.superInterfaces();
                    if (// check for interfaceBindings being null (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=139689)
                    superInterfaces != null && (length = superInterfaces.length) > (interfaceBindings == null ? 0 : interfaceBindings.length)) {
 ...
 */
    private void reportHierarchy(IType focus, TypeDeclaration focusLocalType, ReferenceBinding binaryTypeBinding) {
        ...
        for (int current = this.typeIndex; current >= 0; current--) {
            ...
            IType superclass;
            if (// do not connect interfaces to Object
    ...
    public void resolve(IGenericType suppliedType) {
        try {
            if (suppliedType.isBinaryType()) {
                BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment.cacheBinaryType((IBinaryType) suppliedType, /*don't need field and method (bug 125067)*/
                false, /*no access restriction*/
                null);
        ...
        try {
            ...
            for (int i = 0; i < openablesLength; i++) {
                ...
                if (openable instanceof org.eclipse.jdt.core.ICompilationUnit) {
                    ...
                    if (// case of hierarchy on region
                    localTypes == null) {
                        containsLocalType = true;
                    } else {
                        IPath path = cu.getPath();
                        containsLocalType = cu.isWorkingCopy() ? /* presume conservatively */
                        true : localTypes.contains(path.toString());
                    }
                    // build parsed unit
                    ...
                    if (cu.isOpen()) {
                        ...
                        int flags = !containsLocalType ? SourceTypeConverter.MEMBER_TYPE | (isJava8 ? SourceTypeConverter.METHOD : 0) : SourceTypeConverter.FIELD_AND_METHOD | SourceTypeConverter.MEMBER_TYPE | SourceTypeConverter.LOCAL_TYPE;
                    ...
                    if (binaryType != null) {
                        try {
                            BinaryTypeBinding binaryTypeBinding = this.lookupEnvironment.cacheBinaryType(/*don't need field and method (bug 125067)*/
                            binaryType, false, /*no access restriction*/
                            null);
            ...
            for (int i = 0; i < unitsIndex; i++) {
                ...
                if (parsedUnit != null) {
                    try {
                        if (// NB: no-op if method bodies have been already parsed
                        hasLocalType[i]) {
            ...
            if (focusBinaryBinding == null && focus != null && focus.isBinary()) {
                ...
                if (focusBinaryBinding == null || (focusBinaryBinding.tagBits & TagBits.HasUnresolvedSuperclass) != 0)
                    return;
            }
            reportHierarchy(focus, focusLocalType, focusBinaryBinding);
        } catch (ClassCastException // work-around for 1GF5W1S - can happen in case duplicates are fed to the hierarchy with binaries hiding sources
        e) {
        } catch (AbortCompilation // ignore this exception for now since it typically means we cannot find java.lang.Object
        e) {
