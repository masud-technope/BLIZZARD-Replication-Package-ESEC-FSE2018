...
public class BinaryExpression extends OperatorExpression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        return right.analyseCode(currentScope, flowContext, left.analyseCode(currentScope, flowContext, flowInfo).unconditionalInits()).unconditionalInits();
    }
    ...
    public void computeConstant(BlockScope scope, int leftId, int rightId) {
        ...
        if ((this.left.constant != Constant.NotAConstant) && (this.right.constant != Constant.NotAConstant)) {
            try {
                this.constant = Constant.computeConstantOperation(this.left.constant, leftId, (this.bits & OperatorMASK) >> OperatorSHIFT, this.right.constant, rightId);
            ...
            this.optimizedBooleanConstant(leftId, (this.bits & OperatorMASK) >> OperatorSHIFT, rightId);
        }
    }
        ...
        if ((left.implicitConversion & 0xF) == T_boolean) {
            ...
            if ((condConst = right.optimizedBooleanConstant()) != NotAConstant) {
                if (condConst.booleanValue() == true) {
                    ...
                    Label internalTrueLabel = new Label(codeStream);
                    left.generateOptimizedBoolean(currentScope, codeStream, internalTrueLabel, falseLabel, false);
                    internalTrueLabel.place();
                    right.generateOptimizedBoolean(currentScope, codeStream, trueLabel, falseLabel, false);
                    if (valueRequired) {
                        if ((bits & OnlyValueRequiredMASK) != 0) {
        ...
        if ((left.implicitConversion & 0xF) == T_boolean) {
            if ((condConst = left.optimizedBooleanConstant()) != NotAConstant) {
                if (condConst.booleanValue() == true) {
                    ...
                    right.generateOptimizedBoolean(currentScope, codeStream, trueLabel, internalFalseLabel, false);
            ...
            if ((condConst = right.optimizedBooleanConstant()) != NotAConstant) {
                if (condConst.booleanValue() == true) {
                    // x | <something equivalent to true>
                    Label internalFalseLabel = new Label(codeStream);
                    left.generateOptimizedBoolean(currentScope, codeStream, trueLabel, internalFalseLabel, false);
                    internalFalseLabel.place();
                    right.generateOptimizedBoolean(currentScope, codeStream, trueLabel, falseLabel, false);
                    if (valueRequired) {
                        if ((bits & OnlyValueRequiredMASK) != 0) {
    ...
    public void generateOptimizedStringConcatenation(BlockScope blockScope, CodeStream codeStream, int typeID) {
        if ((((bits & OperatorMASK) >> OperatorSHIFT) == PLUS) && ((bits & ReturnTypeIDMASK) == T_String)) {
            if (constant != NotAConstant) {
                ...
                int pc = codeStream.position;
                left.generateOptimizedStringConcatenation(blockScope, codeStream, left.implicitConversion & 0xF);
                codeStream.recordPositionsFrom(pc, left.sourceStart);
                pc = codeStream.position;
                right.generateOptimizedStringConcatenation(blockScope, codeStream, right.implicitConversion & 0xF);
    ...
    public void generateOptimizedStringConcatenationCreation(BlockScope blockScope, CodeStream codeStream, int typeID) {
        if ((((bits & OperatorMASK) >> OperatorSHIFT) == PLUS) && ((bits & ReturnTypeIDMASK) == T_String)) {
            ...
            // invokespecial: java.lang.StringBuffer.<init>(Ljava.lang.String;)V
                ...
                int pc = codeStream.position;
                left.generateOptimizedStringConcatenationCreation(blockScope, codeStream, left.implicitConversion & 0xF);
                codeStream.recordPositionsFrom(pc, left.sourceStart);
                pc = codeStream.position;
                right.generateOptimizedStringConcatenation(blockScope, codeStream, right.implicitConversion & 0xF);
                codeStream.recordPositionsFrom(pc, right.sourceStart);
            ...
            case AND_AND:
                ...
                if ((cst = left.optimizedBooleanConstant()) != NotAConstant) {
                    ...
                    cst.booleanValue() == false) {
                        // constant(false)
                        optimizedBooleanConstant = // constant(false)
                        cst;
                        return;
            ...
            case OR_OR:
                if ((cst = left.optimizedBooleanConstant()) != NotAConstant) {
                    ...
                    cst.booleanValue() == true) {
                        // constant(true)
                        optimizedBooleanConstant = // constant(true)
                        cst;
                        return;
