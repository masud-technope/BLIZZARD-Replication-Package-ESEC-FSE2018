...
public class BindingKeyResolver extends BindingKeyParser {
    ...
    public void consumeAnyCapture(final int capture18id, final int position) {
        ...
        if (outerParsedUnit == null)
            return;
        final Binding wildcardBinding = // 0 may happen for CaptureBinding18
        this.types.size() > 0 ? ((BindingKeyResolver) this.types.get(0)).compilerBinding : null;
    ...
    public void consumeLocalType(char[] uniqueKey) {
        ...
        for (int i = 0; i < this.parsedUnit.localTypeCount; i++) if (CharOperation.equals(uniqueKey, localTypeBindings[i].computeUniqueKey(/*not a leaf*/
        false))) {
        ...
        for (int i = 0, methodLength = methods.length; i < methodLength; i++) {
            ...
            if (CharOperation.equals(selector, method.selector) || (selector.length == 0 && method.isConstructor())) {
                ...
                if (CharOperation.equals(signature, methodSignature)) {
                    ...
                    this.methodBinding = this.environment.updatePolymorphicMethodReturnType(polymorphicMethod, getType(Signature.getReturnType(signature)));
                    this.compilerBinding = this.methodBinding;
                    return;
                }
            }
    ...
    public void consumeParameterizedType(char[] simpleTypeName, boolean isRaw) {
        ...
        if (simpleTypeName != null) {
            ...
            if (enclosing != null)
                enclosing = (ReferenceBinding) this.environment.convertToRawType(enclosing, /*do not force conversion of enclosing types*/
                false);
    ...
    public void consumeRawType() {
        if (this.typeBinding == null)
            return;
        this.typeBinding = this.environment.convertToRawType(this.typeBinding, /*do not force conversion of enclosing types*/
        false);
    }
    ...
    public void consumeTopLevelType() {
        ...
        if (this.parsedUnit != null && this.compiler != null && !this.resolvedUnits.containsKey(fileName = this.parsedUnit.getFileName())) {
            // unit is resolved only once thanks to the resolvedUnits protection
        ...
        switch(kind) {
            ...
            case Wildcard.SUPER:
                ...
                if (boundBinding instanceof TypeBinding) {
                    this.typeBinding = this.environment.createWildcard((ReferenceBinding) this.typeBinding, this.wildcardRank, (TypeBinding) boundBinding, /*no extra bound*/
                    null, kind);
                } else {
                    this.typeBinding = null;
                }
            ...
            case Wildcard.UNBOUND:
                this.typeBinding = this.environment.createWildcard((ReferenceBinding) this.typeBinding, this.wildcardRank, /*no bound*/
                null, /*no extra bound*/
                null, kind);
                break;
        }
    ...
    private TypeBinding getTypeBinding(char[] simpleTypeName) {
        ...
        TypeDeclaration[] typeDeclarations = this.typeDeclaration == null ? (this.parsedUnit == null ? null : this.parsedUnit.types) : this.typeDeclaration.memberTypes;
