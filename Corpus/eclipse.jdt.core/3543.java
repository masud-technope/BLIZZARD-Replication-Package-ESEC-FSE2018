...
*/
    ...
    boolean areParametersEqual(MethodBinding one, MethodBinding two) {
        ...
        if (length != twoArgs.length)
            return false;
        for (int i = 0; i < length; i++) if (!areTypesEqual(oneArgs[i], twoArgs[i]))
            return false;
        return true;
    }
...
*/
    ...
    void checkInheritedMethods(MethodBinding[] methods, int length) {
        ...
        if (// ignore concrete methods for interfaces
        !type.isInterface()) {
            for (// Remember that only one of the methods can be non-abstract
            int i = length; // Remember that only one of the methods can be non-abstract
            --i >= 0; ) {
        ...
        index = 0;
        for (int i = length; --i >= 0; ) if (methods[i] != concreteMethod)
        ...
        if (concreteMethod.thrownExceptions != NoExceptions)
            for (int i = abstractMethods.length; --i >= 0; ) this.checkExceptions(concreteMethod, abstractMethods[i]);
    }
...
*/
    void checkMethods() {
        ...
        boolean skipInheritedMethods = mustImplementAbstractMethods && this.type.superInterfaces() == NoSuperInterfaces && this.type.superclass() != null && // have a single concrete superclass so only check overridden methods
        !this.type.superclass().isAbstract();
...
*/
    void computeInheritedMethods() {
        ...
        ReferenceBinding superType = this.type.isClass() ? this.type.superclass() : // check interface methods against Object
        ...
        while (superType != null) {
            if (superType.isValidBinding()) {
                ...
                nextMethod: for (int m = methods.length; --m >= 0; ) {
                    ...
                    if (nonVisible != null)
                        for (int i = 0, l = nonVisible.length; i < l; i++) if (doesMethodOverride(nonVisible[i], inheritedMethod))
                            continue nextMethod;
    ...
    boolean mustImplementAbstractMethod(MethodBinding abstractMethod) {
        ...
        if (declaringClass.isClass()) {
            while (superclass.isAbstract() && superclass != declaringClass) // find the first concrete superclass or the abstract declaringClass
    ...
    ProblemReporter problemReporter(MethodBinding currentMethod) {
        ...
        currentMethod.declaringClass == type)
    ...
    ReferenceBinding[] resolvedExceptionTypesFor(MethodBinding method) {
        ...
        for (int i = exceptions.length; --i >= 0; ) exceptions[i] = BinaryTypeBinding.resolveType(exceptions[i], this.environment, true);
        return exceptions;
    }
