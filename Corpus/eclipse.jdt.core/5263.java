...
public class CompletionJavadocParser extends JavadocParser {
    ...
    protected boolean commentParse() {
        ...
        // bug 429340: completion parser needs to parse the whole comment
    ...
    protected Object createArgumentReference(char[] name, int dim, boolean isVarargs, Object typeRef, long[] dimPositions, long argNamePos) throws InvalidInputException {
        ...
        boolean inCompletion = // completion cursor is between first and last stacked identifiers
        (refStart <= this.cursorLocation && this.cursorLocation <= refEnd) || // or it's a completion on empty token
        ((refStart == (refEnd + 1) && refEnd == this.cursorLocation));
    ...
    protected Object createFieldReference(Object receiver) throws InvalidInputException {
        ...
        boolean inCompletion = // completion cursor is between first and last stacked identifiers
        (refStart <= (this.cursorLocation + 1) && this.cursorLocation <= refEnd) || // or it's a completion on empty token
        ((refStart == (refEnd + 1) && refEnd == this.cursorLocation)) || // or it's a completion just after the member separator with an identifier after the cursor
        (this.memberStart == this.cursorLocation);
        if (inCompletion) {
            ...
            if (CompletionEngine.DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "	completion field=" + this.completionNode);
            }
            return this.completionNode;
        }
    ...
    protected Object createMethodReference(Object receiver, List arguments) throws InvalidInputException {
        ...
        if (inCompletion) {
            ...
            if (CompletionEngine.DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "	completion method=" + this.completionNode);
            }
            return this.completionNode;
        }
        ...
        nbIdentifiers > 1) {
            for (int i = startPtr; i < this.identifierPtr; i++) {
                ...
                if (start <= this.cursorLocation && this.cursorLocation <= end) {
                    if (i == startPtr) {
                        this.completionNode = new CompletionOnJavadocSingleTypeReference(this.identifierStack[startPtr], this.identifierPositionStack[startPtr], this.tagSourceStart, this.tagSourceEnd);
                    } else {
    ...
    protected boolean parseParam() throws InvalidInputException {
        ...
        boolean inCompletion = // completion cursor is between first and last stacked identifiers
        (startPosition <= (this.cursorLocation + 1) && this.cursorLocation <= endPosition) || // or it's a completion on empty token
        ((startPosition == (endPosition + 1) && endPosition == this.cursorLocation));
        if (inCompletion) {
            if (this.completionNode == null) {
                if (isTypeParam) {
    ...
    protected boolean parseTag(int previousPosition) throws InvalidInputException {
        ...
        boolean inCompletion = // completion cursor is between first and last stacked identifiers
        (this.tagSourceStart <= (this.cursorLocation + 1) && this.cursorLocation <= this.tagSourceEnd) || // or it's a completion on empty token
        ((this.tagSourceStart == (this.tagSourceEnd + 1) && this.tagSourceEnd == this.cursorLocation));
        if (inCompletion) {
    ...
    protected boolean pushParamName(boolean isTypeParam) {
        if (super.pushParamName(isTypeParam)) {
            ...
            if (expression.sourceStart <= (this.cursorLocation + 1) && this.cursorLocation <= expression.sourceEnd) {
                ...
                if (CompletionEngine.DEBUG) {
                    System.out.println("	completion param=" + //$NON-NLS-1$
                    this.completionNode);
                }
            }
            return true;
                ...
                while (!this.scanner.atEnd() && this.completionNode == null && !this.abort) {
                    ...
                    switch(token) {
                        case TerminalTokens.TokenNameStringLiteral:
                            ...
                            if ((strStart = this.scanner.getCurrentTokenStartPosition() + 1) <= this.cursorLocation && this.cursorLocation <= (strEnd = this.scanner.getCurrentTokenEndPosition() - 1)) {
