...
public class LineBreaksPreparator extends ASTVisitor {
    ...
    public boolean visit(CompilationUnit node) {
        ...
        if (!types.isEmpty()) {
            ...
            for (int i = 1; i < types.size(); i++) putBlankLinesBefore(types.get(i), this.options.blank_lines_between_type_declarations);
        }
        return true;
    }
    ...
    public boolean visit(PackageDeclaration node) {
        ...
        if (blanks > 0) {
            ...
            int firstTokenIndex = annotations.isEmpty() ? this.tm.firstIndexBefore(node.getName(), TokenNamepackage) : this.tm.firstIndexIn(annotations.get(0), -1);
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        if (node.getName().getStartPosition() == -1)
            ...
            return true;
        breakLineBefore(node);
        handleBracedCode(node, node.getName(), this.options.brace_position_for_type_declaration, this.options.indent_body_declarations_compare_to_type_header, this.options.insert_new_line_in_empty_type_declaration);
        this.declarationModifierVisited = false;
        return true;
    }
    ...
    private void handleBodyDeclarations(List<BodyDeclaration> bodyDeclarations) {
        ...
        for (BodyDeclaration bodyDeclaration : bodyDeclarations) {
            if (previous == null) {
                ...
                } else if (bodyDeclaration instanceof MethodDeclaration || bodyDeclaration instanceof AnnotationTypeMemberDeclaration) {
    ...
    private boolean sameChunk(BodyDeclaration bd1, BodyDeclaration bd2) {
        ...
        if (bd1 instanceof AbstractTypeDeclaration && bd2 instanceof AbstractTypeDeclaration)
            return true;
        if ((bd1 instanceof FieldDeclaration || bd1 instanceof Initializer) && (bd2 instanceof FieldDeclaration || bd2 instanceof Initializer))
            return true;
        return false;
    }
    ...
    private void putBlankLinesBefore(ASTNode node, int linesCount) {
        ...
        while (index > 0 && this.tm.get(index - 1).tokenType == TokenNameCOMMENT_JAVADOC) index--;
        this.tm.get(index).putLineBreaksBefore(linesCount + 1);
    }

    @Override
    public boolean visit(EnumDeclaration node) {
        handleBracedCode(node, node.getName(), this.options.brace_position_for_enum_declaration, this.options.indent_body_declarations_compare_to_enum_declaration_header, this.options.insert_new_line_in_empty_enum_declaration);
        handleBodyDeclarations(node.bodyDeclarations());
        ...
        int index = enumConstants.isEmpty() ? this.tm.firstIndexAfter(node.getName(), TokenNameLBRACE) + 1 : this.tm.firstIndexAfter(enumConstants.get(enumConstants.size() - 1), -1);
        for (; ; index++) {
    ...
    public boolean visit(AnnotationTypeDeclaration node) {
        handleBracedCode(node, node.getName(), this.options.brace_position_for_annotation_type_declaration, this.options.indent_body_declarations_compare_to_annotation_declaration_header, this.options.insert_new_line_in_empty_annotation_declaration);
        handleBodyDeclarations(node.bodyDeclarations());
    ...
    public boolean visit(AnonymousClassDeclaration node) {
        if (node.getParent() instanceof EnumConstantDeclaration) {
            handleBracedCode(node, null, this.options.brace_position_for_enum_constant, this.options.indent_body_declarations_compare_to_enum_constant_header, this.options.insert_new_line_in_empty_enum_constant);
        } else {
            handleBracedCode(node, null, this.options.brace_position_for_anonymous_type_declaration, this.options.indent_body_declarations_compare_to_type_header, this.options.insert_new_line_in_empty_anonymous_type_declaration);
        }
        handleBodyDeclarations(node.bodyDeclarations());
        return true;
    }
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        int rParen = node.getBody() == null ? this.tm.lastIndexIn(node, TokenNameRPAREN) : this.tm.firstIndexBefore(node.getBody(), TokenNameRPAREN);
        ...
        handleBracedCode(node.getBody(), null, bracePosition, this.options.indent_statements_compare_to_body, this.options.insert_new_line_in_empty_method_body);
        Token openBrace = this.tm.firstTokenIn(node.getBody(), TokenNameLBRACE);
        if (// if not, these are empty braces
        openBrace.getLineBreaksAfter() > 0)
    ...
    public boolean visit(Block node) {
        ...
        if (parent instanceof MethodDeclaration)
            // braces have been handled in #visit(MethodDeclaration)
            return true;
        ...
        handleBracedCode(node, null, bracePosition, this.options.indent_statements_compare_to_block, this.options.insert_new_line_in_empty_block);
        return true;
    }

    @Override
    public boolean visit(SwitchStatement node) {
        handleBracedCode(node, node.getExpression(), this.options.brace_position_for_switch, this.options.indent_switchstatements_compare_to_switch, true);
        List<Statement> statements = node.statements();
        if (this.options.indent_switchstatements_compare_to_cases) {
            int nonBreakStatementEnd = -1;
            for (Statement statement : statements) {
                boolean isBreaking = statement instanceof BreakStatement || statement instanceof ReturnStatement || statement instanceof ContinueStatement || statement instanceof Block;
        ...
        for (Statement statement : statements) {
            if (statement instanceof Block)
                // will add break in visit(Block) if necessary
                continue;
            if (this.options.put_empty_statement_on_new_line || !(statement instanceof EmptyStatement))
                breakLineBefore(statement);
        }
    ...
    public boolean visit(EnumConstantDeclaration node) {
        ...
        while (this.tm.get(lParen).isComment()) lParen++;
        if (this.tm.get(lParen).tokenType == TokenNameLPAREN) {
            int rParen = node.getAnonymousClassDeclaration() == null ? this.tm.lastIndexIn(node, TokenNameRPAREN) : this.tm.firstIndexBefore(node.getAnonymousClassDeclaration(), TokenNameRPAREN);
    ...
    public boolean visit(IfStatement node) {
        ...
        if (elseNode != null) {
            ...
            boolean keepElseOnSameLine = (this.options.keep_else_statement_on_same_line) || (this.options.compact_else_if && (elseNode instanceof IfStatement));
            if (!keepElseOnSameLine)
                handleLoopBody(elseNode);
        }
        boolean keepThenOnSameLine = this.options.keep_then_statement_on_same_line || (this.options.keep_simple_if_on_one_line && elseNode == null);
    ...
    public boolean visit(ConstructorInvocation node) {
        int lParen = node.arguments().isEmpty() ? this.tm.lastIndexIn(node, TokenNameLPAREN) : this.tm.firstIndexBefore((ASTNode) node.arguments().get(0), TokenNameLPAREN);
        int rParen = this.tm.lastIndexIn(node, TokenNameRPAREN);
        handleParenthesesPositions(lParen, rParen, this.options.parenthesis_positions_in_method_invocation);
        return true;
    }
    ...
    private void handleBracedCode(ASTNode node, ASTNode nodeBeforeOpenBrace, String bracePosition, boolean indentBody, boolean newLineInEmpty) {
        int openBraceIndex = nodeBeforeOpenBrace == null ? this.tm.firstIndexIn(node, TokenNameLBRACE) : this.tm.firstIndexAfter(nodeBeforeOpenBrace, TokenNameLBRACE);
        int closeBraceIndex = this.tm.lastIndexIn(node, TokenNameRBRACE);
        Token openBraceToken = this.tm.get(openBraceIndex);
        Token closeBraceToken = this.tm.get(closeBraceIndex);
    ...
    private void indent(ASTNode node) {
        ...
        while (startIndex > 0 && this.tm.get(startIndex - 1).isComment()) startIndex--;
        ...
        switch(positionsSetting) {
            ...
            case DefaultCodeFormatterConstants.PRESERVE_POSITIONS:
                ...
                if (always || this.tm.countLineBreaksBetween(this.tm.get(openingParenIndex), afterOpening) > 0) {
                    afterOpening.setWrapPolicy(new WrapPolicy(WrapMode.WHERE_NECESSARY, openingParenIndex, this.options.indentation_size));
            ...
            default:
                ...
                "Unrecognized parentheses positions setting: " + positionsSetting);
        }
    }
