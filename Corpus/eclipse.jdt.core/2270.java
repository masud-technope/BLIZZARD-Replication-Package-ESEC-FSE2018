...
public class ConstructorDeclaration extends AbstractMethodDeclaration {
    ...
    public void analyseCode(ClassScope classScope, InitializationFlowContext initializerFlowContext, FlowInfo flowInfo) {
        ...
        if (isRecursive(/*lazy initialized visited list*/
        null)) {
            this.scope.problemReporter().recursiveConstructorInvocation(this.constructorCall);
        }
        try {
            ExceptionHandlingFlowContext constructorContext = new ExceptionHandlingFlowContext(initializerFlowContext.parent, this, binding.thrownExceptions, scope, FlowInfo.DEAD_END);
        ...
        try {
            ...
            if (e.compilationResult == CodeStream.RESTART_IN_WIDE_MODE) {
                try {
                    ...
                    int problemsLength;
                    IProblem[] problems = scope.referenceCompilationUnit().compilationResult.getAllProblems();
                    IProblem[] problemsCopy = new IProblem[problemsLength = problems.length];
                    System.arraycopy(problems, 0, problemsCopy, 0, problemsLength);
    ...
    private void internalGenerateCode(ClassScope classScope, ClassFile classFile) {
        ...
        if ((!binding.isNative()) && (!binding.isAbstract())) {
            ...
            if (declaringClass.isNestedType()) {
                ...
                scope.computeLocalVariablePositions(nestedType.enclosingInstancesSlotSize + 1, codeStream);
    ...
    public StringBuffer printBody(int indent, StringBuffer output) {
        ...
        if (statements != null) {
            for (int i = 0; i < statements.length; i++) {
                output.append('\n');
                //$NON-NLS-1$
                statements[i].printStatement(//$NON-NLS-1$
                indent, //$NON-NLS-1$
                output);
            }
        }
    ...
    public void resolveStatements() {
        ...
        if (this.constructorCall != null) {
            ...
            if (this.binding != null && this.binding.declaringClass.id == T_Object && this.constructorCall.accessMode != ExplicitConstructorCall.This) {
    ...
    public void traverse(ASTVisitor visitor, ClassScope classScope) {
        if (visitor.visit(this, classScope)) {
            ...
            if (arguments != null) {
                ...
                for (int i = 0; i < argumentLength; i++) arguments[i].traverse(visitor, scope);
            }
            if (thrownExceptions != null) {
                int thrownExceptionsLength = thrownExceptions.length;
                for (int i = 0; i < thrownExceptionsLength; i++) thrownExceptions[i].traverse(visitor, scope);
            }
            if (constructorCall != null)
                constructorCall.traverse(visitor, scope);
            if (statements != null) {
                int statementsLength = statements.length;
                for (int i = 0; i < statementsLength; i++) statements[i].traverse(visitor, scope);
            }
        }
