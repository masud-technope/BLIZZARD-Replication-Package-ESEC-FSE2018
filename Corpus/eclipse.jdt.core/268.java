...
public class CompilationResult {
    ...
    // record the fact some toplevel types have inconsistent hierarchies
    ...
    // check for secondary types which were created after the initial buildTypeBindings call
    ...
    private int computePriority(CategorizedProblem problem) {
        ...
        if (context != null) {
            ...
            if (// if context is null, firstErrors is null too
 ...
 */
    ...
    private void quickPrioritize(CategorizedProblem[] problemList, int left, int right) {
        ...
        do {
            while (computePriority(problemList[right]) < mid) right--;
            while (mid < computePriority(problemList[left])) left++;
            if (left <= right) {
