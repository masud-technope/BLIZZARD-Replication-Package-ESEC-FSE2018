...
public class ParameterizedSingleTypeReference extends ArrayTypeReference {
    ...
    private TypeBinding internalResolveType(Scope scope, ReferenceBinding enclosingType) {
        ...
        for (int i = 0; i < argLength; i++) {
            ...
            TypeBinding argType = isClassScope ? typeArgument.resolveTypeArgument((ClassScope) scope, currentType, i) : typeArgument.resolveTypeArgument((BlockScope) scope, currentType, i);
            if (argType == null) {
                argHasError = true;
            } else {
                argTypes[i] = argType;
        ...
        checkGeneric: {
            for (int i = 0; i < argLength; i++) if (typeVariables[i] != argTypes[i])
                break checkGeneric;
            return currentType;
        }
        ...
        !isClassScope)
            for (int i = 0; i < argLength; i++) if (!typeVariables[i].boundCheck(parameterizedType, argTypes[i]))
                scope.problemReporter().typeMismatchError(argTypes[i], typeVariables[i], currentType, this.typeArguments[i]);
