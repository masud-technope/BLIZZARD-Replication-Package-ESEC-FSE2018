...
public class FactoryPath implements IFactoryPath {
	 ...
	 * The factory path.  
	 */
    private final Map<FactoryContainer, Attributes> _path = Collections.synchronizedMap(new LinkedHashMap<FactoryContainer, Attributes>());
    ...
    public void enablePlugin(String pluginId) throws CoreException {
        ...
        if (a == null) {
            Status status = AptPlugin.createWarningStatus(new IllegalArgumentException(), //$NON-NLS-1$
            "Specified plugin was not found, so it could not be added to the annotation processor factory path: " + //$NON-NLS-1$
            pluginId);
    ...
    private void internalAdd(FactoryContainer fc, Attributes a) {
        synchronized (_path) {
            ...
            Map<FactoryContainer, Attributes> newPath = new LinkedHashMap(1 + 4 * (_path.size() + 1) / 3);
            newPath.put(fc, a);
