 ...
 */
    protected void enterAbstractMethod(MethodInfo methodInfo) {
        ...
        int[] sourceRange = { methodInfo.declarationStart, -1 };
        int[] nameRange = { methodInfo.nameSourceStart, methodInfo.nameSourceEnd };
        this.fNode = new DOMMethod(this.fDocument, sourceRange, CharOperation.charToString(methodInfo.name), nameRange, methodInfo.modifiers, methodInfo.isConstructor, CharOperation.charToString(methodInfo.returnType), CharOperation.charArrayToStringArray(methodInfo.parameterTypes), CharOperation.charArrayToStringArray(methodInfo.parameterNames), CharOperation.charArrayToStringArray(methodInfo.exceptionTypes));
        addChild(this.fNode);
 ...
 */
    public void enterField(FieldInfo fieldInfo) {
        ...
        this.fNode = new DOMField(this.fDocument, sourceRange, CharOperation.charToString(fieldInfo.name), nameRange, fieldInfo.modifiers, CharOperation.charToString(fieldInfo.type), isSecondary);
        addChild(this.fNode);
        this.fStack.push(this.fNode);
    }
 ...
 */
    public void enterType(TypeInfo typeInfo) {
        if (this.fBuildingType) {
            ...
            int[] sourceRange = { typeInfo.declarationStart, -1 };
            int[] nameRange = new int[] { typeInfo.nameSourceStart, typeInfo.nameSourceEnd };
            this.fNode = new DOMType(this.fDocument, sourceRange, new String(typeInfo.name), nameRange, typeInfo.modifiers, CharOperation.charArrayToStringArray(typeInfo.superinterfaces), // TODO (jerome) should pass in kind
            TypeDeclaration.kind(typeInfo.modifiers) == TypeDeclaration.CLASS_DECL);
            addChild(this.fNode);
            this.fStack.push(this.fNode);
        // type parameters not supported by JDOM
        }
 ...
 */
    protected SourceElementParser getParser(Map<String, String> settings) {
        return new SourceElementParser(this, new DefaultProblemFactory(), new CompilerOptions(settings), false, /*don't report local declarations*/
        true);
    }
}
