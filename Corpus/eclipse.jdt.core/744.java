 ...
 */
    ...
    public void test012() {
        ...
        this.runConformTest(new String[] { "p1/Updated.java", "package p1; \n" + "public class Updated extends Top { \n" + "	public static void main(String[] arguments) { \n" + "		Y.main(arguments);	\n" + "	}	\n" + "	static String bar = \"Updated.bar\";	\n" + "	String foo = \"Updated.foo\";	\n" + "	Store store = new Store(\"Updated.store\");	\n" + "	static Store sstore = new Store(\"Updated.sstore\");	\n" + "	static Updated ss = new Updated();	\n" + "} \n" }, "*** FIELD ACCESS ***" + "*1* new Updated().bar: Updated.bar" + "*2* new Updated().foo: Updated.foo" + "*3* new Y().foo: Updated.foo" + "*4* new Y().bar: Updated.bar" + "*5* bar: Updated.bar" + "*6* foo: Updated.foo" + "*7* Y.bar: Updated.bar" + "*8* this.bar: Updated.bar" + "*9* this.foo: Updated.foo" + "*10* store.value: Updated.store" + "*11* sstore.value: Updated.sstore" + "*12* ss.sstore.value: Top.sstore", // use default class-path
    ...
    public void test013() {
        ...
        runConformTest(/* do not flush output directory */
        false, new String[] /* test files */
        { "p1/Updated.java", "package p1; \n" + "public class Updated extends Top { \n" + "	public static void main(String[] arguments) { \n" + "		Y.main(arguments);	\n" + "	}	\n" + "	static String bar() { return \"Updated.bar()\"; }	\n" + "	String foo() { return \"Updated.foo()\"; }	\n" + "} \n" }, /* do not check compiler log */
        null, // runtime results
        "*** METHOD ACCESS ***" + /* expected output string */
        "*1* new Updated().bar(): Updated.bar()" + "*2* new Updated().foo(): Updated.foo()" + "*3* new Y().foo(): Updated.foo()" + "*4* new Y().bar(): Updated.bar()" + "*5* bar(): Updated.bar()" + "*6* foo(): Updated.foo()" + "*7* Y.bar(): Updated.bar()" + "*8* this.bar(): Updated.bar()" + "*9* this.foo(): Updated.foo()", /* expected error string */
        "", // javac options
        JavacTestOptions.JavacHasABug.JavacThrowsAnExceptionForJava_1_5_0_16);
    // transient error
    }
 ...
 */
    ...
    public void test021() {
        ...
        boolean check = computedReferences.indexOf("constructorRef/Z/0") >= 0 && computedReferences.indexOf("methodRef/init/0") >= 0;
        if (!check) {
 ...
 */
    public void test037() {
        ...
        { "p/X.java", "package p;	\n" + "public class X {\n" + "}\n" + "// TODO: something" }, /* no class libraries */
        null, customOptions, // compiler results
        "----------\n" + /* expected compiler log */
        "1. WARNING in p\\X.java (at line 4)\n" + "	// TODO: something\n" + "	   ^^^^^^^^^^^^^^^\n" + "TODO: something\n" + "----------\n", /* do not check output string */
 ...
 */
    public void test038() {
        ...
        runConformTest(/* flush output directory */
        true, new String[] /* test files */
        { "X.java", "// TODO: something" }, /* no class libraries */
        null, customOptions, // compiler results
        "----------\n" + /* expected compiler log */
        "1. WARNING in X.java (at line 1)\n" + "	// TODO: something\n" + "	   ^^^^^^^^^^^^^^^\n" + "TODO: something\n" + "----------\n", /* do not check output string */
        null, "java.lang.ClassNotFoundException", // javac options
        JavacTestOptions.Excuse.EclipseHasSomeMoreWarnings);
    }
 ...
 */
    ...
    public void test088() {
        ...
        if (isJRELevel(AbstractCompilerTest.F_1_6 | AbstractCompilerTest.F_1_7 | AbstractCompilerTest.F_1_8) || (AbstractCompilerTest.getPossibleComplianceLevels() == AbstractCompilerTest.F_1_5 && javaVersion.indexOf("1.5") == -1)) {
