...
public abstract class Annotation extends Expression {
    ...
    // Emit this into class file, unless this is a repeating annotation, in which case package this into the designated container.
            ...
            public boolean visit(TypeReference typeReference, BlockScope scope) {
                if (this.continueSearch) {
                    ...
                    if (this.continueSearch) {
                        ...
                        int size = this.typePathEntries.size();
                        for (// traverse outwards, see comment below about type annotations from SE7 locations.
                        int i = levels - 1; // traverse outwards, see comment below about type annotations from SE7 locations.
                        this.continueSearch && i >= 0; // traverse outwards, see comment below about type annotations from SE7 locations.
                        i--) {
                            this.typePathEntries.setSize(size);
                            for (int j = 0, depth = nestingDepths[i]; j < depth; j++) this.typePathEntries.add(TYPE_PATH_INNER_TYPE);
                            if (annotations != null)
                ...
                // if annotation is not found in the type reference, it must be one from SE7 location, typePathEntries captures the proper path entries for them. 
                return false;
            }
        ...
        switch(annotationType.id) {
            ...
            case TypeIds.T_JavaLangAnnotationRetention:
                if (valueAttribute != null) {
                    ...
                    if (// anything but Reference would be a type error anyway
                    (expr.bits & Binding.VARIABLE) == Binding.FIELD && expr instanceof Reference) {
    ...
    public static int nullLocationBitsFromAnnotationValue(Object value) {
        if (value instanceof Object[]) {
            if (((Object[]) value).length == 0) {
                ...
                // ({vals...})
                int bits = 0;
                for (Object single : (Object[]) value) bits |= evaluateDefaultNullnessLocation(single);
                return bits;
            }
        } else {
            // (val)
    ...
    public void checkRepeatableMetaAnnotation(BlockScope scope) {
        ...
        // know it to be an annotation type. On target miss we don't get here
    ...
    private static void checkContainerAnnotationTypeTarget(ASTNode culpritNode, Scope scope, ReferenceBinding containerType, ReferenceBinding repeatableAnnotationType) {
        ...
        if ((tagBits & TagBits.AnnotationTargetMASK) == 0)
            // absence of @Target meta-annotation implies all SE7 targets not all targets.
            tagBits = TagBits.SE7AnnotationTargetMASK;
        ...
        if ((containerAnnotationTypeTargets & ~targets) != 0) {
            class MissingTargetBuilder {
                ...
                void checkAnnotationType(char[] targetName) {
                    if ((containerAnnotationTypeTargets & TagBits.AnnotationForAnnotationType) != 0 && ((targets & (TagBits.AnnotationForAnnotationType | TagBits.AnnotationForType))) == 0) {
                        add(targetName);
                    }
                }
    ...
    public boolean isRuntimeInvisible() {
        ...
        if ((metaTagBits & (TagBits.AnnotationForTypeParameter | TagBits.AnnotationForTypeUse)) != 0) {
            if (// not a hybrid target. 
            (metaTagBits & TagBits.SE7AnnotationTargetMASK) == 0) {
                return false;
            }
        }
    ...
    public boolean isRuntimeTypeInvisible() {
        ...
        // could be forward reference
        long metaTagBits = annotationBinding.getAnnotationTagBits();
        if (// explicit target required for JSR308 style annotations.
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        for (int i = 0; i < pairsLength; i++) {
            if (pairs[i] != null) {
                ...
                // resilient
                pairs[i].resolveTypeExpecting(// resilient
                scope, // resilient
                null);
            }
        }
            ...
            if (tagBits != 0 || defaultNullness != 0) {
                ...
                switch(kind) {
                    ...
                    case Binding.GENERIC_TYPE:
                        ...
                        if (// don't set AnnotationRepeatable on non-annotation types.
                        (tagBits & TagBits.AnnotationRepeatable) == 0 || sourceType.isAnnotationType())
    ...
    static boolean isAnnotationTargetAllowed(Annotation annotation, BlockScope scope, TypeBinding annotationType, int kind) {
        ...
        if ((metaTagBits & TagBits.SE7AnnotationTargetMASK) == 0 && (metaTagBits & (TagBits.AnnotationForTypeUse | TagBits.AnnotationForTypeParameter)) != 0) {
            if (scope.compilerOptions().sourceLevel < ClassFileConstants.JDK1_8) {
                switch(kind) {
                    case Binding.PACKAGE:
                    case Binding.TYPE:
    ...
    public static void checkForInstancesOfRepeatableWithRepeatingContainerAnnotation(BlockScope scope, ReferenceBinding repeatedAnnotationType, Annotation[] sourceAnnotations) {
        ...
        for (int i = 0; i < sourceAnnotations.length; ++i) {
            ...
            if (TypeBinding.equalsEquals(elementsType, annotation.resolvedType)) {
                ...
                // One is enough for this annotation type
                return;
            }
        }
    }
    ...
    public static void isTypeUseCompatible(TypeReference reference, Scope scope, Annotation[] annotations) {
        ...
        if (resolvedType == null || !resolvedType.isNestedType())
            return;
        nextAnnotation: for (int i = 0, annotationsLength = annotations.length; i < annotationsLength; i++) {
    ...
    public Annotation getPersistibleAnnotation() {
        // will be this for non-repeating annotation, the container for the first of the repeating ones and null for the followers.
        return this.persistibleAnnotation;
    }

    public void setPersistibleAnnotation(ContainerAnnotation container) {
        // will be a legitimate container for the first of the repeating ones and null for the followers.
