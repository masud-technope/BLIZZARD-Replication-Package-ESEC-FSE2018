...
public final class LocalTypeBinding extends NestedTypeBinding {
    ...
    // used to cache array bindings of various dimensions for this local type
...
*/
    public void addInnerEmulationDependent(BlockScope dependentScope, boolean wasEnclosingInstanceSupplied) {
        ...
        if (dependents == null) {
            ...
            for (int i = 0; i < index; i++) if (dependents[i].scope == dependentScope)
                // already stored
                return;
...
*/
    ...
    ArrayBinding createArrayType(int dimensionCount) {
        ...
        for (int i = 0; i < length; i++) if (localArrayBindings[i].dimensions == dimensionCount)
