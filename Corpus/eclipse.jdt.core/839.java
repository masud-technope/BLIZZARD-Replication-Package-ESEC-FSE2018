...
public class IndexBasedHierarchyBuilder extends HierarchyBuilder implements SuffixConstants {
    ...
    // heuristic so that there still progress for deep hierachies
    ...
    public void build(boolean computeSubtypes) {
        ...
        try {
            ...
            if (computeSubtypes) {
                ...
                // contains the paths that have potential subtypes that are local/anonymous types
 ...
 */
    private String[] determinePossibleSubTypes(final HashSet localTypes, IProgressMonitor monitor) {
        ...
        searchAllPossibleSubTypes(getType(), this.scope, this.binariesFromIndexMatches, collector, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, monitor);
 ...
 * @param monitor
 */
    public static void searchAllPossibleSubTypes(IType type, IJavaSearchScope scope, final Map binariesFromIndexMatches, final IPathRequestor pathRequestor, // WaitUntilReadyToSearch | ForceImmediateSearch | CancelIfNotReadyToSearch
    int waitingPolicy, final IProgressMonitor monitor) {
        ...
        IndexQueryRequestor searchRequestor = new IndexQueryRequestor() {
            ...
            public boolean acceptIndexMatch(String documentPath, SearchPattern indexRecord, SearchParticipant participant, AccessRuleSet access) {
                ...
                if (// local or anonymous types cannot have subtypes outside the cu that define them
                !isLocalOrAnonymous && !foundSuperNames.containsKey(typeName)) {
        ...
        SuperTypeReferencePattern pattern = new SuperTypeReferencePattern(null, null, superRefKind, SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE);
        MatchLocator.setFocus(pattern, type);
        SubTypeSearchJob job = new SubTypeSearchJob(pattern, // java search only
        new JavaSearchParticipant(), scope, searchRequestor);
