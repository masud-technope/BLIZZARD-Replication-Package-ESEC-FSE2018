...
public class JavaModelCache {
    ...
    public static final int DEFAULT_CHILDREN_SIZE = 250 * 20;
    ...
    public  JavaModelCache() {
        ...
        if (VERBOSE) {
            //$NON-NLS-1$
            this.rootCache = new VerboseElementCache((int) (DEFAULT_ROOT_SIZE * ratio), "Root cache");
            //$NON-NLS-1$
            this.pkgCache = new VerboseElementCache((int) (DEFAULT_PKG_SIZE * ratio), "Package cache");
            //$NON-NLS-1$
            this.openableCache = new VerboseElementCache((int) (DEFAULT_OPENABLE_SIZE * ratio * openableRatio), "Openable cache");
        } else {
    ...
    public IJavaElement getExistingElement(IJavaElement element) {
        switch(element.getElementType()) {
            case IJavaElement.JAVA_MODEL:
                return element;
            case IJavaElement.JAVA_PROJECT:
                // projectCache is a Hashtable and Hashtables don't support getKey(...)
                return element;
            case IJavaElement.PACKAGE_FRAGMENT_ROOT:
            ...
            case IJavaElement.TYPE:
                // jarTypeCache or childrenCache are Hashtables and Hashtables don't support getKey(...)
                return element;
            default:
                // childrenCache is a Hashtable and Hashtables don't support getKey(...)
                return element;
        }
    }
