...
public class JavaElementDeltaTests extends ModifyingResourceTests {
    ...
    public static Test suite() {
        return buildModelTestSuite(JavaElementDeltaTests.class, ALPHABETICAL_SORT);
    }
        ...
        try {
            ...
            ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
                ...
                public void run(IProgressMonitor monitor) throws CoreException {
                    addJavaNature("P");
                    createFolder("/P/src");
                    getJavaProject("P").setRawClasspath(new IClasspathEntry[] { JavaCore.newSourceEntry(new Path("/P/src")) }, new Path("/P/bin"), null);
                }
            }, null);
            startDeltas();
 ...
 */
    public void testChangeCustomOutput() throws CoreException {
        try {
            ...
            startDeltas();
            setClasspath(proj, new IClasspathEntry[] { JavaCore.newSourceEntry(new Path("/P/src"), new IPath[0], new Path("/P/bin2")) });
 ...
 */
    public void testChangeExportFlag() throws CoreException {
        try {
            ...
            startDeltas();
            setClasspath(proj, new IClasspathEntry[] { JavaCore.newProjectEntry(new Path("/P1"), true) });
 ...
 */
    public void testChangeRootKind() throws CoreException {
        try {
            ...
            startDeltas();
            setClasspath(proj, new IClasspathEntry[] { JavaCore.newLibraryEntry(new Path("/P/src"), null, null, false) });
            assertDeltas("Unexpected delta", "P[*]: {CHILDREN | CONTENT | RAW CLASSPATH CHANGED | RESOLVED CLASSPATH CHANGED}\n" + "	src[*]: {ADDED TO CLASSPATH | REMOVED FROM CLASSPATH}\n" + "	ResourceDelta(/P/.classpath)[*]");
        } finally {
            stopDeltas();
            deleteProject("P");
 ...
 */
    public void testCompilationUnitRemoveAndAdd() throws CoreException {
        try {
            createJavaProject("P");
            createFolder("/P/p");
            IFile file = createFile("/P/p/X.java", "package p;\n" + "public class X {\n" + "}");
            // delete cu
            startDeltas();
            deleteResource(file);
 ...
 */
    public void testCopyAndOverwritePackage() throws CoreException {
        try {
            ...
            startDeltas();
            getPackage("/P/src1/p").copy(getPackageFragmentRoot("/P/src2"), /*no sibling*/
            null, /*no rename*/
            null, /*replace*/
            true, /*no progress*/
            null);
 ...
 */
    public void testCUNotOnClasspath() throws CoreException {
        try {
            ...
            IFile file = createFile("/P/src/p/X.java", "package p;\n" + "public class X {\n" + "}");
            startDeltas();
            deleteResource(file);
 ...
 */
    public void testDeleteProjectAfterChangingClasspath() throws CoreException {
        try {
            ...
            startDeltas();
            getWorkspace().run(runnable, /*no progress*/
            null);
            assertDeltas("Unexpected delta", "P[-]: {}");
        } finally {
            stopDeltas();
 ...
 */
    public void testDeleteProjectSetCPAnotherProject() throws CoreException {
        ...
        try {
            ...
            JavaCore.run(new IWorkspaceRunnable() {
                ...
                public void run(IProgressMonitor monitor) throws CoreException {
                    deleteProject("P2");
                    project.setRawClasspath(new IClasspathEntry[] { JavaCore.newSourceEntry(project.getPath()) }, null);
                }
            }, null);
    ...
    public void testDiscardWorkingCopy1() throws CoreException {
        // renamed from testDestroyWorkingCopy
        ICompilationUnit copy = null;
        try {
            createJavaProject("P", new String[] { "" }, "");
            createFile("P/X.java", "public class X {\n" + "}");
    ...
    public void testDiscardWorkingCopy2() throws CoreException {
        // renamed from testDestroySharedWorkingCopy
        ICompilationUnit copy = null;
        try {
            createJavaProject("P", new String[] { "" }, "");
            createFile("P/X.java", "public class X {\n" + "}");
    ...
    public void testDiscardWorkingCopy3() throws CoreException {
        ...
        try {
            ...
            JavaCore.run(new IWorkspaceRunnable() {
                ...
                public void run(IProgressMonitor monitor) throws CoreException {
                    getPackage("/P").createCompilationUnit("Y.java", "public class Y {}", /*don't force*/
                    false, monitor);
 ...
 */
    public void testListenerPostChange() throws CoreException {
        ...
        try {
            ...
            ICompilationUnit cu = pkg.createCompilationUnit("X.java", "public class X {\n" + "}", false, null);
            assertEquals("Unexpected delta after creating CU", "P[*]: {CHILDREN}\n" + "	<project root>[*]: {CHILDREN}\n" + "		<default>[*]: {CHILDREN}\n" + "			X.java[+]: {}", listener.toString());
            listener.flush();
            // type creation
 ...
 */
    public void testListenerReconcile() throws CoreException {
        ...
        try {
            ...
            IPackageFragment pkg = getPackage("P");
            ICompilationUnit cu = pkg.createCompilationUnit("X.java", "public class X {\n" + "}", false, null);
            assertEquals("Unexpected delta after creating CU", "", listener.toString());
            listener.flush();
            // type creation
            cu.createType("class A {\n" + "}", cu.getType("X"), false, null);
            assertEquals("Unexpected delta after creating type", "", listener.toString());
            listener.flush();
            // non-java resource creation
            createFile("P/readme.txt", "");
            assertEquals("Unexpected delta after creating non-java resource", "", listener.toString());
            listener.flush();
            // shared working copy creation
            wc = cu.getWorkingCopy(new WorkingCopyOwner() {
            }, null);
            assertEquals("Unexpected delta after creating shared working copy", "", listener.toString());
            listener.flush();
            // reconcile
            wc.getBuffer().setContents("public class X {\n" + "  public void foo() {\n" + "  }\n" + "}");
            wc.reconcile(ICompilationUnit.NO_AST, false, null, null);
            ...
            // commit
            wc.commitWorkingCopy(false, null);
            assertEquals("Unexpected delta after committing working copy", "", listener.toString());
            listener.flush();
            // shared working copy destruction
            wc.discardWorkingCopy();
            assertEquals("Unexpected delta after destroying shared working copy", "", listener.toString());
            listener.flush();
            wc = null;
        } finally {
            if (wc != null)
        ...
        try {
            ...
            ResourcesPlugin.getWorkspace().run(new IWorkspaceRunnable() {
                ...
                public void run(IProgressMonitor monitor) throws CoreException {
                    ...
                    pkg.createCompilationUnit("X.java", "public class X {\n" + "}", true, null);
 ...
 */
    public void testOverwriteClasspath() throws CoreException {
        try {
            ...
            createFolder("P/src");
            createFolder("P/bin");
            final IFile newCP = createFile("P/.classpath2", "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + "<classpath>\n" + "    <classpathentry kind=\"src\" path=\"src\"/>\n" + "    <classpathentry kind=\"output\" path=\"bin\"/>\n" + "</classpath>");
            startDeltas();
 ...
 */
    public void testRemoveAddBinaryProject() throws CoreException {
        try {
            ...
            project.setRawClasspath(new IClasspathEntry[] { JavaCore.newLibraryEntry(new Path("/P/lib.jar"), null, null) }, null);
            startDeltas();
            getWorkspace().run(new IWorkspaceRunnable() {
 ...
 */
    public void testRenameJavaProject() throws CoreException {
        try {
            ...
            startDeltas();
            renameProject("P", "P1");
            assertDeltas("Unexpected delta", "P1[+]: {MOVED_FROM(P)}\n" + "P[-]: {MOVED_TO(P1)}");
        } finally {
            stopDeltas();
            deleteProject("P");
            deleteProject("P1");
 ...
 */
    public void testSetClasspathOnFreshProject() throws CoreException {
        try {
            ...
            startDeltas();
            IClasspathEntry[] classpath = new IClasspathEntry[] { JavaCore.newSourceEntry(new Path("/P1/src2")), JavaCore.newLibraryEntry(new Path("/LibProj/mylib.jar"), null, null) };
