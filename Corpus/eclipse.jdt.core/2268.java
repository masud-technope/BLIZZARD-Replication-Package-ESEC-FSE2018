        ...
        checkUnused: {
            ...
            if (constructorBinding.isPrivate()) {
                if ((this.binding.declaringClass.tagBits & TagBits.HasNonPrivateConstructor) == 0)
                    // tolerate as known pattern to block instantiation
                    break checkUnused;
            ...
            if (this.constructorCall.accessMode != ExplicitConstructorCall.This) {
                ...
                if (constructorBinding.isPublic() && constructorBinding.parameters.length == 0 && declaringClass.isStatic() && declaringClass.findSuperTypeOriginatingFrom(TypeIds.T_JavaIoExternalizable, false) != null)
                    break checkUnused;
            ...
            if ((this.constructorCall != null) && (this.constructorCall.accessMode != ExplicitConstructorCall.This)) {
                ...
                for (int i = 0, count = fields.length; i < count; i++) {
                    ...
                    if (!field.isStatic() && !flowInfo.isDefinitelyAssigned(field)) {
                        if (field.isFinal()) {
                            this.scope.problemReporter().uninitializedBlankFinalField(field, ((this.bits & ASTNode.IsDefaultConstructor) != 0) ? (ASTNode) this.scope.referenceType().declarationOf(field.original()) : this);
                            ...
                            if (!isValueProvidedUsingAnnotation(fieldDecl))
                                this.scope.problemReporter().uninitializedNonNullField(field, ((this.bits & ASTNode.IsDefaultConstructor) != 0) ? (ASTNode) fieldDecl : this);
                        }
                    }
                }
            }
        ...
        if (syntheticArgs != null) {
            for (int i = 0, max = syntheticArgs.length; i < max; i++) {
                ...
                if ((syntheticArg = syntheticArgs[i]).matchingField != null) {
                    ...
                    codeStream.fieldAccess(Opcodes.OPC_putfield, syntheticArg.matchingField, /* default declaringClass */
                    null);
                }
            }
        }
 ...
 */
    public boolean isRecursive(ArrayList visited) {
        ...
        // https://bugs.eclipse.org/bugs/show_bug.cgi?id=358762
