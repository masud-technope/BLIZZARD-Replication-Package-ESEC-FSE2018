...
public class FlowContext implements TypeConstants {
    ...
    public static final int ASSIGN_TO_NONNULL = 0x0080;
 ...
 */
    ...
    public void extendTimeToLiveForNullCheckedField(int t) {
        if (this.timesToLiveForNullCheckInfo != null) {
            for (int i = 0; i < this.timesToLiveForNullCheckInfo.length; i++) if (this.timesToLiveForNullCheckInfo[i] > 0)
 ...
 */
    public boolean isNullcheckedFieldAccess(Reference reference) {
        if (// always null unless CompilerOptions.enableSyntacticNullAnalysisForFields
 ...
 */
    public void checkExceptionHandlers(TypeBinding raisedException, ASTNode location, FlowInfo flowInfo, BlockScope scope, boolean isExceptionOnAutoClose) {
        ...
        if (scope.compilerOptions().sourceLevel >= ClassFileConstants.JDK1_7 && location instanceof ThrowStatement) {
            ...
            if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=350361 
            throwExpression instanceof SingleNameReference && throwArgBinding instanceof CatchParameterBinding && throwArgBinding.isEffectivelyFinal()) {
            ...
            if (traversedContext instanceof ExceptionHandlingFlowContext) {
                ...
                if ((caughtExceptions = exceptionContext.handledExceptions) != Binding.NO_EXCEPTIONS) {
                    ...
                    for (int caughtIndex = 0, caughtCount = caughtExceptions.length; caughtIndex < caughtCount; caughtIndex++) {
                        ...
                        if (abruptlyExitedLoops != null && caughtException != null && state != Scope.NOT_RELATED) {
                            ...
                            // don't use flow info on first round, flow info will be evaluated during loopback simulation
                            exceptionFlow = FlowInfo.DEAD_END;
                        }
                        switch(state) {
                            case Scope.EQUAL_OR_MORE_SPECIFIC:
                                exceptionContext.recordHandlingException(caughtException, exceptionFlow.unconditionalInits(), raisedException, // precise exception that will be caught
                                raisedException, location, definitelyCaught);
                ...
                if (exceptionContext.isMethodContext) {
                    ...
                    if (shouldMergeUnhandledExceptions) {
                        ...
                        // no need to complain, will fix up constructor/lambda exceptions
                        return;
                    }
            ...
            if (!isExceptionOnAutoClose) {
                if (traversedContext instanceof InsideSubRoutineFlowContext) {
                    ...
                    if (node instanceof TryStatement) {
                        ...
                        flowInfo.addInitializationsFrom(// collect inits
        ...
        while (traversedContext != null) {
            ...
            if (traversedContext instanceof ExceptionHandlingFlowContext) {
                ...
                if ((caughtExceptions = exceptionContext.handledExceptions) != Binding.NO_EXCEPTIONS) {
                    ...
                    boolean[] locallyCaught = new // at most
                    boolean[// at most
                    raisedCount];
                    for (int caughtIndex = 0; caughtIndex < caughtCount; caughtIndex++) {
                        ...
                        for (int raisedIndex = 0; raisedIndex < raisedCount; raisedIndex++) {
                            ...
                            if ((raisedException = raisedExceptions[raisedIndex]) != null) {
                                FlowInfo exceptionFlow = flowInfo;
                                int state = caughtException == null ? Scope.EQUAL_OR_MORE_SPECIFIC : Scope.compareTypes(raisedException, caughtException);
                                if (abruptlyExitedLoops != null && caughtException != null && state != Scope.NOT_RELATED) {
                                    for (int i = 0, abruptlyExitedLoopsCount = abruptlyExitedLoops.size(); i < abruptlyExitedLoopsCount; i++) {
                                        LoopingFlowContext loop = (LoopingFlowContext) abruptlyExitedLoops.get(i);
                                        loop.recordCatchContextOfEscapingException(exceptionContext, caughtException, flowInfo);
                                ...
                                switch(state) {
                                    case Scope.EQUAL_OR_MORE_SPECIFIC:
                                        exceptionContext.recordHandlingException(caughtException, exceptionFlow.unconditionalInits(), raisedException, // precise exception that will be caught
                                        raisedException, location, locallyCaught[raisedIndex]);
        ...
        do {
            if (current instanceof InitializationFlowContext) {
                ...
                if (current instanceof FieldInitsFakingFlowContext) {
                    // isDefinitelyAssigned will return true for all fields
                ...
                if (flowInfo.isDefinitelyNull(local)) {
                    switch(checkTypeWithoutHideNullWarning & CONTEXT_MASK) {
                        case FlowContext.IN_COMPARISON_NULL:
                            if (// check for auto-unboxing first and report appropriate warning
                            ((checkTypeWithoutHideNullWarning & CHECK_MASK) == CAN_ONLY_NULL) && (reference.implicitConversion & TypeIds.UNBOXING) != 0) {
 ...
 */
    public void recordNullityMismatch(BlockScope currentScope, Expression expression, TypeBinding providedType, TypeBinding expectedType, FlowInfo flowInfo, int nullStatus, NullAnnotationMatching annotationStatus) {
        if (providedType == null) {
            // assume type error was already reported
            return;
        }
        if (// flowContext cannot yet handle non-localvar expressions (e.g., fields)
        expression.localVariableBinding() != null) {
