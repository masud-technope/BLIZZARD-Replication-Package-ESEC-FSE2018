    ...
    public void process() {
        ...
        try {
            ...
            for (TypeDeclaration decl : decls) {
                if (//$NON-NLS-1$
                decl.toString().endsWith("DeclarationTestClass")) {
                    testClass = decl;
                }
                if (//$NON-NLS-1$
                decl.toString().endsWith("ClassDec")) {
    ...
    private void testAnnotationImplementations(TypeDeclaration testClass) {
        ...
        for (AnnotationTypeElementDeclaration ated : elementDeclarations) {
            if (ated.toString().startsWith("S")) {
                ...
                // the anno is declared in binary - no AST.
                ProcessorTestStatus.assertTrue("position should be null", posAted == null);
            }
        ...
        // the anno is declared in binary - no AST.
        ProcessorTestStatus.assertTrue("position should be null", posVS == null);
