...
public class BasicBuildTests extends BuilderTests {
    ...
    public void testBug392727() throws JavaModelException {
        ...
        try {
            ...
            env.addClass(root, "pack", "Zork", // pull in Main first
    ...
    public void testbBug386901() throws JavaModelException {
        ...
        try {
            ...
            "AB", "package p;	\n" + "public class AB {}	\n" + //$NON-NLS-1$
            "class AZ {}");
            IPath pathToAA = //$NON-NLS-1$ //$NON-NLS-2$
            env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
            root, //$NON-NLS-1$ //$NON-NLS-2$
            "p", //$NON-NLS-1$ //$NON-NLS-2$
            "AA", "package p;	\n" + //$NON-NLS-1$
            "public class AA extends AZ {}");
            // units compiled in batches of '1' unit
            org.eclipse.jdt.internal.core.builder.AbstractImageBuilder.MAX_AT_ONCE = 1;
            fullBuild(projectPath);
            expectingProblemsFor(pathToAA, "Problem : AZ cannot be resolved to a type [ resource : </Project/src/p/AA.java> range : <36,38> category : <40> severity : <2>]");
            // All units compiled at once
    ...
    public void testBug425420() throws JavaModelException {
        ...
        //$NON-NLS-1$
        env.setOutputFolder(projectPath, "bin");
        IPath path = //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "X", "class X {\n" + "  void test() {\n" + "	int x = 0;\n" + "	int y = 0 >= 0 ? x : \"\"; \n" + "  }\n" + "}");
        fullBuild(projectPath);
        expectingProblemsFor(path, "Problem : The type java.lang.Object cannot be resolved. It is indirectly referenced from required .class files [ resource : </Project/src/X.java> range : <0,1> category : <10> severity : <2>]");
    }
}
