...
public class SourceIndexer extends AbstractIndexer implements ITypeRequestor, SuffixConstants {
    ...
    public void resolveDocument() {
        try {
            ...
            INameEnvironment nameEnvironment = new JavaSearchNameEnvironment(javaProject, JavaModelManager.getJavaModelManager().getWorkingCopies(DefaultWorkingCopyOwner.PRIMARY, /*add primary WCs*/
            true));
    ...
    private void reduceParseTree(CompilationUnitDeclaration unit) {
        ...
        for (int i = 0, l = types == null ? 0 : types.length; i < l; i++) purgeMethodStatements(types[i]);
    }
    ...
    private void purgeMethodStatements(TypeDeclaration type) {
        ...
        if (memberTypes != null)
            for (int i = 0, l = memberTypes.length; i < l; i++) purgeMethodStatements(memberTypes[i]);
    }
            ...
            for (int i = 0, length = this.cud.functionalExpressionsCount; i < length; i++) {
                ...
                if (expression instanceof LambdaExpression) {
                    ...
                    if (lambdaExpression.binding != null && lambdaExpression.binding.isValidBinding()) {
                        ...
                        if (DEBUG) {
                            System.out.println('\t' + new String(superinterface) + '.' + new String(//$NON-NLS-1$
                            lambdaExpression.descriptor.selector) + "-> {}");
                        }
                        ...
                        addClassDeclaration(// most entries are blank, that is fine, since lambda type/method cannot be searched.
                        0, // package name
                        CharOperation.NO_CHAR, ONE_ZERO, // enclosing types.
                        ONE_ZERO_CHAR, // super class
                        CharOperation.NO_CHAR, new char[][] { superinterface }, CharOperation.NO_CHAR_CHAR, // not primary.
                        true);
                    } else {
                        if (DEBUG)
                            System.out.println("\tnull/bad binding in lambda");
                    }
                } else {
                    ...
                    if (binding != null && binding.isValidBinding()) {
                        if (DEBUG) {
                            System.out.println('\t' + new String(//$NON-NLS-1$
                            referenceExpression.resolvedType.sourceName()) + "::" + new String(referenceExpression.descriptor.selector) + " == " + new String(//$NON-NLS-1$
                            binding.declaringClass.sourceName()) + '.' + new String(binding.selector));
                        }
                        ...
                        if (DEBUG)
                            System.out.println("\tnull/bad binding in reference expression");
                    }
                }
            }
        } catch (Exception e) {
