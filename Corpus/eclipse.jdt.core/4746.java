...
public abstract class TypeConverter {
    ...
    private TypeReference decodeType(String typeSignature, int length, int start, int end) {
            ...
            switch(currentChar) {
                ...
                case Signature.C_DOT:
                    if (!nameStarted) {
                        nameFragmentStart = this.namePos + 1;
                        nameStarted = true;
                    } else if (// handle name starting with a $ (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=91709)
                    this.namePos > nameFragmentStart)
                        identCount++;
                    break;
                case Signature.C_GENERIC_START:
                    ...
                    TypeReference[] arguments = decodeTypeArguments(typeSignature, length, start, end);
        ...
        if (// non parameterized
        fragments == null) {
            /* rebuild identifiers and dimensions */
            if (// simple type reference
            identCount == 1) {
                if (dim == 0) {
    ...
    private TypeReference decodeType2(char[] typeName, int length, int start, int end, boolean includeGenericsAnyway) {
        ...
        typeLoop: while (this.namePos < length) {
            ...
            switch(currentChar) {
                ...
                case '<':
                    ...
                    this.namePos++;
                    // positionned on '>' at end
                    TypeReference[] arguments = decodeTypeArguments(typeName, length, start, end, includeGenericsAnyway);
                    if (this.has1_5Compliance || includeGenericsAnyway) {
                        fragments.add(arguments);
                        identCount = 0;
                        nameFragmentStart = -1;
        ...
        return decodeType3(typeName, length, start, end, identCount, dim, nameFragmentStart, nameFragmentEnd, fragments);
    }
