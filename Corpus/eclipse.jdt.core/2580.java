...
public class SelectionParser extends AssistParser {
    ...
    protected static final int K_BETWEEN_CASE_AND_COLON = SELECTION_PARSER + 1;

    // whether we are between the keyword 'return' and the end of a return statement
    protected static final int K_INSIDE_RETURN_STATEMENT = SELECTION_PARSER + 2;

    // whether we are between ')' and the end of a cast statement
    protected static final int K_CAST_STATEMENT = SELECTION_PARSER + 3;
    ...
    protected void attachOrphanCompletionNode() {
        if (this.isOrphanCompletionNode) {
            ...
            if (orphan instanceof Expression) {
                ...
                if (// lambdas are recovered up to the containing expression statement and will carry along the assist node anyways.
                lastIndexOfElement(K_LAMBDA_EXPRESSION_DELIMITER) < 0) {
    ...
    protected void classInstanceCreation(boolean hasClassBody) {
        if ((this.astLengthStack[this.astLengthPtr] == 1) && (this.astStack[this.astPtr] == null)) {
            ...
            if ((index = this.indexOfAssistIdentifier()) < 0) {
                ...
                return;
            } else if (this.identifierLengthPtr > -1 && (this.identifierLengthStack[this.identifierLengthPtr] - 1) != index) {
                super.classInstanceCreation(hasClassBody);
                return;
            }
    ...
    protected void consumeCatchFormalParameter() {
        if (this.indexOfAssistIdentifier() < 0) {
            ...
            this.intPtr--;
            Argument arg = new SelectionOnArgumentName(identifierName, namePositions, type, this.intStack[this.intPtr + 1] & ~// modifiers
    ...
    protected void consumeFieldAccess(boolean isSuperAccess) {
        ...
        FieldReference fieldReference = new SelectionOnFieldReference(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr--]);
        this.identifierLengthPtr--;
        if (//considerates the fieldReferenceerence beginning at the 'super' ....
        isSuperAccess) {
        ...
        {
            if (//fieldReferenceerence begins at the this
            (fieldReference.receiver = this.expressionStack[this.expressionPtr]).isThis()) {
            ...
            if (requireExtendedRecovery()) {
                ...
                if (this.astPtr >= 0 && this.astStack[this.astPtr] == this.assistNode && this.assistNode instanceof ThisReference) {
                    ...
                    if (messageSend.receiver instanceof SingleNameReference) {
                        ...
                        if (// dummy reference created by getUnspecifiedReference ???
                        snr.token == CharOperation.NO_CHAR) {
    ...
    public TypeReference createParameterizedQualifiedAssistTypeReference(char[][] tokens, TypeReference[][] typeArguments, char[] assistname, TypeReference[] assistTypeArguments, long[] positions) {
    ...
    protected NameReference getUnspecifiedReference(boolean rejectTypeAnnotations) {
        ...
        if (CharOperation.equals(assistIdentifier(), SUPER)) {
            ...
            completionIndex > 0) {
                ...
                reference = new SelectionOnQualifiedSuperReference(getTypeReference(0), (int) (this.identifierPositionStack[ptr + 1] >>> 32), (int) this.identifierPositionStack[ptr + 1]);
            } else // standard super
            {
                this.identifierPtr -= length;
                this.identifierLengthPtr--;
 ...
 */
    ...
    public CompilationUnitDeclaration parse(ICompilationUnit sourceUnit, CompilationResult compilationResult, int start, int end) {
        ...
        selectionScanner.selectionStart = start;
        selectionScanner.selectionEnd = end;
        return super.parse(sourceUnit, compilationResult, -1, /*parse without reseting the scanner*/
        -1);
    }
 ...
 */
	...
	*/
    ...
    protected Argument typeElidedArgument() {
        ...
        char[] identifierName = this.identifierStack[this.identifierPtr];
        long namePositions = this.identifierPositionStack[this.identifierPtr--];
        Argument argument = new SelectionOnArgumentName(identifierName, namePositions, // elided type
    ...
    public String toString() {
        ...
        //$NON-NLS-1$
        s = s + "elementKindStack : int[] = {";
        for (int i = 0; i <= this.elementPtr; i++) {
            //$NON-NLS-1$
            s = s + String.valueOf(this.elementKindStack[i]) + ",";
        }
        //$NON-NLS-1$
        s = s + "}\n";
        //$NON-NLS-1$
        s = s + "elementInfoStack : int[] = {";
        for (int i = 0; i <= this.elementPtr; i++) {
            //$NON-NLS-1$
            s = s + String.valueOf(this.elementInfoStack[i]) + ",";
        }
        //$NON-NLS-1$
        s = s + "}\n";
