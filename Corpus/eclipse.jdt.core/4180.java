...
public final class CompoundNameVector {
    ...
    public void addAll(char[][][] newElements) {
        if (size + newElements.length >= maxSize) {
            ...
            maxSize = size + newElements.length;
    ...
    public boolean contains(char[][] element) {
        for (int i = size; --i >= 0; ) if (CharOperation.equals(element, elements[i]))
            return true;
        return false;
    }
    ...
    public char[][] remove(char[][] element) {
        ...
        for (int i = size; --i >= 0; ) if (element == elements[i]) {
    ...
    public void removeAll() {
        for (int i = size; --i >= 0; ) elements[i] = null;
        size = 0;
    }
