...
public class TypeVariableBinding extends ReferenceBinding {
    ...
    // binding of declaring type or method
        ...
        if (code == BoundCheckStatus.MISMATCH) {
            if (argumentType instanceof TypeVariableBinding && scope != null) {
                ...
                if (bound instanceof ParameterizedTypeBinding) {
                    // no capture position needed as this capture will never escape this context
            ...
            switch(wildcard.boundKind) {
                case Wildcard.EXTENDS:
                    ...
                    if (TypeBinding.equalsEquals(wildcardBound, this))
                        // OK per JLS, but may require null checking below
                        checkedAsOK = true;
    ...
    public int boundsCount() {
        ...
        if (this.firstBound.isInterface())
            // only interface bounds
            return this.superInterfaces.length;
        // class or array type isn't contained in superInterfaces
    ...
    public void collectSubstitutes(Scope scope, TypeBinding actualType, InferenceContext inferenceContext, int constraint) {
        ...
        switch(actualType.kind()) {
            case Binding.BASE_TYPE:
                ...
                break;
            // cannot steer inference, only learn from it.
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        if (// see https://bugs.eclipse.org/bugs/show_bug.cgi?id=97902
        !isLeaf && declaring.kind() == Binding.METHOD) {
            MethodBinding methodBinding = (MethodBinding) declaring;
            ReferenceBinding declaringClass = methodBinding.declaringClass;
            buffer.append(declaringClass.computeUniqueKey(/*not a leaf*/
            false));
            buffer.append(':');
            ...
            buffer.append(declaring.computeUniqueKey(/*not a leaf*/
            false));
            buffer.append(':');
        }
        ...
        if (!this.inRecursiveFunction) {
            ...
            try {
                if (this.superclass != null && TypeBinding.equalsEquals(this.firstBound, this.superclass)) {
                    buffer.append(" extends ").append(//$NON-NLS-1$
                    this.superclass.annotatedDebugName());
                }
    ...
    TypeBound[] getTypeBounds(InferenceVariable variable, InferenceSubstitution theta) {
        ...
        for (int i = 0; i < this.superInterfaces.length; i++) bounds[idx++] = TypeBound.createBoundOrDependency(theta, this.superInterfaces[i], variable);
        return bounds;
    }
    ...
    boolean hasOnlyRawBounds() {
        ...
        if (this.superInterfaces != null)
            for (int i = 0, l = this.superInterfaces.length; i < l; i++) if (!this.superInterfaces[i].isRawType())
                return false;
        return true;
    }
    ...
    public boolean isInterchangeableWith(TypeVariableBinding otherVariable, Substitution substitute) {
        ...
        next: for (int i = 0; i < length; i++) {
            ...
            for (int j = 0; j < length; j++) if (TypeBinding.equalsEquals(superType, this.superInterfaces[j]))
                continue next;
            // not a match
            return false;
        }
    ...
    public boolean isSubtypeOf(TypeBinding other) {
        ...
        if (this.superclass != null && this.superclass.isSubtypeOf(other))
            return true;
        if (this.superInterfaces != null)
            for (int i = 0, l = this.superInterfaces.length; i < l; i++) if (this.superInterfaces[i].isSubtypeOf(other))
                return true;
    ...
    public boolean isProperType(boolean admitCapture18) {
        ...
        if (// be optimistic, since this node is not an inference variable
        this.inRecursiveFunction)
            return true;
        this.inRecursiveFunction = true;
        try {
    ...
    ReferenceBinding resolve() {
        ...
        if (this.superclass != null) {
            ReferenceBinding resolveType = (ReferenceBinding) BinaryTypeBinding.resolveType(this.superclass, this.environment, /* raw conversion */
            true);
        ...
        if ((length = interfaces.length) != 0) {
            ...
            for (int i = length; --i >= 0; ) {
                ReferenceBinding resolveType = (ReferenceBinding) BinaryTypeBinding.resolveType(interfaces[i], this.environment, /* raw conversion */
                true);
                this.tagBits |= resolveType.tagBits & TagBits.ContainsNestedTypeReferences;
                long superNullTagBits = resolveType.tagBits & TagBits.AnnotationNullMASK;
                if (superNullTagBits != 0L) {
    ...
    public void setTypeAnnotations(AnnotationBinding[] annotations, boolean evalNullAnnotations) {
        if (getClass() == TypeVariableBinding.class) {
            ...
            // exposes original TVB/capture to type system for id stamping purposes.
    ...
    public char[] nullAnnotatedReadableName(CompilerOptions options, boolean shortNames) {
        ...
        if (!this.inRecursiveFunction) {
            ...
            try {
                if (this.superclass != null && TypeBinding.equalsEquals(this.firstBound, this.superclass)) {
                    nameBuffer.append(" extends ").append(//$NON-NLS-1$
                    this.superclass.nullAnnotatedReadableName(//$NON-NLS-1$
                    options, //$NON-NLS-1$
                    shortNames));
                }
                if (this.superInterfaces != null && this.superInterfaces != Binding.NO_SUPERINTERFACES) {
    ...
    protected void appendNullAnnotation(StringBuffer nameBuffer, CompilerOptions options) {
        ...
        if (// nothing appended in super.appendNullAnnotation()?
        oldSize == nameBuffer.length()) {
    ...
    public long updateTagBits() {
        if (!this.inRecursiveFunction) {
            ...
            try {
                ...
                if (this.superInterfaces != null)
                    for (TypeBinding superIfc : this.superInterfaces) this.tagBits |= superIfc.updateTagBits();
            } finally {
                this.inRecursiveFunction = false;
            }
        }
