 ...
 */
    ...
    public final String getLocalizedMessage(int id, int elaborationId, String[] problemArguments) {
        ...
        if (rawMessage == null) {
            return //$NON-NLS-1$
            "Unable to retrieve the error message for problem id: " + (id & IProblem.IgnoreCategoriesMask) + //$NON-NLS-1$
        ...
        if (elaborationId != 0) {
            ...
            if (elaboration == null) {
                return //$NON-NLS-1$
                "Unable to retrieve the error message elaboration for elaboration id: " + elaborationId + //$NON-NLS-1$
                ". Check compiler resources.";
            }
            ...
            if ((end = CharOperation.indexOf('{', message, start)) > -1) {
                ...
                if ((start = CharOperation.indexOf('}', message, end + 1)) > -1) {
                    try {
                        ...
                        return "Cannot bind message for problem (id: " + (id & IProblem.IgnoreCategoriesMask) + ") \"" + new String(message) + "\" with arguments: {" + Util.toString(problemArguments) + "}";
                    }
                    start++;
                } else {
