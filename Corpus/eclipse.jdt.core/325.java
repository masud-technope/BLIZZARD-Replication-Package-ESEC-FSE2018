 ...
 */
    ...
    private String removeIndentAndNewLines(String code, ICompilationUnit cu) throws JavaModelException {
        ...
        while (firstNonWhiteSpace < length - 1) if (!ScannerHelper.isWhitespace(code.charAt(++firstNonWhiteSpace)))
            break;
        int lastNonWhiteSpace = length;
        while (lastNonWhiteSpace > 0) if (!ScannerHelper.isWhitespace(code.charAt(--lastNonWhiteSpace)))
            break;
 ...
 */
    protected String generateSyntaxIncorrectAST() {
        ...
        //$NON-NLS-1$
        buff.append(lineSeparator + " public class A {" + lineSeparator);
