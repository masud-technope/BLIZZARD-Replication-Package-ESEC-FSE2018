...
public class BinaryMethod extends BinaryMember implements IMethod {
    ...
    // lazily initialized via call to getErasedParameterTypes
 ...
 */
    public String[] getParameterNames() throws JavaModelException {
        ...
        if (this.isConstructor()) {
            ...
            if (declaringType.isMember() && !Flags.isStatic(declaringType.getFlags())) {
                // remove synthetic argument from constructor param count
                paramCount--;
            } else if (declaringType.isEnum()) {
                if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=436347
                paramCount >= 2)
                    paramCount -= 2;
            }
        }
            ...
            if (methodDoc != null) {
                ...
                if (indexOfOpenParen != -1) {
                    ...
                    if (indexOfClosingParen != -1) {
                        final char[] paramsSource = CharOperation.replace(methodDoc.substring(indexOfOpenParen + 1, indexOfClosingParen).toCharArray(), //$NON-NLS-1$
	 ...
	 */
        ...
        while (--index > methodName.length()) {
            ...
            if (endIndex > 0) {
                if (!ScannerHelper.isJavaIdentifierPart(previousChar) || !ScannerHelper.isJavaIdentifierStart(previousChar))
                    return methodName.equals(javaDoc.substring(index + 1, endIndex));
            } else if (!scanningTag) {
                if (previousChar == '>')
                    scanningTag = true;
                else if (ScannerHelper.isJavaIdentifierPart(previousChar) || ScannerHelper.isJavaIdentifierStart(previousChar))
                    endIndex = index + 1;
                ...
                case '&':
                    if ((index + 4) < length) {
                        if (parametersSource[index + 1] == 'l' && parametersSource[index + 2] == 't' && parametersSource[index + 3] == ';') {
                            balance++;
                            index += 4;
                        } else if (parametersSource[index + 1] == 'g' && parametersSource[index + 2] == 't' && parametersSource[index + 3] == ';') {
                            balance--;
                            index += 4;
                        } else {
                            index++;
 ...
 */
    public boolean isSimilar(IMethod method) {
        return areSimilarMethods(getElementName(), getParameterTypes(), method.getElementName(), method.getParameterTypes(), null);
    }
