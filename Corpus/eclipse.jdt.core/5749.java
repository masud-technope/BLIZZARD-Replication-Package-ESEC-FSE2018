...
public final class ASTRewriteAnalyzer extends ASTVisitor {
    ...
    final TokenScanner getScanner() {
        if (this.tokenScanner == null) {
            ...
            if (this.recoveryScannerData == null) {
                scanner = new Scanner(/*tokenizeComments*/
                true, /*tokenizeWhiteSpace*/
                false, /*checkNonExternalizedStringLiterals*/
                false, compilerOptions.sourceLevel, compilerOptions.complianceLevel, /*taskTags*/
                null, /*taskPriorities*/
                null, /*taskCaseSensitive*/
                true);
            } else {
                scanner = new RecoveryScanner(/*tokenizeWhiteSpace*/
                false, /*checkNonExternalizedStringLiterals*/
                false, compilerOptions.sourceLevel, compilerOptions.complianceLevel, /*taskTags*/
                null, /*taskPriorities*/
                null, /*taskCaseSensitive*/
                true, this.recoveryScannerData);
            }
        ...
        if (insertString.length() > 0) {
            ...
            if (this.lineCommentEndOffsets.isEndOfLineComment(offset, this.content)) {
                if (!insertString.startsWith(getLineDelimiter())) {
                    ...
                    TextEdit edit = new InsertEdit(offset, getLineDelimiter());
                    addEdit(edit);
                    if (editGroup != null) {
    ...
    class ListRewriter {
        ...
        private int rewriteList(ASTNode parent, StructuralPropertyDescriptor property, String keyword, String endKeyword, int offset) {
            ...
            if (// only inserts
            insertNew) {
                if (// creating a new list -> insert keyword first (e.g. " throws ")
                keyword.length() > 0) {
                    // first node is insert
                    TextEditGroup editGroup = getEditGroup(this.list[0]);
                    doTextInsert(offset, keyword, editGroup);
                }
                currPos = offset;
            }
            if (// all removed, set back to start so the keyword is removed as well
            lastNonDelete == -1) {
                currPos = offset;
            }
            int prevEnd = currPos;
            ...
            for (int i = 0; i < total; i++) {
                ...
                if (currMark == RewriteEvent.INSERTED) {
                    ...
                    if (// element after last existing element (but not first)
                    separatorState == NONE) {
                        doTextInsert(// insert separator
                        currPos, // insert separator
                        getSeparatorString(i - 1), // insert separator
                        editGroup);
                        separatorState = NEW;
                    }
                    if (separatorState == NEW || insertAfterSeparator(node)) {
                        ...
                        doTextInsert(// insert node
                        currPos, // insert node
                        node, // insert node
                        getNodeIndent(i), // insert node
                        true, // insert node
                        editGroup);
                        separatorState = NEW;
                        if (i != lastNonDelete) {
                            if (this.list[nextIndex].getChangeKind() != RewriteEvent.INSERTED) {
                                doTextInsert(// insert separator
                                currPos, // insert separator
                                getSeparatorString(i), // insert separator
                                editGroup);
                            } else {
                                separatorState = NONE;
                            }
                    ...
                    if (i > lastNonDelete && separatorState == EXISTING) {
                        ...
                        doTextRemove(// remove separator
                        prevEnd, // remove separator
                        currPos - prevEnd, // remove separator
                        editGroup);
                        doTextRemoveAndVisit(// remove node
                        currPos, // remove node
                        currEnd - currPos, // remove node
                        node, // remove node
                        editGroup);
                        ...
                        int end = getStartOfNextNode(// start of next
                        nextIndex, // start of next
                        currEnd);
                        // from getting removed.
                        try {
                            TokenScanner scanner = getScanner();
                        ...
                        doTextRemoveAndVisit(currPos, currEnd - currPos, node, getEditGroup(// remove node
                        currEvent));
                        if (mustRemoveSeparator(currPos, i)) {
                            doTextRemove(// remove separator
                            currEnd, // remove separator
                            end - currEnd, // remove separator
                            editGroup);
                        }
                        currPos = end;
                        prevEnd = currEnd;
                ...
                } else // replaced or unchanged
                {
                    ...
                    if (// last node or next nodes are all inserts
                    i == lastNonInsert) {
                        separatorState = NONE;
                        ...
                        currPos = getStartOfNextNode(// start of next
                        nextIndex, // start of next
                        prevEnd);
    ...
    private int rewriteOptionalTypeParameters(ASTNode parent, StructuralPropertyDescriptor property, int offset, String keyword, boolean adjustOnNext, boolean needsSpaceOnRemoveAll) {
        ...
        if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            try {
                ...
                if (// all removed: set start to left bracket
                isAllRemoved) {
                ...
                pos = //$NON-NLS-1$
                new ListRewriter().rewriteList(//$NON-NLS-1$
                parent, //$NON-NLS-1$
                property, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                String.valueOf('<'), //$NON-NLS-1$
                ", ");
                if (// all removed: remove right and space up to next element
                isAllRemoved) {
                    int endPos = // set pos to '>'
                    getScanner().getTokenEndOffset(// set pos to '>'
                    TerminalTokens.TokenNameGREATER, // set pos to '>'
                    pos);
        ...
        if (// list contained some type -> parse after closing bracket
        pos != offset) {
            try {
    ...
    protected int rewriteExtraDimensionsInfo(ASTNode node, int pos, ChildListPropertyDescriptor property) {
        //$NON-NLS-1$ //$NON-NLS-2$
        return rewriteNodeList(node, property, pos, " ", "");
    }
    ...
    final void doTextInsert(int insertOffset, ASTNode node, int initialIndentLevel, boolean removeLeadingIndent, TextEditGroup editGroup) {
        ...
        for (// markers.size can change!
        int i = 0; // markers.size can change!
        i < markers.size(); // markers.size can change!
        i++) {
            ...
            data instanceof TextEditGroup) {
                ...
                String destIndentString = (lineOffset == 0) ? this.formatter.createIndentString(initialIndentLevel) : this.formatter.getIndentString(formatted.substring(lineOffset, offset));
                if (// replace with a copy/move target
                data instanceof CopyPlaceholderData) {
                ...
                } else if (// replace with a placeholder
                data instanceof StringPlaceholderData) {
    ...
    private int rewriteModifiers2(ASTNode node, ChildListPropertyDescriptor property, int pos) {
        ...
        boolean isAnnotationsProperty = isVarargsAnnotationsProperty || node instanceof AnnotatableType && property == ((AnnotatableType) node).getAnnotationsProperty();
        Prefix formatterPrefix;
        if (property == SingleVariableDeclaration.MODIFIERS2_PROPERTY || property == VariableDeclarationExpression.MODIFIERS2_PROPERTY || property == VariableDeclarationStatement.MODIFIERS2_PROPERTY || property == TypeParameter.MODIFIERS_PROPERTY || isAnnotationsProperty) {
        ...
        //$NON-NLS-1$ 
        int endPos = new ModifierRewriter(formatterPrefix).rewriteList(node, property, pos, keyword, " ");
        try {
            ...
            if (isAllRemove) {
                ...
                return nextPos;
            } else if (// see bug 165654
            (isAllInsert || (nextPos == endPos && lastUnchanged)) && !isVarargsAnnotationsProperty) {
                String separator;
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        if (!isInterface || invertType) {
            ...
            switch(changeKind) {
                case RewriteEvent.INSERTED:
                    {
                        doTextInsert(pos, " extends ", getEditGroup(//$NON-NLS-1$
                        superClassEvent));
        ...
        if (interfaceEvent == null || interfaceEvent.getChangeKind() == RewriteEvent.UNCHANGED) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            String keyword = (isInterface == invertType) ? " implements " : " extends ";
            if (invertType) {
            ...
            //$NON-NLS-1$
            pos = rewriteNodeList(node, superInterfaceProperty, pos, keyword, ", ");
        }
        // type members
    ...
    private void rewriteReturnType(MethodDeclaration node, boolean isConstructor, boolean isConstructorChange) {
        ...
        if (isConstructorChange || !returnTypeExists && newReturnType != originalReturnType) {
            ...
            if (// insert
            isConstructor || !returnTypeExists) {
        ...
        if (parent.getNodeType() == ASTNode.ARRAY_TYPE) {
            ...
            if (oldAnnotationSize > 0 && newAnnotationSize == 0) /* removed all annotations */
            {
                ...
                if (delStart >= 0 && delEnd > delStart) {
                    doTextRemove(delStart, delEnd - delStart, null);
                /* remove spaces before the annotation */
                }
                ...
                try {
                    ...
                    doTextRemove(delStart, delEnd - delStart, null);
                /* remove spaces after the annotation */
                } catch (CoreException e) {
                    e.printStackTrace();
                }
    ...
    public boolean visit(MethodDeclaration node) {
        ...
        if (apiLevel == JLS2_INTERNAL) {
            ...
            //$NON-NLS-1$
            pos = rewriteOptionalTypeParameters(node, MethodDeclaration.TYPE_PARAMETERS_PROPERTY, pos, " ", true, pos != node.getStartPosition());
        }
        ...
        try {
            ...
            //$NON-NLS-1$ 
            pos = rewriteNodeList(node, MethodDeclaration.PARAMETERS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            pos = rewriteNodeList(node, exceptionsProperty, pos, " throws ", ", ");
    ...
    public boolean visit(ArrayCreation node) {
        ...
        int nOldBrackets = getDimensions(arrayType);
        ...
        if (// changed arraytype can have different dimension or type name
        typeEvent != null && typeEvent.getChangeKind() == RewriteEvent.REPLACED) {
            typeReplaced = true;
        ...
        try {
            ...
            if (kind == RewriteEvent.REMOVED) {
                ...
                offset = node.getStartPosition() + // insert pos
                node.getLength();
            }
    ...
    private void insertAnnotationsOnDimension(ArrayType replacingType, int index, int pos, TextEditGroup editGroup, boolean astLevelGTE8) {
        if (astLevelGTE8) {
    ...
    public boolean visit(ArrayInitializer node) {
        ...
        int startPos = getPosAfterLeftBrace(node.getStartPosition());
        //$NON-NLS-1$ 
        rewriteNodeList(node, ArrayInitializer.EXPRESSIONS_PROPERTY, startPos, Util.EMPTY_STRING, ", ");
        return false;
    }

    private int rewriteArrayTypeDimensions(ArrayType node, int pos) {
        //$NON-NLS-1$
        return rewriteNodeList(node, ArrayType.DIMENSIONS_PROPERTY, pos, Util.EMPTY_STRING, "");
    }
    ...
    public boolean visit(BreakStatement node) {
        ...
        try {
            ...
            // space between break and label
            rewriteNode(node, BreakStatement.LABEL_PROPERTY, offset, ASTRewriteFormatter.SPACE);
        } catch (CoreException e) {
            handleException(e);
        }
        return false;
	 ...
	 */
    public // catch (Exception) Block
    boolean visit(// catch (Exception) Block
    CatchClause node) {
        if (!hasChildrenChanges(node)) {
            return doVisitUnchangedChildren(node);
        }
    ...
    public boolean visit(ClassInstanceCreation node) {
        ...
        if (node.getAST().apiLevel() == JLS2_INTERNAL) {
            ...
            if (isChanged(node, ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY)) {
                try {
                    pos = //after 'new'
                    getScanner().getTokenEndOffset(//after 'new'
                    TerminalTokens.TokenNamenew, //after 'new'
                    pos);
                    rewriteOptionalTypeParameters(//$NON-NLS-1$
                    node, //$NON-NLS-1$
                    ClassInstanceCreation.TYPE_ARGUMENTS_PROPERTY, //$NON-NLS-1$
                    pos, //$NON-NLS-1$
                    " ", //$NON-NLS-1$
                    true, //$NON-NLS-1$
                    true);
        ...
        if (isChanged(node, ClassInstanceCreation.ARGUMENTS_PROPERTY)) {
            try {
                ...
                //$NON-NLS-1$ 
                rewriteNodeList(node, ClassInstanceCreation.ARGUMENTS_PROPERTY, startpos, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
	 ...
	 */
    public // expression ? thenExpression : elseExpression
    boolean visit(// expression ? thenExpression : elseExpression
    ...
    public boolean visit(ConstructorInvocation node) {
        ...
        try {
            ...
            //$NON-NLS-1$ 
            rewriteNodeList(node, ConstructorInvocation.ARGUMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        } catch (CoreException e) {
            handleException(e);
        }
        return false;
    ...
    public boolean visit(ContinueStatement node) {
        ...
        try {
            ...
            // space between continue and label
            rewriteNode(node, ContinueStatement.LABEL_PROPERTY, offset, ASTRewriteFormatter.SPACE);
        } catch (CoreException e) {
            handleException(e);
        }
        return false;
	 ...
	 */
    public // do statement while expression
    boolean visit(// do statement while expression
    DoStatement node) {
        if (!hasChildrenChanges(node)) {
            return doVisitUnchangedChildren(node);
        }
        ...
        try {
            ...
            if (event != null && event.getChangeKind() == RewriteEvent.REPLACED) {
                ...
                rewriteBodyNode(node, DoStatement.BODY_PROPERTY, startOffset, endPos, getIndent(node.getStartPosition()), // body
                this.formatter.DO_BLOCK);
            } else {
    ...
    boolean visit(// expression.name
    FieldAccess node) {
        if (!hasChildrenChanges(node)) {
            return doVisitUnchangedChildren(node);
        }
        // expression
        rewriteRequiredNode(node, FieldAccess.EXPRESSION_PROPERTY);
        // name
        rewriteRequiredNode(node, FieldAccess.NAME_PROPERTY);
        return false;
    }
	 ...
	 */
    public //{ Modifier } Type VariableDeclarationFragment { ',' VariableDeclarationFragment } ';'
    boolean visit(//{ Modifier } Type VariableDeclarationFragment { ',' VariableDeclarationFragment } ';'
    FieldDeclaration node) {
        ...
        //$NON-NLS-1$ 
        rewriteNodeList(node, FieldDeclaration.FRAGMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        return false;
    }
    ...
    public boolean visit(ForStatement node) {
        ...
        try {
            ...
            if (isChanged(node, ForStatement.INITIALIZERS_PROPERTY)) {
                ...
                //$NON-NLS-1$ 
                pos = rewriteNodeList(node, ForStatement.INITIALIZERS_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } else {
            ...
            if (isChanged(node, ForStatement.UPDATERS_PROPERTY)) {
                ...
                //$NON-NLS-1$ 
                pos = rewriteNodeList(node, ForStatement.UPDATERS_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } else {
            ...
            if (bodyEvent != null && bodyEvent.getChangeKind() == RewriteEvent.REPLACED) {
                ...
                rewriteBodyNode(node, ForStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), // body
                this.formatter.FOR_BLOCK);
            } else {
    ...
    public boolean visit(IfStatement node) {
        ...
        // statement
        int pos = rewriteRequiredNode(node, IfStatement.EXPRESSION_PROPERTY);
        ...
        if (thenEvent != null && thenEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
            try {
                ...
                int tok = getScanner().readNext(pos, true);
                ...
                if (elseStatement != null) {
                    ...
                    endPos = getScanner().getTokenStartOffset(TerminalTokens.TokenNameelse, thenStatement.getStartPosition() + // else keyword
    ...
    private void ensureSpaceAfterReplace(ASTNode node, ChildPropertyDescriptor desc) {
        if (getChangeKind(node, desc) == RewriteEvent.REPLACED) {
            ...
            try {
                int offset = // instanceof
                getScanner().getNextStartOffset(// instanceof
                leftOperandEnd, // instanceof
                true);
    ...
    public boolean visit(IntersectionType node) {
        ...
        //$NON-NLS-1$
        rewriteNodeList(node, IntersectionType.TYPES_PROPERTY, node.getStartPosition(), Util.EMPTY_STRING, " & ");
        return false;
    }
    ...
    public boolean visit(LambdaExpression node) {
        ...
        if (// Parentheses can be absent if and only if there is one and only one type elided parameter.
        !hasParentheses) {
        ...
        } else if (// parameter property changed to effect parentheses insertion
        !oldHasParentheses && hasParentheses) {
            if (// a null check though event cannot be null here
            (event = getEvent(node, LambdaExpression.PARAMETERS_PROPERTY)) != null) {
                editGroup = getEditGroup(event);
                insertParentheses = true;
            }
        }
        ...
        if (isChanged(node, LambdaExpression.PARAMETERS_PROPERTY)) {
            try {
                ...
                pos = rewriteNodeList(//$NON-NLS-1$
                node, //$NON-NLS-1$
                LambdaExpression.PARAMETERS_PROPERTY, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                Util.EMPTY_STRING, //$NON-NLS-1$
                ", ");
            } catch (CoreException e) {
                handleException(e);
            }
    ...
    public boolean visit(MethodInvocation node) {
        ...
        if (isChanged(node, MethodInvocation.ARGUMENTS_PROPERTY)) {
            ...
            try {
                int startOffset = getScanner().getTokenEndOffset(TerminalTokens.TokenNameLPAREN, pos);
                //$NON-NLS-1$ 
                rewriteNodeList(node, MethodInvocation.ARGUMENTS_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
        ...
        if (apiLevel < AST.JLS8) {
            ...
            if (extraDims > 0) {
                ...
                if (kind == RewriteEvent.REMOVED) {
                    ...
                    pos = // insert pos
                    node.getStartPosition() + // insert pos
                    node.getLength();
                }
            }
        } else {
    ...
    public boolean visit(SuperConstructorInvocation node) {
        ...
        if (isChanged(node, SuperConstructorInvocation.ARGUMENTS_PROPERTY)) {
            ...
            try {
                pos = getScanner().getTokenEndOffset(TerminalTokens.TokenNameLPAREN, pos);
                //$NON-NLS-1$ 
                rewriteNodeList(node, SuperConstructorInvocation.ARGUMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    public boolean visit(SuperMethodInvocation node) {
        ...
        if (isChanged(node, SuperMethodInvocation.ARGUMENTS_PROPERTY)) {
            ...
            try {
                pos = getScanner().getTokenEndOffset(TerminalTokens.TokenNameLPAREN, pos);
                //$NON-NLS-1$ 
                rewriteNodeList(node, SuperMethodInvocation.ARGUMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    class SwitchListRewriter extends ParagraphListRewriter {
        ...
        public  SwitchListRewriter(int initialIndent) {
            ...
            this.indentSwitchStatementsCompareToCases = DefaultCodeFormatterConstants.TRUE.equals(ASTRewriteAnalyzer.this.options.get(DefaultCodeFormatterConstants.FORMATTER_INDENT_SWITCHSTATEMENTS_COMPARE_TO_CASES));
        }
    ...
    public boolean visit(TryStatement node) {
        ...
        if (node.getAST().apiLevel() >= JLS4_INTERNAL) {
            if (isChanged(node, TryStatement.RESOURCES_PROPERTY)) {
                ...
                String newParen = //$NON-NLS-1$
                this.formatter.TRY_RESOURCES_PAREN.getPrefix(indent) + //$NON-NLS-1$
                "(";
                //$NON-NLS-1$ //$NON-NLS-2$
                pos = rewriteNodeList(node, TryStatement.RESOURCES_PROPERTY, getPosAfterTry(pos), newParen, ")", ";" + prefix);
            } else {
    ...
    public boolean visit(UnionType node) {
        ...
        //$NON-NLS-1$
        rewriteNodeList(node, UnionType.TYPES_PROPERTY, node.getStartPosition(), Util.EMPTY_STRING, " | ");
        return false;
    }
    ...
    public boolean visit(VariableDeclarationExpression node) {
        ...
        //$NON-NLS-1$ 
        rewriteNodeList(node, VariableDeclarationExpression.FRAGMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        return false;
    }
    ...
    public boolean visit(VariableDeclarationStatement node) {
        ...
        //$NON-NLS-1$ 
        rewriteNodeList(node, VariableDeclarationStatement.FRAGMENTS_PROPERTY, pos, Util.EMPTY_STRING, ", ");
        return false;
    }
    ...
    public boolean visit(WhileStatement node) {
        ...
        try {
            if (isChanged(node, WhileStatement.BODY_PROPERTY)) {
                int startOffset = getScanner().getTokenEndOffset(TerminalTokens.TokenNameRPAREN, pos);
                rewriteBodyNode(node, WhileStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), // body
                this.formatter.WHILE_BLOCK);
            } else {
    ...
    public boolean visit(MethodRef node) {
        ...
        if (isChanged(node, MethodRef.PARAMETERS_PROPERTY)) {
            ...
            try {
                int startOffset = getScanner().getTokenEndOffset(TerminalTokens.TokenNameLPAREN, pos);
                //$NON-NLS-1$ 
                rewriteNodeList(node, MethodRef.PARAMETERS_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
        ...
        if (node.getAST().apiLevel() >= JLS3_INTERNAL) {
            if (isChanged(node, MethodRefParameter.VARARGS_PROPERTY)) {
                if (getNewValue(node, MethodRefParameter.VARARGS_PROPERTY).equals(Boolean.TRUE)) {
                    doTextInsert(pos, "...", getEditGroup(node, //$NON-NLS-1$
    ...
    public boolean visit(TagElement node) {
        ...
        if (isChanged(node, TagElement.FRAGMENTS_PROPERTY)) {
            ...
            //$NON-NLS-1$//$NON-NLS-2$
            rewriteNodeList(node, TagElement.FRAGMENTS_PROPERTY, endOffset, " ", " ");
        } else {
    ...
    public boolean visit(EnhancedForStatement node) {
        ...
        if (bodyEvent != null && bodyEvent.getChangeKind() == RewriteEvent.REPLACED) {
            ...
            try {
                ...
                rewriteBodyNode(node, EnhancedForStatement.BODY_PROPERTY, startOffset, -1, getIndent(node.getStartPosition()), // body
                this.formatter.FOR_BLOCK);
            } catch (CoreException e) {
                handleException(e);
            }
    ...
    public boolean visit(EnumConstantDeclaration node) {
        ...
        if (argsEvent != null && argsEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
            ...
            try {
                ...
                pos = rewriteNodeList(//$NON-NLS-1$
                node, //$NON-NLS-1$
                EnumConstantDeclaration.ARGUMENTS_PROPERTY, //$NON-NLS-1$
                pos, //$NON-NLS-1$
                prefix, //$NON-NLS-1$
                ", ");
                if (!hasParents) {
                    doTextInsert(pos, ")", getEditGroup(children[//$NON-NLS-1$
        ...
        if (isChanged(node, EnumConstantDeclaration.ANONYMOUS_CLASS_DECLARATION_PROPERTY)) {
            ...
            if (kind == RewriteEvent.REMOVED) {
                ...
                pos = node.getStartPosition() + // insert pos
                node.getLength();
            }
    ...
    public boolean visit(EnumDeclaration node) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        pos = rewriteNodeList(node, EnumDeclaration.SUPER_INTERFACE_TYPES_PROPERTY, pos, " implements ", ", ");
        ...
        //$NON-NLS-1$
        pos = rewriteNodeList(node, EnumDeclaration.ENUM_CONSTANTS_PROPERTY, pos, leadString, ", ");
                ...
                if (!hasSemicolon && isAllOfKind(children, RewriteEvent.INSERTED)) {
                    ...
                    if (token == TerminalTokens.TokenNameCOMMA) {
                        ...
                        if (nextToken != TerminalTokens.TokenNameSEMICOLON) {
                            doTextInsert(endPos, ";", getEditGroup(//$NON-NLS-1$
                            children[0]));
                        } else {
                        ...
                        doTextInsert(pos, ";", getEditGroup(//$NON-NLS-1$
                        children[0]));
                    }
    ...
    public boolean visit(Modifier node) {
        ...
        String newText = getNewValue(node, Modifier.KEYWORD_PROPERTY).toString();
    ...
    public boolean visit(NormalAnnotation node) {
        ...
        if (isChanged(node, NormalAnnotation.VALUES_PROPERTY)) {
            ...
            try {
                int startOffset = getScanner().getTokenEndOffset(TerminalTokens.TokenNameLPAREN, pos);
                //$NON-NLS-1$ 
                rewriteNodeList(node, NormalAnnotation.VALUES_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    public boolean visit(ParameterizedType node) {
        ...
        if (isChanged(node, ParameterizedType.TYPE_ARGUMENTS_PROPERTY)) {
            ...
            try {
                ...
                //$NON-NLS-1$ 
                rewriteNodeList(node, ParameterizedType.TYPE_ARGUMENTS_PROPERTY, startOffset, Util.EMPTY_STRING, ", ");
            } catch (CoreException e) {
                handleException(e);
            }
        } else {
    ...
    public boolean visit(TypeParameter node) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        rewriteNodeList(node, TypeParameter.TYPE_BOUNDS_PROPERTY, pos, " extends ", " & ");
        return false;
    }
    ...
    public boolean visit(WildcardType node) {
        ...
        try {
            // pos after question mark
            int pos = getScanner().getNextEndOffset(node.getStartPosition(), true);
            Prefix prefix;
