    ...
    public void close() {
        try {
            ...
            if (phase == Phase.RECONCILE && _env.currentProcessorSupportsRTTG()) {
                ...
                result = gfm.generateFileDuringReconcile(parentCompilationUnit, _typeName, contents);
            // Need to call ReconcileContext.resetAst() for this to be effective;
            // that will happen in ReconcileEnv.close().
            } else if (phase == Phase.BUILD) {
                result = gfm.generateFileDuringBuild(Collections.singletonList(_env.getFile()), _typeName, contents, _env.currentProcessorSupportsRTTG(), /* progress monitor */
                null);
            }
            if (result != null) {
