...
public class EqualExpression extends BinaryExpression {
    ...
    private void checkNullComparison(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, FlowInfo initsWhenTrue, FlowInfo initsWhenFalse) {
        ...
        if (!leftNonNullChecked) {
            ...
            } else if (this.left instanceof Reference && ((!contextualCheckEquality && rightStatus == FlowInfo.NULL) || (contextualCheckEquality && rightStatus == FlowInfo.NON_NULL)) && scope.compilerOptions().enableSyntacticNullAnalysisForFields) {
        ...
        if (!rightNonNullChecked) {
            ...
            } else if (this.right instanceof Reference && ((!contextualCheckEquality && leftStatus == FlowInfo.NULL) || (contextualCheckEquality && leftStatus == FlowInfo.NON_NULL)) && scope.compilerOptions().enableSyntacticNullAnalysisForFields) {
        ...
        switch(nullStatus) {
            case FlowInfo.NULL:
                if (((this.bits & OperatorMASK) >> OperatorSHIFT) == EQUAL_EQUAL) {
                    flowContext.recordUsingNullReference(scope, local, reference, FlowContext.CAN_ONLY_NULL_NON_NULL | FlowContext.IN_COMPARISON_NULL, flowInfo);
                    // from thereon it is set
                    initsWhenTrue.markAsComparedEqualToNull(local);
                    // from thereon it is set
                    initsWhenFalse.markAsComparedEqualToNonNull(local);
                } else {
                    flowContext.recordUsingNullReference(scope, local, reference, FlowContext.CAN_ONLY_NULL_NON_NULL | FlowContext.IN_COMPARISON_NON_NULL, flowInfo);
                    // from thereon it is set
            ...
            case FlowInfo.NON_NULL:
                if (((this.bits & OperatorMASK) >> OperatorSHIFT) == EQUAL_EQUAL) {
                    flowContext.recordUsingNullReference(scope, local, reference, FlowContext.CAN_ONLY_NULL | FlowContext.IN_COMPARISON_NON_NULL, flowInfo);
                    // from thereon it is set
                    initsWhenTrue.markAsComparedEqualToNonNull(local);
                } else {
                    flowContext.recordUsingNullReference(scope, local, reference, FlowContext.CAN_ONLY_NULL | FlowContext.IN_COMPARISON_NULL, flowInfo);
                }
                break;
        }
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (((this.bits & OperatorMASK) >> OperatorSHIFT) == EQUAL_EQUAL) {
            if ((this.left.constant != Constant.NotAConstant) && (this.left.constant.typeID() == T_boolean)) {
                if (//  true == anything
                this.left.constant.booleanValue()) {
                ...
                if (//  anything == true
                this.right.constant.booleanValue()) {
        ...
        if (result instanceof UnconditionalFlowInfo && // the flow info is flat
        (result.tagBits & FlowInfo.UNREACHABLE) == 0) {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if ((this.left.implicitConversion & COMPILE_TYPE_MASK) == /*compile-time*/
        T_boolean) {
    ...
    public void generateNonBooleanEqual(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (valueRequired) {
            BranchLabel falseLabel = new BranchLabel(codeStream);
            if (isEqualOperator) {
                switch(// operand runtime type
                (this.left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) {
                    case T_int:
                        codeStream.if_icmpne(falseLabel);
                        break;
                    case T_float:
    ...
    public void generateOptimizedNonBooleanEqual(BlockScope currentScope, CodeStream codeStream, BranchLabel trueLabel, BranchLabel falseLabel, boolean valueRequired) {
        ...
        if (this.right instanceof NullLiteral) {
            ...
            codeStream.recordPositionsFrom(pc, this.sourceStart);
            return;
        } else if (// optimized case: null == x
        this.left instanceof NullLiteral) {
            this.right.generateCode(currentScope, codeStream, valueRequired);
            if (valueRequired) {
                if (falseLabel == null) {
                    if (trueLabel != null) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (leftType.isBaseType() && rightType.isBaseType()) {
            ...
            if (leftDirect != null && leftDirect == Expression.getDirectBinding(this.right)) {
                ...
                if ((operator == EQUAL_EQUAL && this.constant == BooleanConstant.fromValue(true)) || (operator == NOT_EQUAL && this.constant == BooleanConstant.fromValue(false)))
                    scope.problemReporter().comparingIdenticalExpressions(this);
            }
            return this.resolvedType = TypeBinding.BOOLEAN;
        }
        // spec 15.20.3
        if (// cannot compare: Object == (int)0
        (!leftType.isBaseType() || leftType == TypeBinding.NULL) && (!rightType.isBaseType() || rightType == TypeBinding.NULL) && (checkCastTypesCompatibility(scope, leftType, rightType, null) || checkCastTypesCompatibility(scope, rightType, leftType, null))) {
            ...
            if (unnecessaryLeftCast || unnecessaryRightCast) {
                ...
                if (checkCastTypesCompatibility(scope, alternateLeftType, alternateRightType, null) || checkCastTypesCompatibility(scope, alternateRightType, alternateLeftType, null)) {
