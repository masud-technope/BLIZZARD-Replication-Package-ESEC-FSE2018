 ...
 */
    public final boolean checkCastTypesCompatibility(Scope scope, TypeBinding castType, TypeBinding expressionType, Expression expression) {
        ...
        if (castType.isBaseType()) {
            ...
            } else if (use17specifics && castType.isPrimitiveType() && expressionType instanceof ReferenceBinding && !expressionType.isBoxedPrimitiveType() && checkCastTypesCompatibility(scope, scope.boxing(castType), expressionType, expression)) {
                // by our own interpretation (in accordance with javac) we reject arays, though.
                return true;
            } else if (use15specifics && // unboxing - only widening match is allowed
            scope.environment().computeBoxingType(expressionType).isCompatibleWith(castType)) {
                tagAsUnnecessaryCast(scope, castType);
                return true;
            }
            return false;
        } else if (use15specifics && expressionType.isBaseType() && // boxing - only widening match is allowed
        scope.environment().computeBoxingType(expressionType).isCompatibleWith(castType)) {
            tagAsUnnecessaryCast(scope, castType);
            return true;
        }
            ...
            case Binding.ARRAY_TYPE:
                ...
                switch(castType.kind()) {
                    ...
                    case Binding.TYPE_PARAMETER:
                        ...
                        if (match == null) {
                            checkUnsafeCast(scope, castType, /*no match*/
            ...
            default:
                if (expressionType.isInterface()) {
                    switch(castType.kind()) {
                        ...
                        default:
                            if (castType.isInterface()) {
                                ...
                                if (use15specifics) {
                                    checkUnsafeCast(scope, /*no match*/
                                    castType, expressionType, null, true);
                    ...
                    switch(castType.kind()) {
                        case Binding.ARRAY_TYPE:
                            // ( ARRAY ) CLASS
                            if (// potential runtime error
                            expressionType.id == TypeIds.T_JavaLangObject) {
                                if (use15specifics)
 ...
 */
    ...
    public boolean checkNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo) {
        // default: don't mark field references as checked for null
    ...
    protected void checkNPEbyUnboxing(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo) {
        int status;
        if ((this.implicitConversion & UNBOXING) != 0 && (this.bits & ASTNode.IsNonNull) == 0 && (status = nullStatus(flowInfo, flowContext)) != FlowInfo.NON_NULL) {
 ...
 */
    public void computeConversion(Scope scope, TypeBinding runtimeType, TypeBinding compileTimeType) {
        ...
        if (runtimeType != TypeBinding.NULL && runtimeType.isBaseType()) {
            ...
            if (// Object o = 12;
            TypeBinding.equalsEquals(boxedType, runtimeType))
                boxedType = compileTimeType;
            if (// (Comparable & Serializable) 0
            boxedType.id >= TypeIds.T_LastWellKnownTypeId) {
                boxedType = compileTimeType;
            }
        ...
        if (// e.g. ? extends String  ==> String (103227); >= TypeIds.T_LastWellKnownTypeId implies TypeIds.NoId
        (compileTimeTypeID = compileTimeType.id) >= TypeIds.T_LastWellKnownTypeId) {
            ...
            // treatment is the same as for jlO.
 ...
 */
    ...
    public static Binding getDirectBinding(Expression someExpression) {
        ...
        } else if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=276741
        someExpression.isThis()) {
    ...
    public boolean isConstantValueOfTypeAssignableToType(TypeBinding constantType, TypeBinding targetType) {
        ...
        if (BaseTypeBinding.isWidening(TypeIds.T_int, constantType.id) && (BaseTypeBinding.isNarrowing(targetType.id, TypeIds.T_int))) {
 ...
 */
    ...
    public Expression resolveExpressionExpecting(TypeBinding targetType, Scope scope, InferenceContext18 context) {
        // subclasses should implement for a better resolved expression if required.
        return this;
    }
    ...
    public boolean forcedToBeRaw(ReferenceContext referenceContext) {
        if (this instanceof NameReference) {
            ...
            if (receiverBinding.isParameter() && (((LocalVariableBinding) receiverBinding).tagBits & TagBits.ForcedToBeRawType) != 0) {
                // parameter is forced to be raw since super method uses raw types.
                return true;
                ...
                if (field.type.isRawType()) {
                    if (referenceContext instanceof AbstractMethodDeclaration) {
                        ...
                        if (// inherited raw field, see https://bugs.eclipse.org/bugs/show_bug.cgi?id=337962
                        TypeBinding.notEquals(field.declaringClass, methodDecl.binding.declaringClass)) {
                            return true;
                        }
                    } else if (referenceContext instanceof TypeDeclaration) {
                        TypeDeclaration type = (TypeDeclaration) referenceContext;
                        if (// inherited raw field, see https://bugs.eclipse.org/bugs/show_bug.cgi?id=337962
                        TypeBinding.notEquals(field.declaringClass, type.binding)) {
                            return true;
                        }
                    }
                }
            ...
            if (!CharOperation.equals(((MessageSend) this).binding.declaringClass.getFileName(), // problem is rooted elsewhere
            ...
            if (!CharOperation.equals(field.declaringClass.getFileName(), // problem is rooted elsewhere
            referenceContext.compilationResult().getFileName())) {
                return true;
            }
            if (field.type.isRawType()) {
        ...
        } else if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=337751
        this instanceof ConditionalExpression) {
...
*/
    ...
    public boolean isPotentiallyCompatibleWith(TypeBinding targetType, Scope scope) {
        // for all but functional expressions, potential compatibility is the same as compatibility.
