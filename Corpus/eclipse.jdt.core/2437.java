...
public class TextEditsBuilder extends TokenTraverser {
    ...
    public  TextEditsBuilder(String source, IRegion[] regions, TokenManager tokenManager, DefaultCodeFormatterOptions options) {
        this.source = source;
    ...
    private List<IRegion> adaptRegions(IRegion[] givenRegions) {
        ...
        for (IRegion region : givenRegions) {
            ...
            if (start > sourceStart) {
                ...
                if ((token.tokenType == TokenNameCOMMENT_BLOCK || token.tokenType == TokenNameCOMMENT_JAVADOC) && start <= token.originalEnd) {
                    start = token.originalStart;
                }
            }
            if (end > start && end > sourceStart) {
                Token token = this.tm.get(this.tm.findIndex(end, -1, false));
                if ((token.tokenType == TokenNameCOMMENT_BLOCK || token.tokenType == TokenNameCOMMENT_JAVADOC) && end < token.originalEnd) {
    ...
    protected boolean token(Token token, int index) {
        ...
        if (getNext() == null) {
            for (int i = 0; i < token.getLineBreaksAfter(); i++) bufferLineSeparator(null, i + 1 == token.getLineBreaksAfter());
            ...
            if (getLineBreaksBefore() > 1) {
                ...
                if (this.options.indent_empty_lines && token.tokenType != TokenNameNotAToken) {
                    if (index == 0) {
                        ...
                        boolean isForced = token.getWrapPolicy() != null && token.getWrapPolicy().wrapMode == WrapMode.FORCED;
    ...
    private void bufferLineSeparator(Token token, boolean emptyLine) {
        ...
        if (token != null && token.tokenType == TokenNameNotAToken)
            // this is an unformatted block comment, don't force asterisk
            return;
        if (getNext() == null && !emptyLine)
            // this is the last token of block comment, asterisk is included
            return;
        ...
        for (int i = this.counter; i < searchLimit; i++) {
            ...
            if (c == '*') {
                this.buffer.append(' ');
                flushBuffer(i);
                while (i + 1 < this.sourceLimit && this.source.charAt(i + 1) == '*') i++;
                this.counter = i + 1;
    ...
    private void bufferIndent(Token token, int index) {
        ...
        int spaces = 0;
        if (this.options.use_tabs_only_for_leading_indentations && this.options.tab_char != DefaultCodeFormatterOptions.SPACE) {
    ...
    private void bufferAlign(Token token, int index) {
        ...
        switch(this.alignChar) {
            ...
            default:
                throw new //$NON-NLS-1$
                IllegalStateException(//$NON-NLS-1$
                "Unrecognized align char: " + this.alignChar);
        }
    }
    ...
    private boolean isOnlyWhitespace(String text) {
        for (int i = 0; i < text.length(); i++) if (!ScannerHelper.isWhitespace(text.charAt(i)))
            return false;
        return true;
    }
        ...
        theLoop: while (textPos >= 0 && textPos < text.length() && sourcePos >= 0 && sourcePos < this.source.length()) {
            ...
            if (c1 == c2 && (c1 == ' ' || c1 == '\t')) {
                ...
                for (i = 0; i < this.options.tab_size; i++) {
                    sourcePos += direction;
                    if (i < this.options.tab_size - 1 && (sourcePos < 0 || sourcePos >= this.source.length() || this.source.charAt(sourcePos) != ' '))
                        continue theLoop;
                }
                textPos -= direction;
            } else if (c2 == '\t' && c1 == ' ') {
                for (i = 0; i < this.options.tab_size; i++) {
                    textPos -= direction;
                    if (i < this.options.tab_size - 1 && (textPos < 0 || textPos >= text.length() || text.charAt(textPos) != ' '))
                        continue theLoop;
                }
                sourcePos += direction;
            } else {
    ...
    private void handleSingleLineComment(Token lineComment, int index) {
        ...
        if (// otherwise it's a forged comment
        lineComment.originalEnd > lineComment.originalStart)
