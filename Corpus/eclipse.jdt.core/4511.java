...
public class IncrementalTests18 extends BuilderTests {
    ...
    public void test423122() throws JavaModelException {
        ...
        expectingProblemsFor(projectPath, "Problem : The target type of this expression must be a functional interface [ resource : </Project/src/p/X.java> range : <35,40> category : <40> severity : <2>]");
    }
    ...
    public void test423122a() throws JavaModelException {
        ...
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : The operator + is undefined for the argument type(s) Object, int [ resource : </Project/src/test1/Ref.java> range : <76,79> category : <60> severity : <2>]");
    ...
    public void test427105() throws JavaModelException {
        ...
        fullBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : List is a raw type. References to generic type List<E> should be parameterized [ resource : </Project/src/I.java> range : <55,59> category : <130> severity : <1>]\n" + "Problem : List is a raw type. References to generic type List<E> should be parameterized [ resource : </Project/src/X.java> range : <68,72> category : <130> severity : <1>]");
        env.addClass(root, "", "X", "import java.util.List;\n" + "public class X implements I {\n" + "	public void f(List x, List<I> ls) {                                      \n" + "	}\n" + "}\n");
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : List is a raw type. References to generic type List<E> should be parameterized [ resource : </Project/src/I.java> range : <55,59> category : <130> severity : <1>]\n" + "Problem : List is a raw type. References to generic type List<E> should be parameterized [ resource : </Project/src/X.java> range : <68,72> category : <130> severity : <1>]");
    }
    ...
    public void testBug442452() throws JavaModelException {
        ...
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "Entity", "public class Entity implements IEntity<Entity> {\n" + "	public int compareTo(IBasicItem o) {\n" + "		return 0;\n" + "	}\n" + //$NON-NLS-1$
        "}\n");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "IEntity", "public interface IEntity<T extends IEntity<T>> extends IBasicItem {\n" + //$NON-NLS-1$
        "}\n");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "IBasicItem", "public interface IBasicItem extends Comparable<IBasicItem> {\n" + //$NON-NLS-1$
        "}\n");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "IAdvancedItem", "public interface IAdvancedItem extends Comparable<IBasicItem> {\n" + //$NON-NLS-1$
        "}\n");
        fullBuild(projectPath);
        expectingNoProblems();
        //$NON-NLS-1$ //$NON-NLS-2$
        ...
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "Entity", "public class Entity implements IEntity<Entity>, IAdvancedItem {\n" + "	public int compareTo(IBasicItem o) {\n" + "		return 0;\n" + "	}\n" + //$NON-NLS-1$
        "}\n");
        incrementalBuild(projectPath);
        expectingNoProblems();
    }
    ...
    public void testBug442755() throws JavaModelException {
        ...
        fullBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : X cannot be resolved to a type [ resource : </Project/Z.java>" + " range : <31,32> category : <40> severity : <2>]\n" + "Problem : Y cannot be resolved to a type [ resource : </Project/Z.java>" + " range : <45,46> category : <40> severity : <2>]");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
        "Unmarshaller", "public abstract class Unmarshaller<CONTEXT extends Context, DESCRIPTOR extends Z> {\n" + "	public CONTEXT getContext() {\n" + "		return null;\n" + "	}\n" + "}\n");
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : The project was not built since its build path is incomplete." + " Cannot find the class file for Y. Fix the build path then try building" + " this project [ resource : </Project> range : <-1,-1> category : <10> severity : <2>]\n" + "Problem : The type Y cannot be resolved. It is indirectly referenced from" + " required .class files [ resource : </Project/Unmarshaller.java> range : <0,1> category : <10> severity : <2>]");
    }
    ...
    public void testBug442755a() throws JavaModelException {
        ...
        fullBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : Y cannot be resolved to a type [ resource : " + "</Project/Z.java> range : <27,28> category : <40> severity : <2>]");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        projectPath, //$NON-NLS-1$ //$NON-NLS-2$
        "", //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void test445049() throws JavaModelException, IOException {
        ...
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : No enclosing instance of the type I is accessible in scope [" + " resource : </Project/src/X.java> range : <31,38> category : <40> severity : <2>]");
    }
    ...
    public void testBug481276b() throws Exception {
        ...
        fullBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : Null type mismatch (type annotations): required \'@NonNull Object @NonNull[]\' but this expression has type \'@Nullable Object @NonNull[]\' [" + " resource : </Project/src/testNullAnnotations/Snippet.java> range : <321,342> category : <90> severity : <2>]\n" + "Problem : Null type mismatch (type annotations): required \'@Nullable Object @Nullable[]\' but this expression has type \'@NonNull Object @Nullable[]\' [" + " resource : </Project/src/testNullAnnotations/Snippet.java> range : <334,341> category : <90> severity : <2>]");
    ...
    public void testBug481276c() throws Exception {
        ...
        env.addClass(root, "testNullAnnotations", "package-info", "@org.eclipse.jdt.annotation.NonNullByDefault({})\n" + "package testNullAnnotations;\n");
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : Null type safety (type annotations): The expression of type \'@NonNull Object []\' needs unchecked conversion to conform to \'@NonNull Object @NonNull[]\' [" + " resource : </Project/src/testNullAnnotations/Snippet.java> range : <303,324> category : <90> severity : <1>]");
    }
