...
public final class ListRewrite {
    ...
    private void validatePropertyType(ASTNode node) {
        ...
        if (!this.childListProperty.getElementType().isAssignableFrom(node.getClass())) {
            ...
            node.getClass().getName() + " is not a valid type for " + this.childListProperty.getNodeClass().getName() + " property '" + this.childListProperty.getId() + "'. Must be " + //$NON-NLS-1$ //$NON-NLS-2$
