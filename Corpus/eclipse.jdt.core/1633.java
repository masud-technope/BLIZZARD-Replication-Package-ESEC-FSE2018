        ...
        try {
            if (this.resolvedType == null) {
                ...
                if (this.resolvedType instanceof ProblemReferenceBinding) {
                    ...
                    this.resolvedType = new ProblemReferenceBinding(org.eclipse.jdt.core.compiler.CharOperation.subarray(this.tokens, 0, tokenIndex + 1), problemBinding.original, this.resolvedType.problemId());
                }
            }
    ...
    protected TypeBinding getTypeBinding(Scope scope) {
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length; i < max; i++) {
            ...
            if (isClassScope)
                ...
                ((ClassScope) scope).detectCycle(this.resolvedType, this, null))
                    return null;
            ReferenceBinding currentType = (ReferenceBinding) this.resolvedType;
            if (currentType.isGenericType()) {
                qualifiedType = scope.environment().createRawType(currentType, qualifiedType);
            } else {
                qualifiedType = (qualifiedType != null && (qualifiedType.isRawType() || qualifiedType.isParameterizedType())) ? scope.createParameterizedType(currentType, null, qualifiedType) : currentType;
            }
        }
