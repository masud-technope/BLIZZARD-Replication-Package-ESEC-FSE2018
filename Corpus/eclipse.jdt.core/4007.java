 ...
 */
                ...
                if (this.constant != Constant.NotAConstant) {
                    ...
                    for (restart = this.arity - 1; restart >= 0; restart--) {
                        if ((cursor = this.referencesTable[restart]).constant != Constant.NotAConstant) {
                            ...
                            codeStream.invokeStringConcatenationAppendForType(cursor.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                            break;
                        }
                    ...
                    restart++;
                    if (// reached the leftmost expression
                    restart == 0) {
                        cursor.left.generateOptimizedStringConcatenation(blockScope, codeStream, cursor.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                    }
                    int pcAux;
                    for (int i = restart; i < this.arity; i++) {
                        ...
                        cursor.right.generateOptimizedStringConcatenation(blockScope, codeStream, cursor.right.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
    ...
    public void generateOptimizedStringConcatenationCreation(BlockScope blockScope, CodeStream codeStream, int typeID) {
        ...
        if (this.referencesTable == null) {
            ...
            if ((((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS) && ((this.bits & ASTNode.ReturnTypeIDMASK) == TypeIds.T_JavaLangString) && this.constant == Constant.NotAConstant) {
                int pc = codeStream.position;
                BinaryExpression cursor = this.referencesTable[this.arity - 1];
                // silence warnings
                int restart = 0;
                for (restart = this.arity - 1; restart >= 0; restart--) {
                    if (((((cursor = this.referencesTable[restart]).bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS) && ((cursor.bits & ASTNode.ReturnTypeIDMASK) == TypeIds.T_JavaLangString)) {
                        ...
                        cursor.generateOptimizedStringConcatenationCreation(blockScope, codeStream, cursor.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                        break;
                    }
                }
                restart++;
                if (// reached the leftmost expression
                restart == 0) {
                    cursor.left.generateOptimizedStringConcatenationCreation(blockScope, codeStream, cursor.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                }
                int pcAux;
                for (int i = restart; i < this.arity; i++) {
                    codeStream.recordPositionsFrom(pc, (cursor = this.referencesTable[i]).left.sourceStart);
