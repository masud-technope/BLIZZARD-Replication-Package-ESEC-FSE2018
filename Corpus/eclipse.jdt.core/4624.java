...
public abstract class HierarchyBuilder {
    ...
    public void connect(IGenericType type, IType typeHandle, IType superclassHandle, IType[] superinterfaceHandles) {
        ...
        if (TypeHierarchy.DEBUG) {
            ...
            System.out.println("  to superclass: " + (superclassHandle == null ? //$NON-NLS-1$
            "<None>" : ((JavaElement) superclassHandle).toStringWithAncestors()));
            //$NON-NLS-1$
            ...
            if (superinterfaceHandles == null || superinterfaceHandles.length == 0) {
                ...
                for (int i = 0, length = superinterfaceHandles.length; i < length; i++) {
                    if (superinterfaceHandles[i] == null)
                        continue;
                    System.out.println("    " + //$NON-NLS-1$
                    ((JavaElement) superinterfaceHandles[i]).toStringWithAncestors());
                }
            }
        }
        // now do the caching
    ...
    protected IType lookupBinaryHandle(IBinaryType typeInfo) {
        ...
        if (qualifiedName.equals(this.focusQualifiedName))
            return getType();
        NameLookup.Answer answer = this.nameLookup.findType(qualifiedName, false, flag, /* consider secondary types */
