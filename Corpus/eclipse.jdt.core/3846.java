...
public abstract class BaseConfigurationBlock {
    ...
    // updated in cacheOriginalValues
    ...
    public void performDefaults() {
        // not same as fLookupOrder!  Starts one layer deeper.
        IScopeContext[] lookupOrder;
        if (fProject != null) {
            lookupOrder = new IScopeContext[] { InstanceScope.INSTANCE, DefaultScope.INSTANCE };
        } else {
            lookupOrder = new IScopeContext[] { DefaultScope.INSTANCE };
        }
    ...
    protected boolean processChanges(IWorkbenchPreferenceContainer container) {
        ...
        // "NO" rebuild unless we put up the dialog.
        int response = 1;
        ...
        } else if (// "CANCEL" - no save, no rebuild.
        response != 1) {
            return false;
        }
        return true;
    ...
    private void testIfOptionsComplete(Key[] allKeys) {
        for (int i = 0; i < allKeys.length; i++) {
            if (allKeys[i].getStoredValue(fLookupOrder, false, fManager) == null) {
                //$NON-NLS-1$//$NON-NLS-2$
                JavaPlugin.logErrorMessage("preference option missing: " + allKeys[i] + " (" + this.getClass().getName() + ')');
            }
        }
    }
