...
class ExecutableUtil {
    ...
    static Collection<TypeParameterDeclaration> getFormalTypeParameters(EclipseDeclarationImpl executable, BaseProcessorEnv env) {
        ...
        if (executable.kind() != MirrorKind.METHOD && executable.kind() != MirrorKind.CONSTRUCTOR)
            ...
            "Executable is not a method " + executable.getClass().getName());
        if (executable.isFromSource()) {
            ...
            for (TypeParameter typeParam : typeParams) {
                ...
                if (typeBinding == null) {
                    ...
                    final TypeParameterDeclaration typeParamDecl = (TypeParameterDeclaration) Factory.createDeclaration(typeBinding, env);
        ...
        } else // binary
        {
            if (!executable.isBindingBased())
                ...
                "binary executable without binding.");
            ...
            for (ITypeBinding typeVar : typeParams) {
                final TypeParameterDeclaration typeParamDecl = (TypeParameterDeclaration) Factory.createDeclaration(typeVar, env);
                if (typeParamDecl != null)
                    result.add(typeParamDecl);
            }
            return result;
	 ...
	 */
    static Collection<ParameterDeclaration> getParameters(final EclipseDeclarationImpl executable, final BaseProcessorEnv env) {
        // that incorrectly includes them in the text
        if (executable == null || executable.kind() == MirrorKind.ANNOTATION_ELEMENT)
            return Collections.emptyList();
        if (executable.kind() != MirrorKind.METHOD && executable.kind() != MirrorKind.CONSTRUCTOR)
        ...
        if (executable.isFromSource()) {
            ...
            for (int i = 0, size = params.size(); i < size; i++) {
                ...
                final ParameterDeclaration param = Factory.createParameterDeclaration(varDecl, executable.getResource(), env);
                result.add(param);
            }
            return result;
        } else {
	 ...
	 */
    static Collection<ReferenceType> getThrownTypes(final EclipseDeclarationImpl executable, final BaseProcessorEnv env) {
        if (executable == null || executable.kind() == MirrorKind.ANNOTATION_ELEMENT)
            return Collections.emptyList();
        if (executable.kind() != MirrorKind.METHOD && executable.kind() != MirrorKind.CONSTRUCTOR)
            throw new IllegalArgumentException(//$NON-NLS-1$
