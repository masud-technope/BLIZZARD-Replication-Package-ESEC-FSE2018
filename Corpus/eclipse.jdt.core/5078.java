...
public class LoopingFlowContext extends SwitchFlowContext {
        ...
        for (int i = 0; i < assignCount; i++) {
            ...
            if (variable instanceof FieldBinding) {
                ...
                if (flowInfo.isPotentiallyAssigned((LocalVariableBinding) variable)) {
                    complained = true;
                    scope.problemReporter().duplicateInitializationOfFinalLocal((LocalVariableBinding) variable, finalAssignments[i]);
                }
            }
    ...
    public String individualToString() {
        ...
        //$NON-NLS-1$
        buffer.append("[initsOnBreak -").append(initsOnBreak.toString()).append(']');
        //$NON-NLS-1$
        buffer.append("[initsOnContinue -").append(initsOnContinue.toString()).append(']');
