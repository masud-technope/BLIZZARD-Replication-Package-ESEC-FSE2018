...
public class Label {
    ...
    // position=POS_NOT_SET Then it's pos is not set.
 ...
 */
        ...
        for (int i = codeStream.countLabels - 1; i >= 0; i--) {
			...
			*/
                ...
                // same target labels should be contiguous
                break;
            }
        }
    }
...
*/
    public void place() {
        //$NON-NLS-1$ //$NON-NLS-2$
        if (CodeStream.DEBUG)
            System.out.println("\t\t\t\t<place at: " + codeStream.position + " - " + this);
        if (position == POS_NOT_SET) {
 ...
 */
    public String toString() {
        ...
        for (int i = 0; i < forwardReferenceCount - 1; i++) //$NON-NLS-1$
