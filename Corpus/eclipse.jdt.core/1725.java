...
public class BuilderTests extends TestBase {
    ...
    public void disabled_testInheritedAnnotation() throws Throwable {
        ...
        IPath projPath = proj.getFullPath();
        IPath root = projPath.append("src");
        env.addClass(root, "", "Base", "import org.eclipse.jdt.apt.pluggable.tests.annotations.InheritedTrigger;\n" + "@InheritedTrigger(0)\n" + "public class Base {}");
        // Because Sub extends Base, it should be treated as if it were annotated with InheritedTrigger
        env.addClass(root, "", "Sub", "public class Sub extends Base {\n" + "}");
        AptConfig.setEnabled(jproj, true);
        fullBuild();
        expectingNoProblems();
    ...
    public void testBug295948() throws Throwable {
        ...
        IPath projPath = proj.getFullPath();
        IPath root = projPath.append("src");
        env.addClass(root, "test295948", "FooEvent", "package test295948;\n" + "public class FooEvent {\n" + "    public interface Handler {\n" + "        void handle(FooEvent event);\n" + "    }\n" + "}\n" + "\n");
        ...
        incrementalBuild();
        expectingProblemsFor(fooImplClass, "Problem : FooEvent cannot be resolved to a type [ resource : </" + _projectName + "/src/test295948/FooImpl.java> range : <108,116> category : <40> severity : <2>]\n" + "Problem : FooEvent cannot be resolved to a type [ resource : </" + _projectName + "/src/test295948/FooImpl.java> range : <52,60> category : <40> severity : <2>]");
    }
    ...
    public void testBug419769() throws Throwable {
        try {
            ...
            AptConfig.setEnabled(jproj, true);
            fullBuild();
            // There should be no compiler errors, i.e. except the APT injected ones.
            expectingNoProblems();
