...
public class SuperTypeNamesCollector {
    ...
    public class TypeDeclarationVisitor extends ASTVisitor {
        ...
        public boolean visit(FieldDeclaration fieldDeclaration, MethodScope scope) {
            // don't visit field declarations
            return false;
        }
    ...
    private boolean addIfSamePackage(char[][] compoundName, char[][] path) {
        ...
        for (int i = 0; i < resultLength; i++) // already known
 ...
 */
    protected CompilationUnitDeclaration buildBindings(ICompilationUnit compilationUnit, boolean isTopLevelOrMember) throws JavaModelException {
        ...
        CompilationUnitDeclaration unit = isTopLevelOrMember ? this.locator.basicParser().dietParse(sourceUnit, compilationResult) : this.locator.basicParser().parse(sourceUnit, compilationResult);
        if (unit != null) {
            this.locator.lookupEnvironment.buildTypeBindings(unit, /*no access restriction*/
            null);
        ...
        for (int i = 0, length = paths.length; i < length; i++) {
            try {
                ...
                if (openable instanceof ICompilationUnit) {
                    ...
                    CompilationUnitDeclaration parsedUnit = buildBindings(/*only top level and member types are visible to the focus type*/
                    unit, true);
                    if (parsedUnit != null)
 ...
 */
    ...
    protected String[] getPathsOfDeclaringType() {
        ...
        SearchPattern searchPattern = new TypeDeclarationPattern(// use the qualification only if no simple name
        this.typeSimpleName != null ? null : this.typeQualification, // do find member types
        null, this.typeSimpleName, IIndexConstants.TYPE_SUFFIX, this.pattern.getMatchRule());
        IndexQueryRequestor searchRequestor = new IndexQueryRequestor() {

            public boolean acceptIndexMatch(String documentPath, SearchPattern indexRecord, SearchParticipant participant, AccessRuleSet access) {
                TypeDeclarationPattern record = (TypeDeclarationPattern) indexRecord;
                if (// filter out local and anonymous classes
                record.enclosingTypeNames != IIndexConstants.ONE_ZERO_CHAR) {
        ...
        indexManager.performConcurrentJob(new PatternSearchJob(searchPattern, new JavaSearchParticipant(), scope, searchRequestor), IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, subMonitor.split(100));
