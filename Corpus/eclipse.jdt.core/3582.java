...
public class DiagnoseParser implements ParserBasicInformation, TerminalTokens {
    ...
    public void diagnoseParse() {
        ...
        do {
            ...
            for (int i = 0; i <= stateStackTop; i++) tempStack[i] = stack[i];
            ...
            //
            while (// SHIFT-REDUCE action or SHIFT action ?
            act > ERROR_ACTION || act < ACCEPT_ACTION) {
                ...
                while (act <= NUM_RULES) {
                    ...
                    do {
                        ...
                        tempStackTop -= (Parser.rhs[act] - 1);
                        act = (tempStackTop > next_pos ? tempStack[tempStackTop] : nextStack[tempStackTop]);
    ...
    private RepairCandidate errorRecovery(int error_token, boolean forcedError) {
        ...
        //
        int i;
        for (i = BUFF_UBOUND; lexStream.kind(buffer[i]) == EOFT_SYMBOL; /*empty*/
        i--) {
        }
        reportError(DELETION_CODE, //Parser.terminal_index[lexStream.kind(prevtok)],
        Parser.terminal_index[prevtokKind], error_token, buffer[i]);
        candidate.symbol = 0;
    ...
    private RepairCandidate primaryPhase(int error_token) {
        ...
        for (int j = i; j > 0; j--) buffer[j - 1] = lexStream.previous(buffer[j]);
        for (int k = i + 1; k < BUFF_SIZE; k++) buffer[k] = lexStream.next(buffer[k - 1]);
        //
        if (nextStackTop >= 0) {
        ...
        //
        if (repair.code == INVALID_CODE || repair.code == DELETION_CODE || repair.code == SUBSTITUTION_CODE || repair.code == MERGE_CODE) {
            repair.distance--;
        }
        //
    ...
    private PrimaryRepairInfo checkPrimaryDistance(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        if (scope_repair.distance > repair.distance)
            repair = scope_repair;
        //
        if (// do not merge the first token
        buffer[repair.bufferPosition] != 0 && buffer[repair.bufferPosition + 1] != 0) {
        ...
        //
        j = parseCheck(stck, stack_top, lexStream.kind(buffer[repair.bufferPosition + 1]), repair.bufferPosition + 2);
        if (lexStream.kind(buffer[repair.bufferPosition]) == EOLT_SYMBOL && lexStream.afterEol(buffer[repair.bufferPosition + 1])) {
            k = 10;
        } else {
            k = 0;
        }
        ...
        while (act <= NUM_RULES) {
            do {
                ...
                act = (tempStackTop > max_pos ? tempStack[tempStackTop] : stck[tempStackTop]);
        ...
        //
        symbol = root;
        if (// do not replace the first token
        buffer[repair.bufferPosition] != 0) {
            while (symbol != 0) {
    ...
    private RepairCandidate primaryDiagnosis(PrimaryRepairInfo repair) {
        ...
        switch(repair.code) {
            ...
            case BEFORE_CODE:
                {
                    if (repair.symbol > NT_OFFSET)
                        name_index = getNtermIndex(stack[stateStackTop], repair.symbol, repair.bufferPosition);
                    else
                        name_index = getTermIndex(stack, stateStackTop, repair.symbol, repair.bufferPosition);
            ...
            case INVALID_CODE:
                {
                    name_index = getNtermIndex(stack[stateStackTop], repair.symbol, repair.bufferPosition + 1);
            ...
            case SUBSTITUTION_CODE:
                {
                    if (repair.misspellIndex >= 6)
                        ...
                        name_index = getTermIndex(stack, stateStackTop, repair.symbol, repair.bufferPosition + 1);
            ...
            case MERGE_CODE:
                {
                    reportError(repair.code, Parser.terminal_index[repair.symbol], curtok, lexStream.next(curtok));
                    break;
                }
            case SCOPE_CODE:
                {
                    for (int i = 0; i < scopeStackTop; i++) {
                        reportError(repair.code, -scopeIndex[i], locationStack[scopePosition[i]], prevtok, Parser.non_terminal_index[Parser.scope_lhs[scopeIndex[i]]]);
                    }
                    repair.symbol = Parser.scope_lhs[scopeIndex[scopeStackTop]] + NT_OFFSET;
                    stateStackTop = scopePosition[scopeStackTop];
                    reportError(repair.code, -scopeIndex[scopeStackTop], locationStack[scopePosition[scopeStackTop]], prevtok, getNtermIndex(stack[stateStackTop], repair.symbol, repair.bufferPosition));
                    break;
                }
            // deletion
            default:
            ...
            default:
                {
                    ...
                    candidate.symbol = lexStream.kind(buffer[repair.bufferPosition + 1]);
                    lexStream.reset(buffer[repair.bufferPosition + 2]);
                    break;
                }
        }
        ...
        while (act <= NUM_RULES) {
            ...
            do {
                ...
                if (tempStackTop < threshold) {
                    return (highest_symbol > NT_OFFSET ? Parser.non_terminal_index[highest_symbol - NT_OFFSET] : Parser.terminal_index[highest_symbol]);
                }
                int lhs_symbol = Parser.lhs[act];
                if (tempStackTop == threshold)
    ...
    private int getNtermIndex(int start, int sym, int buffer_position) {
        int highest_symbol = sym - NT_OFFSET, tok = lexStream.kind(buffer[buffer_position]);
        lexStream.reset(buffer[buffer_position + 1]);
        //
        // Initialize stack index of temp_stack and initialize maximum
        // position of state stack that is still useful.
    ...
    private int misspell(int sym, int tok) {
        ...
        //
        if (n == 1 && m == 1) {
            if ((s1[0] == ';' && s2[0] == ',') || (s1[0] == ',' && s2[0] == ';') || (s1[0] == ';' && s2[0] == ':') || (s1[0] == ':' && s2[0] == ';') || (s1[0] == '.' && s2[0] == ',') || (s1[0] == ',' && s2[0] == '.') || (s1[0] == '\'' && s2[0] == '\"') || (s1[0] == '\"' && s2[0] == '\'')) {
                return 3;
            }
        }
        //
    ...
    private PrimaryRepairInfo scopeTrial(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        for (int i = 0; i < stackLength; i++) stateSeen[i] = NIL;
        statePoolTop = 0;
    ...
    private void scopeTrialCheck(int stck[], int stack_top, PrimaryRepairInfo repair, int indx) {
        ...
        for (int i = 0; i < SCOPE_SIZE; i++) {
            ...
            if (act != ERROR_ACTION) {
                ...
                for (j = tempStackTop + 1; j >= (max_pos + 1) && Parser.in_symbol(tempStack[j]) == Parser.scope_rhs[k]; j--) {
                    k++;
                }
                if (j == max_pos) {
                ...
                if (// match?
                Parser.scope_rhs[k] == 0 && j < marked_pos) {
                    int stack_position = j;
                    for (j = Parser.scope_state_set[i]; stck[stack_position] != Parser.scope_state[j] && Parser.scope_state[j] != 0; j++) /*empty*/
                    {
                    }
                    //
                    if (Parser.scope_state[j] != 0) {
                        ...
                        int distance = parseCheck(stck, stack_position, Parser.scope_lhs[i] + NT_OFFSET, repair.bufferPosition);
                        //
                        ...
                        if (lexStream.kind(buffer[repair.bufferPosition]) == EOFT_SYMBOL && repair.distance == previous_distance) {
                            scopeStackTop = indx;
    ...
    private boolean secondaryCheck(int stck[], int stack_top, int buffer_position, int distance) {
        ...
        for (top = stack_top - 1; top >= 0; top--) {
            j = parseCheck(stck, top, lexStream.kind(buffer[buffer_position]), buffer_position + 1);
    ...
    private RepairCandidate secondaryPhase(int error_token) {
        ...
        if (nextStackTop >= 0) {
            ...
            for (k = 3; k < BUFF_UBOUND; k++) buffer[k] = lexStream.next(buffer[k - 1]);
            // elmt not available
            buffer[BUFF_UBOUND] = lexStream.badtoken();
            //
            for (next_last_index = MAX_DISTANCE - 1; next_last_index >= 1 && lexStream.kind(buffer[next_last_index]) == EOFT_SYMBOL; next_last_index/*empty*/
            --) {
            }
            ...
            misplaced = misplacementRecovery(nextStack, nextStackTop, next_last_index, misplaced, true);
            if (misplaced.recoveryOnNextStack)
                misplaced.distance++;
            repair.numDeletions = nextStackTop + BUFF_UBOUND;
            repair = secondaryRecovery(nextStack, nextStackTop, next_last_index, repair, true);
        ...
        buffer[1] = lexStream.previous(buffer[2]);
        buffer[0] = lexStream.previous(buffer[1]);
        for (k = 4; k < BUFF_SIZE; k++) buffer[k] = lexStream.next(buffer[k - 1]);
        for (last_index = MAX_DISTANCE - 1; last_index >= 1 && lexStream.kind(buffer[last_index]) == EOFT_SYMBOL; last_index--) {
        }
        last_index++;
        ...
        if (misplaced.distance > MIN_DISTANCE) {
            if (misplaced.numDeletions <= repair.numDeletions || (misplaced.distance - misplaced.numDeletions) >= (repair.distance - repair.numDeletions)) {
        ...
        if (repair.recoveryOnNextStack) {
            stateStackTop = nextStackTop;
            for (i = 0; i <= stateStackTop; i++) stack[i] = nextStack[i];
            buffer[2] = error_token;
            buffer[1] = lexStream.previous(buffer[2]);
            buffer[0] = lexStream.previous(buffer[1]);
            for (k = 3; k < BUFF_UBOUND; k++) buffer[k] = lexStream.next(buffer[k - 1]);
        ...
        if (repair.code == SECONDARY_CODE || repair.code == DELETION_CODE) {
            ...
            for (scope_repair.bufferPosition = 2; scope_repair.bufferPosition <= repair.bufferPosition && repair.code != SCOPE_CODE; scope_repair.bufferPosition++) {
        ...
        if (repair.code == 0 && lexStream.kind(buffer[last_index]) == EOFT_SYMBOL) {
            ...
            scope_repair.distance = 0;
            for (top = stateStackTop; top >= 0 && repair.code == 0; top--) {
        ...
        switch(repair.code) {
            ...
            case DELETION_CODE:
                candidate.location = buffer[repair.bufferPosition];
                candidate.symbol = lexStream.kind(buffer[repair.bufferPosition]);
    ...
    private SecondaryRepairInfo secondaryRecovery(int stck[], int stack_top, int last_index, SecondaryRepairInfo repair, boolean stack_flag) {
        ...
        for (int top = stack_top; top >= 0 && repair.numDeletions >= stack_deletions; top--) {
            ...
            previous_loc = locationStack[top];
            for (int i = 2; i <= (last_index - MIN_DISTANCE + 1) && (repair.numDeletions >= (stack_deletions + i - 1)); i++) {
                ...
                if ((j - i + 1) > MIN_DISTANCE) {
                    ...
                    if ((k < repair.numDeletions) || (j - k) > (repair.distance - repair.numDeletions) || ((repair.code == SECONDARY_CODE) && (j - k) == (repair.distance - repair.numDeletions))) {
                        repair.code = DELETION_CODE;
                        repair.distance = j;
                        repair.stackPosition = top;
            ...
            case SCOPE_CODE:
                {
                    if (repair.stackPosition < stateStackTop) {
                        reportError(DELETION_CODE, Parser.terminal_index[ERROR_SYMBOL], locationStack[repair.stackPosition], buffer[1]);
                    }
                    for (int i = 0; i < scopeStackTop; i++) {
                        reportError(SCOPE_CODE, -scopeIndex[i], locationStack[scopePosition[i]], buffer[1], Parser.non_terminal_index[Parser.scope_lhs[scopeIndex[i]]]);
                    }
                    repair.symbol = Parser.scope_lhs[scopeIndex[scopeStackTop]] + NT_OFFSET;
                    stateStackTop = scopePosition[scopeStackTop];
                    reportError(SCOPE_CODE, -scopeIndex[scopeStackTop], locationStack[scopePosition[scopeStackTop]], buffer[1], getNtermIndex(stack[stateStackTop], repair.symbol, repair.bufferPosition));
                    break;
                }
            default:
                {
                    reportError(repair.code, (repair.code == SECONDARY_CODE ? getNtermIndex(stack[repair.stackPosition], repair.symbol, repair.bufferPosition) : Parser.terminal_index[ERROR_SYMBOL]), locationStack[repair.stackPosition], buffer[repair.bufferPosition - 1]);
    ...
    private int parseCheck(int stck[], int stack_top, int first_token, int buffer_position) {
        ...
        process_terminal: for (; ; ) {
            ...
            act < ACCEPT_ACTION || // shift-reduce action
            act > ERROR_ACTION) {
    ...
    private void reportPrimaryError(int msgCode, int nameIndex, int token, int scopeNameIndex) {
        ...
        switch(msgCode) {
            case BEFORE_CODE:
                problemReporter().parseErrorInsertBeforeToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
                break;
            case INSERTION_CODE:
                problemReporter().parseErrorInsertAfterToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
                break;
            case DELETION_CODE:
                problemReporter().parseErrorDeleteToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName);
                break;
            case INVALID_CODE:
                if (name.length() == 0) {
                    problemReporter().parseErrorReplaceToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
                } else {
                    problemReporter().parseErrorInvalidToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
                }
                break;
            case SUBSTITUTION_CODE:
                problemReporter().parseErrorReplaceToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
            ...
            case SCOPE_CODE:
                ...
                for (int i = Parser.scope_suffix[-nameIndex]; Parser.scope_rhs[i] != 0; i++) {
                    ...
                    if (// any more symbols to print?
                    Parser.scope_rhs[i + 1] != 0)
                        buf.append(' ');
                }
                if (scopeNameIndex != 0) {
            ...
            default:
                if (name.length() == 0) {
                    problemReporter().parseErrorNoSuggestion(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName);
                } else {
                    problemReporter().parseErrorReplaceToken(errorStart, errorEnd, currentKind, errorTokenSource, errorTokenName, name);
                }
                break;
