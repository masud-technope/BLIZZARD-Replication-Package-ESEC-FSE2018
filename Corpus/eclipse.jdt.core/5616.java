...
public class NullAnnotationMatching {
    ...
    public enum CheckMode implements  {
    ...
    private enum Severity implements  {
    ...
    public static int checkAssignment(BlockScope currentScope, FlowContext flowContext, VariableBinding var, FlowInfo flowInfo, int nullStatus, Expression expression, TypeBinding providedType) {
        // assume we already reported an error
        ...
        if (!usesNullTypeAnnotations) {
            ...
            if (expression instanceof ConditionalExpression && expression.isPolyExpression()) {
                ...
                if (status1 == status2)
                    return status1;
                // if both branches disagree use the precomputed & merged nullStatus
                return nullStatus;
            }
        ...
        } else if (// provided a legacy type?
        lhsTagBits == TagBits.AnnotationNullable && nullStatus == FlowInfo.UNKNOWN) {
            if (usesNullTypeAnnotations && providedType.isTypeVariable() && (providedType.tagBits & TagBits.AnnotationNullMASK) == 0)
                // -> free type variable can mean either nullable or nonnull
                return FlowInfo.POTENTIALLY_NULL | FlowInfo.POTENTIALLY_NON_NULL;
            // -> combine info from lhs & rhs
        ...
        try {
            ...
            if (mode == CheckMode.BOUND_CHECK && requiredType instanceof TypeVariableBinding) {
                ...
                if (!passedBoundCheck) {
                    ...
                    if (// annotations may enter when substituting a nested type variable
                    superClass != null && (superClass.hasNullTypeAnnotations() || substitution != null)) {
                    ...
                    if (superInterfaces != null) {
                        for (int i = 0; i < superInterfaces.length; i++) {
                            if (// annotations may enter when substituting a nested type variable
                            superInterfaces[i].hasNullTypeAnnotations() || substitution != null) {
            ...
            if (requiredType instanceof ArrayBinding) {
                ...
                if (requiredDimsTagBits != null) {
                    ...
                    if (requiredType.dimensions() == providedType.dimensions()) {
                        ...
                        if (providedDimsTagBits == null)
                            // set to unspec'd at all dimensions
                        ...
                        for (int i = 0; i <= dims; i++) {
                            ...
                            if (i == 0 && requiredBits == TagBits.AnnotationNullable && nullStatus != -1 && mode.requiredNullableMatchesAll()) {
                                ...
                                if (nullStatus == FlowInfo.NULL)
                                    // null value has no details
                                    break;
                            } else {
                                if (i > 0)
                                    // don't use beyond the outermost dimension
                                    currentNullStatus = -1;
                                Severity dimSeverity = computeNullProblemSeverity(requiredBits, providedBits, currentNullStatus, i == 0 ? mode : mode.toDetail(), false);
                                if (i > 0 && dimSeverity == Severity.UNCHECKED && providedExpression instanceof ArrayAllocationExpression && providedBits == 0 && requiredBits != 0) {
                                    ...
                                    if (previousDim instanceof IntLiteral && previousDim.constant.intValue() == 0) {
                                        // element of empty dimension matches anything
                ...
                if (// null value has no details
                severity != Severity.MISMATCH && nullStatus != FlowInfo.NULL) {
                    ...
                    severity == Severity.UNCHECKED && requiredType.isTypeVariable() && providedType.isTypeVariable() && (providedSuper == requiredType || providedSubstituteSuper == requiredType)) {
                        severity = Severity.OK;
                    }
                    if (//$IDENTITY-COMPARISON$
                    providedSuper != providedType)
                        superTypeHint = providedSuper;
                    if (// TODO(stephan): handle providedType.isRaw()
                    requiredType.isParameterizedType() && providedSuper instanceof ParameterizedTypeBinding) {
    ...
    public static NullAnnotationMatching okNonNullStatus(final Expression providedExpression) {
        if (providedExpression instanceof MessageSend) {
            ...
            if (method != null && method.isValidBinding()) {
                ...
                if (originalDeclaringClass instanceof BinaryTypeBinding && ((BinaryTypeBinding) originalDeclaringClass).externalAnnotationStatus.isPotentiallyUnannotatedLib() && originalMethod.returnType.isTypeVariable() && (originalMethod.returnType.tagBits & TagBits.AnnotationNullMASK) == 0) {
                    final int severity = ((BinaryTypeBinding) originalDeclaringClass).externalAnnotationStatus == ExternalAnnotationStatus.NO_EEA_FILE ? ProblemSeverities.Warning : // reduce severity if not configured to for external annotations
    ...
    protected static boolean areSameTypes(TypeBinding requiredType, TypeBinding providedType, TypeBinding providedSubstitute) {
        if (//$IDENTITY-COMPARISON$ // short cut for really-really-same types
        requiredType == providedType)
            return true;
        if (requiredType.isParameterizedType() || requiredType.isArrayType())
            // not analysing details here
            return false;
        if (TypeBinding.notEquals(requiredType, providedType)) {
            ...
            } else if (//$IDENTITY-COMPARISON$
            requiredType.kind() == Binding.TYPE_PARAMETER && requiredType == providedSubstitute) {
                return true;
            ...
            if (type.isCapture()) {
                ...
                if (lowerBound != null) {
                    ...
                    if (tagBits == TagBits.AnnotationNullable)
                        // type cannot require @NonNull
                        return TagBits.AnnotationNullable;
                }
            }
            switch(mode) {
                // no pessimistic checks during boundcheck (we *have* the instantiation)
                case BOUND_CHECK:
                case BOUND_SUPER_CHECK:
                // no pessimistic checks during override check (comparing two *declarations*)
                case OVERRIDE:
                case OVERRIDE_RETURN:
                    break;
                default:
                    // instantiation could require @NonNull
    ...
    static long providedNullTagBits(TypeBinding type) {
        ...
        if (tagBits != 0)
            return validNullTagBits(tagBits);
        if (// wildcard can be 'provided' during inheritance checks
        type.isWildcard()) {
            return TagBits.AnnotationNullMASK;
        }
        if (// incl. captures
        type.isTypeVariable()) {
            ...
            if (typeVariable.isCapture()) {
                ...
                if (lowerBound != null) {
                    tagBits = lowerBound.tagBits & TagBits.AnnotationNullMASK;
                    if (tagBits == TagBits.AnnotationNullable)
                        return // cannot be @NonNull
                        TagBits.AnnotationNullable;
            ...
            if (haveNullBits)
                // could be either, can only match to a wildcard accepting both
                return TagBits.AnnotationNullMASK;
        }
        return 0;
    }
    ...
    public static TypeBinding moreDangerousType(TypeBinding one, TypeBinding two) {
        ...
        if (oneNullBits != twoNullBits) {
            ...
            if (oneNullBits == 0)
                // unknown is more dangerous than nonnull
                return one;
            // unknown is more dangerous than nonnull
            return two;
        } else if (//$IDENTITY-COMPARISON$
        one != two) {
            if (analyse(one, two, -1).isAnyMismatch())
                // two doesn't snugly fit into one, so it must be more dangerous
                return two;
        }
        return one;
    }
    ...
    private static Severity computeNullProblemSeverity(long requiredBits, long providedBits, int nullStatus, CheckMode mode, boolean requiredIsTypeVariable) {
        ...
        if (requiredBits == 0) {
            switch(mode) {
                ...
                case OVERRIDE_RETURN:
                    ...
                    if (!requiredIsTypeVariable)
                        // refining an unconstrained non-TVB return to nullable is also legal
                        return Severity.OK;
                    return Severity.UNCHECKED;
                case OVERRIDE:
                    // warn about dropped annotation
                    return Severity.UNCHECKED;
            }
        } else if (requiredBits == TagBits.AnnotationNullMASK) {
            // OK since LHS accepts either
            return Severity.OK;
            ...
            switch(mode) {
                ...
                case OVERRIDE_RETURN:
                case BOUND_SUPER_CHECK:
                    // in these modes everything is compatible to nullable
                    return Severity.OK;
                case BOUND_CHECK:
                case EXACT:
                    if (providedBits == 0)
        ...
        // shouldn't get here, requiredBits should be one of the listed cases
        return Severity.OK;
    }
    ...
    static class SearchContradictions extends TypeBindingVisitor {
        ...
        public boolean visit(TypeVariableBinding typeVariable) {
            ...
            for (TypeBinding otherBound : typeVariable.otherUpperBounds()) allNullBits |= otherBound.tagBits & TagBits.AnnotationNullMASK;
    ...
    public static TypeBinding strongerType(TypeBinding type1, TypeBinding type2, LookupEnvironment environment) {
        ...
        // don't bother to distinguish unannotated vs. @Nullable, since both can accept null
    ...
    public static TypeBinding[] weakerTypes(TypeBinding[] parameters1, TypeBinding[] parameters2, LookupEnvironment environment) {
        ...
        for (int i = 0; i < newParameters.length; i++) {
            ...
            if ((tagBits1 & TagBits.AnnotationNullable) != 0)
                ...
                // unannotated must be preserved
                newParameters[i] = mergeTypeAnnotations(parameters1[i], parameters2[i], true, environment);
            else
                // either unannotated, or both are @NonNull
                newParameters[i] = mergeTypeAnnotations(parameters2[i], parameters1[i], true, environment);
        }
