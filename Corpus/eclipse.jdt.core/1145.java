...
public class CastExpression extends Expression {
    ...
    public static void checkNeedForEnclosingInstanceCast(BlockScope scope, Expression enclosingInstance, TypeBinding enclosingInstanceType, TypeBinding memberType) {
        if (scope.environment().options.getSeverity(CompilerOptions.UnnecessaryTypeCheck) == ProblemSeverities.Ignore)
            return;
    ...
    public static void checkNeedForArgumentCast(BlockScope scope, int operator, int operatorSignature, Expression expression, int expressionTypeId) {
        ...
        if ((expression.bits & UnnecessaryCastMask) == 0 && expression.resolvedType.isBaseType()) {
            ...
            (alternateLeftTypeId = alternateLeftType.id) == expressionTypeId) {
...
*/
        ...
        if (leftIsCast) {
            if ((left.bits & UnnecessaryCastMask) == 0 && left.resolvedType.isBaseType()) {
                ...
                if (alternateLeftType == null)
                    return;
                if (// obvious identity cast
                (alternateLeftTypeId = alternateLeftType.id) == leftTypeId) {
        ...
        if (rightIsCast) {
            if ((right.bits & UnnecessaryCastMask) == 0 && right.resolvedType.isBaseType()) {
                ...
                if (alternateRightType == null)
                    return;
                if (// obvious identity cast
                (alternateRightTypeId = alternateRightType.id) == rightTypeId) {
    ...
    public boolean checkUnsafeCast(Scope scope, TypeBinding castType, TypeBinding expressionType, TypeBinding match, boolean isNarrowing) {
        ...
        if (castType.isBoundParameterizedType() || castType.isGenericType()) {
            ...
            if ((castType.tagBits & TagBits.HasWildcard) == 0) {
                if ((!match.isParameterizedType() && !match.isGenericType()) || expressionType.isRawType()) {
                    scope.problemReporter().unsafeCast(this);
                    return true;
                }
            }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if ((type instanceof TypeReference) || (type instanceof NameReference) && // no extra parenthesis around type: ((A))exp
        ((type.bits & ASTNode.ParenthesizedMASK) >> ASTNode.ParenthesizedSHIFT) == 0) {
            ...
            if (this.resolvedType != null && expressionType != null) {
                ...
                if ((this.bits & UnnecessaryCastMask) != 0) {
                    if ((this.bits & IgnoreNeedForCastCheckMASK) == 0) {
                        if (// used for generic type inference ?
                        !usedForGenericMethodReturnTypeInference())
