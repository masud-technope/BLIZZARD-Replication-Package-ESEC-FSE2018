 ...
 */
    public static void generateClassFile(String className, String javaSource) throws IOException {
        ...
        BatchCompiler.compile(cu + " -d d:/temp -classpath " + System.getProperty("java.home") + "/lib/rt.jar", new PrintWriter(System.out), new PrintWriter(System.err), null);
 ...
 */
    ...
    protected IPackageFragment getPackage(String path) {
        if (// if path as more than one segment
        path.indexOf('/', 1) != -1) {
    ...
    public IClasspathEntry createSourceEntry(String referingProjectName, String src, String patterns) {
        ...
        for (int j = 0; j < ruleCount; j++) {
            ...
            switch(rule.charAt(0)) {
                ...
                case '-':
                default:
                    // TODO (maxime) consider forbidding unspecified rule start; this one tolerates
        ...
        if (referingProjectName != null && folderPath.segmentCount() == 1 && !referingProjectName.equals(folderPath.lastSegment())) {
            return JavaCore.newProjectEntry(folderPath, accessRules, /*combine access restrictions*/
            true, new IClasspathAttribute[0], false);
        } else {
