...
public class ReferenceCollection {
    ...
    public void addDependencies(String[] typeNameDependencies) {
        ...
        for (int i = typeNameDependencies.length; --i >= 0; ) qNames[i] = CharOperation.splitOn('.', typeNameDependencies[i].toCharArray());
    ...
    public boolean includes(char[][] qualifiedName) {
        for (int i = 0, l = this.qualifiedNameReferences.length; i < l; i++) if (qualifiedName == this.qualifiedNameReferences[i])
            return true;
        return false;
    }
 ...
 */
    ...
    public boolean includes(char[][][] qualifiedNames, char[][] simpleNames, char[][] rootNames) {
        ...
        if (rootNames != null) {
            ...
            for (int i = 0, l = rootNames.length; !foundRoot && i < l; i++) foundRoot = insideRoot(rootNames[i]);
            if (!foundRoot)
                return false;
        }
        if (simpleNames == null || qualifiedNames == null) {
            if (simpleNames == null && qualifiedNames == null) {
                if (JavaBuilder.DEBUG)
                    ...
                    "Found well known match");
                return true;
            } else if (qualifiedNames == null) {
                for (int i = 0, l = simpleNames.length; i < l; i++) {
                    if (includes(simpleNames[i])) {
                        if (JavaBuilder.DEBUG)
                            System.out.println("Found match in well known package to " + new //$NON-NLS-1$
                            String(//$NON-NLS-1$
                            simpleNames[i]));
                        return true;
                    }
                ...
                for (int i = 0, l = qualifiedNames.length; i < l; i++) {
                    ...
                    if (qualifiedName.length == 1 ? includes(qualifiedName[0]) : includes(qualifiedName)) {
                        if (JavaBuilder.DEBUG)
                            System.out.println("Found well known match in " + //$NON-NLS-1$
                            CharOperation.toString(//$NON-NLS-1$
                            qualifiedName));
                        return true;
                    }
                }
            ...
            for (int i = 0; i < sLength; i++) {
                if (includes(simpleNames[i])) {
                    for (int j = 0; j < qLength; j++) {
                        ...
                        if (qualifiedName.length == 1 ? includes(qualifiedName[0]) : includes(qualifiedName)) {
                            if (JavaBuilder.DEBUG)
                                System.out.println("Found match in " + //$NON-NLS-1$
                                CharOperation.toString(//$NON-NLS-1$
                                qualifiedName) + //$NON-NLS-1$
                                " to " + //$NON-NLS-1$
                                new String(simpleNames[i]));
                            return true;
                        }
                    }
                    return false;
    ...
    static final char[][][] WellKnownQualifiedNames = new char[][][] { TypeConstants.JAVA_LANG_RUNTIMEEXCEPTION, TypeConstants.JAVA_LANG_THROWABLE, TypeConstants.JAVA_LANG_OBJECT, TypeConstants.JAVA_LANG, new char[][] { TypeConstants.JAVA }, new char[][] { new char[] { 'o', 'r', 'g' } }, new char[][] { new char[] { 'c', 'o', 'm' } }, // default package
    CharOperation.NO_CHAR_CHAR };

    static final char[][] WellKnownSimpleNames = new char[][] { TypeConstants.JAVA_LANG_RUNTIMEEXCEPTION[2], TypeConstants.JAVA_LANG_THROWABLE[2], TypeConstants.JAVA_LANG_OBJECT[2], TypeConstants.JAVA, TypeConstants.LANG, new char[] { 'o', 'r', 'g' }, new char[] { 'c', 'o', 'm' } };
    ...
    public static char[][][] internQualifiedNames(StringSet qualifiedStrings) {
        ...
        for (int i = 0, l = strings.length; i < l; i++) if (strings[i] != null)
