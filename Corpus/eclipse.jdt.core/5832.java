...
public class SourceElementParser extends CommentRecorderParser {
    ...
    public  SourceElementParser(final ISourceElementRequestor requestor, IProblemFactory problemFactory, CompilerOptions options, boolean reportLocalDeclarations, boolean optimizeStringLiterals) {
    ...
    protected void consumeAnnotationAsModifier() {
        ...
        if (// accept annotation type reference
 ...
 */
	 ...
	 */
        ...
        if (//flag for precompiled type reference on base types
        length < 0) {
    ...
    public NameReference getUnspecifiedReference(boolean rejectTypeAnnotations) {
        ...
        if ((length = this.identifierLengthStack[this.identifierLengthPtr--]) == 1) {
            ...
            SingleNameReference ref = newSingleNameReference(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr--]);
            if (this.reportReferenceInfo) {
                addUnknownRef(ref);
            }
            return ref;
            ...
            long[] positions = new long[length];
            System.arraycopy(this.identifierPositionStack, this.identifierPtr + 1, positions, 0, length);
            QualifiedNameReference ref = newQualifiedNameReference(tokens, positions, // sourceStart
            (int) (this.identifierPositionStack[this.identifierPtr + 1] >> 32), (int) // sourceEnd
            this.identifierPositionStack[// sourceEnd
            this.identifierPtr + length]);
            if (this.reportReferenceInfo) {
                addUnknownRef(ref);
            }
	...
	look for that it is not a type reference */
        ...
        long[] positions = new long[length];
        System.arraycopy(this.identifierPositionStack, this.identifierPtr + 1, positions, 0, length);
        QualifiedNameReference ref = newQualifiedNameReference(tokens, positions, (int) (this.identifierPositionStack[this.identifierPtr + 1] >> 32), // sourceStart
        (int) // sourceEnd
        this.identifierPositionStack[this.identifierPtr + length]);
        ref.bits &= ~ASTNode.RestrictiveFlagMASK;
        ref.bits |= Binding.LOCAL | Binding.FIELD;
        if (this.reportReferenceInfo) {
            addUnknownRef(ref);
