...
public final class BinaryTypeBinding extends ReferenceBinding {
    ...
    public static ReferenceBinding resolveType(ReferenceBinding type, LookupEnvironment environment, boolean convertGenericToRawType) {
        ...
        convertGenericToRawType && type.isGenericType())
    ...
    public static TypeBinding resolveType(TypeBinding type, LookupEnvironment environment, ParameterizedTypeBinding parameterizedType, int rank) {
        ...
        if (// raw reference to generic ?
        parameterizedType == null && type.isGenericType())
    ...
    static ReferenceBinding resolveUnresolvedType(ReferenceBinding type, LookupEnvironment environment, boolean convertGenericToRawType) {
        ...
        if (type.isParameterizedType())
            ...
            resolveUnresolvedType(((ParameterizedTypeBinding) type).type, environment, false);
    ...
    void cachePartsFrom(IBinaryType binaryType, boolean needFieldsAndMethods) {
        ...
        if (typeSignature == null) {
            ...
            if (interfaceNames != null) {
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) // attempt to find each superinterface if it exists in the cache (otherwise - resolve it when requested)
        ...
        if (memberTypeStructures != null) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) // attempt to find each member type if it exists in the cache (otherwise - resolve it when requested)
        ...
        if (iFields != null) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    TypeBinding type = fieldSignature == null ? environment.getTypeFromSignature(field.getTypeName(), 0, -1, false, this) : environment.getTypeFromTypeSignature(new SignatureWrapper(fieldSignature), NoTypeVariables, this);
                    this.fields[i] = new FieldBinding(field.getName(), type, field.getModifiers() | AccUnresolved, this, field.getConstant());
                }
            }
        }
    }
        ...
        methodSignature == null) {
            ...
            while ((nextChar = methodDescriptor[++index]) != ')') {
                if (nextChar != '[') {
                    numOfParams++;
                    if (nextChar == 'L')
                        while ((nextChar = methodDescriptor[/*empty*/
                        ++index]) != ';') {
                        }
                }
            }
            ...
            if (!method.isConstructor())
                ...
                returnType = environment.getTypeFromSignature(methodDescriptor, index + 1, -1, false, this);
        } else {
            ...
            if (wrapper.signature[wrapper.start] == '(') {
                ...
                if (wrapper.signature[wrapper.start] == ')') {
                    ...
                    int startIndex = (method.isConstructor() && isMemberType() && !isStatic()) ? 1 : 0;
                    if (startIndex == 1)
                        // skip synthetic argument
                        environment.getTypeFromTypeSignature(wrapper, typeVars, this);
                    while (wrapper.signature[wrapper.start] != ')') {
 ...
 */
    private void createMethods(IBinaryMethod[] iMethods, boolean checkGenericSignatures) {
        ...
        if (total == initialTotal) {
            for (int i = 0; i < initialTotal; i++) this.methods[i] = createMethod(iMethods[i], checkGenericSignatures);
        } else {
            for (int i = 0, index = 0; i < initialTotal; i++) if (iClinit != i && (toSkip == null || toSkip[i] != -1))
...
*/
    ...
    public FieldBinding[] fields() {
        for (int i = fields.length; --i >= 0; ) resolveTypeFor(fields[i]);
        return fields;
    }
            ...
            if (memberType instanceof UnresolvedReferenceBinding) {
                ...
                name.length == (prefixLength + typeName.length))
                    ...
                    CharOperation.fragmentEquals(typeName, name, prefixLength, true))
                        // no raw conversion for now
                        return this.memberTypes[i] = resolveType(memberType, this.environment, false);
 ...
 */
    ...
    public ReferenceBinding[] memberTypes() {
        ...
        for (int i = this.memberTypes.length; --i >= 0; ) // no raw conversion for now
        this.memberTypes[i] = resolveUnresolvedType(this.memberTypes[i], this.environment, false);
        this.tagBits ^= HasUnresolvedMemberTypes;
        for (int i = this.memberTypes.length; --i >= 0; ) // no raw conversion for now
        this.memberTypes[i] = resolveType(this.memberTypes[i], this.environment, false);
        return this.memberTypes;
    }
    ...
    public MethodBinding[] methods() {
        if ((modifiers & AccUnresolved) == 0)
            return methods;
        for (int i = methods.length; --i >= 0; ) resolveTypesFor(methods[i]);
        modifiers ^= AccUnresolved;
        return methods;
    }
    ...
    MethodBinding resolveTypesFor(MethodBinding method) {
        ...
        for (int i = method.parameters.length; --i >= 0; ) method.parameters[i] = resolveType(method.parameters[i], this.environment, null, 0);
        for (int i = method.thrownExceptions.length; --i >= 0; ) method.thrownExceptions[i] = resolveType(method.thrownExceptions[i], this.environment, true);
        for (int i = method.typeVariables.length; --i >= 0; ) resolveTypesFor(method.typeVariables[i]);
    ...
    private TypeVariableBinding resolveTypesFor(TypeVariableBinding variable) {
        ...
        for (int i = interfaces.length; --i >= 0; ) interfaces[i] = resolveUnresolvedType(interfaces[i], this.environment, true);
        variable.modifiers ^= AccUnresolved;
        ...
        for (int i = interfaces.length; --i >= 0; ) interfaces[i] = resolveType(interfaces[i], this.environment, true);
        return variable;
    }
...
*/
    ...
    public ReferenceBinding[] superInterfaces() {
        ...
        this.tagBits ^= HasUnresolvedSuperinterfaces;
        for (int i = this.superInterfaces.length; --i >= 0; ) this.superInterfaces[i] = resolveType(this.superInterfaces[i], this.environment, true);
        return this.superInterfaces;
    }
    ...
    public TypeVariableBinding[] typeVariables() {
        ...
        for (int i = this.typeVariables.length; --i >= 0; ) resolveTypesFor(this.typeVariables[i]);
        this.tagBits ^= HasUnresolvedTypeVariables;
        return this.typeVariables;
    }
    ...
    public String toString() {
        ...
        if (fields != null) {
            if (fields != NoFields) {
                //$NON-NLS-1$
                s += "\n/*   fields   */";
                for (int i = 0, length = fields.length; i < length; i++) //$NON-NLS-1$ //$NON-NLS-2$
                s += (fields[i] != null) ? "\n" + fields[i].toString() : "\nNULL FIELD";
            }
        } else {
            //$NON-NLS-1$
