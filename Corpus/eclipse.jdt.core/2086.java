...
public class ToolFactory {
            ...
            for (int i = 0; i < extensions.length; i++) {
                ...
                for (int j = 0; j < configElements.length; j++) {
                    try {
                        ...
                        configElements[j].createExecutableExtension(//$NON-NLS-1$
                        "class");
    ...
    public static CodeFormatter createCodeFormatter(Map options, int mode) {
        ...
        if (formatterId != null) {
            IExtensionPoint extension = Platform.getExtensionRegistry().getExtensionPoint(JavaCore.PLUGIN_ID, JavaCore.JAVA_FORMATTER_EXTENSION_POINT_ID);
            if (extension != null) {
                IExtension[] extensions = extension.getExtensions();
                for (int i = 0; i < extensions.length; i++) {
                    IConfigurationElement[] configElements = extensions[i].getConfigurationElements();
            ...
            org.eclipse.jdt.internal.core.util.Util.log(IStatus.WARNING, //$NON-NLS-1$ //$NON-NLS-2$
            "Unable to instantiate formatter extension '" + formatterId + "', returning built-in formatter.");
        }
    ...
    public static IScanner createScanner(boolean tokenizeComments, boolean tokenizeWhiteSpace, boolean assertMode, boolean recordLineSeparator) {
        ...
        if (complianceLevelValue == 0)
            complianceLevelValue = ClassFileConstants.JDK1_4;
        PublicScanner scanner = new PublicScanner(tokenizeComments, tokenizeWhiteSpace, /*nls*/
        false, assertMode ? ClassFileConstants.JDK1_4 : ClassFileConstants.JDK1_3, complianceLevelValue, /*taskTags*/
        null, /*taskPriorities*/
    ...
    public static IScanner createScanner(boolean tokenizeComments, boolean tokenizeWhiteSpace, boolean recordLineSeparator, String sourceLevel) {
        ...
        if (sourceLevelValue == 0)
            sourceLevelValue = ClassFileConstants.JDK1_3;
        PublicScanner scanner = new PublicScanner(tokenizeComments, tokenizeWhiteSpace, /*nls*/
        false, sourceLevelValue, /*sourceLevel*/
        complianceLevelValue, /*taskTags*/
        null, /*taskPriorities*/
        null, /*taskCaseSensitive*/
        true);
        scanner.recordLineSeparator = recordLineSeparator;
    ...
    public static IScanner createScanner(boolean tokenizeComments, boolean tokenizeWhiteSpace, boolean recordLineSeparator, String sourceLevel, String complianceLevel) {
        ...
        if (complianceLevelValue == 0)
            complianceLevelValue = ClassFileConstants.JDK1_4;
        scanner = new PublicScanner(tokenizeComments, tokenizeWhiteSpace, /*nls*/
        false, sourceLevelValue, /*sourceLevel*/
        complianceLevelValue, /*taskTags*/
        null, /*taskPriorities*/
        null, /*taskCaseSensitive*/
