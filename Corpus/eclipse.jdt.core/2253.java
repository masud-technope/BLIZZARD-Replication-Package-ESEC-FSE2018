 ...
 */
    public ICompilationUnit[] getCompilationUnits(WorkingCopyOwner owner) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (// 59933 - excluded wc shouldn't be answered back
            equals(wc.getParent()) && !Util.isExcluded(wc)) {
                result[index++] = wc;
            }
        }
        if (index != length) {
 ...
 */
    public IResource resource(PackageFragmentRoot root) {
        ...
        if (length == 0) {
            ...
            for (int i = 1; i < length; i++) path = path.append(this.names[i]);
 ...
 */
    ...
    public int hashCode() {
        ...
        for (int i = 0, length = this.names.length; i < length; i++) hash = Util.combineHashCodes(this.names[i].hashCode(), hash);
        return hash;
    }
 ...
 */
    public boolean hasSubpackages() throws JavaModelException {
        ...
        nextPackage: for (int i = 0, length = packages.length; i < length; i++) {
            ...
            for (int j = 0; j < namesLength; j++) if (!this.names[j].equals(otherNames[j]))
                continue nextPackage;
            return true;
        }
        return false;
