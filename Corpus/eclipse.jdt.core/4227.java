...
public class ClasspathUtil {
    ...
    public static IClasspathEntry findProjectSourcePath(IJavaProject jp, IFolder folder) throws JavaModelException {
        ...
        IClasspathEntry searchingFor = JavaCore.newSourceEntry(folder.getFullPath());
	 ...
	 */
    public static boolean updateProjectClasspath(IJavaProject jp, IFolder folder, IProgressMonitor progressMonitor) throws JavaModelException {
        IClasspathEntry[] cp = jp.getRawClasspath();
        ...
        if (!found) {
            ...
            for (int i = 0; i < cp.length; i++) {
                if (cp[i].getPath().isPrefixOf(path)) {
                    ...
                    boolean add = true;
                    for (int j = 0; j < oldExclusions.length; j++) if (oldExclusions[j].equals(projectRelativePath))
                        add = false;
                    if (add) {
                        IPath[] newExclusions;
            ...
            IClasspathEntry generatedSourceClasspathEntry = JavaCore.newSourceEntry(folder.getFullPath(), new IPath[] {}, exclusionPatterns, null, attrs);
