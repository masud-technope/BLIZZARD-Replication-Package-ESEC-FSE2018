...
public class InternalNamingConventions {
    ...
    private static Scanner getNameScanner(CompilerOptions compilerOptions) {
        return new Scanner(/*comment*/
        false, /*whitespace*/
        false, /*nls*/
        false, compilerOptions.sourceLevel, /*taskTags*/
        null, /*taskPriorities*/
    ...
    private static char[][] computeNonBaseTypeNames(char[] sourceName, boolean isConstantField, boolean onlyLongest) {
        ...
        int previousCharKind = ScannerHelper.isLowerCase(c) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;
        for (int i = length - 1; i >= 0; i--) {
            c = sourceName[i];
            int charKind = ScannerHelper.isLowerCase(c) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;
            switch(charKind) {
                case IS_LOWER_CASE:
                ...
                case IS_UPPER_CASE:
                    if (previousCharKind == IS_LOWER_CASE) {
                        ...
                        if (i > 0) {
                            ...
                            previousCharKind = ScannerHelper.isLowerCase(pc) ? IS_LOWER_CASE : ScannerHelper.isUpperCase(pc) ? IS_UPPER_CASE : pc == '_' ? IS_UNDERSCORE : IS_OTHER;
                        }
                        endIndex = i;
                    } else {
        ...
        for (int i = 0; i < tempNames.length; i++) {
            ...
            if (!isConstantField) {
                unprefixedName[0] = ScannerHelper.toUpperCase(unprefixedName[0]);
                    ...
                    if (j == internalPrefix.length || CharOperation.prefixEquals(CharOperation.subarray(internalPrefix, j, -1), /*do not check case when there is no prefix*/
                    unprefixedName, j != 0)) {
                        matchingIndex = j;
                        break done;
                    }
                    ...
                    if (j == internalPrefix.length) {
                        matchingIndex = j;
                        break done;
                    } else if (CharOperation.prefixEquals(CharOperation.subarray(internalPrefix, j, -1), unprefixedName, /*do not check case when there is no prefix*/
                    j != 0)) {
            ...
            if (matchingIndex > -1) {
                ...
                for (int k = 0; k < prefixes.length; k++) {
                    if (!isConstantField) {
                        if (prefixes[k].length > 0 && ScannerHelper.isLetterOrDigit(prefixes[k][prefixes[k].length - 1])) {
