...
public class BindingKeyParser {
    ...
    static class Scanner {
        ...
        boolean isAtWildcardStart() {
            return this.index < this.source.length && // e.g {1}+Ljava/lang/String;
        ...
        int nextToken() {
            ...
            while (this.index <= length) {
                ...
                switch(currentChar) {
                    ...
                    case 'Z':
                        // base type
                        if (this.index == previousTokenEnd && // case of field or method starting with one of the character above
                    ...
                    case '@':
                        ...
                        break;
                    case '[':
                        while (this.index < length && this.source[this.index] == '[') this.index++;
                        this.token = ARRAY;
                        return this.token;
                    case '<':
                        if (this.start > 0) {
        ...
        void skipRank() {
            this.start = this.index;
            while (//$NON-NLS-1$
            this.index < this.source.length && "0123456789".indexOf(this.source[this.index]) != -1) this.index++;
        }

        void skipThrownStart() {
            while (this.index < this.source.length && this.source[this.index] == C_THROWN) this.index++;
        }
    ...
    public void parse() {
        parse(/*don't pause after fully qualified name*/
        false);
    }
    ...
    public void parse(boolean pauseAfterFullyQualifiedName) {
        ...
        if (this.scanner.isAtParametersStart()) {
            ...
            if (this.scanner.isAtTypeParameterStart()) {
                ...
                // parameterized type
                parseParameterizedType(/*top level type or member type with raw enclosing type*/
                null, /*no raw*/
                false);
    ...
    private void parseInnerType() {
        ...
        if (Character.isDigit(typeName[0])) {
            ...
            while (this.scanner.isAtMemberTypeStart() && !this.isMalformed) nextToken = this.scanner.nextToken();
