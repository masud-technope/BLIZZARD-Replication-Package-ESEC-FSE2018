...
public class SingleTypeReference extends TypeReference {
    ...
    protected TypeBinding getTypeBinding(Scope scope) {
        ...
        if (this.resolvedType instanceof TypeVariableBinding) {
            ...
            if (typeVariable.declaringElement instanceof SourceTypeBinding) {
                scope.tagAsAccessingEnclosingInstanceStateOf((ReferenceBinding) typeVariable.declaringElement, /* type variable access */
                true);
            }
            ...
            if (methodScope != null && !methodScope.isStatic) {
                methodScope.tagAsAccessingEnclosingInstanceStateOf(localType, /* ! type variable access */
                false);
            }
        }
    ...
    public TypeBinding resolveTypeEnclosing(BlockScope scope, ReferenceBinding enclosingType) {
        ...
        // defaultNullness not relevant, the only caller within the compiler: QAE
        resolveAnnotations(scope, 0);
        // load after possible update in resolveAnnotations()
        ...
        memberType = scope.environment().convertToRawType(memberType, /*do not force conversion of enclosing types*/
        false);
        if (memberType.isRawType() && (this.bits & IgnoreRawTypeCheck) == 0 && scope.compilerOptions().getSeverity(CompilerOptions.RawTypeReference) != ProblemSeverities.Ignore) {
    ...
    public void traverse(ASTVisitor visitor, BlockScope scope) {
        if (visitor.visit(this, scope)) {
            if (this.annotations != null) {
                ...
                for (int i = 0, length = typeAnnotations == null ? 0 : typeAnnotations.length; i < length; i++) typeAnnotations[i].traverse(visitor, scope);
            }
        }
