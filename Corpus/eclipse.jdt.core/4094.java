...
public class SingleNameReference extends NameReference implements OperatorIds {
    ...
    public TypeBinding checkFieldAccess(BlockScope scope) {
        ...
        if ((this.bits & IsStrictlyAssignedMASK) == 0 && ms.enclosingSourceType() == fieldBinding.declaringClass && ms.lastVisibleFieldID >= 0 && fieldBinding.id >= ms.lastVisibleFieldID) {
    ...
    public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
        ...
        if (assignment.expression.isCompactableOperation()) {
            ...
            if ((operation.left instanceof SingleNameReference) && ((variableReference = (SingleNameReference) operation.left).binding == binding)) {
                ...
                variableReference.generateCompoundAssignment(currentScope, codeStream, syntheticAccessors == null ? null : syntheticAccessors[WRITE], operation.right, (operation.bits & OperatorMASK) >> OperatorSHIFT, operation.left.implicitConversion, /*should be equivalent to no conversion*/
                valueRequired);
                return;
            }
            int operator = (operation.bits & OperatorMASK) >> OperatorSHIFT;
            if ((operation.right instanceof SingleNameReference) && // only commutative operations
            ((operator == PLUS) || (operator == MULTIPLY)) && ((variableReference = (SingleNameReference) operation.right).binding == binding) && // exclude non constant expressions, since could have side-effect
            (operation.left.constant != NotAConstant) && // exclude string concatenation which would occur backwards
            ...
            ((operation.right.implicitConversion >> 4) != T_String)) {
                ...
                variableReference.generateCompoundAssignment(currentScope, codeStream, syntheticAccessors == null ? null : syntheticAccessors[WRITE], operation.left, operator, operation.right.implicitConversion, /*should be equivalent to no conversion*/
                valueRequired);
                return;
            }
        }
        switch(bits & RestrictiveFlagMASK) {
            ...
            FIELD:
                FieldBinding fieldBinding;
                if (// need a receiver?
                !(fieldBinding = (FieldBinding) this.codegenBinding).isStatic()) {
                    if ((bits & DepthMASK) != 0) {
            ...
            LOCAL:
                ...
                if (localBinding.resolvedPosition != -1) {
                    ...
                    if (assignment.expression.constant != NotAConstant) {
                        ...
                        if (valueRequired) {
                            codeStream.generateImplicitConversion(// implicit conversion
                ...
                if (localBinding.type.isArrayType() && (// arrayLoc = null
                assignment.expression.resolvedType == NullBinding || (// arrayLoc = (type[])null
                (assignment.expression instanceof CastExpression) && (((CastExpression) assignment.expression).innermostCastedExpression().resolvedType == NullBinding)))) {
                    codeStream.checkcast(localBinding.type);
                }
                // normal local assignment (since cannot store in outer local which are final locations)
                codeStream.store(localBinding, valueRequired);
                if (// for local variable debug attributes
                (bits & FirstAssignmentToLocalMASK) != 0) {
        ...
        if (constant != NotAConstant) {
            ...
            switch(bits & RestrictiveFlagMASK) {
                ...
                case FIELD:
                    FieldBinding fieldBinding;
                    if (valueRequired) {
                        if (// directly use inlined value for constant fields
                        !(fieldBinding = (FieldBinding) this.codegenBinding).isConstantValue()) {
                            boolean isStatic;
                            if (!(isStatic = fieldBinding.isStatic())) {
                                if ((bits & DepthMASK) != 0) {
                                    ReferenceBinding targetType = currentScope.enclosingSourceType().enclosingTypeAt((bits & DepthMASK) >> DepthSHIFT);
                                    Object[] emulationPath = currentScope.getEmulationPath(/*only exact match*/
                                    targetType, /*consider enclosing arg*/
                                    true, false);
                                    codeStream.generateOuterAccess(emulationPath, this, targetType, currentScope);
                                } else {
    ...
    public void generateCompoundAssignment(BlockScope currentScope, CodeStream codeStream, Expression expression, int operator, int assignmentImplicitConversion, boolean valueRequired) {
        this.generateCompoundAssignment(currentScope, codeStream, syntheticAccessors == null ? null : syntheticAccessors[WRITE], expression, operator, assignmentImplicitConversion, valueRequired);
    }
            ...
            LOCAL:
                ...
                switch(localBinding.type.id) {
                    case T_String:
                        ...
                        return;
                    case T_int:
                        if (((assignConstant = expression.constant) != NotAConstant) && (// only for integral types
                        assignConstant.typeID() != T_float) && (assignConstant.typeID() != T_double) && (// 16 bits value
                        (increment = assignConstant.intValue()) == // 16 bits value
                        (short) increment)) {
                            switch(operator) {
                                case PLUS:
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo, boolean isReadAccess) {
        ...
        if ((bits & FIELD) != 0) {
            ...
            if (((bits & DepthMASK) != 0) && (// private access
            ...
            codegenField.isProtected() && codegenField.declaringClass.getPackage() != currentScope.enclosingSourceType().getPackage()))) {
                ...
                syntheticAccessors[isReadAccess ? READ : WRITE] = ((SourceTypeBinding) currentScope.enclosingSourceType().enclosingTypeAt((bits & DepthMASK) >> DepthSHIFT)).addSyntheticMethod(codegenField, isReadAccess);
                currentScope.problemReporter().needToEmulateFieldAccess(codegenField, this, isReadAccess);
                return;
            }
            // and not from Object or implicit static field access.	
            if (fieldBinding.declaringClass != this.actualReceiverType && !this.actualReceiverType.isArrayType() && fieldBinding.declaringClass != null && !fieldBinding.isConstantValue() && ((currentScope.environment().options.targetJDK >= ClassFileConstants.JDK1_2 && !fieldBinding.isStatic() && // no change for Object fields (if there was any)
            fieldBinding.declaringClass.id != T_Object) || !codegenField.declaringClass.canBeSeenBy(currentScope))) {
                this.codegenBinding = currentScope.enclosingSourceType().getUpdatedFieldBinding(codegenField, (ReferenceBinding) this.actualReceiverType.erasure());
            }
        }
    }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if ((this.codegenBinding = this.binding = scope.getBinding(token, bits & RestrictiveFlagMASK, this, /*resolve*/
        true)).isValidBinding()) {
            switch(bits & RestrictiveFlagMASK) {
                // =========only variable============
                case VARIABLE:
