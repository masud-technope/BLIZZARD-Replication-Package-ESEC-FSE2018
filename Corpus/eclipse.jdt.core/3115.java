...
public class HierarchyScope extends AbstractSearchScope implements SuffixConstants {
    ...
    // null means: don't filter for subTypes
    private HashSet subTypes = null;

    // null means: don't constrain the search to a project
    ...
    public  HierarchyScope(IType type, WorkingCopyOwner owner) throws JavaModelException {
        ...
        if (root.isArchive()) {
            ...
            this.focusPath = zipFileName + JAR_FILE_ENTRY_SEPARATOR + type.getFullyQualifiedName().replace('.', '/') + SUFFIX_STRING_class;
        } else {
    ...
    public boolean enclosesFineGrained(IJavaElement element) {
        if ((this.subTypes == null) && this.allowMemberAndEnclosingTypes)
            // no fine grained checking requested
            return true;
