...
public class MethodScope extends BlockScope {
    ...
    // method modifier or initializer one
    ...
    // TODO(stephan): combine flags to a bitset?
        ...
        if ((((ConstructorDeclaration) this.referenceContext).bits & ASTNode.IsDefaultConstructor) != 0) {
            ...
            if ((flags = declaringClass.modifiers & DECLARING_FLAGS) != 0) {
                if ((flags & ClassFileConstants.AccEnum) != 0) {
                    ...
                    // default constructor is implicitly private in enum
                    modifiers |= ClassFileConstants.AccPrivate;
                } else {
                    modifiers &= ~VISIBILITY_FLAGS;
                    // propagate public/protected
                    modifiers |= flags;
                }
            }
        }
 ...
 */
    private void checkAndSetModifiersForMethod(MethodBinding methodBinding) {
        ...
        if (declaringClass.isInterface()) {
            ...
            // no need to check validity, is done by the parser
            ...
            if (compilerOptions().sourceLevel >= ClassFileConstants.JDK1_8 && !declaringClass.isAnnotationType()) {
                expectedModifiers |= ClassFileConstants.AccStrictfp | ExtraCompilerModifiers.AccDefaultMethod | ClassFileConstants.AccStatic;
    ...
    public void checkUnusedParameters(MethodBinding method) {
        if (method.isAbstract() || (method.isImplementing() && !compilerOptions().reportUnusedParameterWhenImplementingAbstract) || (method.isOverriding() && !method.isImplementing() && !compilerOptions().reportUnusedParameterWhenOverridingConcrete) || method.isMain()) {
        ...
        for (int i = 0, maxLocals = this.localIndex; i < maxLocals; i++) {
            ...
            if (local.useFlag == LocalVariableBinding.UNUSED && // do not report fake used variable
 ...
 */
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve) {
        ...
        if (receiverType.isInterface() && invocationSite.isQualifiedSuper())
            ...
            field, field.declaringClass, fieldName, ProblemReasons.NoProperEnclosingInstance);
        if (field.isStatic())
        ...
        if (invocationSite instanceof SingleNameReference)
            return new ProblemFieldBinding(// closest match
            field, field.declaringClass, fieldName, ProblemReasons.NonStaticReferenceInConstructorInvocation);
