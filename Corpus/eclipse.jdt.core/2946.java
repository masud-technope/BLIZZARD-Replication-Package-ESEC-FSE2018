...
public class TryStatement extends SubRoutineStatement {
    ...
    // finally is generated as a subroutine (using jsr/ret bytecodes)
    private static final int FINALLY_SUBROUTINE = 1;

    // non returning finally is optimized with only one instance of finally block
    private static final int FINALLY_DOES_NOT_COMPLETE = 2;

    // finally block must be inlined since cannot use jsr/ret bytecodes >1.5
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (// TODO (philippe) if subroutine is escaping, unused
        this.returnAddressVariable != null) {
        ...
        if (this.subRoutineStartLabel == null) {
            ...
            // start collection initsOnFinally
            ...
            for (int i = 0; i < resourcesLength; i++) {
                ...
                // Is implicitly used anyways.
                resourceBinding.useFlag = LocalVariableBinding.USED;
            ...
            subInfo = this.finallyBlock.analyseCode(currentScope, finallyContext = new FinallyFlowContext(flowContext, this.finallyBlock, handlingContext), flowInfo.nullInfoLessUnconditionalCopy()).unconditionalInits();
            // start collection initsOnFinally only after analysing the finally block
            handlingContext.conditionalLevel = 0;
            if (subInfo == FlowInfo.DEAD_END) {
                ...
                if (this.catchBlocks != null) {
                    for (int i = 0; i < this.catchBlocks.length; i++) this.catchBlocks[i].scope.finallyInfo = finallyInfo;
                }
            }
            ...
            finallyContext.complainOnDeferredChecks(((tryInfo.tagBits & FlowInfo.UNREACHABLE) == 0 ? flowInfo.unconditionalCopy().addPotentialInitializationsFrom(tryInfo).addPotentialInitializationsFrom(// exited at any point
            insideSubContext.initsOnReturn) : insideSubContext.initsOnReturn).addNullInfoFrom(handlingContext.initsOnFinally), currentScope);
            // chain up null info registry
            flowContext.mergeFinallyNullInfo(handlingContext.initsOnFinally);
    ...
    private FlowInfo prepareCatchInfo(FlowInfo flowInfo, ExceptionHandlingFlowContext handlingContext, FlowInfo tryInfo, int i) {
        ...
        if (isUncheckedCatchBlock(i)) {
            ...
            catchInfo = flowInfo.nullInfoLessUnconditionalCopy().addPotentialInitializationsFrom(initsOnException).addNullInfoFrom(// <<== Null info only from here!
            initsOnException).addPotentialInitializationsFrom(tryInfo.nullInfoLessUnconditionalCopy()).addPotentialInitializationsFrom(handlingContext.initsOnReturn.nullInfoLessUnconditionalCopy());
        }
	...
	*/
        if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=350579
        this.tryBlock.statements == null && this.resources == NO_RESOURCES) {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        try {
            ...
            if (resourceCount > 0) {
                ...
                codeStream.store(this.primaryExceptionVariable, /* value not required */
                false);
                codeStream.addVariable(this.primaryExceptionVariable);
                codeStream.aconst_null();
                codeStream.store(this.caughtThrowableVariable, /* value not required */
                false);
            ...
            if (resourceCount > 0) {
                for (int i = resourceCount; i >= 0; i--) {
                    ...
                    // outer handler if any is the one that should catch exceptions out of close()
                    ...
                    if ((this.bits & ASTNode.IsTryBlockExiting) == 0) {
                        ...
                        if (i > 0) {
                            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=343785
            ...
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=375248
 ...
 */
    public boolean generateSubRoutineInvocation(BlockScope currentScope, CodeStream codeStream, Object targetLocation, int stateIndex, LocalVariableBinding secretLocal) {
        int resourceCount = this.resources.length;
        if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=375248
        resourceCount > 0 && this.resourceExceptionLabels != null) {
            for (int i = resourceCount; i > 0; --i) {
    ...
    public void resolve(BlockScope upperScope) {
        ...
        // Single scope to hold all resources and additional secret variables.
        BlockScope resourceManagementScope = null;
        int resourceCount = this.resources.length;
        if (resourceCount > 0) {
            resourceManagementScope = new BlockScope(this.scope);
            this.primaryExceptionVariable = new LocalVariableBinding(TryStatement.SECRET_PRIMARY_EXCEPTION_VARIABLE_NAME, this.scope.getJavaLangThrowable(), ClassFileConstants.AccDefault, false);
            ...
            this.caughtThrowableVariable = new LocalVariableBinding(TryStatement.SECRET_CAUGHT_THROWABLE_VARIABLE_NAME, this.scope.getJavaLangThrowable(), ClassFileConstants.AccDefault, false);
        ...
        for (int i = 0; i < resourceCount; i++) {
            ...
            if (localVariableBinding != null && localVariableBinding.isValidBinding()) {
                ...
                if (resourceType instanceof ReferenceBinding) {
                    if (resourceType.findSuperTypeOriginatingFrom(TypeIds.T_JavaLangAutoCloseable, /*AutoCloseable is not a class*/
                    false) == null && resourceType.isValidBinding()) {
                ...
                } else if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=349862, avoid secondary error in problematic null case
                resourceType != null) {
                    upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType, this.resources[i].type);
                    localVariableBinding.type = new ProblemReferenceBinding(CharOperation.splitOn('.', resourceType.shortReadableName()), null, ProblemReasons.InvalidTypeForAutoManagedResource);
                }
                ...
                if (!methodScope.isInsideInitializer()) {
                    ...
                    if (methodBinding != null) {
                        ...
                        if (methodReturnType.id != TypeIds.T_void) {
                            ...
                            this.secretReturnValue.setConstant(// not inlinable
