...
public class CompilationUnitScope extends Scope {
    ...
    void buildFieldsAndMethods() {
        for (int i = 0, length = topLevelTypes.length; i < length; i++) topLevelTypes[i].scope.buildFieldsAndMethods();
    }
    ...
    void buildTypeBindings() {
        ...
        if (referenceContext.compilationResult.compilationUnit != null) {
            ...
            if (expectedPackageName != null && !CharOperation.equals(currentPackageName, expectedPackageName)) {
                ...
                if (referenceContext.currentPackage != null || referenceContext.types != null || referenceContext.imports != null) {
        ...
        if (currentPackageName == CharOperation.NO_CHAR_CHAR) {
            ...
            recordQualifiedReference(currentPackageName);
        }
        ...
        nextType: for (int i = 0; i < typeLength; i++) {
            ...
            if ((typeDecl.modifiers & AccPublic) != 0) {
                ...
                (mainTypeName = referenceContext.getMainTypeName()) != null && !CharOperation.equals(mainTypeName, typeDecl.name)) {
    ...
    void checkAndSetImports() {
        ...
        nextImport: for (int i = 0; i < numberOfStatements; i++) {
            ...
            for (int j = 0; j < index; j++) if (resolvedImports[j].onDemand == importReference.onDemand)
 ...
 */
    ...
    void connectTypeHierarchy() {
        for (int i = 0, length = topLevelTypes.length; i < length; i++) topLevelTypes[i].scope.connectTypeHierarchy();
    }
    ...
    void faultInImports() {
        ...
        for (int i = 0; i < numberOfStatements; i++) {
            if (!referenceContext.imports[i].onDemand) {
                ...
                for (int j = 0, length = topLevelTypes.length; j < length; j++) typesBySimpleNames.put(topLevelTypes[j].sourceName, topLevelTypes[j]);
                break;
            }
        }
        // allocate the import array, add java.lang.* by default
    ...
    public void faultInTypes() {
        faultInImports();
        for (int i = 0, length = topLevelTypes.length; i < length; i++) topLevelTypes[i].faultInTypesForFieldsAndMethods();
    }
...
*/
    ...
    public void storeDependencyInfo() {
        ...
        i < referencedSuperTypes.size; // grows as more types are added
        i++) {
        ...
        for (int i = 0; i < size; i++) qualifiedRefs[i] = qualifiedReferences.elementAt(i);
        referenceContext.compilationResult.qualifiedReferences = qualifiedRefs;
        size = simpleNameReferences.size;
        char[][] simpleRefs = new char[size][];
        for (int i = 0; i < size; i++) simpleRefs[i] = simpleNameReferences.elementAt(i);
    ...
    public void verifyMethods(MethodVerifier verifier) {
        for (int i = 0, length = topLevelTypes.length; i < length; i++) topLevelTypes[i].verifyMethods(verifier);
    }
}
