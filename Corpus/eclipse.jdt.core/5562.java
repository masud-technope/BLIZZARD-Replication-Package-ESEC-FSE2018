...
public class SearchableEnvironment implements INameEnvironment, IJavaSearchConstants {
    ...
    public  SearchableEnvironment(JavaProject project, org.eclipse.jdt.core.ICompilationUnit[] workingCopies) throws JavaModelException {
        ...
        this.checkAccessRestrictions = !JavaCore.IGNORE.equals(project.getOption(JavaCore.COMPILER_PB_FORBIDDEN_REFERENCE, true)) || !JavaCore.IGNORE.equals(project.getOption(JavaCore.COMPILER_PB_DISCOURAGED_REFERENCE, true));
    ...
    public  SearchableEnvironment(JavaProject project, WorkingCopyOwner owner) throws JavaModelException {
        this(project, owner == null ? null : JavaModelManager.getJavaModelManager().getWorkingCopies(owner, /*add primary WCs*/
        true));
        this.owner = owner;
    }
    ...
    protected NameEnvironmentAnswer find(String typeName, String packageName) {
        ...
        NameLookup.Answer answer = this.nameLookup.findType(typeName, packageName, /*exact match*/
        false, NameLookup.ACCEPT_ALL, this.checkAccessRestrictions);
        if (answer != null) {
            // construct name env answer
            if (// BinaryType
            answer.type instanceof BinaryType) {
                try {
            ...
            } else //SourceType
            {
                try {
                    ...
                    for (int i = 0, index = 1; i < length; i++) {
                        ...
                        if (// check that the index is in bounds (see https://bugs.eclipse.org/bugs/show_bug.cgi?id=62861)
                        !otherType.equals(topLevelType) && index < length)
    ...
    public void findExactTypes(char[] name, final boolean findMembers, int searchFor, final ISearchRequestor storage) {
        try {
            ...
            try {
                new BasicSearchEngine(this.workingCopies).searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH, name, SearchPattern.R_EXACT_MATCH, searchFor, getSearchScope(), typeRequestor, CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
        ...
        try {
            ...
            if (monitor != null) {
                ...
                if (indexManager.awaitingJobsCount() == 0) {
                    ...
                    new BasicSearchEngine(this.workingCopies).searchAllTypeNames(qualification, SearchPattern.R_EXACT_MATCH, simpleName, // not case sensitive
                    matchRule, searchFor, getSearchScope(), typeRequestor, FORCE_IMMEDIATE_SEARCH, progressMonitor);
                } else {
                ...
                try {
                    new BasicSearchEngine(this.workingCopies).searchAllTypeNames(qualification, SearchPattern.R_EXACT_MATCH, simpleName, // not case sensitive
                    matchRule, searchFor, getSearchScope(), typeRequestor, CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
                } catch (OperationCanceledException e) {
                    findTypes(new String(prefix), storage, convertSearchFilterToModelFilter(searchFor));
                }
            }
    ...
    public void findConstructorDeclarations(char[] prefix, boolean camelCaseMatch, final ISearchRequestor storage, IProgressMonitor monitor) {
        try {
            ...
            if (monitor != null) {
                ...
                new BasicSearchEngine(this.workingCopies).searchAllConstructorDeclarations(qualification, simpleName, matchRule, getSearchScope(), constructorRequestor, FORCE_IMMEDIATE_SEARCH, progressMonitor);
            } else {
                try {
                    new BasicSearchEngine(this.workingCopies).searchAllConstructorDeclarations(qualification, simpleName, matchRule, getSearchScope(), constructorRequestor, CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
                } catch (OperationCanceledException e) {
                }
            }
        } catch (JavaModelException e) {
    ...
    private void findTypes(String prefix, ISearchRequestor storage, int type) {
        ...
        if (index == -1) {
            ...
            if (fragments != null) {
                ...
                for (int i = 0, length = fragments.length; i < length; i++) if (fragments[i] != null)
