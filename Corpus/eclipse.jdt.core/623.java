 ...
 */
    ...
    protected void codeComplete(org.eclipse.jdt.internal.compiler.env.ICompilationUnit cu, org.eclipse.jdt.internal.compiler.env.ICompilationUnit unitToSkip, int position, CompletionRequestor requestor, WorkingCopyOwner owner, ITypeRoot typeRoot, IProgressMonitor monitor) throws JavaModelException {
        if (requestor == null) {
            //$NON-NLS-1$
        ...
        if (performanceStats != null) {
            //$NON-NLS-1$
            performanceStats.startRun(new String(cu.getFileName()) + " at " + position);
        }
    ...
    protected IJavaElement[] codeSelect(org.eclipse.jdt.internal.compiler.env.ICompilationUnit cu, int offset, int length, WorkingCopyOwner owner) throws JavaModelException {
        ...
        if (performanceStats != null) {
            ...
            performanceStats.startRun(new String(cu.getFileName()) + " at [" + offset + "," + length + "]");
        }
        JavaProject project = (JavaProject) getJavaProject();
        SearchableEnvironment environment = project.newSearchableNameEnvironment(owner);
 ...
 */
    public boolean hasUnsavedChanges() throws JavaModelException {
        ...
        if (elementType == PACKAGE_FRAGMENT || elementType == PACKAGE_FRAGMENT_ROOT || elementType == JAVA_PROJECT || // fix for 1FWNMHH
 ...
 */
    public void save(IProgressMonitor pm, boolean force) throws JavaModelException {
        ...
        IBuffer buf = getBuffer();
        if (// some Openables (like a JavaProject) don't have a buffer
        buf != null) {
            buf.save(pm, force);
