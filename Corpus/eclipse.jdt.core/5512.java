...
public class ClassScope extends Scope {
    ...
    private void buildFields() {
        ...
        if (referenceContext.fields == null) {
            if (// 72468
            hierarchyIsInconsistent) {
                referenceContext.binding.fields = new FieldBinding[1];
                referenceContext.binding.fields[0] = new FieldBinding(IncompleteHierarchy, VoidBinding, AccPrivate, referenceContext.binding, null);
            } else {
        ...
        int count = 0;
        for (int i = 0; i < size; i++) if (fields[i].isField())
            count++;
        ...
        for (int i = 0; i < count; i++) fieldBindings[i].id = i;
	...
	*/
        ...
        if (interfacesToVisit != null) {
            ...
            for (int i = 0; i <= lastPosition; i++) {
                ...
                for (int j = 0, length = interfaces.length; j < length; j++) {
                    ...
                    (anInterface.tagBits & HasNoMemberTypes) == 0) {
                        if (// avoid resolving member types eagerly
            ...
            if (needToTag) {
                for (int i = 0; i <= lastPosition; i++) {
                    ReferenceBinding[] interfaces = interfacesToVisit[i];
                    for (int j = 0, length = interfaces.length; j < length; j++) interfaces[j].tagBits |= HasNoMemberTypes;
                }
            }
        }
        ...
        if (typeParameters != null) {
            for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++) {
                ...
                if (typeRef != null) {
                    ...
                    if (boundRefs != null)
                        for (int j = 0, k = boundRefs.length; j < k; j++) boundRefs[j].checkBounds(this);
                }
            }
        }
    }
    ...
    private void connectMemberTypes() {
        ...
        if (sourceType.memberTypes != NoMemberTypes)
            for (int i = 0, size = sourceType.memberTypes.length; i < size; i++) ((SourceTypeBinding) sourceType.memberTypes[i]).scope.connectTypeHierarchy();
    }
	...
	*/
    ...
    private boolean detectCycle(SourceTypeBinding sourceType, ReferenceBinding superType, TypeReference reference) {
        ...
        if (superType.isBinaryBinding()) {
            ...
            if (superType.superclass() != null) {
                ...
                if ((parentType.tagBits & HierarchyHasProblems) != 0) {
                    ...
                    parentType.tagBits |= HierarchyHasProblems;
                }
            }
        ...
        if (superType.isHierarchyBeingConnected()) {
            if (// if null then its connecting its type variables
            ((SourceTypeBinding) superType).scope.superTypeReference != null) {
                problemReporter().hierarchyCircularity(sourceType, superType, reference);
                sourceType.tagBits |= HierarchyHasProblems;
                superType.tagBits |= HierarchyHasProblems;
                return true;
