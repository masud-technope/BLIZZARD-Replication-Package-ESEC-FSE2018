...
public class DiagnoseParser implements ParserBasicInformation, TerminalTokens, ConflictedParser {
    ...
    public void diagnoseParse(boolean record) {
        ...
        try {
            ...
            do {
                ...
                for (int i = 0; i <= this.stateStackTop; i++) this.tempStack[i] = this.stack[i];
                ...
                act > ERROR_ACTION || act < ACCEPT_ACTION) {
                    ...
                    for (int i = next_pos + 1; i <= this.nextStackTop; i++) this.nextStack[i] = this.tempStack[i];
                    ...
                    if (act != ERROR_ACTION) {
                        ...
                        for (int i = prev_pos + 1; i <= this.prevStackTop; i++) this.prevStack[i] = this.stack[i];
                        prev_pos = pos;
                        this.stateStackTop = this.nextStackTop;
                        for (int i = pos + 1; i <= this.stateStackTop; i++) this.stack[i] = this.nextStack[i];
                        this.locationStack[this.stateStackTop] = this.currentToken;
                        this.locationStartStack[this.stateStackTop] = this.lexStream.start(this.currentToken);
                        pos = next_pos;
                    }
    ...
    private RepairCandidate errorRecovery(int error_token, boolean forcedError) {
        ...
        //
        int i;
        for (i = BUFF_UBOUND; this.lexStream.kind(this.buffer[i]) == EOFT_SYMBOL; /*empty*/
        i--) {
        }
        reportError(DELETION_CODE, //Parser.terminal_index[lexStream.kind(prevtok)],
        Parser.terminal_index[prevtokKind], error_token, this.buffer[i]);
        candidate.symbol = 0;
    ...
    private RepairCandidate primaryPhase(int error_token) {
        ...
        for (int j = i; j > 0; j--) this.buffer[j - 1] = this.lexStream.previous(this.buffer[j]);
        for (int k = i + 1; k < BUFF_SIZE; k++) this.buffer[k] = this.lexStream.next(this.buffer[k - 1]);
        //
    ...
    private PrimaryRepairInfo checkPrimaryDistance(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        this.buffer[repair.bufferPosition] != 0 && this.buffer[repair.bufferPosition + 1] != 0) {
        ...
        //
        j = parseCheck(stck, stack_top, this.lexStream.kind(this.buffer[repair.bufferPosition + 1]), repair.bufferPosition + 2);
        if (this.lexStream.kind(this.buffer[repair.bufferPosition]) == EOLT_SYMBOL && this.lexStream.afterEol(this.buffer[repair.bufferPosition + 1])) {
            k = 10;
        } else {
            k = 0;
        }
        ...
        this.buffer[repair.bufferPosition] != 0) {
            while (symbol != 0) {
            ...
            case SCOPE_CODE:
                {
                    for (int i = 0; i < this.scopeStackTop; i++) {
                        reportError(repair.code, -this.scopeIndex[i], this.locationStack[this.scopePosition[i]], prevtok, Parser.non_terminal_index[Parser.scope_lhs[this.scopeIndex[i]]]);
                    }
                    ...
                    reportError(repair.code, -this.scopeIndex[this.scopeStackTop], this.locationStack[this.scopePosition[this.scopeStackTop]], prevtok, getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition));
                    break;
                }
            // deletion
            default:
    ...
    private int getNtermIndex(int start, int sym, int buffer_position) {
        int highest_symbol = sym - NT_OFFSET, tok = this.lexStream.kind(this.buffer[buffer_position]);
        this.lexStream.reset(this.buffer[buffer_position + 1]);
        //
        // Initialize stack index of temp_stack and initialize maximum
        // position of state stack that is still useful.
    ...
    private PrimaryRepairInfo scopeTrial(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        for (int i = 0; i < this.stackLength; i++) this.stateSeen[i] = NIL;
        this.statePoolTop = 0;
            ...
            if (act != ERROR_ACTION) {
                ...
                Parser.scope_rhs[k] == 0 && j < marked_pos) {
                    ...
                    if (Parser.scope_state[j] != 0) {
                        ...
                        if (this.lexStream.kind(this.buffer[repair.bufferPosition]) == EOFT_SYMBOL && repair.distance == previous_distance) {
                            this.scopeStackTop = indx;
    ...
    private boolean secondaryCheck(int stck[], int stack_top, int buffer_position, int distance) {
        ...
        for (top = stack_top - 1; top >= 0; top--) {
            j = parseCheck(stck, top, this.lexStream.kind(this.buffer[buffer_position]), buffer_position + 1);
    ...
    private RepairCandidate secondaryPhase(int error_token) {
        ...
        if (this.nextStackTop >= 0) {
            ...
            for (k = 3; k < BUFF_UBOUND; k++) this.buffer[k] = this.lexStream.next(this.buffer[k - 1]);
            // elmt not available
            this.buffer[BUFF_UBOUND] = this.lexStream.badtoken();
            //
            for (next_last_index = MAX_DISTANCE - 1; next_last_index >= 1 && this.lexStream.kind(this.buffer[next_last_index]) == EOFT_SYMBOL; next_last_index/*empty*/
            --) {
            }
        ...
        this.buffer[1] = this.lexStream.previous(this.buffer[2]);
        this.buffer[0] = this.lexStream.previous(this.buffer[1]);
        for (k = 4; k < BUFF_SIZE; k++) this.buffer[k] = this.lexStream.next(this.buffer[k - 1]);
        for (last_index = MAX_DISTANCE - 1; last_index >= 1 && this.lexStream.kind(this.buffer[last_index]) == EOFT_SYMBOL; last_index--) {
        }
        last_index++;
        ...
        if (repair.recoveryOnNextStack) {
            this.stateStackTop = this.nextStackTop;
            for (i = 0; i <= this.stateStackTop; i++) this.stack[i] = this.nextStack[i];
            this.buffer[2] = error_token;
            this.buffer[1] = this.lexStream.previous(this.buffer[2]);
            this.buffer[0] = this.lexStream.previous(this.buffer[1]);
            for (k = 3; k < BUFF_UBOUND; k++) this.buffer[k] = this.lexStream.next(this.buffer[k - 1]);
            ...
            case SCOPE_CODE:
                {
                    if (repair.stackPosition < this.stateStackTop) {
                        reportError(DELETION_CODE, Parser.terminal_index[ERROR_SYMBOL], this.locationStack[repair.stackPosition], this.buffer[1]);
                    }
                    for (int i = 0; i < this.scopeStackTop; i++) {
                        reportError(SCOPE_CODE, -this.scopeIndex[i], this.locationStack[this.scopePosition[i]], this.buffer[1], Parser.non_terminal_index[Parser.scope_lhs[this.scopeIndex[i]]]);
                    }
                    repair.symbol = Parser.scope_lhs[this.scopeIndex[this.scopeStackTop]] + NT_OFFSET;
                    this.stateStackTop = this.scopePosition[this.scopeStackTop];
                    reportError(SCOPE_CODE, -this.scopeIndex[this.scopeStackTop], this.locationStack[this.scopePosition[this.scopeStackTop]], this.buffer[1], getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition));
                    break;
                }
            default:
                {
                    reportError(repair.code, (repair.code == SECONDARY_CODE ? getNtermIndex(this.stack[repair.stackPosition], repair.symbol, repair.bufferPosition) : Parser.terminal_index[ERROR_SYMBOL]), this.locationStack[repair.stackPosition], this.buffer[repair.bufferPosition - 1]);
    ...
    private int parseCheck(int stck[], int stack_top, int first_token, int buffer_position) {
        ...
        process_terminal: for (; ; ) {
            ...
            if (// Stack overflow!!!
            ++this.tempStackTop >= this.stackLength)
                return indx;
