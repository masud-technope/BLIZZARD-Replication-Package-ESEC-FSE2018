...
public abstract class Scope implements BaseTypes, BindingIds, CompilerModifiers, ProblemReasons, TagBits, TypeConstants, TypeIds {
	...
	*/
    ...
    protected final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments, InvocationSite invocationSite) {
        ...
        if (parameters == arguments && (method.returnType.tagBits & HasTypeVariable) == 0 && genericTypeArguments == null)
            return method;
        ...
        argumentCompatibility: {
            for (int i = 0; i < argLength; i++) if (parameters[i] != arguments[i] && !arguments[i].isCompatibleWith(parameters[i]))
    ...
    protected boolean connectTypeVariables(TypeParameter[] typeParameters) {
        ...
        nextVariable: for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++) {
            ...
            ReferenceBinding superType = this.kind == METHOD_SCOPE ? (ReferenceBinding) typeRef.resolveType((BlockScope) this) : (ReferenceBinding) typeRef.resolveType((ClassScope) this);
            if (superType == null) {
            ...
            if (boundRefs != null) {
                for (int j = 0, k = boundRefs.length; j < k; j++) {
                    typeRef = boundRefs[j];
                    superType = this.kind == METHOD_SCOPE ? (ReferenceBinding) typeRef.resolveType((BlockScope) this) : (ReferenceBinding) typeRef.resolveType((ClassScope) this);
                    if (superType == null) {
                        typeVariable.tagBits |= HierarchyHasProblems;
                        noProblems = false;
                        continue nextVariable;
    ...
    public TypeVariableBinding[] createTypeVariables(TypeParameter[] typeParameters, Binding declaringElement) {
        ...
        nextParameter: for (int i = 0; i < length; i++) {
            ...
            if (knownTypeParameterNames.containsKey(typeParameter.name)) {
                ...
                // ensure that the duplicate parameter is found & removed
	...
	*/
    ...
    public MethodBinding findDefaultAbstractMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite, ReferenceBinding classHierarchyStart, MethodBinding matchingMethod, ObjectVector found) {
        ...
        if (// try to find a close match when the parameter order is wrong or missing some parameters
        candidatesCount == 0) {
            ...
            nextMethod: for (int i = 0; i < foundSize; i++) {
                ...
                nextArg: for (int a = 0; a < argLength; a++) {
                    ...
                    for (int p = 0; p < paramLength; p++) if (params[p] == arg)
                        continue nextArg;
                    continue nextMethod;
                }
    ...
    public ReferenceBinding findDirectMemberType(char[] typeName, ReferenceBinding enclosingType) {
        ...
        if (memberType != null) {
            ...
            if (enclosingSourceType == null ? memberType.canBeSeenBy(getCurrentPackage()) : memberType.canBeSeenBy(enclosingType, enclosingSourceType))
                return memberType;
    ...
    public MethodBinding findExactMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite) {
        ...
        if (exactMethod != null) {
            ...
            if (receiverType.isInterface() || exactMethod.canBeSeenBy(receiverType, invocationSite, this)) {
                if (receiverType.id != T_Object && argumentTypes == NoParameters && CharOperation.equals(selector, GETCLASS) && exactMethod.returnType.isParameterizedType()) {
	...
	*/
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve) {
        ...
        FieldBinding field = currentType.getField(fieldName, /*resolve*/
        true);
        if (field != null) {
            if (field.canBeSeenBy(currentType, invocationSite, this))
                return field;
            return new ProblemFieldBinding(/* closest match*/
            field, field.declaringClass, fieldName, NotVisible);
        }
            ...
            if ((field = currentType.getField(fieldName, needResolve)) != null) {
                ...
                if (field.canBeSeenBy(receiverType, invocationSite, this)) {
                    if (visibleField == null)
                        ...
                        return new ProblemFieldBinding(/* closest match*/
                        visibleField, visibleField.declaringClass, fieldName, Ambiguous);
                } else {
                    notVisible = true;
                }
            }
        ...
        if (interfacesToVisit != null) {
            ProblemFieldBinding ambiguous = null;
                ...
                for (int j = 0, length = interfaces.length; j < length; j++) {
                    ...
                    if ((anInterface.tagBits & InterfaceVisited) == 0) {
                        ...
                        if ((field = anInterface.getField(/*resolve*/
                        fieldName, true)) != null) {
                            if (visibleField == null) {
                                visibleField = field;
                            } else {
            ...
            for (int i = 0; i <= lastPosition; i++) {
                ReferenceBinding[] interfaces = interfacesToVisit[i];
                for (int j = 0, length = interfaces.length; j < length; j++) interfaces[j].tagBits &= ~InterfaceVisited;
            }
            if (ambiguous != null)
                return ambiguous;
        }
    ...
    public MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes, InvocationSite invocationSite) {
        ...
        //TODO (kent) should rewrite to remove #matchingMethod since found is allocated anyway
        ...
        while (currentType != null) {
            ...
            if (isCompliant14) {
                nextMethod: for (int i = 0; i < currentLength; i++) {
                    ...
                    if ((currentMethod.modifiers & AccProtected) == 0)
                        continue nextMethod;
                    if (matchingMethod != null) {
            ...
            if (currentLength == 1 && matchingMethod == null && found.size == 0) {
                ...
                if (// no method was eliminated for 1.4 compliance (see above)
                maxMethod == currentLength) {
                    found.addAll(currentMethods);
                } else {
        ...
        int candidatesCount = 0;
        // is matchingMethod meeting argument expectation ?
                ...
                if (compatibleMethod != null) {
                    if (compatibleMethod.isValidBinding()) {
                        switch(candidatesCount) {
                            case 0:
                                // if only one match, reuse matchingMethod
                                matchingMethod = compatibleMethod;
                                // matchingMethod is known to exist and match params here
                                checkedMatchingMethod = true;
                                break;
                            case 1:
                                // only lazily created if more than one match
                                candidates = new MethodBinding[foundSize];
                                // copy back
                                candidates[// copy back
                                0] = matchingMethod;
                                matchingMethod = null;
                            // fall through
                            default:
        ...
        if (candidatesCount > 0)
            // forget the problem method if candidates were found
            problemMethod = null;
        ...
        if (compilationUnitScope().environment.options.sourceLevel >= ClassFileConstants.JDK1_5) {
            for (int i = 0; i < candidatesCount; i++) {
                ...
                if (current instanceof ParameterizedMethodBinding)
                    for (int j = i + 1; j < candidatesCount; j++) if (current.declaringClass == candidates[j].declaringClass && current.areParametersEqual(candidates[j]))
        ...
        return candidates[0].declaringClass.isClass() ? mostSpecificClassMethodBinding(candidates, visiblesCount, invocationSite) : mostSpecificInterfaceMethodBinding(candidates, visiblesCount, invocationSite);
    }
                ...
                switch(selector[0]) {
                    case 'c':
                        if (CharOperation.equals(selector, CLONE))
                            return new UpdatedMethodBinding(// remember its array type for codegen purpose on target>=1.4.0
                            environment().options.targetJDK >= ClassFileConstants.JDK1_4 ? (TypeBinding) receiverType : (TypeBinding) object, (methodBinding.modifiers ^ AccProtected) | AccPublic, CLONE, methodBinding.returnType, argumentTypes, null, object);
                        break;
                    case 'g':
                        if (CharOperation.equals(selector, GETCLASS) && methodBinding.returnType.isParameterizedType()) {
                        ...
                        case BLOCK_SCOPE:
                            ...
                            if (variableBinding != null) {
                                if (foundField != null && foundField.isValidBinding())
                                    return new ProblemFieldBinding(// closest match
                                    foundField, foundField.declaringClass, name, InheritedNameHidesEnclosingName);
                                if (depth > 0)
                                ...
                                if (fieldBinding.isValidBinding()) {
                                    if (!fieldBinding.isStatic()) {
                                        if (insideConstructorCall) {
                                            ...
                                            fieldBinding, fieldBinding.declaringClass, name, NonStaticReferenceInConstructorInvocation);
                                        } else if (insideStaticContext) {
                                            insideProblem = new ProblemFieldBinding(// closest match
                                            fieldBinding, fieldBinding.declaringClass, name, NonStaticReferenceInStaticContext);
                                        }
                                    }
        ...
        true) {
            switch(scope.kind) {
                ...
                case CLASS_SCOPE:
                    ...
                    MethodBinding methodBinding = (foundMethod == null) ? classScope.findExactMethod(receiverType, selector, argumentTypes, invocationSite) : classScope.findExactMethod(receiverType, foundMethod.selector, foundMethod.parameters, invocationSite);
                    ...
                    methodBinding != null) {
                        if (methodBinding.problemId() == Ambiguous) {
                            ...
                            methodBinding, selector, argumentTypes, InheritedNameHidesEnclosingName);
                        }
                        ...
                        if (methodBinding.isValidBinding()) {
                            ...
                            if (fuzzyProblem == null && !methodBinding.isStatic()) {
                                if (insideConstructorCall) {
                                    insideProblem = new ProblemMethodBinding(// closest match
                                    methodBinding, methodBinding.selector, methodBinding.parameters, NonStaticReferenceInConstructorInvocation);
                                } else if (insideStaticContext) {
                                    insideProblem = new ProblemMethodBinding(// closest match
                                    methodBinding, methodBinding.selector, methodBinding.parameters, NonStaticReferenceInStaticContext);
                                }
                            }
                            if (receiverType == methodBinding.declaringClass || (receiverType.getMethods(selector)) != NoMethods || ((fuzzyProblem == null || fuzzyProblem.problemId() != NotVisible) && environment().options.complianceLevel >= ClassFileConstants.JDK1_4)) {
                                ...
                                if (foundMethod.declaringClass != methodBinding.declaringClass)
                                    ...
                                    return new ProblemMethodBinding(// closest match
                                    methodBinding, methodBinding.selector, methodBinding.parameters, InheritedNameHidesEnclosingName);
                            }
                        }
                        if (foundMethod == null || (foundMethod.problemId() == NotVisible && methodBinding.problemId() != NotVisible)) {
                            ...
                            foundInsideProblem = insideProblem;
                            if (fuzzyProblem == null)
                                // only keep it if no error was found
	...
	*/
    public final TypeBinding getType(char[] name, PackageBinding packageBinding) {
        ...
        if (binding == null)
            return new ProblemReferenceBinding(CharOperation.arrayConcat(packageBinding.compoundName, name), NotFound);
        if (!binding.isValidBinding())
        ...
        if (!typeBinding.canBeSeenBy(this))
            return new ProblemReferenceBinding(CharOperation.arrayConcat(packageBinding.compoundName, name), typeBinding, NotVisible);
        return typeBinding;
    }
	...
	*/
    public final TypeBinding getType(char[][] compoundName, int typeNameLength) {
        ...
        checkVisibility)
            if (!typeBinding.canBeSeenBy(this))
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), typeBinding, NotVisible);
        while (currentIndex < typeNameLength) {
            ...
            true) {
                switch(scope.kind) {
                    ...
                    case CLASS_SCOPE:
                        ...
                        insideStaticContext |= // not isStatic()
                        (sourceType.modifiers & AccStatic) != // not isStatic()
                        0;
                        ...
                        memberType != null) {
                            ...
                            if (memberType.isValidBinding()) {
                                if (sourceType == memberType.enclosingType() || environment().options.complianceLevel >= ClassFileConstants.JDK1_4) {
                                    // OR in 1.4 mode (inherited shadows enclosing)
                                    if (foundType == null)
                                        return memberType;
            ...
            if (imports != null) {
                ...
                for (int i = 0, length = imports.length; i < length; i++) {
                    ...
                    if (someImport.onDemand) {
                        ...
                        ReferenceBinding temp = resolvedImport instanceof PackageBinding ? findType(name, (PackageBinding) resolvedImport, currentPackage) : findDirectMemberType(name, (ReferenceBinding) resolvedImport);
                        if (temp != null) {
	...
	*/
    public final boolean isDefinedInSameUnit(ReferenceBinding type) {
        ...
        for (int i = topLevelTypes.length; --i >= 0; ) if (topLevelTypes[i] == enclosingType)
            return true;
        return false;
    }
	...
	*/
                ...
                switch(wildU.kind) {
                    ...
                    case Wildcard.SUPER:
                        ...
                        if (wildU.kind == Wildcard.SUPER) {
                            ...
                            return environment().createWildcard(genericType, rank, glb[0], Wildcard.SUPER);
                        }
                }
            } else {
                switch(wildV.kind) {
    ...
    private TypeBinding minimalErasedCandidate(TypeBinding[] types, List invocations) {
        ...
        if (firstType.isBaseType()) {
            ...
            superTypes = new TypeBinding[] { firstType.erasure(), getJavaIoSerializable(), getJavaLangCloneable(), getJavaLangObject() };
        } else {
        ...
        nextOtherType: for (int i = indexOfFirst + 1; i < length; i++) {
            ...
            nextSuperType: for (int j = 0; j < superLength; j++) {
                ...
                if (otherRefType.erasure().isCompatibleWith(superType)) {
                    ...
                    if (// match can be null: interface.findSuperTypeErasingTo(Object)
                    match != null && match.erasure() != match) {
	...
	*/
    ...
    public TypeBinding mostSpecificCommonType(TypeBinding[] types) {
        ...
        if (firstType.isBaseType()) {
            ...
            superLength = 4;
            superTypes = new TypeBinding[] { firstType, getJavaIoSerializable(), getJavaLangCloneable(), getJavaLangObject() };
        } else {
            ArrayList typesToVisit = new ArrayList(5);
            ...
            for (int i = 0, max = 1; i < max; i++) {
                ...
                ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
                for (int j = 0, count = itsInterfaces.length; j < count; j++) if (!typesToVisit.contains(itsInterfaces[j])) {
