...
public abstract class AssistParser extends Parser {
    ...
    // whether we are inside a lambda expression
 ...
 */
    public RecoveredElement buildInitialRecoveryState() {
        ...
        if (this.referenceContext instanceof AbstractMethodDeclaration) {
            ...
            if (this.referenceContext instanceof TypeDeclaration) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (field != null && field.getKind() == AbstractVariableDeclaration.INITIALIZER && field.declarationSourceStart <= this.scanner.initialPosition && this.scanner.initialPosition <= field.declarationSourceEnd && this.scanner.eofPosition <= field.declarationSourceEnd + 1) {
        ...
        for (int i = 0; i <= this.astPtr; i++, lastNode = node) {
            ...
            for (int j = blockIndex; j <= this.realBlockPtr; j++) {
                if (this.blockStarts[j] >= 0) {
                    ...
                    if (// avoid multiple block if at same position
                    this.blockStarts[j] != lastStart) {
                        block = new Block(0);
        ...
        for (int j = blockIndex; j <= this.realBlockPtr; j++) {
            if (this.blockStarts[j] >= 0) {
                if (// avoid multiple block if at same position
                (this.blockStarts[j] < pos || createLambdaBlock) && (this.blockStarts[j] != lastStart)) {
                    block = new Block(0);
                    block.sourceStart = lastStart = this.blockStarts[j];
                    element = element.add(block, 1);
    ...
    protected boolean triggerRecoveryUponLambdaClosure(Statement statement, boolean shouldCommit) {
        ...
        if (lambdaClosed && this.currentElement != null && !(this.currentElement instanceof RecoveredField)) {
            if (// added already as part of standard recovery since these contribute a name to the scope prevailing at the cursor.
            !(statement instanceof AbstractVariableDeclaration)) {
			...
			*/
			 ...
			 */
                statement = replaceAssistStatement(this.currentElement.topElement(), this.assistNodeParent(), statementStart, statementEnd, statement);
                if (statement != null) {
                    RecoveredBlock recoveredBlock = (RecoveredBlock) (this.currentElement instanceof RecoveredBlock ? this.currentElement : (this.currentElement.parent instanceof RecoveredBlock) ? this.currentElement.parent : this.currentElement instanceof RecoveredMethod ? ((RecoveredMethod) this.currentElement).methodBody : null);
                        ...
                        if (parseTree != null) {
                            if ((parseTree.sourceStart == 0 || parseTree.sourceEnd == 0) || (parseTree.sourceStart >= statementStart && parseTree.sourceEnd <= statementEnd)) {
                                ...
                                if (local.localDeclaration != null && local.localDeclaration.initialization != null) {
                                    if ((local.localDeclaration.initialization.sourceStart == 0 || local.localDeclaration.initialization.sourceEnd == 0) || (local.localDeclaration.initialization.sourceStart >= statementStart && local.localDeclaration.initialization.sourceEnd <= statementEnd)) {
    ...
    protected void consumeOpenBlock() {
        ...
        if (requireExtendedRecovery()) {
            ...
            if (// wait for chain reductions to finish before commit.
            this.currentToken == TokenNameLBRACE && this.unstackedAct > NUM_RULES) {
                stackLength = this.stack.length;
                if (// Need two slots.
                ++this.stateStackTop >= stackLength - 1) {
                    System.arraycopy(this.stack, 0, this.stack = new int[stackLength + StackIncrement], 0, stackLength);
                }
                // transition to Block ::= OpenBlock  .LBRACE BlockStatementsopt RBRACE
                this.stack[this.stateStackTop++] = this.unstackedAct;
                // transition to Block ::= OpenBlock LBRACE  .BlockStatementsopt RBRACE 
	...
	stored in the identifier stack. */
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1, reference.annotations = new Annotation[length], 0, length);
        }
        /* build specific assist node on package statement */
        this.assistNode = reference;
        this.lastCheckPoint = reference.sourceEnd + 1;
 ...
 */
    protected TypeReference getTypeReference(int dim) {
        ...
        if (length != numberOfIdentifiers || this.genericsLengthStack[this.genericsLengthPtr] != 0) {
            ...
            if (index == 0) {
                ...
                reference = createQualifiedAssistTypeReference(subset, assistIdentifier(), positions);
            }
            this.assistNode = reference;
            this.lastCheckPoint = reference.sourceEnd + 1;
        }
    ...
    protected TypeReference getAssistTypeReferenceForGenericType(int dim, int identifierLength, int numberOfIdentifiers) {
        ...
        (identifierLength == 1 && numberOfIdentifiers == 1)) {
            ...
            System.arraycopy(this.identifierPositionStack, this.identifierPtr, positions, 0, identifierLength);
            this.identifierPtr--;
            TypeReference reference = createParameterizedSingleAssistTypeReference(typeArguments, assistIdentifier(), positions[0]);
            this.assistNode = reference;
            this.lastCheckPoint = reference.sourceEnd + 1;
            return reference;
        }
 ...
 */
    protected char[][] identifierSubSet(int subsetLength) {
        if (subsetLength == 0)
            return null;
        char[][] subset;
        System.arraycopy(this.identifierStack, this.identifierPtr - this.identifierLengthStack[this.identifierLengthPtr] + 1, (subset = new char[subsetLength][]), 0, subsetLength);
        return subset;
    }
 ...
 */
    ...
    protected void pushOnElementStack(int kind, int info, Object objectInfo) {
        ...
        if (++this.elementPtr >= stackLength) {
            ...
            System.arraycopy(this.elementObjectInfoStack, 0, this.elementObjectInfoStack = new Object[stackLength + StackIncrement], 0, stackLength);
        }
        ...
        switch(this.currentToken) {
            case TokenNameLBRACE:
                ...
                if (this.currentElement instanceof RecoveredInitializer) {
                    ...
                    if (this.currentElement != oldElement && topKnownElementKind(ASSIST_PARSER) != K_METHOD_DELIMITER) {
                        pushOnElementStack(K_METHOD_DELIMITER);
                    }
                }
                break;
            case TokenNameRBRACE:
                ...
                if (this.currentElement != oldElement && !isInsideAttributeValue() && !isIndirectlyInsideLambdaExpression()) {
                    if (oldElement instanceof RecoveredInitializer || oldElement instanceof RecoveredMethod || (oldElement instanceof RecoveredBlock && oldElement.parent instanceof RecoveredInitializer) || (oldElement instanceof RecoveredBlock && oldElement.parent instanceof RecoveredMethod)) {
                        popUntilElement(K_METHOD_DELIMITER);
                        popElement(K_METHOD_DELIMITER);
    ...
    protected int fallBackToSpringForward(Statement unused) {
        ...
        if (this.currentToken == TokenNameEOF) {
            if (this.scanner.eofPosition < this.scanner.source.length) {
                ...
                // spit out what has been bitten more than we can chew.
            ...
            if (nextToken == TokenNameRBRACE)
                // having ungotten it, recoveryTokenCheck will see this again. 
    ...
    protected int resumeAfterRecovery() {
        if (requireExtendedRecovery()) {
            ...
            // else fall through and RESTART
                ...
                if (this.currentToken == TokenNameLBRACE)
                    // already accounted for in recovery token check.
        ...
        if (this.unstackedAct != ERROR_ACTION && this.resumedAfterRepair) {
            // effectively move recovery checkpoint *backwards*.
 ...
 */
    protected ASTNode wrapWithExplicitConstructorCallIfNeeded(ASTNode ast) {
        int selector;
        if (ast != null && topKnownElementKind(ASSIST_PARSER) == K_SELECTOR && ast instanceof Expression && (((selector = topKnownElementInfo(ASSIST_PARSER)) == THIS_CONSTRUCTOR) || (selector == SUPER_CONSTRUCTOR))) {
