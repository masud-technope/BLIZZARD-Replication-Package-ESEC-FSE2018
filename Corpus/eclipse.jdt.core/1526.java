        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length; i < max; i++) {
            ...
            if (qualifyingType == null) {
                ...
                if (typeIsConsistent && currentType.isStatic() && (qualifyingType.isParameterizedTypeWithActualArguments() || qualifyingType.isGenericType())) {
                    ...
                    // avoid raw/parameterized enclosing of static member
                    qualifyingType = qualifyingType.actualType();
                }
                ReferenceBinding enclosingType = currentType.enclosingType();
                if (// qualifier != declaring/enclosing
                enclosingType != null && TypeBinding.notEquals(enclosingType.erasure(), qualifyingType.erasure())) {
                ...
                if (typeIsConsistent) {
                    if (currentType.isStatic()) {
                        if (qualifyingType != null && qualifyingType.isRawType())
                            // revert rawification of enclosing, since its generics are inaccessible
