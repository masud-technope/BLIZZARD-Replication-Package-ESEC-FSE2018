...
public abstract class FunctionalExpression extends Expression {
    ...
    // Code generation binding. May include synthetics. See getMethodBinding()
    ...
    // Whether the expression needs access to instance data of enclosing type
    ...
    public boolean isPolyExpression() {
        // always as per introduction of part D, JSR 335
        return true;
    }
    ...
    public boolean isPertinentToApplicability(TypeBinding targetType, MethodBinding method) {
        if (targetType instanceof TypeVariableBinding) {
            ...
            if (// when called from type inference
            method != null) {
        ...
        if (skipKosherCheck || kosherDescriptor(blockScope, sam, true)) {
            if (this.expectedType instanceof IntersectionTypeBinding18) {
                ...
                for (int t = 0, max = intersectingTypes.length; t < max; t++) {
                    if (intersectingTypes[t].findSuperTypeOriginatingFrom(TypeIds.T_JavaIoSerializable, /*Serializable is not a class*/
                    false) != null) {
                        this.isSerializable = true;
                        break;
                    }
                }
            } else if (this.expectedType.findSuperTypeOriginatingFrom(TypeIds.T_JavaIoSerializable, /*Serializable is not a class*/
            false) != null) {
                this.isSerializable = true;
            }
            ...
            void collectBridges(ReferenceBinding[] interfaces) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    for (int j = 0, count = methods == null ? 0 : methods.length; j < count; j++) {
                        ...
                        if (// descriptor declaring class may not be same functional interface target type.
                        inheritedMethod == null || this.method == inheritedMethod)
                            continue;
