...
public class CompletionParser extends AssistParser {
    ...
    protected static final int K_BLOCK_DELIMITER = COMPLETION_PARSER + 1;

    // whether we are inside a message send
    protected static final int K_SELECTOR_INVOCATION_TYPE = COMPLETION_PARSER + 2;

    // whether we are inside a message send
    protected static final int K_SELECTOR_QUALIFIER = COMPLETION_PARSER + 3;

    // whether we are between the keyword 'catch' and the following ')'
    protected static final int K_BETWEEN_CATCH_AND_RIGHT_PAREN = COMPLETION_PARSER + 4;

    // whether the next type reference is a class
    protected static final int K_NEXT_TYPEREF_IS_CLASS = COMPLETION_PARSER + 5;

    // whether the next type reference is an interface
    protected static final int K_NEXT_TYPEREF_IS_INTERFACE = COMPLETION_PARSER + 6;

    // whether the next type reference is an exception
    protected static final int K_NEXT_TYPEREF_IS_EXCEPTION = COMPLETION_PARSER + 7;

    // whether we are between the keyword 'new' and the following left braket, i.e. '[', '(' or '{'
    protected static final int K_BETWEEN_NEW_AND_LEFT_BRACKET = COMPLETION_PARSER + 8;

    // whether we are between the keyword 'throw' and the end of a throw statement
    protected static final int K_INSIDE_THROW_STATEMENT = COMPLETION_PARSER + 9;

    // whether we are between the keyword 'return' and the end of a return statement
    protected static final int K_INSIDE_RETURN_STATEMENT = COMPLETION_PARSER + 10;

    // whether we are between ')' and the end of a cast statement
    protected static final int K_CAST_STATEMENT = COMPLETION_PARSER + 11;
    ...
    public static final char[] FAKE_TYPE_NAME = new char[] { ' ' };

    public static final char[] FAKE_METHOD_NAME = new char[] { ' ' };

    public static final char[] FAKE_ARGUMENT_NAME = new char[] { ' ' };

    public static final char[] VALUE = new char[] { 'v', 'a', 'l', 'u', 'e' };
    ...
    // an enclosing node used by proposals inference
    ...
    private void addPotentialName(char[] potentialVariableName, int start, int end) {
        ...
        if (this.potentialVariableNamesPtr >= length - 1) {
            System.arraycopy(this.potentialVariableNames, 0, this.potentialVariableNames = new char[length * 2][], 0, length);
    ...
    protected void attachOrphanCompletionNode() {
        ...
        if (this.isOrphanCompletionNode) {
            ...
            if (this.currentElement instanceof RecoveredType) {
                ...
                if (recoveredType.foundOpeningBrace) {
                    ...
                    if (orphan instanceof TypeReference) {
                        ...
                        int length;
                        if ((length = this.expressionLengthStack[this.expressionLengthPtr]) != 0 && this.expressionStack[this.expressionPtr] instanceof Annotation) {
                            System.arraycopy(this.expressionStack, this.expressionPtr - length + 1, fieldDeclaration.annotations = new Annotation[length], 0, length);
                        }
            ...
            if (this.currentElement instanceof RecoveredMethod) {
                ...
                if (!recoveredMethod.foundOpeningBrace) {
                    ...
                    if (orphan instanceof TypeReference) {
                        this.currentElement = this.currentElement.parent.add(new CompletionOnFieldType((TypeReference) orphan, true), 0);
                        return;
                    }
                    if (orphan instanceof Annotation) {
                        CompletionOnAnnotationOfType fakeType = new CompletionOnAnnotationOfType(FAKE_TYPE_NAME, this.compilationUnit.compilationResult(), (Annotation) orphan);
            ...
            if ((topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BETWEEN_CATCH_AND_RIGHT_PAREN)) {
                if (this.assistNode instanceof CompletionOnSingleTypeReference && ((CompletionOnSingleTypeReference) this.assistNode).isException()) {
                    ...
                    return;
                } else if (this.assistNode instanceof CompletionOnQualifiedTypeReference && ((CompletionOnQualifiedTypeReference) this.assistNode).isException()) {
                    buildMoreTryStatementCompletionContext((TypeReference) this.assistNode);
                    return;
			 ...
			 */
                ...
                if (method != null) {
                    ...
                    if (// was missing opening brace
                    (methodDecl.bodyStart == methodDecl.sourceEnd + 1) && (Util.getLineNumber(orphan.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0, this.scanner.linePtr))) {
                        return;
                    }
                }
            ...
            if (this.expressionPtr > -1) {
                ...
                if (expression == this.assistNode) {
                    if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_MEMBER_VALUE_ARRAY_INITIALIZER) {
                        ...
                        MemberValuePair valuePair = new MemberValuePair(VALUE, expression.sourceStart, expression.sourceEnd, arrayInitializer);
                        ...
                        if (expression instanceof SingleNameReference) {
                            ...
                            MemberValuePair valuePair = new MemberValuePair(VALUE, expression.sourceStart, expression.sourceEnd, expression);
                            buildMoreAnnotationCompletionContext(valuePair);
                        }
                    } else {
                        int index;
                        if ((index = lastIndexOfElement(K_ATTRIBUTE_VALUE_DELIMITER)) != -1) {
                            ...
                            MemberValuePair memberValuePair = new MemberValuePair(this.identifierStack[this.identifierPtr--], expression.sourceStart, expression.sourceEnd, expression);
        ...
        if (this.genericsPtr > -1) {
			 ...
			 */
                ...
                if (method != null) {
                    AbstractMethodDeclaration methodDecl = method.methodDeclaration;
                    if (// was missing opening brace
                    (methodDecl.bodyStart == methodDecl.sourceEnd + 1) && (Util.getLineNumber(node.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0, this.scanner.linePtr))) {
                        return;
                    }
                }
                if (node == this.assistNode) {
        ...
        if (this.expressionPtr > -1) {
			 ...
			 */
                ...
                if (method != null) {
                    AbstractMethodDeclaration methodDecl = method.methodDeclaration;
                    if (// was missing opening brace
                    (methodDecl.bodyStart == methodDecl.sourceEnd + 1) && (Util.getLineNumber(expression.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util.getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0, this.scanner.linePtr))) {
                        return;
                    }
                }
                if (expression == this.assistNode || (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
                expression instanceof Assignment && ((Assignment) expression).expression == this.assistNode && ((this.expressionPtr > 0 && stackHasInstanceOfExpression(this.expressionStack, this.expressionPtr - 1)) || // In case of error in compilation unit, expression stack might not have instanceof exp, so try elementObjectInfoStack
                (this.elementPtr >= 0 && stackHasInstanceOfExpression(this.elementObjectInfoStack, this.elementPtr)))) || (expression instanceof AllocationExpression && ((AllocationExpression) expression).type == this.assistNode) || (expression instanceof AND_AND_Expression && (this.elementPtr >= 0 && this.elementObjectInfoStack[this.elementPtr] instanceof InstanceOfExpression))) {
        ...
        if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
        this.astPtr > -1 && this.astStack[this.astPtr] instanceof LocalDeclaration) {
    ...
    private void buildMoreAnnotationCompletionContext(MemberValuePair memberValuePair) {
        ...
        if (memberValuePair instanceof CompletionOnMemberValueName) {
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
                if (this.astStack[this.astPtr] instanceof MemberValuePair) {
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1, memberValuePairs = new MemberValuePair[length - nodesToRemove], 0, length - nodesToRemove);
                }
            }
            annotation = new CompletionOnAnnotationMemberValuePair(typeReference, this.intStack[this.intPtr--], memberValuePairs, memberValuePair);
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
                if (this.astStack[this.astPtr] instanceof MemberValuePair) {
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1, memberValuePairs = new MemberValuePair[length - nodesToRemove + 1], 0, length - nodesToRemove);
                }
        ...
        CompletionOnAnnotationOfType fakeType = new CompletionOnAnnotationOfType(FAKE_TYPE_NAME, this.compilationUnit.compilationResult(), annotation);
        this.currentElement.add(fakeType, 0);
        this.pendingAnnotation = fakeType;
    }
            ...
            nextElement: switch(kind) {
                case K_SELECTOR_QUALIFIER:
                    ...
                    if (selector == THIS_CONSTRUCTOR || selector == SUPER_CONSTRUCTOR) {
                        ExplicitConstructorCall call = new ExplicitConstructorCall((selector == THIS_CONSTRUCTOR) ? ExplicitConstructorCall.This : ExplicitConstructorCall.Super);
                ...
                case K_CAST_STATEMENT:
                    Expression castType;
                    if (this.expressionPtr > 0 && ((castType = this.expressionStack[this.expressionPtr - 1]) instanceof TypeReference)) {
                ...
                case K_ARRAY_INITIALIZER:
                    ...
                    if (this.expressionLengthPtr > -1 && this.expressionPtr > -1 && this.expressionStack[this.expressionPtr] != null && this.expressionStack[this.expressionPtr].sourceStart > info) {
                    ...
                    if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_ARRAY_CREATION) {
                        ...
                        allocationExpression.type.bits |= ASTNode.IgnoreRawTypeCheck;
                        int length = this.expressionLengthStack[this.expressionLengthPtr];
                ...
                case K_ASSISGNMENT_OPERATOR:
                    if (this.expressionPtr > 0 && this.expressionStack[this.expressionPtr - 1] != null) {
                        ...
                        if (info == EQUAL) {
                            assignment = new Assignment(this.expressionStack[this.expressionPtr - 1], expression, expression.sourceEnd);
                        } else {
                            assignment = new CompoundAssignment(this.expressionStack[this.expressionPtr - 1], expression, info, expression.sourceEnd);
                        }
                ...
                case K_BETWEEN_WHILE_AND_RIGHT_PAREN:
                    ...
                    break nextElement;
                case // https://bugs.eclipse.org/bugs/show_bug.cgi?id=253008
                K_INSIDE_FOR_CONDITIONAL:
                    ForStatement forStatement = new ForStatement(new Statement[0], expression, new Statement[0], new EmptyStatement(expression.sourceEnd, expression.sourceEnd), false, expression.sourceStart, expression.sourceEnd);
        ...
        while (index >= 0) {
            ...
            if (index != -1 && this.elementInfoStack[index] == IF && this.elementObjectInfoStack[index] != null) {
                ...
                if (this.currentElement instanceof RecoveredLocalVariable && this.currentElement.parent instanceof RecoveredBlock) {
                    RecoveredLocalVariable recoveredLocalVariable = (RecoveredLocalVariable) this.currentElement;
                    if (recoveredLocalVariable.localDeclaration.initialization == null && statement instanceof Expression && condition.sourceStart < recoveredLocalVariable.localDeclaration.sourceStart) {
                ...
                ifStatement = new IfStatement(condition, statement, condition.sourceStart, statement.sourceEnd);
                index--;
                break;
            }
            index--;
            ...
            nextElement: switch(kind) {
                case K_BINARY_OPERATOR:
                    ...
                    if (info == LESS && node instanceof TypeReference) {
                        if (this.identifierLengthPtr > -1 && this.identifierLengthStack[this.identifierLengthPtr] != 0) {
                            ...
                            if (this.currentElement instanceof RecoveredType) {
                                ...
                                if (prevKind == K_BETWEEN_NEW_AND_LEFT_BRACKET) {
                                    ...
                                    if (this.expressionPtr > -1 && this.expressionStack[this.expressionPtr] instanceof AllocationExpression && // https://bugs.eclipse.org/bugs/show_bug.cgi?id=361963
                        ...
                        } else if (this.currentElement.enclosingMethod() != null && this.currentElement.enclosingMethod().methodDeclaration.isConstructor()) {
                            this.currentElement = this.currentElement.add((TypeReference) node, 0);
                        }
                    }
                    break;
 ...
 */
    private boolean checkClassLiteralAccess() {
        if (// (NB: the top id length is 1 and it is for the completion identifier)
        this.identifierLengthPtr >= 1 && this.previousToken == TokenNameDOT) {
            int length;
    ...
    private boolean checkKeyword() {
        if (this.currentElement instanceof RecoveredUnit) {
            ...
            if ((index = this.indexOfAssistIdentifier()) > -1) {
                ...
                int count = 0;
                if (unit.typeCount == 0 && (!this.compilationUnit.isPackageInfo() || this.compilationUnit.currentPackage != null) && this.lastModifiers == ClassFileConstants.AccDefault) {
                    keywords[count++] = Keywords.IMPORT;
                }
                if (unit.typeCount == 0 && unit.importCount == 0 && this.lastModifiers == ClassFileConstants.AccDefault && this.compilationUnit.currentPackage == null) {
                ...
                if (!this.compilationUnit.isPackageInfo()) {
                    ...
                    if ((this.lastModifiers & ClassFileConstants.AccAbstract) == 0 && (this.lastModifiers & ClassFileConstants.AccFinal) == 0) {
    ...
    private boolean checkInstanceofKeyword() {
        if (isInsideMethod()) {
            int kind = topKnownElementKind(COMPLETION_OR_ASSIST_PARSER);
            int index;
            if (kind != K_BLOCK_DELIMITER && (index = indexOfAssistIdentifier()) > -1 && this.expressionPtr > -1 && this.expressionLengthStack[this.expressionPtr] == 1) {
                int ptr = this.identifierPtr - this.identifierLengthStack[this.identifierLengthPtr] + index + 1;
                if (this.identifierStack[ptr].length > 0 && CharOperation.prefixEquals(this.identifierStack[ptr], Keywords.INSTANCEOF)) {
                    this.assistNode = new CompletionOnKeyword3(this.identifierStack[ptr], this.identifierPositionStack[ptr], Keywords.INSTANCEOF);
                    this.lastCheckPoint = this.assistNode.sourceEnd + 1;
                    this.isOrphanCompletionNode = true;
                    return true;
                }
 ...
 */
    private boolean checkInvocation() {
        ...
        if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER && (// e.g. it is something like "this.fred([cursor]" but it is not something like "this.fred(1 + [cursor]"
        (isEmptyNameCompletion = topExpression == this.assistNode && isEmptyNameCompletion()) || // e.g. it is something like "this.fred(1 [cursor]"
        (isEmptyAssistIdentifier = this.indexOfAssistIdentifier() >= 0 && this.identifierStack[this.identifierPtr].length == 0))) {
            ...
            if (invocType != ALLOCATION && invocType != QUALIFIED_ALLOCATION) {
                ...
                messageSend.sourceStart = (int) (this.identifierPositionStack[selectorPtr] >> 32);
                ...
                if (selectorPtr == THIS_CONSTRUCTOR || selectorPtr == SUPER_CONSTRUCTOR) {
                    ...
                    CompletionOnExplicitConstructorCall call = new CompletionOnExplicitConstructorCall((selectorPtr == THIS_CONSTRUCTOR) ? ExplicitConstructorCall.This : ExplicitConstructorCall.Super);
    ...
    private boolean checkLabelStatement() {
        if (isInsideMethod() || isInsideFieldInitialization()) {
            ...
            CompletionOnBranchStatementLabel statementLabel = new CompletionOnBranchStatementLabel(kind == K_INSIDE_BREAK_STATEMENT ? CompletionOnBranchStatementLabel.BREAK : CompletionOnBranchStatementLabel.CONTINUE, this.identifierStack[this.identifierPtr--], (int) (position >>> 32), (int) position, labels);
 ...
 */
	...
	*/
    ...
    private boolean checkParemeterizedMethodName() {
        if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_PARAMETERIZED_METHOD_INVOCATION && topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER) == INSIDE_NAME) {
            if (this.identifierLengthPtr > -1 && this.genericsLengthPtr > -1 && this.genericsIdentifiersLengthPtr == -1) {
                ...
                switch(this.invocationType) {
                    case EXPLICIT_RECEIVER:
                    case // this case occurs with 'bar().foo'
                    NO_RECEIVER:
 ...
 */
    private boolean checkRecoveredMethod() {
        if (this.currentElement instanceof RecoveredMethod) {
            ...
            if (this.lastErrorEndPosition <= this.cursorLocation && Util.getLineNumber(this.lastErrorEndPosition, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util.getLineNumber(((CompletionScanner) this.scanner).completedIdentifierStart, this.scanner.lineEnds, 0, this.scanner.linePtr)) {
                return false;
            }
            RecoveredMethod recoveredMethod = (RecoveredMethod) this.currentElement;
            /* only consider if inside method header */
            if (!recoveredMethod.foundOpeningBrace && this.lastIgnoredToken == -1) {
                //if (rParenPos < lParenPos){ // inside arguments
 ...
 */
    private boolean checkRecoveredType() {
        if (this.currentElement instanceof RecoveredType) {
            ...
            if (recoveredType.foundOpeningBrace) {
                ...
                if (recoveredType.typeDeclaration.superclass == null && this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_EXTENDS_KEYWORD) {
    ...
    private void classHeaderExtendsOrImplements(boolean isInterface) {
        if (this.currentElement != null && this.currentToken == TokenNameIdentifier && this.cursorLocation + 1 >= this.scanner.startPosition && this.cursorLocation < this.scanner.currentPosition) {
            this.pushIdentifier();
            int index = -1;
            /* check if current awaiting identifier is the completion identifier */
            if ((index = this.indexOfAssistIdentifier()) > -1) {
                ...
                if (!recoveredType.foundOpeningBrace) {
                    ...
                    if (!isInterface) {
                        ...
                        System.arraycopy(keywords, 0, keywords = new char[count][], 0, count);
                        if (count > 0) {
                            CompletionOnKeyword1 completionOnKeyword = new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr], keywords);
                        ...
                        if (type.superInterfaces == null) {
                            CompletionOnKeyword1 completionOnKeyword = new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr], Keywords.EXTENDS);
 ...
 */
    public void completionIdentifierCheck() {
        ...
        /*
	 	...
	 	(that is, pushed).
	 */
        if (// Test below copied from CompletionScanner.getCurrentIdentifierSource()
        assistIdentifier() == null && this.currentToken == TokenNameIdentifier) {
            if (// fake empty identifier got issued
            this.cursorLocation < this.scanner.startPosition && this.scanner.currentPosition == this.scanner.startPosition) {
                this.pushIdentifier();
    ...
    protected void consumeCatchFormalParameter() {
        if (this.indexOfAssistIdentifier() < 0) {
            ...
            // dimension from the variabledeclaratorid
            this.intPtr--;
            TypeReference type = (TypeReference) this.astStack[this.astPtr--];
            this.intPtr -= 2;
            CompletionOnArgumentName arg = new CompletionOnArgumentName(identifierName, namePositions, type, this.intStack[this.intPtr + 1] & ~// modifiers
        ...
        if (this.currentElement != null && this.currentToken == TokenNameIdentifier && this.cursorLocation + 1 >= this.scanner.startPosition && this.cursorLocation < this.scanner.currentPosition) {
            ...
            if ((index = this.indexOfAssistIdentifier()) > -1) {
                ...
                if (!recoveredType.foundOpeningBrace) {
                    TypeDeclaration type = recoveredType.typeDeclaration;
                    if (type.superInterfaces == null) {
                        type.superclass = new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr], Keywords.IMPLEMENTS);
                        type.superclass.bits |= ASTNode.IsSuperType;
 ...
 */
    ...
    protected void consumeConstructorHeaderName() {
        ...
        if (indexOfAssistIdentifier() < 0) {
            ...
            this.currentElement != null && this.lastIgnoredToken == TokenNamenew) {
            ...
            if (// if ast node was pushed on the ast stack
            this.sourceEnds != null && this.astPtr > currentAstPtr) {
	...
	*/
        ...
        if (isLocalDeclaration || indexOfAssistIdentifier() < 0 || variableIndex != 0) {
            ...
            if (this.currentElement != null) {
                if (!checkKeyword() && !(this.currentElement instanceof RecoveredUnit && ((RecoveredUnit) this.currentElement).typeCount == 0)) {
                    ...
                    this.intPtr--;
                    if (!(this.currentElement instanceof RecoveredType) && (this.currentToken == TokenNameDOT || (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util.getLineNumber(nameSourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr)))) {
                    ...
                    int length;
                    if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
                        System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1, completionFieldDecl.annotations = new Annotation[length], 0, length);
                    }
    ...
    protected void consumeEnumConstantNoClassBody() {
        ...
        if ((this.currentToken == TokenNameCOMMA || this.currentToken == TokenNameSEMICOLON) && this.astStack[this.astPtr] instanceof FieldDeclaration) {
    ...
    protected void consumeExitVariableWithInitialization() {
        ...
        if (this.cursorLocation + 1 < variable.initialization.sourceStart || this.cursorLocation > variable.initialization.sourceEnd) {
 ...
 */
    ...
    protected void consumeMethodHeaderName(boolean isAnnotationMethod) {
        if (this.indexOfAssistIdentifier() < 0) {
            this.identifierPtr--;
            this.identifierLengthPtr--;
            if (this.indexOfAssistIdentifier() != 0 || this.identifierLengthStack[this.identifierLengthPtr] != this.genericsIdentifiersLengthStack[this.genericsIdentifiersLengthPtr]) {
                this.identifierPtr++;
                this.identifierLengthPtr++;
                long selectorSourcePositions = this.identifierPositionStack[this.identifierPtr];
                int selectorSourceEnd = (int) selectorSourcePositions;
                ...
                if (this.currentElement != null) {
                    ...
                    if (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util.getLineNumber((int) (selectorSource >>> 32), this.scanner.lineEnds, 0, this.scanner.linePtr)) {
                        FieldDeclaration completionFieldDecl = new CompletionOnFieldType(type, false);
                        // consume annotations
                        int length;
                        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
                ...
                if (this.currentElement instanceof RecoveredMethod) {
                    ...
                    if (!recoveredMethod.foundOpeningBrace) {
                        ...
                        if (method.thrownExceptions == null) {
                            CompletionOnKeyword1 completionOnKeyword = new CompletionOnKeyword1(this.identifierStack[ptr], this.identifierPositionStack[ptr], Keywords.THROWS);
    ...
    protected void consumeMarkerAnnotation(boolean isTypeAnnotation) {
        if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN && (this.topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER) & ANNOTATION_NAME_COMPLETION) != 0) {
    ...
    protected void consumeToken(int token) {
        ...
        if (isInsideMethod() || isInsideFieldInitialization() || isInsideAnnotation()) {
            switch(token) {
                case TokenNameLPAREN:
                    if (previous == TokenNameIdentifier && topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_PARAMETERIZED_METHOD_INVOCATION) {
                ...
                case TokenNameRBRACE:
                    ...
                    switch(kind) {
                        ...
                        case K_LAMBDA_EXPRESSION_DELIMITER:
                            // will be popped when the containing block statement is reduced.
                            break;
                        default:
        ...
        super.consumeToken(token);
        // in method bodies)
        if (token == TokenNameIdentifier && this.identifierStack[this.identifierPtr] == assistIdentifier() && this.currentElement == null && (!isIndirectlyInsideLambdaExpression() || isIndirectlyInsideLambdaBlock()) && isIndirectlyInsideFieldInitialization()) {
            ...
            switch(token) {
                case TokenNameDOT:
                    switch(previous) {
                        ...
                        TokenNameIdentifier:
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) != K_BETWEEN_NEW_AND_LEFT_BRACKET) {
                                if (// if identifier has been consumed, e.g. this.x[.]fred()
                                this.identifierPtr != prevIdentifierPtr) {
                                    this.invocationType = EXPLICIT_RECEIVER;
                                } else {
                ...
                case TokenNameIdentifier:
                    if (this.inReferenceExpression)
                        break;
                    if (// e.g. foo().[fred]()
                    previous == TokenNameDOT) {
                        if (// e.g. not super.[fred]()
                        this.invocationType != SUPER_RECEIVER && // e.g. not bar.[fred]()
                        this.invocationType != NAME_RECEIVER && // e.g. not new foo.[Bar]()
                        this.invocationType != ALLOCATION && // e.g. not fred().new foo.[Bar]()
                        this.invocationType != QUALIFIED_ALLOCATION) {
                            this.invocationType = EXPLICIT_RECEIVER;
                            this.qualifier = this.expressionPtr;
                        }
                    ...
                    if (// e.g. foo().<X>[fred]()
                    previous == TokenNameGREATER) {
                        if (// e.g. not super.<X>[fred]()
                        this.invocationType != SUPER_RECEIVER && // e.g. not bar.<X>[fred]()
                        this.invocationType != NAME_RECEIVER && // e.g. not new foo.<X>[Bar]()
                        this.invocationType != ALLOCATION && // e.g. not fred().new foo.<X>[Bar]()
                        this.invocationType != QUALIFIED_ALLOCATION) {
                ...
                case TokenNameLPAREN:
                    ...
                    switch(previous) {
                        case // e.g. fred[(]) or foo.fred[(])
                        TokenNameIdentifier:
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                                int info = 0;
                                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN && (info = topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER, 1) & LPAREN_NOT_CONSUMED) != 0) {
                            ...
                            this.invocationType = NO_RECEIVER;
                            break;
                        case // explicit constructor invocation, e.g. this[(]1, 2)
                        TokenNamethis:
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                            ...
                            this.invocationType = NO_RECEIVER;
                            break;
                        case // explicit constructor invocation, e.g. super[(]1, 2)
                        TokenNamesuper:
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                                this.pushOnElementStack(K_SELECTOR_INVOCATION_TYPE, (this.invocationType == QUALIFIED_ALLOCATION) ? QUALIFIED_ALLOCATION : ALLOCATION);
                                this.pushOnElementStack(K_SELECTOR_QUALIFIER, this.qualifier);
                            ...
                            this.invocationType = NO_RECEIVER;
                            break;
                        // explicit constructor invocation, e.g. Fred<X>[(]1, 2)
                        case TokenNameGREATER:
                        // or fred<X<X>>[(]1, 2)
                        case TokenNameRIGHT_SHIFT:
                        case //or Fred<X<X<X>>>[(]1, 2)
                        TokenNameUNSIGNED_RIGHT_SHIFT:
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                                int info;
                                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BINARY_OPERATOR && ((info = topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER, 1)) == GREATER || info == RIGHT_SHIFT || info == UNSIGNED_RIGHT_SHIFT)) {
                ...
                case TokenNameLBRACE:
                    int kind = topKnownElementKind(COMPLETION_OR_ASSIST_PARSER);
                    if (kind == K_FIELD_INITIALIZER_DELIMITER || kind == K_LOCAL_INITIALIZER_DELIMITER || kind == K_ARRAY_CREATION) {
    ...
    protected void consumeInvocationExpression() {
        // on error, a message send's error reductions will take the expression path rather than the statement path since that is a dead end.
    ...
    protected void consumeTypeParameterHeader() {
        ...
        CompletionOnKeyword1 keyword = new CompletionOnKeyword1(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr], Keywords.EXTENDS);
    ...
    protected void consumeWildcard() {
        ...
        CompletionOnKeyword1 keyword = new CompletionOnKeyword1(this.identifierStack[this.identifierPtr], this.identifierPositionStack[this.identifierPtr], new char[][] { Keywords.EXTENDS, Keywords.SUPER });
    ...
    public NameReference createQualifiedAssistNameReference(char[][] previousIdentifiers, char[] assistName, long[] positions) {
        return new CompletionOnQualifiedNameReference(previousIdentifiers, assistName, positions, isInsideAttributeValue());
    }
        ...
        switch(topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
            case K_NEXT_TYPEREF_IS_EXCEPTION:
                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BETWEEN_CATCH_AND_RIGHT_PAREN)
                    this.isOrphanCompletionNode = true;
                return new CompletionOnQualifiedTypeReference(previousIdentifiers, assistName, positions, CompletionOnQualifiedTypeReference.K_EXCEPTION);
            case K_NEXT_TYPEREF_IS_CLASS:
                return new CompletionOnQualifiedTypeReference(previousIdentifiers, assistName, positions, CompletionOnQualifiedTypeReference.K_CLASS);
            case K_NEXT_TYPEREF_IS_INTERFACE:
                return new CompletionOnQualifiedTypeReference(previousIdentifiers, assistName, positions, CompletionOnQualifiedTypeReference.K_INTERFACE);
            default:
        ...
        if (!isParameterized) {
            ...
            switch(topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
                case K_NEXT_TYPEREF_IS_EXCEPTION:
                    if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1) == K_BETWEEN_CATCH_AND_RIGHT_PAREN)
                        this.isOrphanCompletionNode = true;
                    return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments, assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_EXCEPTION);
                case K_NEXT_TYPEREF_IS_CLASS:
                    return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments, assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_CLASS);
                case K_NEXT_TYPEREF_IS_INTERFACE:
                    return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments, assistName, positions, CompletionOnParameterizedQualifiedTypeReference.K_INTERFACE);
                default:
                    return new CompletionOnParameterizedQualifiedTypeReference(previousIdentifiers, typeArguments, assistName, positions);
            }
        }
    }
    ...
    public NameReference createSingleAssistNameReference(char[] assistName, long position) {
        ...
        if (!isInsideMethod()) {
            if (isInsideFieldInitialization()) {
                return new CompletionOnSingleNameReference(assistName, position, new char[][] { Keywords.FALSE, Keywords.TRUE }, false, isInsideAttributeValue());
            }
            ...
            if ((kind == K_BLOCK_DELIMITER || kind == K_LAMBDA_EXPRESSION_DELIMITER) && this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == DO) {
                return new CompletionOnKeyword3(assistName, position, Keywords.WHILE);
            } else if ((kind == K_BLOCK_DELIMITER || kind == K_LAMBDA_EXPRESSION_DELIMITER) && this.previousKind == K_BLOCK_DELIMITER && this.previousInfo == TRY) {
    ...
    protected StringLiteral createStringLiteral(char[] token, int start, int end, int lineNumber) {
        if (start <= this.cursorLocation && this.cursorLocation <= end) {
            ...
            if (this.cursorLocation != end || end == contentEnd) {
                CompletionOnStringLiteral stringLiteral = new CompletionOnStringLiteral(token, start, end, contentStart, contentEnd, lineNumber);
    ...
    public MethodDeclaration parseSomeStatements(int start, int end, int fakeBlocksCount, CompilationUnitDeclaration unit) {
        ...
        if (!this.hasError) {
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) != 0) {
                System.arraycopy(this.astStack, (this.astPtr -= length) + 1, fakeMethod.statements = new Statement[length], 0, length);
            }
        }
        return fakeMethod;
    }
    ...
    protected void popUntilCompletedAnnotationIfNecessary() {
        if (this.elementPtr < 0)
            return;
        int i = this.elementPtr;
        while (i > -1 && (this.elementKindStack[i] != K_BETWEEN_ANNOTATION_NAME_AND_RPAREN || (this.elementInfoStack[i] & ANNOTATION_NAME_COMPLETION) == 0)) {
            i--;
        }
        if (i >= 0) {
    ...
    protected void pushOnLabelStack(char[] label) {
        ...
        if (++this.labelPtr >= stackLength) {
            System.arraycopy(this.labelStack, 0, this.labelStack = new char[stackLength + LabelStackIncrement][], 0, stackLength);
        }
    ...
    public void recoveryTokenCheck() {
        ...
        switch(this.currentToken) {
            ...
            case TokenNamecase:
                super.recoveryTokenCheck();
                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BLOCK_DELIMITER && topKnownElementInfo(COMPLETION_OR_ASSIST_PARSER) == SWITCH) {
                    pushOnElementStack(K_SWITCH_LABEL);
                }
                break;
            case TokenNamedefault:
