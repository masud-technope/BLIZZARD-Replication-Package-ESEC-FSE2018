...
public class CommentWrapExecutor extends TokenTraverser {
    ...
    protected boolean token(Token token, int index) {
        ...
        if (getLineBreaksBefore() > 0) {
            ...
            this.potentialWrapToken = this.potentialWrapTokenSubstitute = null;
            boolean isFormattedCode = token.getWrapPolicy() != null && token.getWrapPolicy() != WrapPolicy.SUBSTITUTE_ONLY;
        ...
        boolean canWrap = getNext() != null && getLineBreaksBefore() == 0 && index > 1 && positionIfNewLine < this.counter;
        if (canWrap) {
    ...
    private boolean shouldWrap() {
        ...
        if (this.potentialWrapToken == null && this.potentialWrapTokenSubstitute == null) {
            boolean isFormattingEnabled = this.blockStructure.size() > 1 && this.blockStructure.get(1).tokenType == TokenNameNotAToken;
        ...
        if (this.options.comment_new_lines_at_javadoc_boundaries) {
            if (// the closing token will go to the next line anyway
            getNext() == null) {
                this.lineCounter = Math.max(this.lineCounter, 3);
                return false;
            }
    ...
    public void wrapLineComment(Token commentToken, int startPosition) {
        ...
        if (!prefix.hasNLSTag())
            // comments can start with more than 2 slashes
