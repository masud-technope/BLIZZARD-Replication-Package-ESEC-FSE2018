 ...
 */
    ...
    protected ISchedulingRule getSchedulingRule() {
        if (this.pkgName.length == 0)
            // no resource is going to be created
            return null;
        ...
        for (i = 0; i < this.pkgName.length; i++) {
            ...
            if (subFolder != null) {
                if (subFolder.getType() != IResource.FOLDER) {
                    return new JavaModelStatus(IJavaModelStatusConstants.NAME_COLLISION, Messages.bind(Messages.status_nameCollision, subFolder.getFullPath().toString()));
                }
                parentFolder = (IContainer) subFolder;
            }
        }
