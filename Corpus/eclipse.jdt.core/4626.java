...
public class DefaultCodeFormatter extends CodeFormatter {
    ...
    private static final int K_MASK = K_UNKNOWN | K_EXPRESSION | K_STATEMENTS | K_CLASS_BODY_DECLARATIONS | K_COMPILATION_UNIT | K_COMMENTS_MASK;
    ...
    private ASTNode parseSourceCode(int kind) {
        ...
        switch(kind & K_MASK) {
            ...
            case K_UNKNOWN:
                int[] parserModes = { ASTParser.K_COMPILATION_UNIT, ASTParser.K_EXPRESSION, ASTParser.K_CLASS_BODY_DECLARATIONS, ASTParser.K_STATEMENTS };
                for (int parserMode : parserModes) {
                    ASTNode astNode = parseSourceCode(parser, parserMode, false);
                    if (astNode != null)
                        return astNode;
                    // parser loses compiler options after every use
                    parser.setCompilerOptions(parserOptions);
                }
                return null;
            default:
    ...
    private void tokenizeSource() {
        this.tokens.clear();
        Scanner scanner = new Scanner(true, false, /* nls */
        false, CompilerOptions.versionToJdkLevel(this.sourceLevel), /* taskTags */
        null, /* taskPriorities */
    ...
    private void updateWorkingOptions(int indentationLevel, String lineSeparator, int kind) {
        this.workingOptions.line_separator = lineSeparator != null ? lineSeparator : this.originalOptions.line_separator;
        ...
        this.workingOptions.comment_format_javadoc_comment = this.originalOptions.comment_format_javadoc_comment && canFormatComment(kind, K_JAVA_DOC);
        this.workingOptions.comment_format_block_comment = this.originalOptions.comment_format_block_comment && canFormatComment(kind, K_MULTI_LINE_COMMENT);
        this.workingOptions.comment_format_line_comment = this.originalOptions.comment_format_line_comment && canFormatComment(kind, K_SINGLE_LINE_COMMENT);
    }
