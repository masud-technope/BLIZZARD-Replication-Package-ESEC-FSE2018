...
public class AbstractCompilerToolTest extends BatchCompilerTest {
    ...
    protected boolean invokeCompiler(PrintWriter out, PrintWriter err, Object extraArguments, TestCompilationProgress compilationProgress) {
        ...
        try {
            ...
            CompilationTask task = COMPILER.getTask(out, arguments.standardJavaFileManager, /* carry the null over */
            new CompilerInvocationDiagnosticListener(err), arguments.options, null, manager.getJavaFileObjectsFromFiles(files));
            return task.call();
        } finally {
            try {
                if (ownsManager)
