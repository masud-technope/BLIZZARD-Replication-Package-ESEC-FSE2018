...
public final class SelectionEngine extends Engine implements ISearchRequestor {
        ...
        public void initializeImportNodeCaches() {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if ((importReference.bits & ASTNode.OnDemand) != 0) {
                    ...
                    this.importsNodeCache[this.importsNodeCacheCount++] = new char[][] { importReference.tokens[importReference.tokens.length - 1], CharOperation.concatWith(importReference.tokens, '.') };
                }
            }
    ...
    public void acceptType(char[] packageName, char[] simpleTypeName, char[][] enclosingTypeNames, int modifiers, AccessRestriction accessRestriction) {
        char[] typeName = enclosingTypeNames == null ? simpleTypeName : CharOperation.concat(CharOperation.concatWith(enclosingTypeNames, '.'), simpleTypeName, '.');
        if (CharOperation.equals(simpleTypeName, this.selectedIdentifier)) {
            char[] flatEnclosingTypeNames = enclosingTypeNames == null || enclosingTypeNames.length == 0 ? null : CharOperation.concatWith(enclosingTypeNames, '.');
    ...
    private boolean checkSelection(char[] source, int selectionStart, int selectionEnd) {
        ...
        false, /*whitespace*/
        false, /*nls*/
        false, this.compilerOptions.sourceLevel, this.compilerOptions.complianceLevel, /*taskTag*/
        null, /*taskPriorities*/
        ...
        if (selectionStart > selectionEnd) {
            ...
            try {
                lineLoop: while (currentPosition > 0) {
                    if (source[currentPosition] == '\\' && source[currentPosition + 1] == 'u') {
                        ...
                        if ((c1 = ScannerHelper.getHexadecimalValue(source[pos++])) > 15 || c1 < 0 || (c2 = ScannerHelper.getHexadecimalValue(source[pos++])) > 15 || c2 < 0 || (c3 = ScannerHelper.getHexadecimalValue(source[pos++])) > 15 || c3 < 0 || (c4 = ScannerHelper.getHexadecimalValue(source[pos++])) > 15 || c4 < 0) {
                            return false;
                        } else {
            ...
            if (selectionStart == selectionEnd) {
                if (selectionStart > 0 && selectionEnd < source.length - 1) {
                    if ((source[selectionStart] == '>' && source[selectionStart - 1] == '-') || source[selectionStart] == ':' && source[selectionStart - 1] == ':') {
                        selectionStart--;
                    } else {
                        if ((source[selectionStart] == '-' && source[selectionEnd + 1] == '>') || source[selectionStart] == ':' && source[selectionEnd + 1] == ':') {
                            selectionEnd++;
                        }
                    }
                }
    ...
    private void findAllTypes(char[] prefix) {
        try {
            ...
            SelectionTypeNameMatchRequestorWrapper requestorWrapper = new SelectionTypeNameMatchRequestorWrapper(typeNameMatchRequestor, scope, this.unitScope == null ? null : this.unitScope.referenceContext.imports);
            org.eclipse.jdt.core.ICompilationUnit[] workingCopies = this.owner == null ? null : JavaModelManager.getJavaModelManager().getWorkingCopies(this.owner, true);
            try {
                new BasicSearchEngine(workingCopies).searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH, prefix, SearchPattern.R_EXACT_MATCH | SearchPattern.R_CASE_SENSITIVE, IJavaSearchConstants.TYPE, scope, requestorWrapper, IJavaSearchConstants.CANCEL_IF_NOT_READY_TO_SEARCH, progressMonitor);
    ...
    private void selectFrom(Binding binding, CompilationUnitDeclaration parsedUnit, ICompilationUnit unit, boolean isDeclaration) {
        if (binding instanceof TypeVariableBinding) {
            ...
            if (binding instanceof SyntheticMethodBinding) {
                ...
                if (syntheticMethodBinding.purpose == SyntheticMethodBinding.EnumValues || syntheticMethodBinding.purpose == SyntheticMethodBinding.EnumValueOf) {
            ...
            if (!isValuesOrValueOf && !methodBinding.isSynthetic()) {
                ...
                if (isLocal(declaringClass) && this.requestor instanceof SelectionRequestor) {
                    ...
                    this.requestor.acceptMethod(declaringClass.qualifiedPackageName(), declaringClass.qualifiedSourceName(), declaringClass.enclosingType() == null ? null : new String(getSignature(declaringClass.enclosingType())), methodBinding.isConstructor() ? declaringClass.sourceName() : methodBinding.selector, parameterPackageNames, parameterTypeNames, parameterSignatures, typeParameterNames, typeParameterBoundNames, methodBinding.isConstructor(), isDeclaration, methodBinding.computeUniqueKey(), this.actualSelectionStart, this.actualSelectionEnd);
                }
            }
            this.acceptedAnswer = true;
            ...
            declaringClass != null) {
                ...
                if (isLocal(declaringClass) && this.requestor instanceof SelectionRequestor) {
                    ...
                    this.requestor.acceptField(declaringClass.qualifiedPackageName(), declaringClass.qualifiedSourceName(), fieldName, false, key, this.actualSelectionStart, this.actualSelectionEnd);
                }
                this.acceptedAnswer = true;
            }
    ...
    public void selectType(char[] typeName, IType context) throws JavaModelException {
        try {
            ...
            if (CharOperation.indexOf('<', typeName) != -1) {
                char[] typeSig = Signature.createCharArrayTypeSignature(/*not resolved*/
                typeName, false);
            ...
            if (cu != null) {
                ...
                parsedUnit = SourceTypeConverter.buildCompilationUnit(topLevelInfos, flags, this.parser.problemReporter(), result);
            ...
            } else // binary type
            {
                ...
                ClassFileReader reader = (ClassFileReader) classFile.getBinaryTypeInfo((IFile) classFile.resource(), /*don't fully initialize so as to keep constant pool (used below)*/
                false);
    ...
    private boolean selectDeclaration(TypeDeclaration typeDeclaration, char[] assistIdentifier, char[] packageName) {
        ...
        for (int i = 0, length = methods == null ? 0 : methods.length; i < length; i++) {
            ...
            if (method.selector == assistIdentifier) {
                ...
                this.requestor.acceptMethod(packageName, qualifiedSourceName, // SelectionRequestor does not need of declaring type signature for method declaration
                null, method.selector, // SelectionRequestor does not need of parameters type for method declaration
                ...
                null, method.isConstructor(), true, method.binding != null ? method.binding.computeUniqueKey() : null, this.actualSelectionStart, this.actualSelectionEnd);
                this.noProposal = false;
                return true;
            }
        ...
        return new InheritDocVisitor() {
            ...
            public Object visit(ReferenceBinding currType) throws JavaModelException {
                ...
                if (javadocRange == null)
                    // this method doesn't have javadoc, continue to look.
                    return InheritDocVisitor.CONTINUE;
	 ...
	 * </p>
	 */
    abstract static class InheritDocVisitor {
