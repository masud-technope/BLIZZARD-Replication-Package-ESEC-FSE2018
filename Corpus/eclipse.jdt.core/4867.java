 ...
 */
        ...
        try {
            if (resource != null && resource.exists()) {
                ...
                if (markerTypes.size() == 0)
                    return;
                Iterator iterator = markerTypes.iterator();
                while (iterator.hasNext()) resource.deleteMarkers((String) iterator.next(), false, IResource.DEPTH_INFINITE);
            }
        } catch (CoreException e) {
        }
    }
    ...
    protected IProject[] build(int kind, Map ignored, IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            if (isWorthBuilding()) {
                if (kind == FULL_BUILD) {
                    ...
                    if ((this.lastState = getLastState(this.currentProject)) == null) {
                        ...
                        if (// double check that a jar file didn't get replaced in a binary project
                        hasStructuralDelta()) {
                            if (DEBUG)
            ...
            marker.setAttributes(new String[] { IMarker.MESSAGE, IMarker.SEVERITY, IMarker.SOURCE_ID }, new Object[] { Messages.bind(Messages.build_missingSourceFile, e.missingSourceFile), Integer.valueOf(IMarker.SEVERITY_ERROR), JavaBuilder.SOURCE_ID });
        } finally {
            for (int i = 0, l = this.participants == null ? 0 : this.participants.length; i < l; i++) this.participants[i].buildFinished(this.javaProject);
            if (!ok)
        ...
        if (DEBUG)
            System.out.println(//$NON-NLS-1$
            "JavaBuilder: Finished build of " + this.currentProject.getName() + " @ " + new Date(System.currentTimeMillis()) + //$NON-NLS-1$ //$NON-NLS-2$
            "\n");
    ...
    protected void clean(IProgressMonitor monitor) throws CoreException {
        ...
        if (DEBUG)
            System.out.println(//$NON-NLS-1$
            "JavaBuilder: Finished cleaning " + this.currentProject.getName() + " @ " + //$NON-NLS-1$
            new Date(System.currentTimeMillis()));
    }
    ...
    private void createInconsistentBuildMarker(CoreException coreException) throws CoreException {
        ...
        IMarker marker = this.currentProject.createMarker(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER);
        marker.setAttributes(new String[] { IMarker.MESSAGE, IMarker.SEVERITY, IJavaModelMarker.CATEGORY_ID, IMarker.SOURCE_ID }, new Object[] { Messages.bind(Messages.build_inconsistentProject, message), Integer.valueOf(IMarker.SEVERITY_ERROR), Integer.valueOf(CategorizedProblem.CAT_BUILDPATH), JavaBuilder.SOURCE_ID });
    }
    ...
    boolean filterExtraResource(IResource resource) {
        if (this.extraResourceFileFilters != null) {
            ...
            for (int i = 0, l = this.extraResourceFileFilters.length; i < l; i++) if (CharOperation.match(this.extraResourceFileFilters[i], name, true))
                return true;
        }
        if (this.extraResourceFolderFilters != null) {
            ...
            if (resource.getType() == IResource.FILE)
                count--;
            for (int i = 0, l = this.extraResourceFolderFilters.length; i < l; i++) if (pathName.indexOf(this.extraResourceFolderFilters[i]) != -1)
                for (int j = 0; j < count; j++) if (this.extraResourceFolderFilters[i].equals(path.segment(j)))
                    return true;
        }
        return false;
    }
    ...
    private SimpleLookupTable findDeltas() {
        ...
        if (delta != null) {
            if (delta.getKind() != IResourceDelta.NO_CHANGE) {
                if (DEBUG)
                    //$NON-NLS-1$
                    System.out.println(//$NON-NLS-1$
                    "JavaBuilder: Found source delta for: " + this.currentProject.getName());
        ...
        nextProject: for (int i = 0, l = keyTable.length; i < l; i++) {
            ...
            if (p != null && p != this.currentProject) {
                ...
                if (// see if we can skip its delta
                !this.lastState.wasStructurallyChanged(p, s)) {
                    if (s.wasNoopBuild())
                ...
                if (delta != null) {
                    if (delta.getKind() != IResourceDelta.NO_CHANGE) {
                        if (DEBUG)
                            System.out.println(//$NON-NLS-1$
                            "JavaBuilder: Found binary delta for: " + //$NON-NLS-1$
                            p.getName());
                        deltas.put(p, delta);
                    }
                } else {
                    if (DEBUG)
                        //$NON-NLS-1$
                        System.out.println(//$NON-NLS-1$
                        "JavaBuilder: Missing delta for: " + p.getName());
                    //$NON-NLS-1$
...
*/
            ...
            for (int i = 0, l = entries.length; i < l; i++) {
                ...
                switch(entry.getEntryKind()) {
                    case IClasspathEntry.CPE_PROJECT:
                        ...
                        if (// except if entry is optional
                        ((ClasspathEntry) entry).isOptional() && !JavaProject.hasJavaNature(p))
                            p = null;
                        break;
    ...
    boolean hasBuildpathErrors() throws CoreException {
        ...
        for (int i = 0, l = markers.length; i < l; i++) if (markers[i].getAttribute(IJavaModelMarker.CATEGORY_ID, -1) == CategorizedProblem.CAT_BUILDPATH)
            return true;
        return false;
    }
    ...
    private boolean hasClasspathChanged() {
        ...
        for (n = o = 0; n < newLength && o < oldLength; n++, o++) {
            ...
            if (newSourceLocations[n].equals(oldSourceLocations[o]))
                continue;
            try {
                if (// added new empty source folder
                newSourceLocations[n].sourceFolder.members().length == 0) {
                    o--;
                    continue;
    ...
    private boolean hasJavaBuilder(IProject project) throws CoreException {
        ...
        for (int i = 0, l = buildCommands.length; i < l; i++) if (buildCommands[i].getBuilderName().equals(JavaCore.BUILDER_ID))
            return true;
        return false;
    }
    ...
    private int initializeBuilder(int kind, boolean forBuild) throws CoreException {
        ...
        if (forBuild) {
            ...
            if (this.participants != null)
                for (int i = 0, l = this.participants.length; i < l; i++) if (this.participants[i].aboutToBuild(this.javaProject) == CompilationParticipant.NEEDS_FULL_BUILD)
                    kind = FULL_BUILD;
    ...
    private boolean isWorthBuilding() throws CoreException {
        ...
        if (isClasspathBroken(this.javaProject, true)) {
            ...
            removeProblemsAndTasksFor(this.currentProject);
            IMarker marker = this.currentProject.createMarker(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER);
            marker.setAttributes(new String[] { IMarker.MESSAGE, IMarker.SEVERITY, IJavaModelMarker.CATEGORY_ID, IMarker.SOURCE_ID }, new Object[] { Messages.build_abortDueToClasspathProblems, Integer.valueOf(IMarker.SEVERITY_ERROR), Integer.valueOf(CategorizedProblem.CAT_BUILDPATH), JavaBuilder.SOURCE_ID });
            return false;
        }
        ...
        for (int i = 0, l = requiredProjects.length; i < l; i++) {
            ...
            if (getLastState(p) == null) {
                ...
                if (prereq.hasCycleMarker() && JavaCore.WARNING.equals(this.javaProject.getOption(JavaCore.CORE_CIRCULAR_CLASSPATH, true))) {
                    if (DEBUG)
                        System.out.println(//$NON-NLS-1$
                        "JavaBuilder: Continued to build even though prereq project " + //$NON-NLS-1$
                        p.getName() + " was not built since its part of a cycle");
                    continue;
                }
                if (!hasJavaBuilder(p)) {
                    if (DEBUG)
                        System.out.println(//$NON-NLS-1$
                        "JavaBuilder: Continued to build even though prereq project " + //$NON-NLS-1$
                        p.getName() + " is not built by JavaBuilder");
                    continue;
                }
                if (DEBUG)
                    System.out.println(//$NON-NLS-1$
                    "JavaBuilder: Aborted build because prereq project " + //$NON-NLS-1$
                    p.getName() + //$NON-NLS-1$
                    " was not built");
                // make this the only problem for this project
                removeProblemsAndTasksFor(this.currentProject);
                IMarker marker = this.currentProject.createMarker(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER);
                marker.setAttributes(new String[] { IMarker.MESSAGE, IMarker.SEVERITY, IJavaModelMarker.CATEGORY_ID, IMarker.SOURCE_ID }, new Object[] { isClasspathBroken(prereq, true) ? Messages.bind(Messages.build_prereqProjectHasClasspathProblems, p.getName()) : Messages.bind(Messages.build_prereqProjectMustBeRebuilt, p.getName()), Integer.valueOf(IMarker.SEVERITY_ERROR), Integer.valueOf(CategorizedProblem.CAT_BUILDPATH), JavaBuilder.SOURCE_ID });
                return false;
            }
        }
        return true;
 ...
 */
        ...
        while (i.hasNext()) {
            ...
            if (participantPath != currentPath) {
                ...
                if (hasBeenBuilt(project)) {
                    if (DEBUG)
                        System.out.println(//$NON-NLS-1$
                        "JavaBuilder: Requesting another build iteration since cycle participant " + //$NON-NLS-1$
                        project.getName() + " has not yet seen some structural changes");
                    needRebuild();
                    return;
                }
            }
    ...
    private void printLocations(ClasspathLocation[] newLocations, ClasspathLocation[] oldLocations) {
        ...
        for (int i = 0, length = newLocations.length; i < length; i++) //$NON-NLS-1$
        System.out.println("    " + newLocations[i].debugPathString());
        //$NON-NLS-1$
        System.out.println("JavaBuilder: Old locations:");
        for (int i = 0, length = oldLocations.length; i < length; i++) //$NON-NLS-1$
 ...
 */
    public String toString() {
        return this.currentProject == null ? //$NON-NLS-1$
