...
public class AptPlugin extends Plugin implements DebugOptionsListener {
    ...
    //$NON-NLS-1$
    public static final String APT_DEBUG_OPTION = AptPlugin.PLUGIN_ID + "/debug";

    public static boolean DEBUG_GFM = false;

    //$NON-NLS-1$
    public static final String APT_DEBUG_GFM_OPTION = AptPlugin.APT_DEBUG_OPTION + "/generatedFiles";

    public static boolean DEBUG_GFM_MAPS = false;

    //$NON-NLS-1$
    public static final String APT_DEBUG_GFM_MAPS_OPTION = AptPlugin.APT_DEBUG_OPTION + "/generatedFileMaps";

    public static boolean DEBUG_COMPILATION_ENV = false;

    //$NON-NLS-1$
    public static final String APT_COMPILATION_ENV_OPTION = AptPlugin.APT_DEBUG_OPTION + "/compilationEnv";
    ...
    //$NON-NLS-1$
    public static final String APT_BATCH_PROCESSOR_PROBLEM_MARKER = PLUGIN_ID + ".marker";

    /** Marker ID used for build problem, e.g., missing factory jar */
    //$NON-NLS-1$
    public static final String APT_LOADER_PROBLEM_MARKER = PLUGIN_ID + ".buildproblem";

    /** Marker ID used for configuration problem, e.g generated source folder not on classpath */
    //$NON-NLS-1$
    public static final String APT_CONFIG_PROBLEM_MARKER = PLUGIN_ID + ".configproblem";

    /** Marker ID used for posting problems during reconcile/build */
    //$NON-NLS-1$
    public static final String APT_COMPILATION_PROBLEM_MARKER = PLUGIN_ID + ".compile.problem";

    /** Marker ID used for posting problems during build by processors that don't run in reconcile */
    //$NON-NLS-1$
    public static final String APT_NONRECONCILE_COMPILATION_PROBLEM_MARKER = PLUGIN_ID + ".nonreconcile.compile.problem";
    ...
    public void start(BundleContext context) throws Exception {
        ...
        int mask = IResourceChangeEvent.PRE_BUILD | IResourceChangeEvent.PRE_CLOSE | IResourceChangeEvent.PRE_DELETE | IResourceChangeEvent.POST_CHANGE;
    ...
    public static boolean canRunJava6Processors() {
        if (_java6ProcessorClass == null)
            return false;
        //$NON-NLS-1$
        return Platform.getBundle("org.eclipse.jdt.compiler.apt") != null;
    }
