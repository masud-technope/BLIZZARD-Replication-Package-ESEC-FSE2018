...
public class TypeReferenceLocator extends PatternLocator {
    ...
    public int match(Reference node, MatchingNodeSet nodeSet) {
        ...
        if (node instanceof SingleNameReference) {
            ...
            for (int i = 0, max = tokens.length; i < max; i++) if (matchesName(this.pattern.simpleName, tokens[i]))
                // resolution is needed to find out if it is a type ref
                return nodeSet.addMatch(node, POSSIBLE_MATCH);
        }
        return IMPOSSIBLE_MATCH;
    ...
    protected int matchLevel(ImportReference importRef) {
        if (this.pattern.qualification == null) {
            ...
            if (qualifiedTypeName == null)
                return IMPOSSIBLE_MATCH;
            if (// empty name
            qualifiedTypeName.length == 0) {
                if (// can only matches empty pattern
                qualifiedPattern.length == 0) {
                    return ACCURATE_MATCH;
                }
                return IMPOSSIBLE_MATCH;
            }
 ...
 */
    ...
    protected void matchReportImportRef(ImportReference importRef, Binding binding, IJavaElement element, int accuracy, MatchLocator locator) throws CoreException {
        ...
        } else if (// may happen for static import
        ...
        binding instanceof MethodBinding) {
        ...
        switch(qNameRef.bits & ASTNode.RestrictiveFlagMASK) {
            ...
            case Binding.TYPE | Binding.VARIABLE:
                if (binding instanceof ProblemReferenceBinding) {
                    ...
                    ProblemBinding pbBinding = (ProblemBinding) binding;
                    if (// index of last bound token is one before the pb token
                    CharOperation.occurencesOf('.', pbBinding.name) <= 0)
                        return INACCURATE_MATCH;
                    typeBinding = pbBinding.searchType;
                }
                break;
 ...
 */
    ...
    private Map /*<QualifiedTypeReference, List<TypeBinding>>*/
    recordedResolutions = new HashMap();
    ...
    public void recordResolution(QualifiedTypeReference typeReference, TypeBinding resolution) {
        List /*<TypeBinding>*/
        resolutionsForTypeReference = (List) this.recordedResolutions.get(typeReference);
