...
public class Util {
    ...
    public static boolean equalArraysOrNullSortFirst(Comparable[] a, Comparable[] b) {
        ...
        if (len != b.length)
            return false;
        if (// only need to sort if more than two items
        len >= 2) {
            a = sortCopy(a);
            b = sortCopy(b);
        }
    ...
    private static ICompilationUnit getCompilationUnit(char[] fileName, WorkingCopyOwner workingCopyOwner) {
        ...
        if (pkgEnd == -1)
            return null;
        IPackageFragment pkg = getPackageFragment(slashSeparatedFileName, pkgEnd, /*no jar separator for .java files*/
        -1);
        if (pkg == null)
            return null;
        int start;
    ...
    public static char[][] getJavaLikeExtensions() {
        if (JAVA_LIKE_EXTENSIONS == null) {
            ...
            for (int i = 0, length = contentTypes.length; i < length; i++) {
                if (// note that javaContentType.isKindOf(javaContentType) == true
                contentTypes[i].isKindOf(javaContentType)) {
    ...
    public static String getSignature(Type type) {
        ...
        return Signature.createTypeSignature(buffer.toString(), /*not resolved in source*/
        false);
    }
    ...
    public static String localTypeName(String binaryTypeName, int lastDollar, int end) {
        ...
        while (nameStart < end && Character.isDigit(binaryTypeName.charAt(nameStart))) nameStart++;
    ...
    public static int prefixLength(String s1, String s2) {
        int len = 0;
        int max = Math.min(s1.length(), s2.length());
        for (int i = 0; i < max && s1.charAt(i) == s2.charAt(i); ++i) ++len;
        return len;
    }
    ...
    public static final String[] splitOn(char divider, String string, int start, int end) {
        ...
        if (length == 0 || start > end)
            return CharOperation.NO_STRINGS;
        int wordCount = 1;
        for (int i = start; i < end; i++) if (string.charAt(i) == divider)
            wordCount++;
    ...
    public static String typeSignature(TypeReference type) {
        ...
        if ((type.bits & org.eclipse.jdt.internal.compiler.ast.ASTNode.IsUnionType) != 0) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                typeSignatures[i] = Signature.createTypeSignature(/*don't resolve*/
                typeName, false);
            }
            ...
            char[] typeName = CharOperation.concatWith(compoundName, '.');
            signature = Signature.createTypeSignature(typeName, /*don't resolve*/
            false);
        }
        return signature;
    }
    ...
    private static IllegalArgumentException raiseIllegalSignatureException(char[] string, int start) {
        //$NON-NLS-1$ //$NON-NLS-2$
        throw new IllegalArgumentException("\"" + new String(string) + "\" starting at " + start);
    }

    private static IllegalArgumentException raiseUnexpectedCharacterException(char[] string, int start, char unexpected) {
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        throw new IllegalArgumentException("Unexpected '" + unexpected + "' in \"" + new String(string) + "\" starting at " + start);
    }
    ...
    public static void fixTaskTags(Map defaultOptionsMap) {
        ...
        if (taskPriorities == null) {
            if (taskTags != null) {
                //$NON-NLS-1$
                Util.logRepeatedMessage(//$NON-NLS-1$
                TASK_PRIORITIES_PROBLEM, //$NON-NLS-1$
                IStatus.ERROR, //$NON-NLS-1$
                "Inconsistent values for taskTags (not null) and task priorities (null)");
