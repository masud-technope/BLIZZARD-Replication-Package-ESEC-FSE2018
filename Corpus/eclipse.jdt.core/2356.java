...
public class ParameterizedMethodBinding extends MethodBinding {
    ...
    public  ParameterizedMethodBinding(ParameterizedTypeBinding parameterizedDeclaringClass, MethodBinding originalMethod, boolean isStatic) {
        super(originalMethod.modifiers, originalMethod.selector, // no substitution if original was static
        isStatic ? originalMethod.returnType : parameterizedDeclaringClass.substitute(originalMethod.returnType), // no substitution if original was static
        isStatic ? originalMethod.parameters : Scope.substitute(parameterizedDeclaringClass, originalMethod.parameters), // no substitution if original was static
        isStatic ? originalMethod.thrownExceptions : Scope.substitute(parameterizedDeclaringClass, originalMethod.thrownExceptions), parameterizedDeclaringClass);
    ...
    public static ParameterizedMethodBinding instantiateGetClass(TypeBinding receiverType, MethodBinding originalMethod, Scope scope) {
        ...
        method.returnType = scope.createParameterizedType(genericClassType, new TypeBinding[] { scope.environment().createWildcard(genericClassType, 0, receiverType.erasure(), Wildcard.EXTENDS) }, null);
        return method;
    }
