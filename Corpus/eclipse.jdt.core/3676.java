...
public class UnaryExpression extends OperatorExpression {
        ...
        switch((bits & OperatorMASK) >> OperatorSHIFT) {
            case NOT:
                switch(this.expression.implicitConversion >> 4) {
                    ...
                    case T_boolean:
                        ...
                        this.expression.generateOptimizedBoolean(currentScope, codeStream, null, (falseLabel = new Label(codeStream)), valueRequired);
                        if (valueRequired) {
                            codeStream.iconst_0();
                ...
                break;
            case TWIDDLE:
                switch(this.expression.implicitConversion >> /* runtime */
                4) {
                    case T_int:
                        // ~int
            ...
            case MINUS:
                ...
                if (this.constant != NotAConstant) {
                    ...
                    this.expression.generateCode(currentScope, codeStream, valueRequired);
                    if (valueRequired) {
                        switch(expression.implicitConversion >> /* runtime type */
                        4) {
                            case T_int:
                                codeStream.ineg();
                                break;
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        switch(// only switch on possible result type.....
        ...
        if (this.expression.constant != Constant.NotAConstant) {
            this.constant = Constant.computeConstantOperation(this.expression.constant, expressionTypeId, (bits & OperatorMASK) >> OperatorSHIFT);
        } else {
            this.constant = Constant.NotAConstant;
            if (((bits & OperatorMASK) >> OperatorSHIFT) == NOT) {
