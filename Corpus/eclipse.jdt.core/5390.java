...
public class ASTParser {
    ...
    private List getClasspath() throws IllegalStateException {
        Main main = new Main(new PrintWriter(System.out), new PrintWriter(System.err), /*systemExit*/
        false, /*options*/
        null, /*progress*/
        null);
        ...
        try {
            ...
            if (this.sourcepaths != null) {
                for (int i = 0, max = this.sourcepaths.length; i < max; i++) {
                    ...
                    main.processPathEntries(Main.DEFAULT_SIZE_CLASSPATH, allClasspaths, this.sourcepaths[i], encoding, true, false);
                }
            }
            if (this.classpaths != null) {
                for (int i = 0, max = this.classpaths.length; i < max; i++) {
                    main.processPathEntries(Main.DEFAULT_SIZE_CLASSPATH, allClasspaths, this.classpaths[i], null, false, false);
                }
            }
    ...
    public void setKind(int kind) {
        if ((kind != K_COMPILATION_UNIT) && (kind != K_CLASS_BODY_DECLARATIONS) && (kind != K_EXPRESSION) && (kind != K_STATEMENTS)) {
            throw new IllegalArgumentException();
        }
        this.astKind = kind;
    }
    ...
    public IBinding[] createBindings(IJavaElement[] elements, IProgressMonitor monitor) {
        try {
            if (this.project == null)
                throw new //$NON-NLS-1$
                IllegalStateException(//$NON-NLS-1$
                "project or classpath not specified");
            int flags = 0;
            if ((this.bits & CompilationUnitResolver.STATEMENT_RECOVERY) != 0) {
                flags |= ICompilationUnit.ENABLE_STATEMENTS_RECOVERY;
            }
            ...
            case K_COMPILATION_UNIT:
                ...
                try {
                    ...
                    if (this.typeRoot instanceof ICompilationUnit) {
                        ...
                        sourceUnit = new //$NON-NLS-1$
                        BasicCompilationUnit(//$NON-NLS-1$
                        this.rawSource, //$NON-NLS-1$
                        null, //$NON-NLS-1$
                        this.unitName == null ? "" : this.unitName, //$NON-NLS-1$
                        this.project);
                    } else {
                        throw new IllegalStateException();
                    }
                    ...
                    if (needToResolveBindings) {
                        ...
                        try {
                            // parse and resolve
                            compilationUnitDeclaration = CompilationUnitResolver.resolve(sourceUnit, this.project, getClasspath(), searcher, this.compilerOptions, this.workingCopyOwner, flags, monitor);
                        } catch (JavaModelException e) {
                            flags &= ~ICompilationUnit.ENABLE_BINDINGS_RECOVERY;
