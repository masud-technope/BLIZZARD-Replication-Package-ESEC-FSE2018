...
public class ParameterizedQualifiedTypeReference extends ArrayQualifiedTypeReference {
    ...
    public void checkBounds(ReferenceBinding type, Scope scope, int index) {
        ...
        if (type.isParameterizedType()) {
            ...
            if (// argTypes may be null in error cases
            argTypes != null && typeVariables != null) {
                for (int i = 0, argLength = typeVariables.length; i < argLength; i++) if (!typeVariables[i].boundCheck(parameterizedType, argTypes[i]))
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length; i < max; i++) {
            ...
            if (args != null) {
                ...
                for (int j = 0; j < argLength; j++) {
                    ...
                    TypeBinding argType = isClassScope ? arg.resolveTypeArgument((ClassScope) scope, currentType, j) : arg.resolveTypeArgument((BlockScope) scope, currentType, j);
                    if (argType == null) {
                        argHasError = true;
                    } else {
                        argTypes[j] = argType;
                ...
                argLength != typeVariables.length) {
                    scope.problemReporter().incorrectArityForParameterizedType(this, currentType, argTypes);
                    return null;
                }
                // check parameterizing non-static member type of raw type
                if (typeIsConsistent && !currentType.isStatic() && qualifiedType != null && qualifiedType.isRawType()) {
                    scope.problemReporter().rawMemberTypeCannotBeParameterized(this, scope.environment().createRawType(currentType, qualifiedType), argTypes);
                    typeIsConsistent = false;
                }
                ...
                if (// otherwise will do it in Scope.connectTypeVariables()
                !isClassScope)
                    for (int j = 0; j < argLength; j++) if (!typeVariables[j].boundCheck(parameterizedType, argTypes[j]))
                ...
                if (currentType.isGenericType()) {
                    ...
                    qualifiedType = (qualifiedType != null && qualifiedType.isParameterizedType()) ? scope.createParameterizedType(currentType, null, qualifiedType) : currentType;
                }
            }
        }
