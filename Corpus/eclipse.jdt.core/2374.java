class Seq<E> {
    ...
    <T> Seq<Pair<E, T>> zip(Seq<T> that) {
        if (this.isEmpty() || that.isEmpty())
            ...
            return new Seq<Pair<E, T>>(new Pair<E, T>(this.head, that.head), this.tail.zip(that.tail));
    }
    ...
    public class Zipper<T> {
        ...
        Seq<Pair<E, T>> zip(Seq<T> that) {
            if (Seq.this.isEmpty() || that.isEmpty())
                ...
                return new Seq<Pair<E, T>>(new Pair<E, T>(Seq.this.head, that.head), Seq.this.tail.zip(that.tail));
        }
    }
}
