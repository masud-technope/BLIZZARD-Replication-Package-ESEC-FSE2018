...
public final class HashSetOfCharArrayArray implements Cloneable {
    ...
    private int hashCode(char[][] element, int length) {
        int hash = 0;
        for (int i = length - 1; i >= 0; i--) hash = Util.combineHashCodes(hash, CharOperation.hashCode(element[i]));
    ...
    private void rehash() {
        ...
        char[][] currentArray;
        for (int i = this.set.length; --i >= 0; ) if ((currentArray = this.set[i]) != null)
