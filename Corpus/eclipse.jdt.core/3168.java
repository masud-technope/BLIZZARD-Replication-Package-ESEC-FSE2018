...
public class Compiler implements ITypeRequestor, ProblemSeverities {
    ...
    // output for messages that are not sent to problemReporter
    ...
    // by default the compiler will not use worker threads to read/process/write
	 ...
	 */
    public  Compiler(INameEnvironment environment, IErrorHandlingPolicy policy, Map<String, String> settings, final ICompilerRequestor requestor, IProblemFactory problemFactory) {
        this(environment, policy, new CompilerOptions(settings), requestor, problemFactory, /* printwriter */
        null, /* progress */
        null);
    }
    ...
    public  Compiler(INameEnvironment environment, IErrorHandlingPolicy policy, Map settings, final ICompilerRequestor requestor, IProblemFactory problemFactory, boolean parseLiteralExpressionsAsConstants) {
        this(environment, policy, new CompilerOptions(settings, parseLiteralExpressionsAsConstants), requestor, problemFactory, /* printwriter */
        null, /* progress */
        null);
    }
	 ...
	 *      them back as part of the compilation unit result.
	 */
    public  Compiler(INameEnvironment environment, IErrorHandlingPolicy policy, CompilerOptions options, final ICompilerRequestor requestor, IProblemFactory problemFactory) {
        this(environment, policy, options, requestor, problemFactory, /* printwriter */
        null, /* progress */
        null);
    }
    ...
    public  Compiler(INameEnvironment environment, IErrorHandlingPolicy policy, CompilerOptions options, final ICompilerRequestor requestor, IProblemFactory problemFactory, PrintWriter out) {
        this(environment, policy, options, requestor, problemFactory, out, /* progress */
        null);
    }
    ...
    private void compile(ICompilationUnit[] sourceUnits, boolean lastRound) {
        ...
        try {
            ...
            if (this.annotationProcessorManager == null) {
                ...
                ICompilationUnit[] originalUnits = sourceUnits.clone();
                try {
                    beginToCompile(sourceUnits);
                    if (!lastRound) {
                        processAnnotations();
        ...
        try {
            if (this.useSingleThread) {
                ...
                for (int i = startingIndex; i < this.totalUnits; i++) {
                    ...
                    // record accept time as part of generation
                ...
                while (true) {
                    try {
                        // waits if no units are in the processed queue
    ...
    protected void handleInternalException(Throwable internalException, CompilationUnitDeclaration unit, CompilationResult result) {
        ...
        if (result != null) {
            ...
            result.record(this.problemReporter.createProblem(result.getFileName(), IProblem.Unclassified, pbArguments, pbArguments, // severity
            Error, // source start
            0, // source end
            0, // line number
            0, // column number
            ...
            if (abortException.problem != null) {
                recordDistantProblem: {
                    ...
                    for (int i = 0; i < result.problemCount; i++) {
                        if (// already recorded
                        knownProblems[i] == // already recorded
                        distantProblem) {
                            break recordDistantProblem;
                        }
                    }
                    if (// fixup filename TODO (philippe) should improve API to make this official
                    distantProblem instanceof DefaultProblem) {
    ...
    protected void processAnnotations() {
        ...
        do {
            ...
            if (top < this.totalUnits) {
                // NOTE: Reuse the same variable, but make sure it's not used after this point
            ...
            if (newUnitSize != 0) {
                // remember new units in case a source type collision occurs
                ICompilationUnit[] newProcessedUnits = newUnits.clone();
                try {
