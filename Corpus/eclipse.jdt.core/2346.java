 ...
 */
    ...
    public IPath addPackage(IPath packageFragmentRootPath, String packageName) {
        //$NON-NLS-1$
        checkAssertion("a workspace must be open", this.isOpen);
        IPath path = packageFragmentRootPath.append(packageName.replace('.', IPath.SEPARATOR));
        createFolder(path);
        return path;
    }
    ...
    public IPath addPackageFragmentRoot(IPath projectPath, String sourceFolderName, IPath[] inclusionPatterns, IPath[] exclusionPatterns, String specificOutputLocation) throws JavaModelException {
        ...
        IClasspathEntry entry = JavaCore.newSourceEntry(path, inclusionPatterns == null ? new Path[0] : inclusionPatterns, exclusionPatterns == null ? new Path[0] : exclusionPatterns, outputPath);
    ...
    public void addRequiredProject(IPath projectPath, IPath requiredProjectPath) throws JavaModelException {
        addRequiredProject(projectPath, requiredProjectPath, new IPath[] {}, /*include all*/
        new IPath[] {}, /*exclude none*/
        false);
    }
    ...
    public void addLibrary(IPath projectPath, IPath libraryPath, IPath sourceAttachmentPath, IPath sourceAttachmentRootPath) throws JavaModelException {
        addEntry(projectPath, JavaCore.newLibraryEntry(libraryPath, sourceAttachmentPath, sourceAttachmentRootPath));
    }
	...
	*/
    ...
    public Problem[] getProblemsFor(IPath path, String additionalMarkerType) {
        ...
        try {
            ...
            for (int i = 0; i < markers.length; i++) problems.add(new Problem(markers[i]));
	...
	*/
    ...
    File getTmpDirectory() {
        if (this.tmpDirectory == null) {
            this.tmpDirectory = new File(System.getProperty("java.io.tmpdir") + File.separator + "org.eclipse.jdt.core.builder.tests.tmp");
