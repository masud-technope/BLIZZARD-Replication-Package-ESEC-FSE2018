...
public final class CharOperation {
    ...
    public static final boolean equals(char[] first, char[] second, boolean isCaseSensitive) {
        ...
        if (first.length != second.length)
            return false;
        for (int i = first.length; --i >= 0; ) if (Character.toLowerCase(first[i]) != Character.toLowerCase(second[i]))
            return false;
        return true;
    }
    ...
    public static final int hashCode(char[] array) {
        ...
        int length = array.length;
        if (length < 16) {
            for (int i = length; i > 0; i--) hash = (hash * 37) + array[offset++];
        } else {
            // only sample some characters
            int skip = length / 8;
            for (int i = length; i > 0; i -= skip, offset += skip) hash = (hash * 39) + array[offset];
        }
