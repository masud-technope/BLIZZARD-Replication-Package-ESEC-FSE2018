...
public class ProfilingASTConvertionTest extends AbstractJavaModelTests {
    ...
    private void printRange(int counter, int bound, int increment, int totalCounter, int length, int numberOfFiguresForRange, int numberOfFiguresForCounters) {
        if (counter != 0) {
            ...
            buffer.append(display(low, numberOfFiguresForRange)).append(" - ").append(display(bound, numberOfFiguresForRange)).append(" : ").append(display(counter, numberOfFiguresForCounters)).append("\t\t").append(format.format(100.0 * ((double) totalCounter / length)));
    ...
    public void setUpSuite() throws Exception {
        ...
        if (//$NON-NLS-1$
        JavaCore.getClasspathVariable("CONVERTER_JCL_LIB") == null) {
            setupExternalJCL("converterJclMin");
            JavaCore.setClasspathVariables(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            new String[] { "CONVERTER_JCL_LIB", "CONVERTER_JCL_SRC", "CONVERTER_JCL_SRCROOT" }, new Path[] { new Path(getExternalPath() + "converterJclMin.jar"), new Path(getExternalPath() + "converterJclMinsrc.zip"), new Path("") }, null);
        }
        //$NON-NLS-1$ //$NON-NLS-2$
        IJavaProject javaProject = setUpJavaProject("Compiler", "1.4");
