...
public abstract class Expression extends Statement {
	 ...
	 */
    public final boolean checkCastTypesCompatibility(BlockScope scope, TypeBinding castType, TypeBinding expressionType, Expression expression) {
        ...
        if (expressionType.isClass()) {
            if (castType.isArrayType()) {
                ...
                expressionType.id == T_Object) {
                    tagAsNeedCheckCast();
                    return true;
                }
            } else if (// ----- (castType.isClass) expressionType.isClass ------
            castType.isClass()) {
                ...
                if (match != null) {
                    ...
                    if (expression != null && castType.id == T_String)
                        this.constant = expression.constant;
        ...
        if (castType.isArrayType()) {
            ...
            if (expressionType.id == T_JavaLangCloneable || // potential runtime error
            expressionType.id == T_JavaIoSerializable) {
                tagAsNeedCheckCast();
                return true;
            } else {
                reportIllegalCast(scope, castType, expressionType);
        ...
        } else if (// ----- (castType.isClass) expressionType.isInterface --------
        castType.isClass()) {
            if (// no runtime error
            castType.id == T_Object) {
                tagAsUnnecessaryCast(scope, castType);
                return true;
            }
    ...
    public boolean isConstantValueOfTypeAssignableToType(TypeBinding constantType, TypeBinding targetType) {
        ...
        if (constantType.isBaseType() && targetType.isBaseType()) {
            ...
            if ((constantType == IntBinding || BaseTypeBinding.isWidening(T_int, constantType.id)) && (BaseTypeBinding.isNarrowing(targetType.id, T_int))) {
