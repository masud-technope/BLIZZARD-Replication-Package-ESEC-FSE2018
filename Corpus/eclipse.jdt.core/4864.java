...
public class PackageInfoTest extends BuilderTests {
    ...
    public void test001() throws JavaModelException {
        ...
        IPath packageInfoPath = //$NON-NLS-1$ //$NON-NLS-2$
        env.addFile(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "pack/package-info.java", //$NON-NLS-1$
        "@Annot package p1");
        incrementalBuild(projectPath);
        ...
        if (problemsLength == 1) {
            ...
            for (int i = 0; i < problemsLength - 1; i++) writer.println(problems[i].getMessage());
            writer.print(problems[problemsLength - 1].getMessage());
        }
        writer.close();
        final String expectedOutput = "Syntax error on token \"p1\", ; expected after this token\n" + "The declared package \"p1\" does not match the expected package \"pack\"";
    ...
    public void test002() throws JavaModelException {
        ...
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "testcase", //$NON-NLS-1$ //$NON-NLS-2$
        "Main", "package testcase;\n" + "\n" + "public class Main {\n" + "    public static void main(String[] argv) throws Exception {\n" + "		Package pkg = Package.getPackage(\"testcase\");\n" + "		System.out.print(pkg.getAnnotation(TestAnnotation.class));\n" + "		pkg = Class.forName(\"testcase.package-info\").getPackage();\n" + "		System.out.print(pkg.getAnnotation(TestAnnotation.class));\n" + "    }\n" + "}");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addClass(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "testcase", //$NON-NLS-1$ //$NON-NLS-2$
        "TestAnnotation", "package testcase;\n" + "\n" + "import static java.lang.annotation.ElementType.PACKAGE;\n" + "import static java.lang.annotation.RetentionPolicy.RUNTIME;\n" + "\n" + "import java.lang.annotation.Retention;\n" + "import java.lang.annotation.Target;\n" + "\n" + "@Target(PACKAGE)\n" + "@Retention(RUNTIME)\n" + "public @interface TestAnnotation {\n" + "}");
        //$NON-NLS-1$ //$NON-NLS-2$
        env.addFile(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "testcase/package-info.java", //$NON-NLS-1$
        "@TestAnnotation package testcase;");
        incrementalBuild(projectPath);
        expectingNoProblems();
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        executeClass(projectPath, "testcase.Main", "@testcase.TestAnnotation()@testcase.TestAnnotation()", "");
    }
    ...
    public void test003() throws JavaModelException {
        ...
        "");
        incrementalBuild(projectPath);
        expectingOnlySpecificProblemFor(packageInfoPath, //$NON-NLS-1$ //$NON-NLS-2$
        new Problem("testcase/package-info.java", "The declared package \"\" does not match the expected package \"testcase\"", packageInfoPath, 0, 0, CategorizedProblem.CAT_INTERNAL, IMarker.SEVERITY_ERROR));
    }
    ...
    public void test004() throws JavaModelException {
        ...
        env.addFile(//$NON-NLS-1$ //$NON-NLS-2$
        root, //$NON-NLS-1$ //$NON-NLS-2$
        "my/foo/package-info.java", "/**\n" + "* A demo package for foo.\n" + "*/\n" + "package my.foo;\n");
        IPath otherPackageInfoPath = env.addFile(otherRoot, //$NON-NLS-1$ //$NON-NLS-2$
        "my/foo/package-info.java", "/**\n" + "* A demo package for foo.\n" + "*/\n" + "package my.foo;\n");
        incrementalBuild(projectPath);
        expectingOnlySpecificProblemFor(otherPackageInfoPath, //$NON-NLS-1$ //$NON-NLS-2$
        new Problem("my/foo/package-info.java", "The type package-info is already defined", otherPackageInfoPath, 0, 0, CategorizedProblem.CAT_TYPE, IMarker.SEVERITY_ERROR));
    }
    ...
    public void test323785a() throws JavaModelException {
        ...
        incrementalBuild(projectPath);
        expectingOnlySpecificProblemFor(xJavaPath, new Problem("X.java", "The type X cannot subclass the final class Y", xJavaPath, 16, 17, CategorizedProblem.CAT_TYPE, IMarker.SEVERITY_ERROR));
    }
    ...
    public void testBug372012c() throws JavaModelException, IOException {
        ...
        env.addClass(srcRoot, "p1", "package-info", packageInfoCode);
        incrementalBuild(projectPath);
        expectingProblemsFor(projectPath, "Problem : A default nullness annotation has not been specified for the package p1 [ resource : </Project/src/p1/package-info.java> range : <8,10> category : <90> severity : <2>]");
        // verify that all package CU's were recompiled
 ...
 */
    public void testBug367836() throws JavaModelException {
        ...
        src, //$NON-NLS-1$ //$NON-NLS-2$
        "p", //$NON-NLS-1$ //$NON-NLS-2$
        "package-info", //$NON-NLS-1$
        "");
        fullBuild();
        expectingOnlySpecificProblemFor(bPath, //$NON-NLS-1$ //$NON-NLS-2$
        new Problem("", "The declared package \"\" does not match the expected package \"p\"", bPath, 0, 0, CategorizedProblem.CAT_INTERNAL, IMarker.SEVERITY_ERROR));
    }
