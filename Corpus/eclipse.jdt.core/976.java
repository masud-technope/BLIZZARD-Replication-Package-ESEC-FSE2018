...
public class AbstractASTTests extends ModifyingResourceTests implements DefaultMarkedNodeLabelProviderOptions {
	 ...
	 */
    /*package*/
    static final int AST_INTERNAL_JLS3 = AST.JLS3;
	 ...
	 */
    /*package*/
    static final int AST_INTERNAL_JLS4 = AST.JLS4;
    ...
    public class MarkerInfo {
        ...
        public  MarkerInfo(String source, String markerBeginning, String markerEnding, String markerStart, String markerEnd) {
            this(null, source, markerBeginning, markerEnding, markerStart, markerEnd);
        }

        public  MarkerInfo(String path, String source, String markerBeginning, String markerEnding, String markerStart, String markerEnd) {
            this.path = path;
            this.source = source;
            ...
            for (int i = 1; i < markerIndex; i++) setStartAndEnd(i);
        ...
        public int indexOfASTStart(int astStart, int fromIndex) {
            for (int i = fromIndex, length = this.astStarts.length; i < length; i++) if (this.astStarts[i] == astStart)
                return i;
            return -1;
        }
        ...
        private void setStartAndEnd(int markerIndex) {
            ...
            // start of AST inclusive
            int astStart = this.source.indexOf(markerStart);
            this.astStarts[markerIndex - 1] = astStart;
            removeMarkerFromSource(markerStart, astStart, markerIndex - 1);
            // end of AST exclusive
            int astEnd = this.source.indexOf(markerEnd);
    ...
    protected void assertASTResult(String expected, ASTResult actual) throws JavaModelException {
    ...
    protected ASTNode buildAST(MarkerInfo markerInfo, IClassFile classFile, boolean reportErrors) throws JavaModelException {
        ...
        if (reportErrors) {
            ...
            for (int i = 0, length = problems.length; i < length; i++) Util.appendProblem(buffer, problems[i], markerInfo.source.toCharArray(), i + 1);
            if (buffer.length() > 0)
    ...
    private ASTNode[] buildASTs(String newContents, ICompilationUnit cu, int flags) throws JavaModelException {
        ...
        if ((flags & ICompilationUnit.FORCE_PROBLEM_DETECTION) != 0) {
            StringBuffer buffer = new StringBuffer();
            IProblem[] problems = unit.getProblems();
            for (int i = 0, length = problems.length; i < length; i++) Util.appendProblem(buffer, problems[i], newContents.toCharArray(), i + 1);
            if (buffer.length() > 0)
                assertEquals("Unexpected errors", "", buffer.toString());
        }
    ...
    protected ASTNode[] buildASTs(int JLSLevel, String newContents, ICompilationUnit cu, boolean reportErrors, boolean enableStatementRecovery, boolean bindingRecovery) throws JavaModelException {
        ...
        if (cu.isWorkingCopy()) {
            ...
            int flags = 0;
            if (reportErrors)
                flags |= ICompilationUnit.FORCE_PROBLEM_DETECTION;
            if (enableStatementRecovery)
    ...
    protected ASTNode[] buildASTs(String newContents, ICompilationUnit cu, boolean reportErrors, boolean enableStatementRecovery, boolean bindingRecovery) throws JavaModelException {
        ...
        return buildASTs(JLSLevel, newContents, cu, reportErrors, enableStatementRecovery, bindingRecovery);
    }

    protected ASTResult buildMarkedAST(String path, String content) throws JavaModelException {
        return this.buildMarkedAST(path, content, // enable statement recovery
        true, // report ast
        true, // report problems
        true, new MarkedASTFlattener.DefaultMarkedNodeLabelProvider(ALL_OPTIONS));
    }

    protected ASTResult buildMarkedAST(String path, String content, boolean enableStatementRecovery, boolean reportAST, boolean reportProblems, int options) throws JavaModelException {
        return this.buildMarkedAST(path, content, enableStatementRecovery, reportAST, reportProblems, new MarkedASTFlattener.DefaultMarkedNodeLabelProvider(options));
    }
	 ...
	 */
    protected ASTResult buildMarkedAST(String path, String content, boolean enableStatementRecovery, boolean reportAST, boolean reportProblems, MarkedASTFlattener.DefaultMarkedNodeLabelProvider labelProvider) throws JavaModelException {
        MarkerInfo markerInfo;
        ...
        parser.setStatementsRecovery(enableStatementRecovery);
        CompilationUnit unit = (CompilationUnit) parser.createAST(null);
        MarkedASTFlattener flattener = new MarkedASTFlattener(reportAST, reportProblems, labelProvider);
        ...
        class Visitor extends ASTVisitor {
            ...
            public void preVisit(ASTNode node) {
                ...
                if (index != -1 && node.getStartPosition() + node.getLength() == markerInfo.astEnds[index]) {
                    this.found.add(node);
                    // so that 2 nodes with the same start and end will not be found
                    markerInfo.astStarts[index] = -1;
                }
            }
        }
