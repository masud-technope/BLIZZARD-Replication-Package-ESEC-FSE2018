...
public class Util implements SuffixConstants {
    ...
    private static final String bundleName = //$NON-NLS-1$
        ...
        while (true) {
            if ((end = CharOperation.indexOf('{', messageWithNoDoubleQuotes, start)) > -1) {
                ...
                if ((start = CharOperation.indexOf('}', messageWithNoDoubleQuotes, end + 1)) > -1) {
                    ...
                    } catch (NumberFormatException // could be nested message ID {compiler.name}
                    nfe) {
                        boolean done = false;
                        if (!id.equals(argId)) {
	...
	*/
    ...
    public static char[] getInputStreamAsCharArray(InputStream stream, int length, String encoding) throws IOException {
        ...
        if (length == -1) {
            ...
            int start = 0;
            if (//$NON-NLS-1$
            contentsLength > 0 && "UTF-8".equals(encoding)) {
            ...
            int start = 0;
            if (//$NON-NLS-1$
            length > 0 && "UTF-8".equals(encoding)) {
                if (// if BOM char then skip
                contents[0] == 0xFEFF) {
                    len--;
                    start = 1;
