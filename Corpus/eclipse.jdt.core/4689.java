...
public class WrapExecutor {
    ...
    private class LineAnalyzer extends TokenTraverser {
        ...
        protected boolean token(Token token, int index) {
            ...
            if (token.isWrappable()) {
                ...
                if (wrapPolicy.wrapMode == WrapMode.TOP_PRIORITY && getLineBreaksBefore() == 0 && index > this.currentTopPriorityGroupEnd) {
            ...
            boolean isLineEnd = getLineBreaksAfter() > 0 || getNext() == null || (getNext().isNextLineOnWrap() && WrapExecutor.this.tm.get(WrapExecutor.this.tm.findFirstTokenInLine(index)).isWrappable());
            return !isLineEnd;
        }
        ...
        protected boolean token(final Token token, final int index) {
            ...
            if (getLineBreaksAfter() > 0 || getNext() == null) {
                ...
                if (token.tokenType != TokenNameCOMMENT_LINE) {
                    ...
                    structureChanged();
                    // will fill the line comment structure in next step
                    return true;
                }
                ...
                for (int i = 0; i < structure.size(); i++) {
                    ...
                    if (fragment.hasNLSTag()) {
                        ...
                        while (pos <= fragment.originalEnd && ScannerHelper.isWhitespace(WrapExecutor.this.tm.charAt(pos))) pos++;
                        ...
                        if (!//$NON-NLS-1$
                        fragmentString.startsWith(//$NON-NLS-1$
                        "//")) {
                            // forge a prefix
                            Token prefix = new Token(lineComment.originalStart, lineComment.originalStart + 1, TokenNameCOMMENT_LINE);
                            prefix.spaceBefore();
                ...
                if (// all the tags have been moved to other lines
                structure.isEmpty()) {
    ...
    private int applyWraps(int index, int indent) throws WrapRestartThrowable {
        ...
        while (index < this.tm.size()) {
            ...
            if (separateLinesOnWrapFrom >= 0 && token == this.tm.get(separateLinesOnWrapFrom).getSeparateLinesOnWrapUntil()) {
            ...
            while (wrapInfo != null && wrapInfo.wrapTokenIndex < index) wrapInfo = this.wrapSearchResults.get(wrapInfo).nextWrap;
    ...
    private WrapResult findWraps(int wrapTokenIndex, int indent) throws WrapRestartThrowable {
        ...
        int nearestGroupEnd = topPriorityIndex == -1 ? 0 : this.tm.get(topPriorityGroupStarts[topPriorityIndex]).getWrapPolicy().groupEndIndex;
        double bestTotalPenalty = getWrapPenalty(wrapTokenIndex, indent, lastIndex + 1, -1, WrapResult.NO_WRAP_NEEDED);
        // if line is exceeded, accept every wrap
        ...
        if ((!lineExceeded || firstPotentialWrap < 0) && lastIndex + 1 < this.tm.size()) {
            ...
            if (nextLineToken.getWrapPolicy() != null && nextLineToken.getWrapPolicy().wrapMode != WrapMode.FORCED && (this.tm.get(lastIndex).isComment() || nextLineToken.isComment())) {
        ...
        for (int i = lastIndex; firstPotentialWrap >= 0 && i >= firstPotentialWrap; i--) {
            ...
            if (commentIndex > 0 && (token.tokenType == TokenNameCOMMENT_BLOCK || token.tokenType == TokenNameCOMMENT_JAVADOC)) {
            ...
            if (topPriorityIndex >= 0 && i <= nearestGroupEnd) {
                ...
                topPriorityIndex--;
                nearestGroupEnd = topPriorityIndex == -1 ? 0 : this.tm.get(topPriorityGroupStarts[topPriorityIndex]).getWrapPolicy().groupEndIndex;
            }
            if (!token.isWrappable())
                continue;
    ...
    private double getWrapPenalty(int lineStartIndex, int lineIndent, int wrapIndex, int wrapIndent, WrapResult wrapResult) throws WrapRestartThrowable {
        ...
        while (checkDepth && nextWrap != null) {
            ...
            if (nextPolicy.wrapParentIndex == wrapPolicy.wrapParentIndex || (penaltyDiff != 0 && !wrapPolicy.isFirstInGroup)) {
    ...
    private void checkSeparateLinesOnWrap(final int separateLinesOnWrapFrom) throws WrapRestartThrowable {
        ...
        if (next.getWrapPolicy() == null || next.getWrapPolicy().wrapMode == WrapMode.FORCED) {
            next.setWrapPolicy(new WrapPolicy(WrapMode.WHERE_NECESSARY, separateLinesOnWrapFrom, this.options.indentation_size));
        }
        next.breakBefore();
    ...
    private void checkForceWrap(Token token, int index, int currentIndent) throws WrapRestartThrowable {
        ...
        if (token.isWrappable() && this.options.wrap_outer_expressions_when_nested && getWrapIndent(token) < currentIndent) {
    ...
    private void checkTopPriorityWraps(int wrapIndex) throws WrapRestartThrowable {
        ...
        if (wrapPolicy != null && wrapPolicy.wrapMode == WrapMode.TOP_PRIORITY && !this.usedTopPriorityWraps.contains(wrapPolicy))
    ...
    private void handleOnColumnIndent(int tokenIndex, WrapPolicy wrapPolicy) {
        if (wrapPolicy != null && wrapPolicy.indentOnColumn && !wrapPolicy.isFirstInGroup && this.options.tab_char == DefaultCodeFormatterOptions.TAB && !this.options.use_tabs_only_for_leading_indentations) {
    ...
    int getWrapIndent(Token token) {
        ...
        if (this.options.never_indent_line_comments_on_first_column && token.tokenType == TokenNameCOMMENT_LINE && token.getIndent() == 0)
            return 0;
        if (this.options.never_indent_block_comments_on_first_column && token.tokenType == TokenNameCOMMENT_BLOCK && token.getIndent() == 0)
            return 0;
