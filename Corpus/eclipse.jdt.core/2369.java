...
public class ArrayTypeImpl implements ArrayType, EclipseMirrorType {
    ...
    public EclipseMirrorType getComponentType() {
        ...
        if (// the element type is the component type.
        dimension == 1)
            result = elementType;
        else {
            ...
            if (result == null)
                ...
                "unknown component type for " + _arrayBinding);
        }
    ...
    public String toString() {
        ...
        buffer.append(name);
        for (int i = 0, dim = _arrayBinding.getDimensions(); i < dim; i++) //$NON-NLS-1$
        buffer.append("[]");
    ...
    public int hashCode() {
        return _arrayBinding.hashCode();
    }

    public MirrorKind kind() {
        return MirrorKind.TYPE_ARRAY;
    }
    ...
    public boolean isSubTypeCompatible(EclipseMirrorType type) {
        ...
        if (type.kind() == MirrorKind.TYPE_INTERFACE)
            return //$NON-NLS-1$
            "java.lang.Cloneable".equals(type.getTypeBinding().getQualifiedName()) || //$NON-NLS-1$
            "java.io.Serializable".equals(//$NON-NLS-1$
            type.getTypeBinding().getQualifiedName());
