...
public class Javadoc extends ASTNode {
                ...
                if (methDecl != null && (methDecl.isConstructor() || overriding) && !superRef) {
                    if (this.references[i] instanceof JavadocMessageSend) {
                        ...
                        if (messageSend.binding != null && messageSend.binding.isValidBinding()) {
                            if (methDecl.binding.declaringClass.isCompatibleWith(messageSend.receiverType) && CharOperation.equals(messageSend.selector, methDecl.selector) && (messageSend.binding.returnType == methDecl.binding.returnType)) {
    ...
    private void resolveReference(Expression reference, Scope scope) {
        ...
        if (reference instanceof JavadocFieldReference) {
            ...
            if (verifyValues && fieldRef.binding != null && fieldRef.binding.isValidBinding()) {
        ...
        if (!verifyValues)
            return;
        else // Verify that message reference are not used for @value tags
        if (reference instanceof JavadocMessageSend) {
            ...
            msgSend.tagValue == AbstractCommentParser.TAG_VALUE_VALUE) {
                scope.problemReporter().javadocInvalidValueReference(msgSend.sourceStart, msgSend.sourceEnd);
            }
        } else // Verify that constructorreference are not used for @value tags
        if (reference instanceof JavadocAllocationExpression) {
            ...
            if (// cannot refer to method for @value tag
            alloc.tagValue == AbstractCommentParser.TAG_VALUE_VALUE) {
