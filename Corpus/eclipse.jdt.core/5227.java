        ...
        for (int i = 0; i < argumentsSize; i++) {
            ...
            for (int j = 0; j < paramTagsSize && !found; j++) {
                ...
                if (arg.binding == param.binding) {
                    if (// do not count first occurence of param name reference
                    param.binding == paramNameRefBinding) {
                        paramNameRefCount++;
            ...
            for (int i = 0; i < typeParametersLength; i++) {
                ...
                for (int j = 0; j < paramTypeParamLength && !found; j++) {
                    if (TypeBinding.equalsEquals(parameter.binding, this.paramTypeParameters[j].resolvedType)) {
                        if (// do not count first occurence of param nmae reference
                        parameter.binding == paramNameRefBinding) {
                            paramNameRefCount++;
                            found = paramNameRefCount > 1;
                        } else {
                            found = true;
