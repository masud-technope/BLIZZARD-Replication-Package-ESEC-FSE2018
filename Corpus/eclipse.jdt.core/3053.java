...
*/
        ...
        if (arguments != null) {
            ...
            if (considerTypeAnnotations || hasJDK15NullAnnotations) {
                ...
                if (methodBinding.isVarargs()) {
                    ...
                    if (!passThrough)
                        // with non-passthrough varargs last param is fed from individual args -> don't check
                        numParamsToCheck--;
                }
            }
            if (considerTypeAnnotations) {
                for (int i = 0; i < numParamsToCheck; i++) {
                    ...
                    analyseOneArgument18(currentScope, flowContext, flowInfo, expectedType, arguments[i], specialCaseNonNullness, methodBinding.original().parameters[i]);
                }
                if (!passThrough && varArgsType instanceof ArrayBinding) {
                    ...
                    for (int i = numParamsToCheck; i < arguments.length; i++) {
                        analyseOneArgument18(currentScope, flowContext, flowInfo, expectedType, arguments[i], specialCaseNonNullness, methodBinding.original().parameters[varArgPos]);
                    }
                }
                ...
                for (int i = 0; i < numParamsToCheck; i++) {
                    if (methodBinding.parameterNonNullness[i] == Boolean.TRUE) {
                        ...
                        // slight loss of precision: should also use the null info from the receiver.
                        int nullStatus = argument.nullStatus(flowInfo, flowContext);
                        if (// if required non-null is not provided
    ...
    void analyseOneArgument18(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, TypeBinding expectedType, Expression argument, Boolean expectedNonNullness, TypeBinding originalExpected) {
        ...
        int nullStatus = argument.nullStatus(flowInfo, flowContext);
        internalAnalyseOneArgument18(currentScope, flowContext, expectedType, argument, flowInfo, nullStatus, expectedNonNullness, originalExpected);
    }
    ...
    void internalAnalyseOneArgument18(BlockScope currentScope, FlowContext flowContext, TypeBinding expectedType, Expression argument, FlowInfo flowInfo, int nullStatus, Boolean expectedNonNullness, TypeBinding originalExpected) {
        ...
        if (!annotationStatus.isAnyMismatch() && statusFromAnnotatedNull != 0)
            // to avoid reports mentioning '@NonNull null'!
 ...
 */
    public void generateArguments(MethodBinding binding, Expression[] arguments, BlockScope currentScope, CodeStream codeStream) {
        ...
        } else if (// standard generation for method arguments
        arguments != null) {
            for (int i = 0, max = arguments.length; i < max; i++) arguments[i].generateCode(currentScope, codeStream, true);
        }
    }
    ...
    public boolean isBoxingCompatible(TypeBinding expressionType, TypeBinding targetType, Expression expression, Scope scope) {
        if (scope.isBoxingCompatibleWith(expressionType, targetType))
            return true;
        return // narrowing then boxing ? Only allowed for some target types see 362279
        expressionType.isBaseType() && !targetType.isBaseType() && !targetType.isTypeVariable() && // autoboxing
        scope.compilerOptions().sourceLevel >= org.eclipse.jdt.internal.compiler.classfmt.ClassFileConstants.JDK1_5 && (targetType.id == TypeIds.T_JavaLangByte || targetType.id == TypeIds.T_JavaLangShort || targetType.id == TypeIds.T_JavaLangCharacter) && expression.isConstantValueOfTypeAssignableToType(expressionType, scope.environment().computeBoxingType(targetType));
    }
