...
public class JavaSearchPattern extends SearchPattern implements IIndexConstants {
	 ...
	 */
    public static final int MATCH_MODE_MASK = R_EXACT_MATCH | R_PREFIX_MATCH | R_PATTERN_MATCH | R_REGEXP_MATCH | R_CAMELCASE_MATCH | R_CAMELCASE_SAME_PART_COUNT_MATCH;
    ...
    protected StringBuffer print(StringBuffer output) {
        ...
        switch(getMatchMode()) {
            ...
            case R_CAMELCASE_SAME_PART_COUNT_MATCH:
                //$NON-NLS-1$
                output.append(//$NON-NLS-1$
                "camel case same part count match, ");
                break;
        }
        if (isCaseSensitive())
            //$NON-NLS-1$
            ...
            while (parent != null && parent.getElementType() == IJavaElement.TYPE) {
                ...
                if (parameters != null) {
                    ...
                    if (length > 0) {
                        ...
                        for (int i = 0; i < length; i++) typeParameters[ptr][i] = Signature.createTypeSignature(parameters[i].getElementName(), false).toCharArray();
                    }
                }
