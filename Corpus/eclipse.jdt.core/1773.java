 ...
 */
    ...
    protected void updateProblemCounts(CategorizedProblem[] newProblems) {
        for (int i = 0, l = newProblems.length; i < l; i++) if (newProblems[i].isError())
            this.newErrorCount++;
        else
            ...
            next: for (int i = 0, l = newProblems.length; i < l; i++) {
                ...
                if (oldProblems != null) {
                    for (int j = 0, m = oldProblems.length; j < m; j++) {
                        ...
                        if (isError == wasError && message.equals(//$NON-NLS-1$
                        pb.getAttribute(//$NON-NLS-1$
