...
public class NameLookupTests2 extends ModifyingResourceTests {
    ...
    public void testAddPackageFragmentRootAndPackageFrament() throws CoreException {
        try {
            ...
            IClasspathEntry[] classpath = new IClasspathEntry[] { JavaCore.newProjectEntry(new Path("/P1")) };
            ...
            IClasspathEntry[] classpath2 = new IClasspathEntry[] { JavaCore.newSourceEntry(new Path("/P1/src1")), JavaCore.newSourceEntry(new Path("/P1/src2")) };
            p1.setRawClasspath(classpath2, null);
            createFolder("/P1/src2/p1");
            res = getNameLookup((JavaProject) p2).findPackageFragments("p1", false);
            assertTrue("Should get 'p1' package fragment", res != null && res.length == 1 && res[0].getElementName().equals("p1"));
        } finally {
            deleteProject("P1");
            deleteProject("P2");
        }
 ...
 */
    public void testAddPackageFragment2() throws CoreException {
        try {
            ...
            assertElementsEqual("Didn't find p1", "p1 [in src [in P]]", pkgs);
            createFolder("/P/src/p2");
            pkgs = getNameLookup(project).findPackageFragments("p2", false);
            assertElementsEqual("Didn't find p2", "p2 [in src [in P]]", pkgs);
        } finally {
            deleteProject("P");
        }
    }
 ...
 */
    public void testDuplicateTypesInWorkingCopies() throws CoreException {
        ...
        try {
            ...
            // TODO (jerome) should use seekTypes
            IType type = nameLookup.findType("Other", false, NameLookup.ACCEPT_ALL);
 ...
 */
    public void testFindDefaultPackageFragmentInNonDefaultRoot() throws CoreException {
        try {
            ...
            assertElementsEqual("Didn't find default package", "<default> [in src [in P]]", new IJavaElement[] { pkg });
        } finally {
            deleteProject("P");
        }
    }
        ...
        try {
            ...
            IWorkspaceRunnable runnable = new IWorkspaceRunnable() {
                ...
                public void run(IProgressMonitor monitor) throws CoreException {
                    p1.getPackageFragmentRoot(p1.getProject()).createPackageFragment(/*don't force*/
                    "pkg", false, monitor);
                    IPackageFragment[] pkgs = getNameLookup(p2).findPackageFragments(/*exact match*/
                    "pkg", false);
                    assertElementsEqual("Unexpected package fragments", "pkg [in <project root> [in P1]]", pkgs);
                }
            };
            JavaCore.run(runnable, /*no progress*/
            null);
        } finally {
            deleteProject("P1");
            deleteProject("P2");
 ...
 */
    public void testFindPackageFragmentWithWorkingCopy() throws CoreException {
        ...
        try {
            ...
            NameLookup nameLookup = project.newNameLookup(this.workingCopies);
            IJavaElement[] pkgs = nameLookup.findPackageFragments("p1", /*not a partial match*/
            false);
            assertElementsEqual("Unexpected packages", "p1 [in <project root> [in P]]", pkgs);
        } finally {
            deleteProject("P");
        }
    }
 ...
 */
    public void testFindPackageFragment2() throws CoreException {
        try {
            ...
            assertElementEquals("Unexpected package", "<null>", pkg);
        } finally {
            deleteProject("P");
        }
    }
 ...
 */
	 ...
	 */
        ...
        try {
            Util.createJar(new String[] { "test1/IResource.java", //$NON-NLS-1$
            "package test1;\n" + "public class IResource {\n" + //$NON-NLS-1$
            "}" }, new String[] { "META-INF/MANIFEST.MF", "Manifest-Version: 1.0\n" }, goodJar, JavaCore.VERSION_1_4);
            ...
            waitForAutoBuild();
            assertTrue("The invalid archive cache should report that the jar is invalid", JavaModelManager.getJavaModelManager().isInvalidArchive(transitioningIPath));
            ...
            JavaModelManager.getJavaModelManager().getJavaModel().refreshExternalArchives(null, null);
            assertTrue("External archive refresh sees no changes, so the invalid archive cache should be unchanged", JavaModelManager.getJavaModelManager().isInvalidArchive(transitioningIPath));
            type = getNameLookup(proj).findType("test1.IResource", false, NameLookup.ACCEPT_CLASSES);
            assertEquals("External archive refresh sees no changes, so the project cache should be unchanged", null, type);
            ...
            assertFalse("The invalid archive cache should no longer report the jar as invalid", JavaModelManager.getJavaModelManager().isInvalidArchive(transitioningIPath));
            type = getNameLookup(proj).findType("test1.IResource", false, NameLookup.ACCEPT_CLASSES);
