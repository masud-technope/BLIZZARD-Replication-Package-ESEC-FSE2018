...
public class QualifiedAllocationExpression extends AllocationExpression {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        checkCapturedLocalInitializationIfNecessary(this.superTypeBinding == null ? this.binding.declaringClass : this.superTypeBinding, currentScope, flowInfo);
        // process arguments
        if (arguments != null) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (enclosingInstance != null) {
            if (enclosingInstance instanceof CastExpression) {
                // will check later on
                enclosingInstance.bits |= IgnoreNeedForCastCheckMASK;
                enclosingInstanceContainsCast = true;
            }
            if ((enclosingInstanceType = enclosingInstance.resolveType(scope)) == null) {
                hasError = true;
            } else if (enclosingInstanceType.isBaseType() || enclosingInstanceType.isArrayType()) {
                scope.problemReporter().illegalPrimitiveOrArrayTypeForEnclosingInstance(enclosingInstanceType, enclosingInstance);
                hasError = true;
    ...
    public void traverse(ASTVisitor visitor, BlockScope scope) {
        if (visitor.visit(this, scope)) {
            ...
            if (arguments != null) {
                ...
                for (int i = 0; i < argumentsLength; i++) arguments[i].traverse(visitor, scope);
            }
            if (anonymousType != null)
