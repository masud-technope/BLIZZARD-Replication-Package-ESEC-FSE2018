 ...
 */
    protected void setUp() throws Exception {
        ...
        if (TEST_PROJECT == null) {
            ...
            createFile("/TestProject/src" + length + "/org/eclipse/jdt/core/test" + length + "/Foo.java", "package org.eclipse.jdt.core.test" + length + ";\n" + "public class Foo {\n" + "}\n" + "class Secondary {\n" + "}\n");
 ...
 */
    ...
    protected void assertTypeFound(String typeName, String expectedResult) throws JavaModelException {
        ...
        assertEquals("Expected type " + typeName + " NOT found!", expectedResult, ((SourceType) type).toStringWithAncestors());
    }
 ...
 */
    public void testFindTypeWithDot() throws JavaModelException, CoreException {
        try {
            ...
            this.createFile("/P/p/X.java", "package p;\n" + "public class X {\n" + "  public class Y {\n" + "  }\n" + "}");
            IType type = javaProject.findType("p.X.Y");
            assertEquals("Unexpected type found", "class Y [in X [in X.java [in p [in <project root> [in P]]]]]", type == null ? "null" : type.toString());
        } finally {
            deleteProject("P");
        }
    }
 ...
 */
    public void testFindSecondaryType_Exist01() throws JavaModelException, CoreException {
        int length = SF_LENGTH - 1;
        assertTypeFound("org.eclipse.jdt.core.test" + length + ".Foo", "Foo [in Foo.java [in org.eclipse.jdt.core.test" + length + " [in src" + length + " [in TestProject]]]]");
    }

    public void testFindSecondaryType_Exist02() throws JavaModelException, CoreException {
        int length = SF_LENGTH - 1;
        assertTypeFound("org.eclipse.jdt.core.test" + length + ".Secondary", "Secondary [in Foo.java [in org.eclipse.jdt.core.test" + length + " [in src" + length + " [in TestProject]]]]");
    }
 ...
 */
    public void testBug306477() throws Exception {
        try {
            ...
            assertElementEquals("We should have found the secondary enum 'Bob'!", "Bob [in Misc.java [in p [in <project root> [in P]]]]", type);
        } finally {
            deleteProject("P");
        }
    }
