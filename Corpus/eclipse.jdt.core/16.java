 ...
 */
    public void codeComplete(char[] snippet, int insertion, int position, char[][] localVariableTypeNames, char[][] localVariableNames, int[] localVariableModifiers, boolean isStatic, CompletionRequestor requestor, WorkingCopyOwner owner, IProgressMonitor monitor) throws JavaModelException {
        ...
        if (source != null && insertion > -1 && insertion < source.length()) {
            ...
            BasicCompilationUnit cu = new BasicCompilationUnit(fakeSource, null, getElementName(), // use project to retrieve corresponding .java IFile
            project);
            engine.complete(cu, prefix.length + position, prefix.length, /*extended context isn't computed*/
            null);
        } else {
    ...
    public IType getDeclaringType() {
        ...
        if (classFile.isOpen()) {
            ...
            if (lastDollar == -1) {
                ...
                return new BinaryType((JavaElement) getPackageFragment().getClassFile(enclosingClassFileName), Util.localTypeName(enclosingName, enclosingName.lastIndexOf('$'), enclosingName.length()));
            }
        }
    }
 ...
 */
    public String getFullyQualifiedName(char enclosingTypeSeparator) {
        try {
            return getFullyQualifiedName(enclosingTypeSeparator, /*don't show parameters*/
            false);
        } catch (JavaModelException e) {
            return null;
        }
                ...
                nextParam: while (memento.hasMoreTokens()) {
                    ...
                    switch(token.charAt(0)) {
                        ...
                        case JEM_METHOD:
                            ...
                            while (// backward compatible with 3.0 mementos
                            param.length() == 1 && Signature.C_ARRAY == param.charAt(0)) {
 ...
 */
    public IType getType(String typeName) {
        //$NON-NLS-1$
        IClassFile classFile = getPackageFragment().getClassFile(getTypeQualifiedName() + "$" + typeName + SUFFIX_STRING_class);
 ...
 */
    public String getTypeQualifiedName(char enclosingTypeSeparator) {
        try {
            return getTypeQualifiedName(enclosingTypeSeparator, /*don't show parameters*/
            false);
        } catch (JavaModelException e) {
            return null;
        }
 ...
 */
    public boolean isInterface() throws JavaModelException {
        ...
        switch(TypeDeclaration.kind(info.getModifiers())) {
            ...
            case // annotation is interface too
