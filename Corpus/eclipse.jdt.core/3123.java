...
public class NameEnvironment implements INameEnvironment, SuffixConstants {
    ...
    // assumed that each name is of the form "a/b/ClassName"
...
*/
    private void computeClasspathLocations(IWorkspaceRoot root, JavaProject javaProject, SimpleLookupTable binaryLocationsPerProject) throws CoreException {
        ...
        if (cycleMarker != null) {
            int severity = JavaCore.ERROR.equals(javaProject.getOption(JavaCore.CORE_CIRCULAR_CLASSPATH, true)) ? IMarker.SEVERITY_ERROR : IMarker.SEVERITY_WARNING;
        ...
        nextEntry: for (int i = 0, l = classpathEntries.length; i < l; i++) {
            ...
            switch(entry.getEntryKind()) {
                case IClasspathEntry.CPE_SOURCE:
                    ...
                    sLocations.add(ClasspathLocation.forSourceFolder((IContainer) target, outputFolder, entry.fullInclusionPatternChars(), entry.fullExclusionPatternChars(), entry.ignoreOptionalProblems()));
                    continue nextEntry;
                case IClasspathEntry.CPE_PROJECT:
                    if (!(target instanceof IProject))
                        continue nextEntry;
                    IProject prereqProject = (IProject) target;
                    // if project doesn't have java nature or is not accessible
                    ...
                    nextPrereqEntry: for (int j = 0, m = prereqClasspathEntries.length; j < m; j++) {
                        ...
                        if (prereqEntry.getEntryKind() == IClasspathEntry.CPE_SOURCE) {
                            ...
                            IContainer binaryFolder = prereqOutputPath.segmentCount() == 1 ? (IContainer) prereqProject : (IContainer) root.getFolder(prereqOutputPath);
                            if (binaryFolder.exists() && !seen.contains(binaryFolder)) {
                                ...
                                if (// normal builder mode
                                binaryLocationsPerProject != // normal builder mode
                                null) {
                ...
                case IClasspathEntry.CPE_LIBRARY:
                    if (target instanceof IResource) {
                        ...
                        if (resource instanceof IFile) {
                            AccessRuleSet accessRuleSet = (JavaCore.IGNORE.equals(javaProject.getOption(JavaCore.COMPILER_PB_FORBIDDEN_REFERENCE, true)) && JavaCore.IGNORE.equals(javaProject.getOption(JavaCore.COMPILER_PB_DISCOURAGED_REFERENCE, true))) ? null : entry.getAccessRuleSet();
                        ...
                        bLocations.add(bLocation);
                        if (// normal builder mode
                        binaryLocationsPerProject != null) {
        ...
        if (!sLocations.isEmpty()) {
            ...
            next: for (int i = 0, l = this.sourceLocations.length; i < l; i++) {
                ...
                for (// compare against previously walked source folders
                int j = 0; // compare against previously walked source folders
                j < i; // compare against previously walked source folders
                j++) {
                ...
                for (int j = 0, m = this.sourceLocations.length; j < m; j++) if (outputPath.equals(this.sourceLocations[j].sourceFolder.getFullPath()))
                    continue next;
    ...
    public void cleanup() {
        ...
        for (int i = 0, l = this.sourceLocations.length; i < l; i++) this.sourceLocations[i].cleanup();
        for (int i = 0, l = this.binaryLocations.length; i < l; i++) this.binaryLocations[i].cleanup();
    }
    ...
    private NameEnvironmentAnswer findClass(String qualifiedTypeName, char[] typeName) {
        ...
        if (this.additionalUnits != null && this.sourceLocations.length > 0) {
            ...
            if (unit != null)
                return new NameEnvironmentAnswer(/*no access restriction*/
                unit, null);
            int index = qualifiedTypeName.indexOf('$');
            if (index > 0) {
                String enclosingTypeName = qualifiedTypeName.substring(0, index);
                // doesn't have file extension
                unit = (SourceFile) this.additionalUnits.get(enclosingTypeName);
                if (unit != null)
                    return new NameEnvironmentAnswer(/*no access restriction*/
                    unit, null);
            }
    ...
    public boolean isPackage(String qualifiedPackageName) {
        // NOTE: the output folders are added at the beginning of the binaryLocations
        for (int i = 0, l = this.binaryLocations.length; i < l; i++) if (this.binaryLocations[i].isPackage(qualifiedPackageName))
            return true;
        return false;
    }
    ...
    void setNames(String[] typeNames, SourceFile[] additionalFiles) {
        ...
        if (typeNames == null) {
            ...
            for (int i = 0, l = typeNames.length; i < l; i++) this.initialTypeNames.add(typeNames[i]);
        }
        ...
        for (int i = 0, l = this.sourceLocations.length; i < l; i++) this.sourceLocations[i].reset();
        for (int i = 0, l = this.binaryLocations.length; i < l; i++) this.binaryLocations[i].reset();
    }
}
