...
public class QualifiedNameReference extends NameReference {
    ...
    // points into tokens & sourcePositions for the first token that corresponds to first FieldBinding
    ...
    public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, Assignment assignment, boolean isCompound) {
        ...
        if (lastFieldBinding.isFinal()) {
            ...
            if (otherBindingsCount == 0 && this.indexOfFirstFieldBinding == 1 && lastFieldBinding.isBlankFinal() && !isCompound && currentScope.allowBlankFinalFieldAssignment(lastFieldBinding)) {
                ...
                if (// pretend it got assigned
                otherBindingsCount == 0 && currentScope.allowBlankFinalFieldAssignment(lastFieldBinding)) {
                    flowInfo.markAsDefinitelyAssigned(lastFieldBinding);
                }
            }
        }
        // note: not covering def.assign for @NonNull: QNR cannot provably refer to a variable of the current object
        manageSyntheticAccessIfNecessary(currentScope, lastFieldBinding, /*write-access*/
        -1, flowInfo);
        return flowInfo;
    }
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo, boolean valueRequired) {
        ...
        switch(this.bits & ASTNode.RestrictiveFlagMASK) {
            ...
            Binding.FIELD:
                ...
                if (// was an implicit reference to the first field binding
    ...
    private void checkInternalNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, boolean checkString) {
        if ((this.bits & ASTNode.RestrictiveFlagMASK) == Binding.LOCAL) {
            ...
            if (local != null && (local.type.tagBits & TagBits.IsBaseType) == 0 && (checkString || local.type.id != TypeIds.T_JavaLangString)) {
 ...
 */
        ...
        if (this.constant != Constant.NotAConstant) {
            ...
            if (lastFieldBinding != null) {
                ...
                if (fieldConstant != Constant.NotAConstant) {
                    ...
                    if (// inline the last field constant
                    valueRequired) {
                    ...
                    if (valueRequired || (!isFirst && currentScope.compilerOptions().complianceLevel >= ClassFileConstants.JDK1_4) || ((this.implicitConversion & TypeIds.UNBOXING) != 0) || requiredGenericCast != null) {
                        int lastFieldPc = codeStream.position;
                        if (// array length
                        lastFieldBinding.declaringClass == // array length
                        null) {
                            ...
                            if (accessor == null) {
                                ...
                                codeStream.invoke(Opcodes.OPC_invokestatic, /* default declaringClass */
                                accessor, null);
                            }
    ...
    public void generateCompoundAssignment(BlockScope currentScope, CodeStream codeStream, Expression expression, int operator, int assignmentImplicitConversion, boolean valueRequired) {
        ...
        if (lastFieldBinding.isStatic()) {
            if (accessor == null) {
                ...
                codeStream.invoke(Opcodes.OPC_invokestatic, accessor, /* default declaringClass */
                null);
            }
        } else {
            codeStream.dup();
    ...
    public void generatePostIncrement(BlockScope currentScope, CodeStream codeStream, CompoundAssignment postIncrement, boolean valueRequired) {
        ...
        if (lastFieldBinding.isStatic()) {
            ...
            codeStream.dup();
            if (accessor == null) {
                codeStream.fieldAccess(Opcodes.OPC_getfield, lastFieldBinding, /* default declaringClass */
                null);
            } else {
                codeStream.invoke(Opcodes.OPC_invokestatic, accessor, /* default declaringClass */
                null);
 ...
 */
    public FieldBinding generateReadSequence(BlockScope currentScope, CodeStream codeStream) {
        ...
        // can be null if initial was a local binding
 ...
 */
    ...
    public TypeBinding getOtherFieldBindings(BlockScope scope) {
        ...
        if (//	restrictiveFlag == FIELD
        index == length) {
            this.constant = ((FieldBinding) this.binding).constant(scope);
            // perform capture conversion if read access
            return (type != null && (this.bits & ASTNode.IsStrictlyAssigned) == 0) ? type.capture(scope, this.sourceStart, this.sourceEnd) : type;
        }
        ...
        while (index < length) {
            ...
            if (field.isValidBinding()) {
                ...
                if (previousField != null) {
                    ...
                    if (// record need for explicit cast at codegen
                    TypeBinding.notEquals(fieldReceiverType, oldReceiverType) || originalBinding.type.leafComponentType().isTypeVariable()) {
                        // type cannot be base-type even in boxing case
                        setGenericCast(index - 1, originalBinding.type.genericCast(fieldReceiverType));
                    }
                }
                ...
                if (field.isStatic()) {
                    ...
                    (field.modifiers & ClassFileConstants.AccEnum) != 0) {
                        ...
                        if ((this.bits & ASTNode.IsStrictlyAssigned) == 0 && TypeBinding.equalsEquals(sourceType, declaringClass) && methodScope.lastVisibleFieldID >= 0 && field.id >= methodScope.lastVisibleFieldID && (!field.isStatic() || methodScope.isStatic)) {
                            scope.problemReporter().forwardReference(this, index, field);
                        }
                        // check if accessing enum static field in initializer
                        if (// enum constant body
                        (TypeBinding.equalsEquals(sourceType, declaringClass) || TypeBinding.equalsEquals(sourceType.superclass, declaringClass)) && field.constant(scope) == Constant.NotAConstant && !methodScope.isStatic && methodScope.isInsideInitializerOrConstructor()) {
    ...
    public boolean isEquivalent(Reference reference) {
        if (reference instanceof FieldReference) {
            // comparison FR <-> QNR is implemented only once
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FieldBinding fieldBinding, int index, FlowInfo flowInfo) {
        ...
        fieldBinding.isPrivate()) {
            ...
            if (TypeBinding.notEquals(declaringClass, currentScope.enclosingSourceType())) {
                setSyntheticAccessor(fieldBinding, index, ((SourceTypeBinding) declaringClass).addSyntheticMethod(codegenField, index >= 0, /*not super access*/
                false));
                currentScope.problemReporter().needToEmulateFieldAccess(codegenField, this, /*read-access?*/
                index >= 0);
                return;
            }
        } else if (fieldBinding.isProtected()) {
            int depth = (index == 0 || (index < 0 && this.otherDepths == null)) ? (this.bits & ASTNode.DepthMASK) >> ASTNode.DepthSHIFT : this.otherDepths[index < 0 ? this.otherDepths.length - 1 : index - 1];
            // implicit protected access
            if (depth > 0 && (fieldBinding.declaringClass.getPackage() != currentScope.enclosingSourceType().getPackage())) {
                FieldBinding codegenField = getCodegenBinding(index < 0 ? (this.otherBindings == null ? 0 : this.otherBindings.length) : index);
                setSyntheticAccessor(fieldBinding, index, ((SourceTypeBinding) currentScope.enclosingSourceType().enclosingTypeAt(depth)).addSyntheticMethod(codegenField, index >= 0, /*not super access*/
                false));
                currentScope.problemReporter().needToEmulateFieldAccess(codegenField, this, /*read-access?*/
                index >= 0);
                return;
                    ...
                    if (this.binding instanceof FieldBinding) {
                        ...
                        if (fieldBinding.isStatic()) {
                            ...
                            if (declaringClass.isEnum()) {
                                if ((TypeBinding.equalsEquals(sourceType, declaringClass) || // enum constant body
                                TypeBinding.equalsEquals(// enum constant body
                                sourceType.superclass, // enum constant body
                                declaringClass)) && fieldBinding.constant(scope) == Constant.NotAConstant && !methodScope.isStatic && methodScope.isInsideInitializerOrConstructor()) {
                            ...
                            this.indexOfFirstFieldBinding > 1 || // the field is the first token of the qualified reference....
                            inStaticContext) {
