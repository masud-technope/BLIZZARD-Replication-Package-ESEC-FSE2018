...
public class MethodScope extends BlockScope {
    ...
    public void computeLocalVariablePositions(int initOffset, CodeStream codeStream) {
        ...
        if (referenceContext instanceof AbstractMethodDeclaration) {
            ...
            if (!(method.isAbstract() || (method.isImplementing() && !options.reportUnusedParameterWhenImplementingAbstract) || (method.isOverriding() && !method.isImplementing() && !options.reportUnusedParameterWhenOverridingConcrete) || method.isMain())) {
        ...
        while (ilocal < maxLocals) {
            ...
            // done with arguments
            if (local == null || !local.isArgument)
                break;
            // do not report fake used variable
            if (isReportingUnusedArgument && local.useFlag == LocalVariableBinding.UNUSED && // declaration is reachable
	...
	*/
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite, boolean needResolve) {
        ...
        if (invocationSite instanceof SingleNameReference)
            ...
            field, field.declaringClass, fieldName, NonStaticReferenceInConstructorInvocation);
	...
	*/
    ...
    String basicToString(int tab) {
        ...
        //$NON-NLS-1$
        s += newLine + "initializedField = " + initializedField;
        //$NON-NLS-1$
        s += newLine + "lastVisibleFieldID = " + lastVisibleFieldID;
        //$NON-NLS-1$
        s += newLine + "referenceContext = " + referenceContext;
        return s;
    }
}
