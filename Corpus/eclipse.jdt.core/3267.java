            ...
            switch(c) {
                ...
                default:
                    if (innerTypeStart != -1 && !inAnonymousType && Character.isDigit(c)) {
                        ...
                        // remove '.'
                        buffer.setLength(// remove '.'
                        innerTypeStart);
                        //$NON-NLS-1$
                        buffer.insert(//$NON-NLS-1$
 ...
 */
    ...
    private static int encodeQualifiedName(char[] typeName, int pos, int length, StringBuffer buffer) {
        ...
        nameLoop: while (pos < length) {
            ...
            switch(currentChar) {
                ...
                default:
                    if (currentChar == ' ' || ScannerHelper.isWhitespace(currentChar)) {
                        if (// allow spaces after a dot
                        lastAppendedChar == C_DOT) {
                            // will be incremented
                            pos = consumeWhitespace(typeName, pos, length) - 1;
                            break;
                        }
    ...
    private static int encodeTypeSignature(char[] typeName, int start, boolean isResolved, int length, StringBuffer buffer) {
        ...
        while (// loop on qualifiedName[<args>][.qualifiedName[<args>]*
        true) {
 ...
 */
    public static int getArrayCount(char[] typeSignature) throws IllegalArgumentException {
        try {
            ...
            return count;
        } catch (ArrayIndexOutOfBoundsException // signature is syntactically incorrect if last character is C_ARRAY
        e) {
            throw new IllegalArgumentException();
        }
    }
 ...
 */
    public static char[][] getSimpleNames(char[] name) {
        ...
        if (length == 0)
            return CharOperation.NO_CHAR_CHAR;
        int wordCount = 1;
        countingWords: for (int i = 0; i < length; i++) switch(name[i]) {
            case C_DOT:
                wordCount++;
                break;
            case C_GENERIC_START:
 ...
 */
    public static char[][] getTypeArguments(char[] parameterizedTypeSignature) throws IllegalArgumentException {
        ...
        if (length < 2 || parameterizedTypeSignature[length - 2] != C_GENERIC_END)
            ...
            return CharOperation.NO_CHAR_CHAR;
        // start to count generic end/start peers
        int count = 1;
        int start = length - 2;
        ...
        if (// invalid number of generic start/end
        start < 0)
            throw new IllegalArgumentException();
        ArrayList args = new ArrayList();
        int p = start + 1;
