...
*/
    public final boolean canBeSeenByForCodeSnippet(ReferenceBinding referenceBinding, ReferenceBinding receiverType) {
        ...
        if (referenceBinding.isProtected()) {
            ...
            return receiverType.fPackage == referenceBinding.fPackage || referenceBinding.isSuperclassOf(receiverType) || // protected types always have an enclosing one
    ...
    public FieldBinding findFieldForCodeSnippet(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite) {
        ...
        while (keepLooking) {
            ...
            if ((currentType = currentType.superclass()) == null)
                break;
            if ((field = currentType.getField(fieldName, /*resolve*/
            true)) != null) {
                keepLooking = false;
...
*/
    public Binding getBinding(char[][] compoundName, int mask, InvocationSite invocationSite, ReferenceBinding receiverType) {
        Binding binding = getBinding(compoundName[0], mask | Binding.TYPE | Binding.PACKAGE, invocationSite, true);
        ...
        foundType: if (binding instanceof PackageBinding) {
            ...
            while (currentIndex < length) {
                ...
                if (binding == null) {
                    if (// must be a type if its the last name, otherwise we have no idea if its a package or type
                    currentIndex == length)
        ...
        if (// was looking for a field and found a field
        (mask & Binding.FIELD) != 0 && (binding instanceof FieldBinding)) {
        ...
        if (// was looking for a type and found a type
        (mask & Binding.TYPE) != 0 && (binding instanceof ReferenceBinding)) {
            return binding;
        }
