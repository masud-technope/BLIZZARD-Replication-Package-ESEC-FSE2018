...
public class FilerTesterProc extends AbstractProcessor {
    ...
    private void round(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        for (Element annotatedEl : annotatedEls) {
            ...
            if (null != testMethodName && testMethodName.length() > 0) {
                try {
                    Method testMethod = FilerTesterProc.class.getMethod(testMethodName, Element.class, String.class, String.class);
    ...
    public void testCreateNonSourceFile(Element e, String pkg, String relName) throws Exception {
        FileObject fo = _filer.createResource(StandardLocation.SOURCE_OUTPUT, pkg, relName, e);
        PrintWriter pw = null;
        try {
        ...
        if (!name.contains(relName)) {
            ProcessorTestStatus.fail("File object getName() returned " + name + ", expected it to contain " + relName);
        }
    }
    ...
    public void testNullParents(Element e, String pkg, String relName) throws Exception {
        FileObject fo = _filer.createResource(StandardLocation.SOURCE_OUTPUT, pkg, relName + ".txt", (Element[]) null);
        PrintWriter pw = null;
        try {
            pw = new PrintWriter(fo.openWriter());
            pw.println("Hello world");
    ...
    public void testURI(Element e, String pkg, String relName) throws Exception {
        ...
        FileObject foGenNonSrc = _filer.createResource(StandardLocation.SOURCE_OUTPUT, pkg, relName, e);
    ...
    private void checkGenUri(FileObject fo, String name, String content, String category) throws Exception {
        ...
        if (!uri.toString().contains(name)) {
            ProcessorTestStatus.fail("toUri() on " + category + " returned " + uri.toString() + ", expected it to contain " + name);
        }
        ...
        if (!Arrays.equals(buf, content.toCharArray())) {
            ProcessorTestStatus.fail("toUri() on " + category + " returned " + uri.toString() + ", but reading that URI produced \"" + new String(buf) + "\" instead of expected \"" + content + "\"");
        }
    }
    ...
    private void checkResourceContents01(FileObject resource, String expectedName, String expectedContents) throws Exception {
        ...
        if (!expectedName.equals(actualName)) {
            System.out.println("Resource had unexpected name.  Expected " + expectedName + ", actual was " + actualName);
