...
public abstract class TypeDeclarationImpl extends MemberDeclarationImpl implements TypeDeclaration, DeclaredType, ReferenceType, EclipseMirrorType {
    ...
    public  TypeDeclarationImpl(final ITypeBinding binding, final BaseProcessorEnv env) {
        super(binding, env);
    }
    ...
    public ITypeBinding getTypeBinding() {
        return (ITypeBinding) _binding;
    }

    private void getASTFields(final AbstractTypeDeclaration typeDecl, final List<FieldDeclaration> results) {
        ...
        for (int i = 0, len = bodyDecls.size(); i < len; i++) {
            ...
            if (bodyDecl.getNodeType() == ASTNode.FIELD_DECLARATION) {
                final List<VariableDeclarationFragment> fragments = ((org.eclipse.jdt.core.dom.FieldDeclaration) bodyDecl).fragments();
            ...
            if (node != null) {
                switch(node.getNodeType()) {
                    ...
                    default:
                        ...
                        "expecting a AbstractTypeDeclaration but got " + node.getClass().getName());
                }
            }
        }
    ...
    public TypeDeclaration getDeclaration() {
        ...
        if (declBinding == _binding)
            return this;
        else
            return Factory.createReferenceType(declBinding, _env);
    }
    ...
    protected void getASTMethods(final AbstractTypeDeclaration typeDecl, final List<MethodDeclaration> results) {
        final List bodyDecls = typeDecl.bodyDeclarations();
        IFile file = null;
        for (int i = 0, len = bodyDecls.size(); i < len; i++) {
            final BodyDeclaration bodyDecl = (BodyDeclaration) bodyDecls.get(i);
            switch(bodyDecl.getNodeType()) {
                case ASTNode.METHOD_DECLARATION:
                    final org.eclipse.jdt.core.dom.MethodDeclaration methodDecl = (org.eclipse.jdt.core.dom.MethodDeclaration) bodyDecl;
                    if (!methodDecl.isConstructor()) {
                        ...
                        if (methodBinding == null) {
                            if (file == null)
                                file = getResource();
                            MethodDeclaration mirrorDecl = (MethodDeclaration) Factory.createDeclaration(methodDecl, file, _env);
                            if (mirrorDecl != null)
                ...
                case ASTNode.ANNOTATION_TYPE_MEMBER_DECLARATION:
                    final AnnotationTypeMemberDeclaration memberDecl = (AnnotationTypeMemberDeclaration) bodyDecl;
                    final IMethodBinding methodBinding = memberDecl.resolveBinding();
                    // built an ast based representation.
                    if (methodBinding == null) {
                        if (file == null)
                            file = getResource();
                        MethodDeclaration mirrorDecl = (MethodDeclaration) Factory.createDeclaration(memberDecl, file, _env);
                        if (mirrorDecl != null)
                            results.add(mirrorDecl);
                    }
                    break;
    ...
    public boolean isFromSource() {
        return getDeclarationBinding().isFromSource();
    }
    ...
    public boolean isSubTypeCompatible(EclipseMirrorType type) {
        ...
        } else //kind() == MirrorKind.TYPE_INTERFACE
        {
            ...
            if (type.kind() == MirrorKind.TYPE_CLASS)
                return //$NON-NLS-1$
                "java.lang.Object".equals(//$NON-NLS-1$
                getQualifiedName());
            return false;
        }
    }
