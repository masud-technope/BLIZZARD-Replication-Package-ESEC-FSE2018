...
public class CompilerOptions {
    ...
    // 0 or TagBits#AnnotationNonNull
...

    {
        ...
        //$NON-NLS-1$
        useunspecdtypeinferenceperformanceoptimization = useunspecdtypeinferenceoptimization != null && useunspecdtypeinferenceoptimization.equalsIgnoreCase("true");
    }
    ...
    public static final String[] warningTokens = { //$NON-NLS-1$
    "all", //$NON-NLS-1$
    "boxing", //$NON-NLS-1$
    "cast", //$NON-NLS-1$
    "dep-ann", //$NON-NLS-1$
    ...
    public static IrritantSet warningTokenToIrritants(String warningToken) {
        ...
        switch(warningToken.charAt(0)) {
            case 'a':
                if (//$NON-NLS-1$
                "all".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.ALL;
                break;
            case 'b':
                if (//$NON-NLS-1$
                "boxing".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.BOXING;
                break;
            case 'c':
                if (//$NON-NLS-1$
                "cast".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.CAST;
                break;
            case 'd':
                if (//$NON-NLS-1$
                "deprecation".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.DEPRECATION;
                if (//$NON-NLS-1$
                "dep-ann".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.DEP_ANN;
                break;
            case 'f':
                if (//$NON-NLS-1$
                "fallthrough".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.FALLTHROUGH;
                if (//$NON-NLS-1$
                "finally".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.FINALLY;
                break;
            case 'h':
                if (//$NON-NLS-1$
                "hiding".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.HIDING;
                break;
            case 'i':
                if (//$NON-NLS-1$
                "incomplete-switch".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.INCOMPLETE_SWITCH;
                break;
            case 'j':
                if (//$NON-NLS-1$
                "javadoc".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.JAVADOC;
                break;
            case 'n':
                if (//$NON-NLS-1$
                "nls".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.NLS;
                if (//$NON-NLS-1$
                "null".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.NULL;
                break;
            case 'r':
                if (//$NON-NLS-1$
                "rawtypes".equals(//$NON-NLS-1$
                warningToken))
                ...
                if (//$NON-NLS-1$
                "restriction".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.RESTRICTION;
                break;
            case 's':
                if (//$NON-NLS-1$
                "serial".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.SERIAL;
                if (//$NON-NLS-1$
                "static-access".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.STATIC_ACCESS;
                if (//$NON-NLS-1$
                "static-method".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.STATIC_METHOD;
                if (//$NON-NLS-1$
                "synthetic-access".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.SYNTHETIC_ACCESS;
                if (//$NON-NLS-1$
                "super".equals(warningToken)) {
                    return IrritantSet.SUPER;
                }
                if (//$NON-NLS-1$
                "sync-override".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.SYNCHRONIZED;
                break;
            case 'u':
                if (//$NON-NLS-1$
                "unused".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.UNUSED;
                if (//$NON-NLS-1$
                "unchecked".equals(//$NON-NLS-1$
                warningToken))
                    return IrritantSet.UNCHECKED;
                if (//$NON-NLS-1$
                "unqualified-field-access".equals(//$NON-NLS-1$
                warningToken))
    ...
    public int getSeverity(int irritant) {
        if (this.errorThreshold.isSet(irritant)) {
            if ((irritant & (IrritantSet.GROUP_MASK | UnusedWarningToken)) == UnusedWarningToken) {
                // cannot be treated as fatal - codegen already occurred
                return ProblemSeverities.Error | ProblemSeverities.Optional;
            }
            return this.treatOptionalErrorAsFatal ? ProblemSeverities.Error | ProblemSeverities.Optional | ProblemSeverities.Fatal : ProblemSeverities.Error | ProblemSeverities.Optional;
        }
    ...
    protected void resetDefaults() {
        ...
        this.complianceLevel = this.originalComplianceLevel = ClassFileConstants.JDK1_4;
        //1.3 source behavior by default
        this.sourceLevel = this.originalSourceLevel = ClassFileConstants.JDK1_3;
        ...
        // max problems per compilation unit
        // no more than 100 problems per default
    ...
    public void set(Map<String, String> optionsMap) {
        ...
        if ((optionValue = optionsMap.get(OPTION_InlineJsr)) != null) {
            if (// only optional if target < 1.5 (inlining on from 1.5 on)
        ...
        if (getSeverity(UnclosedCloseable) == ProblemSeverities.Ignore && getSeverity(PotentiallyUnclosedCloseable) == ProblemSeverities.Ignore && getSeverity(ExplicitlyClosedAutoCloseable) == ProblemSeverities.Ignore) {
    ...
    public String toString() {
        ...
        buf.append("\n\t- local variables debug attributes: ").append((this.produceDebugAttributes & ClassFileConstants.ATTR_VARS) != 0 ? "ON" : " OFF");
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        buf.append("\n\t- line number debug attributes: ").append((this.produceDebugAttributes & ClassFileConstants.ATTR_LINES) != 0 ? "ON" : " OFF");
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        buf.append("\n\t- source debug attributes: ").append((this.produceDebugAttributes & ClassFileConstants.ATTR_SOURCE) != 0 ? "ON" : " OFF");
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- method with constructor name: ").append(getSeverityString(MethodWithConstructorName));
        //$NON-NLS-1$
        buf.append("\n\t- overridden package default method: ").append(getSeverityString(OverriddenPackageDefaultMethod));
        //$NON-NLS-1$
        buf.append("\n\t- deprecation: ").append(getSeverityString(UsingDeprecatedAPI));
        //$NON-NLS-1$
        buf.append("\n\t- masked catch block: ").append(getSeverityString(MaskedCatchBlock));
        //$NON-NLS-1$
        buf.append("\n\t- unused local variable: ").append(getSeverityString(UnusedLocalVariable));
        //$NON-NLS-1$
        buf.append("\n\t- unused parameter: ").append(getSeverityString(UnusedArgument));
        //$NON-NLS-1$
        buf.append("\n\t- unused exception parameter: ").append(getSeverityString(UnusedExceptionParameter));
        //$NON-NLS-1$
        buf.append("\n\t- unused import: ").append(getSeverityString(UnusedImport));
        //$NON-NLS-1$
        buf.append("\n\t- synthetic access emulation: ").append(getSeverityString(AccessEmulation));
        //$NON-NLS-1$
        buf.append("\n\t- assignment with no effect: ").append(getSeverityString(NoEffectAssignment));
        //$NON-NLS-1$
        buf.append("\n\t- non externalized string: ").append(getSeverityString(NonExternalizedString));
        //$NON-NLS-1$
        buf.append("\n\t- static access receiver: ").append(getSeverityString(NonStaticAccessToStatic));
        //$NON-NLS-1$
        buf.append("\n\t- indirect static access: ").append(getSeverityString(IndirectStaticAccess));
        //$NON-NLS-1$
        buf.append("\n\t- incompatible non inherited interface method: ").append(getSeverityString(IncompatibleNonInheritedInterfaceMethod));
        //$NON-NLS-1$
        buf.append("\n\t- unused private member: ").append(getSeverityString(UnusedPrivateMember));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- type hiding another type: ").append(getSeverityString(TypeHiding));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- uncommented empty block: ").append(getSeverityString(UndocumentedEmptyBlock));
        //$NON-NLS-1$
        buf.append("\n\t- unnecessary type check: ").append(getSeverityString(UnnecessaryTypeCheck));
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        buf.append("\n\t- javadoc comment support: ").append(this.docCommentSupport ? "ON" : " OFF");
        //$NON-NLS-1$
        buf.append("\n\t\t+ invalid javadoc: ").append(getSeverityString(InvalidJavadoc));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t\t+ missing javadoc comments: ").append(getSeverityString(MissingJavadocComments));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- finally block not completing normally: ").append(getSeverityString(FinallyBlockNotCompleting));
        //$NON-NLS-1$
        buf.append("\n\t- report unused declared thrown exception: ").append(getSeverityString(UnusedDeclaredThrownException));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- unnecessary else: ").append(getSeverityString(UnnecessaryElse));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- unsafe type operation: ").append(getSeverityString(UncheckedTypeOperation));
        //$NON-NLS-1$
        buf.append("\n\t- unsafe raw type: ").append(getSeverityString(RawTypeReference));
        //$NON-NLS-1$
        buf.append("\n\t- final bound for type parameter: ").append(getSeverityString(FinalParameterBound));
        //$NON-NLS-1$
        buf.append("\n\t- missing serialVersionUID: ").append(getSeverityString(MissingSerialVersion));
        //$NON-NLS-1$
        buf.append("\n\t- varargs argument need cast: ").append(getSeverityString(VarargsArgumentNeedCast));
        //$NON-NLS-1$
        buf.append("\n\t- forbidden reference to type with access restriction: ").append(getSeverityString(ForbiddenReference));
        //$NON-NLS-1$
        buf.append("\n\t- discouraged reference to type with access restriction: ").append(getSeverityString(DiscouragedReference));
        //$NON-NLS-1$
        buf.append("\n\t- null reference: ").append(getSeverityString(NullReference));
        //$NON-NLS-1$
        buf.append("\n\t- potential null reference: ").append(getSeverityString(PotentialNullReference));
        //$NON-NLS-1$
        buf.append("\n\t- redundant null check: ").append(getSeverityString(RedundantNullCheck));
        //$NON-NLS-1$
        buf.append("\n\t- autoboxing: ").append(getSeverityString(AutoBoxing));
        //$NON-NLS-1$
        buf.append("\n\t- annotation super interface: ").append(getSeverityString(AnnotationSuperInterface));
        //$NON-NLS-1$
        buf.append("\n\t- missing @Override annotation: ").append(getSeverityString(MissingOverrideAnnotation));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- missing @Deprecated annotation: ").append(getSeverityString(MissingDeprecatedAnnotation));
        //$NON-NLS-1$
        buf.append("\n\t- incomplete enum switch: ").append(getSeverityString(MissingEnumConstantCase));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- unhandled warning token: ").append(getSeverityString(UnhandledWarningToken));
        //$NON-NLS-1$
        buf.append("\n\t- unused warning token: ").append(getSeverityString(UnusedWarningToken));
        //$NON-NLS-1$
        buf.append("\n\t- unused label: ").append(getSeverityString(UnusedLabel));
        //$NON-NLS-1$
        buf.append("\n\t- treat optional error as fatal: ").append(this.treatOptionalErrorAsFatal ? ENABLED : DISABLED);
        //$NON-NLS-1$
        buf.append("\n\t- parameter assignment: ").append(getSeverityString(ParameterAssignment));
        //$NON-NLS-1$
        ...
        //$NON-NLS-1$
        buf.append("\n\t- unused type arguments for method/constructor invocation: ").append(getSeverityString(UnusedTypeArguments));
        //$NON-NLS-1$
        buf.append("\n\t- redundant superinterface: ").append(getSeverityString(RedundantSuperinterface));
        //$NON-NLS-1$
        buf.append("\n\t- comparing identical expr: ").append(getSeverityString(ComparingIdentical));
        //$NON-NLS-1$
        buf.append("\n\t- missing synchronized on inherited method: ").append(getSeverityString(MissingSynchronizedModifierInInheritedMethod));
        //$NON-NLS-1$
        buf.append("\n\t- should implement hashCode() method: ").append(getSeverityString(ShouldImplementHashcode));
        //$NON-NLS-1$
        buf.append("\n\t- dead code: ").append(getSeverityString(DeadCode));
        //$NON-NLS-1$
        buf.append("\n\t- dead code in trivial if statement: ").append(this.reportDeadCodeInTrivialIfStatement ? ENABLED : DISABLED);
        //$NON-NLS-1$
        buf.append("\n\t- tasks severity: ").append(getSeverityString(Tasks));
        //$NON-NLS-1$
        buf.append("\n\t- unused object allocation: ").append(getSeverityString(UnusedObjectAllocation));
        //$NON-NLS-1$
        buf.append("\n\t- method can be static: ").append(getSeverityString(MethodCanBeStatic));
        //$NON-NLS-1$
        buf.append("\n\t- method can be potentially static: ").append(getSeverityString(MethodCanBePotentiallyStatic));
        //$NON-NLS-1$
        buf.append("\n\t- redundant specification of type arguments: ").append(getSeverityString(RedundantSpecificationOfTypeArguments));
        //$NON-NLS-1$
        buf.append("\n\t- resource is not closed: ").append(getSeverityString(UnclosedCloseable));
        //$NON-NLS-1$
        buf.append("\n\t- resource may not be closed: ").append(getSeverityString(PotentiallyUnclosedCloseable));
        //$NON-NLS-1$
        buf.append("\n\t- resource should be handled by try-with-resources: ").append(getSeverityString(ExplicitlyClosedAutoCloseable));
        //$NON-NLS-1$
        buf.append("\n\t- Unused Type Parameter: ").append(getSeverityString(UnusedTypeParameter));
        //$NON-NLS-1$
