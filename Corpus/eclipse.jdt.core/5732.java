 ...
 */
    public void testBug101022() throws CoreException {
        try {
            ...
            assertSearchResults("test/Test.java [in P1] void Test.foo() [foo]", resultCollector);
        } finally {
            deleteProject("P1");
        }
    }
 ...
 */
    public void testBug101777() throws CoreException {
        try {
            ...
            assertSearchResults("Test.java [in P1] void Test.main(String[]) [main]", resultCollector);
        } finally {
            deleteProject("P1");
        }
    }
 ...
 */
    public void testBug119203() throws CoreException {
        try {
            ...
            assertSearchResults("src/X.java [in P1] X.test [Test]", resultCollector);
        } finally {
            deleteProject("P1");
        }
    }
 ...
 */
    public void testBug179199() throws CoreException {
        try {
            ...
            new SearchEngine().searchAllTypeNames(null, SearchPattern.R_EXACT_MATCH, "Test".toCharArray(), SearchPattern.R_PREFIX_MATCH, IJavaSearchConstants.TYPE, scope, collector, IJavaSearchConstants.WAIT_UNTIL_READY_TO_SEARCH, null);
            // Verify results
            assertSearchResults("Test (not open) [in Test.class [in <default> [in bin [in P1]]]]", collector);
        } finally {
            deleteProject("P1");
        }
    }
                    ...
                    for (int i = 0; i < max; i++) {
                        ...
                        for (int j = 0; j < max; j++) {
                            ...
                            if (idx != -1) {
                                dependents[idx] = "/P" + j;
                                // export all projects
                                exportedProjects[idx] = // export all projects
                                true;
                            }
                        }
