...
public class ClassFile implements AttributeNamesConstants, CompilerModifiers, TypeConstants, TypeIds {
    ...
    // flag set when header/contents are set to shared arrays
    ...
    public  ClassFile(SourceTypeBinding aType, ClassFile enclosingClassFile, boolean creatingProblemType) {
        ...
        // minor high
        header[headerOffset++] = (byte) (this.targetJDK >> 8);
        // minor low
        header[headerOffset++] = (byte) (this.targetJDK >> 0);
        // major high
        header[headerOffset++] = (byte) (this.targetJDK >> 24);
        // major low
        header[headerOffset++] = (byte) (this.targetJDK >> 16);
    ...
    public void addProblemClinit(IProblem[] problems) {
        ...
        attributeNumber++;
        completeCodeAttributeForClinit(codeAttributeOffset, referenceBinding.scope.referenceCompilationUnit().compilationResult.lineSeparatorPositions, problemLine);
        contents[attributeOffset++] = (byte) (attributeNumber >> 8);
        contents[attributeOffset] = (byte) attributeNumber;
    }
    ...
    public void addProblemConstructor(AbstractMethodDeclaration method, MethodBinding methodBinding, IProblem[] problems) {
        ...
        codeStream.generateCodeAttributeForProblemMethod(problemString);
        completeCodeAttributeForProblemMethod(method, methodBinding, codeAttributeOffset, ((SourceTypeBinding) methodBinding.declaringClass).scope.referenceCompilationUnit().compilationResult.lineSeparatorPositions, problemLine);
        completeMethodInfo(methodAttributeOffset, attributeNumber);
    }
        ...
        if (methodDeclarations != null) {
            for (int i = 0, max = methodDeclarations.length; i < max; i++) {
                ...
                for (int j = 0; j < problemsCount; j++) {
                    ...
                    if (problem != null && problem.getID() == IProblem.AbstractMethodMustBeImplemented && problem.getMessage().indexOf(readableName) != -1) {
    ...
    private void addMissingAbstractProblemMethod(MethodDeclaration methodDeclaration, MethodBinding methodBinding, IProblem problem, CompilationResult compilationResult) {
        ...
        codeStream.generateCodeAttributeForProblemMethod(problemString);
        completeCodeAttributeForMissingAbstractProblemMethod(methodBinding, codeAttributeOffset, compilationResult.lineSeparatorPositions, problem.getSourceLineNumber());
        completeMethodInfo(methodAttributeOffset, attributeNumber);
    }
    ...
    public static String buildAllDirectoriesInto(String outputPath, String relativeFileName) throws IOException {
        ...
        if (f.exists()) {
            if (!f.isDirectory()) {
                ...
                Util.bind("output.isFile", f.getAbsolutePath()));
                throw new //$NON-NLS-1$
                IOException(//$NON-NLS-1$
                Util.bind("output.isFileNotDirectory"));
            }
        } else {
            ...
            if (!f.mkdirs()) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                Util.bind("output.dirName", f.getAbsolutePath()));
                throw new //$NON-NLS-1$
                IOException(//$NON-NLS-1$
                Util.bind("output.notValidAll"));
            }
        }
    ...
    public void completeCodeAttribute(int codeAttributeOffset) {
        ...
        if (codeStream.generateLocalVariableTableAttributes) {
            ...
            if (!methodDeclarationIsStatic) {
                ...
                descriptorIndex = constantPool.literalIndex(declaringClassBinding.signature());
            ...
            for (int i = 0, max = codeStream.allLocalsCounter; i < max; i++) {
                ...
                for (int j = 0; j < localVariable.initializationCount; j++) {
                    ...
                    startPC != endPC) {
                        if (endPC == -1) {
                            localVariable.declaringScope.problemReporter().abortDueToInternalError(Util.bind("abort.invalidAttribute", new //$NON-NLS-1$
                            String(//$NON-NLS-1$
                            localVariable.name)), (ASTNode) localVariable.declaringScope.methodScope().referenceContext);
                        }
            ...
            attributeNumber++;
            final boolean currentInstanceIsGeneric = !methodDeclarationIsStatic && declaringClassBinding != null && declaringClassBinding.typeVariables != NoTypeVariables;
            if (genericLocalVariablesCounter != 0 || currentInstanceIsGeneric) {
                ...
                int localVariableTypeNameIndex = constantPool.literalIndex(AttributeNamesConstants.LocalVariableTypeTableName);
            ...
            if ((codeStream.pcToSourceMap != null) && (codeStream.pcToSourceMapSize != 0)) {
                ...
                for (int i = 0, max = codeStream.allLocalsCounter; i < max; i++) {
                    ...
                    for (int j = 0; j < localVariable.initializationCount; j++) {
                        ...
                        startPC != endPC) {
                            if (endPC == -1) {
                                localVariable.declaringScope.problemReporter().abortDueToInternalError(//$NON-NLS-1$
                                Util.bind(//$NON-NLS-1$
                                "abort.invalidAttribute", //$NON-NLS-1$
                                new String(localVariable.name)), (ASTNode) localVariable.declaringScope.methodScope().referenceContext);
                            }
    ...
    public void recordNestedLocalAttribute(ReferenceBinding binding) {
        ...
        if (depth >= 2) {
            ...
            for (int i = 0; i < depth; i++) addInnerClasses(enclosingTypes[i]);
        } else {
    ...
    public static void writeToDisk(boolean generatePackagesStructure, String outputPath, String relativeFileName, byte[] contents) throws IOException {
        ...
        if (generatePackagesStructure) {
            output = new BufferedOutputStream(new FileOutputStream(new File(buildAllDirectoriesInto(outputPath, relativeFileName))));
        } else {
