                    ...
                    if (proxy.getType() == IResource.FOLDER) {
                        ...
                        if (prefixesOneOf(path, nestedFolders)) {
                            if (equalsOneOf(path, nestedFolders)) {
                                ...
                                if ((CopyPackageFragmentRootOperation.this.updateModelFlags & IPackageFragmentRoot.REPLACE) != 0 && folder.exists()) {
                                    return true;
                                }
                            ...
                            IResource destRes;
                            if ((CopyPackageFragmentRootOperation.this.updateModelFlags & IPackageFragmentRoot.REPLACE) != 0 && (destRes = workspaceRoot.findMember(destPath)) != null) {
            ...
            if (JavaProject.hasJavaNature(project)) {
                try {
                    ...
                    if (foundExistingEntry && (this.updateModelFlags & IPackageFragmentRoot.REPLACE) == 0) {
                        return new JavaModelStatus(IJavaModelStatusConstants.NAME_COLLISION, Messages.bind(Messages.status_nameCollision, new String[] { this.destination.toString() }));
                    }
