 ...
 */
    ...
    public void generateStoreSaveValueIfNecessary(CodeStream codeStream) {
        ...
        if (// expressionType == VoidBinding if code snippet is the expression "System.out.println()"
        this.expression == null || this.expression.resolvedType == TypeBinding.VOID) {
            // push null
        ...
        if (this.expression != null) {
            if (this.expression.resolveType(scope) != null) {
                ...
                if (!javaLangClass.isValidBinding()) {
                    //$NON-NLS-1$
                    scope.problemReporter().codeSnippetMissingClass(//$NON-NLS-1$
                    "java.lang.Class", //$NON-NLS-1$
                    this.sourceStart, //$NON-NLS-1$
                    this.sourceEnd);
                    return;
                }
