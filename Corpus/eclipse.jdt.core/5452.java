 ...
 */
    public RecoveredElement add(LocalDeclaration localDeclaration, int bracketBalanceValue) {
        if (// current element is really the lambda which is recovered in full elsewhere.
        this.lambdaNestLevel > 0)
            return this;
 ...
 */
    public RecoveredElement add(TypeDeclaration typeDeclaration, int bracketBalanceValue) {
        if (this.alreadyCompletedFieldInitialization || ((typeDeclaration.bits & ASTNode.IsAnonymousType) == 0) || (this.fieldDeclaration.declarationSourceEnd != 0 && typeDeclaration.sourceStart > this.fieldDeclaration.declarationSourceEnd)) {
                ...
                for (int i = 0; i < this.anonymousTypeCount; i++) {
                    ...
                    if (recoveredType.preserveContent) {
                        ...
                        if (anonymousType != null) {
                            ...
                            if (// https://bugs.eclipse.org/bugs/show_bug.cgi?id=307337
                            end > this.fieldDeclaration.declarationSourceEnd) {
 ...
 */
    public RecoveredElement updateOnOpeningBrace(int braceStart, int braceEnd) {
        ...
        if (this.fieldDeclaration.declarationSourceEnd == 0 && this.fieldDeclaration.getKind() == AbstractVariableDeclaration.ENUM_CONSTANT) {
            this.bracketBalance++;
