...
public class LocalVariableBinding extends VariableBinding {
    ...
    // for flow analysis (default is UNUSED), values < 0 indicate the number of compound uses (postIncrement or compoundAssignment)
    public int useFlag;
    ...
    // track closing of instances of type AutoCloseable, maybe null
	...
	*/
        ...
        if (scope != null) {
            ...
            if (referenceContext instanceof AbstractMethodDeclaration) {
                ...
                if (methodBinding != null) {
                    buffer.append(methodBinding.computeUniqueKey(/*not a leaf*/
                    false));
                }
            } else if (referenceContext instanceof TypeDeclaration) {
                TypeBinding typeBinding = ((TypeDeclaration) referenceContext).binding;
                if (typeBinding != null) {
                    buffer.append(typeBinding.computeUniqueKey(/*not a leaf*/
                    false));
                }
            ...
            for (// use linear search assuming the number of locals per scope is low
            int i = 0; // use linear search assuming the number of locals per scope is low
            i < scope.localIndex; // use linear search assuming the number of locals per scope is low
            i++) {
    ...
    public String toString() {
        ...
        switch(this.useFlag) {
            case USED:
                //$NON-NLS-2$ //$NON-NLS-1$
                s += "[pos: " + String.valueOf(this.resolvedPosition) + "]";
                break;
            case UNUSED:
                s += "[pos: unused]";
                break;
        ...
        if (this.initializationCount > 0) {
            ...
            for (int i = 0; i < this.initializationCount; i++) {
                if (i > 0)
                    ...
                    s += ", ";
                //$NON-NLS-2$ //$NON-NLS-1$
                s += String.valueOf(this.initializationPCs[i << 1]) + "-" + ((this.initializationPCs[(i << 1) + 1] == -1) ? "?" : String.valueOf(this.initializationPCs[(i << 1) + 1]));
            }
            //$NON-NLS-1$
            s += "]";
        }
