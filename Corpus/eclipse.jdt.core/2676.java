...
public class ArrayCreation extends Expression {
    ...
    new ChildPropertyDescriptor(ArrayCreation.class, "type", ArrayType.class, MANDATORY, NO_CYCLE_RISK);

    /**
	 * The "dimensions" structural property of this node type (element type: {@link Expression}).
	 * @since 3.0
	 */
    public static final ChildListPropertyDescriptor DIMENSIONS_PROPERTY = //$NON-NLS-1$
    new ChildListPropertyDescriptor(ArrayCreation.class, "dimensions", Expression.class, CYCLE_RISK);

    /**
	 * The "initializer" structural property of this node type (child type: {@link ArrayInitializer}).
	 * @since 3.0
	 */
    public static final ChildPropertyDescriptor INITIALIZER_PROPERTY = //$NON-NLS-1$
    new ChildPropertyDescriptor(ArrayCreation.class, "initializer", ArrayInitializer.class, OPTIONAL, CYCLE_RISK);
	 ...
	 */
    private ASTNode.NodeList dimensions = new ASTNode.NodeList(DIMENSIONS_PROPERTY);
	 ...
	 */
    int treeSize() {
        int size = memSize() + (this.arrayType == null ? 0 : getType().treeSize()) + (this.optionalInitializer == null ? 0 : getInitializer().treeSize()) + this.dimensions.listSize();
        return size;
    }
}
