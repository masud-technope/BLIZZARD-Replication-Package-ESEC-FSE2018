...
public class WildcardBinding extends ReferenceBinding {
    ...
    public  WildcardBinding(ReferenceBinding genericType, int rank, TypeBinding bound, int kind, LookupEnvironment environment) {
        ...
        this.modifiers = AccPublic | AccGenericSignature;
    ...
    public ReferenceBinding superclass() {
        if (this.superclass == null) {
            ...
            this.superclass = superType != null && superType.isClass() ? (ReferenceBinding) superType : environment.getType(JAVA_LANG_OBJECT);
        }
    ...
    public ReferenceBinding[] superInterfaces() {
        if (this.superInterfaces == null) {
            ...
            if (this.kind == Wildcard.EXTENDS) {
                ...
                // TODO (philippe) shouldn't it retrieve variable superinterfaces ?
                superType = this.typeVariable.firstBound;
            }
            this.superInterfaces = superType != null && superType.isInterface() ? new ReferenceBinding[] { (ReferenceBinding) superType } : NoSuperInterfaces;
        }
