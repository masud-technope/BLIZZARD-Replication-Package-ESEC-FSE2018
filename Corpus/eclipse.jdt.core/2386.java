...
public class CaptureBinding extends TypeVariableBinding {
    ...
    // to facilitate recaptures.
    public ASTNode cud;

    // for substitution of recursive captures, see https://bugs.eclipse.org/456924
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        if (isLeaf) {
            buffer.append(this.sourceType.computeUniqueKey(/*not a leaf*/
            false));
            buffer.append('&');
        }
        buffer.append(TypeConstants.WILDCARD_CAPTURE);
        buffer.append(this.wildcard.computeUniqueKey(/*not a leaf*/
        false));
    ...
    public char[] nullAnnotatedReadableName(CompilerOptions options, boolean shortNames) {
        ...
        if (// CaptureBinding18 can be recursive indeed
            ...
            try {
                if (this.wildcard != null) {
                    ...
                    if (otherUpperBounds != NO_TYPES)
                        //$NON-NLS-1$ // only hint at more bounds, we currently don't evaluate null annotations on otherUpperBounds
                        nameBuffer.append(" & ...");
                }
            } finally {
    ...
    TypeBinding substituteInferenceVariable(InferenceVariable var, TypeBinding substituteType) {
        ...
        try {
            ...
            if (//$IDENTITY-COMPARISON$
            substitutedWildcard != this.wildcard) {
                ...
                if (this.superInterfaces != null) {
                    ...
                    for (int i = 0; i < length; i++) substitute.superInterfaces[i] = (ReferenceBinding) this.superInterfaces[i].substituteInferenceVariable(var, substituteType);
                }
                return substitute;
            }
            return this;
