 ...
 */
    public static IMethod[] findMethods(IMethod method, IMethod[] methods) {
        ...
        for (int i = 0, length = methods.length; i < length; i++) {
            ...
            if (areSimilarMethods(elementName, parameters, existingMethod.getElementName(), existingMethod.getParameterTypes(), simpleNames)) {
 ...
 */
    ...
    protected boolean isMainMethod(IMethod method) throws JavaModelException {
        if (//$NON-NLS-1$
        "main".equals(method.getElementName()) && Signature.SIG_VOID.equals(method.getReturnType())) {
            ...
            if (Flags.isStatic(flags) && (Flags.isPublic(flags) || ((declaringType = getDeclaringType()) != null && declaringType.isInterface()))) {
                ...
                if (paramTypes.length == 1) {
                    ...
                    Signature.getSimpleName(typeSignature));
                }
            }
        }
        return false;
