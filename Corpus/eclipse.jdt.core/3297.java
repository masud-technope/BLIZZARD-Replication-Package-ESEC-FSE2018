...
public class LookupEnvironment implements ProblemReasons, TypeConstants {
    ...
    // the package supposed to contain the Nullable annotation type
    PackageBinding nullableAnnotationPackage;

    // the package supposed to contain the NonNull annotation type
    PackageBinding nonnullAnnotationPackage;

    // the package supposed to contain the NonNullByDefault annotation type
    ...
    // during early initialization we cannot mark Enum-methods as nonnull.
 ...
 */
    public ReferenceBinding askForType(char[][] compoundName) {
        ...
        if (answer.isBinaryType()) {
            ...
            this.typeRequestor.accept(answer.getBinaryType(), computePackageFrom(compoundName, /* valid pkg */
            false), answer.getAccessRestriction());
        } else if (answer.isCompilationUnit()) {
            // the type was found as a .java file, try to build it then search the cache
            this.typeRequestor.accept(answer.getCompilationUnit(), answer.getAccessRestriction());
        } else if (answer.isSourceType()) {
            // the type was found as a source model
            this.typeRequestor.accept(answer.getSourceTypes(), computePackageFrom(compoundName, /* valid pkg */
            false), answer.getAccessRestriction());
        }
...
*/
    public BinaryTypeBinding cacheBinaryType(IBinaryType binaryType, boolean needFieldsAndMethods, AccessRestriction accessRestriction) {
        ...
        if (existingType == null || existingType instanceof UnresolvedReferenceBinding)
            ...
            return createBinaryTypeFrom(binaryType, computePackageFrom(compoundName, /* valid pkg */
            false), needFieldsAndMethods, accessRestriction);
    ...
    public TypeBinding convertToRawType(TypeBinding type, boolean forceRawEnclosingType) {
        ...
        if (originalEnclosing == null) {
            ...
            } else if (forceRawEnclosingType && !needToConvert) /*stop recursion when conversion occurs*/
            {
                ...
                // only convert generic or parameterized types
 ...
 */
    ...
    public BinaryTypeBinding createBinaryTypeFrom(IBinaryType binaryType, PackageBinding packageBinding, boolean needFieldsAndMethods, AccessRestriction accessRestriction) {
        ...
        if (cachedType != null && !cachedType.isUnresolvedType()) {
            if (// sanity check... at this point the cache should ONLY contain unresolved types
            cachedType.isBinaryBinding())
...
*/
    public MissingTypeBinding createMissingType(PackageBinding packageBinding, char[][] compoundName) {
        ...
        if (packageBinding == null) {
            packageBinding = computePackageFrom(compoundName, /* missing */
            true);
        ...
        if (missingType.id != TypeIds.T_JavaLangObject) {
            ...
            if (objectType == null) {
                ...
                objectType = createMissingType(null, TypeConstants.JAVA_LANG_OBJECT);
            }
...
*/
    ...
    public ParameterizedGenericMethodBinding createParameterizedGenericMethod(MethodBinding genericMethod, RawTypeBinding rawType) {
        ...
        int index = 0;
        if (cachedInfo != null) {
            nextCachedMethod: // iterate existing parameterized for reusing one with same type arguments if any
    ...
    public ParameterizedGenericMethodBinding createParameterizedGenericMethod(MethodBinding genericMethod, TypeBinding[] typeArguments, boolean inferredWithUncheckedConversion, boolean hasReturnProblem) {
        ...
        if (cachedInfo != null) {
            ...
            for (int max = cachedInfo.length; index < max; index++) {
                ...
                if (// JSL 18.5.2: "If unchecked conversion was necessary..."
                inferredWithUncheckedConversion) {
                    ...
                    if (cachedMethod.returnType.isParameterizedType() || cachedMethod.returnType.isTypeVariable())
                        continue;
                    for (TypeBinding exc : cachedMethod.thrownExceptions) if (exc.isParameterizedType() || exc.isTypeVariable())
                        continue nextCachedMethod;
                }
        ...
        // add new binding
        ParameterizedGenericMethodBinding parameterizedGenericMethod = new ParameterizedGenericMethodBinding(genericMethod, typeArguments, this, inferredWithUncheckedConversion, hasReturnProblem);
        cachedInfo[index] = parameterizedGenericMethod;
        return parameterizedGenericMethod;
    }
    ...
    public PolymorphicMethodBinding createPolymorphicMethod(MethodBinding originalPolymorphicMethod, TypeBinding[] parameters) {
        ...
        int index = 0;
        if (cachedInfo != null) {
            nextCachedMethod: // iterate existing polymorphic method for reusing one with same type arguments if any
            for (int max = cachedInfo.length; index < max; index++) {
 ...
 */
    public ReferenceBinding getCachedType(char[][] compoundName) {
        ...
        if (packageBinding == null || packageBinding == TheNotFoundPackage)
            return null;
        for (int i = 1, packageLength = compoundName.length - 1; i < packageLength; i++) if ((packageBinding = packageBinding.getPackage0(compoundName[i])) == null || packageBinding == TheNotFoundPackage)
            return null;
    ...
    int getNullAnnotationBit(char[][] qualifiedTypeName) {
        if (this.allNullAnnotations == null) {
            ...
            for (String name : this.globalOptions.nullableAnnotationSecondaryNames) this.allNullAnnotations.put(name, TypeIds.BitNullableAnnotation);
            for (String name : this.globalOptions.nonNullAnnotationSecondaryNames) this.allNullAnnotations.put(name, TypeIds.BitNonNullAnnotation);
            for (String name : this.globalOptions.nonNullByDefaultAnnotationSecondaryNames) this.allNullAnnotations.put(name, TypeIds.BitNonNullByDefaultAnnotation);
        }
...
*/
    ...
    private TypeBinding[] getTypeArgumentsFromSignature(SignatureWrapper wrapper, TypeVariableBinding[] staticVariables, ReferenceBinding enclosingType, ReferenceBinding genericType, char[][][] missingTypeNames, ITypeAnnotationWalker walker) {
        ...
        int rank = 0;
        do {
            args.add(getTypeFromVariantTypeSignature(wrapper, staticVariables, enclosingType, genericType, rank, missingTypeNames, walker.toTypeArgument(rank++)));
...
*/
    private ReferenceBinding getTypeFromCompoundName(char[][] compoundName, boolean isParameterized, boolean wasMissingType) {
        ...
        if (binding == null) {
            PackageBinding packageBinding = computePackageFrom(compoundName, /* valid pkg */
            false);
            binding = new UnresolvedReferenceBinding(compoundName, packageBinding);
            if (wasMissingType) {
                // record it was bound to a missing type
...
*/
    TypeBinding getTypeFromSignature(char[] signature, int start, int end, boolean isParameterized, TypeBinding enclosingType, char[][][] missingTypeNames, ITypeAnnotationWalker walker) {
        ...
        if (start == end) {
            ...
            // skip leading 'L' or 'T'
            binding = getTypeFromConstantPoolName(signature, start + 1, end, isParameterized, missingTypeNames);
        }
        if (isParameterized) {
            if (dimension != 0)
    ...
    boolean qualifiedNameMatchesSignature(char[][] name, char[] signature) {
        ...
        for (int i = 0; i < name.length; i++) {
            char[] n = name[i];
            for (int j = 0; j < n.length; j++) if (n[j] != signature[s++])
                return false;
    ...
    public TypeBinding getTypeFromTypeSignature(SignatureWrapper wrapper, TypeVariableBinding[] staticVariables, ReferenceBinding enclosingType, char[][][] missingTypeNames, ITypeAnnotationWalker walker) {
        ...
        if (wrapper.signature[wrapper.start] == 'T') {
            ...
            for (int i = staticVariables.length; --i >= 0; ) if (CharOperation.equals(staticVariables[i].sourceName, wrapper.signature, varStart, varEnd))
            ...
            do {
                ...
                if (// compiler normal case, no eager resolution of binary variables
                enclosingType instanceof BinaryTypeBinding) {
                ...
                for (int i = enclosingTypeVariables.length; --i >= 0; ) if (CharOperation.equals(enclosingTypeVariables[i].sourceName, wrapper.signature, varStart, varEnd))
        ...
        if (actualType instanceof UnresolvedReferenceBinding)
            if (actualType.depth() > 0)
                // must resolve member types before asking for enclosingType
                actualType = (ReferenceBinding) BinaryTypeBinding.resolveType(actualType, this, /* no raw conversion */
                false);
        ...
        while (wrapper.signature[wrapper.start] == '.') {
            ...
            if (memberType == null)
                // aborts
                this.problemReporter.corruptedSignature(// aborts
                currentType, // aborts
                wrapper.signature, // aborts
                memberStart);
            ...
            if (// has type arguments, or ... 
            typeArguments != null || // ... can see type arguments of enclosing
            (!memberType.isStatic() && currentType.isParameterizedType())) {
                if (memberType.isStatic())
                    // ignore bogus parameterization of enclosing
    ...
    TypeBinding getTypeFromVariantTypeSignature(SignatureWrapper wrapper, TypeVariableBinding[] staticVariables, ReferenceBinding enclosingType, ReferenceBinding genericType, int rank, char[][][] missingTypeNames, ITypeAnnotationWalker walker) {
        ...
        switch(wrapper.signature[wrapper.start]) {
            case '-':
                ...
                AnnotationBinding[] annotations = BinaryTypeBinding.createAnnotations(walker.getAnnotationsAtCursor(-1), this, missingTypeNames);
                return this.typeSystem.getWildcard(genericType, rank, bound, /*no extra bound*/
                null, Wildcard.SUPER, annotations);
            case '+':
                // ? extends aType
                wrapper.start++;
                ...
                return this.typeSystem.getWildcard(genericType, rank, bound, /*no extra bound*/
                null, Wildcard.EXTENDS, annotations);
            case '*':
                // ?
                wrapper.start++;
                annotations = BinaryTypeBinding.createAnnotations(walker.getAnnotationsAtCursor(-1), this, missingTypeNames);
                return this.typeSystem.getWildcard(genericType, rank, /*no extra bound*/
                null, null, Wildcard.UNBOUND, annotations);
            default:
...
*/
    ...
    public void releaseClassFiles(org.eclipse.jdt.internal.compiler.ClassFile[] classFiles) {
        for (int i = 0, fileCount = classFiles.length; i < fileCount; i++) this.classFilePool.release(classFiles[i]);
    }
    ...
    public AnnotationBinding[] filterNullTypeAnnotations(AnnotationBinding[] typeAnnotations) {
        ...
        for (int i = 0; i < typeAnnotations.length; i++) {
            ...
            if (typeAnnotation == null) {
                // sentinel in annotation sequence for array dimensions
                count++;
            } else {
