...
public class UnresolvedReferenceNameFinder extends ASTVisitor {
    ...
    private void acceptName(char[] name) {
        ...
        if (name == null)
            return;
        if (!CharOperation.prefixEquals(this.completionEngine.completionToken, name, /* ignore case */
        false) && !(this.completionEngine.options.camelCaseMatch && CharOperation.camelCaseMatch(this.completionEngine.completionToken, name)))
            return;
    ...
    public void find(char[] startWith, Initializer initializer, ClassScope scope, int from, char[][] discouragedNames, UnresolvedReferenceNameRequestor nameRequestor) {
        MethodDeclaration fakeMethod = this.findAfter(startWith, scope, from, initializer.bodyEnd, MAX_LINE_COUNT, false, discouragedNames, nameRequestor);
        if (fakeMethod != null)
    ...
    public void find(char[] startWith, AbstractMethodDeclaration methodDeclaration, int from, char[][] discouragedNames, UnresolvedReferenceNameRequestor nameRequestor) {
        MethodDeclaration fakeMethod = this.findAfter(startWith, methodDeclaration.scope, from, methodDeclaration.bodyEnd, MAX_LINE_COUNT, false, discouragedNames, nameRequestor);
        if (fakeMethod != null)
    ...
    public void findAfter(char[] startWith, Scope scope, ClassScope classScope, int from, int to, char[][] discouragedNames, UnresolvedReferenceNameRequestor nameRequestor) {
        MethodDeclaration fakeMethod = this.findAfter(startWith, scope, from, to, MAX_LINE_COUNT / 2, true, discouragedNames, nameRequestor);
        if (fakeMethod != null)
    ...
    private MethodDeclaration findAfter(char[] startWith, Scope s, int from, int to, int maxLineCount, boolean outsideEnclosingBlock, char[][] discouragedNames, UnresolvedReferenceNameRequestor nameRequestor) {
        ...
        int maxEnd = this.completionScanner.getLineEnd(Util.getLineNumber(from, this.completionScanner.lineEnds, 0, this.completionScanner.linePtr) + maxLineCount);
        int end;
        if (maxEnd < 0) {
            end = to;
        } else {
        ...
        MethodDeclaration fakeMethod = this.parser.parseSomeStatements(from, end, outsideEnclosingBlock ? FAKE_BLOCKS_COUNT : 0, s.compilationUnitScope().referenceContext);
    ...
    public void findBefore(char[] startWith, Scope scope, ClassScope classScope, int from, int recordTo, int parseTo, char[][] discouragedNames, UnresolvedReferenceNameRequestor nameRequestor) {
        MethodDeclaration fakeMethod = this.findBefore(startWith, scope, from, recordTo, parseTo, MAX_LINE_COUNT / 2, discouragedNames, nameRequestor);
        if (fakeMethod != null)
            fakeMethod.traverse(this, classScope);
    }
    ...
    private int indexOfFisrtNameAfter(int position) {
        ...
        next: while (true) {
            ...
            if (midPosition < 0) {
                ...
                if (// no next index or next index is after 'right'
                nextMid < 0 || right < nextMid) {
                    right = mid - 1;
                    continue next;
                }
                mid = nextMid;
                midPosition = this.potentialVariableNameStarts[nextMid];
                if (// mid and right are at the same index, we must move 'left'
                mid == right) {
                    int leftPosition = this.potentialVariableNameStarts[left];
                    if (// 'left' is empty or 'left' is before the position
                    leftPosition < 0 || leftPosition < position) {
