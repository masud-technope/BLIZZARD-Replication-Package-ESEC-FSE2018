...
public class ServerSOContainer extends SOContainer implements ISharedObjectContainerGroupManager {
    ...
    public void connect(ID groupID, IConnectContext joinContext) throws ContainerConnectException {
        //$NON-NLS-1$
        final ContainerConnectException e = new ContainerConnectException("Server container cannot connect");
        throw e;
    }
                ...
                if (!excluding.equals(oldID) && !from.equals(oldID)) {
                    ...
                    if (conn != null) {
                        try {
                            ...
                            traceStack("Exception in forwardExcluding from " + from + " with oldID " + oldID, e);
                        }
                    }
                }
            }
    ...
    protected ContainerMessage acceptNewClient(Socket socket, String target, Serializable data, ISynchAsynchConnection conn) {
        ...
        try {
            ...
            if (connectMessage == null)
                ...
                "Connect message cannot be null");
            remoteID = connectMessage.getFromContainerID();
            if (remoteID == null)
                throw new //$NON-NLS-1$
                NullPointerException(//$NON-NLS-1$
                "fromID cannot be null");
            final ContainerMessage.JoinGroupMessage jgm = (ContainerMessage.JoinGroupMessage) connectMessage.getData();
            if (jgm == null)
                throw new //$NON-NLS-1$
                NullPointerException(//$NON-NLS-1$
                "Join group message cannot be null");
            ID memberIDs[] = null;
            synchronized (getGroupMembershipLock()) {
                if (isClosing) {
                    final Exception e = new IllegalStateException("Server container is closing");
                    throw e;
                }
                ...
                if (addNewRemoteMember(remoteID, conn)) {
                    ...
                    final ConnectException e = new ConnectException("server refused connection");
                    throw e;
                }
            }
            // notify listeners
            ...
            traceStack("Exception in acceptNewClient(" + socket + "," + target + "," + data + "," + conn, e);
        ...
        for (int i = 0; i < members.length; i++) {
            ...
            if (conn != null) {
                try {
                    ...
                    traceStack("Exception in queueToAll for ContainerMessage " + message, e);
                }
            }
        }
    }
