...
public final class Client implements ISynchAsynchConnection {
    ...
    public  Client(Socket aSocket, ObjectInputStream iStream, ObjectOutputStream oStream, ISynchAsynchEventHandler handler, int maxmsgs) throws IOException {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        keepAlive = Integer.valueOf(System.getProperty("org.eclipse.ecf.provider.generic.keepalive", "30000")).intValue();
        if (keepAlive > 0)
    ...
    protected Socket createConnectSocket(URI remote, int timeout) throws ECFException {
        ...
        try {
            ...
            throw new ECFException("Could not create socket to connect to id=" + remote, e);
        }
    }
