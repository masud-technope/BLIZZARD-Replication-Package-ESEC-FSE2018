...
public final class SLPDaemonImpl implements SLPDaemon {
	 ...
	 */
    private void announceService(final String dAAddress, final ServiceRegistration reg) throws ServiceLocationException {
        try {
                ...
                while (running) {
                    synchronized (serviceDisposalQueue) {
                        if (serviceDisposalQueue.isEmpty()) {
                            ...
                            Long nextActivity;
                            while (!serviceDisposalQueue.isEmpty() && (nextActivity = ((Long) serviceDisposalQueue.firstKey())).longValue() <= System.currentTimeMillis()) {
                                ServiceURL service = (ServiceURL) serviceDisposalQueue.get(nextActivity);
                            ...
                            if (!serviceDisposalQueue.isEmpty()) {
								 ...
								 */
                                nextActivity = ((Long) serviceDisposalQueue.firstKey());
                                long waitTime = nextActivity.longValue() - System.currentTimeMillis();
                                if (waitTime > 0) {
                                    SLPCore.platform.logDebug("sleeping for " + waitTime / 1000 + " seconds.");
