...
public class XMPPContainerPresenceHelper implements ISharedObject {
    ...
    private void addUnique(Collection existingItems, IRosterItem newItem) {
        ...
        for (Iterator i = toAdd.iterator(); i.hasNext(); ) existingItems.add(i.next());
    }

    private void addUniqueToRoster(IRosterItem[] newItems) {
        Collection existingItems = roster.getItems();
        synchronized (existingItems) {
            for (int i = 0; i < newItems.length; i++) addUnique(existingItems, newItems[i]);
        }
    ...
    protected IRosterEntry createRosterEntry(RosterEntry entry) {
        ...
        return createRosterEntry(xmppid, getEntryName(xmppid, entry.getName()), entry.getGroups().iterator());
    }

    private String getEntryName(XMPPID xmppid, String entryName) {
        return (entryName == null) ? xmppid.getUsername() : XMPPID.unfixEscapeInNode(entryName);
    }

    protected IRosterEntry createRosterEntry(XMPPID xmppid, RosterPacket.Item entry) {
        return createRosterEntry(xmppid, getEntryName(xmppid, entry.getName()), entry.getGroupNames().iterator());
    }
    ...
    protected void handleMessageEvent(MessageEvent evt) {
        ...
        if (msg != null) {
            if (//$NON-NLS-1$
            msg.getExtension(//$NON-NLS-1$
            "composing", //$NON-NLS-1$
            "http://jabber.org/protocol/chatstates") != //$NON-NLS-1$
            null) {
    ...
    private AdditionalClientRosterEntry removeEntryFromRoster(XMPPID oldID, org.eclipse.ecf.presence.roster.RosterEntry entry, IPresence newPresence, IUser user) {
    ...
    protected IPresence createIPresence(final Presence xmppPresence) {
        final IFuture asyncResult = new ThreadsExecutor().execute(new IProgressRunnable() {
