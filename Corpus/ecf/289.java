...
public class CapsExtensionProvider implements PacketExtensionProvider {
    ...
    public PacketExtension parseExtension(XmlPullParser parser) throws XmlPullParserException, IOException, XMPPException {
        String hash = null;
        String version = null;
        String node = null;
        if (parser.getEventType() == XmlPullParser.START_TAG && parser.getName().equalsIgnoreCase(EntityCapsManager.ELEMENT)) {
        ...
        parser.next();
        if (!(parser.getEventType() == XmlPullParser.END_TAG && parser.getName().equalsIgnoreCase(EntityCapsManager.ELEMENT))) {
