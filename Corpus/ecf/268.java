...
public class InBandBytestreamManager implements BytestreamManager {
     ...
     */
    public enum StanzaType implements  {
    ...
    private static final Map<Connection, InBandBytestreamManager> managers = new HashMap<Connection, InBandBytestreamManager>();
    ...
    public void setDefaultBlockSize(int defaultBlockSize) {
        if (defaultBlockSize <= 0 || defaultBlockSize > MAXIMUM_BLOCK_SIZE) {
            throw new IllegalArgumentException("Default block size must be between 1 and " + MAXIMUM_BLOCK_SIZE);
        }
    ...
    public void setMaximumBlockSize(int maximumBlockSize) {
        if (maximumBlockSize <= 0 || maximumBlockSize > MAXIMUM_BLOCK_SIZE) {
            throw new IllegalArgumentException("Maximum block size must be between 1 and " + MAXIMUM_BLOCK_SIZE);
        }
