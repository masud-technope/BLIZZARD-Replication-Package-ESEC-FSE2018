/*
 * $Header: /home/data/cvs/rt/org.eclipse.ecf/tests/bundles/org.eclipse.ecf.tests.apache.httpclient.server/src/org/apache/commons/httpclient/TestStreams.java,v 1.1 2009/02/13 18:07:48 slewis Exp $
 * $Revision: 1.1 $
...
public class TestStreams extends TestCase {
    ...
    public void testChunkedInputStream() throws IOException {
        ...
        InputStream in = new ChunkedInputStream(new ByteArrayInputStream(EncodingUtil.getBytes(correctInput, CONTENT_CHARSET)), method);
        ...
        in = new ChunkedInputStream(new ByteArrayInputStream(EncodingUtil.getBytes(correctInput, CONTENT_CHARSET)), method);
        buffer = new byte[7];
    ...
    public void testCorruptChunkedInputStream1() throws IOException {
        ...
        HttpMethod method = new FakeHttpMethod();
        InputStream in = new ChunkedInputStream(new ByteArrayInputStream(EncodingUtil.getBytes(corrupInput, CONTENT_CHARSET)), method);
        byte[] buffer = new byte[300];
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        int len;
        try {
    ...
    public void testEmptyChunkedInputStream() throws IOException {
        String input = "0\r\n";
        HttpMethod method = new FakeHttpMethod();
        InputStream in = new ChunkedInputStream(new ByteArrayInputStream(EncodingUtil.getBytes(input, CONTENT_CHARSET)), method);
        byte[] buffer = new byte[300];
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        int len;
        while ((len = in.read(buffer)) > 0) {
    ...
    public void testContentLengthInputStream() throws IOException {
        ...
        InputStream in = new ContentLengthInputStream(new ByteArrayInputStream(EncodingUtil.getBytes(correct, CONTENT_CHARSET)), 10L);
