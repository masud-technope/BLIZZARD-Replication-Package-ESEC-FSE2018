...
public class MultiUserChat {
    ...
    public String getReservedNickname() {
        try {
            DiscoverInfo result = ServiceDiscoveryManager.getInstanceFor(connection).discoverInfo(room, "x-roomuser-item");
            // Look for an Identity that holds the reserved nickname and return its name
            for (Iterator<DiscoverInfo.Identity> identities = result.getIdentities(); identities.hasNext(); ) {
    ...
    public Iterator<String> getOccupants() {
        return Collections.unmodifiableList(new ArrayList<String>(occupantsMap.keySet())).iterator();
    }
    ...
    */
    ...
    private void checkRoleModifications(String oldRole, String newRole, boolean isUserModification, String from) {
        ...
        } else // The participant's voice was revoked from the room
        if ("participant".equals(oldRole) && ("visitor".equals(newRole) || "none".equals(newRole))) {
            if (isUserModification) {
        ...
        } else // Moderator privileges were revoked from a participant
        if ("moderator".equals(oldRole) && !"moderator".equals(newRole)) {
    ...
    private void checkAffiliationModifications(String oldAffiliation, String newAffiliation, boolean isUserModification, String from) {
        ...
        } else // The user's administrative privileges to the room were revoked
        ...
        } else // The user's membership to the room was revoked
        ...
        } else // The user was granted administrative privileges to the room
        if (!"admin".equals(oldAffiliation) && "admin".equals(newAffiliation)) {
            if (isUserModification) {
        ...
        } else // The user was granted membership to the room
        if (!"member".equals(oldAffiliation) && "member".equals(newAffiliation)) {
            if (isUserModification) {
    ...
    private void checkPresenceCode(String code, boolean isUserModification, MUCUser mucUser, String from) {
        ...
        } else // A user was banned from the room
        if ("301".equals(code)) {
        ...
        } else // A user's membership was revoked from the room
        if ("321".equals(code)) {
        ...
        } else // A occupant has changed his nickname in the room
        if ("303".equals(code)) {
            List<String> params = new ArrayList<String>();
            params.add(from);
    ...
    private static class InvitationsMonitor implements ConnectionListener {
        ...
        private void init() {
            ...
            invitationFilter = new PacketExtensionFilter("x", "http://jabber.org/protocol/muc#user");
            invitationPacketListener = new PacketListener() {
                ...
                public void processPacket(Packet packet) {
                    ...
                    if (mucUser.getInvite() != null && ((Message) packet).getType() != Message.Type.error) {
                        ...
                        fireInvitationListeners(packet.getFrom(), mucUser.getInvite().getFrom(), mucUser.getInvite().getReason(), mucUser.getPassword(), (Message) packet);
                    }
                }
            };
