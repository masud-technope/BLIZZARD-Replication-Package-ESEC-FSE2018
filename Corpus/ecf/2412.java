...
public class ServerManager {
    ...
    public static final String EXTENSION_POINT = //$NON-NLS-1$
    Activator.PLUGIN_ID + "." + EXTENSION_POINT_NAME;
    ...
    public  ServerManager() {
        SafeRunner.run(new ExtensionRegistryRunnable(Activator.getDefault().getContext()) {
            ...
            protected void runWithoutRegistry() throws Exception {
                createServersFromConfigurationFile(//$NON-NLS-1$
                Activator.getDefault().getBundle().getEntry("server.xml").openStream());
            }
    ...
    private synchronized void createServersFromConnectorList(List connectors) throws IDCreateException, IOException {
        ...
        for (final Iterator i = connectors.iterator(); i.hasNext(); ) {
            ...
            for (final Iterator g = groups.iterator(); g.hasNext(); ) {
                ...
                final String msg = "Starting server with id=" + //$NON-NLS-1$
                cont.getID().getName();
