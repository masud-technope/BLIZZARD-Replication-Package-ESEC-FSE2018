...
public class RemoteServiceTracker implements IRemoteServiceTrackerCustomizer {
    ...
    public  RemoteServiceTracker(IRemoteServiceContainerAdapter containerAdapter, ID[] containerIDs, IRemoteServiceReference reference, IRemoteServiceTrackerCustomizer customizer) {
        ...
        this.listenerFilter = //$NON-NLS-1$ //$NON-NLS-2$
        "(&(" + Constants.OBJECTCLASS + "=" + ((String[]) reference.getProperty(Constants.OBJECTCLASS))[0] + ")(" + Constants.SERVICE_ID + "=" + reference.getProperty(Constants.SERVICE_ID).toString() + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "))";
        this.noUserFilter = true;
        try {
            this.filter = new RemoteFilterImpl(listenerFilter);
        } catch (InvalidSyntaxException // we could only get this exception
        e) {
    ...
    public synchronized void open() {
        ...
        if (DEBUG) {
            //$NON-NLS-1$
            System.out.println("RemoteServiceTracker.open: " + filter);
        }
        tracked = new Tracked();
        trackingCount = 0;
        synchronized (tracked) {
            try {
                ...
                if (// tracking a single reference 
                trackReference != null) {
                ...
                // set tracked with
                tracked.setInitialServices(references);
            // the initial
            // references
    ...
    public synchronized void close() {
        ...
        if (DEBUG) {
            //$NON-NLS-1$
            System.out.println("RemoteServiceTracker.close: " + filter);
        }
        tracked.close();
        ...
        if (DEBUG) {
            if ((cachedReference == null) && (cachedService == null)) {
                ...
                "RemoteServiceTracker.close[cached cleared]: " + filter);
            }
        }
    }
    ...
    public IRemoteServiceReference getRemoteServiceReference() {
        ...
        if (reference != null) {
            if (DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "RemoteServiceTracker.getRemoteServiceReference[cached]: " + filter);
            }
            return reference;
        }
        if (DEBUG) {
            //$NON-NLS-1$
            System.out.println("RemoteServiceTracker.getRemoteServiceReference: " + filter);
        }
        IRemoteServiceReference[] references = getRemoteServiceReferences();
        ...
        if (length > 1) /* if more than one service, select highest ranking */
        {
            ...
            if (/* if still more than one service, select lowest id */
            count > 1) {
    ...
    public IRemoteService getRemoteService() {
        ...
        if (service != null) {
            if (DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "RemoteServiceTracker.getRemoteService[cached]: " + filter);
            }
            return service;
        }
        if (DEBUG) {
            //$NON-NLS-1$
            System.out.println("RemoteServiceTracker.getRemoteService: " + filter);
        }
    ...
    void modified() {
        ...
        if (DEBUG) {
            //$NON-NLS-1$
            System.out.println("RemoteServiceTracker.modified: " + filter);
        }
    }
    ...
    class Tracked extends Hashtable implements IRemoteServiceListener {
        ...
        protected void setInitialServices(IRemoteServiceReference[] references) {
            ...
            for (int i = 0; i < size; i++) {
                if (DEBUG) {
                    System.out.println(//$NON-NLS-1$
                    "RemoteServiceTracker.Tracked.setInitialServices: " + //$NON-NLS-1$
                    references[i]);
                }
            ...
            while (true) {
                ...
                synchronized (this) {
                    ...
                    if (this.get(reference) != null) {
                        ...
                        if (DEBUG) {
                            System.out.println(//$NON-NLS-1$
                            "RemoteServiceTracker.Tracked.trackInitialServices[already tracked]: " + //$NON-NLS-1$
                            reference);
                        }
                        /* skip this service */
                        continue;
                    ...
                    if (adding.contains(reference)) {
                        ...
                        if (DEBUG) {
                            System.out.println(//$NON-NLS-1$
                            "RemoteServiceTracker.Tracked.trackInitialServices[already adding]: " + //$NON-NLS-1$
                            reference);
                        }
                        /* skip this service */
                        continue;
                ...
                if (DEBUG) {
                    System.out.println(//$NON-NLS-1$
                    "RemoteServiceTracker.Tracked.trackInitialServices: " + //$NON-NLS-1$
                    reference);
                }
        ...
        public void handleServiceEvent(IRemoteServiceEvent event) {
            ...
            switch(type) {
                ...
                case ServiceEvent.MODIFIED:
                    if (// no user supplied filter to be checked
                    noUserFilter) {
                        track(reference);
                    /*
        ...
        private void track(IRemoteServiceReference reference) {
            ...
            if (object != null) /* we are already tracking the service */
            {
                if (DEBUG) {
                    System.out.println(//$NON-NLS-1$
                    "RemoteServiceTracker.Tracked.track[modified]: " + //$NON-NLS-1$
                    reference);
                }
                synchronized (this) {
                    /* increment modification count */
            ...
            synchronized (this) {
                if (adding.contains(reference)) {
                    ...
                    if (DEBUG) {
                        System.out.println(//$NON-NLS-1$
                        "RemoteServiceTracker.Tracked.track[already adding]: " + //$NON-NLS-1$
                        reference);
                    }
                    return;
                }
                adding.add(reference);
            /* mark this service is being added */
            }
            trackAdding(reference);
        /*
        ...
        private void trackAdding(IRemoteServiceReference reference) {
            if (DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "RemoteServiceTracker.Tracked.trackAdding: " + reference);
            }
            ...
            if (becameUntracked) {
                if (DEBUG) {
                    System.out.println(//$NON-NLS-1$
                    "RemoteServiceTracker.Tracked.trackAdding[removed]: " + //$NON-NLS-1$
                    reference);
                }
                /* Call customizer outside of synchronized region */
        ...
        protected void untrack(IRemoteServiceReference reference) {
            ...
            synchronized (this) {
                if (initial.remove(reference)) {
                    ...
                    if (DEBUG) {
                        System.out.println(//$NON-NLS-1$
                        "RemoteServiceTracker.Tracked.untrack[removed from initial]: " + //$NON-NLS-1$
                        reference);
                    }
                    return;
                /*
                ...
                if (adding.remove(reference)) {
                    ...
                    if (DEBUG) {
                        System.out.println(//$NON-NLS-1$
                        "RemoteServiceTracker.Tracked.untrack[being added]: " + //$NON-NLS-1$
                        reference);
                    }
                    return;
                /*
            ...
            if (DEBUG) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "RemoteServiceTracker.Tracked.untrack[removed]: " + reference);
            }
            customizer.removedService(reference, object);
        }
    }
