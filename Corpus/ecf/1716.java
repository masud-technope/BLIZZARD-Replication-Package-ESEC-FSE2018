...
public class Activator implements BundleActivator {
    ...
    public static final String CHATROOM_COMMAND_HANDLER_EPOINT = PLUGIN_ID + //$NON-NLS-1$
    "." + CHATROOM_COMMAND_HANDLER_EPOINT_NAME;

    //$NON-NLS-1$
    public static final String IM_COMMAND_HANDLER_EPOINT_NAME = "imMessageHandler";

    public static final String IM_COMMAND_HANDLER_EPOINT = //$NON-NLS-1$
    PLUGIN_ID + "." + IM_COMMAND_HANDLER_EPOINT_NAME;

    //$NON-NLS-1$
    public static final String CHATROOM_BOT_EPOINT_NAME = "chatRoomRobot";

    public static final String CHATROOM_BOT_EPOINT = PLUGIN_ID + "." + //$NON-NLS-1$
    CHATROOM_BOT_EPOINT_NAME;

    //$NON-NLS-1$
    public static final String IM_BOT_EPOINT_NAME = "imRobot";

    public static final String IM_BOT_EPOINT = PLUGIN_ID + "." + //$NON-NLS-1$
    IM_BOT_EPOINT_NAME;
        ...
        if (reg != null) {
            ...
            for (int i = 0; i < elements.length; i++) {
                ...
                for (int j = 0; j < chatRoomElements.length; j++) {
                    chatRoomNames[j] = chatRoomElements[j].getAttribute(CHATROOMNAME_ATT);
                    chatRoomPasswords[j] = chatRoomElements[j].getAttribute(CHATROOMPASSWORD_ATT);
                }
                List c = (List) chatbotcommands.get(id);
                if (c == null)
                    c = new ArrayList();
                IChatRoomBotEntry bot = new ChatRoomBotEntry(id, name, containerFactoryName, connectID, password, chatRoomNames, chatRoomPasswords, c);
    ...
    private void loadIMBotExtensions() throws CoreException {
        ...
        if (reg != null) {
            ...
            for (int i = 0; i < elements.length; i++) {
                ...
                if (c == null)
                    c = new ArrayList();
                IIMBotEntry bot = new IMBotEntry(id, name, containerFactoryName, connectID, password, c);
                imbots.put(id, bot);
            }
        }
    }
