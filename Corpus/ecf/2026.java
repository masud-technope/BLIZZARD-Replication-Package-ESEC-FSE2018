        ...
        if (sharedObjectClassNames != null) {
            for (int j = 0; j < clientCount; j++) {
                ...
                for (int i = 0; i < sharedObjectClassNames.length; i++) {
                    ...
                    sharedObjects[i] = IDFactory.getDefault().createStringID(//$NON-NLS-1$
                    sharedObjectClassNames[i] + "_" + //$NON-NLS-1$
                    i);
                    scg.getSharedObjectManager().addSharedObject(sharedObjects[i], so, new HashMap());
                    System.out.println("Created sharedObject for client " + //$NON-NLS-1$
                    scg.getID().getName());
                }
            }
        }
