...
class TeamChat extends Composite {
    ...
    static final int DEFAULT_TREE_WIDGET_PERCENT = 15;
     ...
     TeamChat(LineChatClientView view, Composite parent, int options, String initText) {
        ...
        boolean useChatWindow = ClientPlugin.getDefault().getPluginPreferences().getBoolean(ClientPlugin.PREF_USE_CHAT_WINDOW);
        int[] w = null;
        if (!useChatWindow) {
        ...
        tableView = new TableViewer(useChatWindow ? (Composite) this : (Composite) sash, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
    ...
    void appendText(ChatLine text) {
        if (chatWindow != null && chatWindow.getShell() != null && !chatWindow.getShell().isDisposed() && !chatWindow.hasFocus()) {
    ...
    private class ViewLabelProvider extends LabelProvider {
        ...
        public Image getImage(Object obj) {
            return obj instanceof IUser ? SharedImages.getImage(SharedImages.IMG_USER_AVAILABLE) : null;
        }
    }
}
