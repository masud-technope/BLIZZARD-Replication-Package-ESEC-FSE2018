...
public class AgentSession {
    ...
    public void setOnline(boolean online) throws XMPPException {
        ...
        if (online) {
            ...
            presence.addExtension(new DefaultPacketExtension(AgentStatus.ELEMENT_NAME, AgentStatus.NAMESPACE));
    ...
    public void setStatus(Presence.Mode presenceMode, int maxChats, String status) throws XMPPException {
        ...
        DefaultPacketExtension agentStatus = new DefaultPacketExtension(AgentStatus.ELEMENT_NAME, AgentStatus.NAMESPACE);
    ...
    public ReportedData searchTranscripts(Form completedForm) throws XMPPException {
        return transcriptSearchManager.submitSearch(StringUtils.parseServer(workgroupJID), completedForm);
    }
    ...
    private void fireOfferRequestEvent(OfferRequestProvider.OfferRequestPacket requestPacket) {
        Offer offer = new Offer(this.connection, this, requestPacket.getUserID(), requestPacket.getUserJID(), this.getWorkgroupJID(), new Date((new Date()).getTime() + (requestPacket.getTimeout() * 1000)), requestPacket.getSessionID(), requestPacket.getMetaData(), requestPacket.getContent());
        synchronized (offerListeners) {
    ...
    private void fireInvitationEvent(String groupChatJID, String sessionID, String body, String from, Map<String, List<String>> metaData) {
        WorkgroupInvitation invitation = new WorkgroupInvitation(connection.getUser(), groupChatJID, workgroupJID, sessionID, body, from, metaData);
        synchronized (invitationListeners) {
