...
public class SOManager implements ISharedObjectManager {
    ...
    protected void debug(String msg) {
        //$NON-NLS-1$
        Trace.trace(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.SHAREDOBJECTMANAGER, msg + ":" + container.getID());
    }

    protected void traceStack(String msg, Throwable e) {
        //$NON-NLS-1$
        Trace.catching(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.EXCEPTIONS_CATCHING, SOManager.class, container.getID() + ":" + msg, e);
    }
    ...
    protected ISharedObject loadSharedObject(SharedObjectDescription sd) throws Exception {
        //$NON-NLS-1$
        Assert.isNotNull(sd, "shared object description cannot be null");
    ...
    public ID createSharedObject(SharedObjectDescription sd) throws SharedObjectCreateException {
        ...
        try {
            ...
            throw new SharedObjectCreateException("container=" + containerID + " had exception creating shared object=" + sd.getID(), e);
    ...
    public ID addSharedObject(ID sharedObjectID, ISharedObject sharedObject, Map properties) throws SharedObjectAddException {
        debug(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "addSharedObject(" + sharedObjectID + "," + sharedObject + "," + properties + //$NON-NLS-1$
        ")");
        ID containerID = container.getID();
        ...
        try {
            ...
            throw new SharedObjectAddException("container=" + containerID + " had exception adding shared object=" + sharedObjectID, e);
        }
        // notify listeners
    ...
    public ISharedObjectConnector connectSharedObjects(ID sharedObjectFrom, ID[] sharedObjectsTo) throws SharedObjectConnectException {
        ...
        "connectSharedObjects(" + sharedObjectFrom + "," + sharedObjectsTo + //$NON-NLS-1$
        ")");
        ...
        synchronized (container.getGroupMembershipLock()) {
            ...
            if (wrap == null)
                //$NON-NLS-1$ //$NON-NLS-2$
                throw new SharedObjectConnectException("sender=" + sharedObjectFrom.getName() + " not found");
            ...
            for (int i = 0; i < sharedObjectsTo.length; i++) {
                ...
                if (w == null)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    throw new SharedObjectConnectException("receiver=" + sharedObjectsTo[i].getName() + " not found");
    ...
    public void disconnectSharedObjects(ISharedObjectConnector connector) throws SharedObjectDisconnectException {
        ...
        if (!removeConnector(connector)) {
            //$NON-NLS-1$ //$NON-NLS-2$
            throw new SharedObjectDisconnectException("connector=" + connector + " not found");
        }
        connector.dispose();
