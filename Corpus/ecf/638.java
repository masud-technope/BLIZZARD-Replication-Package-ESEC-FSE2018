...
public class AdvertisedService extends ServiceInfo implements INode, IService {
    ...
    public  AdvertisedService(ServiceReference ref) {
        ...
        for (String propertyKey : this.serviceReference.getPropertyKeys()) {
            super.properties.setProperty(propertyKey, this.serviceReference.getProperty(propertyKey));
        }
        IServiceTypeID serviceTypeID = ServiceIDFactory.getDefault().createServiceTypeID(ZooDiscoveryContainer.getSingleton().getConnectNamespace(), services, IServiceTypeID.DEFAULT_PROTO);
        serviceTypeID = new ZooDiscoveryServiceTypeID((ZooDiscoveryNamespace) ZooDiscoveryContainer.getSingleton().getConnectNamespace(), serviceTypeID, this.serviceReference.getProperty(Constants.SERVICE_ID).toString());
        serviceID = new ZooDiscoveryServiceID(ZooDiscoveryContainer.getSingleton().getConnectNamespace(), serviceTypeID, Geo.getLocation());
        setNodeProperties();
    }
    ...
    public int compareTo(Object o) {
        Assert.isTrue(o != null && o instanceof IServiceInfo, //$NON-NLS-1$
    ...
    public byte[] getPropertiesAsBytes() {
        ...
        try {
            ...
            Logger.log(LogService.LOG_ERROR, "Error while serializing node data ", e);
        } finally {
            if (oout != null) {
                try {
                    oout.close();
    ...
    public String getPath() {
        return getNodeId() + INode._URI_ + Geo.getHost() + INode._ZOODISCOVERYID_ + Configurator.INSTANCE.getID().getName();
    }
