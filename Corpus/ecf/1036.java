    ...
    public void start() {
        ...
        for (Iterator i = in.getQuestions().iterator(); i.hasNext(); ) {
            ...
            if (entry instanceof DNSQuestion) {
                ...
                iAmTheOnlyOne &= (q.getType() == DNSConstants.TYPE_SRV || q.getType() == DNSConstants.TYPE_TXT || q.getType() == DNSConstants.TYPE_A || q.getType() == DNSConstants.TYPE_AAAA || this.jmDNSImpl.getLocalHost().getName().equalsIgnoreCase(q.getName()) || this.jmDNSImpl.getServices().containsKey(q.getName().toLowerCase()));
                if (!iAmTheOnlyOne) {
                    break;
                }
            }
                                        ...
                                        for (Iterator serviceIterator = this.jmDNSImpl.getServices().values().iterator(); serviceIterator.hasNext(); ) {
                                            ...
                                            if (info.getState() == DNSState.ANNOUNCED) {
                                                if (q.getName().equalsIgnoreCase(info.getType())) {
                                                    ...
                                                    answers.add(new DNSRecord.Service(info.getQualifiedName(), DNSConstants.TYPE_SRV, DNSConstants.CLASS_IN | DNSConstants.CLASS_UNIQUE, DNSConstants.DNS_TTL, info.getPriority(), info.getWeight(), info.getPort(), this.jmDNSImpl.getLocalHost().getName()));
