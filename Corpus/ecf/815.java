...
public class MultiRosterView extends ViewPart implements IMultiRosterViewPart {
    ...
    private static final String ROSTER_VIEWER_DROP_TARGET_EPOINT = //$NON-NLS-1$
    Activator.PLUGIN_ID + "." + //$NON-NLS-1$
    "rosterViewerDropTarget";
    ...
    private class ViewerToolTip extends ToolTip {
        ...
        private String getRosterEntryChildrenFromPresence(IRosterEntry entry) {
            ...
            buffer.append("<p>");
            //$NON-NLS-1$
            buffer.append(NLS.bind(Messages.RosterWorkbenchAdapterFactory_Resource, ((resourceName == null) ? "" : resourceName)));
            //$NON-NLS-1$
            buffer.append("</p>");
            for (Iterator i = properties.keySet().iterator(); i.hasNext(); ) {
                ...
                Object key = i.next();
                //$NON-NLS-1$
                buffer.append(key).append(": ").append(//$NON-NLS-1$
                properties.get(key));
                //$NON-NLS-1$
                buffer.append(//$NON-NLS-1$
                "</p>");
