...
public class EclipseFileTransferAndLaunch extends EclipseFileTransfer {
    ...
    private void launchFile(String fileName) {
        try {
            ...
            ClientPlugin.log(MessageLoader.getFormattedString("EclipseFileTransferAndLaunch.EXCEPTION_LAUNCHING", localFile), e1);
            Display.getDefault().asyncExec(new Runnable() {

                public void run() {
                    MessageDialog.openInformation(null, MessageLoader.getString("EclipseFileTransferAndLaunch.PROGRAM_LAUNCH_MSGBOX_TITLE"), NLS.bind(MessageLoader.getString("EclipseFileTransferAndLaunch.PROGRAM_LAUNCH_MSGBOX_TEXT"), localFile.getAbsolutePath(), e1.getMessage()));
                }
            });
        }
    }
