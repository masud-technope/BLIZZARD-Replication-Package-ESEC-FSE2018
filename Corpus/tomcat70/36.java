...
public class InternalNioInputBuffer extends AbstractInputBuffer<NioChannel> {
    ...
    enum HeaderParseStatus implements  {
    ...
    enum HeaderParsePosition implements  {
    ...
    private int readSocket(boolean timeout, boolean block) throws IOException {
        ...
        if (block) {
            ...
            try {
                ...
                nRead = pool.read(socket.getBufHandler().getReadBuffer(), socket, selector, socket.getIOChannel().socket().getSoTimeout());
    ...
    protected class SocketInputBuffer implements InputBuffer {
        ...
        public int doRead(ByteChunk chunk, Request req) throws IOException {
            if (pos >= lastValid) {
                if (//read body, must be blocking, as the thread is inside the app
                !fill(true, true))
                    return -1;
            }
