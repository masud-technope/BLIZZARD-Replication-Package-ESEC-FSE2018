...
public class PerMessageDeflate implements Transformation {
    ...
    public List<MessagePart> sendMessagePart(List<MessagePart> uncompressedParts) {
        ...
        for (MessagePart uncompressedPart : uncompressedParts) {
            ...
            if (Util.isControl(opCode)) {
            ...
            } else if (uncompressedPart.getPayload().limit() == 0 && uncompressedPart.isFin() && deflater.getBytesRead() == 0) {
                ...
                while (deflateRequired) {
                    ...
                    if (fin && !full && needsInput) {
                        ...
                        compressedPart = new MessagePart(true, getRsv(uncompressedPart), opCode, compressedPayload, uncompressedIntermediateHandler, uncompressedIntermediateHandler);
                        deflateRequired = false;
                        startNewMessage();
                    } else if (full && !needsInput) {
                        // Write buffer full and input message not fully read.
                        // Output and start new compressed part.
                        compressedPart = new MessagePart(false, getRsv(uncompressedPart), opCode, compressedPayload, uncompressedIntermediateHandler, uncompressedIntermediateHandler);
