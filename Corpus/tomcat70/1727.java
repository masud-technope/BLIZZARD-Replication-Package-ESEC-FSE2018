...
class Parser implements TagConstants {
     ...
     * The constructor
     */
    private  Parser(ParserController pc, JspReader reader, boolean isTagFile, boolean directivesOnly, JarResource jarResource) {
     ...
     */
    public static Node.Nodes parse(ParserController pc, JspReader reader, Node parent, boolean isTagFile, boolean directivesOnly, JarResource jarResource, String pageEnc, String jspConfigPageEnc, boolean isDefaultPageEncoding, boolean isBomPresent) throws JasperException {
        Parser parser = new Parser(pc, reader, isTagFile, directivesOnly, jarResource);
    ...
    private String parseAttributeValue(String watch, boolean ignoreEL) throws JasperException {
        ...
        try {
            ...
            ret = AttributeParser.getUnquoted(reader.getText(start, stop), quote, isElIgnored, pageInfo.isDeferredSyntaxAllowedAsLiteral(), quoteAttributeEL);
        } catch (IllegalArgumentException iae) {
                ...
                if (pageInfo.getTaglib(uri) == null) {
                    ...
                    if (impl == null) {
                        ...
                        for (TagFileInfo info : impl.getTagFiles()) {
                            ctxt.setTagFileJarResource(info.getPath(), ctxt.getTagFileJarResource());
                        }
                    }
    ...
    private void parseBody(Node parent, String tag, String bodyType) throws JasperException {
        ...
        } else if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_JSP) || bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_SCRIPTLESS) || (bodyType == JAVAX_BODY_CONTENT_PARAM) || (bodyType == JAVAX_BODY_CONTENT_TEMPLATE_TEXT)) {
            while (reader.hasMoreInput()) {
                if (reader.matchesETag(tag)) {
                    return;
                }
