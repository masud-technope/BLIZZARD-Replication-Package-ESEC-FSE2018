...
public class TestSlowQueryComparator {
    ...
    private Comparator<QueryStats> createComparator() throws ClassNotFoundException, InstantiationException, IllegalAccessException, InvocationTargetException, SecurityException, NoSuchMethodException {
        Class<?> comparatorClass = Class.forName("org.apache.tomcat.jdbc.pool.interceptor.SlowQueryReport$QueryStatsComparator");
        Constructor<?> comparatorConstructor = comparatorClass.getDeclaredConstructor();
        comparatorConstructor.setAccessible(true);
        @SuppressWarnings("unchecked") Comparator<QueryStats> queryStatsComparator = (Comparator<QueryStats>) comparatorConstructor.newInstance();
