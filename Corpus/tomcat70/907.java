...
public abstract class AbstractReplicatedMap<K, V> implements Map<K, V>, Serializable, RpcCallback, ChannelListener, MembershipListener, Heartbeat {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o.getClass().equals(this.getClass())))
            return false;
        @SuppressWarnings("unchecked") AbstractReplicatedMap<K, V> other = (AbstractReplicatedMap<K, V>) o;
    ...
    public Member[] getMapMembersExcl(Member[] exclude) {
        synchronized (mapMembers) {
            ...
            @SuppressWarnings("unchecked") HashMap<Member, Long> list = (HashMap<Member, Long>) mapMembers.clone();
    ...
    public boolean inSet(Member m, Member[] set) {
        if (set == null)
            return false;
        boolean result = false;
        for (int i = 0; i < set.length && (!result); i++) if (m.equals(set[i]))
            result = true;
        return result;
    }
    ...
    public Member[] excludeFromSet(Member[] mbrs, Member[] set) {
        ...
        for (int i = 0; i < set.length; i++) {
            ...
            for (int j = 0; j < mbrs.length && include; j++) if (mbrs[j].equals(set[i]))
                include = false;
            if (include)
                result.add(set[i]);
        }
    ...
    public void memberDisappeared(Member member) {
        ...
        long complete = System.currentTimeMillis() - start;
        if (log.isInfoEnabled())
            log.info("Relocation of map entries was complete in " + complete + " ms.");
    }
        ...
        if (!entry.isPrimary()) {
            ...
            try {
                ...
                if (entry.isBackup()) {
                    ...
                    // notify member
                    msg = new MapMessage(getMapContextName(), MapMessage.MSG_NOTIFY_MAPMEMBER, false, (Serializable) entry.getKey(), null, null, channel.getLocalMember(false), backup);
                    ...
                    if (backup.length > 0) {
                        msg = new MapMessage(getMapContextName(), MapMessage.MSG_NOTIFY_MAPMEMBER, false, (Serializable) key, null, null, channel.getLocalMember(false), backup);
                        getChannel().send(backup, msg, getChannelSendOptions());
                    }
                }
                entry.setPrimary(channel.getLocalMember(false));
    ...
    protected void printMap(String header) {
        try {
            System.out.println("\nDEBUG MAP:" + header);
            System.out.println("Map[" + new String(mapContextName, CHARSET_ISO_8859_1) + ", Map Size:" + innerMap.size());
    ...
    public void putAll(Map<? extends K, ? extends V> m) {
        ...
        while (i.hasNext()) {
            @SuppressWarnings("unchecked") Map.Entry<K, V> entry = (Map.Entry<K, V>) i.next();
