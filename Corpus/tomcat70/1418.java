...
public class TesterAccessLogValve extends ValveBase implements AccessLog {
    ...
    public void validateAccessLog(int count, int status, long minTime, long maxTime) throws Exception {
        ...
        for (Entry entry : entries) {
            ...
            assertTrue(entry.toString() + " duration is not >= " + (minTime - ERROR_MARGIN), entry.getTime() >= minTime - ERROR_MARGIN);
            assertTrue(entry.toString() + " duration is not < " + (maxTime + ERROR_MARGIN), entry.getTime() < maxTime + ERROR_MARGIN);
        }
    }
