...
class PageDataImpl extends PageData implements TagConstants {
        ...
        private void addAttributes(Attributes attrs) {
            if (attrs != null) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (rootAttrs.getIndex(qName) == -1) {
                        rootAttrs.addAttribute(attrs.getURI(i), attrs.getLocalName(i), qName, attrs.getType(i), attrs.getValue(i));
                    }
                }
            }
        }
    ...
    static class SecondPassVisitor extends Node.Visitor implements TagConstants {
        ...
        private void appendPageDirective(Node.PageDirective n) {
            ...
            for (int i = 0; i < len; i++) {
                // If attrs==null, len == 0
                @SuppressWarnings("null") String attrName = attrs.getQName(i);
        ...
        private void printAttributes(Node n, boolean addDefaultNS) {
            ...
            for (int i = 0; i < len; i++) {
                // If attrs==null, len == 0
                @SuppressWarnings("null") String name = attrs.getQName(i);
                String value = attrs.getValue(i);
