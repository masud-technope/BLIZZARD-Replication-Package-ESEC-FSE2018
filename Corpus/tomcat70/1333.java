...
public class Cookie implements Cloneable, Serializable {
    ...
    public  Cookie(String name, String value) {
        ...
        if (!isToken(name) || // rfc2019
        name.equalsIgnoreCase("Comment") || // 2019++
        name.equalsIgnoreCase("Discard") || name.equalsIgnoreCase("Domain") || // (old cookies)
        name.equalsIgnoreCase("Expires") || // rfc2019
        name.equalsIgnoreCase("Max-Age") || name.equalsIgnoreCase("Path") || name.equalsIgnoreCase("Secure") || name.equalsIgnoreCase("Version") || name.startsWith("$")) {
    ...
    private boolean isToken(String possibleToken) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c < 0x20 || c >= 0x7f || tspecials.indexOf(c) != -1 || (STRICT_NAMING && tspecials2.indexOf(c) != -1)) {
                return false;
            }
        }
        return true;
