...
public class Http11NioProcessor extends AbstractHttp11Processor<NioChannel> {
    ...
    public  Http11NioProcessor(int maxHttpHeaderSize, NioEndpoint endpoint, int maxTrailerSize, Set<String> allowedTrailerHeaders, int maxExtensionSize, int maxSwallowSize) {
        super(endpoint);
    ...
    protected boolean breakKeepAliveLoop(SocketWrapper<NioChannel> socketWrapper) {
        ...
        if (sendfileData != null && !getErrorState().isError()) {
            ...
            switch(socketWrapper.getSocket().getPoller().processSendfile(key, (KeyAttachment) socketWrapper, true)) {
                case DONE:
    ...
    protected boolean prepareSendfile(OutputFilter[] outputFilters) {
        ...
        if (fileName != null) {
            ...
            sendfileData.pos = ((Long) request.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_START_ATTR)).longValue();
            sendfileData.length = ((Long) request.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_END_ATTR)).longValue() - sendfileData.pos;
            return true;
        }
        return false;
    }
