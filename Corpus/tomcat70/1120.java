...
public class AsyncStateMachine<S> {
    ...
    private static enum AsyncState implements  {
    ...
    public synchronized boolean asyncError() {
        boolean doDispatch = false;
        if (state == AsyncState.STARTING || state == AsyncState.DISPATCHED || state == AsyncState.TIMING_OUT || state == AsyncState.MUST_COMPLETE) {
