...
public class AjpNioProcessor extends AbstractAjpProcessor<NioChannel> {
    ...
    private int readSocket(byte[] buf, int pos, int n, boolean block) throws IOException {
        ...
        if (block) {
            ...
            try {
                NioEndpoint.KeyAttachment att = (NioEndpoint.KeyAttachment) socketWrapper.getSocket().getAttachment();
                if (att == null)
                    throw new IOException("Key must be cancelled.");
    ...
    protected int readMessage(AjpMessage message, boolean blockFirstRead) throws IOException {
        ...
        if (messageLength < 0) {
            ...
            if (messageLength > buf.length) {
                ...
                throw new IllegalArgumentException(sm.getString("ajpprocessor.header.tooLong", Integer.valueOf(messageLength), Integer.valueOf(buf.length)));
            }
