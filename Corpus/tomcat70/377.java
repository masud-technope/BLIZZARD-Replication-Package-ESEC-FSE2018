...
public abstract class TomcatBaseTest extends LoggingBaseTest {
    ...
    public static final class RequestDescriptor {
        ...
        private final Map<String, String> requestInfo = new HashMap<String, String>();

        private final Map<String, String> contextInitParameters = new HashMap<String, String>();

        private final Map<String, String> contextAttributes = new HashMap<String, String>();

        private final Map<String, String> headers = new CaseInsensitiveKeyMap<String>();
    ...
    public static final class SnoopServlet extends HttpServlet {
        ...
        public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
            ...
            for (Enumeration<String> e = ctx.getInitParameterNames(); e.hasMoreElements(); ) {
                name = e.nextElement();
                out.println("CONTEXT-INIT-PARAM:" + name + ": " + ctx.getInitParameter(name));
            }
            for (Enumeration<String> e = ctx.getAttributeNames(); e.hasMoreElements(); ) {
                name = e.nextElement();
                out.println("CONTEXT-ATTRIBUTE:" + name + ": " + ctx.getAttribute(name));
            }
            ...
            out.println("REQUEST-PATH-TRANSLATED: " + request.getPathTranslated());
            ...
            out.println("REQUEST-USER-PRINCIPAL: " + request.getUserPrincipal());
            out.println("REQUEST-CHARACTER-ENCODING: " + request.getCharacterEncoding());
            out.println("REQUEST-CONTENT-LENGTH: " + request.getContentLength());
            out.println("REQUEST-CONTENT-TYPE: " + request.getContentType());
            out.println("REQUEST-LOCALE: " + request.getLocale());
            for (Enumeration<String> e = request.getHeaderNames(); e.hasMoreElements(); ) {
                name = e.nextElement();
                value = new StringBuilder();
                for (Enumeration<String> h = request.getHeaders(name); h.hasMoreElements(); ) {
            ...
            for (Enumeration<String> e = request.getAttributeNames(); e.hasMoreElements(); ) {
                ...
                out.println("ATTRIBUTE:" + name + ": " + (attribute != null ? attribute : "(null)"));
            }
            ...
            out.println("SESSION-REQUESTED-ID: " + request.getRequestedSessionId());
            out.println("SESSION-REQUESTED-ID-COOKIE: " + request.isRequestedSessionIdFromCookie());
            out.println("SESSION-REQUESTED-ID-URL: " + request.isRequestedSessionIdFromUrl());
            out.println("SESSION-REQUESTED-ID-VALID: " + request.isRequestedSessionIdValid());
            if (session != null) {
                ...
                for (Enumeration<String> e = session.getAttributeNames(); e.hasMoreElements(); ) {
                    ...
                    out.println("SESSION-ATTRIBUTE:" + name + ": " + (attribute != null ? attribute : "(null)"));
                }
            }
            int bodySize = 0;
    ...
    public static int getUrl(String path, ByteChunk out, Map<String, List<String>> resHead) throws IOException {
        return getUrl(path, out, null, resHead);
    }

    public static int headUrl(String path, ByteChunk out, Map<String, List<String>> resHead) throws IOException {
        return methodUrl(path, out, 1000000, null, resHead, "HEAD");
    }

    public static int getUrl(String path, ByteChunk out, Map<String, List<String>> reqHead, Map<String, List<String>> resHead) throws IOException {
        return getUrl(path, out, 1000000, reqHead, resHead);
    }

    public static int getUrl(String path, ByteChunk out, int readTimeout, Map<String, List<String>> reqHead, Map<String, List<String>> resHead) throws IOException {
        return methodUrl(path, out, readTimeout, reqHead, resHead, "GET");
    }

    public static int methodUrl(String path, ByteChunk out, int readTimeout, Map<String, List<String>> reqHead, Map<String, List<String>> resHead, String method) throws IOException {
    ...
    public static ByteChunk postUrl(byte[] body, String path) throws IOException {
        ByteChunk out = new ByteChunk();
        postUrl(body, path, out, null);
        return out;
    }

    public static int postUrl(byte[] body, String path, ByteChunk out, Map<String, List<String>> resHead) throws IOException {
        return postUrl(body, path, out, null, resHead);
    }

    public static int postUrl(byte[] body, String path, ByteChunk out, Map<String, List<String>> reqHead, Map<String, List<String>> resHead) throws IOException {
        URL url = new URL(path);
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
