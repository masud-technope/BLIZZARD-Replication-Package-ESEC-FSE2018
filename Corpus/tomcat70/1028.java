...
public class ReplicatedMap<K, V> extends AbstractReplicatedMap<K, V> {
    ...
    public  ReplicatedMap(MapOwner owner, Channel channel, long timeout, String mapContextName, ClassLoader[] cls, boolean terminate) {
        super(owner, channel, timeout, mapContextName, AbstractReplicatedMap.DEFAULT_INITIAL_CAPACITY, AbstractReplicatedMap.DEFAULT_LOAD_FACTOR, Channel.SEND_OPTIONS_DEFAULT, cls, terminate);
    }
            ...
            if (realFaultyMembers.length != 0) {
                ...
                if (backup.length == 0) {
                    ...
                    if (log.isWarnEnabled()) {
                        log.warn("Unable to replicate backup key:" + key + ". Success nodes:" + Arrays.toString(backup) + ". Failed nodes:" + Arrays.toString(realFaultyMembers), e);
                    }
                }
            }
        }
