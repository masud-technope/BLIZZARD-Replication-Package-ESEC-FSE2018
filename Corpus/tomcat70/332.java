...
public class DeltaManager extends ClusterManagerBase {
    ...
    public Session createSession(String sessionId, boolean distribute) {
        ...
        if (log.isDebugEnabled())
            log.debug(sm.getString("deltaManager.createSession.newSession", session.getId(), Integer.valueOf(sessions.size())));
        return (session);
    }
    ...
    protected void handleCHANGE_SESSION_ID(SessionMessage msg, Member sender) throws IOException {
        ...
        if (session != null) {
            ...
            if (notifyContainerListenersOnReplication) {
                getContainer().fireContainerEvent(Context.CHANGE_SESSION_ID_EVENT, new String[] { msg.getSessionID(), newSessionID });
            }
        }
    }
