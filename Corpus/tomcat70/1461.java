...
public class Membership implements Cloneable {
    ...
    public Object clone() {
        synchronized (membersLock) {
            ...
            @SuppressWarnings("unchecked") final HashMap<MemberImpl, MbrEntry> tmpclone = (HashMap<MemberImpl, MbrEntry>) map.clone();
            clone.map = tmpclone;
    ...
    public synchronized MemberImpl[] expire(long maxtime) {
        ...
        while (i.hasNext()) {
            ...
            if (entry.hasExpired(maxtime)) {
                if (// only need a list when members are expired (smaller gc)
                list == null)
    ...
    protected synchronized MbrEntry[] getMemberEntries() {
        ...
        int pos = 0;
        while (i.hasNext()) result[pos++] = i.next().getValue();
        return result;
    }
