...
public class AprEndpoint extends AbstractEndpoint<Long> {
    ...
    @Override
    public boolean getUseCometTimeout() // Not supported
    {
        return false;
    }

    @Override
    public boolean getUsePolling() // Always supported
    {
        return true;
    }
    ...
    protected boolean processSocketWithOptions(long socket) {
        try {
            ...
            if (running) {
                if (log.isDebugEnabled()) {
                    log.debug(sm.getString("endpoint.debug.socket", Long.valueOf(socket)));
                }
                AprSocketWrapper wrapper = new AprSocketWrapper(Long.valueOf(socket));
            ...
            synchronized (socket) {
                if (waitingRequests.remove(socket)) {
                    ...
                    try {
                        ...
                        Executor executor = getExecutor();
                        if (executor == null) {
                            log.warn(sm.getString("endpoint.warn.noExector", socket, status));
                            return;
                        } else {
    ...
    protected class AsyncTimeout implements Runnable {
        ...
        public void run() {
            ...
            while (asyncTimeoutRunning) {
                ...
                Iterator<SocketWrapper<Long>> sockets = waitingRequests.iterator();
    ...
    public static class SocketInfo {
        ...
        public static int merge(int flag1, int flag2) {
            return ((flag1 & Poll.APR_POLLIN) | (flag2 & Poll.APR_POLLIN)) | ((flag1 & Poll.APR_POLLOUT) | (flag2 & Poll.APR_POLLOUT));
        }
    ...
    public class Poller implements Runnable {
        ...
        public void add(long socket, int timeout, boolean read, boolean write) {
            add(socket, timeout, (read ? Poll.APR_POLLIN : 0) | (write ? Poll.APR_POLLOUT : 0));
        }

        private void add(long socket, int timeout, int flags) {
            if (log.isDebugEnabled()) {
                String msg = sm.getString("endpoint.debug.pollerAdd", Long.valueOf(socket), Integer.valueOf(timeout), Integer.valueOf(flags));
                if (log.isTraceEnabled()) {
                    log.trace(msg, new Exception());
                } else {
                    log.debug(msg);
    ...
    public interface Handler extends AbstractEndpoint.Handler {
        ...
        public SocketState process(SocketWrapper<Long> socket, SocketStatus status);
    }
