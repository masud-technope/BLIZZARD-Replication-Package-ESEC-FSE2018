...
public class MbeansSource extends ModelerSource implements MbeansSourceMBean {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            // Process nodes
            for (Node mbeanN = firstMbeanN; mbeanN != null; mbeanN = DomUtil.getNext(mbeanN, null, Node.ELEMENT_NODE)) {
                ...
                if ("mbean".equals(nodeName) || "MLET".equals(nodeName)) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("Processing mbean objectName=" + objectName + " code=" + code);
                    ...
                    for (Node descN = firstAttN; descN != null; descN = DomUtil.getNext(descN)) {
                    ...
                    if (log.isDebugEnabled())
                        log.debug("Processing invoke objectName=" + name + " code=" + operation);
                    try {
    ...
    private void processAttribute(MBeanServer server, Node descN, String objectName) {
        ...
        // DomUtil.getAttribute(descN, "type");
        String type = null;
        if (value == null) {
