...
public class SampleStandbyContent implements IStandbyContentPart {
    ...
    public void createPartControl(Composite parent, FormToolkit toolkit) {
        ...
        moreLink = //$NON-NLS-1$
        toolkit.createHyperlink(//$NON-NLS-1$
        form.getBody(), //$NON-NLS-1$
        "Read More", SWT.NULL);
        ...
        instText.setImage("run", //$NON-NLS-1$
        PDEPlugin.getDefault().getLabelProvider().get(PDEPluginImages.DESC_RUN_EXC));
        instText.setImage("debug", //$NON-NLS-1$
        PDEPlugin.getDefault().getLabelProvider().get(PDEPluginImages.DESC_DEBUG_EXC));
        instText.setImage("help", //$NON-NLS-1$
        ...
        for (int i = 0; i < projects.length; i++) {
            ...
            if (pfile.exists()) {
                try {
                    ...
                    is.close();
                    String id = //$NON-NLS-1$
                    prop.getProperty(//$NON-NLS-1$
                    "id");
                    if (id != null && id.equals(sid)) {
                        //match
                        IResource res = findSelectReveal(project, //$NON-NLS-1$
                        prop.getProperty(//$NON-NLS-1$
                        "projectName"));
                        if (res != null)
                            items.add(res);
    ...
    private IResource findSelectReveal(IProject project, String originalName) {
        ...
        for (int i = 0; i < projects.length; i++) {
            if (//$NON-NLS-1$
            originalName.equals(projects[i].getAttribute("name"))) {
    ...
    private void update(IConfigurationElement sample) {
        ...
        //$NON-NLS-1$
        IConfigurationElement[] descConfig = sample != null ? sample.getChildren("description") : null;
        if (descConfig != null && descConfig.length == 1) {
            String desc = descConfig[0].getValue();
            //$NON-NLS-1$
            String content = NLS.bind(PDEUIMessages.SampleStandbyContent_desc, (desc != null ? desc : ""));
            //$NON-NLS-1$
