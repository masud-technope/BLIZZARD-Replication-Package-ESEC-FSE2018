...
public class PDEJavaHelperUI {
    ...
    public static String selectType(IResource resource, int scope) {
        ...
        try {
            //$NON-NLS-1$
            SelectionDialog dialog = JavaUI.createTypeDialog(PDEPlugin.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), PDEJavaHelper.getSearchScope(project), scope, false, "");
    ...
    public static String selectType(IResource resource, int scope, String filter, String superTypeName) {
        ...
        try {
            ...
            if (//$NON-NLS-1$
            superTypeName != null && !superTypeName.equals("java.lang.Object")) {
    ...
    public static String getOSGIConstantJavaDoc(String constant, IJavaProject jp) {
        //$NON-NLS-1$
        return getJavaDoc(constant, jp, "org.osgi.framework.Constants");
    }
        ...
        if (javaDoc == null) {
            try {
                ...
                if (type != null) {
                    ...
                    for (int i = 0; i < chars.length; i++) chars[i] = chars[i] == '-' ? '_' : Character.toUpperCase(chars[i]);
