...
public class XMLErrorReporter extends DefaultHandler {
    ...
    private String generateLocationPath(Node node, String attrName) {
        if (node == null)
            ...
            return "";
        int childIndex = 0;
        for (Node previousSibling = node.getPreviousSibling(); previousSibling != null; previousSibling = previousSibling.getPreviousSibling()) childIndex += 1;
    ...
    private int getAttributeOffset(String name, String value, int offset) throws BadLocationException {
        //$NON-NLS-1$
        IRegion nameRegion = fFindReplaceAdapter.find(offset, name + "=\"" + getWritableString(value), true, false, false, false);
        if (nameRegion != null) {
    ...
    protected String getTextContent(Element element) {
        ...
        try {
            ...
            //$NON-NLS-1$
            IRegion openElement = fFindReplaceAdapter.find(data.offset, ">", true, true, false, false);
            //$NON-NLS-1$ //$NON-NLS-2$
            IRegion closeElement = fFindReplaceAdapter.find(data.offset, "</" + element.getNodeName() + ">", true, true, false, false);
    ...
    public InputSource resolveEntity(String publicId, String systemId) throws SAXException {
        //$NON-NLS-1$
        int x = fTextDocument.get().indexOf("!DOCTYPE");
        if (x > 0) {
            try {
