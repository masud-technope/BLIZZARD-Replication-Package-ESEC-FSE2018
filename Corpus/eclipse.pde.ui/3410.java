...
public class AnnotationIncrementalBuildTests extends EnumIncrementalBuildTests {
    ...
    public void testIncrementalAnnot() throws Exception {
        int[] problemids = new int[] { ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_COMPATIBILITY, IDelta.ANNOTATION_ELEMENT_TYPE, IDelta.REMOVED, IDelta.METHOD_WITH_DEFAULT_VALUE), ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_USAGE, IElementDescriptor.TYPE, IApiProblem.UNSUPPORTED_TAG_USE, IApiProblem.NO_FLAGS) };
        deployIncrementalPerformanceTest(//$NON-NLS-1$
        "Incremetal Build (Annotation) - All", //$NON-NLS-1$
        "test12", JAVA_5_PERF, //$NON-NLS-1$
        "api.TestAnnot", problemids, 500);
    }
    ...
    public void _testIncrementalAnnotCompat() throws Exception {
        int[] problemids = new int[] { ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_COMPATIBILITY, IDelta.ANNOTATION_ELEMENT_TYPE, IDelta.REMOVED, IDelta.METHOD_WITH_DEFAULT_VALUE) };
        deployIncrementalPerformanceTest(//$NON-NLS-1$
        "Incremetal Build (Annotation) - Compatibility", //$NON-NLS-1$
        "test13", JAVA_5_PERF, //$NON-NLS-1$
        "api.TestAnnot", problemids, 500);
    }
    ...
    public void _testIncrementalAnnotTags() throws Exception {
        int[] problemids = new int[] { ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_USAGE, IElementDescriptor.TYPE, IApiProblem.UNSUPPORTED_TAG_USE, IApiProblem.NO_FLAGS), ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_USAGE, IElementDescriptor.METHOD, IApiProblem.UNSUPPORTED_TAG_USE, IApiProblem.NO_FLAGS) };
        deployIncrementalPerformanceTest(//$NON-NLS-1$
        "Incremetal Build (Annotation) - Unsupported Tags", //$NON-NLS-1$
        "test14", JAVA_5_PERF, //$NON-NLS-1$
        "api.TestAnnot", problemids, 500);
    }
}
