...
public class ProjectState implements Serializable, Cloneable {
    ...
    private String fromLegacyCUKey(String cuKey) {
        //$NON-NLS-1$
        return String.format("%s.java", cuKey.replace('.', '/'));
    }
    ...
    public ProjectState clone() {
        ...
        for (Map.Entry<String, Collection<String>> entry : mappings.entrySet()) clone.mappings.put(entry.getKey(), new HashSet(entry.getValue()));
        if (types != null) {
            clone.types = new HashMap(types.size());
            for (Map.Entry<String, Collection<String>> entry : types.entrySet()) clone.types.put(entry.getKey(), new HashSet(entry.getValue()));
        }
        if (files != null) {
    ...
    public boolean equals(Object obj) {
        ...
        return formatVersion == o.formatVersion && (path == null ? o.path == null : path.equals(o.path)) && errorLevel == o.errorLevel && missingUnbindMethodLevel == o.missingUnbindMethodLevel && mappings.equals(o.mappings) && (files == null ? o.files == null : files.equals(o.files)) && (types == null ? o.types == null : types.equals(o.types));
    }
    ...
    public String toString() {
        ...
        //$NON-NLS-1$
        buf.append(path).append(";mappings=");
        //$NON-NLS-1$
        buf.append(mappings).append(";types=");
        //$NON-NLS-1$
        buf.append(types).append(";files=");
        //$NON-NLS-1$
        buf.append(files).append(";errorLevel=");
        //$NON-NLS-1$
        buf.append(errorLevel).append(";missingUnbindMethodLevel=");
        //$NON-NLS-1$
        buf.append(missingUnbindMethodLevel).append(";formatVersion=");
