...
public class TocTreeSection extends TreeSection {
    ...
    private void initDragAndDrop() {
        ...
        if (// Model objects and files can be dropped onto the
        isEditable()) {
            // viewer
    ...
    public boolean setFormInput(Object object) {
        if (// Select the item in the tree
        object instanceof TocObject) {
    ...
    protected void fillContextMenu(IMenuManager manager) {
        ...
        if (tocObject != null) {
            ...
            if (// Create the "New" sub-menu
            tocObject.canBeParent()) {
            ...
            if (// Add a separator to the main context menu
            !emptyMenu) {
        ...
        manager.add(new Separator());
        if (// Add the Remove action and Show In action if
        tocObject != null) {
    ...
    private void fillContextMenuShowInAction(IMenuManager manager) {
        ...
        if (bindingService != null) {
            String keyBinding = bindingService.getBestActiveBindingFormattedFor("org.eclipse.ui.navigate.showInQuickMenu");
            if (keyBinding != null) {
    ...
    private void fillContextMenuAddActions(MenuManager submenu, TocObject tocObject) {
        if (// Add the 'Add
        tocObject != null && tocObject.canBeParent()) {
            // Topic' action to
            // the sub-menu
    ...
    private void openResource(IResource resource, boolean tocFile) {
        ...
        if (isFileValidInContext(tocFile, path)) {
            ...
            } catch (PartInitException // suppress exception
            e) {
            }
        }
    }
    ...
    public boolean performDrop(Object currentTarget, Object dropped, int location) {
        if (dropped instanceof Object[]) {
            ...
            if (location == TocDropAdapter.LOCATION_JUST_AFTER && targetParent == tocTarget && !tocTarget.getChildren().isEmpty() && // If the drop
            fTocTree.getExpandedState(// If the drop
            tocTarget)) {
                // occurs just
                // after a
                // parentable
            ...
            if (// Get the TocObject versions of the
            targetParent != null) {
                // dropped objects
                ArrayList objectsToAdd = getObjectsToAdd((Object[]) dropped, targetParent);
                if (objectsToAdd != null && !objectsToAdd.isEmpty()) {
                    if (fDragAdapter.getDraggedElements() != null && fDragAdapter.getDraggedElements().size() == 1 && currentTarget == fDragAdapter.getDraggedElements().get(0)) {
    ...
    private TocTopic determineParent(TocObject dropTarget, int dropLocation) {
        if (dropTarget == null || dropTarget.getType() == // Since
        ITocConstants.TYPE_TOC) {
            // parent
            return fModel.getToc();
        } else if (// If the object is a leaf, it
        !dropTarget.canBeParent()) {
        ...
        } else // In all other cases, it depends on the location of the drop
        {
            ...
            switch(dropLocation) {
                case // if the drop occurred
                TocDropAdapter.LOCATION_JUST_AFTER:
                    {
                ...
                case // if the drop is before or
                ViewerDropAdapter.LOCATION_AFTER:
                    {
                        // make the drop target's parent the target parent object
                        return (TocTopic) dropTarget.getParent();
                    }
    ...
    private ArrayList getObjectsToAdd(Object[] droppings, TocTopic targetParent) {
        ...
        if (// If there are items
        fDragAdapter.getDraggedElements() != null) {
            // in the drag
            // adapter, then the
            // current drag must
            // be from
    ...
    private void handleDeleteAction() {
        ...
        if (// If any object cannot be removed, beep to notify the user
        beep) {
    ...
    public void handleRemove(List itemsToRemove) {
        if (// Target the objects for removal
        !itemsToRemove.isEmpty()) {
    ...
    private void handleMoveAction(int positionFlag) {
        ...
        for (Iterator iter = sel.iterator(); iter.hasNext(); ) {
            ...
            if (object == null) {
                ...
                if (// Move the object up or down one position
                parent != null) {
    ...
    private void handleModelChangeTypeSwap(IModelChangedEvent event) {
        ...
        if (// Update the element in the tree viewer
        object != null) {
    ...
    protected TreeViewer createTreeViewer(Composite parent, int style) {
        ...
        //$NON-NLS-1$
        parent.setData("filtered", Boolean.TRUE);
