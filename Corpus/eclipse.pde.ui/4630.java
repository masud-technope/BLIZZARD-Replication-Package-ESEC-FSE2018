...
public class DSOptionsSection extends PDESection {
    ...
    protected void createClient(Section section, FormToolkit toolkit) {
        ...
        // Attribute: factory
        fFactoryEntry = new FormEntry(client, toolkit, Messages.DSComponentDetails_factoryEntry, SWT.NONE);
        // Attribute: configuration policy
        Label label = toolkit.createLabel(client, Messages.DSComponentDetails_configurationPolicy, SWT.WRAP);
        ...
        "", IDSConstants.VALUE_CONFIGURATION_POLICY_OPTIONAL, IDSConstants.VALUE_CONFIGURATION_POLICY_REQUIRE, IDSConstants.VALUE_CONFIGURATION_POLICY_IGNORE };
        fConfigurationPolicy.setItems(items);
        GridDataFactory.fillDefaults().grab(true, false).indent(3, 0).applyTo(fConfigurationPolicy.getControl());
    ...
    private void createButtons(Composite parent, FormToolkit toolkit) {
        fEnabledButton = toolkit.createButton(parent, Messages.DSServiceComponentSection_enabledButtonMessage, SWT.CHECK);
        GridData data = new GridData(GridData.FILL_HORIZONTAL);
        data.horizontalSpan = 2;
        ...
        fImmediateButton = toolkit.createButton(parent, Messages.DSServiceComponentSection_immediateButtonMessage, SWT.CHECK);
    ...
    public void updateUIFields() {
        if (fComponent != null) {
            if (fComponent.getFactory() == null) {
                // Attribute: factory
                //$NON-NLS-1$
                fFactoryEntry.setValue(//$NON-NLS-1$
                "", //$NON-NLS-1$
                true);
            } else {
                // Attribute: factory
    ...
    private void enableOrDisableImmediate() {
        ...
        boolean isFactory = fComponent.getFactory() != null && !//$NON-NLS-1$
