...
public abstract class MethodLeakDetector extends AbstractLeakProblemDetector {
        ...
        try {
            ...
            if (annotations != null) {
                ...
                if (isEnclosingTypeVisible(type)) {
                    ...
                    ApiPlugin.log(new Status(IStatus.INFO, ApiPlugin.PLUGIN_ID, MessageFormat.format(BuilderMessages.AbstractTypeLeakDetector_vis_type_has_no_api_description, type.getName())));
                } else {
    ...
    protected String[] getMessageArgs(IReference reference) throws CoreException {
        IApiMethod method = (IApiMethod) reference.getMember();
        IApiType type = (IApiType) reference.getResolvedReference();
        return new String[] { getSimpleTypeName(type), getSimpleTypeName(method), Signatures.getMethodSignature(method) };
    }
    ...
    protected String[] getQualifiedMessageArgs(IReference reference) throws CoreException {
        IApiMethod method = (IApiMethod) reference.getMember();
        IApiType type = (IApiType) reference.getResolvedReference();
        return new String[] { getQualifiedTypeName(type), getQualifiedTypeName(method), Signatures.getMethodSignature(method) };
    }
