...
public abstract class BaseExtensionPointMainPage extends WizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        fIdText.addModifyListener(new ModifyListener() {
            ...
            public void modifyText(ModifyEvent e) {
                ...
                fSchemaText.setText(getSchemaLocation() + (getSchemaLocation().length() > 0 ? "/" : "") + fIdText.getText() + ".exsd");
            }
        });
        label = new Label(container, SWT.NONE);
    ...
    private InputStream createSchemaStream(String pluginId, String pointId, String name, boolean shared, IFile schemaFile) {
        ...
        if (!shared) {
            //$NON-NLS-1$
            element = new SchemaRootElement(schema, "extension");
        ...
        schema.addDocumentSection(section);
        //$NON-NLS-1$
        section = new DocumentSection(schema, IDocumentSection.IMPLEMENTATION, "Supplied Implementation");
    ...
    private void handleSchemaLocation() {
        ...
        if (dialog.open() == Window.OK) {
            ...
            if (elements.length > 0) {
                ...
                //$NON-NLS-1$
                fSchemaLocationText.setText(//$NON-NLS-1$
                newPath + "/");
            }
        }
    }
