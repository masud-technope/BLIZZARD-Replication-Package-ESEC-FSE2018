...
public class XMLContentAssistProcessor extends TypePackageCompletionProcessor implements ICompletionListener {
    ...
    static {
        //$NON-NLS-1$
        F_V_BOOLS.add(new VirtualSchemaObject("true", null, F_ATTRIBUTE_BOOLEAN_VALUE));
        //$NON-NLS-1$
        F_V_BOOLS.add(new VirtualSchemaObject("false", null, F_ATTRIBUTE_BOOLEAN_VALUE));
    }
    ...
    public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
        ...
        if (docLen == fDocLen)
            // left/right cursor has been pressed - cancel content assist
            return null;
        fDocLen = docLen;
    ...
    private ICompletionProposal[] computeCompletionProposal(IDocumentAttributeNode attr, int offset, IDocument doc) {
        ...
        if (obj instanceof IPluginExtension) {
            ...
            // provide proposals with all resources in current plugin?
                ...
                for (int i = 0; i < validAttributes.length; i++) objs.add(new VirtualSchemaObject(validAttributes[i], null, F_ATTRIBUTE_ID_VALUE));
            ...
            } else // we have an IMetaAttribute.STRING kind
            {
                ...
                if (sRestr == null) {
                    ...
                    type.getName().equals(//$NON-NLS-1$
                    "boolean"))
                        objs = F_V_BOOLS;
                } else {
                    Object[] restrictions = sRestr.getChildren();
                    for (int i = 0; i < restrictions.length; i++) if (restrictions[i] instanceof ISchemaObject)
    ...
    private ICompletionProposal[] computeAttributeProposal(IDocumentAttributeNode attr, int offset, String currValue, List<VirtualSchemaObject> validValues) {
        ...
        for (int i = 0; i < validValues.size(); i++) addToList(list, currValue, validValues.get(i));
        ...
        if (offset > node.getXMLTagName().length() + 1) {
            try {
                ...
                if (ind == 0 && offset == len - 1)
                    // childless node - check if it can be cracked open
                    return F_OPEN_TAG;
        ...
        try {
            while (--offset[0] >= 0) {
                ...
                if (c == '"') {
                    ...
                    if (// ran into 2nd quotation mark, we are out of range
                    attVal != null)
                        continue;
                    offset[2] = offset[0];
    ...
    private ICompletionProposal[] computeAttributeProposals(ISchemaObject[] sAttrs, IDocumentElementNode node, int offset, String filter, String parentName) {
        ...
        for (int i = 0; i < sAttrs.length; i++) {
            // if we break early we wont add
            int k;
            for (k = 0; k < attrs.length; k++) if (attrs[k].getAttributeName().equals(sAttrs[i].getName()))
                break;
            if (k == attrs.length)
    ...
    private ICompletionProposal[] convertListToProposal(ArrayList<ISchemaObject> list, IDocumentRange range, int offset) {
        ...
        if (proposals.length == 0)
            return null;
        for (int i = 0; i < proposals.length; i++) proposals[i] = new XMLCompletionProposal(range, list.get(i), offset, this);
        return proposals;
    }
