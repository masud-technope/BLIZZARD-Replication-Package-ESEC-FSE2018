            ...
            if (set.add(exporter.getSymbolicName())) {
                ...
                for (ExportPackageDescription exportedPkg : exportedPkgs) {
                    if (exportedPkg.getName().equals(pkgName)) {
                        // any one is fine, so simply remember the last one
        ...
        if (supplier != null) {
            ...
            if (!supplierImported) {
                ...
                if (proposeImportPackage) {
                    // relevance should actually be 16...
        ...
        if (selectedNode != null) {
            ...
            if (node == null) {
                if (selectedNode instanceof Name) {
                    ...
                    if (// fallback if the type cannot be resolved
                    className == null && selectedNode instanceof SimpleName) {
                ...
                if (!((ImportDeclaration) node).isOnDemand()) {
                    // if there is no period assume we are importing a single name package
