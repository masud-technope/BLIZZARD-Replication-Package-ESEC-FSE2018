...
public class PluginImportOperation extends // The IBundleImporter API is currently provisional
WorkspaceJob {
    ...
    public IStatus runInWorkspace(IProgressMonitor monitor) throws CoreException {
        ...
        if (fImportType == IMPORT_FROM_REPOSITORY) {
            ...
            for (int i = 0; i < fModels.length; i++) {
                subMonitor.setTaskName(NLS.bind(PDEUIMessages.PluginImportOperation_Importing_plugin, fModels[i].getPluginBase().getId()));
                try {
    ...
    private void importPlugin(IPluginModelBase model, int importType, IProgressMonitor monitor) throws CoreException {
        ...
        if (Platform.inDevelopmentMode()) {
            ...
            if (location.isDirectory()) {
                File classpathFile = new //$NON-NLS-1$
                File(//$NON-NLS-1$
                location, //$NON-NLS-1$
                ".classpath");
                File projectFile = new //$NON-NLS-1$
                File(//$NON-NLS-1$
                location, //$NON-NLS-1$
                ".project");
                if (classpathFile.exists() && classpathFile.isFile() && projectFile.exists() && projectFile.isFile()) {
                    PluginImportHelper.importContent(location, project.getFullPath(), FileSystemStructureProvider.INSTANCE, null, subMonitor.split(4));
                    return;
                }
            }
        }
        ...
        // Set the classpath
        if (//$NON-NLS-1$
        project.hasNature(JavaCore.NATURE_ID) && project.findMember(".classpath") == null)
    ...
    private Map<String, IPath> importAsBinary(IProject project, IPluginModelBase model, IProgressMonitor monitor) throws CoreException {
        ...
        if (isJARd(model)) {
            ...
            PluginImportHelper.importContent(new File(model.getInstallLocation()), project.getFullPath(), FileSystemStructureProvider.INSTANCE, null, subMonitor.split(1));
        }
    ...
    private Map<String, IPath> importAsBinaryWithLinks(IProject project, IPluginModelBase model, IProgressMonitor monitor) throws CoreException {
        ...
        Map<String, IPath> sourceMap = importSourceArchives(project, model, IMPORT_BINARY_WITH_LINKS, subMonitor.split(1));
        // Import additional source files such as schema files for easy access,
        // see bug 139161
        importAdditionalSourceFiles(project, model, subMonitor.split(1));
        // Extract the required bundle files and modify the imported manifest to
    ...
    private void importAsSource(IProject project, IPluginModelBase model, IProgressMonitor monitor) throws CoreException {
        try {
            ...
            // maps package path to a src folder
            Map<IPath, IPath> packageLocations = new HashMap();
            boolean sourceFound = extractSourceFolders(project, model, buildModel, packageLocations, subMonitor.split(1));
            ...
            if (isJARd(model)) {
                ...
                try {
                    ...
                    for (Iterator<IPath> iterator = collected.keySet().iterator(); iterator.hasNext(); ) {
                        ...
                        PluginImportHelper.importContent(provider.getRoot(), destination, provider, collected.get(currentDestination), iterationMonitor.split(1));
                    }
                } finally {
                    if (zip != null) {
                        zip.close();
                ...
                for (Iterator<IPath> iterator = collected.keySet().iterator(); iterator.hasNext(); ) {
                    ...
                    IPath destination = project.getFullPath();
                    destination = destination.append(currentDestination);
                    PluginImportHelper.importContent(srcFile, destination, FileSystemStructureProvider.INSTANCE, collected.get(currentDestination), iterationMonitor.split(1));
                }
            }
    ...
    private IProject createProject(IPluginModelBase model, IProgressMonitor monitor) throws TeamException, CoreException {
        ...
        if (project.exists() || new File(project.getParent().getLocation().toFile(), project.getName()).exists()) {
            ...
            if (project.exists()) {
                ...
                int jarIndex = //$NON-NLS-1$
                projectName.toLowerCase().lastIndexOf(//$NON-NLS-1$
                ".jar");
                if (jarIndex >= 0) {
    ...
    private Map<String, IPath> importSourceArchives(IProject project, IPluginModelBase model, int mode, IProgressMonitor monitor) throws CoreException {
        String[] libraries = getLibraryNames(model);
        SubMonitor subMonitor = SubMonitor.convert(monitor, PDEUIMessages.ImportWizard_operation_importingSource, libraries.length);
                ...
                if (srcPath != null) {
                    ...
                    if (project.findMember(dstPath) == null) {
                        if (mode == IMPORT_BINARY) {
                            PluginImportHelper.copyArchive(new File(srcPath.toOSString()), project.getFile(dstPath), subMonitor.split(1));
        ...
        if (manager != null) {
                ...
                for (int i = 0; i < libraries.length; i++) {
                    if (libraries[i].equals(DEFAULT_LIBRARY_NAME)) {
                        ...
                        if (!destination.exists()) {
                            ...
                            PluginImportHelper.extractJavaSourceFromArchive(srcFile, excludeFolders, destination.getFullPath(), collectedPackages, subMonitor.split(1));
                            //$NON-NLS-1$ //$NON-NLS-2$
                            addBuildEntry(buildModel, "source." + DEFAULT_LIBRARY_NAME, DEFAULT_SOURCE_DIR + "/");
                        ...
                        if (!project.getFolder(sourceDir).exists()) {
                            Set<IPath> collectedPackages = new HashSet();
                            PluginImportHelper.extractFolderFromArchive(srcFile, sourceDir, project.getFullPath(), collectedPackages, subMonitor.split(1));
                            addBuildEntry(//$NON-NLS-1$
                            buildModel, //$NON-NLS-1$
                            "source." + libraries[i], //$NON-NLS-1$
                            sourceDir.toString());
            ...
            for (int i = 0; i < libraries.length; i++) {
                ...
                if (srcPath != null) {
                    ...
                    if (!destination.exists()) {
                        Set<IPath> collectedPackages = new HashSet();
                        PluginImportHelper.extractArchive(new File(srcPath.toOSString()), destination.getFullPath(), collectedPackages, subMonitor.split(1));
                        addBuildEntry(//$NON-NLS-1$
                        buildModel, //$NON-NLS-1$
                        "source." + libraries[i], //$NON-NLS-1$
                        dstPath.toString());
        ...
        for (Iterator<IPath> iterator = packages.iterator(); iterator.hasNext(); ) {
            ...
            while (currentPackage.segmentCount() > 1) {
                ...
                if (packageLocations.containsKey(currentPackage)) {
                    // Don't overwrite existing entries, we assume that if one parent has an entry, all further parents already have an entry
                    break;
                }
                packageLocations.put(currentPackage, destination);
            }
            ...
            if (classpath == null) {
                ...
                try {
                    ...
                    for (int i = 0; i < elements.length; i++) {
                        ...
                        if (//$NON-NLS-1$
                        elements[i].getValue().equals(//$NON-NLS-1$
                        "."))
    ...
    private boolean needsJavaNature(IProject project, IPluginModelBase model) {
        ...
        if (buildProperties.exists()) {
            ...
            if (build != null) {
                IBuildEntry buildEntry = //$NON-NLS-1$
