...
public class DirectoryBasedApiBaselineWizardPage extends ApiBaselineWizardPage {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (newPath != null && (!new Path(loctext).equals(new Path(newPath)) || getCurrentComponents().length == 0)) {
                    ...
                    if (nametext.getText().equals(new Path(loctext).lastSegment())) {
                        ...
                        new Path(updatedLocation).lastSegment() == //$NON-NLS-1$
                        null ? "" : new Path(updatedLocation).lastSegment());
                        nametext.setFocus();
                        nametext.selectAll();
                    }
    ...
    private String getUpdatedLocationForMacOS(String newPath) {
        ...
        System.getProperty("os.name").startsWith("Mac")) {
            ...
            if (//$NON-NLS-1$
            nPath.lastSegment().endsWith(".app")) {
                //$NON-NLS-1$ //$NON-NLS-2$
                IPath appendedPath = nPath.append("Contents").append("Eclipse");
    ...
    protected void doReload() {
        ...
        try {
            ...
            if (nametext.getText().isEmpty()) {
                nametext.setText(//$NON-NLS-1$
                Paths.get(location).getFileName() == null ? //$NON-NLS-1$
