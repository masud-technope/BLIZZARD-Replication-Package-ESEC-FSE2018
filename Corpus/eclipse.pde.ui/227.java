        ...
        try {
            ...
            for (int i = 0; i < scopeelements.length; i++) {
                try {
                    ...
                    if (ApiPlugin.DEBUG_SEARCH_ENGINE) {
                        //$NON-NLS-1$ //$NON-NLS-2$
                        System.out.println(Math.round((((float) (i + 1)) / scopeelements.length) * 100) + "% done in " + (System.currentTimeMillis() - loopstart) + " ms");
                    }
                } catch (CoreException ce) {
                    if (mstatus == null) {
            ...
            if (ApiPlugin.DEBUG_SEARCH_ENGINE) {
                //$NON-NLS-1$//$NON-NLS-2$
                System.out.println("Total Search Time: " + ((System.currentTimeMillis() - start) / 1000) + " seconds");
            }
            if (mstatus != null) {
