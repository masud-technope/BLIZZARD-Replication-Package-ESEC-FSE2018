...
public abstract class TypePackageCompletionProcessor implements IContentAssistProcessor {
    ...
    abstract class ProposalGenerator {
        ...
        protected abstract ICompletionProposal generateClassCompletion(String pName, String cName, boolean isClass);

        protected abstract ICompletionProposal generatePackageCompletion(String pName);
    }
        ...
        class TypePackageCompletionRequestor extends CompletionRequestor {
            ...
            public void accept(CompletionProposal proposal) {
                ...
                if (proposal.getKind() == CompletionProposal.PACKAGE_REF) {
                    ...
                    addProposalToCollection(c, startOffset, length, pkgName, pkgName, images.getImage(ISharedImages.IMG_OBJS_PACKAGE));
                } else {
                    boolean isInterface = Flags.isInterface(proposal.getFlags());
                    String completion = new String(proposal.getCompletion());
                    if (isInterface && typeScope == IJavaSearchConstants.CLASS || (!isInterface && typeScope == IJavaSearchConstants.INTERFACE) || //$NON-NLS-1$
                    completion.equals(//$NON-NLS-1$
                    "Dummy2"))
                    ...
                    Image image = isInterface ? images.getImage(ISharedImages.IMG_OBJS_INTERFACE) : images.getImage(ISharedImages.IMG_OBJS_CLASS);
                    addProposalToCollection(c, startOffset, length, cName + //$NON-NLS-1$
                    " - " + pName, completion, image);
                }
            }
        }
        try {
    ...
    private ICompilationUnit getWorkingCopy(IProject project) throws JavaModelException {
        ...
        if (roots.length > 0) {
            ...
            for (int i = 0; i < roots.length; i++) if (roots[i].getKind() == IPackageFragmentRoot.K_SOURCE || project.equals(roots[i].getCorrespondingResource()) || (roots[i].isArchive() && !roots[i].isExternal())) {
            ...
            if (frag != null)
                return //$NON-NLS-1$
                frag.getCompilationUnit("Dummy2.java").getWorkingCopy(//$NON-NLS-1$
        ...
        try {
            TypeNameRequestor req = new TypeNameRequestor() {
                ...
                public void acceptType(int modifiers, char[] packageName, char[] simpleTypeName, char[][] enclosingTypeNames, String path) {
                    ...
                    cName;
                    ISharedImages images = JavaUI.getSharedImages();
                    Image image = (Flags.isInterface(modifiers)) ? images.getImage(ISharedImages.IMG_OBJS_INTERFACE) : images.getImage(ISharedImages.IMG_OBJS_CLASS);
    ...
    protected final String removeLeadingSpaces(String value) {
        ...
        int i = 0;
        for (; i < valueArray.length; i++) if (!Character.isWhitespace(valueArray[i]))
            break;
        //$NON-NLS-1$
