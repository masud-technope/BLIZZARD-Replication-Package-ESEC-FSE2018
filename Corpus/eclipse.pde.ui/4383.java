...
public class PDEAuxiliaryState {
    ...
    private void createPluginInfo(Element element) {
        ...
        //$NON-NLS-1$
        info.hasBundleStructure = !"false".equals(element.getAttribute(ATTR_BUNDLE_STRUCTURE));
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (info.hasExtensibleAPI)
                    ...
                    ATTR_EXTENSIBLE_API, //$NON-NLS-1$
                    "true");
                if (info.isPatchFragment)
                    //$NON-NLS-1$
                    element.setAttribute(//$NON-NLS-1$
                    ATTR_PATCH, //$NON-NLS-1$
                    "true");
                if (!info.hasBundleStructure)
                    //$NON-NLS-1$
                    element.setAttribute(//$NON-NLS-1$
                    ATTR_BUNDLE_STRUCTURE, //$NON-NLS-1$
                    "false");
            ...
            for (int i = 0; i < models.length; i++) {
                ...
                for (int j = 0; j < libraries.length; j++) {
                    ...
                    if (!libraries[j].isExported())
                        //$NON-NLS-1$
                        lib.setAttribute(//$NON-NLS-1$
                        ATTR_EXPORTED, //$NON-NLS-1$
                        "false");
                    element.appendChild(lib);
                }
                root.appendChild(element);
    ...
    protected void addAuxiliaryData(BundleDescription desc, Map<String, String> manifest, boolean hasBundleStructure) {
        ...
        //$NON-NLS-1$
        info.hasExtensibleAPI = "true".equals(manifest.get(ICoreConstants.EXTENSIBLE_API));
        //$NON-NLS-1$
