...
public class ExternalizeStringsOperation extends WorkspaceModifyOperation {
        ...
        TextFileChange[] changes = PDEModelUtility.changesForModelModication(new ModelModification(buildPropsFile) {
            ...
            protected void modifyModel(IBaseModel model, IProgressMonitor monitor) throws CoreException {
                ...
                if (model instanceof IBuildModel) {
                    ...
                    binIncludes.addToken(//$NON-NLS-1$
                    localization + //$NON-NLS-1$
                    ".properties");
                }
            }
        }, monitor);
    ...
    private TextFileChange getChangeForFile(IFile file, CompositeChange parentChange) {
        ...
        String textType = //$NON-NLS-1$
        file.getName().equals(ICoreConstants.PLUGIN_FILENAME_DESCRIPTOR) || file.getName().equals(ICoreConstants.FRAGMENT_FILENAME_DESCRIPTOR) ? //$NON-NLS-1$
        "PLUGIN2" : file.getFileExtension();
    ...
    public static String getPropertiesFileComment(IFile file) {
        ...
        if (model != null) {
            ...
            if (pluginBase != null)
                return //$NON-NLS-1$
                NLS.bind(//$NON-NLS-1$
                "#Properties file for {0}", //$NON-NLS-1$
                pluginBase.getId());
        }
        //$NON-NLS-1$
        return NLS.bind("#{0}", file.getName());
    }
}
