    ...
    class ManifestNode {
        ...
        protected boolean hasApiVisibility(ManifestNode node) {
            if (ApiPlugin.DEBUG_API_DESCRIPTION) {
                ...
                "Checking node for API visibility:" + node);
            }
            if (node != null) {
    ...
    public IStatus setRestrictions(IElementDescriptor element, int restrictions) {
        ...
        if (node != null) {
            if (ApiPlugin.DEBUG_API_DESCRIPTION) {
                ...
                "Setting restrictions for manifest node: ");
                buffer.append(node);
                //$NON-NLS-1$
                buffer.append(//$NON-NLS-1$
                " to be ");
        ...
        //$NON-NLS-1$
        return new Status(IStatus.ERROR, ApiPlugin.PLUGIN_ID, ELEMENT_NOT_FOUND, MessageFormat.format("Failed to set API restriction: {0} not found in {1}", element.toString(), fOwningComponentId), null);
    }
    ...
    public IStatus setVisibility(IElementDescriptor element, int visibility) {
        ...
        if (node != null) {
            if (ApiPlugin.DEBUG_API_DESCRIPTION) {
                ...
                //$NON-NLS-1$
                buffer.append(//$NON-NLS-1$
                "Setting visibility for manifest node: ");
                buffer.append(node);
                //$NON-NLS-1$
                buffer.append(//$NON-NLS-1$
                " to be ");
        ...
        //$NON-NLS-1$
        return new Status(IStatus.ERROR, ApiPlugin.PLUGIN_ID, ELEMENT_NOT_FOUND, MessageFormat.format("Failed to set API visibility: {0} not found in {1}", element.toString(), fOwningComponentId), null);
    }
