...
public abstract class XMLInputContext extends UTF8InputContext {
    ...
    private void addElementContentOperation(IDocumentTextNode textNode, ArrayList<TextEdit> ops) {
        ...
        if (textNode.getOffset() > -1) {
            ...
            if (parent.getOffset() > -1) {
                ...
                //$NON-NLS-1$
                buffer.append(//$NON-NLS-1$
                "   " + getWritableTextNodeString(textNode));
    ...
    private DeleteEdit getDeleteNodeOperation(IDocumentElementNode node) {
        ...
        try {
            ...
            for (startOffset = offset - 1; startOffset >= startLineOffset; startOffset -= 1) if (!Character.isWhitespace(doc.getChar(startOffset)))
                break;
