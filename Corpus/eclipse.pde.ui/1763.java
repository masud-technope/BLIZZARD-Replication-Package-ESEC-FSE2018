...
public class PreferencesSection extends PDESection {
    ...
    protected void createClient(Section section, FormToolkit toolkit) {
        ...
        configText.addHyperlinkListener(new IHyperlinkListener() {
            ...
            public void linkActivated(HyperlinkEvent e) {
                ...
                if (//$NON-NLS-1$
                href.equals("command.generate")) {
                    handleGenerate();
                } else if (//$NON-NLS-1$
                href.equals("navigate.overview")) {
    ...
    void handleGenerate() {
        ...
        if (model == null || model.getUnderlyingResource() == null) {
            //$NON-NLS-1$
            MessageDialog.openError(getSection().getShell(), NLS.bind(PDEUIMessages.PreferencesSection_errorNoDefiningPluginTitle, id == null ? "" : id), PDEUIMessages.PreferencesSection_errorNoDefiningPlugin);
            return;
        }
            ...
            if (productExtension != null) {
                ...
                if (productElement != null) {
                    ...
                    if (propertyElement != null) {
                        IPluginAttribute valueAttribute = //$NON-NLS-1$
                        propertyElement.getAttribute(//$NON-NLS-1$
                        "value");
        ...
        try {
            ...
            if (buildProps.exists()) {
                ...
                if (wkspc.isLoaded()) {
                    ...
                    wkspc.getBuild().getEntry(//$NON-NLS-1$
                    "bin.includes");
                    if (entry == null) {
                        entry = //$NON-NLS-1$
                        wkspc.getFactory().createEntry(//$NON-NLS-1$
                        "bin.includes");
    ...
    private IPluginElement findPrefCustPropertyElement(IPluginElement productElement) {
        ...
        for (int i = 0; i < objects.length; i++) {
            ...
            if (//$NON-NLS-1$
            objects[i].getName().equals("property") == false) {
                continue;
            }
