...
public abstract class PDESourcePage extends TextEditor implements IFormPage, IGotoMarker, ISelectionChangedListener, IOutlineContentCreator, IOutlineSelectionHandler {
    ...
    protected void createActions() {
        super.createActions();
        //$NON-NLS-1$
        PDESelectAnnotationRulerAction action = new PDESelectAnnotationRulerAction(getBundleForConstructedKeys(), "PDESelectAnnotationRulerAction.", this, getVerticalRuler());
    ...
    private void createQuickOutlineAction() {
        ...
        TextOperationAction(//$NON-NLS-1$
        getBundleForConstructedKeys(), //$NON-NLS-1$
        "QuickOutline.", //$NON-NLS-1$
        this, PDEProjectionViewer.QUICK_OUTLINE, true);
    ...
    protected void updateOutlinePageSelection(Object rangeElement) {
        // feature is on
        if (//$NON-NLS-1$
        PDEPlugin.getDefault().getPreferenceStore().getBoolean("ToggleLinkWithEditorAction.isChecked")) {
    ...
    protected IDocumentRange findNode(Object[] nodes, int offset, boolean searchChildren) {
        for (int i = 0; i < nodes.length; i++) {
            ...
            if (node.getOffset() <= offset && offset < node.getOffset() + node.getLength()) {
                ...
                if (attrs != null)
                    for (int a = 0; a < attrs.length; a++) if (attrs[a].getNameOffset() <= offset && offset <= attrs[a].getValueOffset() + attrs[a].getValueLength())
                        return attrs[a];
                ...
                if (children != null)
                    for (int c = 0; c < children.length; c++) if (children[c].getOffset() <= offset && offset < children[c].getOffset() + children[c].getLength())
