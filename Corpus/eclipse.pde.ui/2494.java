...
public class RuntimeInfoSection extends PDESection implements IBuildPropertiesConstants {
    ...
    public class LibraryContentProvider implements IStructuredContentProvider {
        ...
        public Object[] getElements(Object parent) {
            if (parent instanceof IBuildModel) {
                ...
                for (int i = 0; i < libraries.length; i++) if (!libList.contains(libraries[i]))
    ...
    class JarsNewContentProvider extends WorkbenchContentProvider {
        ...
        public boolean hasChildren(Object element) {
            ...
            for (int i = 0; i < children.length; i++) if (children[i] instanceof IFolder)
                return true;
            return false;
        }
    }
    ...
    protected void handleLibInBinBuild(boolean isSelected, String libName) {
        ...
        try {
            ...
            if (libPath != null) {
                if (//$NON-NLS-1$
                !isSelected && libPath.segmentCount() == 1 && binIncl.contains("*.jar")) {
    ...
    protected void addAllJarsToBinIncludes(IBuildEntry binIncl, IProject project, IBuildModel model) {
        try {
            ...
            for (int i = 0; i < members.length; i++) if (!(members[i] instanceof IFolder) && //$NON-NLS-1$
            members[i].getFileExtension().equals(//$NON-NLS-1$
            "jar"))
    ...
    private void setOutputEntryTokens(Set<String> outputFolders, IBuildEntry outputEntry) {
        ...
        try {
            while (iter.hasNext()) {
                ...
                if (!//$NON-NLS-1$
                outputFolder.endsWith(//$NON-NLS-1$
                "" + IPath.SEPARATOR))
                    outputFolder = outputFolder.concat(//$NON-NLS-1$
                    "" + //$NON-NLS-1$
                    IPath.SEPARATOR);
    ...
    private IPackageFragmentRoot[] computeSourceFolders() {
        ...
        try {
            if (project.hasNature(JavaCore.NATURE_ID)) {
                ...
                for (int i = 0; i < roots.length; i++) if (roots[i].getKind() == IPackageFragmentRoot.K_SOURCE)
                    folders.add(roots[i]);
            }
    ...
    protected void entryModified(IBuildEntry oldEntry, String newValue) {
        ...
        try {
            ...
            if (//$NON-NLS-1$
            !newValue.endsWith(".jar") && !//$NON-NLS-1$
            newValue.endsWith(//$NON-NLS-1$
            "/") && !newValue.equals(//$NON-NLS-1$
            IBuildEntry.JAR_PREFIX + //$NON-NLS-1$
            "."))
                //$NON-NLS-1$
                newValue += //$NON-NLS-1$
                "/";
    ...
    protected String[] getLibraryNames() {
        ...
        for (int i = 0; i < libNames.length; i++) libNames[i] = fLibraryViewer.getTable().getItem(i).getText();
        return libNames;
    }
                ...
                try {
                    if (dialog.open() == Window.OK) {
                        ...
                        if (jarOrderEntry == null) {
                            ...
                            String[] tokens = jarOrderEntry.getTokens();
                            for (int i = 0; i < tokens.length; i++) jarOrderEntry.removeToken(tokens[i]);
                            for (int i = 0; i < numLib; i++) {
                                String lib = ((IBuildEntry) fLibraryViewer.getElementAt(i)).getName().substring(7);
                                jarOrderEntry.addToken(lib);
                            }
    ...
    private IPackageFragmentRoot getSourceFolder(String folderName, IPackageFragmentRoot[] sourceFolders) {
        for (int i = 0; i < sourceFolders.length; i++) if (sourceFolders[i].getPath().removeFirstSegments(1).equals(new Path(folderName)))
        ...
        if (index != -1) {
            ...
            try {
                ...
                if (entry == null) {
                    ...
                    for (int i = 0; i < tokens.length; i++) entry.removeToken(tokens[i]);
                    for (int i = 0; i < numLib; i++) {
    ...
    protected void updateJarsCompileOrder(boolean up) {
        ...
        try {
            ...
            for (int i = 0; i < numLib; i++) jarOrderEntry.addToken(names[i]);
        } catch (CoreException e) {
            PDEPlugin.logException(e);
        }
    }
