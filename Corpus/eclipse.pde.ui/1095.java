...
public abstract class ChangeAwareSourceViewerConfiguration extends TextSourceViewerConfiguration {
    ...
    public  ChangeAwareSourceViewerConfiguration(PDESourcePage page, IColorManager manager) {
        this(page, manager, new ChainedPreferenceStore(new IPreferenceStore[] { // general text editor store
        PDEPlugin.getDefault().getPreferenceStore(), // general text editor store
    ...
    public IInformationPresenter getInformationPresenter(ISourceViewer sourceViewer) {
        ...
        if (fInfoPresenter == null && getInfoImplementationType() != SourceInformationProvider.F_NO_IMP) {
            ...
            for (int i = 0; i < contentTypes.length; i++) fInfoPresenter.setInformationProvider(provider, contentTypes[i]);
    ...
    public IQuickAssistAssistant getQuickAssistAssistant(ISourceViewer sourceViewer) {
        if (sourceViewer.isEditable()) {
            if (fQuickAssistant == null) {
                ...
                //$NON-NLS-1$
                fQuickAssistant.setRestoreCompletionProposalSize(//$NON-NLS-1$
                PDEPlugin.getDefault().getDialogSettingsSection("quick_assist_proposal_size"));
            }
