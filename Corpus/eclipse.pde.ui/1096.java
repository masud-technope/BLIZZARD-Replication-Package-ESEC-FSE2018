...
public abstract class TextUtil {
    ...
    public static URL getJavaDocStyleSheerURL() {
        if (fJavaDocStyleSheet == null) {
            ...
            //$NON-NLS-1$
            fJavaDocStyleSheet = bundle.getEntry("/css/JavadocHoverStyleSheet.css");
            if (fJavaDocStyleSheet != null) {
                try {
    ...
    public static String trimNonAlphaChars(String value) {
        value = value.trim();
        while (value.length() > 0 && !Character.isLetter(value.charAt(0))) value = value.substring(1, value.length());
        //$NON-NLS-1$
