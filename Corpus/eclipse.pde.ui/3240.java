...
public class FeatureExportOperation extends Job {
    ...
    private static final String[] GENERIC_CONFIG = new String[] { "*", "*", "*", "" };
    ...
    public  FeatureExportOperation(FeatureExportInfo info, String name) {
        ...
        //$NON-NLS-1$
        fBuildTempMetadataLocation = PDECore.getDefault().getStateLocation().append("tempp2metadata").toString();
    }
    ...
    protected IStatus run(IProgressMonitor monitor) {
        try {
            ...
            //$NON-NLS-1$
            SubMonitor subMonitor = SubMonitor.convert(monitor, "Exporting...", (fInfo.items.length * 23) + 5 + 10);
    ...
    protected void appendMetadataToArchive(String[] configuration, IProgressMonitor monitor) {
        ...
        try {
            //$NON-NLS-1$
            scriptFile = createScriptFile("append.xml");
            ...
            //$NON-NLS-1$
            child.setAttribute("dir", fBuildTempMetadataLocation);
            ...
            //$NON-NLS-1$
            child.setAttribute("basedir", fBuildTempMetadataLocation);
            //$NON-NLS-1$ //$NON-NLS-2$
    ...
    protected void doExport(String featureID, String version, String featureLocation, String[][] configs, IProgressMonitor monitor) throws CoreException, InvocationTargetException {
        ...
        runScript(featureLocation + IPath.SEPARATOR + "compile." + featureID + ".xml", new String[] { "main" }, properties, subMonitor.split(1));
        // grab the source if needed
        if (fInfo.exportSource && !fInfo.exportSourceBundle)
            //$NON-NLS-1$
            runScript(getBuildScriptName(featureLocation), new String[] { "build.sources" }, properties, subMonitor.split(1));
        if (publishingP2Metadata()) {
            subMonitor.setTaskName(PDECoreMessages.FeatureExportOperation_publishingMetadata);
            //$NON-NLS-1$
            runScript(getAssembleP2ScriptName(featureID, featureLocation), new String[] { "main" }, properties, subMonitor.split(2));
        ...
        for (int i = 0; i < configs.length; i++) {
            ...
            runScript(//$NON-NLS-1$
            getAssemblyScriptName(featureID, configs[i][0], configs[i][1], configs[i][2], featureLocation), //$NON-NLS-1$
            new String[] { "main" }, properties, subMonitor.split(2));
        }
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        properties.put("destination.temp.folder", fBuildTempLocation + "/pde.logs");
        //$NON-NLS-1$
        runScript(getBuildScriptName(featureLocation), new String[] { "gather.logs" }, properties, subMonitor.split(2));
    }
            ...
            if (children != null) {
                for (int i = 0; i < children.length; i++) {
                    if (!children[i].isDirectory()) {
                        ...
                        filename.startsWith("javaCompiler.") && //$NON-NLS-1$ //$NON-NLS-2$
                        filename.endsWith(".args")) || (//$NON-NLS-1$ //$NON-NLS-2$
                        filename.startsWith("assemble.") && //$NON-NLS-1$ //$NON-NLS-2$
                        filename.endsWith(".xml")) || (//$NON-NLS-1$ //$NON-NLS-2$
                        filename.startsWith("package.") && //$NON-NLS-1$ //$NON-NLS-2$
                        filename.endsWith(".xml")) || (//$NON-NLS-1$ //$NON-NLS-2$
                        filename.startsWith("compile.") && //$NON-NLS-1$ //$NON-NLS-2$
                        filename.endsWith(".xml")) || filename.equals(FEATURE_POST_PROCESSING) || filename.equals(PLUGIN_POST_PROCESSING)) {
                            children[i].delete();
                        }
                    } else if (//$NON-NLS-1$
                    children[i].getName().equals(//$NON-NLS-1$
                    "temp.folder")) {
    ...
    private String getBuildScriptName(String featureLocation) {
        //$NON-NLS-1$
        return featureLocation + IPath.SEPARATOR + "build.xml";
    }

    protected String getAssemblyScriptName(String featureID, String os, String ws, String arch, String featureLocation) {
        return //$NON-NLS-1$
        featureLocation + IPath.SEPARATOR + "assemble." + featureID + "." + os + //$NON-NLS-1$ //$NON-NLS-2$
        "." + ws + //$NON-NLS-1$
        "." + //$NON-NLS-1$
        arch + //$NON-NLS-1$
        ".xml";
    ...
    protected String getAssembleP2ScriptName(String featureID, String featureLocation) {
        //$NON-NLS-1$ //$NON-NLS-2$
        return featureLocation + IPath.SEPARATOR + "assemble." + featureID + ".p2.xml";
    }
    ...
    protected String getPackagerScriptName(String featureID, String os, String ws, String arch, String featureLocation) {
        return //$NON-NLS-1$
        featureLocation + IPath.SEPARATOR + "package." + featureID + "." + os + //$NON-NLS-1$ //$NON-NLS-2$
        "." + ws + //$NON-NLS-1$
        "." + //$NON-NLS-1$
        arch + //$NON-NLS-1$
        ".xml";
	...
	*/
    ...
    private String getOS(IFeature feature) {
        ...
        if (//$NON-NLS-1$
        os == null || os.trim().length() == 0 || os.indexOf(',') != -1 || os.equals("*"))
    ...
    private String getWS(IFeature feature) {
        ...
        if (//$NON-NLS-1$
        ws == null || ws.trim().length() == 0 || ws.indexOf(',') != -1 || ws.equals("*"))
    ...
    private String getOSArch(IFeature feature) {
        ...
        if (//$NON-NLS-1$
        arch == null || arch.trim().length() == 0 || arch.indexOf(',') != -1 || arch.equals("*"))
    ...
    protected void setupGenerator(BuildScriptGenerator generator, String featureID, String versionId, String[][] configs, String featureLocation) throws CoreException {
        ...
        //This needs to be set before we set the format
        AbstractScriptGenerator.setConfigInfo(getConfigInfo(configs));
    ...
    private String getDevProperties() {
        if (fDevProperties == null) {
            //$NON-NLS-1$
            fDevProperties = ClasspathHelper.getDevEntriesProperties(fBuildTempLocation + "/dev.properties", false);
        }
    ...
    protected String[] getPaths() {
        // merge workspace and external features using workspace over external
    ...
    protected void cleanup(IProgressMonitor monitor) {
        ...
        try {
            ...
            Element child = doc.createElement("delete");
            //$NON-NLS-1$
            child.setAttribute("dir", fBuildTempLocation);
            target.appendChild(child);
            root.appendChild(target);
            if (hasAntErrors()) {
                target = //$NON-NLS-1$
                ...
                "zip");
                //$NON-NLS-1$
                child.setAttribute(//$NON-NLS-1$
                "zipfile", //$NON-NLS-1$
                fInfo.destinationDirectory + logName(null));
                //$NON-NLS-1$ //$NON-NLS-2$
                child.setAttribute("basedir", fBuildTempLocation + "/pde.logs");
                target.appendChild(child);
                root.appendChild(target);
            }
            XMLPrintHandler.writeFile(doc, scriptFile);
            String[] targets = //$NON-NLS-1$ //$NON-NLS-2$
            hasAntErrors() ? //$NON-NLS-1$ //$NON-NLS-2$
            new String[] { "zip.logs", "clean" } : new //$NON-NLS-1$
            String[] //$NON-NLS-1$
            { "clean" };
            AntRunner runner = new AntRunner();
            runner.setBuildFileLocation(scriptFile.getAbsolutePath());
        ...
        try {
            ...
            for (int i = 0; i < featuresExported.length; i++) {
                if (featuresExported[i] instanceof IFeatureModel) {
                    ...
                    Element includes = //$NON-NLS-1$
                    doc.createElement(//$NON-NLS-1$
                    "includes");
                    //$NON-NLS-1$ //$NON-NLS-2$
                    includes.setAttribute("id", feature.getId() + ".source");
                    includes.setAttribute("version", //$NON-NLS-1$
                    feature.getVersion());
                    ...
                    "includes");
                    //$NON-NLS-1$
                    includes.setAttribute(//$NON-NLS-1$
                    "id", //$NON-NLS-1$
                    feature.getId());
                    includes.setAttribute("version", //$NON-NLS-1$
                    feature.getVersion());
            ...
            if (includeLauncher) {
                ...
                if (model != null) {
                    ...
                    if (launcherPlugin != null) {
                        ...
                        if (bundle != null) {
                            Element plugin = //$NON-NLS-1$
                            doc.createElement(//$NON-NLS-1$
                            "plugin");
                            //$NON-NLS-1$
                            plugin.setAttribute(//$NON-NLS-1$
                            "id", //$NON-NLS-1$
                            bundle.getSymbolicName());
                            plugin.setAttribute("version", //$NON-NLS-1$
                            ...
                            for (int i = 0; i < configurations.length; i++) {
                                ...
                                if (launcherFragment != null) {
                                    Element //$NON-NLS-1$
                                    fragment = //$NON-NLS-1$
                                    doc.createElement("plugin");
                                    fragment.setAttribute("id", launcherFragment//$NON-NLS-1$
                                    .getSymbolicName());
                                    fragment.setAttribute("version", //$NON-NLS-1$
                                    launcherFragment.getVersion().toString());
                                    //$NON-NLS-1$ //$NON-NLS-2$
                                    fragment.setAttribute(//$NON-NLS-1$ //$NON-NLS-2$
                                    "fragment", //$NON-NLS-1$ //$NON-NLS-2$
                                    "true");
                ...
                if (fInfo.items[i] instanceof IFeatureModel) {
                    ...
                    for (Iterator<String[]> iterator = configs.iterator(); iterator.hasNext(); ) {
                        ...
                        if (shouldAddPlugin(bundle, environment)) {
                            ...
                            root.appendChild(plugin);
                            if (fInfo.exportSource && fInfo.exportSourceBundle) {
                                if (// Is it a workspace plugin?
                                workspacePlugins.contains(PluginRegistry.findModel(bundle))) {
                                    //$NON-NLS-1$
                                    plugin = //$NON-NLS-1$
                                    doc.createElement("plugin");
                                    plugin.setAttribute("id", //$NON-NLS-1$ //$NON-NLS-2$
                                ...
                                {
                                    IPluginModelBase model = PluginRegistry.findModel(//$NON-NLS-1$
                                    bundle.getSymbolicName() + ".source");
                                    if (model != null) {
