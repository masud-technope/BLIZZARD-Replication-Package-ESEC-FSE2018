...
public class TocDragAdapter implements DragSourceListener {
    ...
    public void dragStart(DragSourceEvent event) {
        if (//The event should only be enabled if there is a selection to drag
        event.doit) {
    ...
    public void dragSetData(DragSourceEvent event) {
        ...
        if (event.doit) {
            ...
            if (//If the expected data is text, then write out the selection
            TextTransfer.getInstance().isSupportedType(event.dataType)) {
                ...
                if (//disable the drag if there are no items selected
    ...
    private ArrayList getSelectedObjects(IStructuredSelection selection) {
        ...
        for (Iterator iter = selection.iterator(); iter.hasNext(); ) {
            Object obj = iter.next();
            if (//If the object is a removable TocObject, add it
            obj instanceof TocObject && ((TocObject) obj).canBeRemoved()) {
                objects.add(obj);
