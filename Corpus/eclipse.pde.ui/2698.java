...
public class WindowImages extends ProductObject implements IWindowImages {
    ...
    public void write(String indent, PrintWriter writer) {
        ...
        if (f16ImagePath != null && f16ImagePath.length() > 0) {
            ...
            writer.print(" " + P_16 + "=\"" + getWritableString(f16ImagePath) + "\"");
        }
        if (f32ImagePath != null && f32ImagePath.length() > 0) {
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            writer.print(" " + P_32 + "=\"" + getWritableString(f32ImagePath) + "\"");
        }
        if (f48ImagePath != null && f48ImagePath.length() > 0) {
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            writer.print(" " + P_48 + "=\"" + getWritableString(f48ImagePath) + "\"");
        }
        if (f64ImagePath != null && f64ImagePath.length() > 0) {
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            writer.print(" " + P_64 + "=\"" + getWritableString(f64ImagePath) + "\"");
        }
        if (f128ImagePath != null && f128ImagePath.length() > 0) {
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            writer.print(" " + P_128 + "=\"" + getWritableString(f128ImagePath) + "\"");
        }
        if (f256ImagePath != null && f256ImagePath.length() > 0) {
            //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            writer.print(" " + P_256 + "=\"" + getWritableString(f256ImagePath) + "\"");
        }
        //$NON-NLS-1$
        writer.println("/>");
    }
