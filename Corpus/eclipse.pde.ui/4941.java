...
public class P2Utils {
        ...
        if (osgiBundleList != null) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (index != -1) {
                    ...
                    index > 0 ? //$NON-NLS-1$
                    startData.substring(0, index) : //$NON-NLS-1$
                    "default";
                    String auto = //$NON-NLS-1$
                    index > 0 && index < startData.length() - 1 ? //$NON-NLS-1$
                    startData.substring(index + 1) : //$NON-NLS-1$
                    "default";
                    if (//$NON-NLS-1$
                    "start".equals(//$NON-NLS-1$
                    auto)) {
                        //$NON-NLS-1$
                        auto = "true";
                    }
        ...
        for (Iterator<?> iterator = bundles.keySet().iterator(); iterator.hasNext(); ) {
            ...
            if (installLocation != null) {
                ...
                if (base instanceof PluginBase && ((PluginBase) base).getBundleSourceEntry() != null) {
                    ...
                    index > 0 ? //$NON-NLS-1$
                    currentLevel.substring(0, index) : //$NON-NLS-1$
                    "default";
                    String auto = //$NON-NLS-1$
                    index > 0 && index < currentLevel.length() - 1 ? //$NON-NLS-1$
                    currentLevel.substring(index + 1) : //$NON-NLS-1$
                    "default";
                    boolean isAuto = true;
                    int level = -1;
                    if (//$NON-NLS-1$
                    "default".equals(//$NON-NLS-1$
                    auto)) {
                    ...
                    levelString)) {
                        ...
                        try {
                            ...
                            PDECore.log(new Status(IStatus.ERROR, PDECore.PLUGIN_ID, "Error writing bundles, could not parse start level for bundle " + currentModel));
                        }
                    }
                ...
                //$NON-NLS-1$
                PDECore.log(//$NON-NLS-1$
                new Status(IStatus.ERROR, PDECore.PLUGIN_ID, "Error writing bundles, could not find the bundle location for bundle " + currentModel));
            }
        }
    ...
    public static void createProfile(String profileID, File p2DataArea, Collection<?> bundles) throws CoreException {
        ...
        for (Iterator<IInstallableUnit> iter = ius.iterator(); iter.hasNext(); ) {
            ...
            //$NON-NLS-1$
            plan.setInstallableUnitProfileProperty(iu, "org.eclipse.equinox.p2.internal.inclusion.rules", ProfileInclusionRules.createOptionalInclusionRule(iu));
        }
    ...
    private static IInstallableUnit createBundleIU(BundleDescription bd) {
        ...
        for (int j = 0; j < requiredBundles.length; j++) reqsDeps.add(MetadataFactory.createRequirement(CAPABILITY_NS_OSGI_BUNDLE, requiredBundles[j].getName(), fromOSGiVersionRange(requiredBundles[j].getVersionRange()), null, requiredBundles[j].isOptional(), false));
