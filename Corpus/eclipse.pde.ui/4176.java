...
public class FilterProblemWithCommentResolution extends FilterProblemResolution {
    ...
    public String getLabel() {
        if (plural) {
            ...
            if (element != null) {
                return MessageFormat.format(MarkerMessages.FilterProblemWithCommentResolution_create_commented_filter, JavaElementLabels.getTextLabel(element, JavaElementLabels.M_PARAMETER_TYPES), resolveCategoryName());
            } else {
                IResource res = fBackingMarker.getResource();
                return MessageFormat.format(MarkerMessages.FilterProblemWithCommentResolution_create_commented_filter, res.getFullPath().removeFileExtension().lastSegment(), resolveCategoryName());
            }
        }
    }
