...
public class OrganizeManifestsProcessor extends RefactoringProcessor implements IOrganizeManifestsSettings {
    ...
    // dependencies - long
    // running op
    // remove lazy/auto start if no
    protected boolean fRemoveLazy = true;

    // activator
    // remove fragment/plugin.xml
    ...
    private CompositeChange cleanProject(IProject project, IProgressMonitor monitor) {
        ...
        final SubMonitor subMonitor = SubMonitor.convert(monitor, NLS.bind(PDEUIMessages.OrganizeManifestsProcessor_rootMessage, new String[] { fCurrentProject.getName() }), 1);
        ...
        for (int i = 0; i < changes.length; i++) change.add(changes[i]);
        if (result[0] != null)
            change.add(result[0]);
        if (result[1] != null)
            change.add(result[1]);
    ...
    private void runCleanup(IProgressMonitor monitor, IBundlePluginModelBase modelBase, Change[] result) throws InvocationTargetException, InterruptedException {
        ...
        SubMonitor subMonitor = SubMonitor.convert(monitor, NLS.bind(PDEUIMessages.OrganizeManifestsOperation_export, projectName), 20);
        ...
        if (fRemoveUselessFiles) {
            subMonitor.subTask(NLS.bind(PDEUIMessages.OrganizeManifestsOperation_uselessPluginFile, fCurrentProject.getName()));
            if (!subMonitor.isCanceled()) {
