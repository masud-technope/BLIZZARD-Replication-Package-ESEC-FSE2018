...
public class RequiresSection extends TableSection implements IPluginModelListener, IPropertyChangeListener {
    ...
    protected void registerPopupMenu(MenuManager popupMenuManager) {
        ...
        //$NON-NLS-1$
        site.registerContextMenu(site.getId() + ".requires", popupMenuManager, fViewerPart.getViewer(), false);
    }
    ...
    private void handleRemove() {
        ...
        if (!ssel.isEmpty()) {
            ...
            int i = 0;
            for (Iterator<?> iter = ssel.iterator(); iter.hasNext(); i++) imports[i] = ((ImportObject) iter.next()).getImport();
            try {
    ...
    private IPluginModelBase[] getAvailablePlugins(IPluginModelBase model) {
        ...
        if (//$NON-NLS-1$
        !existingImports.contains("system.bundle"))
    ...
    private void addSystemBundle(java.util.List<IPluginModelBase> list) {
        try {
            ...
            //$NON-NLS-1$
            IPluginModelBase osgi = PluginRegistry.findModel("system.bundle");
            if (osgi == null)
                return;
