...
public class APIFreezeTask extends CommonUtilsTask {
    ...
    public void execute() throws BuildException {
        ...
        if (//$NON-NLS-1$
        index == -1 || !this.reportLocation.substring(index).toLowerCase().equals(".xml")) {
    ...
    private IApiScope getScope(IApiBaseline currentBaseline) {
        ...
        for (IApiComponent apiComponent : apiComponents) {
            try {
                ...
                if (errors != null) {
                    if (this.debug) {
                        System.out.println("Resolver errors found for component : " + //$NON-NLS-1$
                        apiComponent.getSymbolicName());
