                    ...
                    for (int i = 0; i < projects.length; i++) {
                        ...
                        if (projectToBuild.hasNature(PDE.PLUGIN_NATURE)) {
                            if (fBuilders.contains(PDE.MANIFEST_BUILDER_ID)) {
                                projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, PDE.MANIFEST_BUILDER_ID, null, iterationMonitor.split(1));
                            }
                            if (fBuilders.contains(PDE.SCHEMA_BUILDER_ID)) {
                                projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, PDE.SCHEMA_BUILDER_ID, null, iterationMonitor.split(1));
                            }
                        } else if (projectToBuild.hasNature(PDE.FEATURE_NATURE)) {
                            if (fBuilders.contains(PDE.FEATURE_BUILDER_ID)) {
                                projectToBuild.build(IncrementalProjectBuilder.FULL_BUILD, PDE.FEATURE_BUILDER_ID, null, iterationMonitor.split(2));
                            }
                        }
                    }
