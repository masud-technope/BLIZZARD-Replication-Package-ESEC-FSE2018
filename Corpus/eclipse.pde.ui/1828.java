...
public class DSErrorReporter extends XMLErrorReporter {
    ...
    private void reportIllegalAttributeValue(Element element, Attr attr) {
        if (attr == null || attr.getValue() == null || attr.getName() == null)
            return;
        String message = NLS.bind(Messages.DSErrorReporter_attrValue, attr.getValue(), attr.getName());
        report(message, getLine(element, attr.getName()), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validateReferenceElements(IDSReference[] references) {
        ...
        for (int i = 0; i < references.length; i++) {
            ...
            if (reference.getReferenceInterface() == null) {
                reportMissingRequiredAttribute(element, IDSConstants.ATTRIBUTE_REFERENCE_INTERFACE, ERROR);
            } else {
                // Validate Resource Existence
                validateJavaElement(reference.getReferenceInterface(), IDSConstants.ELEMENT_REFERENCE, IDSConstants.ATTRIBUTE_REFERENCE_INTERFACE, i);
            }
    ...
    private void validateTargetAttribute(Element element) {
        Attr attr = element.getAttributeNode(IDSConstants.ATTRIBUTE_REFERENCE_TARGET);
        if (attr != null) {
            String value = attr.getValue();
            try {
    ...
    private void reportInvalidTarget(Element element, String target) {
        ...
        String message = NLS.bind(Messages.DSErrorReporter_invalidTarget, name, target);
        report(message, getLine(element), ERROR, DSMarkerFactory.CAT_OTHER);
    }

    private void validateReferenceElementNames(Hashtable<String, String> referencedNames, Element element) {
        String name = element.getAttribute(IDSConstants.ATTRIBUTE_REFERENCE_NAME);
    ...
    private void reportDuplicateReferenceElementName(Element element, String name) {
        Attr attr = element.getAttributeNode(IDSConstants.ATTRIBUTE_REFERENCE_NAME);
        if (attr == null || attr.getValue() == null || attr.getName() == null)
            return;
        String message = NLS.bind(Messages.DSErrorReporter_duplicateReferenceName, name);
        report(message, getLine(element, IDSConstants.ATTRIBUTE_REFERENCE_NAME), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validateReferencePolicy(Element element) {
        ...
        String allowedValues[] = new String[] { IDSConstants.VALUE_REFERENCE_POLICY_DYNAMIC, IDSConstants.VALUE_REFERENCE_POLICY_STATIC };
    ...
    private void reportIllegalPolicy(Element element, String policy) {
        String name = element.getAttribute(IDSConstants.ATTRIBUTE_REFERENCE_NAME);
        if (name == null)
            name = element.getAttribute(IDSConstants.ATTRIBUTE_REFERENCE_INTERFACE);
        String message = NLS.bind(Messages.DSErrorReporter_invalidPolicyValue, name, policy);
        Attr attr = element.getAttributeNode(IDSConstants.ATTRIBUTE_REFERENCE_POLICY);
        report(message, getLine(element, attr.getName()), ERROR, DSMarkerFactory.CAT_OTHER);
    }

    private void validateReferenceCardinality(Element element) {
        String cardinality = element.getAttribute(IDSConstants.ATTRIBUTE_REFERENCE_CARDINALITY);
        String allowedValues[] = new String[] { IDSConstants.VALUE_REFERENCE_CARDINALITY_ONE_N, IDSConstants.VALUE_REFERENCE_CARDINALITY_ONE_ONE, IDSConstants.VALUE_REFERENCE_CARDINALITY_ZERO_N, IDSConstants.VALUE_REFERENCE_CARDINALITY_ZERO_ONE };
        if (cardinality != null) {
            for (String allowedValue : allowedValues) {
    ...
    private void reportIllegalCardinality(Element element, String cardinality) {
        ...
        String message = NLS.bind(Messages.DSErrorReporter_invalidCardinalityValue, name, cardinality);
        Attr attr = element.getAttributeNode(IDSConstants.ATTRIBUTE_REFERENCE_CARDINALITY);
        if (attr == null || attr.getValue() == null || attr.getName() == null)
            return;
        report(message, getLine(element, attr.getName()), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validatePropertiesElements(IDSProperties[] propertiesElements) {
        for (int i = 0; i < propertiesElements.length; i++) {
            ...
            if (properties.getEntry() == null || properties.getEntry().length() == 0) {
                reportMissingRequiredAttribute(element, IDSConstants.ATTRIBUTE_PROPERTIES_ENTRY, ERROR);
            } else {
                if (!fProject.exists(new Path((properties.getEntry())))) {
                    report(NLS.bind(Messages.DSErrorReporter_cannotFindProperties, properties.getEntry()), getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
                }
            }
        }
    }
    ...
    private void validatePropertyElements(IDSProperty[] propertyElements) {
        for (int i = 0; i < propertyElements.length; i++) {
            ...
            if (name == null || name.length() == 0) {
                reportMissingRequiredAttribute(element, IDSConstants.ATTRIBUTE_PROPERTY_NAME, ERROR);
            }
            // Validate Allowed Values
    ...
    private void validatePropertySpecificTypeValue(String type, String value, Element element) {
        if (// if null, we assume string
        type == null) {
            type = IDSConstants.VALUE_PROPERTY_TYPE_STRING;
        }
        // Validate Double, Long, Float, Integer, Byte, Short and String
        if (!type.equals(IDSConstants.VALUE_PROPERTY_TYPE_CHAR) && !type.equals(IDSConstants.VALUE_PROPERTY_TYPE_BOOLEAN)) {
            try {
                ...
                for (Constructor constructor : constructors) {
                    ...
                    if (parameterTypes.length == 1) {
                        if (parameterTypes[0].equals(Class.forName("java.lang.String"))) {
            ...
            if (type.equals(IDSConstants.VALUE_PROPERTY_TYPE_BOOLEAN)) {
                if (!value.equals(IDSConstants.VALUE_FALSE) && !value.equals(IDSConstants.VALUE_TRUE)) {
                    reportPropertyTypeCastException(element, value, type);
                }
            } else {
                // Validate Chars
    ...
    private void reportPropertyTypeCastException(Element element, String value, String type) {
        String message = NLS.bind(Messages.DSErrorReporter_propertyTypeCastException, new String[] { value, type });
    ...
    private void validatePropertyAttrValueAndBody(Element element, IDSProperty property) {
        if (property.getPropertyValue() != null) {
            if (property.getPropertyElemBody() != null && !//$NON-NLS-1$
            property.getPropertyElemBody().equals(//$NON-NLS-1$
            "")) {
                String propertyName = property.getPropertyName();
                reportSingleAndMultiplePropertyValues(element, propertyName, property.getPropertyValue());
            }
            String propertyType = property.getPropertyType();
            if (propertyType == null || propertyType.equals(IDSConstants.VALUE_PROPERTY_TYPE_STRING))
                // It's OK for a property of type "String" to have a
                return;
            // value of "".
            if (//$NON-NLS-1$
    ...
    private void reportEmptyPropertyValue(Element element, String propertyName) {
        String message = NLS.bind(Messages.DSErrorReporter_emptyPropertyValue, propertyName);
        report(message, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
    }

    private void reportSingleAndMultiplePropertyValues(Element element, String propertyName, String value) {
        String message = NLS.bind(Messages.DSErrorReporter_singleAndMultipleAttrValue, propertyName, value);
        report(message, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validateImplementationElement(IDSImplementation implementation) {
        if (implementation != null) {
            ...
            if (className == null) {
                // Validate Required Attributes
                reportMissingRequiredAttribute(element, IDSConstants.ATTRIBUTE_IMPLEMENTATION_CLASS, ERROR);
            } else {
                // validate Resource Existence
    ...
    private void reportMissingRequiredAttribute(Element element, String attName, int severity) {
        String message = NLS.bind(Messages.DSErrorReporter_requiredAttribute, attName, element.getNodeName());
    ...
    private void reportJavaTypeNotFound(String elementConstant, String attributeConstant, String resource, int index) {
        ...
        report(NLS.bind(Messages.DSErrorReporter_cannotFindJavaType, resource, attributeConstant), getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validateComponentElement(IDSComponent component) {
        if (component != null) {
            ...
            if (component.getImplementation() == null) {
                report(NLS.bind(Messages.DSErrorReporter_requiredElement, IDSConstants.ELEMENT_IMPLEMENTATION), getLine(getDocumentRoot()), ERROR, DSMarkerFactory.CAT_OTHER);
            }
            // validate boolean values
            validateBoolean(element, element.getAttributeNode(IDSConstants.ATTRIBUTE_COMPONENT_IMMEDIATE));
            validateBoolean(element, element.getAttributeNode(IDSConstants.ATTRIBUTE_COMPONENT_ENABLED));
            // validate non-empty values
            validateEmpty(element, element.getAttributeNode(IDSConstants.ATTRIBUTE_COMPONENT_FACTORY));
            validateEmpty(element, element.getAttributeNode(IDSConstants.ATTRIBUTE_COMPONENT_NAME));
    ...
    private void validateConfigurationPolicyAttribute(Element element, IDSComponent component) {
        ...
        if (policy != null && policy.length() > 0) {
            if (policy.equalsIgnoreCase(IDSConstants.VALUE_CONFIGURATION_POLICY_IGNORE)) {
                if (modified != null && modified.length() > 0) {
                    report(Messages.DSErrorReporter_invalidConfigurationPolicyValue, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
                }
            }
        }
    }
    ...
    private void validateImmediateAttribute(Element element, IDSComponent component) {
        ...
        if (component.getService() != null) {
            IDSProvide[] providedServices = component.getService().getProvidedServices();
            if (providedServices != null && providedServices.length > 0) {
                isService = true;
            }
        }
        if (!isService && !isFactory && !isImmediate && component.getXMLAttributeValue(IDSConstants.ATTRIBUTE_COMPONENT_IMMEDIATE) != null) {
    ...
    private void reportInvalidImmediateFactory(Element element) {
        report(Messages.DSErrorReporter_invalidImmediateValueFactory, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
    }

    private void reportInvalidImmediate(Element element) {
        report(Messages.DSErrorReporter_invalidImmediateValue, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void reportIllegalEmptyAttributeValue(Element element, Attr attr) {
        if (attr == null || attr.getValue() == null || attr.getName() == null)
            return;
        String message = NLS.bind(Messages.DSErrorReporter_emptyAttrValue, attr.getName());
        report(message, getLine(element, attr.getName()), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void reportEmptyService(Element element) {
        report(Messages.DSErrorReporter_illegalEmptyService, getLine(element), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void reportIllegalServiceFactory_Immediate(Element element) {
        report(Messages.DSErrorReporter_illegalServiceFactory_Immediate, getLine(element), ERROR, DSMarkerFactory.CAT_OTHER);
    }

    private void reportIllegalServiceFactory(Element element) {
        report(Messages.DSErrorReporter_illegalServiceFactory, getLine(element), ERROR, DSMarkerFactory.CAT_OTHER);
    }
    ...
    private void validateProvideElement(IDSProvide[] providedServices) {
        ...
        for (int i = 0; i < providedServices.length; i++) {
            ...
            if (provide.getInterface() == null) {
                reportMissingRequiredAttribute(element, IDSConstants.ATTRIBUTE_PROVIDE_INTERFACE, ERROR);
            } else {
                validateJavaElement(provide.getInterface(), IDSConstants.ELEMENT_PROVIDE, IDSConstants.ATTRIBUTE_PROVIDE_INTERFACE, i);
    ...
    private void validateDuplicateInterface(Hashtable<String, String> providedInterfaces, IDSProvide provide, Element element) {
        String interface1 = provide.getInterface();
        if (providedInterfaces.get(interface1) != null) {
            String message = NLS.bind(Messages.DSErrorReporter_duplicatedInterface, interface1);
            report(message, getLine(element), WARNING, DSMarkerFactory.CAT_OTHER);
        } else {
