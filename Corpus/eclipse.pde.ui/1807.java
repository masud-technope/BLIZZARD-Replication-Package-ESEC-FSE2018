...
public class P2TargetUtils {
	 ...
	 */
    //$NON-NLS-1$
    static final String PROP_INSTALLED_IU = PDECore.PLUGIN_ID + ".installed_iu";
	 ...
	 */
    //$NON-NLS-1$
    static final String PROP_PROVISION_MODE = PDECore.PLUGIN_ID + ".provision_mode";
	 ...
	 */
    //$NON-NLS-1$
    static final String PROP_ALL_ENVIRONMENTS = PDECore.PLUGIN_ID + ".all_environments";

    /**
	 * Profile property that keeps track of the target sequence number
	 */
    //$NON-NLS-1$
    static final String PROP_SEQUENCE_NUMBER = PDECore.PLUGIN_ID + ".sequence";

    /**
	 * Profile property that tracks whether or not source to be auto-included
	 */
    //$NON-NLS-1$
    static final String PROP_AUTO_INCLUDE_SOURCE = PDECore.PLUGIN_ID + ".autoIncludeSource";

    /**
	 * Profile property that tracks whether or not the configuration phase should be executed when installing
	 */
    //$NON-NLS-1$
    static final String PROP_INCLUDE_CONFIGURE_PHASE = PDECore.PLUGIN_ID + ".includeConfigure";
    ...
    private IInstallableUnit createSourceIU(IQueryable<IInstallableUnit> queryable, Version iuVersion) {
        ...
        for (Iterator<IInstallableUnit> i = profileIUs.iterator(); i.hasNext(); ) {
            ...
            if (profileIU.satisfies(bundleRequirement)) {
                ...
                IRequirement sourceRequirement = //$NON-NLS-1$
                MetadataFactory.createRequirement(//$NON-NLS-1$
                "osgi.bundle", //$NON-NLS-1$
                id, //$NON-NLS-1$
                range, //$NON-NLS-1$
                null, //$NON-NLS-1$
                true, //$NON-NLS-1$
    ...
    private void findWorkspaceRepos(Set<URI> additionalRepos) {
        ...
        for (int i = 0; i < recents.length; i++) {
            //$NON-NLS-1$
            File bundlePool = new File(recents[i] + "/.metadata/.plugins/org.eclipse.pde.core/.bundle_pool");
    ...
    protected static class CollectNativesAction extends ProvisioningAction {
        ...
        public IStatus execute(Map<String, Object> parameters) {
            ...
            try {
                ...
                if (toDownload == null)
                    return Status.OK_STATUS;
                @SuppressWarnings("unchecked") List<IArtifactRequest> artifactRequests = (List<IArtifactRequest>) parameters.get(NATIVE_ARTIFACTS);
