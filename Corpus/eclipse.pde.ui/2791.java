...
public class AddNewDependenciesOperation extends WorkspaceModifyOperation {
    ...
    protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
        ...
        findSecondaryDependencies(secDeps, ignorePkgs, additionalDeps, bundle, useRequireBundle, subMonitor.split(80));
    ...
    protected String[] findSecondaryBundles(IBundle bundle, Set<String> ignorePkgs) {
        ...
        for (int i = 0; i < secDeps.length; i++) if (!manifestPlugins.contains(secDeps[i]))
    ...
    private Set<String> findManifestPlugins(RequireBundleHeader header, Set<String> ignorePkgs) {
        ...
        for (int i = 0; i < bundles.length; i++) {
            ...
            if (base != null) {
                ...
                for (int j = 0; j < exportedPkgs.length; j++) ignorePkgs.add(exportedPkgs[j].getName());
    ...
    protected final ExportPackageDescription[] findExportedPackages(BundleDescription desc) {
        if (desc != null) {
            ...
            while (!stack.isEmpty()) {
                ...
                for (int i = 0; i < expkgs.length; i++) if (addPackage(projectBundleId, expkgs[i]))
                    result.add(expkgs[i]);
    ...
    protected final void findImportPackages(IBundle bundle, Set<String> ignorePkgs) {
        ...
        if (header instanceof ImportPackageHeader) {
            ...
            for (int i = 0; i < pkgs.length; i++) ignorePkgs.add(pkgs[i].getName());
        } else {
    ...
    protected void findSecondaryDependencies(String[] secDeps, Set<String> ignorePkgs, Map<ExportPackageDescription, String> newDeps, IBundle bundle, boolean useRequireBundle, IProgressMonitor monitor) {
        ...
        SubMonitor subMonitor = SubMonitor.convert(monitor, PDEUIMessages.AddNewDependenciesOperation_searchProject, secDeps.length);
                        ...
                        if (!ignorePkgs.contains(pkgName)) {
                            ...
                            if (requestor.foundMatches()) {
                                ...
                                if (useRequireBundle) {
                                    ...
                                    for (; i < exported.length; i++) ignorePkgs.add(exported[i].getName());
                                }
                            }
                        }
            ...
            for (int i = 0; i < elems.length; i++) {
                ...
                if (binIncludes.contains(library)) {
                    ...
                    if (entry != null) {
                        ...
                        for (int j = 0; j < resources.length; j++) addPackagesFromResource(jProject, fProject.findMember(resources[j]), ignorePkgs);
                    } else {
    ...
    protected void addDependencies(final Map<ExportPackageDescription, String> depsToAdd, boolean useRequireBundle) {
        if (useRequireBundle) {
            ...
            try {
                //$NON-NLS-1$
                build.write(//$NON-NLS-1$
                "", //$NON-NLS-1$
                new PrintWriter(new FileOutputStream(PDEProject.getBuildProperties(fProject).getFullPath().toFile())));
    ...
    protected final void addImportPackages(final Collection<ExportPackageDescription> depsToAdd, final IBundle bundle) {
        ...
        if (mheader instanceof ImportPackageHeader) {
            ...
            while (it.hasNext()) {
                ...
                //$NON-NLS-1$ //$NON-NLS-2$
                String value = (desc.getVersion().equals(Version.emptyVersion)) ? desc.getName() : desc.getName() + "; version=\"" + desc.getVersion() + "\"";
