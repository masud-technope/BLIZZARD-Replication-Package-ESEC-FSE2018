...
public class EclipseLaunchShortcut extends AbstractLaunchShortcut {
    ...
    private String[] getAvailableApplications() {
        ...
        for (int i = 0; i < extensions.length; i++) {
            ...
            if (//$NON-NLS-1$
            "org.eclipse.core.runtime.applications".equals(extension.getPoint())) {
            ...
            for (int i = 0; i < extensions.length; i++) {
                ...
                "org.eclipse.core.runtime.products".equals(point)) {
                    if (ext.getChildCount() == 1) {
                        ...
                        if (//$NON-NLS-1$
                        prod.getName().equals(//$NON-NLS-1$
                        "product")) {
                            IPluginAttribute attr = //$NON-NLS-1$
                            prod.getAttribute(//$NON-NLS-1$
                            "application");
    ...
    protected void initializeConfiguration(ILaunchConfigurationWorkingCopy wc) {
        if (TargetPlatformHelper.usesNewApplicationModel())
            ...
            //$NON-NLS-1$
            wc.setAttribute(IPDEConstants.LAUNCHER_PDE_VERSION, "3.2a");
