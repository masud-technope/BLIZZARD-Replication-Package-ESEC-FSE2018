        ...
        if (header instanceof RequireBundleHeader) {
            ...
            if (iimport.isOptional())
                if (bundleManifestVersion > 1)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    buffer.append(";" + Constants.RESOLUTION_DIRECTIVE + ":=" + Constants.RESOLUTION_OPTIONAL);
                else
                    //$NON-NLS-1$ //$NON-NLS-2$
                    buffer.append(";" + ICoreConstants.OPTIONAL_ATTRIBUTE + "=true");
            if (iimport.isReexported())
                if (bundleManifestVersion > 1)
                    //$NON-NLS-1$ //$NON-NLS-2$
                    buffer.append(";" + Constants.VISIBILITY_DIRECTIVE + ":=" + Constants.VISIBILITY_REEXPORT);
                else
                    //$NON-NLS-1$ //$NON-NLS-2$
                    buffer.append(";" + ICoreConstants.REPROVIDE_ATTRIBUTE + "=true");
        ...
        if (header instanceof BundleSymbolicNameHeader)
            ...
            if (singleton) {
                ...
                if (value.indexOf(singletonValue) != -1)
                    return;
                //$NON-NLS-1$
                getBundle().setHeader(//$NON-NLS-1$
                Constants.BUNDLE_SYMBOLICNAME, //$NON-NLS-1$
