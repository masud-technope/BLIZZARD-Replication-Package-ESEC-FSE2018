...
public class ManifestContentAssistProcessor extends TypePackageCompletionProcessor implements ICompletionListener {
    ...
    protected static final // header proposal
    short // header proposal
    F_TYPE_HEADER = 0, // package proposal
    F_TYPE_PKG = 1, // bundle proposal
    F_TYPE_BUNDLE = 2, // class proposal
    F_TYPE_CLASS = 3, // directive proposal
    F_TYPE_DIRECTIVE = 4, // attribute proposal
    F_TYPE_ATTRIBUTE = 5, // value of attribute or directive proposal
    F_TYPE_VALUE = 6, // value of execution env., added since we use a unique icon for exec envs.
    F_TYPE_EXEC_ENV = 7, F_TOTAL_TYPES = 8;
    ...
    static {
        ...
        for (int i = 0; i < envs.length; i++) fExecEnvs[i] = envs[i].getId();
                ...
                if (containsNewHeader(doc.get(nextLine.getOffset(), nextLine.getLength())) || i == (numLines - 1)) {
                    ...
                    try {
                        ...
                        if (shouldStoreSet(header)) {
                            ...
                            for (ManifestElement elem : elems) set.add(elem.getValue());
    ...
    protected ICompletionProposal[] computeHeader(String currentValue, int startOffset, int offset) {
        ...
        for (String element : fHeader) {
            if (element.regionMatches(true, 0, currentValue, 0, currentValue.length()) && fHeaders.get(element) == null) {
                TypeCompletionProposal proposal = new TypeCompletionProposal(element + ": ", getImage(//$NON-NLS-1$
                F_TYPE_HEADER), element, startOffset, currentValue.length());
    ...
    protected ICompletionProposal[] handleImportPackageCompletion(String currentValue, int offset) {
        ...
        if (comma > semicolon || comma == semicolon) {
            @SuppressWarnings("unchecked") HashSet<String> set = (HashSet<String>) fHeaders.get(Constants.IMPORT_PACKAGE);
            if (set == null)
                set = parseHeaderForValues(currentValue, offset);
            @SuppressWarnings("unchecked") HashSet<String> importedBundles = (HashSet<String>) fHeaders.get(Constants.REQUIRE_BUNDLE);
            ...
            for (// Remove any packages already imported through Require-Bundle
            IPluginModelBase base : // Remove any packages already imported through Require-Bundle
            bases) {
        ...
        if (Constants.RESOLUTION_DIRECTIVE.regionMatches(true, 0, attributeValue, 0, Constants.RESOLUTION_DIRECTIVE.length()))
            //$NON-NLS-1$
            return matchValueCompletion(currentValue.substring(equals + 1), new String[] { Constants.RESOLUTION_MANDATORY, Constants.RESOLUTION_OPTIONAL }, new int[] { F_TYPE_VALUE, F_TYPE_VALUE }, offset, "RESOLUTION_");
    ...
    protected ICompletionProposal[] handleRequireBundleCompletion(String currentValue, int offset) {
        ...
        String value = comma != -1 ? currentValue.substring(comma + 1) : currentValue;
        if (comma > semicolon || comma == semicolon) {
            @SuppressWarnings("unchecked") HashSet<String> set = (HashSet<String>) fHeaders.get(Constants.REQUIRE_BUNDLE);
            if (set == null)
                set = parseHeaderForValues(currentValue, offset);
        ...
        if (Constants.VISIBILITY_DIRECTIVE.regionMatches(true, 0, attributeValue, 0, Constants.VISIBILITY_DIRECTIVE.length()))
            //$NON-NLS-1$
            return matchValueCompletion(currentValue.substring(equals + 1), new String[] { Constants.VISIBILITY_PRIVATE, Constants.VISIBILITY_REEXPORT }, new int[] { F_TYPE_VALUE, F_TYPE_VALUE }, offset, "VISIBILITY_");
        if (Constants.RESOLUTION_DIRECTIVE.regionMatches(true, 0, attributeValue, 0, Constants.RESOLUTION_DIRECTIVE.length()))
            //$NON-NLS-1$
            return matchValueCompletion(currentValue.substring(equals + 1), new String[] { Constants.RESOLUTION_MANDATORY, Constants.RESOLUTION_OPTIONAL }, new int[] { F_TYPE_VALUE, F_TYPE_VALUE }, offset, "RESOLUTION_");
    ...
    protected ICompletionProposal[] handleExportPackageCompletion(String currentValue, int offset) {
        ...
        if (!insideQuotes(currentValue) && comma > semicolon || comma == semicolon) {
            String value = comma != -1 ? currentValue.substring(comma + 1) : currentValue;
            @SuppressWarnings("unchecked") HashSet<String> set = (HashSet<String>) fHeaders.get(Constants.EXPORT_PACKAGE);
            if (set == null)
                set = parseHeaderForValues(currentValue, offset);
            value = removeLeadingSpaces(value);
        ...
        if (semicolon != -1) {
            ...
            if (equals == -1) {
                ...
                if (Constants.SINGLETON_DIRECTIVE.regionMatches(true, 0, attribute, 0, attribute.length())) {
                    ...
                    TypeCompletionProposal proposal = new TypeCompletionProposal(//$NON-NLS-1$
                    Constants.SINGLETON_DIRECTIVE + //$NON-NLS-1$
                    ":=", getImage(type), Constants.SINGLETON_DIRECTIVE, offset - length, length);
                    proposal.setAdditionalProposalInfo(getJavaDoc("SINGLETON_DIRECTIVE"));
    ...
    protected ICompletionProposal[] handleBundleActivationPolicyCompletion(final String currentValue, final int offset) {
        ...
        if (semicolon == -1) {
            ...
            if (!insideQuotes(currentValue)) {
                ...
                StringTokenizer parser = new //$NON-NLS-1$
    ...
    protected ICompletionProposal[] handleBuddyPolicyCompletion(String currentValue, int offset) {
        String[] validValues = new String[] { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "dependent", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "global", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        "registered", "app", "ext", "boot", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        "parent" };
        int comma = currentValue.lastIndexOf(',');
        if (comma != -1)
        ...
        if (elems != null)
            for (ManifestElement elem : elems) set.add(elem.getValue());
        int length = currentValue.length();
        for (int i = 0; i < validValues.length; i++) if (validValues[i].regionMatches(true, 0, currentValue, 0, length) && !set.contains(validValues[i]))
    ...
    protected ICompletionProposal[] handleRequiredExecEnv(String currentValue, int offset) {
        ...
        currentValue = removeLeadingSpaces(currentValue);
        ArrayList<TypeCompletionProposal> completions = new ArrayList();
        @SuppressWarnings("unchecked") HashSet<String> set = (HashSet<String>) fHeaders.get(Constants.BUNDLE_REQUIREDEXECUTIONENVIRONMENT);
        if (set == null)
            set = new HashSet(0);
        int length = currentValue.length();
        for (int i = 0; i < fExecEnvs.length; i++) if (fExecEnvs[i].regionMatches(true, 0, currentValue, 0, length) && !set.contains(fExecEnvs[i]))
    ...
    protected ICompletionProposal[] handleTrueFalseValue(String currentValue, int offset) {
        ...
        int length = currentValue.length();
        if (length == 0)
            return new ICompletionProposal[] { //$NON-NLS-1$ //$NON-NLS-2$
        ...
        length < 5 && "true".regionMatches(true, 0, currentValue, 0, length))
            return new ICompletionProposal[] { //$NON-NLS-1$ //$NON-NLS-2$
            new TypeCompletionProposal("true", getImage(F_TYPE_VALUE), "true", offset - length, length) };
        else if (//$NON-NLS-1$
        length < 6 && "false".regionMatches(true, 0, currentValue, 0, length))
            return new ICompletionProposal[] { //$NON-NLS-1$ //$NON-NLS-2$
    ...
    protected ICompletionProposal[] handleEclipseBundleShape(String currentValue, int offset) {
        ...
        for (int i = 0; i < values.length; i++) if (values[i].regionMatches(true, 0, currentValue, 0, length) && !currentValue.equals(values[i]))
    ...
    protected ICompletionProposal[] matchValueCompletion(String value, String[] attrs, int[] types, int offset) {
        //$NON-NLS-1$
        return matchValueCompletion(value, attrs, types, offset, "");
    }
    ...
    protected ICompletionProposal[] matchValueCompletion(String value, String[] attrs, int[] types, int offset, String prefixCostant) {
        ...
        for (int i = 0; i < attrs.length; i++) if (attrs[i].regionMatches(true, 0, value, 0, length)) {
            if (types[i] == F_TYPE_ATTRIBUTE) {
                proposal = new //$NON-NLS-1$
                TypeCompletionProposal(//$NON-NLS-1$
                attrs[i] + "=", //$NON-NLS-1$
                getImage(F_TYPE_ATTRIBUTE), //$NON-NLS-1$
                attrs[i], //$NON-NLS-1$
                offset - length, //$NON-NLS-1$
                length);
                proposal.setAdditionalProposalInfo(getJavaDoc(//$NON-NLS-1$
                attrs[i] + //$NON-NLS-1$
                "_ATTRIBUTE"));
                ...
                TypeCompletionProposal(//$NON-NLS-1$
                attrs[i] + ":=", //$NON-NLS-1$
                getImage(F_TYPE_DIRECTIVE), //$NON-NLS-1$
                attrs[i], //$NON-NLS-1$
                offset - length, //$NON-NLS-1$
                length);
                proposal.setAdditionalProposalInfo(getJavaDoc(//$NON-NLS-1$
    ...
    public void dispose() {
        for (int i = 0; i < fImages.length; i++) if (fImages[i] != null && !fImages[i].isDisposed())
            fImages[i].dispose();
    }
