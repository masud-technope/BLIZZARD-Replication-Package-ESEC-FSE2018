...
public class LaunchAction extends Action {
    ...
    private String getProgramArguments(String os, String arch) {
        ...
        //$NON-NLS-1$
        String userArgs = (info != null) ? CoreUtility.normalize(info.getCompleteProgramArguments(os, arch)) : "";
    ...
    private String getVMArguments(String os, String arch) {
        ...
        IArgumentsInfo info = fProduct.getLauncherArguments();
        //$NON-NLS-1$
        String userArgs = (info != null) ? CoreUtility.normalize(info.getCompleteVMArguments(os, arch)) : "";
        return concatArgs(buffer, userArgs);
    }
    ...
    private String removeDuplicateArguments(StringBuffer initialArgs) {
        String[] progArguments = { '-' + IEnvironment.P_OS, '-' + IEnvironment.P_WS, '-' + IEnvironment.P_ARCH, '-' + IEnvironment.P_NL };
        //$NON-NLS-1$ // see
        String defaultStart = "${target.";
        // LaunchArgumentHelper
        ArrayList<String> userArgsList = new ArrayList(Arrays.asList(DebugPlugin.splitArguments(initialArgs.toString())));
    ...
    private String getTemplateConfigIni(String os) {
        ...
        if (info != null) {
            ...
            if (// if we can't find an os path, let's try the normal one
            path == null)
    ...
    private ILaunchConfiguration[] getLaunchConfigurations() throws CoreException {
        ...
        for (int i = 0; i < configs.length; i++) {
            if (!DebugUITools.isPrivate(configs[i])) {
                ...
                configs[i].getAttribute(//$NON-NLS-1$
                IPDELauncherConstants.PRODUCT_FILE, //$NON-NLS-1$
                "");
