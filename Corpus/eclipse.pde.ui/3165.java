...
public class DependenciesView extends PageBookView implements IPreferenceConstants, IHelpContextIds {
    ...
    private void contributeToLocalToolBar(IToolBarManager manager) {
        ...
        //$NON-NLS-1$
        manager.add(new Separator("type"));
        //$NON-NLS-1$
        manager.appendToGroup("type", fShowCallees);
        //$NON-NLS-1$
        manager.appendToGroup("type", fShowCallers);
        //$NON-NLS-1$
        manager.add(new Separator("presentation"));
        //$NON-NLS-1$
        manager.appendToGroup("presentation", fShowTree);
        //$NON-NLS-1$
        manager.appendToGroup("presentation", fShowList);
        //$NON-NLS-1$
        manager.add(new Separator("history"));
        //$NON-NLS-1$
        manager.appendToGroup("history", fShowLoops);
        //$NON-NLS-1$
        manager.appendToGroup("history", fHistoryDropDownAction);
    }
    ...
    void updateTitle(Object newInput) {
        if (newInput == null) {
            ...
            if (fLoops != NO_LOOPS) {
                ...
                PDEUIMessages.DependenciesView_cycles_title;
            }
            updateTitle(title);
        }
    }
