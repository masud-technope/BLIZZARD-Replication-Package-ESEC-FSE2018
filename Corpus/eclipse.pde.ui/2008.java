...
public class BundleProjectDescription implements IBundleProjectDescription {
    ...
    private void initialize(IProject project) throws CoreException {
        ...
        if (manifest.exists()) {
            ...
            if (elements != null && elements.length > 0) {
                ...
                classpath = getClasspathEntries(//$NON-NLS-1$
                project, //$NON-NLS-1$
                build, //$NON-NLS-1$
                ".");
            }
            ...
            if (elements != null && elements.length > 0) {
                ...
                setSingleton(//$NON-NLS-1$
                "true".equals(directive));
            }
            ...
            if (elements != null) {
                if (elements.length > 0) {
                    ...
                    for (int i = 0; i < elements.length; i++) {
                        boolean optional = Constants.RESOLUTION_OPTIONAL.equals(elements[i].getDirective(Constants.RESOLUTION_DIRECTIVE)) || "true".equals(elements[i].getAttribute(//$NON-NLS-1$
            ...
            if (elements != null && elements.length > 0) {
                ...
                for (int i = 0; i < elements.length; i++) {
                    ...
                    boolean internal = //$NON-NLS-1$
                    "true".equals(exp.getDirective(ICoreConstants.INTERNAL_DIRECTIVE)) || //$NON-NLS-1$
                    directive != null;
                    String[] friends = null;
                    if (directive != null) {
            ...
            if (elements != null && elements.length > 0) {
                ...
                for (int i = 0; i < elements.length; i++) {
                    ...
                    boolean reexport = Constants.VISIBILITY_REEXPORT.equals(rb.getDirective(Constants.VISIBILITY_DIRECTIVE)) || "true".equals(rb.getAttribute(//$NON-NLS-1$
                    ICoreConstants.REPROVIDE_ATTRIBUTE));
                    boolean optional = Constants.RESOLUTION_OPTIONAL.equals(rb.getDirective(Constants.RESOLUTION_DIRECTIVE)) || "true".equals(rb.getAttribute(//$NON-NLS-1$
                    ICoreConstants.OPTIONAL_ATTRIBUTE));
    ...
    public String getHeader(String header) {
        if (// might be null so check contains
        fHeaders.containsKey(header)) {
