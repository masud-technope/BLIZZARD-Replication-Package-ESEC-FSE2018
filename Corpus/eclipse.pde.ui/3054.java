...
public abstract class Resource extends PlatformObject implements IResource, ICoreConstants, Cloneable, IPathRequestor {
    ...
    public void accept(final IResourceVisitor visitor, int depth, int memberFlags) throws CoreException {
        ...
        for (int i = 0; i < members.length; i++) members[i].accept(visitor, DEPTH_ZERO, memberFlags);
    }
    ...
    public void deleteResource(boolean convertToPhantom, MultiStatus status) throws CoreException {
        ...
        if (links != null)
            for (Iterator it = links.iterator(); it.hasNext(); ) workspace.broadcastEvent(LifecycleEvent.newEvent(LifecycleEvent.PRE_LINK_DELETE, (IResource) it.next()));
        ...
        if (getType() != IResource.PROJECT && links != null) {
            ...
            for (Iterator it = links.iterator(); it.hasNext(); ) description.setLinkLocation(((IResource) it.next()).getProjectRelativePath(), null);
    ...
    protected void internalSetLocal(boolean flag, int depth) throws CoreException {
        ...
        for (int i = 0; i < children.length; i++) ((Resource) children[i]).internalSetLocal(flag, depth);
    }
    ...
    private void broadcastPreDeleteEvent() throws CoreException {
        switch(getType()) {
            ...
            case IResource.ROOT:
                ...
                for (int i = 0; i < projects.length; i++) workspace.broadcastEvent(LifecycleEvent.newEvent(LifecycleEvent.PRE_PROJECT_DELETE, projects[i]));
        }
    }
