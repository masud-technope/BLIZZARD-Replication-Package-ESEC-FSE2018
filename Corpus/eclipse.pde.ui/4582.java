...
public class VMHelper {
    ...
    public static String getDefaultEEName(ILaunchConfiguration configuration) throws CoreException {
        ...
        // Use a list to keep order
        List<IExecutionEnvironment> validEEs = new LinkedList();
        ...
        for (IPluginModelBase plugin : plugins) {
            if (validEEs.isEmpty()) {
                // No valid EEs left, short circuit
                break;
            }
            if (plugin.isFragmentModel()) {
                // The default EE shouldn't depend on fragments
                continue;
            }
            ...
            if (desc != null) {
                ...
                if (bundleEnvs.length > 0) {
                    ...
                    if (isUnbound) {
                        // Use another bundle to determine best EE
                        continue;
                    }
                    ...
                    for (Iterator<IExecutionEnvironment> iterator = validEEs.iterator(); iterator.hasNext(); ) {
                        ...
                        for (String bundleEnv : bundleEnvs) {
                            if (isValid) {
                                // sub environment was valid
                                break;
                            }
                            if (bundleEnv.equals(currentEE.getId())) {
                                isValid = true;
                                // No need to check subEnvironments at all
                                break;
                            }
                            ...
                            for (IExecutionEnvironment element : currentSubEE) {
                                if (bundleEnv.equals(element.getId())) {
                                    isValid = true;
                                    // No need to check other subEnvironments
                                    break;
                                }
                            }
                        }
