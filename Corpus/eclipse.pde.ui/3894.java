...
public class ClasspathUtilCore {
    ...
    public static boolean containsVariables(String name) {
        return //$NON-NLS-1$
        name.indexOf("$os$") != -1 || //$NON-NLS-1$
        name.indexOf("$ws$") != //$NON-NLS-1$
        -1 || //$NON-NLS-1$
        name.indexOf("$nl$") != //$NON-NLS-1$
        -1 || //$NON-NLS-1$
        name.indexOf("$arch$") != //$NON-NLS-1$
        -1;
    }
    ...
    public static String expandLibraryName(String source) {
        ...
        source.indexOf("$ws$") != -1)
            source = //$NON-NLS-1$
            source.replaceAll(//$NON-NLS-1$
            "\\$ws\\$", "ws" + IPath.SEPARATOR + //$NON-NLS-1$
            TargetPlatform.getWS());
        if (//$NON-NLS-1$
        source.indexOf("$os$") != -1)
            source = //$NON-NLS-1$
            source.replaceAll(//$NON-NLS-1$
            "\\$os\\$", "os" + IPath.SEPARATOR + //$NON-NLS-1$
            TargetPlatform.getOS());
        if (//$NON-NLS-1$
        source.indexOf("$nl$") != -1)
            source = //$NON-NLS-1$
            source.replaceAll(//$NON-NLS-1$
            "\\$nl\\$", "nl" + IPath.SEPARATOR + //$NON-NLS-1$
            TargetPlatform.getNL());
        if (//$NON-NLS-1$
        source.indexOf("$arch$") != -1)
            source = //$NON-NLS-1$
            source.replaceAll(//$NON-NLS-1$
            "\\$arch\\$", "arch" + IPath.SEPARATOR + //$NON-NLS-1$
