...
public class PointSelectionPage extends BaseWizardSelectionPage {
    ...
    class WildcardFilter extends ViewerFilter {
        ...
        protected void setMatchText(String match) {
            ...
            match.indexOf("*") != 0 & match.indexOf("?") != 0 & match.indexOf(".") != 0) {
                //$NON-NLS-1$
                match = //$NON-NLS-1$
                "*" + match;
            }
        ...
        public Object[] getElements(Object parent) {
            ...
            for (int i = 0; i < plugins.length; i++) {
                ...
                if (id.equals(fModel.getPluginBase().getId()))
                    continue;
                for (int j = 0; j < points.length; j++) extPoints.add(points[j]);
            }
            IPluginExtensionPoint[] points = fModel.getPluginBase().getExtensionPoints();
            for (int i = 0; i < points.length; i++) extPoints.add(points[i]);
    ...
    public void createControl(Composite parent) {
        ...
        //$NON-NLS-1$
        fDescLink.setText(NLS.bind(PDEUIMessages.PointSelectionPage_extPointDesc, ""));
    ...
    public void selectionChanged(SelectionChangedEvent event) {
        ...
        if (selection instanceof IStructuredSelection) {
            ...
            if (!ssel.isEmpty()) {
                ...
                //$NON-NLS-1$
                fDescLink.setText(//$NON-NLS-1$
                NLS.bind(PDEUIMessages.PointSelectionPage_extPointDesc, ""));
        ...
        return new WizardNode(this, element) {
            ...
            protected IExtensionWizard createWizard(WizardElement element) throws CoreException {
                if (element.isTemplate()) {
                    ...
                    if (template == null)
                        return null;
                    ITemplateSection section = (ITemplateSection) //$NON-NLS-1$
                    template.createExecutableExtension(//$NON-NLS-1$
                    "class");
