...
public class SplashInfo extends ProductObject implements ISplashInfo {
    ...
    private void writeProperty(String indent, PrintWriter writer, String name, String value) {
        writer.println();
        //$NON-NLS-1$ //$NON-NLS-2$
        writer.print(indent + indent + name + "=\"" + value + "\"");
    }
    ...
    public static String getGeometryString(int[] geometry) {
        if (geometry == null || geometry.length < 4)
            return null;
        return //$NON-NLS-1$
        Integer.toString(geometry[0]) + "," + Integer.toString(geometry[1]) + //$NON-NLS-1$
    ...
    public static int[] getGeometryArray(String tokenizedValue) {
        ...
        int position = 0;
        int[] geo = new int[4];
        while (tokenizer.hasMoreTokens()) geo[position++] = Integer.parseInt(tokenizer.nextToken());
        return geo;
    }
    ...
    public void addProgressBar(boolean add, boolean blockNotification) {
        ...
        if (add)
            ...
            //$NON-NLS-1$
            firePropertyChanged("", Boolean.toString(old), Boolean.toString(add));
    }
    ...
    public void addProgressMessage(boolean add, boolean blockNotification) {
        ...
        if (add) {
            ...
            //$NON-NLS-1$
            setForegroundColor(foreground != null ? foreground : "000000", blockNotification);
        } else if (!blockNotification && isEditable())
            //$NON-NLS-1$
            firePropertyChanged("", Boolean.toString(mold || cold), Boolean.toString(add));
    }
