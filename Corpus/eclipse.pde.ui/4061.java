...
public class BuildErrorReporter extends ErrorReporter implements IBuildPropertiesConstants {
    ...
    private static final String[] RESERVED_NAMES = new String[] { "meta-inf", "osgi-inf", ICoreConstants.BUILD_FILENAME_DESCRIPTOR, ICoreConstants.PLUGIN_FILENAME_DESCRIPTOR, "plugin.properties" };
    ...
    private void validateBuild(IBuild build) {
        ...
        for (int i = 0; i < entries.length; i++) {
            ...
            if (entries[i].getTokens().length == 0)
                ...
                if (tokens.length == 1 && //$NON-NLS-1$
                tokens[0].equalsIgnoreCase(//$NON-NLS-1$
                "true"))
        ...
        if (project.exists()) {
            ...
            if (projectComplianceLevel != null) {
                ...
                if (projectComplianceLevel.equals(findMatchingEE(projectSourceCompatibility, projectClassCompatibility, false)) && execEnv.equals(findMatchingEE(projectSourceCompatibility, projectClassCompatibility, true))) {
                    //The project compliance settings matches the BREE
                    return;
                }
            ...
            if (complianceErrorSettings.size() > 0) {
                for (Iterator<String> iterator = libraryNames.iterator(); iterator.hasNext(); ) {
                    ...
                    if (matchingEntry == null) {
                        ...
                        String //$NON-NLS-1$
                        missingTokens = "";
                        for (Iterator<String> iterator2 = complianceErrorSettings.iterator(); iterator2.hasNext(); ) {
                            String currentIdentifier = iterator2.next();
                            missingTokens = join(missingTokens, '-' + currentIdentifier);
                        }
    ...
    private void validateBinIncludes(IBuildEntry binIncludes) {
        ...
        if (OSGinf.exists()) {
            try {
                if (// only validate if we have something in it
                OSGinf.members().length > 0) {
                    validateBinIncludes(binIncludes, ICoreConstants.OSGI_INF_FOLDER_NAME);
                }
            } catch (CoreException // do nothing
            e) {
            }
        }
        ...
        if (model instanceof IBundlePluginModelBase && !(model instanceof IBundleFragmentModel)) {
            ...
            String entry = null;
            if (// check for default location
            (mh == null || mh.getValue() == null)) {
    ...
    private void validateApplicationContributions(IBuildEntry binIncludes) {
        try {
            ...
            if (nodeProduct != null) {
                Node attValue = (Node) //$NON-NLS-1$
                xpath.evaluate(//$NON-NLS-1$
                "property[@name='applicationXMI']/@value", //$NON-NLS-1$
                nodeProduct, //$NON-NLS-1$
                XPathConstants.NODE);
                if (attValue != null) {
                    ...
                    if (fProject.exists(new Path("Application.e4xmi"))) {
                        // Default if not specified
                        validateBinIncludes(//$NON-NLS-1$
                        binIncludes, //$NON-NLS-1$
                        "Application.e4xmi");
                    }
                }
            }
    ...
    private void validateMissingSourceInBinIncludes(IBuildEntry binIncludes, ArrayList<String> sourceEntryKeys, IBuild build) {
        ...
        for (int i = 0; i < sourceEntryKeys.size(); i++) {
            ...
            if (!pluginLibraryNames.contains(key)) {
                // do not report error for folders if the library itself does not exists on plug-in classpath
                return;
            }
        ...
        for (int j = 0; j < fragments.length; j++) {
            ...
            if (fragmentModel != null && fragmentModel.getUnderlyingResource() != null) {
                ...
                try {
                    ...
                    if (build != null) {
                        IBuildEntry[] entries = build.getBuildEntries();
                        for (int i = 0; i < entries.length; i++) if (entries[i].getName().equals(PROPERTY_SOURCE_PREFIX + libname))
                            return true;
                        return false;
                    }
    ...
    private void validateSourceFoldersInSrcIncludes(IBuildEntry includes) {
        ...
        } catch (JavaModelException //do nothing
        e) {
        }
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            tokens[i].startsWith(".") || reservedTokens.contains(res.getName().toString().toLowerCase())) {
    ...
    private void validateIncludes(IBuildEntry includes, ArrayList<String> sourceIncludes, int severity) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            token.endsWith("/") && !(member instanceof IFolder)) {
            ...
            !token.endsWith("/") && !(member instanceof IFile)) {
                ...
                if (useJavaProjectSettings != null) {
                    ...
                    if (!entryCorrect) {
                        ...
                        if (fProject.equals(root)) {
                            ...
                            //$NON-NLS-1$ //$NON-NLS-2$
                            IPath prefFile = fProject.getFullPath().append(".settings").append(JavaCore.PLUGIN_ID + ".prefs");
        ...
        for (int i = 0; i < fProblemList.size(); i++) {
            ...
            if (lineNum > 0) {
                ...
                if (marker != null && bp.attributes != null) {
                    for (Iterator<String> iterator = bp.attributes.keySet().iterator(); iterator.hasNext(); ) {
