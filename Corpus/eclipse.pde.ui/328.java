...
public class TagScanner {
    ...
    public void scan(CompilationUnit source, IApiDescription description, IApiTypeContainer container, Map<String, String> options, IProgressMonitor monitor) throws CoreException {
        ...
        try {
            ...
            throw new CoreException(new Status(IStatus.ERROR, ApiPlugin.PLUGIN_ID, MessageFormat.format("Compilation unit source not found: {0}", source.getName()), e));
        } catch (IOException e) {
            if (ApiPlugin.DEBUG_TAG_SCANNER) {
                System.err.println(source.getName());
            }
            throw new CoreException(new Status(IStatus.ERROR, ApiPlugin.PLUGIN_ID, MessageFormat.format("Error reading compilation unit: {0}", source.getName()), e));
        } finally {
            if (inputStream != null) {
                try {
                    inputStream.close();
