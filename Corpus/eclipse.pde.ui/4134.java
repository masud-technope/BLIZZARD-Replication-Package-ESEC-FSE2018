...
public class TargetContentsGroup {
    ...
    protected void createFormContents(Composite parent, FormToolkit toolkit) {
        ...
        //$NON-NLS-1$
        fCountLabel = toolkit.createLabel(comp, "");
    ...
    protected void createDialogContents(Composite parent) {
        ...
        //$NON-NLS-1$
        fCountLabel = SWTFactory.createLabel(comp, "", 2);
        updateButtons();
        initializeFilters();
    }
    ...
    private List<TargetBundle> getRequiredPlugins(final Collection<TargetBundle> allBundles, final Object[] checkedBundles) {
        ...
        IRunnableWithProgress op = new IRunnableWithProgress() {
            ...
            public void run(IProgressMonitor monitor) {
                ...
                PDEState state = new PDEState(allLocations.toArray(new URL[allLocations.size()]), true, false, subMonitor.split(50));
                if (subMonitor.isCanceled()) {
                    return;
                }
                ...
                dependencies.addAll(DependencyManager.getDependencies(checkedModels.toArray(), implicitIDs.toArray(new String[implicitIDs.size()]), state.getState(), null));
    ...
    public void saveIncludedBundleState() {
        if (fFeaureModeButton.getSelection()) {
            ...
            // BSNs of bundles with multiple versions available
        ...
        public Object[] getElements(Object inputElement) {
            if (inputElement instanceof ITargetDefinition) {
                ...
                if (fMissing == null || fMissing.isEmpty()) {
                    // A set is used to remove copies of problem bundles
                    fMissing = new HashSet();
                    TargetBundle[] bundles = fTargetDefinition.getBundles();
                    for (int i = 0; i < bundles.length; i++) {
                        if (!bundles[i].getStatus().isOK()) {
