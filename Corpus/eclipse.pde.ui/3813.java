...
public class PluginModelManager implements IModelProviderListener {
    ...
    // keeps track of changes in target models
    private ExternalModelManager fExternalManager;

    // keeps track of changes in the workspace
    ...
    // a master table keyed by plugin ID and the value is a ModelEntry
    ...
    public void modelsChanged(IModelProviderEvent e) {
        ...
        if ((e.getEventTypes() & IModelProviderEvent.MODELS_CHANGED) != 0) {
            ...
            for (int i = 0; i < changed.length; i++) handleChange((IPluginModelBase) changed[i], delta);
        }
        if (fState != null) {
    ...
    private void fireStateDelta(StateDelta delta) {
        if (fStateListeners != null) {
            ...
            while (li.hasNext()) li.next().stateResolved(delta);
        }
    }
    ...
    private void fireStateChanged(PDEState newState) {
        if (fStateListeners != null) {
            ListIterator<IStateDeltaListener> li = fStateListeners.listIterator();
            while (li.hasNext()) li.next().stateChanged(newState.getState());
        }
    }
    ...
    private synchronized void initializeTable(IProgressMonitor monitor) {
        ...
        if (subMon.isCanceled()) {
            ...
            if (PDECore.DEBUG_MODEL) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Target platform initialization cancelled by user");
            }
        ...
        if (PDECore.DEBUG_MODEL) {
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(fState.getTargetModels().length + " target models created in  " + (System.currentTimeMillis() - startTargetModels) + " ms");
        }
        // Workspace models
        ...
        if (PDECore.DEBUG_MODEL) {
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(fWorkspaceManager.getModels().length + " workspace models created in  " + (System.currentTimeMillis() - startWorkspaceAdditions) + " ms");
        }
        ...
        if (externalPluginsChanged) {
            ...
            if (PDECore.DEBUG_MODEL) {
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Loaded target models differ from saved list, PDE builder will run on all projects.");
            }
        }
        subMon.worked(25);
        if (PDECore.DEBUG_MODEL) {
    ...
    private synchronized void addWorkspaceBundleToState(Map<String, LocalModelEntry> entries, IPluginModelBase model) {
        ...
        //$NON-NLS-1$
        boolean isQualifierVersion = "qualifier".equals(version.getQualifier());
        ModelEntry entry = entries.get(id);
        if (entry != null) {
            IPluginModelBase[] models = entry.getExternalModels();
            for (int i = 0; i < models.length; i++) {
                Version entryVersion = models[i].getBundleDescription().getVersion();
                if (isQualifierVersion && version.getMajor() == entryVersion.getMajor() && version.getMinor() == entryVersion.getMinor() && version.getMicro() == entryVersion.getMicro()) {
        ...
        if (desc != null) {
            ...
            if (spec != null && (//$NON-NLS-1$ //$NON-NLS-2$
            "true".equals(System.getProperty("pde.allowCycles")) || isPatchFragment(entries, desc) || desc.getImportPackages().length > 0 || desc.getRequiredBundles().length > 0)) {
    ...
    private boolean isSavedExternalPluginListDifferent(URL[] newUrls) {
        ...
        try {
            ...
            while (reader.ready()) {
                ...
                if (//$NON-NLS-1$
                url != null && !url.trim().isEmpty() && !url.startsWith("#")) {
