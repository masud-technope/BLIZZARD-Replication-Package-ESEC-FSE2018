...
public class GenerateFeatureBuildFileAction extends BaseBuildAction {
    ...
    protected void makeScripts(IProgressMonitor monitor) throws InvocationTargetException, CoreException {
        ...
        //$NON-NLS-1$
        String url = ClasspathHelper.getDevEntriesProperties(fManifestFile.getProject().getLocation().addTrailingSeparator().toString() + "dev.properties", false);
        generator.setDevEntries(url);
        generator.setWorkingDirectory(fManifestFile.getProject().getLocation().toOSString());
        //$NON-NLS-1$ //$NON-NLS-2$
        String configInfo = TargetPlatform.getOS() + ", " + TargetPlatform.getWS() + ", " + TargetPlatform.getOSArch();
        ...
        generator.setElements(new String[] { "feature@" + fFeatureModel.getFeature().getId() + (fFeatureModel.getFeature().getVersion() == null ? "" : ":" + fFeatureModel.getFeature().getVersion()) });
