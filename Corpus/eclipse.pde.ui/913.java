...
public class SchemaJavaAttributeDetails extends SchemaAttributeDetails {
    ...
    public void updateFields(ISchemaObject object) {
        ...
        if ((basedOn != null) && (basedOn.length() > 0)) {
            ...
            if (index == -1) {
                ...
                basedOn.substring(//$NON-NLS-1$
                basedOn.lastIndexOf(".") + 1);
    ...
    private void setBasedOn() {
        //$NON-NLS-1$ //$NON-NLS-2$
        String classEntry = fClassEntry.getValue().replaceAll(":", "");
        //$NON-NLS-1$ //$NON-NLS-2$
        String interfaceEntry = fInterfaceEntry.getValue().replaceAll(":", "");
