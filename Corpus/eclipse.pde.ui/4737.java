...
public class ConsumerReportConvertor extends UseReportConverter {
        ...
        public void endVisitScan() {
            try {
                ...
                if (ApiPlugin.DEBUG_USE_REPORT_CONVERTER) {
                    ...
                    "Writing consumer report for bundle: " + //$NON-NLS-1$
                    consumer.name);
        ...
        public void endVisitComponent(IComponentDescriptor target) {
            if (this.currentProducer.counts.getTotalRefCount() > 0) {
                try {
                    ...
                    if (ApiPlugin.DEBUG_USE_REPORT_CONVERTER) {
                        System.out.println(//$NON-NLS-1$
                        "Writing producer report for bundle: " + //$NON-NLS-1$
    ...
    protected void writeIndexPage(List<?> scanResult) throws Exception {
        ...
        try {
            ...
            if (hasMissing()) {
                ...
                buffer.append(NLS.bind(SearchMessages.UseReportConverter_missing_bundles_prevented_scan, new String[] { " <a href=\"./missing.html\">", //$NON-NLS-1$ //$NON-NLS-2$
                "</a>" }));
                buffer.append(CLOSE_P);
            }
            buffer.append(OPEN_P);
            buffer.append(NLS.bind(SearchMessages.UseReportConverter_bundles_that_were_not_searched, new String[] { "<a href=\"./not_searched.html\">", //$NON-NLS-1$//$NON-NLS-2$
            "</a></p>\n" }));
                ...
                if (scanResult.size() > 0) {
                    ...
                    for (Object obj : scanResult) {
                        if (obj instanceof Consumer) {
                            ...
                            refereehtml = new File(getReportsRoot(), //$NON-NLS-1$
                            consumer.name + File.separator + //$NON-NLS-1$
                            "index.html");
    ...
    protected void writeConsumerReport(Consumer consumer, Map<String, Producer> producers) throws Exception {
        ...
        try {
            ...
            for (String string : producerNames) {
                ...
                if (producer != null) {
                    refereehtml = new File(getReportsRoot(), //$NON-NLS-1$
                    producer.name + File.separator + //$NON-NLS-1$
                    "index.html");
                    link = extractLinkFrom(getReportsRoot(), refereehtml.getAbsolutePath());
    ...
    protected void writeProducerReport(Consumer parentConsumer, Producer producer) throws Exception {
        ...
        try {
            ...
            buffer.append(BR);
            //$NON-NLS-1$
            buffer.append(OPEN_P).append("<a href=\"../index.html\">").append(NLS.bind(SearchMessages.ConsumerReportConvertor_BackLinkToConsumer, parentConsumer.name)).append(CLOSE_A).append(CLOSE_P);
            buffer.append(W3C_FOOTER);
            writer = new PrintWriter(new OutputStreamWriter(new FileOutputStream(originhtml), IApiCoreConstants.UTF_8));
            writer.println(buffer.toString());
            writer.flush();
