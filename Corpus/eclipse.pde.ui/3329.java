...
public class TargetDefinition implements ITargetDefinition {
            ...
            if (containers != null) {
                ...
                for (int i = 0; i < containers.length; i++) {
                    ...
                    if (synchronizer != null && !seen.contains(synchronizer)) {
                        ...
                        try {
                            synchronizer.synchronize(this, subMonitor.split(synchronizerNumContainerMap.get(synchronizer).intValue() * 95));
        ...
        if (list != null) {
            ...
            while (iterator.hasNext()) {
                ...
                try {
                    ...
                    if (bundleVersion.getMajor() == infoVersion.getMajor() && bundleVersion.getMinor() == infoVersion.getMinor() && bundleVersion.getMicro() == infoVersion.getMicro()) {
                        return bundle;
                    }
                } catch (IllegalArgumentException e) {
                }
    ...
    public String toString() {
        ...
        //$NON-NLS-1$
        buf.append(fName != null ? fName : "No Name");
        if (fContainers == null) {
            //$NON-NLS-1$
            buf.append("\n\tNo containers");
        } else {
            for (int i = 0; i < fContainers.length; i++) {
                //$NON-NLS-1$
                buf.append("\n\t").append(//$NON-NLS-1$
                fContainers[i].toString());
            }
        }
        //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
        buf.append("\nEnv: ").append(fOS).append("/").append(fWS).append("/").append(fArch).append("/").append(fNL);
        //$NON-NLS-1$
        buf.append("\nJRE: ").append(fJREContainer);
        //$NON-NLS-1$ //$NON-NLS-2$
        buf.append("\nArgs: ").append(fProgramArgs).append("/").append(fVMArgs);
        //$NON-NLS-1$ //$NON-NLS-2$
        buf.append("\nImplicit: ").append(fImplicit == null ? "null" : Integer.toString(fImplicit.length));
        //$NON-NLS-1$
        buf.append("\nHandle: ").append(fHandle.toString());
        return buf.toString();
    }
    ...
    public TargetFeature[] resolveFeatures(String locationPath, IProgressMonitor monitor) throws CoreException {
        ...
        if (models != null) {
            return models;
        /*(IFeatureModel[])models.toArray(new IFeatureModel[models.size()]);*/
        }
