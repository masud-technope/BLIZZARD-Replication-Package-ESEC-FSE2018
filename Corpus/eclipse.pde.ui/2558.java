...
public class NewProjectCreationOperation extends WorkspaceModifyOperation {
    ...
    protected void adjustManifests(IProgressMonitor monitor, IProject project, IPluginBase bundle) throws CoreException {
        ...
        for (int i = 0; i < libs.length; i++) {
            ...
            if (//$NON-NLS-1$
            filters.length == 1 && filters[0].equals("**")) {
                addAllSourcePackages(project, packages);
                break;
            }
            for (int j = 0; j < filters.length; j++) {
                if (//$NON-NLS-1$
                filters[j].endsWith(//$NON-NLS-1$
                ".*"))
    ...
    private void createManifest(IProject project) throws CoreException {
        ...
        if (pluginBase instanceof BundlePluginBase) {
            ...
            if (fData instanceof IPluginFieldData && ((IPluginFieldData) fData).doGenerateClass()) {
                if (//$NON-NLS-1$
                targetVersion.equals(//$NON-NLS-1$
                "3.1"))
                    //$NON-NLS-1$
                    bundle.setHeader(//$NON-NLS-1$
                    ICoreConstants.ECLIPSE_AUTOSTART, //$NON-NLS-1$
                    "true");
                else {
                    ...
                    if (version >= 3.4) {
                        ...
                        //$NON-NLS-1$
                        bundle.setHeader(//$NON-NLS-1$
                        ICoreConstants.ECLIPSE_LAZYSTART, //$NON-NLS-1$
                        "true");
                    }
                }
            }
            if (fContentWizard != null) {
                ...
                if (newFiles != null)
                    for (int i = 0; i < newFiles.length; i++) {
                        if (//$NON-NLS-1$
                        "plugin.properties".equals(//$NON-NLS-1$
                        newFiles[i])) {
                            bundle.setHeader(//$NON-NLS-1$
                            Constants.BUNDLE_LOCALIZATION, "plugin");
                            break;
                        }
                    }
            }
    ...
    protected void execute(IProgressMonitor monitor) throws CoreException, InvocationTargetException, InterruptedException {
        SubMonitor subMonitor = SubMonitor.convert(monitor, PDEUIMessages.NewProjectCreationOperation_creating, getNumberOfWorkUnits());
        // start task
    ...
    protected String getCommaValuesFromPackagesSet(Set<String> values, String version) {
        ...
        while (iter.hasNext()) {
            ...
            buffer.append(value);
            if (//$NON-NLS-1$
            value.indexOf(";version=") == -1 && (version != null) && (values.size() == 1)) {
                //$NON-NLS-1$ //$NON-NLS-2$
