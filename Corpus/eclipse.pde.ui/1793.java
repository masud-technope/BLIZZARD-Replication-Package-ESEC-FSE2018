...
public class FeatureImportWizardPage extends WizardPage {
    ...
    public void createControl(Composite parent) {
        ...
        //$NON-NLS-1$
        fCounterLabel = SWTFactory.createLabel(underTableComp, "", 1);
    ...
    public void storeSettings(boolean finishPressed) {
        ...
        if (finishPressed || fDropLocation.getText().length() > 0 && other) {
            ...
            for (int i = 0; i < nEntries; i++) settings.put(SETTINGS_DROPLOCATION + String.valueOf(i + 1), items[i]);
        }
        if (finishPressed) {
        ...
        IRunnableWithProgress runnable = new IRunnableWithProgress() {
            ...
            public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
                ...
                if (useRuntimeLocation) {
                    ...
                    for (int i = 0; i < allModels.length; i++) if (allModels[i].getUnderlyingResource() == null)
