...
public class LibrarySection extends TableSection implements IBuildPropertiesConstants {
    ...
    private void handleRemove() {
        ...
        for (int i = 0; i < indices.length; i++) if (indices[i] < index)
            index = indices[i];
    ...
    private void checkSourceRootEntry() {
        ...
        for (int i = 0; i < libraries.length; i++) if (//$NON-NLS-1$
        libraries[i].getName().equals("."))
            return;
        ...
        try {
            if (newPath != null) {
                if (entry == null) {
                    ...
                    for (int i = 0; i < entries.length; i++) if (entries[i].getEntryKind() == IClasspathEntry.CPE_SOURCE)
                    ...
                    for (int i = 0; i < tokens.size(); i++) entry.addToken(tokens.get(i));
                    build.add(entry);
                } else
    ...
    private void updateBuildProperties(final String[] oldPaths, final String[] newPaths, boolean modifySourceEntry) {
        ...
        try {
            // adding new entries
            if (oldPaths[0] == null) {
                for (int i = 0; i < newPaths.length; i++) if (newPaths[i] != null) {
            ...
            // removing entries
                ...
                for (int i = 0; i < oldPaths.length; i++) if (oldPaths[i] != null) {
            ...
            // rename entries
                ...
                for (int i = 0; i < oldPaths.length; i++) if (newPaths[i] != null && oldPaths[i] != null) {
        ...
        try {
            ...
            entryLoop: for (int i = 0; i < entries.length; i++) {
                if (entries[i].getEntryKind() == IClasspathEntry.CPE_LIBRARY) {
                    ...
                    for (int j = 0; j < oldPaths.length; j++) if (oldPaths[j] != null && path.equals(new Path(oldPaths[j]).removeTrailingSeparator()))
                        continue entryLoop;
