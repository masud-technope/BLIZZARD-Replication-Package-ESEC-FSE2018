            ...
            case IReference.REF_INTERFACEMETHOD:
                ...
                if (interfacesNames != null) {
                    for (String interfacesName : interfacesNames) {
                        ...
                        if (classFile == null) {
                            //$NON-NLS-1$ //$NON-NLS-2$
                            ApiPlugin.logErrorMessage("Class file for " + interfacesName + " was not found for " + sourceComponent.getName());
                            return false;
                        }
                ...
                if (Flags.isAbstract(type.getModifiers())) {
                    ...
                    if (interfacesNames != null) {
                        for (String interfacesName : interfacesNames) {
                            IApiTypeRoot classFile = Util.getClassFile(new IApiComponent[] { sourceComponent }, interfacesName);
                            if (classFile == null) {
                                ApiPlugin.logErrorMessage(//$NON-NLS-1$ //$NON-NLS-2$
                                "Class file for " + interfacesName + " was not found for " + //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (annot != null) {
            ...
            if (annot.getVisibility() == VisibilityModifiers.PRIVATE) {
                ...
                if (host != null && host.getSymbolicName().equals(rcomponent.getSymbolicName())) {
                    ...
                    IApiAccess access = description.resolveAccessLevel(Factory.componentDescriptor(mcomponent.getSymbolicName()), getResolvedReference().getHandle().getPackage());
