...
public class CommandDetails {
    ...
    private void createPreviewLabelComp(Composite parent) {
        ...
        for (int i = 0; i < filters.length; i++) fFilterCombo.add(filters[i].getLabelText());
        ...
        //$NON-NLS-1$
        fComPrev = fToolkit.createText(parent, "", SWT.MULTI | SWT.V_SCROLL | SWT.BORDER | SWT.WRAP);
    ...
    protected void dispose() {
        for (int i = 0; i < fObjectParamList.size(); i++) fObjectParamList.get(i).dispose();
        for (int i = 0; i < fValueParamList.size(); i++) fValueParamList.get(i).dispose();
        for (int i = 0; i < fTextParamList.size(); i++) fTextParamList.get(i).dispose();
    }
        ...
        if (parameters == null || parameters.length == 0) {
            ...
            for (int i = 0; i < parameters.length; i++) {
                ...
                if (parameterValues != null) {
                    ...
                    for (Iterator<?> keys = values.keySet().iterator(); keys.hasNext(); ) parameterValuesCombo.add((String) keys.next());
                ...
                if ((parameterType != null) && (parameterType.getValueConverter() != null)) {
                    Text parameterText = fToolkit.createText(paramLine, "", //$NON-NLS-1$
                ...
                Text parameterText = //$NON-NLS-1$
                fToolkit.createText(//$NON-NLS-1$
                paramLine, //$NON-NLS-1$
                "", //$NON-NLS-1$
