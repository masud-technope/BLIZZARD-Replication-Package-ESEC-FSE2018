...
public class ProductDefinitonWizardPage extends WizardPage implements IHyperlinkListener {
    ...
    private void createProductGroup(FormToolkit toolkit, Composite comp) {
        ...
        if (pluginId != null) {
            ...
            int idNum = 1;
            while (//$NON-NLS-1$
            getProductNameSet().contains(pluginId + "." + productId + numString)) {
    ...
    private void validateIdAndProduct(String error) {
        if (error == null) {
            ...
            if (error == null)
                error = validateId();
            if (//$NON-NLS-1$
            error == null && getProductNameSet().contains(pluginId + "." + fProductText.getText().trim())) {
    ...
    private String validateId() {
        ...
        for (int i = 0; i < id.length(); i++) {
            if (//$NON-NLS-1$
            !id.substring(i, i + 1).matches("[a-zA-Z0-9_]"))
    ...
    public void linkActivated(HyperlinkEvent e) {
        //$NON-NLS-1$
        String extPoint = Platform.PI_RUNTIME + "." + e.getHref().toString();
    ...
    private boolean productNameDefined() {
        //$NON-NLS-1$
        return (fProduct.getName() != null && !fProduct.getName().equals(""));
    }
