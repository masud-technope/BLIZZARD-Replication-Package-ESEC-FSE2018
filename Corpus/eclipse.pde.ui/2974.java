...
public class ExtensionsSection extends TreeSection implements IPropertyChangeListener {
    ...
    public static final String[] COMMON_LABEL_ATTRIBUTES = { //$NON-NLS-1$
    "label", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    "name", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    "locationURI", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    "class", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    "id", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    "commandId", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    "property", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    "activityId", "attribute", //$NON-NLS-1$ //$NON-NLS-2$
    "value" };
    ...
    public  ExtensionsSection(PDEFormPage page, Composite parent) {
        super(page, parent, Section.DESCRIPTION, new String[] /*PDEUIMessages.Actions_search_targetplatform,*/
        { PDEUIMessages.ManifestEditor_DetailExtension_new, PDEUIMessages.ManifestEditor_DetailExtension_remove, PDEUIMessages.ManifestEditor_DetailExtension_edit, PDEUIMessages.ManifestEditor_DetailExtension_up, PDEUIMessages.ManifestEditor_DetailExtension_down });
    ...
    protected void fillContextMenu(IMenuManager manager) {
        ...
        manager.add(new Separator());
        if (// only cut things when the selection is one
        ssel.size() < 2) {
            ...
            try {
                ...
                if (object instanceof IPluginElement) {
                    ...
                    if (!sorted) {
                        ...
                        for (int i = 0; i < original.length; i++) objects[i] = original[i];
                    ...
                    if (!sorted) {
                        ...
                        for (int i = 0; i < original.length; i++) extensions[i] = original[i];
        ...
        if (filterControl != null && attributeValue != null) {
            ...
            if (trimmedValue.length() > 0 && !ExtensionsFilterUtil.isBoolean(trimmedValue)) {
                ...
                if (clearFilterText) {
                    ...
                    if (filterPattern.length() > 0 && !//$NON-NLS-1$
                    filterPattern.endsWith(//$NON-NLS-1$
                    "/")) {
                        //$NON-NLS-1$
                        filterPattern += "/";
                    }
                ...
                if (// quote when
                filterPattern.indexOf('/') != -1) {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    filterPattern = "\"" + filterPattern + "\"";
                }
                // force immediate job run
                setBypassFilterDelay(true);
    ...
    private void loadExtensionWizards() {
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            if (//$NON-NLS-1$
            element.getName().equals("editorWizard")) {
                String pointId = //$NON-NLS-1$
    ...
    public static String resolveObjectName(SchemaRegistry schemaRegistry, Object obj) {
        ...
        if (obj instanceof IPluginExtension) {
            ...
            if (labelAtt != null && labelAtt.getValue() != null) {
                ...
                if (// remove package from handler class
                labelAtt.getName().equals(COMMON_LABEL_ATTRIBUTES[3])) {
            ...
            protected WorkbenchJob doCreateRefreshJob() {
                ...
                job.addJobChangeListener(new JobChangeAdapter() {
                    ...
                    public void done(IJobChangeEvent event) {
                        if (// restoring is only required if the job actually ran
                        aboutToRunPassed) {
                            try {
                                fFilteredTree.setRedraw(false);
