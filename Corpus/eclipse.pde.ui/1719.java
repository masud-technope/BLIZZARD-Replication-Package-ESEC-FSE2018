...
public class FragmentGeneralInfoSection extends GeneralInfoSection {
    ...
    private void createPluginIdEntry(Composite parent, FormToolkit toolkit, IActionBars actionBars) {
        fPluginIdEntry = new FormEntry(parent, toolkit, PDEUIMessages.GeneralInfoSection_pluginId, //
        ...
        fPluginIdEntry.setFormEntryListener(new FormEntryAdapter(this, actionBars) {
            ...
            private void createFragmentPlugin() {
                NewPluginProjectWizard wizard = new //$NON-NLS-1$
                NewPluginProjectWizard(//$NON-NLS-1$
                "Equinox");
    ...
    private void createMatchCombo(Composite client, FormToolkit toolkit, IActionBars actionBars) {
        ...
        String[] items = new //$NON-NLS-1$
        String[] { "", PDEUIMessages.ManifestEditor_MatchSection_equivalent, PDEUIMessages.ManifestEditor_MatchSection_compatible, PDEUIMessages.ManifestEditor_MatchSection_perfect, PDEUIMessages.ManifestEditor_MatchSection_greater };
        ...
        if (isBundle()) {
            if (!fPluginMinVersionEntry.getValue().equals(fPluginMaxVersionEntry.getValue()) && fPluginMaxVersionEntry.getText().getEnabled()) {
                ...
                if (fPluginMinVersionBound.getSelectionIndex() == 0)
                    ...
                    //$NON-NLS-1$
                    version = "(";
                version += fPluginMinVersionEntry.getValue() + "," + //$NON-NLS-1$
                fPluginMaxVersionEntry.getValue();
    ...
    private void refreshVersion() {
        ...
        if (comInd == -1) {
            //$NON-NLS-1$
            setVersionFields(version, true, "", false);
