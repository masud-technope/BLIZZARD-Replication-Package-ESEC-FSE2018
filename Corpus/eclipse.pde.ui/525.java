...
public class UpdateSplashProgressOperation implements IWorkspaceRunnable {
    ...
    public void run(IProgressMonitor monitor) throws CoreException {
        SubMonitor subMonitor = SubMonitor.convert(monitor, PDEUIMessages.UpdateSplashProgressAction_msgProgressCustomizingSplash, 1);
    ...
    private void createPluginCustomizationFile() throws CoreException {
        ...
        if (buildProps.exists()) {
            ...
            if (model.isLoaded()) {
                ...
                model.getBuild().getEntry(//$NON-NLS-1$
                "bin.includes");
                if (entry == null) {
                    entry = //$NON-NLS-1$
                    model.getFactory().createEntry(//$NON-NLS-1$
                    "bin.includes");
