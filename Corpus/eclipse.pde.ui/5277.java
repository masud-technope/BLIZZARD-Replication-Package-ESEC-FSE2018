...
public class GatherUnusedDependenciesOperation implements IRunnableWithProgress {
    ...
    public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
        ...
        if (ClasspathUtilCore.hasBundleStructure(fModel)) {
            ...
            if (header instanceof ImportPackageHeader) {
                ...
                header = new //$NON-NLS-1$
                ImportPackageHeader(//$NON-NLS-1$
                Constants.IMPORT_PACKAGE, //$NON-NLS-1$
                header.getValue(), //$NON-NLS-1$
                bundle, //$NON-NLS-1$
            ...
            if (header instanceof ExportPackageHeader) {
                ...
                header = new //$NON-NLS-1$
                ExportPackageHeader(//$NON-NLS-1$
                Constants.EXPORT_PACKAGE, //$NON-NLS-1$
                header.getValue(), //$NON-NLS-1$
                bundle, //$NON-NLS-1$
    ...
    private void updateMonitor(IProgressMonitor monitor, int size) {
        ...
        PDEUIMessages.UnusedDependencies_analyze + size + " " + //$NON-NLS-1$
        ...
        try {
            ...
            for (int i = 0; i < packageFragments.length; i++) {
                ...
                if (pkgFragment.hasChildren()) {
                    ...
                    engine.search(SearchPattern.createPattern(pkgFragment, IJavaSearchConstants.REFERENCES), new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, searchScope, requestor, iterationMonitor.split(1));
                    if (requestor.foundMatches()) {
                        if (provideJavaClasses(packageFragments[i], engine, searchScope, iterationMonitor.split(1))) {
                            return true;
                        }
                    }
                }
        ...
        for (int j = 0; j < children.length; j++) {
            ...
            if (types != null) {
                ...
                for (int t = 0; t < types.length; t++) {
                    ...
                    engine.search(SearchPattern.createPattern(types[t], IJavaSearchConstants.REFERENCES), new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, searchScope, requestor, iterationMonitor.split(1));
                    if (requestor.foundMatches()) {
                        return true;
                    }
                }
    ...
    private boolean provideJavaClasses(ImportPackageObject pkg, IProgressMonitor monitor) {
        try {
            ...
            engine.search(SearchPattern.createPattern(packageName, IJavaSearchConstants.PACKAGE, IJavaSearchConstants.REFERENCES, SearchPattern.R_EXACT_MATCH), new SearchParticipant[] { SearchEngine.getDefaultSearchParticipant() }, searchScope, requestor, subMonitor.split(1));
            if (requestor.foundMatches())
                return true;
        } catch (CoreException e) {
            PDEPlugin.logException(e);
    ...
    private void minimizeDependencies(HashMap<String, IPluginImport> usedPlugins, ArrayList<ImportPackageObject> usedPackages, IProgressMonitor monitor) {
        ...
        while (it.hasNext()) plugins.push(it.next().toString());
