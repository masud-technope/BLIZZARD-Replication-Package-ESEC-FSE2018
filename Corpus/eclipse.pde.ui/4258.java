...
public class DSProvideSection extends TableSection implements IDoubleClickListener {
    ...
    public  DSProvideSection(PDEFormPage page, Composite parent) {
        super(page, parent, Section.DESCRIPTION | Section.TWISTIE, new String[] { Messages.DSProvideSection_add, Messages.DSProvideSection_remove, Messages.DSProvideSection_edit });
    ...
    private void handleEdit() {
        ...
        if (selection != null) {
            ...
            if (selectionIndex != -1) {
                DSEditProvideDialog dialog = new DSEditProvideDialog(Activator.getActiveWorkbenchShell(), (IDSProvide) fProvidesTable.getElementAt(selectionIndex), this);
                dialog.create();
    ...
    private void updateButtons() {
        ...
        tablePart.setButtonEnabled(2, isEditable() && table.getSelection().length == 1);
    }
    ...
    private void doOpenSelectionDialog(int scopeType) {
        try {
            ...
            SelectionDialog dialog = JavaUI.createTypeDialog(Activator.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), SearchEngine.createWorkspaceScope(), scopeType, true, filter, new DSTypeSelectionExtension(getDSModel()));
    ...
    public void modelChanged(IModelChangedEvent e) {
        if (e.getChangeType() == IModelChangedEvent.WORLD_CHANGED) {
            ...
            if (objects.length > 0) {
                fProvidesTable.refresh();
                fProvidesTable.setSelection(new StructuredSelection(objects[objects.length - 1]));
            }
            updateButtons();
            updateTitle();
        } else {
    ...
    private void updateTitle() {
        ...
        getSection().setText(NLS.bind(Messages.DSProvideSection_title, Integer.valueOf(itemCount)));
    }

    @Override
    public void doubleClick(DoubleClickEvent event) {
        IDSProvide provide = (IDSProvide) ((IStructuredSelection) fProvidesTable.getSelection()).getFirstElement();
