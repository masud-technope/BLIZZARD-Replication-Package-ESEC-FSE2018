            ...
            public void run() {
                try {
                    ...
                    if (log != null && log.exists()) {
                        MessageDialog dialog = new // accept the default window icon
                        MessageDialog(// accept the default window icon
                        PDEPlugin.getActiveWorkbenchShell(), // accept the default window icon
                        PDEUIMessages.Launcher_error_title, // accept the default window icon
                        null, PDEUIMessages.Launcher_error_code13, MessageDialog.ERROR, new String[] { PDEUIMessages.Launcher_error_displayInLogView, PDEUIMessages.Launcher_error_displayInSystemEditor, IDialogConstants.NO_LABEL }, OPEN_IN_ERROR_LOG_VIEW);
                        int dialog_value = dialog.open();
                        if (dialog_value == OPEN_IN_ERROR_LOG_VIEW) {
                            IWorkbenchPage page = PDEPlugin.getActivePage();
                            if (page != null) {
                                LogView errlog = (LogView) page.showView("org.eclipse.pde.runtime.LogView");
