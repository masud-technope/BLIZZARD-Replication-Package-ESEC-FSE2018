    ...
    public static MoveFromChange createMovePackageChange(IFile file, Object[] elements, IProgressMonitor monitor) throws CoreException {
        try {
            ...
            if (operations.length > 0) {
                MoveFromChange change = new //$NON-NLS-1$
                MoveFromChange(//$NON-NLS-1$
                "", //$NON-NLS-1$
                file);
        ...
        try {
            ...
            for (int i = 0; i < elements.length; i++) {
                ...
                if (element instanceof IFile) {
                    ...
                    if (!localizationRenamed && //$NON-NLS-1$
                    fileName.endsWith(//$NON-NLS-1$
                    ".properties")) {
                        String oldText = fileName.substring(0, //$NON-NLS-1$
