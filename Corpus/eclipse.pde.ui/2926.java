...
public class PluginContentPage extends ContentPage {
    ...
    private void createExecutionEnvironmentControls(Composite container) {
        ...
        fExeEnvButton.addListener(SWT.Selection, new Listener() {
            ...
            public void handleEvent(Event event) {
                ...
                new String[] { "org.eclipse.jdt.debug.ui.jreProfiles" }, //$NON-NLS-1$
                null).open();
            }
        });
    }
    ...
    protected void createPluginClassGroup(Composite container) {
        ...
        fGenerateActivator = SWTFactory.createCheckButton(classGroup, PDEUIMessages.ContentPage_generate, null, generateActivator, 2);
    ...
    public void setVisible(boolean visible) {
        if (visible) {
            ...
            } else // if the fGenerateActivator was disabled and is now enabled, then set the selection to true
            if (!wasGenActivatorEnabled) {
            ...
            } else // if the fUIPlugin was disabled and is now enabled, then set the selection to true
            if (!wasUIPluginEnabled) {
            ...
            if (((fChangedGroups & P_CLASS_GROUP) == 0)) {
                ...
                fClassText.setText(computeId().replaceAll("-", "_").toLowerCase(Locale.ENGLISH) + ".Activator");
