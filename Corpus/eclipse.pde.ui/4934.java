...
public class FilterStore implements IApiFilterStore {
    ...
    protected synchronized void initializeApiFilters() {
        if (fFilterMap == null) {
            ...
            try {
                if (//$NON-NLS-1$
                extension != null && extension.equals("jar") && loc.isFile()) {
                    ...
                    if (filterfile != null) {
                        if (ApiPlugin.DEBUG_FILTER_STORE) {
                            //$NON-NLS-1$ //$NON-NLS-2$
                            System.out.println("found api filter file: [" + fComponent.getName() + "] inside jar file " + loc);
                        }
                    ...
                    if (file.exists()) {
                        if (ApiPlugin.DEBUG_FILTER_STORE) {
                            //$NON-NLS-1$ //$NON-NLS-2$
                            System.out.println("found api filter file: [" + fComponent.getName() + "] at " + file);
                        }
        ...
        if (length == filterProblemMessageArguments.length) {
            for (int i = 0; i < length; i++) {
                ...
                if (index == -1) {
                    ...
                    // fully qualified name should match
                    return false;
                } else {
    ...
    protected void readFilterFile(InputStream contents) throws IOException {
        ...
        for (int i = 0; i < resources.getLength(); i++) {
            ...
            if (path.trim().length() == 0) {
                // it is valid to have a filter without a path
                path = null;
            }
        ...
        if (ApiPlugin.DEBUG_FILTER_STORE) {
            //$NON-NLS-1$ //$NON-NLS-2$
            System.out.println(newfilters.size() + " filters found and added for: [" + component + "]");
        }
