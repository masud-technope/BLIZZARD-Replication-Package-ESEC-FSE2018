...
public class EditorUtilities {
    ...
    private static boolean imageEntryInternalValidate(IValidatorMessageHandler validator, FormEntry provider, IProduct product, ValidationInfo info, int validationType) {
        ...
        switch(validationType) {
            ...
            case F_ICO_IMAGE:
                ...
                break;
            case // do not break after F_IMAGEDEPTH since we are also checking exact size
            F_IMAGE_DEPTH:
    ...
    private static ValidationMessage getMS_icoImage(ImageData[] imagedata) {
        ...
        for (int i = 0; i < imagedata.length; i++) {
            ...
            for (int w = 0; w < totalSizes; w++) if (width == LauncherSection.F_WIN_ICON_DIMENSIONS[w][0] && height == LauncherSection.F_WIN_ICON_DIMENSIONS[w][1] && depth == LauncherSection.F_WIN_ICON_DEPTHS[w])
                found[w] = true;
        }
    ...
    private static IPath getFullPath(IPath path, IProduct product) throws MalformedURLException {
        ...
        if (model != null && model.getInstallLocation() != null) {
            ...
            if (modelNode.isFile() && CoreUtility.jarContainsResource(modelNode, filePath, false))
                //$NON-NLS-1$ //$NON-NLS-2$
                return new Path("jar:file:", pluginPath + "!/" + filePath);
            //$NON-NLS-1$ //$NON-NLS-2$
            return new Path("file:", pluginPath + "/" + filePath);
        }
    ...
    public static void openImage(String value, String definingPluginId) {
        ...
        try {
            if (resource != null && resource instanceof IFile)
                ...
                MessageDialog.openWarning(PDEPlugin.getActiveWorkbenchShell(), PDEUIMessages.AboutSection_open, //
                PDEUIMessages.AboutSection_warning);
