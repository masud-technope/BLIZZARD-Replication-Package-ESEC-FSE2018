...
public class FilterProblemResolution extends WorkbenchMarkerResolution {
    ...
    public String getLabel() {
        if (plural) {
            ...
            if (element != null) {
                return MessageFormat.format(MarkerMessages.FilterProblemResolution_0, JavaElementLabels.getTextLabel(element, JavaElementLabels.M_PARAMETER_TYPES), resolveCategoryName());
            } else {
                IResource res = fBackingMarker.getResource();
                return MessageFormat.format(MarkerMessages.FilterProblemResolution_0, res.getFullPath().removeFileExtension().lastSegment(), resolveCategoryName());
            }
        }
    }
