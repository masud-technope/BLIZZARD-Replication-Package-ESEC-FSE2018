...
public class ProductConfigurationSection {
    ...
    private void handleBrowse() {
        ...
        //$NON-NLS-1$
        dialog.addFilter(new FileExtensionFilter("product"));
    ...
    protected void initialize(IStructuredSelection selection, IDialogSettings settings) {
        ...
        if (selection.size() > 0) {
            ...
            if (object instanceof IFile) {
                ...
                if (//$NON-NLS-1$
                "product".equals(file.getFileExtension())) {
                ...
                try {
                    if (container.isAccessible()) {
                        ...
                        for (int i = 0; i < resources.length; i++) {
                            ...
                            if (resource instanceof IFile && //$NON-NLS-1$
                            resource.getName().endsWith(//$NON-NLS-1$
                            ".product")) {
    ...
    protected String validate() {
        ...
        if (//$NON-NLS-1$
        !resource.getName().endsWith(".product"))
