...
public class SourceLocationManager implements ICoreConstants {
    ...
    private static List<SourceLocation> processExtensions() {
        ...
        //$NON-NLS-1$
        IExtension[] extensions = PDECore.getDefault().getExtensionsRegistry().findExtensions(PDECore.PLUGIN_ID + ".source", false);
        for (int i = 0; i < extensions.length; i++) {
            ...
            else // desc might be null if the workspace contains a plug-in with the same Bundle-SymbolicName
            {
            ...
            for (int j = 0; j < children.length; j++) {
                if (//$NON-NLS-1$
                children[j].getName().equals("location")) {
