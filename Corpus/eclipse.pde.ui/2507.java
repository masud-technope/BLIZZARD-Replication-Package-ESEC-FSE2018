...
public class PackageFinder {
    ...
    static void computeReferencedTypes(IClassFileReader cfr, Set<String> packages) {
        ...
        // removes default package if it exists //$NON-NLS-1$
        packages.remove("");
    }

    static boolean isPrimitiveTypeSignature(String typeSig) {
        //$NON-NLS-1$
        if (typeSig.length() >= 2 && typeSig.startsWith("[") && isPrimitiveTypeSignature(typeSig.substring(1, typeSig.length())))
            return true;
    ...
    static String getPackage(String classType) {
        ...
        // if no period, then we have a class in the default package, return "" for packagename //$NON-NLS-1$
        ...
        if (res instanceof IContainer) {
            ...
            while (!stack.isEmpty()) {
                try {
                    ...
                    for (int i = 0; i < children.length; i++) if (children[i] instanceof IFile && "class".equals(//$NON-NLS-1$
                    children[i].getFileExtension())) {
            ...
            res.getFileExtension().equals("jar") || res.getFileExtension().equals("zip")) {
                ...
                try {
                    ...
                    for (int i = 0; i < children.length; i++) {
                        if (children[i] instanceof IPackageFragment) {
                            ...
                            for (int j = 0; j < files.length; j++) classFiles.add(files[j]);
                        }
                    }
