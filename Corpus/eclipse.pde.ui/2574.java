    ...
    public static void collectRequiredBundleFiles(IImportStructureProvider provider, Object element, ArrayList<Object> collected) {
        ...
        if (children != null && !children.isEmpty()) {
            for (int i = 0; i < children.size(); i++) {
                ...
                if (provider.isFolder(curr)) {
                    if (!name.equals("src") && !isClassFolder(//$NON-NLS-1$
                    provider, //$NON-NLS-1$
                    curr)) {
                ...
                !name.endsWith(".class") && !name.endsWith(".RSA") && !name.endsWith(".DSA") && !name.endsWith(".SF")) {
        ...
        if (children != null && !children.isEmpty()) {
            for (int i = 0; i < children.size(); i++) {
                ...
                if (provider.isFolder(curr)) {
                    ...
                    if (folderContainsFileExtension(//$NON-NLS-1$
                    provider, //$NON-NLS-1$
                    curr, //$NON-NLS-1$
                    ".java"))
                        continue;
                    if (//$NON-NLS-1$
                    provider.getFullPath(curr).equals(//$NON-NLS-1$
                    "META-INF/")) {
                        continue;
                    }
                    if (//$NON-NLS-1$
                ...
                !provider.getLabel(curr).endsWith(".java")) {
                    collected.add(curr);
                }
            }
        }
    ...
    public static void collectResources(IImportStructureProvider provider, Object element, ArrayList<Object> collected) {
        ...
        if (children != null && !children.isEmpty()) {
            for (int i = 0; i < children.size(); i++) {
                ...
                !provider.getLabel(curr).endsWith(".class")) {
                    collected.add(curr);
                }
            }
        }
    ...
    private static void collectJavaSourceFromRoot(ZipFileStructureProvider provider, List<IPath> ignoreFolders, ArrayList<Object> collected) {
        ...
        if (children != null && !children.isEmpty()) {
            for (int i = 0; i < children.size(); i++) {
                Object curr = children.get(i);
                if (//$NON-NLS-1$
                provider.isFolder(curr) && folderContainsFileExtension(provider, curr, ".java")) {
