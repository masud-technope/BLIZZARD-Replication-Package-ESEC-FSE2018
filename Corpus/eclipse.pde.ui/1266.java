...
public class DSComponentSection extends PDESection {
    ...
    protected void createClient(Section section, FormToolkit toolkit) {
        ...
        // Attribute: name
        fNameEntry = new FormEntry(client, toolkit, Messages.DSComponentDetails_nameEntry, SWT.NONE);
        // Attribute: class
        fClassEntry = new FormEntry(client, toolkit, Messages.DSImplementationDetails_classEntry, Messages.DSImplementationDetails_browse, isEditable(), 0);
        // Attribute: activate
        fActivateEntry = new FormEntry(client, toolkit, Messages.DSComponentDetails_activateEntry, SWT.NONE);
        fActivateEntry.getLabel().setToolTipText(Messages.DSComponentDetails_activateTooltip);
        // Attribute: deactivate
        fDeactivateEntry = new FormEntry(client, toolkit, Messages.DSComponentDetails_deactivateEntry, SWT.NONE);
        fDeactivateEntry.getLabel().setToolTipText(Messages.DSComponentDetails_deactivateTooltip);
        // Attribute: modified
        fModifiedEntry = new FormEntry(client, toolkit, Messages.DSComponentDetails_modifiedEntry, SWT.NONE);
        fModifiedEntry.getLabel().setToolTipText(Messages.DSComponentDetails_modifiedTooltip);
        setListeners();
        updateUIFields();
    ...
    public void updateUIFields() {
        if (fComponent != null) {
            ...
            if (fComponent.getActivateMethod() == null) {
                //$NON-NLS-1$
                fActivateEntry.setValue(//$NON-NLS-1$
                "", //$NON-NLS-1$
                true);
            } else {
            ...
            if (fComponent.getDeactivateMethod() == null) {
                //$NON-NLS-1$
                fDeactivateEntry.setValue(//$NON-NLS-1$
                "", //$NON-NLS-1$
                true);
            } else {
                fDeactivateEntry.setValue(fComponent.getDeactivateMethod(), true);
            }
            fDeactivateEntry.setEditable(isEditable());
            if (fComponent.getModifiedMethod() == null) {
                //$NON-NLS-1$
                fModifiedEntry.setValue(//$NON-NLS-1$
                "", //$NON-NLS-1$
                true);
            } else {
    ...
    public void setListeners() {
        ...
        fClassEntry.setFormEntryListener(new FormEntryAdapter(this, actionBars) {
            ...
            public void textValueChanged(FormEntry entry) {
                if (fImplementation == null) {
                    if (fComponent != null) {
                        fImplementation = fComponent.getModel().getFactory().createImplementation();
                        fImplementation.setClassName(fClassEntry.getValue());
                        fComponent.addChildNode(fImplementation, 0, true);
                    }
                } else {
                    fImplementation.setClassName(fClassEntry.getValue());
                }
            ...
            public void browseButtonSelected(FormEntry entry) {
                doOpenSelectionDialog(fClassEntry);
            }
        });
    }
    ...
    private void doOpenSelectionDialog(FormEntry entry) {
        ...
        String type = PDEJavaHelperUI.selectType(fModel.getUnderlyingResource(), IJavaElementSearchConstants.CONSIDER_CLASSES, filter, null);
        if (type != null) {
            entry.setValue(type);
            entry.commit();
        }
