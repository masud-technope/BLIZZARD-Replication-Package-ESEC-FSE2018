...
public class WorkspacePluginModelManager extends WorkspaceModelManager {
    ...
    protected boolean isInterestingFolder(IFolder folder) {
        ...
        ("META-INF".equals(folderName) || "OSGI-INF".equals(folderName) || "schema".equals(folderName)) && folder.getParent().equals(root)) {
            return true;
        }
        if (//$NON-NLS-1$
        "OSGI-INF/l10n".equals(folder.getProjectRelativePath().toString())) {
            return true;
        }
        return false;
    }
    ...
    protected void createAndFireEvent(String eventId, int type, Collection<IModel> added, Collection<IModel> removed, Collection<IModel> changed) {
        if (//$NON-NLS-1$
        eventId.equals("org.eclipse.pde.internal.core.IExtensionDeltaEvent")) {
