...
public class PluginVersionPart {
    ...
    public void preloadFields() {
        if (fRangeAllowed) {
            //$NON-NLS-1$
            fMinVersionText.setText((fVersionRange != null) ? fVersionRange.getMinimum().toString() : "");
            //$NON-NLS-1$
            fMaxVersionText.setText((fVersionRange != null && fVersionRange.getMaximum().getMajor() != Integer.MAX_VALUE) ? fVersionRange.getMaximum().toString() : "");
    ...
    private IStatus validateVersion(String text, Text textWidget, boolean shortErrorMessage) {
        ...
        if (VersionUtil.validateVersion(text).getSeverity() != IStatus.OK) {
            ...
            "org.eclipse.pde.ui", IStatus.ERROR, PDELabelUtility.qualifyMessage(PDELabelUtility.getFieldLabel(textWidget), errorMessage), null);
        }
        return Status.OK_STATUS;
    }
    ...
    private IStatus validateVersionRange(boolean shortErrorMessage) {
        ...
        try {
            ...
            return new Status(IStatus.ERROR, "org.eclipse.pde.ui", IStatus.ERROR, PDELabelUtility.qualifyMessage(PDELabelUtility.getFieldLabel(fMinVersionText), errorMessage), null);
        }
        if (// version created fine
        !fRangeAllowed)
            return Status.OK_STATUS;
        try {
            v2 = new Version(getMaxVersion());
        } catch (IllegalArgumentException e) {
            return new Status(IStatus.ERROR, "org.eclipse.pde.ui", IStatus.ERROR, PDELabelUtility.qualifyMessage(PDELabelUtility.getFieldLabel(fMaxVersionText), errorMessage), null);
        }
        ...
        Status(//$NON-NLS-1$
        IStatus.ERROR, //$NON-NLS-1$
        "org.eclipse.pde.ui", IStatus.ERROR, PDEUIMessages.DependencyPropertiesDialog_versionRangeError, null);
    }
