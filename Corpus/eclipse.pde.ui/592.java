...
public class DSLabelProvider extends LabelProvider {
    ...
    private Image getObjectImage(IDSObject obj) {
        if (obj.getType() == IDSConstants.TYPE_IMPLEMENTATION) {
            ...
            IDSReference reference = (IDSReference) obj;
            int flags = 0;
            if (reference == null || reference.getReferencePolicy() == null || reference.getReferenceCardinality() == null)
                return SharedImages.getImage(SharedImages.DESC_REFERENCE, flags);
            if (reference.getReferencePolicy().equals(IDSConstants.VALUE_REFERENCE_POLICY_DYNAMIC)) {
                flags |= SharedImages.F_DYNAMIC;
            }
            String cardinality = reference.getReferenceCardinality();
            if (cardinality.equals(IDSConstants.VALUE_REFERENCE_CARDINALITY_ZERO_ONE)) {
                return SharedImages.getImage(SharedImages.DESC_REFERENCE_ZERO_ONE, flags);
            } else if (cardinality.equals(IDSConstants.VALUE_REFERENCE_CARDINALITY_ZERO_N)) {
                return SharedImages.getImage(SharedImages.DESC_REFERENCE_ZERO_N, flags);
            } else if (cardinality.equals(IDSConstants.VALUE_REFERENCE_CARDINALITY_ONE_N)) {
                return SharedImages.getImage(SharedImages.DESC_REFERENCE_ONE_N, flags);
            }
            return SharedImages.getImage(SharedImages.DESC_REFERENCE, flags);
