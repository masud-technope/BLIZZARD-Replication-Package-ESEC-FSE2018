...
public class DSReferenceSection extends TableSection implements IDoubleClickListener {
    ...
    class ReferenceLabelProvider extends StyledCellLabelProvider {
        ...
        public void update(ViewerCell cell) {
            ...
            styledString.append(" [" + bind + "," + unbind + "]", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
            StyledString.DECORATIONS_STYLER);
            String target = reference.getReferenceTarget();
            if (target != null)
                //$NON-NLS-1$
                styledString.append(//$NON-NLS-1$
                " " + target, StyledString.QUALIFIER_STYLER);
    ...
    public  DSReferenceSection(PDEFormPage page, Composite parent) {
        super(page, parent, Section.DESCRIPTION | Section.TWISTIE, new String[] { Messages.DSReferenceSection_add, Messages.DSReferenceSection_remove, Messages.DSReferenceSection_edit, Messages.DSReferenceSection_up, Messages.DSReferenceSection_down });
    ...
    private void moveDown(Object[] array) {
        for (int i = array.length - 1; i >= 0; i--) {
            ...
            if (object == null) {
                ...
                getDSModel().getDSComponent().moveChildNode((IDocumentElementNode) object, 1, true);
            }
        }
    }
    ...
    private void handleEdit() {
        ...
        if (selection != null) {
            ...
            if (selectionIndex != -1) {
                DSEditReferenceDialog dialog = new DSEditReferenceDialog(Activator.getActiveWorkbenchShell(), (IDSReference) fReferencesTable.getElementAt(selectionIndex), this);
                dialog.create();
        ...
        if (ssel.size() > 0) {
            ...
            while (iter.hasNext()) {
                ...
                if (object instanceof IDSReference) {
                    getDSModel().getDSComponent().removeReference((IDSReference) object);
                }
            }
        }
    }
    ...
    private void doOpenSelectionDialog(int scopeType) {
        try {
            ...
            SelectionDialog dialog = JavaUI.createTypeDialog(Activator.getActiveWorkbenchShell(), PlatformUI.getWorkbench().getProgressService(), SearchEngine.createWorkspaceScope(), scopeType, true, filter);
    ...
    public void modelChanged(IModelChangedEvent e) {
        if (e.getChangeType() == IModelChangedEvent.WORLD_CHANGED) {
            ...
            if (objects.length > 0) {
                fReferencesTable.refresh();
                fReferencesTable.setSelection(new StructuredSelection(objects[objects.length - 1]));
            }
            updateButtons();
            updateTitle();
        } else {
    ...
    private void updateTitle() {
        ...
        getSection().setText(NLS.bind(Messages.DSReferenceSection_title, Integer.valueOf(itemCount)));
    }

    @Override
    public void doubleClick(DoubleClickEvent event) {
        IDSReference reference = (IDSReference) ((IStructuredSelection) fReferencesTable.getSelection()).getFirstElement();
