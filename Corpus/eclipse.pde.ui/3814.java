...
public class LogView extends ViewPart implements ILogListener {
    ...
    private void showFilterText(boolean visible) {
        ...
        // FilteredTree new look lays filter Text on additional composite
    ...
    private void handleExport(boolean exportWholeLog) {
        ...
        if (path != null) {
            if (//$NON-NLS-1$
            path.indexOf('.') == -1 && !path.endsWith(".log"))
                //$NON-NLS-1$
                path += //$NON-NLS-1$
                ".log";
            ...
            try {
                ...
                if (exportWholeLog)
                    in = new BufferedReader(new //$NON-NLS-1$
                    InputStreamReader(//$NON-NLS-1$
                    new FileInputStream(fInputFile), //$NON-NLS-1$
                    "UTF-8"));
                else {
    ...
    private void limitEntriesCount() {
        ...
        if (//$NON-NLS-1$
        fMemento.getString(LogView.P_USE_LIMIT).equals("true")) {
    ...
    private void initializeMemento() {
        if (fMemento.getString(P_USE_LIMIT) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_USE_LIMIT, "true");
        }
        ...
        if (fMemento.getString(P_LOG_INFO) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_LOG_INFO, "true");
        }
        if (fMemento.getString(P_LOG_OK) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_LOG_OK, "true");
        }
        if (fMemento.getString(P_LOG_WARNING) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_LOG_WARNING, "true");
        }
        if (fMemento.getString(P_LOG_ERROR) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_LOG_ERROR, "true");
        }
        if (fMemento.getString(P_SHOW_ALL_SESSIONS) == null) {
            //$NON-NLS-1$
            fMemento.putString(P_SHOW_ALL_SESSIONS, "true");
        }
    }
        ...
        if (sortType == PLUGIN) {
            ...
            return new ViewerComparator() {
                ...
                private int indexOf(Object[] array, Object o) {
                    if (o == null)
                        return -1;
                    for (int i = 0; i < array.length; ++i) if (o.equals(array[i]))
                        return i;
                    return -1;
                }
    ...
    private void readSettings() {
        ...
        if (s == null) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            fMemento.putString(P_LOG_OK, s.getBoolean(P_LOG_OK) ? "true" : "false");
            //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private void writeFilterSettings() {
        ...
        //$NON-NLS-1$
        settings.put(P_LOG_OK, fMemento.getString(P_LOG_OK).equals("true"));
        //$NON-NLS-1$
