    ...
    private void createRenames(IPackageFragment fragment, RefactoringArguments args, ProjectState state, IResourceChangeDescriptionFactory deltaFactory) throws JavaModelException {
        ...
        for (ICompilationUnit cu : fragment.getCompilationUnits()) {
            for (IType type : cu.getTypes()) {
                createRenames(//$NON-NLS-1$
                type, //$NON-NLS-1$
                type, //$NON-NLS-1$
                compName.length() == 0 ? type.getElementName() : String.format("%s.%s", compName, type.getElementName()), //$NON-NLS-1$
                args, //$NON-NLS-1$
                state, //$NON-NLS-1$
                deltaFactory);
            }
        ...
        if (ComponentPropertyTester.hasImplicitName(type)) {
            ...
            if (modelPath != null) {
                ...
                if (modelFile.isAccessible()) {
                    ...
                    String compName = String.format("%s%s", rootName, getTypeRelativeName(//$NON-NLS-1$
                    type, //$NON-NLS-1$
                    rootType));
                    componentNames.put(modelFile, compName);
                    // TODO centralize this?
                    IPath newPath = new Path(state.getPath()).addTrailingSeparator().append(//$NON-NLS-1$
                    compName).addFileExtension(//$NON-NLS-1$
                    "xml");
