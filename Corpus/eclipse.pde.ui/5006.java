...
public class APIToolsAnalysisTask extends CommonUtilsTask {
    ...
    private static class Summary {
        ...
        public String getDetails() {
            ...
            printWriter.println("==================================================================================");
            //$NON-NLS-1$
            dumpProblems("Usage:", apiUsageProblems, printWriter);
            //$NON-NLS-1$
            dumpProblems("Compatibility:", apiCompatibilityProblems, printWriter);
            //$NON-NLS-1$
            dumpProblems("Bundle Versions:", apiBundleVersionProblems, printWriter);
            //$NON-NLS-1$
            printWriter.println("==================================================================================");
            printWriter.flush();
            printWriter.close();
        ...
        private void printTitle(PrintWriter printWriter) {
            //$NON-NLS-1$
            printWriter.print(this.componentID + " : ");
            //$NON-NLS-1$
    ...
    private void dumpReport(Summary[] summaries, List<String> nonAPIBundleNames, Map<String, Object> bundlesWithErrors) {
        ...
        if (nonAPIBundleNames != null && nonAPIBundleNames.size() != 0) {
            ...
            if (contents != null) {
                //$NON-NLS-1$ //$NON-NLS-2$
                saveReport("Skipped Bundles", contents, "report.xml");
            }
        }
        ...
        if (contents != null) {
            //$NON-NLS-1$
            saveReport(null, contents, "counts.xml");
        }
    }
    ...
    public void execute() throws BuildException {
        if (this.referenceBaselineLocation == null || this.currentBaselineLocation == null || this.reportLocation == null) {
            ...
            writer.println(NLS.bind(Messages.printArguments, new String[] { this.referenceBaselineLocation, this.currentBaselineLocation, this.reportLocation }));
            writer.flush();
            writer.close();
        ...
        try {
            ...
            if (debug) {
                ...
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Total number of components in current baseline :" + length);
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "==========================");
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Total number of api tools components in current baseline :" + allApiBundles.size());
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Details:");
                ...
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Total number of non-api tools components in current baseline :" + allNonApiBundles.size());
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Details:");
                ...
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Total number of components with resolver errors :" + bundlesWithErrors.size());
                //$NON-NLS-1$
                System.out.println(//$NON-NLS-1$
                "Details:");
            ...
            for (IApiComponent apiComponent : baselineApiComponents) {
                ...
                if (!visitedApiComponentNames.remove(id)) {
                    // filters
                    if (!isFiltered(id)) {
                        IApiProblem problem = ApiProblemFactory.newApiProblem(id, null, new String[] { id }, new String[] { IApiMarkerConstants.MARKER_ATTR_HANDLE_ID, IApiMarkerConstants.API_MARKER_ATTR_ID }, new Object[] { id, Integer.valueOf(IApiMarkerConstants.COMPATIBILITY_MARKER_ID) }, 0, -1, -1, IApiProblem.CATEGORY_COMPATIBILITY, IDelta.API_BASELINE_ELEMENT_TYPE, IDelta.REMOVED, IDelta.API_COMPONENT);
