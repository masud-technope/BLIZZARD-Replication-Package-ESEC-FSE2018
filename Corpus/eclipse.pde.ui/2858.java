...
public class UnusedApiProblemFilterTests extends UsageTest {
    ...
    private void x3(boolean inc) throws Exception {
        ...
        setExpectedProblemIds(new int[] { getDefaultProblemId(), ApiProblemFactory.createProblemId(IApiProblem.CATEGORY_USAGE, IElementDescriptor.METHOD, IApiProblem.API_LEAK, IApiProblem.LEAK_METHOD_PARAMETER) });
        setExpectedMessageArgs(new String[][] { { //$NON-NLS-1$
        "testUF3.m2() has non-API return type internal" }, //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
