...
public class FeatureBlock {
    ...
    class ButtonSelectionListener extends SelectionAdapter {
        ...
        private void handleAddPlugin() {
            ...
            if (dialog.open() == Window.OK) {
                ...
                @SuppressWarnings("unchecked") List<Object> input = (List<Object>) fTree.getInput();
        ...
        private void handleRestoreDefaults() {
            ...
            fTree.getControl().setRedraw(false);
            fTree.removeFilter(fSelectedOnlyFilter);
            // Make sure the check state cache is cleared
    ...
    public void createControl(Composite parent, int span, int indent) {
        ...
        if (fTab instanceof PluginsTab) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            fAutoValidate = SWTFactory.createCheckButton(validatecomp, NLS.bind(PDEUIMessages.PluginsTabToolBar_auto_validate, fTab.getName().replaceAll("&", "").toLowerCase(Locale.ENGLISH)), null, false, 1);
        }
        ...
        if (fTab instanceof PluginsTab) {
            ...
            //$NON-NLS-1$//$NON-NLS-2$
            fValidateButton = SWTFactory.createPushButton(rightAlignComp, NLS.bind(PDEUIMessages.PluginsTabToolBar_validate, fTab.getName().replaceAll("&", "")), null);
        }
    ...
    private void createButtonContainer(Composite parent, int vOffset) {
        ...
        if (fTab instanceof PluginsTab) {
            ...
            //$NON-NLS-1$//$NON-NLS-2$
            fAddPluginButton = SWTFactory.createPushButton(buttonComp, NLS.bind(PDEUIMessages.FeatureBlock_AddPluginsLabel, fTab.getName().replaceAll("&", "")), null);
        }
        ...
        if (fTab instanceof PluginsTab) {
            ...
            //$NON-NLS-1$//$NON-NLS-2$
            fRemovePluginButton = SWTFactory.createPushButton(buttonComp, NLS.bind(PDEUIMessages.FeatureBlock_RemovePluginsLabel, fTab.getName().replaceAll("&", "")), null);
        }
        ...
        //$NON-NLS-1$
        fFilterButton = SWTFactory.createCheckButton(countComp, NLS.bind(PDEUIMessages.AdvancedLauncherTab_selectedBundles, ""), null, false, 1);
        fFilterButton.addSelectionListener(fListener);
        //$NON-NLS-1$
        fCounter = SWTFactory.createLabel(countComp, "", 1);
        ...
        if (fTab instanceof PluginsTab) {
            ...
            //$NON-NLS-1$ //$NON-NLS-2$
            fAdditionalPluginsParentElement = new NamedElement(NLS.bind(PDEUIMessages.FeatureBlock_AdditionalPluginsEntry, fTab.getName().replaceAll("&", "")), siteImage);
        }
    }
    ...
    private void savePluginState(ILaunchConfigurationWorkingCopy config) {
        // By using a set, debug will sort the attribute for us
        Set<String> featuresEntry = new HashSet();
