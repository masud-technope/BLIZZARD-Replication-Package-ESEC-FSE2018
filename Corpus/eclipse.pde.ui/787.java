...
public interface IApiBaselineManager {
    ...
    /**
	 * Allows a new profile to be added to the managers' cache of baselines. New
	 * baselines are added to the cache on a replace policy, meaning if there
	 ...
	 *         the manager and/or the persisted state file being removed from
	 *         disk.
	 */
	 ...
	 * If this method is called without the framework running it returns
	 * <code>null</code>
	 * <p>
	 * The workspace baseline should be re-retrieved each time it is required as
	 * some workspace modifications cause the underlying baseline object to
