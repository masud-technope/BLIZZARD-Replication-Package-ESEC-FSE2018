...
public class FilteredPluginArtifactsSelectionDialog extends FilteredItemsSelectionDialog {
    ...
    private class SearchLabelProvider extends LabelProvider implements ILabelDecorator {
        ...
        public String decorateText(String text, Object element) {
            if (element instanceof ExportPackageDescription) {
                ...
                return //$NON-NLS-1$
                text.concat(//$NON-NLS-1$
                " - " + epd.getSupplier().getSymbolicName());
            }
            return text;
        }
    }
    ...
    protected void fillContentProvider(AbstractContentProvider contentProvider, ItemsFilter itemsFilter, IProgressMonitor progressMonitor) throws CoreException {
        ...
        SubMonitor subMonitor = SubMonitor.convert(progressMonitor, PDEUIMessages.FilteredPluginArtifactsSelectionDialog_searching, models.length * 2 + fModels.length);
        ...
        for (int i = 0; i < models.length; i++) {
            ...
            if (desc != null) {
                ...
                for (int j = 0; j < epds.length; j++) {
                    ...
                    int ee = //$NON-NLS-1$
                    ((Integer) epd.getDirective("x-equinox-ee")).intValue();
                    if (ee < 0) {
    ...
    protected IStatus validateItem(Object item) {
        //$NON-NLS-1$ //$NON-NLS-2$
        return new Status(IStatus.OK, "org.eclipse.pde.ui", 0, "", null);
    }
