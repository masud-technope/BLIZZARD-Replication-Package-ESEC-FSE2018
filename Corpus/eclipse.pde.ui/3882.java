...
public class PDEClasspathContainer {
    ...
    //$NON-NLS-1$
    private static final IAccessRule EXCLUDE_ALL_RULE = JavaCore.newAccessRule(new Path("**/*"), IAccessRule.K_NON_ACCESSIBLE | IAccessRule.IGNORE_IF_BETTER);
    ...
    protected static void addExternalPlugin(IPluginModelBase model, Rule[] rules, ArrayList<IClasspathEntry> entries) {
        if (new File(model.getInstallLocation()).isFile()) {
            ...
            if (libraries.length == 0) {
                ...
                IPath srcPath = //$NON-NLS-1$
                ClasspathUtilCore.getSourceAnnotation(//$NON-NLS-1$
                model, //$NON-NLS-1$
                ".");
                if (srcPath == null)
                    srcPath = new Path(model.getInstallLocation());
