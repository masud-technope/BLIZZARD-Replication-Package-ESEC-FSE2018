...
public class BundleErrorReporter extends JarManifestErrorReporter {
    ...
    private boolean validateBundleManifestVersion() {
        ...
        if (header != null) {
            ...
            !(fOsgiR4 = "2".equals(version)) && !"1".equals(version)) {
            ...
            for (int i = 0; i < roots.length; i++) {
                try {
                    if (ManifestUtils.isImmediateRoot(roots[i])) {
                        ...
                        for (int j = 0; j < javaElements.length; j++) if (javaElements[j] instanceof IPackageFragment) {
        ...
        for (ManifestElement element : required) {
            ...
            if (versionRange != null) {
                try {
                    ...
                    report(e.getMessage(), getLine(header, element.getValue()), CompilerFlags.ERROR, PDEMarkerFactory.CAT_FATAL);
                    return false;
                }
            }
        }
    ...
    private void validateSingleton(IHeader header, ManifestElement element) {
                ...
                !"true".equals(singletonDir)) {
                    ...
                    singletonAttr)) {
                        if (isCheckDeprecated() && fOsgiR4) {
                            ...
                            report(message, getLine(header, //$NON-NLS-1$
                            ICoreConstants.SINGLETON_ATTRIBUTE + "="), CompilerFlags.P_DEPRECATED, PDEMarkerFactory.M_SINGLETON_DIR_NOT_SET, PDEMarkerFactory.CAT_DEPRECATION);
                            return;
                        }
                    } else {
        ...
        if (fOsgiR4) {
            ...
            if (isCheckDeprecated()) {
                ...
                report(message, getLine(//$NON-NLS-1$
                header, //$NON-NLS-1$
                Constants.SINGLETON_DIRECTIVE + ":="), CompilerFlags.P_DEPRECATED, PDEMarkerFactory.M_SINGLETON_DIR_NOT_SUPPORTED, PDEMarkerFactory.CAT_OTHER);
            }
        }
    ...
    private void validateRequiredExecutionEnvironment() {
            ...
            if (vm != null) {
                for (int i = 0; i < systemEnvs.length; i++) {
                    ...
                    if (systemEnvs[i].isStrictlyCompatible(vm)) {
                        IMarker marker = report(PDECoreMessages.BundleErrorReporter_noExecutionEnvironmentSet, 1, sev, PDEMarkerFactory.M_EXECUTION_ENVIRONMENT_NOT_SET, PDEMarkerFactory.CAT_EE);
                        addMarkerAttribute(marker, "ee_id", //$NON-NLS-1$
                        systemEnvs[i].getId());
                        break;
                    }
                }
        ...
        @SuppressWarnings("deprecation") IHeader header = getHeader(Constants.BUNDLE_REQUIREDEXECUTIONENVIRONMENT);
        if (header == null)
            return;
    ...
    private void validateRequireBundle(IProgressMonitor monitor) {
        ...
        BundleDescription desc = fModel.getBundleDescription();
        if (desc == null) {
            for (int i = 0; i < required.length; i++) validateBundleVersionAttribute(header, required[i]);
            return;
        }
        ...
        for (int i = 0; i < required.length; i++) {
            ...
            if (specs != null && specs[i].getSupplier() == null) {
                if (desc.getContainingState().getBundle(specs[i].getName(), null) == null) {
                    ...
                    report(NLS.bind(PDECoreMessages.BundleErrorReporter_BundleRangeInvalidInBundleVersion, bundleID + ": " + //$NON-NLS-1$
                    specs[i].getVersionRange()), getPackageLine(header, required[i]), severity, PDEMarkerFactory.CAT_FATAL);
                }
            }
        }
        // Header introduced in OSGi R4 - warn if R3 manifest
    ...
    private void validateReprovideAttribute(IHeader header, ManifestElement element) {
        ...
        if (rexport != null) {
            ...
            if (fOsgiR4 && isCheckDeprecated()) {
                ...
                report(message, getLine(//$NON-NLS-1$
                header, //$NON-NLS-1$
                ICoreConstants.REPROVIDE_ATTRIBUTE + "="), CompilerFlags.P_DEPRECATED, PDEMarkerFactory.CAT_DEPRECATION);
            }
        }
    }
    ...
    private boolean isOptional(ManifestElement element) {
        //$NON-NLS-1$
        return Constants.RESOLUTION_OPTIONAL.equals(element.getDirective(Constants.RESOLUTION_DIRECTIVE)) || "true".equals(element.getAttribute(ICoreConstants.OPTIONAL_ATTRIBUTE));
    }
    ...
    private void validateOptionalAttribute(IHeader header, ManifestElement element) {
        ...
        if (rexport != null) {
            ...
            if (fOsgiR4 && isCheckDeprecated()) {
                report(NLS.bind(PDECoreMessages.BundleErrorReporter_deprecated_attribute_optional, ICoreConstants.OPTIONAL_ATTRIBUTE), getLine(//$NON-NLS-1$
                header, //$NON-NLS-1$
                ICoreConstants.OPTIONAL_ATTRIBUTE + "="), CompilerFlags.P_DEPRECATED, PDEMarkerFactory.CAT_DEPRECATION);
            }
        }
    }
            ...
            if (resource != null) {
                ...
                try {
                    if (fProject.hasNature(JavaCore.NATURE_ID)) {
                        ...
                        for (int i = 0; i < packages.length; i++) fProjectPackages.add(packages[i].getElementName());
                    }
                } catch (CoreException ce) {
                }
            }
            ...
            if (header != null) {
                ...
                !value.startsWith("%")) {
                    ...
                    if (helper == null || !helper.resourceExists(value)) {
                        report(NLS.bind(PDECoreMessages.Builders_Manifest_key_not_found, value.substring(1), PDEManager.getBundleLocalization(fModel).concat(".properties")), getLine(header, value), severity, //$NON-NLS-1$
    ...
    private void validateX_FriendsDirective(IHeader header, ManifestElement element) {
        ...
        if (friends != null && internal != null) {
            ...
            //$NON-NLS-1$
            addMarkerAttribute(marker, "packageName", element.getValue());
        }
    }
    ...
    private void validateAutoStart() {
        ...
        if (!validateStartHeader(header))
            // valid start header problems already reported
            return;
        int severity = CompilerFlags.getFlag(fProject, CompilerFlags.P_DEPRECATED);
    ...
    private boolean exceptionsAttributesValid(IHeader header, ManifestElement[] elements) {
        ...
        if (keys != null && keys.hasMoreElements()) {
            ...
            if (//$NON-NLS-1$
            "exceptions".equals(key)) {
                String[] values = elements[0].getAttributes(key);
                for (int i = 0; i < values.length; i++) {
                    StringTokenizer st = new //$NON-NLS-1$
    ...
    private void validateBundleLocalization() {
        ...
        try {
            ...
            for (int i = 0; i < children.length; i++) {
                ...
                if (childName.endsWith(".properties") && //$NON-NLS-1$
                childName.startsWith(//$NON-NLS-1$
                fileName))
                    return;
            }
        } catch (CoreException e) {
    ...
    private void validateFragmentHost(IHeader requireBundleHeader, ManifestElement element) {
        ...
        if (elements[0] != null && elements[0].getValue().equals(element.getValue())) {
            ...
            //$NON-NLS-1$
            addMarkerAttribute(marker, "bundleId", element.getValue());
        }
    }
