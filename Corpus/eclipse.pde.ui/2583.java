...
public class OrganizeManifest implements IOrganizeManifestsSettings {
    ...
    private static String[] F_ICON_EXTENSIONS = new String[] { //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "BMP", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "ICO", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "JPEG", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "JPG", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "GIF", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "PNG", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
    "TIFF" };
    ...
    public static void organizeExportPackages(IBundle bundle, IProject project, boolean addMissing, boolean removeUnresolved) {
        ...
        for (int i = 0; i < roots.length; i++) {
            try {
                if (ManifestUtils.isImmediateRoot(roots[i])) {
                    ...
                    for (int j = 0; j < elements.length; j++) if (elements[j] instanceof IPackageFragment) {
        ...
        if (removeUnresolved)
            for (int i = 0; i < currentPkgs.length; i++) if (!packages.contains(currentPkgs[i].getName()))
    ...
    public static TextFileChange[] removeUnusedKeys(final IProject project, final IBundle bundle, final IPluginModelBase modelBase) {
        ...
        if (localization == null)
            ...
            localization = "plugin";
        //$NON-NLS-1$
        IFile propertiesFile = project.getFile(localization + ".properties");
        ...
        return PDEModelUtility.changesForModelModication(new ModelModification(propertiesFile) {
            ...
            protected void modifyModel(IBaseModel model, IProgressMonitor monitor) throws CoreException {
                ...
                for (int i = 0; i < entries.length; i++) if (!allKeys.contains(entries[i].getName()))
                ...
                for (int i = 0; i < usedkeys.size(); i++) allKeys.remove(usedkeys.get(i));
                if (allKeys.size() == 0)
                    return;
                ...
                for (int i = 0; i < entries.length; i++) {
                    ...
                    for (int k = 1; k < tokens.length; k++) entry += ',' + tokens[k];
    ...
    private static void findTranslatedXMLStrings(IPluginModelBase model, ArrayList<String> list) {
        ...
        for (int i = 0; i < extensions.length; i++) if (extensions[i] instanceof IDocumentElementNode)
    ...
    private static void inspectElementForTranslation(IDocumentElementNode parent, ArrayList<String> list) {
        ...
        for (int i = 0; i < children.length; i++) if (children[i] instanceof IDocumentElementNode)
